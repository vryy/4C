...
if grep '^[ \t]*TRILINOS_PACKAGE' "$definefile" 2>&1 > /dev/null || grep '^[ \t]*CCADISCRET' "$definefile" 2>&1 > /dev/null ; then
    ...
    FILTER_LIBS="-L$TRILINOS_LIBDIR $TRILINOS_FILTER_LIB $HDF5_LIB $LIBS"
    LIBS="-L$TRILINOS_LIBDIR $TRILINOS_LIB $SUPERLU_LIB $HDF5_LIB $LIBS"
...
if grep '^[ \t]*QHULL' "$definefile" 2>&1 > /dev/null ; then
    ...
    FILTER_LIBS="$QHULL_LIB $FILTER_LIBS"
    LIBS="$QHULL_LIB $LIBS"
