...
\$(OBJS_DRT) \$(OBJS_DRT_LIB) \$(OBJS_DRT_INPAR) \$(OBJS_DRT_NURBS_DISCRET_LIB) \$(OBJS_DRT_NURBS_DISCRET) \
\$(OBJS_DRT_MORTAR) \$(OBJS_DRT_MORTAR_LIB) \$(OBJS_DRT_BEAMCONTACT) \$(OBJS_DRT_BEAMCONTACT_LIB) \$(OBJS_DRT_CONTACT) \$(OBJS_DRT_CONTACT_LIB) \$(OBJS_DRT_STATMECH) \$(OBJS_DRT_STATMECH_LIB) \
\$(OBJS_DRT_CONSTRAINT) \$(OBJS_DRT_CONSTRAINT_LIB) \$(OBJS_DRT_STRUCTURE) \$(OBJS_DRT_THERMO) \$(OBJS_DRT_THERMO_LIB) \$(OBJS_DRT_TSI) \$(OBJS_PSS) \$(OBJS_INPUT) \$(OBJS_INPUT_CPP) \$(OBJS_PAR) \$(OBJS_MATH) \
\$(OBJS_OUTPUT) \$(OBJS_FORTRAN) \$(OBJS_VISUAL) \$(OBJS_STRUCTURE) \$(OBJS_IO) \$(OBJS_DRT_IO) \$(OBJS_DRT_IO_LIB) \
\$(OBJS_INTERSECTION) \$(OBJS_INV_ANALYSIS) \$(OBJS_DRT_ALEALG) \$(OBJS_DRT_ADAPT) \$(OBJS_DRT_FSI) \$(OBJS_DRT_FLUID) \$(OBJS_DRT_MAT_LIB) \
\$(OBJS_DRT_MAT) \$(OBJS_DRT_STRU_MULTI) \$(OBJS_DRT_SURFSTRESS) \$(OBJS_DRT_POTENTIAL) \$(OBJS_DRT_LOMA) \$(OBJS_DRT_ELCH) \$(OBJS_DRT_SCATRA) \$(OBJS_DRT_SCATRA_LIB) \$(OBJS_DRT_COMBUST) \$(OBJS_DRT_BELE3_LIB)" 

FILTER_OBJECTS="\$(OBJS_POST_DRT_COMMON) \
\$(OBJS_DRT_IO_LIB) \$(OBJS_DRT_LIB) \$(OBJS_DRT_INPAR) \$(OBJS_DRT_NURBS_DISCRET_LIB) \$(OBJS_DRT_MAT_LIB) \$(OBJS_PSS) \$(OBJS_PAR) \$(OBJS_DRT_MORTAR_LIB) \$(OBJS_DRT_BEAMCONTACT_LIB) \$(OBJS_DRT_CONTACT_LIB) \
\$(OBJS_INTERSECTION) \$(OBJS_DRT_CONSTRAINT_LIB) \$(OBJS_DRT_F2_LIB) \$(OBJS_DRT_B2_LIB) \$(OBJS_DRT_B2R_LIB) \$(OBJS_DRT_B3_LIB) \$(OBJS_DRT_B3II_LIB) \$(OBJS_DRT_BELE3_LIB) \$(OBJS_DRT_T3_LIB) \$(OBJS_DRT_T2_LIB) \$(OBJS_DRT_SR_LIB) \$(OBJS_DRT_TO3_LIB) \$(OBJS_DRT_TO2_LIB) \$(OBJS_DRT_F3_LIB)  \$(OBJS_DRT_S8_LIB) \
\$(OBJS_SOH8_LIB) \$(OBJS_DRT_W1_LIB) \$(OBJS_DRT_ALE_LIB) \$(OBJS_DRT_COMBUST3_LIB) \$(OBJS_DRT_STATMECH_LIB) \$(OBJS_DRT_SURFSTRESS) \$(OBJS_DRT_POTENTIAL) \$(OBJS_DRT_SCATRA_LIB) \$(OBJS_DRT_THERMO_LIB) \$(OBJS_FILTER_COMMON)"
...
# BEAM3II
if grep '^[[:blank:]]*D_BEAM3II' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_BEAM3II)"
fi
...
# BEAM3II
if grep '^[[:blank:]]*D_BEAM3II' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_DRT_B3II) \$(OBJS_DRT_B3II_LIB)"
fi

# SMOOTHROD
if grep '^[[:blank:]]*D_SMOOTHROD' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_DRT_SR) \$(OBJS_DRT_SR_LIB)"
fi
...
# TRUSS2
if grep '^[[:blank:]]*D_TRUSS2' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_DRT_T2) \$(OBJS_DRT_T2_LIB)"
fi

# TORSION3
if grep '^[[:blank:]]*D_TORSION3' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_DRT_TO3) \$(OBJS_DRT_TO3_LIB)"
fi

# TORSION2
if grep '^[[:blank:]]*D_TORSION2' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_DRT_TO2) \$(OBJS_DRT_TO2_LIB)"
fi
...
if grep '^[[:blank:]]*D_ARTNET' "$definefile" 2>&1 > /dev/null ; then
    ...
    FILTER_OBJECTS="$FILTER_OBJECTS \$(OBJS_DRT_ARTNET_LIB)"
fi
...
if grep '^[[:blank:]]*D_RED_AIRWAYS' "$definefile" 2>&1 > /dev/null ; then
    OBJECTS="$OBJECTS \$(OBJS_DRT_RED_AIRWAYS) \$(OBJS_DRT_RED_AIRWAYS_LIB)"
    FILTER_OBJECTS="$FILTER_OBJECTS \$(OBJS_DRT_RED_AIRWAYS_LIB)"
fi
