...
\$(OBJS_DRT_MAT) \$(OBJS_DRT_STRU_MULTI) \$(OBJS_DRT_SURFSTRESS) \$(OBJS_DRT_TIMADA) \$(OBJS_DRT_ELCH) \$(OBJS_DRT_SCATRA) \$(OBJS_DRT_BELE3_LIB)" 
...
if grep '^[[:blank:]]*D_FLUID3' "$definefile" 2>&1 > /dev/null ; then
    if [ "x$OBJ_FLUID" = "x0" ] ; then
        ...
        \$(OBJS_DRT_CONDIF3_LIB) \$(OBJS_DRT_COMBUST3_LIB) \$(OBJS_DRT_COMBUST)"
        DEFINES="$DEFINES -DD_FLUID"
        OBJ_FLUID=1
    else
