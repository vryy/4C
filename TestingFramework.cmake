...
macro (pre_processing testname nproc)
  set (RUNPREEXODUS_NOHEAD ./pre_exodus\ --exo=${PROJECT_SOURCE_DIR}/tests/pre_processing_test/${testname}.e) # run pre_exodus to generate default head and bc file
  set (RUNPREEXODUS_DEFAULTHEAD ./pre_exodus\ --exo=${PROJECT_SOURCE_DIR}/tests/pre_processing_test/${testname}.e\ --bc=${PROJECT_SOURCE_DIR}/tests/pre_processing_test/${testname}.bc\ --head=default.head\ --dat=xxx.dat) # run pre_exodus to generate dat file using the default head file
  add_test(NAME ${testname}-p${nproc}-pre_processing
  COMMAND sh -c "${RUNPREEXODUS_NOHEAD} && ${RUNPREEXODUS_DEFAULTHEAD}")
  set_tests_properties(${testname}-p${nproc}-pre_processing PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED})
  set_tests_properties ( ${testname}-p${nproc}-pre_processing PROPERTIES FAIL_REGULAR_EXPRESSION "ERROR:; ERROR ;Error " )
  set_tests_properties ( ${testname}-p${nproc}-pre_processing PROPERTIES ENVIRONMENT "PATH=$ENV{PATH}" )
