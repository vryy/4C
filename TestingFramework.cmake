...
macro (baci_test name_of_input_file num_proc restart_step)
  add_test(NAME ${name_of_input_file}-p${num_proc}
    COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file} && ${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file}/xxx")
  if( "${ARGN}" STREQUAL "")
    set_tests_properties(${name_of_input_file}-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED})
  else ()
    set_tests_properties(${name_of_input_file}-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED} LABELS ${ARGN})
  endif ()

  if (${restart_step})
    add_test(NAME ${name_of_input_file}-p${num_proc}-restart
      COMMAND bash -c "${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file}/xxx restart=${restart_step}")
...
macro (baci_test_and_post_ensight_test name_of_input_file num_proc restart_step)
  ...
  set(RUNPOSTFILTER_SER ./post_drt_ensight\ --file=framework_test_output/${name_of_input_file}/xxx\ --output=framework_test_output/${name_of_input_file}/xxx_SER\ --outputtype=bin\ --stress=ndxyz)

  add_test(NAME ${name_of_input_file}-p${num_proc}-post_ensight-ser
    COMMAND sh -c " ${RUNPOSTFILTER_SER} && ${PROJECT_SOURCE_DIR}/utilities/baci-python-venv/bin/python3 ${PROJECT_SOURCE_DIR}/tests/post_processing_test/ensight_comparison.py ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file}/xxx_SER_structure.case")
  ...
  set(RUNPOSTFILTER_PAR ${MPI_RUN}\ ${MPIEXEC_EXTRA_OPTS}\ -np\ ${num_proc}\ ./post_drt_ensight\ --file=framework_test_output/${name_of_input_file}/xxx\ --output=framework_test_output/${name_of_input_file}/xxx_PAR\ --outputtype=bin\ --stress=ndxyz)

  add_test(NAME ${name_of_input_file}-p${num_proc}-post_ensight-par
    COMMAND sh -c " ${RUNPOSTFILTER_PAR} && ${PROJECT_SOURCE_DIR}/utilities/baci-python-venv/bin/python3 ${PROJECT_SOURCE_DIR}/tests/post_processing_test/ensight_comparison.py ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file}/xxx_PAR_structure.case")
...
macro (baci_test_restartonly name_of_input_file name_of_input_file_restart num_proc restart_step)
  ...
  add_test(NAME ${name_of_input_file}-p${num_proc}-restart
    COMMAND bash -c "${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file_restart}/xxx${IDENTIFIER} restart=${restart_step}")
...
macro (baci_test_Nested_Par name_of_input_file_1 name_of_input_file_2 restart_step)
  add_test(NAME ${name_of_input_file_1}-nestedPar
    COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file_1} &&  ${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np 3 $<TARGET_FILE:${baciname}> -ngroup=2 -glayout=1,2 -nptype=separateDatFiles ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file_1}.dat framework_test_output/${name_of_input_file_1}/xxx ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file_2}.dat framework_test_output/${name_of_input_file_1}/xxxAdditional")

  if (${restart_step})
    add_test(NAME ${name_of_input_file_1}-nestedPar-restart
    COMMAND bash -c "${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np 3 $<TARGET_FILE:${baciname}> -ngroup=2 -glayout=1,2 -nptype=separateDatFiles ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file_1}.dat framework_test_output/${name_of_input_file_1}/xxx restart=${restart_step} ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file_2}.dat framework_test_output/${name_of_input_file_1}/xxxAdditional restart=${restart_step}")
  endif (${restart_step})
...
macro (baci_test_Nested_Par_CopyDat name_of_input_file num_proc num_groups)
  add_test(NAME ${name_of_input_file}-nestedPar_CopyDat-p${num_proc}
    COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file} &&  ${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> -ngroup=${num_groups} -nptype=copyDatFile ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file}/xxx")
    if( "${ARGN}" STREQUAL "")
      set_tests_properties(${name_of_input_file}-nestedPar_CopyDat-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED})
    else ()
      set_tests_properties(${name_of_input_file}-nestedPar_CopyDat-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED} LABELS ${ARGN})
    endif ()
...
macro (baci_test_Nested_Par_CopyDat_prepost name_of_input_file_precursor name_of_input_file name_of_input_file_post num_proc num_groups restart_step)
  ...
  add_test(NAME ${name_of_input_file}_precursor-p${num_proc}
    COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file} &&  ${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file_precursor}.dat framework_test_output/${name_of_input_file}/xxx")
  # restart from precursor output
  add_test(NAME ${name_of_input_file}-p${num_proc}
    COMMAND bash -c "${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> -ngroup=${num_groups} -nptype=copyDatFile ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}.dat framework_test_output/${name_of_input_file}/xxx")
  ...
  if (NOT ${name_of_input_file_post} STREQUAL "")
    add_test(NAME ${name_of_input_file}_postprocess-p${num_proc}
      COMMAND COMMAND bash -c "${MPI_RUN} ${MPIEXEC_EXTRA_OPTS} -np ${num_proc} $<TARGET_FILE:${baciname}> -ngroup=${num_groups} -nptype=copyDatFile ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file_post}.dat framework_test_output/${name_of_input_file}/xxx restart=${restart_step}")
  ...
  if( "${ARGN}" STREQUAL "")
    ...
    set_tests_properties(${name_of_input_file}_precursor-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED} LABELS ${ARGN})
    set_tests_properties(${name_of_input_file}-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED} LABELS ${ARGN})
    if (NOT ${name_of_input_file_post} STREQUAL "")
      set_tests_properties(${name_of_input_file}_postprocess-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED} LABELS ${ARGN})
    endif(NOT ${name_of_input_file_post} STREQUAL "")
  endif ()
...
macro (baci_framework_test name_of_input_file num_proc xml_filename)
  ...
  set (RUNPREEXODUS ./pre_exodus\ --exo=framework_test_output/${name_of_input_file}/xxx_${name_of_input_file}.e\ --bc=${PROJECT_SOURCE_DIR}/tests/framework-test/${name_of_input_file}.bc\ --head=${PROJECT_SOURCE_DIR}/tests/framework-test/${name_of_input_file}.head\ --dat=framework_test_output/${name_of_input_file}/xxx.dat) # pre_exodus is run to generate a Dat file
  if (NOT ${xml_filename} STREQUAL "")
    file(COPY ${PROJECT_SOURCE_DIR}/Input/${xml_filename} DESTINATION ./framework_test_output/${name_of_input_file}) # if a XML file name is given, it is copied from the baci input directory to the build directory
  endif(NOT ${xml_filename} STREQUAL "")
  set (RUNBACI ${MPI_RUN}\ ${MPIEXEC_EXTRA_OPTS}\ -np\ ${num_proc}\ $<TARGET_FILE:${baciname}>\ framework_test_output/${name_of_input_file}/xxx.dat\ framework_test_output/${name_of_input_file}/xxx) # baci is run using the generated dat file
  set (RUNPOSTFILTER ${MPI_RUN}\ ${MPIEXEC_EXTRA_OPTS}\ -np\ ${num_proc}\ ./post_drt_ensight\ --file=framework_test_output/${name_of_input_file}/xxx) # post_drt_ensight is run for the resulting output
  add_test(NAME ${name_of_input_file}-p${num_proc}-fw
  COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file} && ${RUNCUBIT} && ${RUNPREEXODUS} && ${RUNBACI} && ${RUNPOSTFILTER}")
...
macro (cut_test num_proc)
  ...
  COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/cut_test && cd ${PROJECT_BINARY_DIR}/framework_test_output/cut_test && ${RUNTESTS}")
...
macro (pre_processing name_of_input_file num_proc)
  ...
  set (RUNPREEXODUS_DEFAULTHEAD ./pre_exodus\ --exo=${PROJECT_SOURCE_DIR}/tests/pre_processing_test/${name_of_input_file}.e\ --bc=${PROJECT_SOURCE_DIR}/tests/pre_processing_test/${name_of_input_file}.bc\ --head=default.head\ --dat=framework_test_output/${name_of_input_file}/xxx.dat) # run pre_exodus to generate dat file using the default head file
  add_test(NAME ${name_of_input_file}-p${num_proc}-pre_processing
  COMMAND bash -c "mkdir -p ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file} && ${RUNPREEXODUS_NOHEAD} && ${RUNPREEXODUS_DEFAULTHEAD}")
...
macro(post_processing name_of_input_file num_proc stresstype straintype startstep)
  ...
  set(RUNPOSTFILTER_SER ./post_drt_ensight\ --file=framework_test_output/${name_of_input_file}/xxx${IDENTIFIER}\ --output=framework_test_output/${name_of_input_file}/xxx${IDENTIFIER}_SER_${name_of_input_file}\ --stress=${stresstype}\ --strain=${straintype}\ --start=${startstep})
  set(RUNPOSTFILTER_PAR ${MPI_RUN}\ ${MPIEXEC_EXTRA_OPTS}\ -np\ ${num_proc}\ ./post_drt_ensight\ --file=framework_test_output/${name_of_input_file}/xxx${IDENTIFIER}\ --output=framework_test_output/${name_of_input_file}/xxx${IDENTIFIER}_PAR_${name_of_input_file}\ --stress=${stresstype}\ --strain=${straintype}\ --start=${startstep})

  # specify test case
  add_test(NAME ${name_of_input_file}${IDENTIFIER}${FIELD}-p${num_proc}-pp
    COMMAND sh -c " ${RUNPOSTFILTER_PAR} && ${RUNPOSTFILTER_SER} && ${PVPYTHON} ${PROJECT_SOURCE_DIR}/tests/post_processing_test/comparison.py framework_test_output/${name_of_input_file}/xxx${IDENTIFIER}_PAR_${name_of_input_file}${FIELD}*.case framework_test_output/${name_of_input_file}/xxx${IDENTIFIER}_SER_${name_of_input_file}${FIELD}*.case ${PROJECT_SOURCE_DIR}/Input/${name_of_input_file}${IDENTIFIER}${FIELD}.csv framework_test_output/${name_of_input_file}")
...
macro(result_file_abs name_of_input_file num_proc filetag resultfilename referencefilename tolerance)
  ...
  add_test(NAME ${name_of_input_file}-p${num_proc}-${filetag} COMMAND ${PROJECT_SOURCE_DIR}/utilities/baci-python-venv/bin/python3 ${PROJECT_SOURCE_DIR}/utilities/diff_with_tolerance.py ${tolerance} ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file}/${resultfilename} ${PROJECT_SOURCE_DIR}/Input/${referencefilename} abs_tol 0.0)
...
macro(result_file_rel name_of_input_file num_proc filetag resultfilename referencefilename tolerance min_val)
  # add test to testing framework
  add_test(NAME ${name_of_input_file}-p${num_proc}-${filetag} COMMAND ${PROJECT_SOURCE_DIR}/utilities/baci-python-venv/bin/python3 ${PROJECT_SOURCE_DIR}/utilities/diff_with_tolerance.py ${tolerance} ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file}/${resultfilename} ${PROJECT_SOURCE_DIR}/Input/${referencefilename} rel_tol ${min_val})
...
macro(vtk_test name_of_test name_of_input_file num_proc pvd_resultfilename pvd_referencefilename tolerance)
...
# if no extra arguments are given test checks every timestep
  ...
  # add test to testing framework
  add_test(NAME ${name_of_test}-p${num_proc} COMMAND ${PROJECT_SOURCE_DIR}/utilities/baci-python-venv/bin/python3 ${PROJECT_SOURCE_DIR}/tests/output_test/vtk_compare.py ${PROJECT_BINARY_DIR}/framework_test_output/${name_of_input_file}/${pvd_resultfilename} ${PROJECT_SOURCE_DIR}/Input/${pvd_referencefilename} ${tolerance} ${num_extra_args} ${extra_macro_args})

  # set maximum test runtime
  set_tests_properties(${name_of_test}-p${num_proc} PROPERTIES TIMEOUT ${GLOBAL_TEST_TIMEOUT_SCALED})
endmacro(vtk_test)
...
add_test(NAME test_cleanup COMMAND sh -c "if [ -f *_CUTFAIL.pos ]; then mkdir -p ../cut-debug ; cp *_CUTFAIL.pos ../cut-debug/ ; fi ; rm -vfr xxx* framework_test_output* core.* amesos-failure.dat default.bc default.head")
