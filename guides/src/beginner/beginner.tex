...
\begin{enumerate}
  ...
  git svn clone -r (Revision number) svn://gauss/trunk/ccarat Baci \\
  Revision number: actual Baci reversion number ($\rightarrow $ entire Baci starting from revision 0 is checked out without this number)
  ...
  git clone Baci Baci\_stud
  \item generate a new branch (you can have as many branches as students): \\
  git branch student\_name
  \item change to new branch: \\
  git checkout student\_name
\end{enumerate}
...
\begin{itemize}
  ...
  git config -$\,$-list
  \item git pull cannot be executed in branch student\_name: \\
  git config branch.student\_name.merge refs/heads/master \\
  git config  branch.student\_name.remote origin 
\end{itemize}
...
\begin{enumerate}
  ...
  add \texttt{alias eclipse="/lnm/programs/eclipse\_cdt6/eclipse"} $ \rightarrow $ eclipse can be executed by \texttt{eclipse} in the shell
  \item close actual shell and open a new shell
  \item clone the repository Baci\_stud: \\
  git clone $ ~$ -$\,$-bare -b student\_name myname@mycomputer.lnm.mw.tum.de:/myDirectory/Baci\_stud Baci\_bare
  \item delete master branch (if there is one): \\
  git branch $\rightarrow $ list all branches in the actual clone \\
  git branch -d master \\
\end{enumerate}

\paragraph{On the local hard drive /scratch}

\begin{enumerate}
  \item generate your own folder structure:
\begin{itemize}
  \item mkdir student\_name
  \item cd student\_name
  \item mkdir workspace
  \item cd workspace
\end{itemize}
  \item clone baci\_bare to this folder (it will be your future workspace): \\
  git clone -b student\_name $\sim$/Baci\_bare baci
\end{enumerate}
...
\begin{itemize}
  \item git branch \\ 
  list all branches in the actual clone
  \item git diff \$*$\mid$ kompare - \\
  compare the files in kompare
  \item git diff branch\_name \$*$\mid$ kompare - \\
  compare actual branch to branch\_name in kompare
  ...
  \end{itemize}
  \item At different computers the student has allways access to the actual baci version stored in his home directory (Baci\_bare). Therefore, Baci\_bare need to be updated regularly.
  \begin{itemize}
    \item git pull cannot be executed from baci\_bare $\rightarrow$ update Baci\_bare: \\
    git push $\sim$ /student/Baci\_bare $\rightarrow$ update Baci\_bare
    \item getting actual version from Baci\_bare: \\
    git pull $\sim$/bBaci\_bare $\rightarrow$ update Baci\_bare
  \end{itemize}
  \item update baci version of the student
  \begin{enumerate}
    \item git svn rebase \\
    get actual version from the svn repository
    \item git pull $\sim$/Baci (in folder Baci\_stud) \\
    pull the actual version to Baci\_stud (only the active branch is updated)
    \item pull student@red??.stud:$\sim$/Baci\_bare\\
    this command is necessary to get the actual version of the student (merge student version with version Baci\_stud) \\
    possible conflicts need to be resolved
    \item push ehrl@red??.stud:$\sim$/Baci\_bare
  \end{enumerate}
  \item gitk \\
...
\begin{itemize}
  ...
  \item change line \texttt{BUILD\_TYPE="Release"} to \texttt{BUILD\_TYPE="Debug"}
