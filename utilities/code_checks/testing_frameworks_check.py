...
def check_unittests(look_cmd, allerrors):
    ...
    unit_tests = [
        ...
        if utils.is_unittest_file(ff)
    ]
    unit_tests_cmake = [
        str(ff)
        for ff in utils.files_changed(look_cmd)[:-1]
        if utils.is_unittest_cmakefile(ff)
    ]
    ...
    for fname in unit_tests:
        ...
        if not os.path.isfile(os.path.join(unittest_package, "CMakeLists.txt")):
            ...
            with open(
                os.path.join(unittest_package, "CMakeLists.txt"), "r"
            ) as cmakefile:
                found = False
        ...
        # the name of the class must be in the same line as ": public FourCCxxTestWrapper"
        with open(fname, "r") as file:
            # go through all lines
            testsuite_re = re.compile(r":\s*public\s*FourCCxxTestWrapper")
            testsuite_test_re = re.compile(
                r"class *([a-zA-Z0-9\-_:]*) *: *public *FourCCxxTestWrapper"
            )
            for line in file:
    ...
    if len(naming_convention) > 0:
        errors += 1
        allerrors.append(
            "The following unit tests are not compliant with the naming conventions."
        )
        allerrors.append(
            "Unit tests have to start with unit_ and must have a .H file extension."
        )
        allerrors.append("")
    ...
    if len(cmakelists) > 0:
        errors += 1
        allerrors.append(
            "The following unit tests are not added to a local CMakeLists.txt:"
        )
        allerrors.append("")
    ...
    if len(alphabetical_order) > 0:
        errors += 1
        allerrors.append(
            "The items in the following local CMakeLists.txt are not in alphabetical order:"
        )
        allerrors.append("")
...
def main():
    ...
    try:
        ...
        if errors > 0:
            ...
            allerrors.extend(
                [
                    "Refer to our Wiki-page for good-style unittests:",
