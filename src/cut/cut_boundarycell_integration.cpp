...
#include "cut_least_squares.H"
#include "cut_base_vol.H"
...
Epetra_SerialDenseVector GEO::CUT::BoundarycellIntegration::GenerateBoundaryCellIntegrationRule()
{
  ...
  while (1)
  {
    ...
    // deleted this ensures non-zero diagonal elements in the matrix it always occurs when the
    // boundarycell is oriented along any of the coordinate planes
    ...
    for (unsigned row = 0; row < moment_matbc.size(); row++)
    {
      ...
      if (deleteRow == true) deleteRowNos.push_back(row);
    }
    ...
    int ptNos)
{
  ...
  for (unsigned i = 0; i < bcGausspts.size(); i++)
  {
    if (projType == "xy")
      bcGausspts[i][dim3] =
          (eqn[3] - eqn[1] * bcGausspts[i][dim2] - eqn[0] * bcGausspts[i][dim1]) / eqn[2];
    else if (projType == "yz")
      bcGausspts[i][dim3] =
          (eqn[3] - eqn[1] * bcGausspts[i][dim1] - eqn[2] * bcGausspts[i][dim2]) / eqn[0];
    else if (projType == "zx")
      bcGausspts[i][dim3] =
