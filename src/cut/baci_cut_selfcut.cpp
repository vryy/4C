 ...
 *-------------------------------------------------------------------------------------*/
bool CORE::GEO::CUT::SelfCut::MergeCoincidingNodes(Side* keep, Side* replace)
{
  bool merge = false;
 ...
 *-----------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::operationsForNodeMerging(
 ...
 *-------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::FindSelfCutTriangulation()
{
  for (std::map<plain_int_set, Teuchos::RCP<Side>>::iterator i = selfcut_sides_.begin();
       i != selfcut_sides_.end(); ++i)
  {
    ...
    for (std::vector<Cycle*>::iterator i = maincycles.begin(); i != maincycles.end(); ++i)
    {
           ...
           i != maincycletriangles.end(); ++i)
      {
        ...
        {
          //          Output(*cutside, "FindSelfCutTriangulation");
          throw CORE::Exception("SelfCut: triangulation unsuccessful; triangle without 3 points");
        }
       ...
       i != selfcut_sides_.end(); ++i)
  {
         ...
         i != selfcuttriangles.end(); ++i)
    {
      ...
      if (selfcutsidenodeidsset.size() != 3)
      {
        ...
        throw CORE::Exception("SelfCut: creating sides unsuccessful; didn't find 3-id-set");
      }
      if (selfcutsidenodeids.size() != 3)
      {
        std::cout << "selfcutsidenodeids.size(): " << selfcutsidenodeids.size() << "\n";
        //        Output(*cutside, "CreateSelfCutSides");
        throw CORE::Exception("SelfCut: creating sides unsuccessful; didn't find 3 ids");
      }
...
bool CORE::GEO::CUT::SelfCut::ConnectedtoBackground(Edge* edge)
{
...
bool CORE::GEO::CUT::SelfCut::ConnectedtoBackground(Node* node)
{
 ...
 *-------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::DetermineSelfCutPosition()
{
  for (std::map<plain_int_set, Teuchos::RCP<Edge>>::iterator i = selfcut_edges_.begin();
       i != selfcut_edges_.end(); ++i)
  {
 ...
 *-------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::PropagateSelfCutPosition()
{
           ...
           ++i)
      {
        ...
        switch (cutsideedgepos)
        {
          ...
          case Point::outside:
            ...
            {
              throw CORE::Exception("SelfCut: mixed PointPosition of cutsideedges");
            }
  ...
  {
    throw CORE::Exception("SelfCut: all cutsides undecided");
  }
  while (undecidedsides.size() > 0)
  {
          ...
          if (siblingside != nullptr)
          {
            ...
            switch (siblingsideposition)
            {
              case Point::undecided:
                ...
                {
                  throw CORE::Exception("SelfCut: uncomplete set of undecided cutsides");
                }
                break;
              case Point::oncutsurface:
                throw CORE::Exception("SelfCut: illegal side position");
                break;
              case Point::inside:
              case Point::outside:
    ...
    if (undecidedsidesize == undecidedsides.size())
      throw CORE::Exception("SelfCut: no progress in cutside position");
  }
}
 ...
 *                                                                          wirtz 07/16
 *-------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::ConstructConnectivity()
{
  const std::map<int, Teuchos::RCP<Node>>& cutsidenodes = mesh_.Nodes();
 ...
 *-------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::CuttedSideStatusText()
{
 ...
 *-------------------------------------------------------------------------------------*/
void CORE::GEO::CUT::SelfCut::CuttedSideStatusGmsh(const std::string& name)
{
       ...
       i != cutsides.end(); ++i)
  {
    ...
    {
      throw CORE::Exception("SelfCut: irregular side");
    }
