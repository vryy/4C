      ...
      RCP<Epetra_SerialDenseMatrix> dseg)
{
  ...
  {
    if (sele.Shape()!=MortarElement::line2 && sele.Shape()!=MortarElement::tri3)
    {
      duallin = true;
    ...
    for (int i=0;i<nrow;++i)
    {
      ...
      if (!bound)
      {
        ...
        for (int k=0; k<nrow; ++k)
        {
          ...
          map<int,double>& ddmap_ik = mymrtrnode->GetDerivD()[sgid];
      ...
      else
      {
        ...
        for (int k=0;k<nrow;++k)
        {
          ...
          if (bound2) continue;
          map<int,double>& nodemmap = mymrtrnode2->GetDerivM()[bgid];
          ...
          if (duallin)
          {
            for (int j=0;j<nrow;++j)
            {
              ...
              if (i==0) sele.ShapeFunctions(MortarElement::dual1D_base_for_edge0,eta,vallin,derivlin);
              else if (i==1) sele.ShapeFunctions(MortarElement::dual1D_base_for_edge1,eta,vallin,derivlin);
     ...
     RCP<Epetra_SerialDenseVector> gseg)
{
  ...
  bool duallin = false;
  vector<vector<map<int,double> > > dualmap(nrow,vector<map<int,double> >(nrow));
  if ((shapefcn_ == Interface::DualFunctions) && (sele.Shape()==MortarElement::line3))
  {
    duallin=true;
    sele.DerivShapeDual(dualmap);
  }
  ...
  for (int gp=0;gp<nGP();++gp)
  {
    ...
    for (int i=0;i<nrow;++i)
    {
      map<int,double>& dmap_nxsl_i = smrtrnodes[i]->GetDerivN()[0];
      map<int,double>& dmap_nysl_i = smrtrnodes[i]->GetDerivN()[1];
    ...
    if (dod && bound)
    {
      ...
      for (int j=0;j<nrow;++j)
      {
        ...
        map<int,double>& nodemap = mymrtrnode->GetDerivD()[sgid];
        double fac = 0.0;
          ...
          for (int k=0;k<nrow;++k)
          {
            ...
            if (duallin)
              for (int m=0;m<nrow;++m)
              {
                LINALG::SerialDenseVector vallin(nrow-1);
                LINALG::SerialDenseMatrix derivlin(nrow-1,1);
                if (j==0) sele.ShapeFunctions(MortarElement::dual1D_base_for_edge0,sxi,vallin,derivlin);
                else if (j==1) sele.ShapeFunctions(MortarElement::dual1D_base_for_edge1,sxi,vallin,derivlin);
    ...
    for (int j=0;j<nrow;++j)
    {
      ...
      if (shapefcn_ == Interface::StandardFunctions)
      {
        ...
        for (int k=0; k<ncol; ++k)
        {
          ...
          // get the correct map as a reference
          map<int,double>& dmmap_jk = mymrtrnode->GetDerivM()[mgid];
        ...
        if (dod)
        {
          ...
          for (int k=0; k<nrow; ++k)
          {
            ...
            // get the correct map as a reference
            map<int,double>& ddmap_jk = mymrtrnode->GetDerivD()[sgid];
     ...
     RCP<Epetra_SerialDenseVector> gseg)
{
  ...
  bool duallin = false;
  vector<vector<map<int,double> > > dualmap(nrow,vector<map<int,double> >(nrow));
  if ((shapefcn_ == Interface::DualFunctions) && (sele.Shape()!=MortarElement::tri3))
  {
    duallin = true;
    sele.DerivShapeDual(dualmap);
  }
  ...
  for (int gp=0;gp<nGP();++gp)
  {
    ...
    for (int i=0;i<nrow;++i)
    {
      map<int,double>& dmap_nxsl_i = smrtrnodes[i]->GetDerivN()[0];
      map<int,double>& dmap_nysl_i = smrtrnodes[i]->GetDerivN()[1];
      map<int,double>& dmap_nzsl_i = smrtrnodes[i]->GetDerivN()[2];
    ...
    for (int j=0; j<nrow; ++j)
    {
      ...
      {
        // get the D-map as a reference
        map<int,double>& ddmap_jj = mymrtrnode->GetDerivD()[sgid];
     ...
     RCP<Epetra_SerialDenseVector> gseg)
{
  ...
  if ( (shapefcn_ == Interface::DualFunctions) && (sintele.Shape()!=MortarElement::tri3))
  {
    dualintlin = true;
...
#else
    if (sele.Shape()==MortarElement::tri6 || sele.Shape()==MortarElement::quad8) 
 ...
 *----------------------------------------------------------------------*/
void MORTAR::Integrator::IntegrateKappaPenalty(MORTAR::MortarElement& sele,
                             double* sxia, double* sxib,
                             RCP<Epetra_SerialDenseVector> gseg)
{
  // explicitely defined shapefunction type needed
  ...
  for (int gp=0;gp<nGP();++gp)
  {
    ...
    {
#ifdef MORTARPETROVGALERKIN
      dserror("MORTARPETROVGALERKIN flag invalid for std. shape fct. other than 3D quad.");
#endif
      // add current Gauss point's contribution to gseg
 ...
 *----------------------------------------------------------------------*/
void MORTAR::Integrator::DerivXiAB2D(MORTAR::MortarElement& sele,
                                    double& sxia, double& sxib,
                                    MORTAR::MortarElement& mele,
                                    double& mxia, double& mxib,
                                    ...
                                    bool& startslave, bool& endslave)
{
  ...
  if (startslave==true)
  {
    ...
    map<int,double>& nxmap_b = smrtrnodes[0]->GetDerivN()[0];
    map<int,double>& nymap_b = smrtrnodes[0]->GetDerivN()[1];
  ...
  if (endslave==true)
  {
    ...
    map<int,double>& nxmap_a = smrtrnodes[1]->GetDerivN()[0];
    map<int,double>& nymap_a = smrtrnodes[1]->GetDerivN()[1];
  ...
  if (startslave==false)
  {
    ...
    for (int i=0;i<numsnode;++i)
    {
      map<int,double>& nxmap_curr = smrtrnodes[i]->GetDerivN()[0];
      map<int,double>& nymap_curr = smrtrnodes[i]->GetDerivN()[1];
 ...
 *----------------------------------------------------------------------*/
void MORTAR::Integrator::DerivXiGP2D(MORTAR::MortarElement& sele,
                                    MORTAR::MortarElement& mele,
 ...
 *----------------------------------------------------------------------*/
void MORTAR::Integrator::DerivXiGP3D(MORTAR::MortarElement& sele,
                                      MORTAR::MortarElement& mele,
                                      double* sxigp, double* mxigp,
 ...
 *----------------------------------------------------------------------*/
void MORTAR::Integrator::DerivXiGP3DAuxPlane(MORTAR::MortarElement& ele,
 ...
 *----------------------------------------------------------------------*/
bool MORTAR::Integrator::AssembleG(const Epetra_Comm& comm,
                                    MORTAR::MortarElement& sele,
                                    Epetra_SerialDenseVector& gseg)
{

#ifdef MORTARPETROVGALERKIN
  MORTAR::IntElement& sintref = static_cast<MORTAR::IntElement&>(sele);
#else
  MORTAR::MortarElement& sintref = sele;
#endif // #ifdef MORTARPETROVGALERKIN
