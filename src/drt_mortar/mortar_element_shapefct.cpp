    ...
    LINALG::SerialDenseMatrix& deriv)
{
  ...
  case MortarElement::biquad2D_modified:
  {
    ...
    deriv(0, 0) = derivtmp(0, 0) + (derivtmp(4, 0) + derivtmp(7, 0)) * fac
        + 0.5 * derivtmp(8, 0) * fac;
    deriv(0, 1) = derivtmp(0, 1) + (derivtmp(4, 1) + derivtmp(7, 1)) * fac
        + 0.5 * derivtmp(8, 1) * fac;
    deriv(1, 0) = derivtmp(1, 0) + (derivtmp(4, 0) + derivtmp(5, 0)) * fac
        + 0.5 * derivtmp(8, 0) * fac;
    deriv(1, 1) = derivtmp(1, 1) + (derivtmp(4, 1) + derivtmp(5, 1)) * fac
        + 0.5 * derivtmp(8, 1) * fac;
  ...
  case MortarElement::serendipitydual2D_only_lin:
  {
    ...
    if (MoData().DualShape() == Teuchos::null)
    {
      ...
      for (int i = 0; i < integrator.nGP(); ++i)
      {
        double gpc[2] =
        { integrator.Coordinate(i, 0), integrator.Coordinate(i, 1) };
        ShapeFunctions(MortarElement::serendipity2D_only_lin, gpc, valquad,
            derivquad);
    ...
    // evaluate dual shape functions at loc. coord. xi
    ShapeFunctions(MortarElement::serendipity2D_only_lin, xi, valquad,
        derivquad);
    val.Zero();
    deriv.Zero();
  ...
  case MortarElement::biquaddual2D_only_lin:
  {
    ...
    if (MoData().DualShape() == Teuchos::null)
    {
      ...
      for (int i = 0; i < integrator.nGP(); ++i)
      {
        double gpc[2] =
        { integrator.Coordinate(i, 0), integrator.Coordinate(i, 1) };
        ShapeFunctions(MortarElement::biquad2D_only_lin, gpc, valquad,
            derivquad);
        detg = Jacobian(gpc);
  ...
  case MortarElement::quaddual1D_edge0:
  {
    ...
    {
      double gpc[2] =
      { integrator.Coordinate(i, 0), 0.0 };
      ShapeFunctions(MortarElement::quad1D, gpc, valquad, derivquad);
      ShapeFunctions(MortarElement::dual1D_base_for_edge0, gpc, vallin,
          derivlin);
      detg = Jacobian(gpc);
      ...
      for (int j = 1; j < nnodes; ++j)
        ...
        {
          me(j - 1, k - 1) += integrator.Weight(i) * vallin[j - 1] * valquad[k]
              * detg;
          de(j - 1, k - 1) += (j == k) * integrator.Weight(i) * valquad[k]
              * detg;
        }
    }
  ...
  case MortarElement::quaddual1D_edge1:
  {
    ...
    for (int i = 0; i < integrator.nGP(); ++i)
    {
      ...
      { integrator.Coordinate(i, 0), 0.0 };
      ShapeFunctions(MortarElement::quad1D, gpc, valquad, derivquad);
      ShapeFunctions(MortarElement::dual1D_base_for_edge1, gpc, vallin,
          derivlin);
      detg = Jacobian(gpc);
  ...
  case DRT::Element::quad9:
  {
    ...
    if (dual)
    {
      if (Shape() == tri3)
        ...
        /*Shape()==quad9*/ShapeFunctions(MortarElement::biquaddual2D, xi, val,
            deriv);
    }
    ...
    else
    {
      if (Shape() == tri3)
        ...
        /*Shape()==quad9*/ShapeFunctions(MortarElement::biquad2D, xi, val,
            deriv);
    }
  ...
  case DRT::Element::quad9:
  {
    ...
    if (dual)
    {
      ...
      if (Shape() == tri6)
        ...
        ShapeFunctions(MortarElement::serendipitydual2D_only_lin, xi, val,
            deriv);
      else
        /*Shape()==quad9*/ShapeFunctions(MortarElement::biquaddual2D_only_lin,
            xi, val, deriv);
    }
    ...
    else
    {
      if (Shape() == tri6)
        ...
        /*Shape()==quad9*/ShapeFunctions(MortarElement::biquad2D_only_lin, xi,
            val, deriv);
    }
 ...
 *----------------------------------------------------------------------*/
void MORTAR::MortarElement::ShapeFunctionLinearizations(
    MORTAR::MortarElement::ShapeType shape,
    GEN::pairedvector<int,Epetra_SerialDenseMatrix>& derivdual)
{
  ...
  case MORTAR::MortarElement::bilindual2D:
  {
    ...
    else
    {
      ...
      GEN::pairedvector<int,Epetra_SerialDenseMatrix> derivde_me(nnodes*3,0,Epetra_SerialDenseMatrix(nnodes+1,nnodes));
      ...
      for (int i = 0; i < integrator.nGP(); ++i)
      {
        double gpc[2] =
        { integrator.Coordinate(i, 0), integrator.Coordinate(i, 1) };
        UTILS::mortar_shape_function_2D(val, gpc[0], gpc[1],
            MortarElement::bilin2D);
        detg = Jacobian(gpc);
  ...
  case MORTAR::MortarElement::quaddual1D:
  {
    ...
    else
    {
      ...
      MoData().DerivDualShape() =
          Teuchos::rcp(new GEN::pairedvector<int,Epetra_SerialDenseMatrix>(nnodes*2,0,Epetra_SerialDenseMatrix(nnodes,nnodes)));
      GEN::pairedvector<int,Epetra_SerialDenseMatrix>& derivae=*(MoData().DerivDualShape());
      ...
      // two-dim arrays of maps for linearization of me/de
      GEN::pairedvector<int,Epetra_SerialDenseMatrix> derivde_me(nnodes*2,0,Epetra_SerialDenseMatrix(nnodes+1,nnodes));
...
//      }
  ...
  case MORTAR::MortarElement::quaddual1D_edge0:
  {
    ...
    for (int i = 0; i < integrator.nGP(); ++i)
    {
      ...
      ShapeFunctions(MORTAR::MortarElement::dual1D_base_for_edge0, gpc, vallin,
          derivlin);
      detg = Jacobian(gpc);
    ...
    for (int i = 1; i < nnodes; ++i)
    {
      for (int j = 1; j < nnodes; ++j)
      {
        ...
        {
          // part1: Lin(De)*Inv(Me)
          for (CI p = derivde[i - 1][l - 1].begin();
              p != derivde[i - 1][l - 1].end(); ++p)
            derivdual[i][j][p->first] += me(l - 1, j - 1) * (p->second);

          // part2: Ae*Lin(Me)*Inv(Me)
          for (int k = 1; k < nnodes; ++k) // loop over sum k
          {
            for (CI p = derivme[k - 1][l - 1].begin();
                p != derivme[k - 1][l - 1].end(); ++p)
              derivdual[i][j][p->first] -= ae(i - 1, k - 1) * me(l - 1, j - 1)
                  * (p->second);
          }
        }
      }
  ...
  case MORTAR::MortarElement::quaddual1D_edge1:
  {
    ...
    // two-dim arrays of maps for linearization of me/de
    std::vector<std::vector<GEN::pairedvector<int, double> > > derivme(nnodes,
        std::vector<GEN::pairedvector<int, double> >(nnodes, 2 * nnodes));
    std::vector<std::vector<GEN::pairedvector<int, double> > > derivde(nnodes,
        std::vector<GEN::pairedvector<int, double> >(nnodes, 2 * nnodes));
    ...
    for (int i = 0; i < integrator.nGP(); ++i)
    {
      ...
      { integrator.Coordinate(i, 0), 0.0 };
      ShapeFunctions(MORTAR::MortarElement::quad1D, gpc, valquad, derivquad);
      ShapeFunctions(MORTAR::MortarElement::dual1D_base_for_edge1, gpc, vallin,
          derivlin);
      detg = Jacobian(gpc);
    ...
    LINALG::SerialDenseMatrix& secderiv, const int& valdim)
{
  ...
  case DRT::Element::nurbs4:
  {
    ...
    DRT::NURBS::UTILS::nurbs_get_2D_funct_deriv_deriv2(auxval, auxderiv,
        auxderiv2, uv, Knots(), weights, nurbs4);
  ...
  case DRT::Element::nurbs8:
  {
    ...
    DRT::NURBS::UTILS::nurbs_get_2D_funct_deriv_deriv2(auxval, auxderiv,
        auxderiv2, uv, Knots(), weights, nurbs8);
  ...
  case DRT::Element::nurbs9:
  {
    ...
    DRT::NURBS::UTILS::nurbs_get_2D_funct_deriv_deriv2(auxval, auxderiv,
        auxderiv2, uv, Knots(), weights, nurbs9);
