...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
MORTAR::MatrixRowColTransformer::MatrixRowColTransformer(
    const unsigned num_transformer )
    : isinit_( false ),
      issetup_( false ),
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void MORTAR::MatrixRowColTransformer::Init(
...
void MORTAR::MatrixRowColTransformer::Setup()
{
  ...
  for ( plain_block_map_pairs::const_iterator cit = slave_row_.begin();
        cit != slave_row_.end(); ++cit )
  {
...
MORTAR::MatrixRowColTransformer::RedistributedToUnredistributed(
    ...
    const LINALG::SparseMatrix& src_mat )
{
  ThrowIfNotInitAndSetup();

  Teuchos::RCP<LINALG::SparseMatrix> dst_mat =
      Teuchos::rcp( new LINALG::SparseMatrix( **master_row_[ bt ] ,
          src_mat.EpetraMatrix()->MaxNumEntries(), false, true ) );
    ...
    LINALG::SparseMatrix& dst_mat )
{
  ...
  const int err = dst_mat.EpetraMatrix()->Import( *src_mat.EpetraMatrix(),
      *slave_to_master_[bt], Insert );
    ...
    const LINALG::SparseMatrix& src_mat )
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> dst_mat =
      Teuchos::rcp( new LINALG::SparseMatrix( **slave_row_[ bt ] ,
          src_mat.EpetraMatrix()->MaxNumEntries(), false, true ) );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void MORTAR::MatrixRowColTransformer::ResetExporter(
    Teuchos::RCP<Epetra_Export>& exporter ) const
{
