...
void STR::IMPLICIT::GenAlphaLieGroup::UpdateConstantStateContributions()
{
  ...
  // ---------------------------------------------------------------------------
  (*const_vel_acc_update_ptr_)(0)->Scale(1.0 - gamma_ / beta_, *GlobalState().GetVelN());
  (*const_vel_acc_update_ptr_)(0)->Update((1.0 - gamma_ / (2.0 * beta_)) * dt, *accn_mod_, 1.0);
  ...
  // ---------------------------------------------------------------------------
  (*const_vel_acc_update_ptr_)(1)->Update(
      -(1.0 - alpham_) / (beta_ * dt * dt * (1.0 - alphaf_)), *disnp0_ptr, 1.0);
}
