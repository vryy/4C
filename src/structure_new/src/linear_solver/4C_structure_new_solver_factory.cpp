...
std::shared_ptr<Solid::SOLVER::Factory::LinSolMap> Solid::SOLVER::Factory::build_lin_solvers(
    const std::set<Solid::ModelType>& modeltypes, const Teuchos::ParameterList& sdyn,
  ...
  for (mt_iter = modeltypes.begin(); mt_iter != modeltypes.end(); ++mt_iter)
  {
    switch (*mt_iter)
    {
      ...
      case Solid::model_multiscale:
      {
         ...
         * if true. */
        LinSolMap::iterator iter = linsolvers->find(Solid::model_structure);
        if (iter == linsolvers->end())
          (*linsolvers)[Solid::model_structure] = build_structure_lin_solver(sdyn, actdis);
        break;
      }
      ...
      case Solid::model_beaminteraction:
        (*linsolvers)[Solid::model_structure] = build_beam_interaction_lin_solver(sdyn, actdis);
        break;
      default:
    ...
    const Teuchos::ParameterList& sdyn, Core::FE::Discretization& actdis) const
{
  ...
  auto algochoice = Teuchos::getIntegralValue<Solid::ConSolveAlgo>(strparams, "UZAWAALGO");
...
Solid::SOLVER::build_lin_solvers(const std::set<Solid::ModelType>& modeltypes,
