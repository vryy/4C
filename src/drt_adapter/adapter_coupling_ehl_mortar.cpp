...
contact_regularization_(DRT::INPUT::IntegralValue<int>(DRT::Problem::Instance()->ContactDynamicParams(),"REGULARIZED_NORMAL_CONTACT")),
regularization_thickness_(DRT::Problem::Instance()->ContactDynamicParams().get<double>("REGULARIZATION_THICKNESS")),
regularization_compliance_(DRT::Problem::Instance()->ContactDynamicParams().get<double>("REGULARIZATION_STIFFNESS"))
{
  ...
  if (contact_regularization_)
    regularization_compliance_=1./regularization_compliance_;
  if (DRT::INPUT::IntegralValue<int>(DRT::Problem::Instance()->ContactDynamicParams(),"REGULARIZED_NORMAL_CONTACT")==true
      &&
      DRT::INPUT::IntegralValue<bool>(DRT::Problem::Instance()->ElastoHydroDynamicParams(),"DRY_CONTACT_MODEL")==false)
...
void ADAPTER::CouplingEhlMortar::Setup(
           Teuchos::RCP<DRT::Discretization>   masterdis,
           Teuchos::RCP<DRT::Discretization>   slavedis,
           std::vector<int>                    coupleddof,
           const std::string&                  couplingcond)
{
  ...
  INPAR::CONTACT::FrictionType ftype = DRT::INPUT::IntegralValue<
        INPAR::CONTACT::FrictionType>(DRT::Problem::Instance()->ContactDynamicParams(), "FRICTION");
  ...
  {
    const std::vector<int>* group1v = ehl_conditions[i]->Get<std::vector<int> >(
        "Interface ID");
    if (!group1v)
      dserror("no interface Id found");
    if (group1v->operator [](0)!=1)
      dserror("only one interface id expected");
    ...
    )
{
  ...
  const INPAR::CONTACT::ConstraintDirection& constr_direction_=DRT::INPUT::IntegralValue<INPAR::CONTACT::ConstraintDirection>(Interface()->IParams(),"CONSTRAINT_DIRECTIONS");

  // return if this state has already been evaluated
  if (not AlreadyEvaluated(disp))
    Integrate(disp,dt);
  ...
  Teuchos::RCP<LINALG::SparseMatrix> dcsdd =
      Teuchos::rcp(new LINALG::SparseMatrix(*interface_->ActiveDofs(),100,true,false,LINALG::SparseMatrix::FE_MATRIX));
  ...
  Teuchos::RCP<LINALG::SparseMatrix> dmatrix = Teuchos::rcp(new LINALG::SparseMatrix(*interface_->SlaveRowDofs(), 10));
  Teuchos::RCP<LINALG::SparseMatrix> mmatrix = Teuchos::rcp(new LINALG::SparseMatrix(*interface_->SlaveRowDofs(), 100));
  interface_->AssembleDM(*dmatrix, *mmatrix);
  dmatrix->Complete();
  mmatrix->Complete(*masterdofrowmap_, *slavedofrowmap_);

  // setup some linearizations
  LINALG::SparseMatrix linDcontactLM            (*interface_->SlaveRowDofs(),100,true,false,LINALG::SparseMatrix::FE_MATRIX);
  LINALG::SparseMatrix linMcontactLM            (*interface_->MasterRowDofs(),100,true,false,LINALG::SparseMatrix::FE_MATRIX);
  interface_->AssembleLinDM(linDcontactLM,linMcontactLM);

  // D and M matrix for the active nodes
  Teuchos::RCP<LINALG::SparseMatrix> dInv = Teuchos::rcp(new LINALG::SparseMatrix(*interface_->SlaveRowDofs(),100,true,false));
  ...
  // ****************************************************
  // split first row
  LINALG::SplitMatrix2x2(kss,str_gni_dofs,dummy_map1,gdisp_DofRowMap,dummy_map2,
  ...
  LINALG::SplitMatrix2x2(tmp,gmdof,dummy_map1,gdisp_DofRowMap,dummy_map2,
  ...
  // ****************************************************
  // split first row
  LINALG::SplitMatrix2x2(kst,str_gni_dofs,dummy_map1,gpres_DofRowMap,dummy_map2,
  ...
  // split the remaining two rows
  LINALG::SplitMatrix2x2(tmp,gmdof,dummy_map1,gpres_DofRowMap,dummy_map2,
  ...
  Epetra_Vector rsm (*interface_->MasterRowDofs()); LINALG::Export(*rs,rsm);
  Teuchos::RCP<Epetra_Vector> rsa = Teuchos::rcp(new Epetra_Vector(*interface_->ActiveDofs())); LINALG::Export(*rs,*rsa);
  // ****************************************************
  // split rhs vectors***********************************
  // ****************************************************

  // D and M matrix for the active nodes
  Teuchos::RCP<LINALG::SparseMatrix> dInvA = Teuchos::rcp(new LINALG::SparseMatrix(*interface_->ActiveDofs(),100,true,false));
  Teuchos::RCP<LINALG::SparseMatrix> mA    = Teuchos::rcp(new LINALG::SparseMatrix(*interface_->ActiveDofs(),100,true,false));

  dummy_map1 = dummy_map2 = Teuchos::null;
  dummy1 = dummy2 = dummy3 = Teuchos::null;
  LINALG::SplitMatrix2x2(dmatrix,active_dofs,dummy_map1,active_dofs,dummy_map2,dInvA,dummy1,dummy2,dummy3);
  ...
  dummy_map1 = dummy_map2 = Teuchos::null;
  dummy1 = dummy2 = dummy3 = Teuchos::null;
  LINALG::SplitMatrix2x2(mmatrix,active_dofs,dummy_map1,gmdof,dummy_map2,mA,dummy1,dummy2,dummy3);
  ...
  if (sdirichtoggle_.is_null())
    dserror("you didn't call StoreDirichletStatus");
  if (constr_direction_==INPAR::CONTACT::constr_xyz)
  {
    double haveDBC=0;
...
void ADAPTER::CouplingEhlMortar::AssembleRealGap()
{
  ...
  static const double offset =DRT::Problem::Instance()->LubricationDynamicParams().get<double>("GAP_OFFSET");
  for (int i=0;i<nodal_gap_->Map().NumMyElements();++i)
...
void ADAPTER::CouplingEhlMortar::CreateForceVec(Teuchos::RCP<Epetra_Vector>& n,Teuchos::RCP<Epetra_Vector>& t)
{
  n=Teuchos::rcp(new Epetra_Vector(*interface_->SlaveRowDofs()));
  t=Teuchos::rcp(new Epetra_Vector(*interface_->SlaveRowDofs()));
  for (int i=0;i<interface_->SlaveRowNodes()->NumMyElements();++i)
  {
    CONTACT::FriNode* cnode = dynamic_cast<CONTACT::FriNode*>(
        interface_->Discret().lRowNode(i));
...
void ADAPTER::CouplingEhlMortar::CreateActiveSlipToggle(
    Teuchos::RCP<Epetra_Vector>* active,
    Teuchos::RCP<Epetra_Vector>* slip,
    Teuchos::RCP<Epetra_Vector>* active_old
    )
{
...
void ADAPTER::CouplingEhlMortar::ReadRestart(IO::DiscretizationReader& reader)
{
  ...
  Teuchos::RCP<Epetra_Vector> active_toggle=Teuchos::rcp(new Epetra_Vector(*interface_->SlaveRowNodes()));
  Teuchos::RCP<Epetra_Vector> active_old_toggle=Teuchos::rcp(new Epetra_Vector(*interface_->SlaveRowNodes()));
  Teuchos::RCP<Epetra_Vector>   slip_toggle=Teuchos::rcp(new Epetra_Vector(*interface_->SlaveRowNodes()));
