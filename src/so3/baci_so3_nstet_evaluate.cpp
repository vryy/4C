    ...
    CORE::LINALG::SerialDenseVector& elevec3_epetra)
{
  ...
  if (action == "none")
    ...
    act = NStet::calc_struct_linstiff;
  else if (action == "calc_struct_nlnstiff")
    act = NStet::calc_struct_nlnstiff;
  else if (action == "calc_struct_internalforce")
    act = NStet::calc_struct_internalforce;
  else if (action == "calc_struct_linstiffmass")
    act = NStet::calc_struct_linstiffmass;
  else if (action == "calc_struct_nlnstiffmass")
    act = NStet::calc_struct_nlnstiffmass;
  else if (action == "calc_struct_nlnstifflmass")
    act = NStet::calc_struct_nlnstifflmass;
  else if (action == "calc_struct_stress")
    act = NStet::calc_struct_stress;
  else if (action == "calc_struct_eleload")
    act = NStet::calc_struct_eleload;
  else if (action == "calc_struct_fsiload")
    act = NStet::calc_struct_fsiload;
  else if (action == "calc_struct_update_istep")
    act = NStet::calc_struct_update_istep;
  else if (action == "calc_struct_reset_istep")
    act = NStet::calc_struct_reset_istep;
  else if (action == "multi_calc_dens")
    act = NStet::multi_calc_dens;
  else if (action == "multi_readrestart")
    act = NStet::multi_readrestart;
  ...
  switch (act)
  {
    ...
    case calc_struct_nlnstifflmass:
    {
      ...
      nstetnlnstiffmass(lm, mydisp, myres, &elemat1, &elemat2, &elevec1, nullptr, nullptr,
    ...
    case calc_struct_nlnstiff:
    {
      ...
      std::vector<double> myres(lm.size());
      DRT::UTILS::ExtractMyValues(*res, myres, lm);
      CORE::LINALG::Matrix<12, 12>* elemat1ptr = nullptr;
      if (elemat1.IsInitialized()) elemat1ptr = &elemat1;
      nstetnlnstiffmass(lm, mydisp, myres, elemat1ptr, nullptr, &elevec1, nullptr, nullptr,
          INPAR::STR::stress_none, INPAR::STR::strain_none);
    }
    break;
    ...
    case calc_struct_stress:
    {
      ...
      if (discretization.Comm().MyPID() == Owner())
      {
        ...
        nstetnlnstiffmass(lm, mydisp, myres, nullptr, nullptr, nullptr, &elestress, &elestrain,
            iostress, iostrain);
        ...
        auto ele = ElementType().pstab_cid_mis_.find(Id());
        if (ele == ElementType().pstab_cid_mis_.end()) dserror("Cannot find this element");
        auto elew = ElementType().pstab_cid_mis_weight_.find(Id());
        if (elew == ElementType().pstab_cid_mis_weight_.end())
          dserror("Cannot find this element weight");
        std::vector<int>& mis = ele->second;
        const auto nummis = (int)mis.size();
        if (nummis < 1) dserror("Element not associated with any mis node");
        std::vector<double>& misw = elew->second;
        const auto nummisw = (int)misw.size();
    ...
    case calc_struct_internalforce:
    {
      ...
      CORE::LINALG::Matrix<12, 12> myemat(true);
      nstetnlnstiffmass(lm, mydisp, myres, &myemat, nullptr, &elevec1, nullptr, nullptr,
          INPAR::STR::stress_none, INPAR::STR::strain_none);
    }
    break;
 ...
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::NStet::nstetnlnstiffmass(std::vector<int>& lm,  // location matrix
    std::vector<double>& disp,                                      // current displacements
    std::vector<double>& residual,                                  // current residuum
    CORE::LINALG::Matrix<12, 12>* stiffmatrix,                      // element stiffness matrix
    CORE::LINALG::Matrix<12, 12>* massmatrix,                       // element mass matrix
    CORE::LINALG::Matrix<12, 1>* force,                             // stress output options
...
{
  //--------------------------------------------------- geometry update
  CORE::LINALG::Matrix<3, 3>& defgrd = F();
  ...
  */
...
#ifndef PUSOSOLBERG  // dev stab on cauchy stresses
  {
    ...
    CORE::LINALG::Matrix<3, 3> Fbar(false);
    ...
    CORE::LINALG::Matrix<6, 6> cmatdev;  // set to zero?
  ...
  //---------------------------------------------- output of stress and strain
  {
    switch (iostrain)
    {
      case INPAR::STR::strain_gl:
      {
        ...
        for (int i = 0; i < 3; ++i) (*elestrain)(0, i) = ALPHA_NSTET * glstrainbar(i);
        for (int i = 0; i < 3; ++i) (*elestrain)(0, i) = ALPHA_NSTET * 0.5 * glstrainbar(i);
      }
      break;
  ...
  if (stiffmatrix)
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      {
        double BsB = 0.0;
        for (int dim = 0; dim < 3; ++dim) BsB += nxyz_(j, dim) * sBL[dim];
 ...
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::NStet::nstetlumpmass(CORE::LINALG::Matrix<12, 12>* emass)
{
  // lump mass matrix
  if (emass != nullptr)
  {
 ...
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::NStet::SelectMaterial(CORE::LINALG::Matrix<6, 1>& stress,
