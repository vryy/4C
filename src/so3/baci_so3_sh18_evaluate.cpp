    ...
    const int gp, CORE::LINALG::Matrix<9, 9>& dsg_shear_r)
{
  ...
  CORE::DRT::UTILS::shape_function_deriv1<DRT::Element::DiscretizationType::quad9>(xsi_[gp], deriv);
  ...
  for (int i = 0; i < 9; ++i)
  {
    ...
    if (coord_i(0) != coord_refNode[0])
      ...
      for (int gp = 0; gp < ip.IP().nquad; ++gp)
      {
        ...
        CORE::DRT::UTILS::shape_function<DRT::Element::DiscretizationType::quad9>(xi_gp, shape_gp);
        // derivative
    ...
    const int gp, CORE::LINALG::Matrix<9, 9>& dsg_transverse_t)
{
  ...
  CORE::DRT::UTILS::shape_function<DRT::Element::DiscretizationType::quad9>(xsi_[gp], shape);

  for (int i = 0; i < 9; ++i)
  {
    const CORE::LINALG::Matrix<3, 1> coord_i = NodeParamCoord(i);
    CORE::LINALG::Matrix<9, 1> shape_gp;
    CORE::DRT::UTILS::shape_function<DRT::Element::DiscretizationType::quad9>(coord_i, shape_gp);
    for (int k = 0; k < 9; ++k)
