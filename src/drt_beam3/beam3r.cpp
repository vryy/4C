  ...
  if (!isinit_)
  {
    ...
    for (int numgp=0; numgp<gausspoints_inertia.nquad; numgp++)
    {
      ...
      triad_interpolation_scheme_ptr->GetInterpolatedQuaternionAtXi(
          QnewGPmass_[numgp],
          gausspoints_inertia.qxg[numgp][0]);
    ...
    for (int numgp=0; numgp < gausspoints_damp_stoch.nquad; numgp++)
    {
      ...
      triad_interpolation_scheme_ptr->GetInterpolatedQuaternionAtXi(
          QnewGPdampstoch_[numgp],
          gausspoints_damp_stoch.qxg[numgp][0]);
...
//  }
  ...
  // Fixme @grill
  dserror("Beam3r::Tcurr(): the implementation of this method is highly questionable "
      "and needs rework!");
    ...
    const double                                      tolerance)
{
  ...
  if (!(nnode==2 && vpernode==1))
  {
    ...
    if (numiter>100)
      dserror("failed to compute length of element in reference configuration iteratively: "
          "Newton unconverged!");
  }
    ...
    std::vector<LINALG::TMatrix<T,3,3> >&       Itilde) const
{
  ...
  triad_interpolation_scheme_ptr->GetNodalGeneralizedRotationInterpolationMatricesAtXi(
      Itilde, xi);
