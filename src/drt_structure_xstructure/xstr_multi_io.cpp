...
XSTR::IO::MultiDiscretizationReader::MultiDiscretizationReader(
    ...
    const Teuchos::RCP< ::IO::InputControl> & input,
    int step)
    : ::IO::DiscretizationReader(),
      dis_wrapper_(Teuchos::rcp(dis_wrapper.get(),false))
{
  XSTR::MultiDiscretizationWrapper::XDisMap::const_iterator cit;
  for (cit  = dis_wrapper_->DiscretMap().begin();
       cit != dis_wrapper_->DiscretMap().end(); ++cit)
  {
    readers_[ cit->first ] =
      ...
      dis_wrapper_(dis_wrapper)
{
  ...
  {
    readers_[ cit->first ] =
        Teuchos::rcp(new XSTR::IO::DiscretizationReader(cit->second,step));
  }
}
...
{
  // get the vector type of the first discretization
  const enum ::IO::VectorType vt =
      readers_.begin()->second->VectorType(name);
    ...
    const DRT::DiscretizationInterface & dis)
{
  ...
  switch (vt)
  {
    ...
    {
      if (dis.NumDofSets()>1)
        dserror("Currently we support only one dofset!");
...
XSTR::IO::DiscretizationReader::DiscretizationReader(
    ...
    int step)
    : ::IO::DiscretizationReader(
        Teuchos::rcp_dynamic_cast<DRT::Discretization>(dis,true), step)
{
  // should stay empty, see base class
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
enum ::IO::VectorType XSTR::IO::DiscretizationReader::VectorType(
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::NewStep(const int step,
    const double time)
{
  ...
  for (it=writers_.begin();it!=writers_.end();++it)
    it->second->NewStep(step,time);
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::WriteDouble(
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::WriteInt(
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::WriteVector(
    const std::string name,
    Teuchos::RCP<const Epetra_MultiVector> full_vec,
    enum ::IO::VectorType vt)
{
  const int num_vecs = full_vec->NumVectors();
  XSTR::MultiDiscretizationWrapper::XDisMap::const_iterator cit;
  for (cit  = dis_wrapper_->DiscretMap().begin();
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::WriteMesh(const int step,
    const double time)
{
  XWriterMap::const_iterator cit;
  for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
    cit->second->WriteMesh(step,time);
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::WriteMesh(const int step,
    const double time, std::string name_base_file )
{
  dserror("Currently unsupported!");
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::ParticleOutput(const int step,
    const double time,
...
void XSTR::IO::MultiDiscretizationWriter::WriteNodeData(bool writeowner)
{
  XWriterMap::const_iterator cit;
  for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
    cit->second->WriteNodeData(writeowner);
}
...
void XSTR::IO::MultiDiscretizationWriter::OverwriteResultFile()
{
  XWriterMap::const_iterator cit;
  for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
    cit->second->OverwriteResultFile();
}
...
void XSTR::IO::MultiDiscretizationWriter::NewResultFile(int numb_run)
{
  XWriterMap::const_iterator cit;
  for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
    cit->second->NewResultFile(numb_run);
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::NewResultFile(
    std::string name_appendix, int numb_run)
{
  XWriterMap::const_iterator cit;
  for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
...
void XSTR::IO::MultiDiscretizationWriter::ClearMapCache()
{
  XWriterMap::const_iterator cit;
    for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
      cit->second->ClearMapCache();
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Teuchos::RCP< ::IO::OutputControl>
XSTR::IO::MultiDiscretizationWriter::Output() const
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::IO::MultiDiscretizationWriter::SetOutput(
    Teuchos::RCP< ::IO::OutputControl> output)
{
  // we set the same output control file pointer in all wrapped writers
  XWriterMap::const_iterator cit;
  for (cit =writers_.begin(); cit!=writers_.end(); ++cit)
    cit->second->SetOutput(output);
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
XSTR::IO::DiscretizationWriter::DiscretizationWriter(
    Teuchos::RCP<DRT::DiscretizationInterface> dis)
    : ::IO::DiscretizationWriter(
        Teuchos::rcp_dynamic_cast<DRT::Discretization>(dis,true))
{
  // empty constructor
}
    ...
    const int & step, Teuchos::RCP<char>& ebuffer)
{
  ...
  if (icontrol.eof())
    dserror("We couldn't find the right result section!");
