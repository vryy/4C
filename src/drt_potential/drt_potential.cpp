 ...
 *-------------------------------------------------------------------*/
POTENTIAL::Potential::Potential(Teuchos::RCP<DRT::Discretization> discretRCP,
    DRT::Discretization& discret) :
    discretRCP_(discretRCP), discret_(discret), searchTree_(
        Teuchos::rcp(new GEO::SearchTree(5)))
{
...
POTENTIAL::Potential::Potential(const POTENTIAL::Potential& old) :
    discretRCP_(old.discretRCP_), discret_(old.discret_), searchTree_(
        old.searchTree_), prob_dim_(old.prob_dim_)
{
  return;
}
...
void POTENTIAL::Potential::treeSearchElementsInCutOffRadius(
    ...
    const int label)
{
  potentialElementIds = searchTree_->searchElementsInRadius(*potentialdis,
    ...
    const int label)
{
  ...
  searchTree_->queryPotentialElements(elemXAABBList, eleXAABB,
      potentialElementIds, label);
...
void POTENTIAL::Potential::EvaluatePotentialfromCondition(
    Teuchos::RCP<DRT::Condition> cond, const LINALG::Matrix<3, 1>& x,
    ...
    LINALG::Matrix<3, 3>& potderiv2)
{
      ...
      || cond->Type() == DRT::Condition::ElectroRepulsion_Potential_Line)
  {
    ...
    EvaluateElectrostaticRepulsion(zeta_param_1, zeta_param_2, x, y, potderiv1,
        potderiv2);
  }
  else
  {
 ...
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::EvaluatePotentialfromCondition(
    Teuchos::RCP<DRT::Condition> cond, const LINALG::Matrix<2, 1>& x,
    ...
    LINALG::Matrix<3, 3>& potderiv2)
{
      ...
      || cond->Type() == DRT::Condition::LJ_Potential_Surface)
  {
    ...
    EvaluateLennardJonesPotential_Approx1(depth, rootDist, x, y, potderiv1,
        potderiv2);
 ...
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::EvaluateLennardJonesPotential(const double depth,
    const double rootDist, const LINALG::Matrix<3, 1>& x,
    const LINALG::Matrix<3, 1>& y,
    LINALG::Matrix<3, 1>& potderiv1,
    LINALG::Matrix<3, 3>& potderiv2)
{
  ...
  //----------------------------------------------------------------------
  const double dpotdr = ((12 * depth) / rootDist)
      * ((-1) * pow((double) (rootDist / distance), 13)
          + pow((double) (rootDist / distance), 7));
  for (int i = 0; i < 3; i++)
  ...
  //----------------------------------------------------------------------
  const double dpotdrdr = ((12 * depth) / (rootDist * rootDist))
      * (13 * pow((double) (rootDist / distance), 14)
          - 7 * pow((double) (rootDist / distance), 8));
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      potderiv2(i, j) = 0.0;
 ...
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::EvaluateLennardJonesPotential(const double depth,
    const double rootDist, const LINALG::Matrix<2, 1>& x,
    const LINALG::Matrix<2, 1>& y,
    LINALG::Matrix<2, 1>& potderiv1,
    LINALG::Matrix<2, 2>& potderiv2)
{
  ...
  // evaluate 1.derivative dphi/du_i
  //----------------------------------------------------------------------
  const double dpotdr = (-1.0) * M_PI * depth
      * ((693.0 / 256.0) * pow((double) (rootDist / distance), 12)
          - (15.0 / 4.0) * pow((double) (rootDist / distance), 6));
  for (int i = 0; i < 2; i++)
    potderiv1(i) = dpotdr * distance_unit(i);
  ...
  // evaluate 2.derivative dphi/du_i d_uiI  (this is not a mistake !!!!)
  //----------------------------------------------------------------------
  const double dpotdrdr = (M_PI * depth / rootDist)
      * ((2079.0 / 64.0) * pow((double) (rootDist / distance), 13)
          - (45.0 / 2.0) * pow((double) (rootDist / distance), 7));
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++)
      potderiv2(i, j) = 0.0;
...
void POTENTIAL::Potential::EvaluateLennardJonesPotential_Approx1(
    const double depth, const double rootDist, const LINALG::Matrix<3, 1>& x,
    const LINALG::Matrix<3, 1>& y,
    LINALG::Matrix<3, 1>& potderiv1,
    LINALG::Matrix<3, 3>& potderiv2)
{
  ...
  double dpotdr = (12.0 * depth * rootDist)
      * ((1.0 / 20.0) * (pow((double) (rootDist / distance), 5))
          - (1.0 / 110.0) * pow((double) (rootDist / distance), 11));
  ...
  const double dpotdrdr = (12.0 * depth)
    ...
    LINALG::Matrix<3, 3>& Fsderiv)
{
  ...
  //dpotdr entspricht -Fs
  double F = M_PI * depth * pow(rootDist, 3)
      * ((1.0 / 45.0) * pow((double) (rootDist / distance), 9)
          - (1.0 / 3.0) * pow((double) (rootDist / distance), 3));
  ...
  //----------------------------------------------------------------------
  const double dFdr = M_PI * depth * pow(rootDist, 2)
      * (pow((double) (rootDist / distance), 4)
          - (1.0 / 5.0) * pow((double) (rootDist / distance), 10));
    ...
    LINALG::Matrix<3, 3>& potderiv2)
{
  ...
  // evaluate 1.derivative dphi/du_i
  //----------------------------------------------------------------------
  const double dpotdr = zeta_param_1 * (-1) * zeta_param_2
      * exp((-1) * zeta_param_2 * distance);
  for (int i = 0; i < 3; i++)
    potderiv1(i) = dpotdr * distance_unit(i);
  ...
  // evaluate 2.derivative dphi/du_i d_uiI  (this is not a mistake !!!!)
  //----------------------------------------------------------------------
  const double dpotdrdr = (-1) * zeta_param_1 * zeta_param_2 * (-1)
      * zeta_param_2 * exp((-1) * zeta_param_2 * distance);
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      potderiv2(i, j) = 0.0;
 ...
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::EvaluateVanDerWaals(const double lambda,
    const LINALG::Matrix<3, 1>& x,
    const LINALG::Matrix<3, 1>& y,
    LINALG::Matrix<3, 1>& potderiv1,
    LINALG::Matrix<3, 3>& potderiv2)
    ...
    LINALG::Matrix<2, 2>& potderiv2)
{
  ...
  // evaluate 1.derivative dphi/du_i
  //----------------------------------------------------------------------
  const double dpotdr = (M_PI * lambda) * (3.0 / 8.0)
      * (pow((double) (1.0 / distance), 6));
  for (int i = 0; i < 2; i++)
    potderiv1(i) = dpotdr * distance_unit(i);
  ...
  // evaluate 2.derivative dphi/du_i d_uiI  (this is not a mistake !!!!)
  //----------------------------------------------------------------------
  const double dpotdrdr = (M_PI * lambda) * (45.0 / 4.0)
      * (pow((double) (1.0 / distance), 7));
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++)
      potderiv2(i, j) = 0.0;
 ...
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::EvaluateVanDerWaals_Approx1(const double lambda,
    const LINALG::Matrix<3, 1>& x,
    const LINALG::Matrix<3, 1>& y,
    LINALG::Matrix<3, 1>& potderiv1,
    LINALG::Matrix<3, 3>& potderiv2)
{
  ...
  //----------------------------------------------------------------------
  //dpotdrdr entspricht -Fsdr
  const double dpotdrdr = -(3.0 / 2.0) * lambda
      * (pow((double) (1.0 / distance), 6));
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      potderiv2(i, j) = 0.0;
...
/*-------------------------------------------------------------------*
 ...
 | evaluate Van der Waals potential (for volume approximation) 3D |
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::EvaluateVanDerWaals_Approx2(const double lambda,
    const LINALG::Matrix<3, 1>& x,
    const LINALG::Matrix<3, 1>& y,
    LINALG::Matrix<3, 1>& Fs,
    LINALG::Matrix<3, 3>& Fsderiv)
{
  ...
  //dpotdr entspricht -Fs
  double F = (-1.0) * lambda * M_PI * (1.0 / 6.0)
      * (pow((double) (1.0 / distance), 3));
  ...
  // evaluate 1. derivative
  //----------------------------------------------------------------------
  const double dFdr = (1.0 / 2.0) * M_PI * lambda
      * (pow((double) (1.0 / distance), 4));
 ...
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::computeDistance(const LINALG::Matrix<3, 1>& x,
const LINALG::Matrix<3, 1>& y,
LINALG::Matrix<3, 3>& du_tensor_du,
...
/*-------------------------------------------------------------------*
 ...
 | and r_unit tensorproduct r_unit |
 *--------------------------------------------------------------------*/
void POTENTIAL::Potential::computeDistance(const LINALG::Matrix<2, 1>& x,
const LINALG::Matrix<2, 1>& y,
LINALG::Matrix<2, 2>& du_tensor_du,
    ...
    std::vector<int>& lmcol)
{
  for (std::map<int, std::set<int> >::const_iterator labelIter =
      potentialElementIds.begin(); labelIter != potentialElementIds.end();
      labelIter++)
    ...
    std::vector<int>& lmcol)
{
  ...
  for (std::map<int, std::vector<PotentialElementContainer> >::iterator labelIter =
      nonlocalPecs.begin(); labelIter != nonlocalPecs.end(); labelIter++)
    for (std::vector<PotentialElementContainer>::iterator pecIter =
        (labelIter->second).begin(); pecIter != (labelIter->second).end();
        pecIter++)
    {
    ...
    std::vector<int> & lmcol)
{
  ...
  for (std::map<int, std::map<int, GEO::NearestObject> >::const_iterator gpIter =
      potentialObjectsAtGP.begin(); gpIter != potentialObjectsAtGP.end();
      gpIter++)
    for (std::map<int, GEO::NearestObject>::const_iterator labelIter =
        (gpIter->second).begin(); labelIter != (gpIter->second).end();
        labelIter++)
    {
 ...
 *--------------------------------------------------------------------*/
int POTENTIAL::Potential::GetLocalIndex(std::vector<int>& lmcol, int value)
{
  int localindex = -1;
 ...
 *----------------------------------------------------------------------*/
void POTENTIAL::Potential::ReferenceConfiguration(const DRT::Element* element,
    Epetra_SerialDenseMatrix& X, const int numdim) const
{
...
Epetra_SerialDenseMatrix POTENTIAL::Potential::ReferenceConfiguration(
    const DRT::Element* element, const int numdim) const
{
  const int numnode = element->NumNode();
 ...
 *----------------------------------------------------------------------*/
double POTENTIAL::Potential::GetTimeCurveFactor(Teuchos::ParameterList& params)
{
  Teuchos::RCP<DRT::Condition> cond = params.get<Teuchos::RCP<DRT::Condition> >(
      "condition", Teuchos::null);
    ...
    std::map<int, int>& labelByElements)
{
  ...
  for (std::map<int, std::set<int> >::const_iterator conditer =
      elementsByLabel.begin(); conditer != elementsByLabel.end(); ++conditer)
  {
    const int potentiallabel = conditer->first;
    for (std::set<int>::const_iterator eleiditer = conditer->second.begin();
        eleiditer != conditer->second.end(); ++eleiditer)
    {
      const int eleid = *eleiditer;
      if (labelByElements.count(eleid) == 1)
        dserror(
            "Assumption violation: there should be exactly ONE potential condition per element id!");
 ...
 *----------------------------------------------------------------------*/
double POTENTIAL::Potential::GetAtomicDensity(const int elementId,
    const std::string& conditionName, const std::map<int, int>& labelByElements)
{
  ...
  for (std::vector<DRT::Condition*>::iterator condIter = potentialcond.begin();
      condIter != potentialcond.end(); ++condIter)
    ...
    const double n_offset)
{
      ...
      condIter != potentialcond.end(); ++condIter)
  {
    ...
    if (label == 0 || label == 1)
    {
      ...
      vol_sphere_local[label] = computeLocalForceAndCOG(potentialsurfdis,
          fpot[label], cog[label], fint, disp,
  ...
  computeGlobalForceAndCOG(vol_sphere_local[0], fpot[0], cog[0], fpot_global[0],
      cog_global[0]);
  std::cout << "force sphere 1" << std::endl;
  computeGlobalForceAndCOG(vol_sphere_local[1], fpot[1], cog[1], fpot_global[1],
      cog_global[1]);
  ...
  if (discretRCP_->Comm().MyPID() == 0)
  {
    ...
    const double A_ham = M_PI * M_PI * (beta - n_offset) * (beta - n_offset)
        * lambda;
    const double force_analytical = (-1.0) * (A_ham / (2.0 * radius * 6.0))
        * (((2.0 * (x + 1.0)) / (x * x + 2.0 * x))
            - ((x + 1.0) / pow((x * x + 2.0 * x), 2)) - (2.0 / (x + 1.0))
            - (1.0 / pow((x + 1.0), 3)));

    std::cout << std::endl << "The analytical sphere solution = "
        << force_analytical << std::endl;
    // write output
    ...
    const double thickness)
{
  ...
  if (discretRCP_->Comm().MyPID() == 0)
  {
    ...
    std::cout
        << "Analytical volume = "
        << 4.0 * M_PI * pow(radius, 3) / 3.0
            - 4.0 * M_PI * pow((radius - thickness), 3) / 3.0 << std::endl;
    //double globalv = 63527.0;
    // const double radius_test = std::pow( ((3*globalv)/(4*M_PI) ) ,(1.0/3.0));
    //std::cout << "radius_test = " << radius_test << std::endl;
    const double d = distance - 2.0 * radius;
    ...
    const double force_i1o2 = (-1.0) * (A_ham / (2.0 * radius_i * 12.0))
        * (((2.0 * (2.0 * x_io + (y_io + 1.0)))
            / (x_io * x_io + x_io * (y_io + 1.0)))
            - ((2.0 * (2.0 * x_io + (y_io + 1.0)))
                / (x_io * x_io + x_io * (y_io + 1.0) + y_io))
            - ((y_io * (2.0 * x_io + (y_io + 1.0)))
                / pow((x_io * x_io + x_io * (y_io + 1.0) + y_io), 2))
            - ((y_io * (2.0 * x_io + (y_io + 1.0)))
                / pow((x_io * x_io + x_io * (y_io + 1.0)), 2)));
    ...
    // total force
    const double force_analytical = force_o1o2 - force_i1o2 - force_o1i2
        + force_i1i2;

    std::cout << "The hamaker constant = " << A_ham << std::endl;
    std::cout << std::endl << "The analytical membrane solution = "
        << force_analytical << std::endl;
    // write output
    std::cout << "distance = " << distance << std::endl;
    std::cout << "force sphere 1 = " << force1 << std::endl;
    ...
    const std::set<int>& surfElementIds)
{
  ...
  for (int i_rowele = 0; i_rowele < discretRCP_->NumMyRowElements(); i_rowele++)
  {
    ...
    if (elementIds.find(element->Id()) == elementIds.end()
        || !(discretRCP_->HaveGlobalElement(element->Id())))
      continue;
  ...
  {
    for (int i_rowele = 0; i_rowele < potentialdis->NumMyRowElements();
        i_rowele++)
    {
      const DRT::Element* element = potentialdis->lRowElement(i_rowele);
      if (surfElementIds.find(element->Id()) == surfElementIds.end()
          || !(potentialdis->HaveGlobalElement(element->Id())))
        continue;
    ...
    for (int i_rownode = 0; i_rownode < potentialdis->NumMyRowNodes();
        i_rownode++)
    {
      ...
      for (int dim = 0; dim < 3; dim++)
      {
        ...
        if (dim == 0)
          std::cout << " f = " << (*fint)[local_dofId] << "     dof = "
              << local_dofId << std::endl;
      }
    }
  }
  else
  {
    ...
    for (int i_rownode = 0; i_rownode < discretRCP_->NumMyRowNodes();
        i_rownode++)
    {
 ...
 *----------------------------------------------------------------------*/
double POTENTIAL::Potential::computeGlobalForceAndCOG(double vol_sphere_local,
    ...
    LINALG::Matrix<3, 1>& cog_sphere_global)
{
  ...
  (discretRCP_->Comm()).SumAll(&fpot_sphere_local(0), &fpot_sphere_global(0),
      1);
  (discretRCP_->Comm()).SumAll(&fpot_sphere_local(1), &fpot_sphere_global(1),
      1);
  (discretRCP_->Comm()).SumAll(&fpot_sphere_local(2), &fpot_sphere_global(2),
      1);
...
void POTENTIAL::Potential::getPhysicalEleCoords(
    ...
    Teuchos::RCP<const Epetra_Vector> idisp_solid, const DRT::Element* element,
 ...
 *----------------------------------------------------------------------*/
void POTENTIAL::Potential::WriteTestOutput(const double distance,
    const double force1, const double force2, const double force_analytical,
    const double time, const int step, const std::string name)
{
  if (discret_.Comm().MyPID() == 0)
  {
    ...
    if (step < 1)
    {
      ...
      file << "Time\t\t\tDistance\t\tForce_numerical\t\t\tForce_analytical"
          << std::endl;
      file << time << "\t\t\t" << distance << "\t\t\t" << force1 << "\t\t\t"
          << force_analytical << std::endl;
      file.close();
    }
    else
    {
      ...
      while (!file_in.eof())
      {
        ...
        if ((found_time == std::string::npos || found_time > found_tab)
            && line.length() > 0)
          out << line << std::endl;
      }
