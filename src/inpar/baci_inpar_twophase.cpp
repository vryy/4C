...
void INPAR::TWOPHASE::SetValidParameters(Teuchos::RCP<Teuchos::ParameterList> list)
{
  ...
  BoolParameter(
      "RESTART_SCATRA_INPUT", "No", "Use ScaTra field from .dat-file instead", &twophasedyn);

  Teuchos::ParameterList& twophasedyn_smeared = twophasedyn.sublist("SMEARED", false, "");
  DoubleParameter("INTERFACE_THICKNESS", 0.0, "Thickness of interface for multiphase flow",
      &twophasedyn_smeared);
  BoolParameter("ENHANCED_GAUSSRULE", "No",
      "Set higher order gaussrule within the interface layer.", &twophasedyn_smeared);


  Teuchos::ParameterList& twophase_surftens = twophasedyn.sublist("SURFACE TENSION", false, "");
  setStringToIntegralParameter<int>("SURFTENSAPPROX", "surface_tension_approx_none",
  ...
  setStringToIntegralParameter<int>("SMOOTHGRADPHI", "smooth_grad_phi_l2_projection",
      "Type of smoothing for grad(phi)",
      tuple<std::string>(
          //"smooth_grad_phi_none",
          "smooth_grad_phi_meanvalue", "smooth_grad_phi_l2_projection",
  ...
  BoolParameter("L2_PROJECTION_SECOND_DERIVATIVES", "No",
      "L2 Projection Second Derivatives of Level Set", &twophase_surftens);

  BoolParameter("SCALE_SMOOTHED_GRADIENTS", "No", "Scale the smoothed normal to unit length",
      &twophase_surftens);

  setStringToIntegralParameter<int>("NODAL_CURVATURE", "l2_projected",
      "Type of calculation of nodal curvature value",
      tuple<std::string>("l2_projected", "averaged"), tuple<int>(l2_projected, averaged),
      &twophase_surftens);

  setStringToIntegralParameter<int>("LAPLACE_BELTRAMI", "matrix_mixed_smoothed",
      "Type of calculation of Laplace-Beltrami projection matrix",
      tuple<std::string>("matrix_non_smoothed", "matrix_smoothed", "matrix_mixed_smoothed"),
      tuple<int>(matrix_non_smoothed, matrix_smoothed, matrix_mixed_smoothed), &twophase_surftens);

  DoubleParameter("SMOOTHING_PARAMETER", 0.0, "Diffusion Coefficient for Smoothing",
      &twophase_surftens);  // Added diffusion for L2_projection
}
