...
void INPAR::FSI::SetValidParameters(Teuchos::RCP<Teuchos::ParameterList> list)
{
  ...
  name[14] = "pseudo_structure";
  label[14] = fsi_pseudo_structureale;
  name[15] = "iter_constr_monolithicfluidsplit";
  label[15] = fsi_iter_constr_monolithicfluidsplit;
  name[16] = "iter_constr_monolithicstructuresplit";
  label[16] = fsi_iter_constr_monolithicstructuresplit;
  name[17] = "iter_mortar_monolithicstructuresplit";
  label[17] = fsi_iter_mortar_monolithicstructuresplit;
  name[18] = "iter_mortar_monolithicfluidsplit";
  label[18] = fsi_iter_mortar_monolithicfluidsplit;
  name[19] = "iter_fluidfluid_monolithicstructuresplit";
  label[19] = fsi_iter_fluidfluid_monolithicstructuresplit;
  name[20] = "iter_fluidfluid_monolithicfluidsplit";
  label[20] = fsi_iter_fluidfluid_monolithicfluidsplit;
  name[21] = "iter_fluidfluid_monolithicstructuresplit_nonox";
  label[21] = fsi_iter_fluidfluid_monolithicstructuresplit_nonox;
  name[22] = "iter_fluidfluid_monolithicfluidsplit_nonox";
  label[22] = fsi_iter_fluidfluid_monolithicfluidsplit_nonox;
  name[23] = "iter_sliding_monolithicfluidsplit";
  label[23] = fsi_iter_sliding_monolithicfluidsplit;
  name[24] = "iter_sliding_monolithicstructuresplit";
  label[24] = fsi_iter_sliding_monolithicstructuresplit;
  ...
  BoolParameter("MATCHGRID_STRUCTALE", "Yes", "is matching grid (structure-ale)", &fsidyn);

  BoolParameter("MATCHALL", "Yes",
  ...
  DoubleParameter("SIZERATIOMAX", 2.0,
      "Limit maximally permitted change of\n"
      "time step size compared to previous size (>0).",
      &fsiadapt);
  DoubleParameter("SIZERATIOMIN", 0.5,
      "Limit minimally permitted change of\n"
      "time step size compared to previous size (>0).",
      &fsiadapt);

  BoolParameter("TIMEADAPTON", "No", "Activate or deactivate time step size adaptivity", &fsiadapt);
  ...
  DoubleParameter("CONVTOL", 1e-6, "Nonlinear tolerance for lung/constraint/fluid-fluid FSI",
      &fsimono);  // ToDo remove

  BoolParameter("ENERGYFILE", "No",
      "Write artificial interface energy due to temporal discretization to file", &fsimono);

  BoolParameter("FSIAMGANALYZE", "No", "run analysis on fsiamg multigrid scheme", &fsimono);
  ...
  IntParameter("HYBRID_FILL_LEVEL", 0, "Level of fill of the hybrid ILU preconditioner.", &fsimono);
  ...
  IntParameter("KRYLOV_ITEMAX", 1000, "Max Iterations for linear solver.", &fsimono);

  IntParameter("KRYLOV_SIZE", 50, "Size of Krylov Subspace.", &fsimono);
    ...
    std::vector<Teuchos::RCP<DRT::INPUT::ConditionDefinition>>& condlist)
{
  ...
  Teuchos::RCP<ConditionDefinition> linefsi =
      Teuchos::rcp(new ConditionDefinition("DESIGN FSI COUPLING LINE CONDITIONS", "FSICoupling",
          "FSI Coupling", DRT::Condition::FSICoupling, true, DRT::Condition::Line));
  Teuchos::RCP<ConditionDefinition> surffsi =
      Teuchos::rcp(new ConditionDefinition("DESIGN FSI COUPLING SURF CONDITIONS", "FSICoupling",
          "FSI Coupling", DRT::Condition::FSICoupling, true, DRT::Condition::Surface));
  ...
  Teuchos::RCP<ConditionDefinition> linefsins = Teuchos::rcp(
      new ConditionDefinition("DESIGN FSI COUPLING NO SLIDE LINE CONDITIONS", "FSICouplingNoSlide",
          "FSI Coupling No Slide", DRT::Condition::FSICouplingNoSlide, true, DRT::Condition::Line));
  Teuchos::RCP<ConditionDefinition> surffsins = Teuchos::rcp(new ConditionDefinition(
      "DESIGN FSI COUPLING NO SLIDE SURF CONDITIONS", "FSICouplingNoSlide", "FSI Coupling No Slide",
  ...
  Teuchos::RCP<ConditionDefinition> surffsicd =
      Teuchos::rcp(new ConditionDefinition("DESIGN FSI COUPLING CENTER DISP SURF CONDITIONS",
          "FSICouplingCenterDisp", "FSI Coupling Center Disp",
          DRT::Condition::FSICouplingCenterDisp, true, DRT::Condition::Surface));
  ...
  Teuchos::RCP<ConditionDefinition> surfsac = Teuchos::rcp(
      new ConditionDefinition("DESIGN STRUCTURE ALE COUPLING SURF CONDITIONS", "StructAleCoupling",
          "StructAleCoupling", DRT::Condition::StructAleCoupling, true, DRT::Condition::Surface));
  ...
  Teuchos::RCP<ConditionDefinition> surfsfv =
      Teuchos::rcp(new ConditionDefinition("DESIGN STRUCTURE FLUID VOLUME COUPLING SURF CONDITIONS",
          ...
          DRT::Condition::StructFluidSurfCoupling, true, DRT::Condition::Surface));
  Teuchos::RCP<ConditionDefinition> volsfv =
      Teuchos::rcp(new ConditionDefinition("DESIGN STRUCTURE FLUID VOLUME COUPLING VOL CONDITIONS",
          ...
          DRT::Condition::StructFluidVolCoupling, false, DRT::Condition::Volume));
