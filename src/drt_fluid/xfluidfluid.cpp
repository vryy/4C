    ...
    wizard_( *xfluid.discret_, *xfluid.boundarydis_ )
{
  ...
  if (true)
  {
    FLD::UTILS::SetupFluidSplit(*xfluid.soliddis_,xfluid.numdim_,alevelpressplitter_);

    const Epetra_Map* aledofrowmap = xfluid.soliddis_->DofRowMap();

    alesysmat_ = Teuchos::rcp(new LINALG::SparseMatrix(*aledofrowmap,108,false,true));

    // Vectors passed to the element
    // -----------------------------
    // velocity/pressure at time n+1, n and n-1
    alevelnp_ = LINALG::CreateVector(*aledofrowmap,true);
    aleveln_  = LINALG::CreateVector(*aledofrowmap,true);
    alevelnm_ = LINALG::CreateVector(*aledofrowmap,true);

    aledispnp_ = LINALG::CreateVector(*aledofrowmap,true);
    aledispn_  = LINALG::CreateVector(*aledofrowmap,true);
    aledispnm_ = LINALG::CreateVector(*aledofrowmap,true);
    alegridv_  = LINALG::CreateVector(*aledofrowmap,true);

    // acceleration/(scalar time derivative) at time n+1 and n
    aleaccnp_ = LINALG::CreateVector(*aledofrowmap,true);
    aleaccn_  = LINALG::CreateVector(*aledofrowmap,true);

    // velocity/pressure at time n+alpha_F
    alevelaf_ = LINALG::CreateVector(*aledofrowmap,true);

    // rhs: standard (stabilized) residual vector (rhs for the incremental form)
    aleresidual_     = LINALG::CreateVector(*aledofrowmap,true);
    aletrueresidual_ = LINALG::CreateVector(*aledofrowmap,true);

    // acceleration/(scalar time derivative) at time n+alpha_M/(n+alpha_M/n)
    aleaccam_ = LINALG::CreateVector(*aledofrowmap,true);

    // scalar at time n+alpha_F/n+1 and n+alpha_M/n
    // (only required for low-Mach-number case)
    alescaaf_ = LINALG::CreateVector(*aledofrowmap,true);
    alescaam_ = LINALG::CreateVector(*aledofrowmap,true);

    // history vector
    alehist_ = LINALG::CreateVector(*aledofrowmap,true);

    // right hand side vector for linearised solution;
    alerhs_ = LINALG::CreateVector(*aledofrowmap,true);

    // Nonlinear iteration increment vector
    aleincvel_ = LINALG::CreateVector(*aledofrowmap,true);

    // a vector of zeros to be used to enforce zero dirichlet boundary conditions
    alezeros_   = LINALG::CreateVector(*aledofrowmap,true);
...

    {
      ParameterList eleparams;
      // other parameters needed by the elements
      xfluid.soliddis_->EvaluateDirichlet(eleparams, alezeros_, Teuchos::null, Teuchos::null,
    ...
    RCP<Epetra_Map> fluiddofrowmap = rcp(new Epetra_Map(*xfluid.discret_->DofRowMap()));
    RCP<Epetra_Map> alefluiddofrowmap = rcp(new Epetra_Map(*xfluid.soliddis_->DofRowMap()));
    ...
    FLD::UTILS::SetupFluidFluidVelPresSplit(*xfluid.discret_,xfluid.numdim_,*xfluid.soliddis_,fluidfluidvelpressplitter_,
                                           fluidfluiddofrowmap);
    ...
    // -------------------------------------------------------
    const Epetra_Map* boundarydofrowmap = xfluid.boundarydis_->DofRowMap();
...
// -------------------------------------------------------------------
// -------------------------------------------------------------------
void FLD::XFluidFluid::XFluidFluidState::Evaluate( Teuchos::ParameterList & eleparams,
...
//       strategy.AssembleVector3(la[0].lm_,la[0].lmowner_);
    ...
    for (int i=0; i<numcolaleele; ++i)
    {
      ...
      {
        TEUCHOS_FUNC_TIME_MONITOR( "FLD::XFluidFluid::XFluidFluidState::Evaluate normal" );
        cout << "gevaluate normal ALE" << endl;
    ...
    step_(0)
{
  ...
  if (boundarydis_->NumGlobalNodes() == 0)
  {
    ...
    boundarydis_ = DRT::UTILS::CreateDiscretizationFromCondition(soliddis, "XFEMCoupling", "boundary", "BELE3", conditions_to_copy);
  }
...

  {
    ...
    IO::GMSH::disToStream("embeddedFluid", 0.0, soliddis_,gmshfilecontent);
...
void FLD::XFluidFluid::SolveStationaryProblemFluidFluid()
{
  ...
  while (step_< stepmax_)
  {
    ...
    // -------------------------------------------------------------------
    {
      ...
      soliddis_->SetState("velaf",state_->alevelnp_);
      soliddis_->EvaluateDirichlet(eleparams,state_->alevelnp_,null,null,null);
      soliddis_->ClearState();
...
void FLD::XFluidFluid::NonlinearSolveFluidFluid()
{
  ...
  while (stopnonliniter==false)
  {
    ...
    // -------------------------------------------------------------------
    {
      ...
      state_->EvaluateFluidFluid( eleparams, *discret_, *boundarydis_, *soliddis_,  itnum );
