...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void FLD::UTILS::FluidInfNormScaling::ScaleSystem(Teuchos::RCP<LINALG::SparseOperator> matrix, Epetra_Vector& b)
{
  ...
  if (matrcp != Teuchos::null) // yes, we have a block sparse matrix
  {
    ...
    if (leftscale_momentum_)
    {
      ...
      Teuchos::RCP<Epetra_Vector> temp1 = Teuchos::rcp(new Epetra_Vector(mat.Matrix(0,0).EpetraMatrix()->RowMap(),false));
    ...
    if (A00->LeftScale(*srowsum_) or
        A00->RightScale(*scolsum_) or
    ...
    if (px->Multiply(1.0, *prowsum_, *px, 0.0))
      dserror("fluid scaling failed");
  ...
  {
    Teuchos::RCP<LINALG::SparseMatrix> smat = Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(matrix);
    if (smat == Teuchos::null)
    ...
    if (smat->LeftScale(*srowsum_))
      dserror("fluid scaling failed");
    if (b.Multiply(1.0, *srowsum_, b, 0.0))
      dserror("fluid scaling failed");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void FLD::UTILS::FluidInfNormScaling::UnscaleSolution(Teuchos::RCP<LINALG::SparseOperator> matrix, Epetra_Vector& x, Epetra_Vector& b)
{
  ...
  if (matrcp != Teuchos::null) // yes, we have a block sparse matrix
  {
    ...
    prowsum_->Reciprocal(*prowsum_);
    if (A11->LeftScale(*prowsum_) or
        mat.Matrix(1,0).EpetraMatrix()->LeftScale(*prowsum_))
      dserror("fluid scaling failed");

  }
  else
  {
    if (x.Multiply(1.0, *scolsum_, x, 0.0))
      dserror("fluid unscaling failed");
