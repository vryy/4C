 ...
 *----------------------------------------------------------------------*/
void FLD::TimIntTwoPhaseOst::SetIterScalarFieldsn(
   Teuchos::RCP<const Epetra_Vector> curvaturen,
   ...
   Teuchos::RCP<DRT::Discretization> scatradis)
{
    ...
    for(int lnodeid=0;lnodeid<discret_->NumMyRowNodes();lnodeid++)
    {
      ...
      if (err != 0) dserror("error while inserting value into tpf_gradphi_curvn_, for curvature entry");
      ...
      for(int ndimcounter=0; ndimcounter<3; ndimcounter++)
      {
        ...
        if (err != 0) dserror("error while inserting value into tpf_gradphi_curvn_, for gradphi entry");
      }
