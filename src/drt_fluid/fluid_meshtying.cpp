    ...
    std::vector<Teuchos::RCP<Epetra_Vector> >&         splitvel)
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> kms = MLMultiply(*P,true,*(splitmatrix[8]),false,false,false,true);
...
/*                                          wirtz 11/14  */
/*-------------------------------------------------------*/
Teuchos::RCP<LINALG::BlockSparseMatrixBase> FLD::Meshtying::MshtSplit()
{
  // generate map for blockmatrix
  std::vector<Teuchos::RCP<const Epetra_Map> > fluidmaps;
  fluidmaps.push_back(gndofrowmap_);
...
/*                                          wirtz 11/14  */
/*-------------------------------------------------------*/
void FLD::Meshtying::CondensationShapeDerivatives(Teuchos::RCP<LINALG::BlockSparseMatrixBase>& shapederivatives)
{
  TEUCHOS_FUNC_TIME_MONITOR("Meshtying:  2.1)   - Condensation Operation");
