...
/*------------------------------------------------------------------------------------------------------------------------------*
 | split sparse global system matrix into 3x3 block sparse matrix associated with interior, master, and slave dofs   fang 08/15 |
 *------------------------------------------------------------------------------------------------------------------------------*/
void FLD::Meshtying::SplitMatrix(
    Teuchos::RCP<LINALG::SparseOperator>           matrix,       //!< original sparse global system matrix before split
    Teuchos::RCP<LINALG::BlockSparseMatrixBase>&   splitmatrix   //!< resulting block sparse matrix after split
    )
{
  ...
  // | ksn | ksm | kss |
  // -------------------
  splitmatrix = Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(matrix)->Split<LINALG::DefaultBlockMatrixStrategy>(extractor,extractor);
...
void FLD::Meshtying::CondensationOperationSparseMatrix(
    const Teuchos::RCP<LINALG::SparseOperator>&              sysmat,       ///> sysmat established by the element routine
    const Teuchos::RCP<Epetra_Vector>&                       residual,     ///> residual established by the element routine
    const Teuchos::RCP<LINALG::BlockSparseMatrixBase>&       splitmatrix,  ///> container with split original sysmat
    const std::vector<Teuchos::RCP<Epetra_Vector> >&         splitres,     ///> container with split original residual
    const std::vector<Teuchos::RCP<Epetra_Vector> >&         splitvel      ///> container with split velocity vector
    )
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> knm_add = MLMultiply(splitmatrix->Matrix(0,2),false,*P,false,false,false,true);
  ...
  Teuchos::RCP<LINALG::SparseMatrix> kmn_add = MLMultiply(*P,true,splitmatrix->Matrix(2,0),false,false,false,true);
  ...
  Teuchos::RCP<LINALG::SparseMatrix> kms = MLMultiply(*P,true,splitmatrix->Matrix(2,2),false,false,false,true);
  Teuchos::RCP<LINALG::SparseMatrix> kmm_add = MLMultiply(*kms,false,*P,false,false,false,true);
  ...
  Teuchos::RCP<LINALG::SparseOperator> sysmatnew = Teuchos::rcp(new LINALG::SparseMatrix(*dofrowmapsysmatnew->Matrix(0,2)_,81,true,false));
  ...
  // r_n: add K_ns*vp_i^s
  Teuchos::RCP<LINALG::SparseMatrix> knm = Teuchos::rcp(new LINALG::SparseMatrix(splitmatrix->Matrix(0,2)));
...
/*                                                       */
/*-------------------------------------------------------*/
void FLD::Meshtying::IsMultifield(Teuchos::RCP<std::set<int> > condelements,           ///< conditioned elements of fluid
  ...
  if (is_multifield_)
  {
    ...
    if (multifield_splitmatrix_)
    {
      Teuchos::RCP<LINALG::MapExtractor> extractor = Teuchos::rcp(
          new LINALG::MapExtractor(*multifield_domainmaps_.FullMap(),
              multifield_domainmaps_.Map(1)));
      Teuchos::RCP<LINALG::BlockSparseMatrix<FLD::UTILS::InterfaceSplitStrategy> > mat =
          mergedmatrix->Split<FLD::UTILS::InterfaceSplitStrategy>(*extractor,
              *extractor);
...
/*-------------------------------------------------------*/
void FLD::Meshtying::MultifieldSplitShape(Teuchos::RCP<LINALG::BlockSparseMatrixBase>& shapederivatives)
{

  if (is_multifield_)
  {
    Teuchos::RCP<Epetra_Vector> ones = Teuchos::rcp(
        new Epetra_Vector(shapederivatives->Matrix(2, 2).RowMap()));
    ones->PutScalar(1.0);
    Teuchos::RCP<LINALG::SparseMatrix> onesdiag = Teuchos::rcp(
        new LINALG::SparseMatrix(*ones));
    onesdiag->Complete();
    ...
    Teuchos::RCP<LINALG::MapExtractor> extractor = Teuchos::rcp(new LINALG::MapExtractor(*multifield_domainmaps_shape_.FullMap(),multifield_domainmaps_shape_.Map(1)));
    Teuchos::RCP<LINALG::BlockSparseMatrix<FLD::UTILS::InterfaceSplitStrategy> > matshapederivatives = mergedshapederivatives->Split<FLD::UTILS::InterfaceSplitStrategy>(*extractor,*extractor);
...
/*                                           wirtz 01/16 */
/*-------------------------------------------------------*/
void FLD::Meshtying::CondensationOperationBlockMatrixShape(Teuchos::RCP<LINALG::BlockSparseMatrixBase>& shapederivatives)
{
  TEUCHOS_FUNC_TIME_MONITOR("Meshtying:  2.1)   - Condensation Operation");
  ...
  /*--------------------------------------------------------------------*/
  // compute modification for block nm
  Teuchos::RCP<LINALG::SparseMatrix> knm_mod = MLMultiply(shapederivatives->Matrix(0,2),false,*P,false,false,false,true);
  ...
  /*--------------------------------------------------------------------*/
  // compute modification for block kmn
  Teuchos::RCP<LINALG::SparseMatrix> kmn_mod = MLMultiply(*P,true,shapederivatives->Matrix(2,0),false,false,false,true);
  ...
  /*--------------------------------------------------------------------*/
  // compute modification for block kmm
  Teuchos::RCP<LINALG::SparseMatrix> kss_mod = MLMultiply(*P,true,shapederivatives->Matrix(2,2),false,false,false,true);
  Teuchos::RCP<LINALG::SparseMatrix> kmm_mod = MLMultiply(*kss_mod,false,*P,false,false,false,true);
