  ...
  output_ (output)
{
  ...
  // ---------------------------------------------------------------------
  vector<DRT::Condition*> couplingcond;
  discret3D_->GetCondition("Art_3D_redD_CouplingCond",couplingcond);

  vector<DRT::Condition*> couplingcond2;
...
//    params->set("reducedD map of values", mapRed_Dnp_);
  ...
  for (unsigned int i = 0; i< conds3D.size(); i++)
  {
    ...
    string ReturnedVariable = *(conds_redD[i]->Get<string>("ReturnedVariable"));
...
/*!
*/
void FLD::UTILS::Fluid_couplingBc::UpdateResidual(RCP<Epetra_Vector>  residual )
{
...
/*!
*/
void FLD::UTILS::Fluid_couplingBc::EvaluateDirichlet(RCP<Epetra_Vector>    velnp ,
                                                     const Epetra_Map & condmap,
                                                     double                time)
{
  return;
