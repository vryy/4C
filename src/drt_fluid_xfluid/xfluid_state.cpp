...
//  std::cout << "number of nonzeros: C_xs" << C_xs_->EpetraMatrix()->MaxNumEntries() << std::endl;
//  std::cout << "number of nonzeros: C_sx" << C_sx_->EpetraMatrix()->MaxNumEntries() << std::endl;
//  std::cout << "number of nonzeros: C_ss" << C_ss_->EpetraMatrix()->MaxNumEntries() << std::endl;
  //-------------------------------------------------------------------------------
...
void FLD::XFluidState::InitSystemMatrix()
{
  ...
  // REMARK: call the SparseMatrix: * explicitdirichlet = false (is used in ApplyDirichlet, true would create a new matrix when DBS will be applied)
  //                                    setting flag to false can cause problems with ML preconditioner (see remark in LINALG::Sparsematrix)
  //                                    however, we prefer not to build new matrices in ApplyDirichlet
  ...
  sysmat_ = Teuchos::rcp(new LINALG::SparseMatrix(*xfluiddofrowmap_,162,false,true,LINALG::SparseMatrix::FE_MATRIX));
}
...
void FLD::XFluidState::InitCouplingMatricesAndRhs()
{
  ...
  for(int coup_idx=0; coup_idx<condition_manager_->NumCoupling(); coup_idx++)
  {
    ...
    if(!condition_manager_->IsCouplingCondition(coupling->GetName())) // coupling or one-sided non-coupling object
    {
    ...
    else
    {
      if(condition_manager_->IsLevelSetCondition(coup_idx))
      {
        ...
        Teuchos::RCP<LINALG::SparseMatrix> sysmat_weakRCP       = sysmat_.create_weak();   // no increment in strong reference counter
        ...
        coup_state = Teuchos::rcp(new XFluidState::CouplingState(sysmat_weakRCP,sysmat_weakRCP,sysmat_weakRCP,residual_weakRCP,residual_col_weakRCP));
      }
 ...
 *----------------------------------------------------------------------*/
void FLD::XFluidState::InitALEStateVectors(
...
)
{
  ...
  for(std::map<int, Teuchos::RCP<CouplingState> >::iterator cs = coup_state_.begin(); cs!=coup_state_.end(); cs++)
  {
    ...
    // complete only the mesh coupling objects, levelset-couplings are completed by the sysmat->Complete call
    if(condition_manager_->IsMeshCondition(coupl_idx))
    {
      ...
      cs->second->CompleteCouplingMatricesAndRhs(*fluiddofrowmap, *coupling->GetCondDis()->DofRowMap()); // complete w.r.t
    }
  }
}
...
bool FLD::XFluidState::Destroy()
{
      ...
      ++i)
  {
    ...
    if(cs != Teuchos::null)
    {
      cs->Destroy(true);  // throw exception when object could not be deleted or more than one strong RCP points to it
