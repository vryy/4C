...
Converter::Converter(PostField* field)
  : field_(field)
{
  s8dis_ = field->discretization();
  h8dis_ = Teuchos::rcp(new DRT::Discretization("structure",
                                    ...
                                    PostResult* result)
{
  ...
  for (int i=0; i<numnodes; ++i)
  {
    ...
    {
      (*h8data)[h8map.LID(lh8dof[i])] = (*s8data)[s8map.LID(s8dof[i])] - (*s8data)[s8map.LID(s8dof[i+3])];
      (*h8data)[h8map.LID(uh8dof[i])] = (*s8data)[s8map.LID(s8dof[i])] + (*s8data)[s8map.LID(s8dof[i+3])];
    }
  }
