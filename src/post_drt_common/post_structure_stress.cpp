...
{
  WriteNodalStressStep (StructureFilter &filter)
  :
    filter_(filter)
  {}
                           ...
                           const std::vector<std::string> &name)
  {
    ...
    filter_.GetWriter().WriteNodalResultStep(*files[0], nodal_stress, resultfilepos, groupname, name[0], 6);
  }
...
{
  WriteElementCenterStressStep (StructureFilter &filter)
  :
    filter_(filter)
  {}
                           ...
                           const std::vector<std::string> &name)
  {
    ...
    filter_.GetWriter().WriteElementResultStep(*files[0], elestress, resultfilepos, groupname, name[0], 6, 0);
  }
...
{
  WriteNodalEigenStressStep (StructureFilter &filter)
  :
    filter_(filter)
  {}
                           ...
                           const std::vector<std::string> &name)
  {
    ...
    for (int i=0; i<3; ++i)
      filter_.GetWriter().WriteNodalResultStep(*files[i], nodal_eigen_val_vec[i], resultfilepos, groupname, name[i], 1);
    for (int i=3; i<6; ++i)
      filter_.GetWriter().WriteNodalResultStep(*files[i], nodal_eigen_val_vec[i], resultfilepos, groupname, name[i], 3);
  }
...
{
  WriteElementCenterEigenStressStep (StructureFilter &filter)
  :
    filter_(filter)
  {}
                           ...
                           const std::vector<std::string> &name)
  {
    ...
    for (int i=0; i<3; ++i)
      filter_.GetWriter().WriteElementResultStep(*files[i], nodal_eigen_val_vec[i], resultfilepos, groupname, name[i], 1, 0);
    for (int i=3; i<6; ++i)
      filter_.GetWriter().WriteElementResultStep(*files[i], nodal_eigen_val_vec[i], resultfilepos, groupname, name[i], 3, 0);
  }
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void StructureFilter::WriteEigenStress(const std::string groupname,
                                       PostResult& result,
                                       const ResultType stresskind)
{
