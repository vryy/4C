 ...
 *----------------------------------------------------------------------*/
void ThermoFilter::PostHeatflux(
...
{
  WriteNodalHeatfluxStep (ThermoFilter &filter)
  :
    filter_(filter)
  {}
  ...
  int Numdf()
  {
    ...
    else if (filter_.GetWriter().GetField()->problem()->num_dim() == 2) numdf = 2;
    else if (filter_.GetWriter().GetField()->problem()->num_dim() == 1) numdf = 1;
    else dserror("Cannot handle dimension %g", filter_.GetWriter().GetField()->problem()->num_dim());
    return numdf;
  }

  virtual std::vector<int> NumDfMap()
  {
    return std::vector<int>(1, Numdf());
  }

  virtual void operator()(
      std::vector<Teuchos::RCP<std::ofstream> >& files,
      PostResult& result,
      ...
      const std::vector<std::string> &name)
  {
    ...
    filter_.GetWriter().WriteNodalResultStep(*files[0], nodal_heatfluxes, resultfilepos, groupname, name[0], numdf);
  }
...
{
  WriteElementCenterHeatfluxStep (ThermoFilter &filter)
  :
    filter_(filter)
  {}
      ...
      const std::vector<std::string> &name)
  {
    ...
    filter_.GetWriter().WriteElementResultStep(*files[0], eleheatflux, resultfilepos, groupname, name[0], numdf, 0);
  }
  ...
  )
{
  ...
  if (kind == nodebased)
  {
    ...
    writer_->WriteSpecialField(heatflux, result, nodebased, groupname,
  ...
  else if (kind == nodebased)
  {
    ...
    writer_->WriteSpecialField(heatflux, result, elementbased, groupname,
                               std::vector<std::string>(1,name), out);
  }
  else
