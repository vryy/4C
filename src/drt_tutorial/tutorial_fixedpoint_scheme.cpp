...
TUTORIAL::FixedPointScheme::FixedPointScheme()
{
  ...
  convtol_=DRT::Problem::Instance()->TutorialParams().sublist("FIXED POINT SCHEME").get<double>("CONVTOL");
  omega_=DRT::Problem::Instance()->TutorialParams().sublist("FIXED POINT SCHEME").get<double>("RELAX_PARAMETER");
...
void TUTORIAL::FixedPointScheme::TimeLoop()
{
  ...
  while(Time()<=MaxTime())
  {
    ...
    std::cout<<"\nTIMESTEP "<<step<<"/"<<MaxTime()/Dt()<<" time="<< Time()<<"/"<<MaxTime()<<" Fact="<<Fact_<<std::endl;
...
void TUTORIAL::FixedPointScheme::IterateFixedPoint()
{
  ...
  if(converged)
    std::cout<<"Time="<< Time()<<" Converged in "<<iter<<" steps. disp="<<disp_<<" Fact="<<Fact_<<std::endl;
...
/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
double TUTORIAL::FixedPointScheme::Operator1(double Fadh)
{

  return Fadh/(E_1_+E_2_);
}


/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
double TUTORIAL::FixedPointScheme::Operator2(double disp)
{

  return Fact_-K_cell_*disp;
}
...
/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
double TUTORIAL::FixedPointScheme::InitialGuess()
{

  return Fact_-K_cell_*disp_;
}
