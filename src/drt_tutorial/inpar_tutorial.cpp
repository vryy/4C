...
void INPAR::TUTORIAL::SetValidParameters(Teuchos::RCP<Teuchos::ParameterList> list)
{
  ...
  Teuchos::ParameterList& tutorialparams = list->sublist(
    "TUTORIAL DYNAMIC",false,
    "General parameters for any tutorial"
    );

  setStringToIntegralParameter<int>(
                               "TYPE","NonlinearTruss",
                               "Choose type of tutorial.",
                               tuple<std::string>(
                                 "NonlinearTruss",
                                 ...
                                 tuple<int>(
                                 nonlinear_truss,
                                 partitioned_fixed_point_scheme,
                                 struct_ale_coupling),
                                 &tutorialparams);
  ...
  Teuchos::ParameterList& nlntrussdyn = tutorialparams.sublist("NONLINEAR TRUSS",false,
  ...
  Teuchos::ParameterList& fixedpointdyn = tutorialparams.sublist("FIXED POINT SCHEME",false,
                                                  "Control the Fixed-Point Tutorial");

  DoubleParameter("RELAX_PARAMETER",1.0,"Relaxation Parameter omega",&fixedpointdyn);
  DoubleParameter("CONVTOL",1e-07,"Convergence Criterion for Increment of Partitioned Scheme",&fixedpointdyn);
