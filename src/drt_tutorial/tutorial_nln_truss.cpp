...
std::vector<LINALG::Matrix<7,1> > TUTORIAL::NonlinearTruss::ProblemDefinition()
{
  ...
  X(0)=0.0; X(1) = 1.0; X(2)=2.0; X(3)=3.0; X(4)=4.0; X(5)=5.0; X(6)=6.0;
...
void TUTORIAL::NonlinearTruss::SetupProblem()
{
  ...
  static std::vector<LINALG::Matrix<numele+1,1> > vector_of_condition_matrices = ProblemDefinition();
...
void TUTORIAL::NonlinearTruss::PrintDiscretization()
{
  ...
  for (it=discretization_->begin(); it!=discretization_->end(); ++it)
    std::cout <<"ID "<< (it->second)->Id() << " => " <<" numnodes="<<(it->second)->NumNode()<<" Young's Modulus="<<(it->second)->Material()->YoungModulus()<<" Dofs="<<(it->second)->Dof(0)<<" "<<" Dofs="<<(it->second)->Dof(1)<<'\n';
  std::cout<<"\n"<<std::endl;
}
...
void TUTORIAL::NonlinearTruss::Newton()
{
  ...
  while(not converged and iter<maxiter)
  {
    ...
    std::cout<<"iter "<<iter<<": res="<<std::setprecision(7)<<rhs_.Norm2()<<"  inc="<<std::setprecision(7)<<inc_.Norm2()<<" -> converged="<<converged<<std::endl;

  }

  if(not converged)
    dserror("Newton did not converge in %d iterations.",maxiter);
...
/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
void TUTORIAL::NonlinearTruss::Evaluate(LINALG::Matrix<numele+1,1>* rhs, LINALG::Matrix<numele+1,numele+1>* stiff, bool eval_rhs, bool eval_stiff)
{
  ...
  for (ele=discretization_->begin(); ele!=discretization_->end(); ++ele)
  {
    ...
    if(eval_stiff)
      AssembleMatrix(ele->second, &ele_stiff, stiff);

    //assemble the element vector into the global vector
    if(eval_rhs)
      AssembleVector(ele->second,&ele_rhs, rhs);
...
/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
void TUTORIAL::NonlinearTruss::Solve(LINALG::Matrix<numele+1,1> rhs, LINALG::Matrix<numele+1,numele+1> stiff, LINALG::Matrix<numele+1,1> inc)
{
  // solve
  ...
  if(err!=0)
    dserror("Solver returned err=%d",err);
}
...
/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
void TUTORIAL::NonlinearTruss::AssembleMatrix(TUTORIAL::ELEMENTS::TutorialElement* ele, LINALG::Matrix<2,2>* ele_stiff_mat, LINALG::Matrix<numele+1,numele+1>* glob_stiff)
{
  ...
  if(numnode!=numdof)
    dserror("Number of dofs per element != number of nodes per element!\n "
...
/*-----------------------------------------------------------------------/
/-----------------------------------------------------------------------*/
void TUTORIAL::NonlinearTruss::AssembleVector(TUTORIAL::ELEMENTS::TutorialElement* ele, LINALG::Matrix<2,1>* ele_rhs_vec, LINALG::Matrix<numele+1,1>* glob_rhs)
{
  // get number of nodes from element
  int numnode = ele->NumNode();
  int numdof = ele->NumDof();
