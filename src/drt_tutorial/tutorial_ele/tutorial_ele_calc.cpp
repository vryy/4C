 ...
 * evaluate terms                                                       |
 *----------------------------------------------------------------------*/
void TUTORIAL::ELEMENTS::TutorialEleCalc::Evaluate(TUTORIAL::ELEMENTS::TutorialElement* element, LINALG::Matrix<7,1>* state, LINALG::Matrix<2,2>* elematrix, LINALG::Matrix<2,1>* elevector, bool eval_mat, bool eval_vec)
{
  ...
  for(int gp=0;gp<numgp;gp++)
  {
    ...
    N_X(1)=N_xi(1)/dX_dxi;
    // evaluate defgrd
    double defgrd = 1.0 + (((*state)((element->Id())+1)-(*state)(element->Id()))/(element->X(1)-element->X(0)));
