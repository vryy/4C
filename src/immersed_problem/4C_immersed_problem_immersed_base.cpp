    ...
    const std::string condname, bool buildgeometry)
{
  ...
  // build condition
  Teuchos::RCP<CORE::Conditions::Condition> condition = Teuchos::rcp(
      new CORE::Conditions::Condition(id, condtype, CORE::Conditions::to_string(condtype).data(),
          buildgeometry, CORE::Conditions::geometry_type_volume));
