...
/*!
  ...
  This has been the CPostBinary class in gidpost.
...
typedef struct _GID_POST_BINARY {
  gzFile file;
  int fail;
  int LastID;
  int connectivity;
...
void init_gid_post_binary(GID_POST_BINARY* gid)
{
  gid->file = NULL;
  gid->fail = 0;
  gid->LastID = -1;
...
int gid_close(GID_POST_BINARY* gid);

void destroy_gid_post_binary(GID_POST_BINARY* gid)
{
  gid_close(gid);
}


int gid_match_connectivity(GID_POST_BINARY* gid, int written)
{
...
int gid_open(GID_POST_BINARY* gid, char* name)
{
  gid->file = gzopen(name, "wb1");
  /* a magic number */
  gzwrite(gid->file, &ByteOrderCheck, sizeof(ByteOrderCheck));
...
int gid_flush(GID_POST_BINARY* gid)
{
  return gid->file ? gzflush(gid->file, Z_FULL_FLUSH) : 1;
}

int gid_write_string(GID_POST_BINARY* gid, char* str)
{
  int fail = 1;
  int size, written;
  ...
  if (gid->file) {
    ...
    if (str) buf = str;
    tam = strlen(buf) + 1;
    written = gzwrite(gid->file, &tam, sizeof(int));
    size = sizeof(char) * tam;
    written += gzwrite(gid->file, (void*)buf, size);
...
int gid_begin_coordinates(GID_POST_BINARY* gid)
{
  return gid_write_string(gid, "Coordinates -1 Indexed");
}

int gid_begin_elements(GID_POST_BINARY* gid)
{
  return gid_write_string(gid, "Elements -1 Indexed");
}

int gid_begin_values(GID_POST_BINARY* gid)
{
  return gid_write_string(gid, "Values -1 Indexed");
}

int gid_end_values(GID_POST_BINARY* gid)
{
  int idxend = -1;

  if ( gzwrite(gid->file, &idxend, sizeof(int)) != sizeof(int) )
    return 1;
  return gid_write_string(gid, "End Values");
}

int gid_write_values(GID_POST_BINARY* gid, int id, int n, ... )
{
  va_list ap;
  int i;
  float value;
  ...
  if (gid->LastID != id) {
    gzwrite(gid->file, &id, sizeof(id));
    gid->LastID = id;
  }
  va_start(ap, n);
  for ( i = 0; i < n; i++ ) {
    value = (float)va_arg(ap, double);
    gzwrite(gid->file, &value, sizeof(value));
  }
  va_end(ap);
  return 0;
}

int gid_write_buffer(GID_POST_BINARY* gid, int id, int n, double* buffer)
{
  int i;
  float value;
...
int gid_write2d(GID_POST_BINARY* gid, double x, double y)
{
  float values[2];

  values[0] = (float)x;
  values[1] = (float)y;
  gzwrite(gid->file, values, sizeof(float)*2);
  return 0;
}

int gid_write3d(GID_POST_BINARY* gid, double x, double y, double z)
{
  float values[3];
  ...
  values[1] = (float)y;
  values[3] = (float)z;
  gzwrite(gid->file, values, sizeof(float)*3);
  return 0;
}

int gid_write_element(GID_POST_BINARY* gid, int id, int n, int nid[])
{
  int i;

  gzwrite(gid->file, &id, sizeof(id));
  for ( i = 0; i < n; i++ ) {
    gzwrite(gid->file, nid+i, sizeof(int));
  }
  /* verify connectivity */
  switch (gid_match_connectivity(gid, n)) {
    case 0:
      /* match exactly */
      i = 1;
      /* so write material 1 */
      gzwrite(gid->file, &i, sizeof(int));
    case 1:
...
int gid_write_post_header(GID_POST_BINARY* gid)
{
  return gid_write_string(gid, "GiDPostEx1.1");
}
...
static GID_POST_BINARY ResultFile;
...
int GiD_BeginCoordinates()
{
  ...
  return gid_begin_coordinates(&ResultFile);
}
...
int GiD_EndCoordinates()
{
  ...
  return gid_write_string(&ResultFile, "End Coordinates");
}
...
int GiD_WriteCoordinates( int id, double x, double y, double z )
{
  ...
  return gid_write_values(&ResultFile, id, 3, x, y, z);
}
...
int GiD_WriteCoordinates2D(int id, double x, double y)
{
  ...
  assert(CheckState(POST_MESH_COORD0, level_mesh));
  /* keep in the same level */
  return gid_write_values(&ResultFile, id, 3, x, y, 0.0);
}
...
int GiD_BeginElements()
{
  ...
  return gid_begin_elements(&ResultFile);
}
...
int GiD_EndElements()
{
  ...
  assert(CheckState(POST_MESH_ELEM, level_mesh));
  level_mesh = POST_S0;
  return gid_write_string(&ResultFile, "End Elements");
}
...
int GiD_WriteElement( int id, int nid[] )
{
  ...
  return gid_write_element(&ResultFile, id, ResultFile.connectivity, nid);
}
...
int GiD_WriteElementMat( int id, int nid[] )
{
  ...
  assert(CheckState(POST_MESH_ELEM, level_mesh));
  /* keep on the same state */
  return gid_write_element(&ResultFile, id, ResultFile.connectivity+1, nid);
}
...
int GiD_OpenPostResultFile( char * FileName )
{
  ...
  init_gid_post_binary(&ResultFile);
...
int GiD_ClosePostResultFile()
{
  ...
  destroy_gid_post_binary(&ResultFile);
