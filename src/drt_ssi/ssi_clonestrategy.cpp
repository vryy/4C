...
std::map<std::string,std::string> SSI::ScatraStructureCloneStrategy::ConditionsToCopy()
{
  ...
  conditions_to_copy.insert(std::pair<std::string,std::string>("S2ICouplingPartitioning","S2ICouplingPartitioning"));
    ...
    )
{
  ...
  if(eletypename == "So_tet4ScatraType")
    impltype = (dynamic_cast<DRT::ELEMENTS::So3_Scatra<DRT::ELEMENTS::So_tet4, DRT::Element::tet4>*>(ele))->ImplType();
  // tet10 solid scatra
  else if(eletypename == "So_tet10ScatraType")
    impltype = (dynamic_cast<DRT::ELEMENTS::So3_Scatra<DRT::ELEMENTS::So_tet10, DRT::Element::tet10>*>(ele))->ImplType();
  // HEX 8 Elements
  // hex8 solid scatra
  else if(eletypename == "So_hex8ScatraType")
    impltype = (dynamic_cast<DRT::ELEMENTS::So3_Scatra<DRT::ELEMENTS::So_hex8, DRT::Element::hex8>*>(ele))->ImplType();
  // hex8fbar solid scatra
  else if(eletypename == "So_hex8fbarScatraType")
    impltype = (dynamic_cast<DRT::ELEMENTS::So3_Scatra<DRT::ELEMENTS::So_hex8fbar, DRT::Element::hex8>*>(ele))->ImplType();
  // hex27 solid scatra
  else if(eletypename == "So_hex27ScatraType")
    impltype = (dynamic_cast<DRT::ELEMENTS::So3_Scatra<DRT::ELEMENTS::So_hex27, DRT::Element::hex27>*>(ele))->ImplType();
  // wedge6
  else if(eletypename == "So_weg6ScatraType")
    impltype = (dynamic_cast<DRT::ELEMENTS::So3_Scatra<DRT::ELEMENTS::So_weg6, DRT::Element::wedge6>*>(ele))->ImplType();
    ...
    const bool isnurbsdis)
{
  ...
  if (trans!=NULL)
  {
    ...
    if(impltype == INPAR::SCATRA::impltype_undefined)
      dserror("ScatraStructureCloneStrategy copies scatra discretization from structure discretization, but the "
