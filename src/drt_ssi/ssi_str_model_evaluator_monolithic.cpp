...
void STR::MODELEVALUATOR::MonolithicSSI::DetermineStressStrain()
{
  ...
  const Teuchos::RCP<std::map<int,Teuchos::RCP<Epetra_SerialDenseMatrix> > > stresses = Teuchos::rcp(new std::map<int,Teuchos::RCP<Epetra_SerialDenseMatrix> >);
  ...
  for(int i=0; i<Discret().ElementRowMap()->NumMyElements(); ++i)
  {
    ...
    const Teuchos::RCP<Epetra_SerialDenseMatrix> stresses_ele = Teuchos::rcp(new Epetra_SerialDenseMatrix);
  ...
  DRT::Exporter exporter(*Discret().ElementRowMap(),*Teuchos::rcp_dynamic_cast<DRT::Discretization>(DiscretPtr())->ElementColMap(),Discret().Comm());
  ...
  Discret().Evaluate(parameters,Teuchos::null,Teuchos::null,Teuchos::null,Teuchos::null,Teuchos::null);

  // initialize vectors for stresses and element numbers on slave and master sides of scatra-scatra coupling interfaces
  const Teuchos::RCP<Epetra_MultiVector> stresses_slave(Teuchos::rcp(new Epetra_MultiVector(*ssi_mono_->CouplingAdapterStructure()->SlaveDofMap(),2))), stresses_master(Teuchos::rcp(new Epetra_MultiVector(*ssi_mono_->CouplingAdapterStructure()->MasterDofMap(),2)));
  Epetra_IntVector numelement_slave(*ssi_mono_->CouplingAdapterStructure()->SlaveDofMap()), numelement_master(*ssi_mono_->CouplingAdapterStructure()->MasterDofMap());
  ...
  for(unsigned icond=0; icond<conditions.size(); ++icond)
  {
    ...
    Epetra_MultiVector& stresses_vector = side == INPAR::S2I::side_slave ? *stresses_slave : *stresses_master;
    Epetra_IntVector& numelement_vector = side == INPAR::S2I::side_slave ? numelement_slave : numelement_master;
  ...
  const Teuchos::RCP<Epetra_MultiVector> stresses_temp(Teuchos::rcp(new Epetra_MultiVector(*ssi_mono_->CouplingAdapterStructure()->SlaveDofMap(),2)));
 ...
 *----------------------------------------------------------------------*/
void STR::MODELEVALUATOR::MonolithicSSI::WriteRestart(IO::DiscretizationWriter& iowriter,const bool& forced_writerestart) const
{
  // write nodal stresses
  iowriter.WriteVector("nodal_stresses_xyz",stresses_,IO::nodevector);
