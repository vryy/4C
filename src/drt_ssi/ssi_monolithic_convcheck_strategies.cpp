...
\brief strategies for Newton-Raphson convergence check for monolithic scalar-structure interaction problems

To keep the time integrator class for monolithic scalar-structure interaction problems as plain as possible,
the convergence check for the Newton-Raphson iteration has been encapsulated within separate strategy classes.
Every specific convergence check strategy (e.g., for monolithic scalar-structure interaction problems involving
standard scalar transport or electrochemistry) computes, checks, and outputs different relevant vector norms
and is implemented in a subclass derived from an abstract, purely virtual interface class.
    ...
    ) const
{
  ...
  // safety checks
  if(std::isnan(scatradofnorm) or
     std::isnan(scatraresnorm) or
     std::isnan(scatraincnorm) or
     std::isnan(structuredofnorm) or
     std::isnan(structureresnorm) or
     std::isnan(structureincnorm))
    dserror("Vector norm is not a number!");
  if(std::isinf(scatradofnorm) or
     std::isinf(scatraresnorm) or
     std::isinf(scatraincnorm) or
     std::isinf(structuredofnorm) or
     std::isinf(structureresnorm) or
     std::isinf(structureincnorm))
    dserror("Vector norm is infinity!");

  // prevent division by zero
  if(scatradofnorm < 1.e-10)
    scatradofnorm = 1.e-10;
  if(structuredofnorm < 1.e-10)
    structuredofnorm = 1.e-10;
  ...
  if(timint.iter_ == 1)
  {
    if(timint.Comm().MyPID() == 0)
    {
      ...
      std::cout << "|  " << std::setw(3) << timint.iter_ << "/" << std::setw(3) << itermax_ << "   | "
                ...
                << std::setw(10) << std::setprecision(3) << std::scientific << scatraresnorm
                << "   |      --      | "
                << std::setw(10) << std::setprecision(3) << std::scientific << structureresnorm
                << "   |      --      | "
                << "(       --      , te = "
                << std::setw(10) << std::setprecision(3) << timint.dtele_ << ")" << std::endl;
    }
  }
  ...
  else
  {
    ...
    if(timint.Comm().MyPID() == 0)
      std::cout << "|  " << std::setw(3) << timint.iter_ << "/" << std::setw(3) << itermax_ << "   | "
                << std::setw(10) << std::setprecision(3) << std::scientific << itertol_ << "[L_2 ]  | "
                << std::setw(10) << std::setprecision(3) << std::scientific << scatraresnorm << "   | "
                << std::setw(10) << std::setprecision(3) << std::scientific << scatraincnorm/scatradofnorm << "   | "
                << std::setw(10) << std::setprecision(3) << std::scientific << structureresnorm << "   | "
                << std::setw(10) << std::setprecision(3) << std::scientific << structureincnorm/structuredofnorm << "   | (ts = "
                << std::setw(10) << std::setprecision(3) << timint.dtsolve_ << ", te = "
                << std::setw(10) << std::setprecision(3) << timint.dtele_ << ")" << std::endl;

    // convergence check
    if(scatraresnorm <= itertol_ and
       structureresnorm <= itertol_ and
  ...
  if(scatraresnorm < restol_ and structureresnorm < restol_)
    exit = true;
    ...
    ) const
{
  ...
  timint.scatra_->ScaTraField()->Splitter()->ExtractOtherVector(timint.scatra_->ScaTraField()->Phinp())->Norm2(&concdofnorm);

  //! compute L2 norm of concentration increment vector
  double concincnorm(0.);
  timint.scatra_->ScaTraField()->Splitter()->ExtractOtherVector(timint.maps_->ExtractVector(timint.increment_,0))->Norm2(&concincnorm);

  //! compute L2 norm of concentration residual vector
  double concresnorm(0.);
  timint.scatra_->ScaTraField()->Splitter()->ExtractOtherVector(timint.maps_->ExtractVector(timint.residual_,0))->Norm2(&concresnorm);

  //! compute L2 norm of potential state vector
  double potdofnorm(0.);
  timint.scatra_->ScaTraField()->Splitter()->ExtractCondVector(timint.scatra_->ScaTraField()->Phinp())->Norm2(&potdofnorm);

  //! compute L2 norm of potential increment vector
  double potincnorm(0.);
  timint.scatra_->ScaTraField()->Splitter()->ExtractCondVector(timint.maps_->ExtractVector(timint.increment_,0))->Norm2(&potincnorm);

  //! compute L2 norm of potential residual vector
  double potresnorm(0.);
  timint.scatra_->ScaTraField()->Splitter()->ExtractCondVector(timint.maps_->ExtractVector(timint.residual_,0))->Norm2(&potresnorm);
  ...
  if(timint.iter_ == 1)
  {
    if(timint.Comm().MyPID() == 0)
    {
      // print header of convergence table to screen
      std::cout << "+------------+-------------------+--------------+--------------+--------------+--------------+--------------+--------------+" << std::endl;
      std::cout << "|- step/max -|- tolerance[norm] -|-- conc-res --|-- conc-inc --|-- pot-res ---|-- pot-inc ---|- struct-res -|- struct-inc -|" << std::endl;
  ...
  // exit Newton-Raphson iteration when residuals are small enough to prevent unnecessary additional solver calls
  if(concresnorm < restol_ and potresnorm < restol_ and structureresnorm < restol_)
    exit = true;
