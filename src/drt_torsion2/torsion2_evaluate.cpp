...
int DRT::ELEMENTS::Torsion2::EvaluateStatForceDamp(ParameterList& params,
    ...
    int& ActNumDof0)
{
  ...
  {
  case tr3_totlag:
    t2_nlnstiffmass_totlag(disp,stiffmatrix,massmatrix,force,ActNumDof0);
    return;
  case tr3_engstrain:
    t2_nlnstiffmass_engstr(disp,stiffmatrix,massmatrix,force,ActNumDof0);
    return;
  }
}
 ...
 *-----------------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Torsion2::t2_nlnstiffmass_totlag( vector<double>& disp,
    Epetra_SerialDenseMatrix* stiffmatrix,
    Epetra_SerialDenseMatrix* massmatrix,
    Epetra_SerialDenseVector* force,
    int& ActNumDof0)
{
  ...
  /* read material parameters using structure _MATERIAL which is defined by inclusion of      /
   ...
   / function instead of e.g. Torsion2_input.cpp or within the Torsion2Register class since it is not/
...
/*------------------------------------------------------------------------------------------------------------*
  ...
  *-----------------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Torsion2::t2_nlnstiffmass_engstr( vector<double>& disp,
    Epetra_SerialDenseMatrix* stiffmatrix,
    Epetra_SerialDenseMatrix* massmatrix,
    Epetra_SerialDenseVector* force,
    int& ActNumDof0)
...
// lump mass matrix
void DRT::ELEMENTS::Torsion2::t2_lumpmass(Epetra_SerialDenseMatrix* emass)
{
  // lump mass matrix
  if (emass != NULL)
  {
