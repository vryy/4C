...
\brief evaluation of a generic material whose deformation gradient is modeled to be split multiplicatively into elastic and inelastic parts
  ...
  density_(matdata->GetDouble("DENS"))
{
  ...
  if (nummat_elast_ != (int)matids_elast_->size())
    dserror("number of elastic materials %d does not fit to size of elastic material ID vector %d", nummat_elast_, matids_elast_->size());

  if (numfac_inel_ != (int)inel_defgradfacids_->size())
    dserror("number of inelastic deformation gradient factors %d does not fit to size of inelastic deformation gradient ID vector %d", numfac_inel_, inel_defgradfacids_->size());
}
...
MAT::MultiplicativeSplitDefgrad_ElastHyperType MAT::MultiplicativeSplitDefgrad_ElastHyperType::instance_;


DRT::ParObject* MAT::MultiplicativeSplitDefgrad_ElastHyperType::Create( const std::vector<char> & data )
{
  MAT::MultiplicativeSplitDefgrad_ElastHyper* splitdefgrad_elhy = new MAT::MultiplicativeSplitDefgrad_ElastHyper();
 ...
 *--------------------------------------------------------------------*/
MAT::MultiplicativeSplitDefgrad_ElastHyper::MultiplicativeSplitDefgrad_ElastHyper(MAT::PAR::MultiplicativeSplitDefgrad_ElastHyper* params)
 : params_(params),
   potsumel_(0),
   facdefgradin_(0)
{
  ...
  // monolithic ssi coupling algorithm only implemented for MAT_InelasticDefgradLinScalarIso and MAT_InelasticDefgradLinScalarAniso so far
  if (DRT::INPUT::IntegralValue<INPAR::SSI::SolutionSchemeOverFields>(ssicontrol,"COUPALGO") == INPAR::SSI::ssi_Monolithic)
  {
    for (unsigned int p=0; p<facdefgradin_.size(); ++p)
    {
      if((facdefgradin_[p]->MaterialType() != INPAR::MAT::mfi_lin_scalar_aniso) and (facdefgradin_[p]->MaterialType() != INPAR::MAT::mfi_lin_scalar_iso))
        dserror("When you use the 'COUPALGO' 'ssi_Monolithic' from the 'SSI CONTROL' section, you need to"
            " use the material 'MAT_InelasticDefgradLinScalarAniso' or 'MAT_InelasticDefgradLinScalarIso'!"
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::Evaluate(const LINALG::Matrix<3,3>* defgrad,   ///< Deformation gradient
                                                          ...
                                                          const int eleGID)                     ///< Element ID
{
  ...
  EvaluateKinQuantElast(defgrad,iFinM,iCinV,iCinCiCinV,iCV,iCinCM,iFinCeM,CiFin9x1,CiFinCe9x1,CiFiniCe9x1,prinv);
  ...
  // evaluate dSdiFin
  EvaluatedSdiFin(gamma,delta,iFinM,iCinCM,iCinV,CiFin9x1,CiFinCe9x1,iCinCiCinV,CiFiniCe9x1,iCV,iFinCeM,dSdiFin);

  // if cmat != NULL, we are evaluating the structural residual and linearizations, so we need to calculate the stresses and the cmat
  // if you like to evaluate the off-diagonal block of your monolithic system (structural residual w.r.t. dofs of another field),
  // you need to pass NULL as the cmat when you call Evaluate() in the element
  if (cmat != NULL)
  {
    // cmat = 2 dS/dC = 2 \frac{\partial S}{\partial C} + 2 \frac{\partial S}{\partial F_{in}^{-1}} : \frac{\partial F_{in}^{-1}}{\partial C}
    // = cmatiso + cmatadd
    ...
    // cmatadd = 2 \frac{\partial S}{\partial F_{in}^{-1}} : \frac{\partial F_{in}^{-1}}{\partial C},
    // where F_{in}^{-1} can be multiplicatively composed of several inelastic contributions
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluateStressCmatIso(const LINALG::Matrix<6,1> & iCV,         ///< Inverse right Cauchy-Green tensor
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluateKinQuantElast(const LINALG::Matrix<3,3> * const defgrad,  ///< Deformation gradient
                                                                       ...
                                                                       LINALG::Matrix<3,1> & prinv) const          ///< Principal invariants of elastic right Cauchy-Green tensor
{
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::InvariantsPrincipal(const LINALG::Matrix<6,1> & C_strain,  ///< symmetric Cartesian 2-tensor in strain-like 6-Voigt notation
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluateInvariantDerivatives(const LINALG::Matrix<3,1> & prinv,  ///< Principal invariants of the elastic right Cauchy-Green tensor
                                                                              const int eleGID,                   ///< Element ID
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::CalculateGammaDelta(const LINALG::Matrix<3,1> & prinv,  ///< principal invariants of the elastic right Cauchy-Green tensor
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluatedSdiFin(const LINALG::Matrix<3,1> & gamma,        ///< Factors for stress calculation
                                                                 const LINALG::Matrix<8,1> & delta,        ///< Factors for elasticity tensor calculation
                                                                 const LINALG::Matrix<3,3> & iFinM,        ///< Inverse inelastic deformation gradient
                                                                 ...
                                                                 LINALG::Matrix<6,9> & dSdiFin) const      ///< derivative of 2nd Piola Kirchhoff stresses w.r.t. inverse inelastic right Cauchy-Green tensor
{
  // clear variable
  dSdiFin.Clear();
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluateAdditionalCmat(const LINALG::Matrix<3,3> * const defgrad,         ///< Deformation gradient
                                                                        const std::vector<LINALG::Matrix<3,3> > & iFinjM,  ///< Vector that holds all inverse inelastic deformation gradient factors as 3x3 matrices
                                                                        const LINALG::Matrix<6,1> & iCV,                   ///< Inverse right Cauchy-Green tensor
                                                                        const LINALG::Matrix<6,9> & dSdiFin,               ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. the inverse inelastic deformation gradient
                                                                        LINALG::Matrix<6,6> & cmatadd) const               ///< Additional elasticity tensor
{
  // clear variable
  cmatadd.Clear();

  // check amount of factors the inelastic deformation gradient consists of and choose implementation accordingly
  ...
  default:
    dserror("You defined %i inelastic deformation gradient factors. But framework is only implemented for a maximum of 2 inelastic contributions! "
        "If you really need more than 2 inelastic contributions, it's your turn to implement it! ;-)",facdefgradin_.size());
    break;
  }
...
/*--------------------------------------------------------------------*
 | setup                                                schmidt 03/18 |
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::Setup(const int numgp,DRT::INPUT::LineDefinition* linedef)
{
  // elastic materials
  for (unsigned int p=0; p<potsumel_.size(); ++p)
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluateInverseInelasticDefGrad(const LINALG::Matrix<3,3> * const defgrad,   ///< Deformation gradient
                                                                                 std::vector<LINALG::Matrix<3,3> > & iFinjM,  ///< Vector that holds all inverse inelastic deformation gradient factors as 3x3 matrices
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::EvaluateODStiffMat(const LINALG::Matrix<3,3> * const defgrad,         ///< Deformation gradient
                                                                    const std::vector<LINALG::Matrix<3,3> > & iFinMj,  ///< Vector that holds all inverse inelastic deformation gradient factors as 3x3 matrices
                                                                    const LINALG::Matrix<6,9> & dSdiFin,               ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. the inverse inelastic deformation gradient
                                                                    LINALG::Matrix<6,1> & dstressdx)                   ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. primary variable of different field
{
  // clear variable
  dstressdx.Clear();
 ...
 *--------------------------------------------------------------------*/
void MAT::MultiplicativeSplitDefgrad_ElastHyper::PreEvaluate(Teuchos::ParameterList& params) const  ///< parameter list as handed in from the element
{
  // loop over all inelastic contributions
  for (unsigned int p=0; p<facdefgradin_.size(); ++p)
    facdefgradin_[p]->PreEvaluate(params);
