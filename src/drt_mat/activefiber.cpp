...
/*----------------------------------------------------------------------*
 |                                                                      |
 *----------------------------------------------------------------------*/
MAT::PAR::ActiveFiber::ActiveFiber(
 ...
 *----------------------------------------------------------------------*/
MAT::ActiveFiber::ActiveFiber(MAT::PAR::ActiveFiber* params)
  : params_(params)
{
}
 ...
 *----------------------------------------------------------------------*/
void MAT::ActiveFiber::ResetStep()
{
...
//  GLtoEA(*glstrain,invdefgrd,eastrain);
  ...
  // Setting up gauss quadrature (do not forget to adapt defines "numbgp" and "twice" in activefiber.H)
    ...
    for (int i=0; i<gausspoints.nquad; i++)
    {
      ...
      while(abs(residual) > tol)
      {
        ...
        if(exponent < -30.0)
        {
          ...
          inc = residual/
          (1.0 -
              ((denom*sigmamax) - (1.0/etanew(i,j)*sigmamax*efunct*aa*epsomegaphi(i,j)))*deta_dsigma*onebydenom*onebydenom );
...
void MAT::ActiveFiber::SetupRates(
 ...
 *----------------------------------------------------------------------*/
void MAT::ActiveFiber::CauchytoPK2(
 ...
 *-------------------------------------------------------------------------------------*/
void MAT::ActiveFiber::MatrixRootDerivativeSym3x3(const LINALG::Matrix<3,3> MatrixIn, LINALG::Matrix<6,6>& MatrixRootDeriv)
{
 ...
 *-------------------------------------------------------------------------------------------*/
void MAT::ActiveFiber::SetupCmatActive(
...
//  MultMatrixFourTensor(temptens2,tempmat2,RootCInvDerivCRootCInv,true);
  ...
  for (int j=0; j<2*gausspoints.nquad; j++)
  {
    for (int i=0; i<gausspoints.nquad; i++)
    {
      ...
      {
        double temp = - denom*sigmamax*deta_dsigma + sigmamax*efunct*aa*epsomegaphi(i,j)*deta_dsigma/etanew(i,j);
        factor = (3.0/(16.0*((double)gausspoints.nquad)))*((sigmamax*efunct*aa)/(denom*denom + temp));
      }
        ...
        for (int n=0; n<3; n++)
          for (int o=0; o<3; o++)
            for (int p=0; p<3; p++)
              for (int q=0; q<3; q++)
                for (int r=0; r<3; r++)
                  temptensgauss[mm][n][o][p] += temptens8[q][r][o][p]*m(q)*m(r)*factor*gausspoints.qwgt[i]*m(mm)*m(n);
 ...
 *-------------------------------------------------------------------------------------*/
void MAT::ActiveFiber::Setup4Tensor(
 ...
 *------------------------------------------------------------------------------------*/
void MAT::ActiveFiber::MultFourTensorMatrix(
  ...
  for (int i=0; i<3; i++)
    for (int j=0; j<3; j++)
      for (int k=0; k<3; k++)
        for (int l=0; l<3; l++)
          for (int m=0; m<3; m++)
            FourTensorResult[i][j][k][l] += FourTensor[i][j][k][m] * Matrix(m,l); // C^ijkl = A^ijkm * B_m^l
...
void MAT::ActiveFiber::MultMatrixFourTensor(
    double FourTensorResult[3][3][3][3],
    LINALG::Matrix<3,3> Matrix,    // B^i_m
 ...
 *----------------------------------------------------------------------*/
bool MAT::ActiveFiber::VisData(const std::string& name, std::vector<double>& data, int numgp , int eleID)
{
  if (name == "Etahat")
  {
    if ((int)data.size()!=1)
 ...
 *------------------------------------------------------------------------------------------*/
void MAT::ActiveFiber::PrintFourTensor(
    double FourTensor[3][3][3][3]
                                            )
{
  for(int i=0;i<3;++i)
