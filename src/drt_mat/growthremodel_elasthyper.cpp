  ...
  membrane_(matdata->GetInt("MEMBRANE"))
{
  ...
  if( membrane_ == 1 ) {
    if( (growthtype_ != 1) || (loctimeint_ != 0) )
      dserror("using membrane elements you can only simulate anisotropic growth in thickness direction"
    ...
    if (nummat_elastiniso_ == 0)
      dserror("you have to set a 3D elastin material");
    if (matid_penalty_ == -1)
      dserror("you have to set a volumetric penalty material");
    ...
    nr_rf_tot_(0)
{
  ...
  for (m=params_->matids_elastinmem_->begin(); m!=params_->matids_elastinmem_->end(); ++m) {
    ...
    if(sum->MaterialType() != INPAR::MAT::mes_isoneohooke)
      dserror("2D Elastin Material: So far, you have to use a IsoNeoHooke material as the prestretching algorithm needs it. "
  ...
  if(params_->membrane_ != 1) {
    ...
    for (m=params_->matids_elastiniso_->begin(); m!=params_->matids_elastiniso_->end(); ++m) {
      ...
      if(sum == Teuchos::null) dserror("Failed to allocate");
      if(sum->MaterialType() != INPAR::MAT::mes_isoneohooke)
        dserror("3D Elastin Material: So far, you have to use an IsoNeoHooke material as the prestretching algorithm needs it"
            "The prestretching algorithm can easily be expanded to other materials!");
    ...
    if(sum->MaterialType() != INPAR::MAT::mes_volsussmanbathe)
      dserror("Volumetric Penalty Material: So far, you have to use a CoupNeoHooke material as the prestretching algorithm needs it. "
...
void MAT::GrowthRemodel_ElastHyper::Setup(int numgp, DRT::INPUT::LineDefinition* linedef)
{
  ...
  for(unsigned p=0;p<potsumrf_.size();++p)
    nr_rf_tot_ += potsumrf_[p]->GetNumFibers();
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::GrowthRemodel_ElastHyper::Update(LINALG::Matrix<3,3> const& defgrd,
                                           int const gp,
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::GrowthRemodel_ElastHyper::EvaluatePrestretch(LINALG::Matrix<3,3> const * const defgrd,
                                                       int const gp,
                                                       int const eleGID)
{
  ...
  while(fabs(R) > 1.0e-10) {
    R = matiso->Mue()*init_rho_el_[gp]*std::pow(params_->lamb_prestretch_cir_*params_->lamb_prestretch_cir_*
        params_->lamb_prestretch_ax_*params_->lamb_prestretch_ax_*lamb_pre*lamb_pre,-2./3.)*(lamb_pre*lamb_pre -
        (1./3.)*(params_->lamb_prestretch_cir_*params_->lamb_prestretch_cir_ +
        params_->lamb_prestretch_ax_*params_->lamb_prestretch_ax_ + lamb_pre*lamb_pre)) +
        matvol->Kappa()*init_rho_el_[gp]*((params_->lamb_prestretch_cir_*params_->lamb_prestretch_ax_*lamb_pre)*
        (params_->lamb_prestretch_cir_*params_->lamb_prestretch_ax_*lamb_pre)-
        (params_->lamb_prestretch_cir_*params_->lamb_prestretch_ax_*lamb_pre)) +
        ((1.0-(gp_rad_[gp]-10.0e-3)/params_->t_ref_)*params_->p_mean_);

    dRdlamb_pre = matiso->Mue()*init_rho_el_[gp]*(-(4./3.)*std::pow(params_->lamb_prestretch_cir_*params_->lamb_prestretch_ax_*
                  lamb_pre,-7./3.)*params_->lamb_prestretch_cir_*params_->lamb_prestretch_ax_)*
                  (lamb_pre*lamb_pre -(1./3.)*(params_->lamb_prestretch_cir_*params_->lamb_prestretch_cir_ +
                  params_->lamb_prestretch_ax_*params_->lamb_prestretch_ax_ + lamb_pre*lamb_pre)) +
                  matiso->Mue()*init_rho_el_[gp]*std::pow(params_->lamb_prestretch_cir_*params_->lamb_prestretch_cir_*
                  params_->lamb_prestretch_ax_*params_->lamb_prestretch_ax_*lamb_pre*lamb_pre,-2./3.)*
                  (2.0*lamb_pre - (1./3.)*(2.0*lamb_pre)) +
                  matvol->Kappa()*init_rho_el_[gp]*(2.0*(params_->lamb_prestretch_cir_*params_->lamb_prestretch_ax_*lamb_pre)*
  ...
  else
  {
    ...
    case 1:
    {
      ...
      for(unsigned p=0;p<potsumrf_.size();++p) {
        ...
        stress->Update(1.0,stressaniso,1.0);
        cmat->Update(1.0,cmataniso,1.0);
        potsumrf_[p]->EvaluateAdditionalGrowthRemodelCmat(defgrd,nr_grf_proc,iFgM,diFgdrhoM,drhodC,dlambrdC,cmatanisoadd,gp,eleGID);
...
void MAT::GrowthRemodel_ElastHyper::SolveForRhoLambr(LINALG::SerialDenseMatrix & K_T,
                                                     ...
                                                     LINALG::Matrix<3,3> & dFgdrhoM,
                                                     LINALG::Matrix<3,3> & diFgdrhoM,
                                                     LINALG::Matrix<3,3> const * const defgrd,
                                                     double const& dt,
                                                     int const gp,
                                                     int const eleGID)
{
  ...
  for(unsigned i=0;i<2*nr_rf_tot_;++i)
    R(i,0) = 1.0;
  ...
  while(R.NormOne() > 1.0e-10)
  {
    ...
    nr_grf_proc = 0;
    for(unsigned p=0;p<potsumrf_.size();++p) {
      potsumrf_[p]->EvaluateDerivativesInternalNewton(defgrd,nr_grf_proc,nr_rf_tot_,gp,dt,eleGID,iFgM,dFgdrhoM,diFgdrhoM,dWdrho,dWdlambr,W,dEdrho,dEdlambr,E);
      nr_grf_proc += potsumrf_[p]->GetNumFibers();
    }
    ...
    if(iter > 95) {
      std::cout <<"iteration:  " <<iter <<std::endl <<std::endl <<"Vector of residuals: " <<std::endl <<R <<std::endl
          <<"Matrix of derivatives:" <<std::endl <<K_T;
      std::cout <<"=================================================================" <<std::endl <<std::endl;
      if(iter>100)
        dserror("Internal Newton (at Gauß-Point) does not converge!");
    }
    ++iter;
  }
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::GrowthRemodel_ElastHyper::SolveFordrhodCdlambrdC(std::vector<LINALG::Matrix<1,6> > & drhodC,
                                                           std::vector<LINALG::Matrix<1,6> > & dlambrdC,
                                                           LINALG::Matrix<1,6> & sum_drhodC,
                                                           LINALG::SerialDenseMatrix & K_T,
                                                           ...
                                                           int const eleGID) const
{
  ...
  int nr_grf_proc = 0;
  for(unsigned p=0;p<potsumrf_.size();++p) {
    potsumrf_[p]->EvaluateDerivativesCauchyGreen(defgrd,nr_grf_proc,gp,dt,iFgM,dWdC,dEdC,eleGID);
    nr_grf_proc += potsumrf_[p]->GetNumFibers();
  }
...
void MAT::GrowthRemodel_ElastHyper::EvaluateStressCmatIso(LINALG::Matrix<3,3> const* const defgrd,
                                                          LINALG::Matrix<3,3> const& iFinM,
                                                          LINALG::Matrix<6,1> & stressiso,
                                                          LINALG::Matrix<6,6> & cmatiso,
                                                          LINALG::Matrix<6,9> & dSdiFg,
                                                          int const gp,
                                                          int const eleGID) const
{
  ...
  EvaluateKinQuantElast(defgrd,iFinM,iCinv,iCinCiCinv,iCv,iCinCM,iFinCeM,CiFin9x1,CiFinCe9x1,CiFiniCe9x1,prinv,gp);
  ...
  EvaluatedSdiFg(dSdiFg,gamma,delta,iFinM,iCinCM,iCinv,CiFin9x1,CiFinCe9x1,iCinCiCinv,CiFiniCe9x1,iCv,iFinCeM,gp);
...
void MAT::GrowthRemodel_ElastHyper::EvaluateKinQuantElast(LINALG::Matrix<3,3> const* const defgrd,
                                                          LINALG::Matrix<3,3> const& iFinM,
                                                          LINALG::Matrix<6,1> & iCinv,
                                                          LINALG::Matrix<6,1> & iCinCiCinv,
                                                          LINALG::Matrix<6,1> & iCv,
                                                          LINALG::Matrix<3,3> & iCinCM,
                                                          LINALG::Matrix<3,3> & iFinCeM,
                                                          LINALG::Matrix<9,1> & CiFin9x1,
                                                          LINALG::Matrix<9,1> & CiFinCe9x1,
                                                          LINALG::Matrix<9,1> & CiFiniCe9x1,
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::GrowthRemodel_ElastHyper::InvariantsPrincipal(LINALG::Matrix<3,1> & prinv,
                                                        LINALG::Matrix<6,1> const& C_strain) const
{
  // 1st invariant, trace
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::GrowthRemodel_ElastHyper::InvariantsModified(LINALG::Matrix<3,1> & modinv,
                                                       LINALG::Matrix<3,1> const& prinv) const
{
  // 1st invariant, trace
...
void MAT::GrowthRemodel_ElastHyper::ConvertModToPrinc(LINALG::Matrix<3,1> const& prinv,
                                                      LINALG::Matrix<3,1> const& dPmodI,
                                                      LINALG::Matrix<6,1> const& ddPmodII,
...
void MAT::GrowthRemodel_ElastHyper::CalculateGammaDelta(LINALG::Matrix<3,1> & gamma,
                                                        ...
                                                        LINALG::Matrix<3,1> const& prinv,
                                                        LINALG::Matrix<3,1> const& dPI,
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::GrowthRemodel_ElastHyper::EvaluateIsotropicPrincElast(LINALG::Matrix<6,1> & stressisoprinc,
                                                                LINALG::Matrix<6,6> & cmatisoprinc,
                                                                LINALG::Matrix<6,1> const& iCinv,
                                                                LINALG::Matrix<6,1> const& iCinCiCinv,
                                                                LINALG::Matrix<6,1> const& iCv,
                                                                LINALG::Matrix<3,1> const& gamma,
...
void MAT::GrowthRemodel_ElastHyper::EvaluatedSdiFg(LINALG::Matrix<6,9> & dSdiFg,
                                                    LINALG::Matrix<3,1> const& gamma,
                                                    LINALG::Matrix<8,1> const& delta,
                                                    LINALG::Matrix<3,3> const& iFinM,
                                                    LINALG::Matrix<3,3> const& iCinCM,
                                                    LINALG::Matrix<6,1> const& iCinv,
                                                    LINALG::Matrix<9,1> const& CiFin9x1,
                                                    LINALG::Matrix<9,1> const& CiFinCe9x1,
                                                    LINALG::Matrix<6,1> const& iCinCiCinv,
                                                    LINALG::Matrix<9,1> const& CiFiniCe9x1,
                                                    LINALG::Matrix<6,1> const& iCv,
                                                    LINALG::Matrix<3,3> const& iFinCeM,
                                                    int const gp) const
{
  // clear some variables
  dSdiFg.Clear();
...
void MAT::GrowthRemodel_ElastHyper::EvaluateAdditionalCmat(LINALG::Matrix<6,6> & cmatadd,
                                                           ...
                                                           LINALG::Matrix<1,6> const& sum_drhodC,
                                                           LINALG::Matrix<6,9> const& dSdiFg,
                                                           int const gp) const
{
  // clear some variables
  cmatadd.Clear();
                                                               ...
                                                               const int eleGID) const
{
  ...
  static LINALG::FADMatrix<3,3> ZM_fad(true);   // Z = F_{in}^{-T}*A_{gr}^{T}*X^{-1}*A_{gr}^{T}*F_{in}^{-1}
  ...
  for(int i=0;i<3;++i)
    for(int j=0;j<9;++j)
      dSdiFg(i,j) = stress_fad(i,i).dx(j);
  for(int j=0;j<9;++j) {
...
void MAT::GrowthRemodel_ElastHyper::EvaluateElastinDamage(double const& dt_pre)
{
  ...
  for(unsigned gp=0;gp<cur_rho_el_.size();++gp)
    cur_rho_el_[gp] = init_rho_el_[gp]*(exp(-(t_tot_-dt_pre)/T_el) - 0.5*(1.0-exp(-(t_tot_-dt_pre)/t_dam))*exp(-0.5*(100.0*(fabs(gp_ax_[gp])-0.09))*(100.0*(fabs(gp_ax_[gp])-0.09))));
                                                          ...
                                                          const int gp)
{
   ...
   {
   case 1:
     // build growth and inverse growth deformation gradient (and its derivative) for anisotropic growth
     ...
     break;
   case 0:
     // build growth and inverse growth deformation gradient (and its derivative) for isotropic growth
     ...
     diFgdrhoM.Update(-1./3.*std::pow(v_[gp],-4./3.)*(1./params_->density_),AcirM_,0.0);
     diFgdrhoM.Update(-1./3.*std::pow(v_[gp],-4./3.)*(1./params_->density_),AradM_,1.0);
     diFgdrhoM.Update(-1./3.*std::pow(v_[gp],-4./3.)*(1./params_->density_),AaxM_,1.0);
