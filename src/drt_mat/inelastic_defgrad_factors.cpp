 ...
 *--------------------------------------------------------------------*/
MAT::InelasticDefgradFactors::InelasticDefgradFactors()
 : params_(NULL)
{
}
...
Teuchos::RCP<MAT::InelasticDefgradFactors> MAT::InelasticDefgradFactors::Factory(int matnum)
{
  ...
  case INPAR::MAT::mfi_lin_scalar_iso:
  {
    ...
    MAT::PAR::InelasticDefgradLinScalarIso* params = dynamic_cast<MAT::PAR::InelasticDefgradLinScalarIso*>(curmat->Parameter());
  ...
  case INPAR::MAT::mfi_lin_scalar_aniso:
  {
    ...
    MAT::PAR::InelasticDefgradLinScalarAniso* params = dynamic_cast<MAT::PAR::InelasticDefgradLinScalarAniso*>(curmat->Parameter());
 ...
 *--------------------------------------------------------------------*/
MAT::PAR::InelasticDefgradLinScalarIso::InelasticDefgradLinScalarIso(Teuchos::RCP<MAT::PAR::Material> matdata)
 : Parameter(matdata),
...
{
  // safety checks
  // in case not all scatra dofs are transported scalars, the last scatra dof is a potential and can not be treated as a concentration
  // but it is treated like that in so3_scatra_evaluate.cpp in the PreEvaluate method!
  if (Scalar1_ != 1)
  ...
  if (DRT::INPUT::IntegralValue<INPAR::STR::MassLin>(sdyn,"MASSLIN") != INPAR::STR::ml_none)
    dserror("If you use the material 'InelasticDefgradLinScalarIso' please set 'MASSLIN' in the "
 ...
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarIso::EvaluateInverseInelasticDefGrad(const LINALG::Matrix<3,3>* defgrad,  ///< Deformation gradient
                                                                        LINALG::Matrix<3,3> & iFinM)         ///< Inverse inelastic deformation gradient
{
  ...
  const double deltagrowth = Sc1growthfac*detjacobian*(concentrations_->at(gp_).at(Sc1-1) - Sc1refconc);
  const double isoinelasticdefo = pow(1.0 + deltagrowth, (1.0/3.0));

  // calculate inverse inelastic deformation gradient (FinM is modeled, such that the volume change is a linear function
  // of the scalar (mapped to reference frame) that causes it)
  for(int i=0;i<3;++i)
    iFinM(i,i) = 1.0/isoinelasticdefo;
 ...
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarIso::EvaluateAdditionalCmat(const LINALG::Matrix<3,3> * const defgrad,  ///< Deformation gradient
                                                               const LINALG::Matrix<3,3> & iFinjM,         ///< Inverse inelastic deformation gradient of current inelastic contribution as 3x3 matrix
                                                               const LINALG::Matrix<6,1> & iCV,            ///< Inverse right Cauchy-Green tensor
                                                               const LINALG::Matrix<6,9> & dSdiFinj,       ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. the inverse inelastic deformation gradient of current inelastic contribution
                                                               LINALG::Matrix<6,6> & cmatadd)              ///< Additional elasticity tensor
{
  ...
  // calculate scalefac
  const double temp = Sc1growthfac*detjacobian*(concentrations_->at(gp_).at(Sc1-1) - Sc1refconc);
 ...
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarIso::EvaluateODStiffMat(const LINALG::Matrix<3,3> * const defgrad,  ///< Deformation gradient
                                                           const LINALG::Matrix<3,3> & iFinjM,         ///< Inverse inelastic deformation gradient of current inelastic contribution as 3x3 matrix
                                                           const LINALG::Matrix<6,9> & dSdiFinj,       ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. the inverse inelastic deformation gradient of current inelastic contribution
                                                           LINALG::Matrix<6,1> & dstressdc)            ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. concentration
{
  static LINALG::Matrix<9,1> id9x1(true);
  // prepare id9x1 (identity matrix written as a 9x1 vector)
  for (int i=0; i<3; ++i)
  ...
  const double scalefac = -temp/(3.0*pow(1.0 + temp*(concentrations_->at(gp_).at(Sc1-1) - Sc1refconc),4.0/3.0));
 ...
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarIso::PreEvaluate(Teuchos::ParameterList& params)  ///< parameter list as handed in from the element
{
  ...
  if (gp_ == 0)
    concentrations_ = params.get< Teuchos::RCP<std::vector<std::vector<double> > > >("gp_conc",Teuchos::rcp(new std::vector<std::vector<double> >(30,std::vector<double>(20,0.0))));
   ...
   growthdirmat_(true)
{
  ...
  const Teuchos::ParameterList& sdyn = DRT::Problem::Instance()->StructuralDynamicParams();
  if (DRT::INPUT::IntegralValue<INPAR::STR::MassLin>(sdyn,"MASSLIN") != INPAR::STR::ml_none)
    dserror("If you use the material 'InelasticDefgradLinScalarAniso' please set 'MASSLIN' in the "
        "STRUCTURAL DYNAMIC Section to 'None', or feel free to implement other possibility!");
...
/*--------------------------------------------------------------------*
 ...
 | gradient                                             schmidt 03/18 |
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarAniso::EvaluateInverseInelasticDefGrad(const LINALG::Matrix<3,3>* defgrad,  ///< Deformation gradient
                                                                          LINALG::Matrix<3,3> & iFinM)         ///< Inverse inelastic deformation gradient
{
  ...
  // finalize inelastic deformation gradient matrix (FinM is calculated, such that the volume change is a linear function
  // of the scalar (mapped to reference frame) that causes it)
...
/*--------------------------------------------------------------------*
 | evaluate additional contribution to cmat             schmidt 03/18 |
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarAniso::EvaluateAdditionalCmat(const LINALG::Matrix<3,3> * const defgrad,  ///< Deformation gradient
                                                                 const LINALG::Matrix<3,3> & iFinjM,         ///< Inverse inelastic deformation gradient of current inelastic contribution as 3x3 matrix
                                                                 const LINALG::Matrix<6,1> & iCV,            ///< Inverse right Cauchy-Green tensor
                                                                 const LINALG::Matrix<6,9> & dSdiFinj,       ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. the inverse inelastic deformation gradient of current inelastic contribution
                                                                 LINALG::Matrix<6,6> & cmatadd)              ///< Additional elasticity tensor
{
  ...
  const double scalefac = -Sc1growthfac*detjacobian*(concentrations_->at(gp_).at(Sc1-1) - Sc1refconc)/2.0;
  ...
  // diFinjdC = - growthfac*det(F)*(c-c_{ref})/2.0 (F_{in,j}^{-1} . G . F_{in,j}^{-1}) \otimes C^{-1}
...
/*--------------------------------------------------------------------*
 ...
 | stiffness matrix (for monolithic calculation)        schmidt 03/18 |
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarAniso::EvaluateODStiffMat(const LINALG::Matrix<3,3> * const defgrad,  ///< Deformation gradient
                                                             const LINALG::Matrix<3,3> & iFinjM,         ///< Inverse inelastic deformation gradient of current inelastic contribution as 3x3 matrix
                                                             const LINALG::Matrix<6,9> & dSdiFinj,       ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. the inverse inelastic deformation gradient of current inelastic contribution
                                                             LINALG::Matrix<6,1> & dstressdc)            ///< Derivative of 2nd Piola Kirchhoff stresses w.r.t. concentration
{
  ...
  // diFinjdc = - growthfac*det(F) F_{in,j}^{-1} . G . F_{in,j}^{-1} with F_{in,j}, the j-th factor of F_{in}
...
/*--------------------------------------------------------------------*
 | pre evaluate                                         schmidt 03/18 |
 *--------------------------------------------------------------------*/
void MAT::InelasticDefgradLinScalarAniso::PreEvaluate(Teuchos::ParameterList& params)  ///< parameter list as handed in from the element
{
  // get Gauss point number
  gp_ = params.get<int>("gp",-1);
  if (gp_== -1)
