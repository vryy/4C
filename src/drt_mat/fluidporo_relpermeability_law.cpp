/*----------------------------------------------------------------------*/
/*!
 \file fluidporo_relpermeability_law.cpp
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
MAT::PAR::FluidPoroRelPermeabilityLaw* MAT::PAR::FluidPoroRelPermeabilityLaw::CreateRelPermeabilityLaw(int matID)
{
  ...
  case INPAR::MAT::m_fluidporo_relpermeabilitylaw_constant:
  {
    ...
    relpermeabilitylaw = static_cast<MAT::PAR::FluidPoroRelPermeabilityLawConstant*>(curmat->Parameter());
    break;
  }
  case INPAR::MAT::m_fluidporo_relpermeabilitylaw_exp:
  {
    ...
    relpermeabilitylaw = static_cast<MAT::PAR::FluidPoroRelPermeabilityLawExponent*>(curmat->Parameter());
    break;
  }
  default:
...
: FluidPoroRelPermeabilityLaw(matdata,true),
  relpermeability_(matdata->GetDouble("VALUE"))
{
  if(relpermeability_>1.0)
    dserror("relative permeability (actually the sum of the relative permeabilites) of phase cannot be greater than 1.0");
  return;
}
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
...
{
  if(exp_<=1.0)
    dserror("exponent in relative permeability phase law has to be bigger than 1.0");
