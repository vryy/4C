...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
MAT::Maxwell_0d_acinus_Ogden::Maxwell_0d_acinus_Ogden()
  :Maxwell_0d_acinus()
{
}
    ...
    double                                   dt)
{
  ...
  //Linear branches of the Maxwell model (Stiffness2(), B=R_t, B_a=R_a), notation according to interacinar dependency paper
  ...
  double Kq_npNL = (Viscosity1()/Stiffness2()) * (-kappa_*Vo/(pow(vi_np,2.0)*beta_) +
