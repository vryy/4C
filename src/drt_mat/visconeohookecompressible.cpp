/*!----------------------------------------------------------------------
\file visconeohookecompressible.cpp
\brief
...
#include <vector>
#include "visconeohookecompressible.H"
...
/*----------------------------------------------------------------------*
 |                                                                      |
 *----------------------------------------------------------------------*/
MAT::PAR::ViscoNeoHookeCompressible::ViscoNeoHookeCompressible(
...
Teuchos::RCP<MAT::Material> MAT::PAR::ViscoNeoHookeCompressible::CreateMaterial()
{
  return Teuchos::rcp(new MAT::ViscoNeoHookeCompressible(this));
}


MAT::ViscoNeoHookeCompressibleType MAT::ViscoNeoHookeCompressibleType::instance_;


DRT::ParObject* MAT::ViscoNeoHookeCompressibleType::Create( const std::vector<char> & data )
{
  MAT::ViscoNeoHookeCompressible* visco = new MAT::ViscoNeoHookeCompressible();
  visco->Unpack(data);
  return visco;
}
 ...
 *----------------------------------------------------------------------*/
MAT::ViscoNeoHookeCompressible::ViscoNeoHookeCompressible()
  : params_(NULL)
{
  isinit_=false;
 ...
 *----------------------------------------------------------------------*/
MAT::ViscoNeoHookeCompressible::ViscoNeoHookeCompressible(MAT::PAR::ViscoNeoHookeCompressible* params)
  : params_(params)
{
}
...
/*----------------------------------------------------------------------*
 |  Pack                                          (public)         05/08|
 *----------------------------------------------------------------------*/
void MAT::ViscoNeoHookeCompressible::Pack(DRT::PackBuffer& data) const
{
...
/*----------------------------------------------------------------------*
 |  Unpack                                        (public)         05/08|
 *----------------------------------------------------------------------*/
void MAT::ViscoNeoHookeCompressible::Unpack(const vector<char>& data)
{
  isinit_=true;
  ...
  if (DRT::Problem::Instance()->Materials() != Teuchos::null)
    if (DRT::Problem::Instance()->Materials()->Num() != 0)
    {
      ...
      if (mat->Type() == MaterialType())
        params_ = static_cast<MAT::PAR::ViscoNeoHookeCompressible*>(mat);
      else
 ...
 *----------------------------------------------------------------------*/
void MAT::ViscoNeoHookeCompressible::Setupvisco(const int numgp)
{
  histstresscurr_=rcp(new vector<LINALG::Matrix<NUM_STRESS_3D,1> >);
  artstresscurr_=rcp(new vector<LINALG::Matrix<NUM_STRESS_3D,1> >);
  histstresslast_=rcp(new vector<LINALG::Matrix<NUM_STRESS_3D,1> >);
 ...
 *----------------------------------------------------------------------*/
void MAT::ViscoNeoHookeCompressible::Update()
{
 ...
 *----------------------------------------------------------------------*/
void MAT::ViscoNeoHookeCompressible::Reset()
{
  // do nothing,
 ...
 *----------------------------------------------------------------------*/
void MAT::ViscoNeoHookeCompressible::Evaluate(const LINALG::Matrix<NUM_STRESS_3D,1>* glstrain,
                                  const int gp,
  ...
  *stress += Q;
  ...
  for (int i=0; i<6; ++i)
  {
     for (int j=0; j<6; ++j)
     {
       ...
       (*cmat)(i,j) += scalar1 * scalarvisco*Cinv(i) * Cinv(j) // add scalar Cinv x Cinv
