...
/*-----------------------------------------------------------------------------------------------*
 *-----------------------------------------------------------------------------------------------*/
double MAT::PAR::DetermineShearModulus(
    const Teuchos::RCP<MAT::PAR::Material>& matdata)
{
  ...
  {
    dserror("You specified both of the redundant material parameters SHEARMOD and POISSONRATIO! "
        "Specify exactly one of them in the material definition of your input file!");
  }
  else
  {
    dserror("You specified none of the material parameters SHEARMOD and POISSONRATIO! "
        "Specify exactly one of them in the material definition of your input file!");
  }
    ...
    const Teuchos::RCP<MAT::PAR::Material>& matdata)
{
  ...
  if ( radius == -1.0 and Iyy == Izz )
    radius = std::pow( 4.0 * Iyy / M_PI, 0.25 );
  ...
  radius_interaction_( DetermineDefaultInteractionRadius(matdata) )
{
  if ( youngs_modulus_ <= 0.0 )
    dserror("Young's modulus must be positive value");

  if ( shear_modulus_ <= 0.0 )
    dserror("shear modulus must be positive value");

  if ( density_ < 0.0 )
    dserror("density must not be negative value");

  if ( cross_section_area_ <= 0.0 )
    dserror("cross-section area must be positive value");

  if ( shear_correction_factor_ <= 0.0 )
    dserror("shear correction factor must be positive value");

  if ( area_moment_inertia_polar_ <= 0.0 )
    dserror("polar/axial area moment of inertia must be positive value");

  if ( area_moment_inertia_2_ <= 0.0 )
    dserror("area moment of inertia must be positive value");

  if ( area_moment_inertia_3_ <= 0.0 )
    dserror("area moment of inertia must be positive value");
  ...
  if ( radius_interaction_ != -1.0 and radius_interaction_ <= 0.0 )
    dserror("if specified (only required if any kind of beam interactions are considered and you "
...
{
  if ( axial_rigidity_ <= 0.0 )
    dserror("axial rigidity must be positive value");

  if ( shear_rigidity_2_ <= 0.0 or shear_rigidity_3_ <= 0.0 )
    dserror("shear rigidity must be positive value");

  if ( torsional_rigidity_ <= 0.0 )
    dserror("torsional rigidity must be positive value");
   ...
   * of all kinds of interactions. it can hence be ignored if no interaction are considered. */
  if ( radius_interaction_ != -1.0 and radius_interaction_ <= 0.0 )
    dserror("if specified (only required if any kind of beam interactions are considered), the "
        "given interaction radius must be a positive value");
}
...
/*-----------------------------------------------------------------------------------------------*
 *-----------------------------------------------------------------------------------------------*/
MAT::PAR::BeamKirchhoffTorsionFreeElastHyperMaterialParams::BeamKirchhoffTorsionFreeElastHyperMaterialParams(
  Teuchos::RCP<MAT::PAR::Material> matdata
  )
: BeamElastHyperMaterialParameterGeneric( matdata ),
  youngs_modulus_( matdata->GetDouble("YOUNG") ),
...
/*-----------------------------------------------------------------------------------------------*
 *-----------------------------------------------------------------------------------------------*/
MAT::PAR::BeamKirchhoffTorsionFreeElastHyperMaterialParamsByMode::BeamKirchhoffTorsionFreeElastHyperMaterialParamsByMode(
  Teuchos::RCP<MAT::PAR::Material> matdata
  )
: BeamElastHyperMaterialParameterGeneric( matdata ),
  axial_rigidity_( matdata->GetDouble("EA") ),
