      ...
      myintmethod_(*(matdata->Get<std::string>("METHOD")))
{
  if (DRT::Problem::Instance()->ProblemType() != prb_immersed_cell)
    dserror(
        ...
        "If you want to use it in a different context, have a look at the necessary setup in "
        "immersed_problem_dyn.cpp.");

  if (DRT::INPUT::IntegralValue<int>(
          DRT::Problem::Instance()->CellMigrationParams().sublist("STRUCTURAL DYNAMIC"),
          "MATERIALTANGENT"))
 ...
 *----------------------------------------------------------------------*/
void MAT::BioChemoMechanoCellActiveFiber::ResetStep() { matpassive_->ResetStep(); }  // ResetStep()
...
{
  const Teuchos::ParameterList& strucdynparams =
      DRT::Problem::Instance()->CellMigrationParams().sublist("STRUCTURAL DYNAMIC");
  ...
  const Teuchos::ParameterList& OneStepThetaParams =
      strucdynparams.get<Teuchos::ParameterList>("ONESTEPTHETA");
  ...
  if (time > 0.0)
  {
    if (testphi == Teuchos::null) dserror("testphi = Teuchos::null");
  }

  LINALG::Matrix<1, 8> myphinp;
  DRT::Element* scatraele = DRT::Problem::Instance()->GetDis("cellscatra")->gElement(eleGID);
  if (scatraele == NULL) dserror("could not get 'cellscatra' element with gid %d", eleGID);

  const Teuchos::RCP<DRT::Discretization> discretization =
      DRT::Problem::Instance()->GetDis("cellscatra");
  ...
  if (MyIntMethod == "2DTrapez")
  {
    ...
    for (int i = 0; i < nphi; i++)
    {
      ...
      strainratephi(i) = strainrate(0) * m(0) * m(0) + strainrate(1) * m(1) * m(1) +
                         2. * strainrate(3) * m(0) * m(1);
      if (Theta != 1.0)
      {
        strainratephilast(i) = strainratelast(0) * m(0) * m(0) + strainratelast(1) * m(1) * m(1) +
      ...
      b(i) = etalastphi(i) / dt + Theta * (kforwards * (cROCK)*cActin) +
             (1.0 - Theta) * (thetafac - Dlast);
    ...
    // if ((err3!=0) || (err2!=0))
    if (err3 != 0) dserror("solving linear system in cell contraction for new eta failed");
  ...
  else if (MyIntMethod == "2DGauss")
  {
    ...
    {
      std::cout << "For 2D Gauss integration the following numbers of integration points are "
                   "available: 20, 16, 10, 9, 8, 7 ,6, 5"
                << std::endl;
    ...
    for (int i = 0; i < nphi; i++)
    {
      ...
      // b(i)=etalastphi(i)/dt+Theta*(kforwards*(cROCK)-D) + (1.0-Theta)*(thetafac-Dlast);
      b(i) =
          etalastphi(i) / dt + Theta * (kforwards * (cROCK)) + (1.0 - Theta) * (thetafac - Dlast);
  ...
  else if (MyIntMethod == "3DGauss")
  {
    ...
    {
      std::cout << "For 3D Gauss integration the following numbers of integration points are "
                   "available: 20, 16, 10, 9, 8, 7 ,6, 5, 4"
                << std::endl;
    ...
    {
      std::cout << "For 3D Gauss integration over theta the following numbers of integration "
    ...
    for (int i = 0; i < nphiphi; i++)
    {
      for (int j = 0; j < ntheta; j++)
      {
        ...
        Nfilold += etalastphi(j + i * ntheta) * sin(theta) * gausspointsphi.qwgt[i] *
                   gausspointstheta.qwgt[j];
      }
    }
    ...
    for (int i = 0; i < nphiphi; i++)
    {
      for (int j = 0; j < ntheta; j++)
      {
        ...
        strainrate(j + i * ntheta) =
            strainrate(0) * m(0) * m(0) + strainrate(1) * m(1) * m(1) +
        ...
        if (Theta != 1.0)
        {
          strainratephilast(j + i * ntheta) =
              strainratelast(0) * m(0) * m(0) + strainratelast(1) * m(1) * m(1) +
              strainratelast(2) * m(2) * m(2) + 2. * strainratelast(3) * m(0) * m(1) +
              2. * strainratelast(4) * m(1) * m(2) + 2. * strainratelast(5) * m(0) * m(2);
        }
        ...
        for (int k = 0; k < nphiphi; k++)
        {
          for (int l = 0; l < ntheta; l++)
          {
            ...
            A(j + i * ntheta, l + k * ntheta) = Theta * newfac1 * gausspointsphi.qwgt[k] *
                                                gausspointstheta.qwgt[l] * sin(thetatemp);
          }
        }
        A(j + i * ntheta, j + i * ntheta) =
            1 / dt + Theta * gausspointsphi.qwgt[i] * gausspointstheta.qwgt[j] * newfac1;
        D = 0.;
        ...
        b(j + i * ntheta) = etalastphi(j + i * ntheta) / dt + Theta * (kforwards * (cROCK)-D) +
                            (1.0 - Theta) * (thetafac - Dlast);
    ...
    for (int i = 0; i < nphiphi; i++)
    {
      for (int j = 0; j < ntheta; j++)
      {
        ...
        m(1) = sin(theta) * sin(phi);
        m(2) = cos(theta);
        etanew(0, 0) += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                        gausspointstheta.qwgt[j] * sin(theta) * m(0) * m(0);
        etanew(1, 1) += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                        gausspointstheta.qwgt[j] * sin(theta) * m(1) * m(1);
        etanew(2, 2) += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                        gausspointstheta.qwgt[j] * sin(theta) * m(2) * m(2);
        etanew(0, 1) += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                        gausspointstheta.qwgt[j] * sin(theta) * m(0) * m(1);
        etanew(1, 2) += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                        gausspointstheta.qwgt[j] * sin(theta) * m(1) * m(2);
        etanew(0, 2) += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                        gausspointstheta.qwgt[j] * sin(theta) * m(0) * m(2);
        Nfil += M_PI * M_PI / 2.0 * etanewphi(j + i * ntheta) * gausspointsphi.qwgt[i] *
                gausspointstheta.qwgt[j] * sin(theta);
      }
    }
  ...
  {
    dserror(
        "Choose one of the following integration Methods for internal material law evaluation: "
        "2DTrapez, 2DGauss, 3DGauss");
  }
  ...
  else
  {
    if (time > 0)
    {
      ...
      if (error > 0) std::cout << "etanewphi" << etanewphi << std::endl;
      if (error > 0) dserror("Negative stress fiber concentration in ele %d", eleGID);
    }
    // ToDo Adpated for other Integration methods
    double deltaphi = 2.0 * M_PI / ((double)(nphi));
    // Calculate percentage
    if (Nfil < 0) dserror("Number of Filaments is smaller than zero NFil=%f", Nfil);
    ...
    if (Nfil > 0)
    {
      ...
      nsfdiagup_->at(gp) =
          deltaphi * (etanewphi(2) + etanewphi(3) + etanewphi(12) + etanewphi(13)) / Nfil;
      nsfdiagdown_->at(gp) =
          deltaphi * (etanewphi(7) + etanewphi(8) + etanewphi(17) + etanewphi(18)) / Nfil;
    }
    else
    {
      nsfhor_->at(gp) = 0.0;
  ...
  // Update history only when accessed due to material evaluation NOT via Scatra Boundary Dependent
  // Surface Integral
  if (Scatra != 1)
  {
...
void MAT::BioChemoMechanoCellActiveFiber::Dissociation(
    double rate, double& D, double k_B, double ratemax)
{
  /* D(rate)
     |
     |  \
...
void MAT::BioChemoMechanoCellActiveFiber::GPtoNodes(
    LINALG::Matrix<8, 1> invalue, LINALG::Matrix<8, 1>& outvalue)
{
  LINALG::Matrix<3, 1> xsi(true);
...
*-----------------------------------------------------------------------*/
void MAT::BioChemoMechanoCellActiveFiber::EtaGPtoNodes(LINALG::Matrix<nphi, 1>& node1,
    LINALG::Matrix<nphi, 1>& node2, LINALG::Matrix<nphi, 1>& node3, LINALG::Matrix<nphi, 1>& node4,
    LINALG::Matrix<nphi, 1>& node5, LINALG::Matrix<nphi, 1>& node6, LINALG::Matrix<nphi, 1>& node7,
...
void MAT::BioChemoMechanoCellActiveFiber::EtaAtBrdyGP(
    LINALG::Matrix<nphi, 1>& outvalue, Teuchos::ParameterList& params)
{
...
*-----------------------------------------------------------------------*/
void MAT::BioChemoMechanoCellActiveFiber::DefGradGPtoNodes(LINALG::Matrix<6, 1>& node1,
    LINALG::Matrix<6, 1>& node2, LINALG::Matrix<6, 1>& node3, LINALG::Matrix<6, 1>& node4,
    LINALG::Matrix<6, 1>& node5, LINALG::Matrix<6, 1>& node6, LINALG::Matrix<6, 1>& node7,
...
void MAT::BioChemoMechanoCellActiveFiber::DefGradAtBrdyGP(
    LINALG::Matrix<6, 1>& outvalue, Teuchos::ParameterList& params)
{
...
void MAT::BioChemoMechanoCellActiveFiber::cROCKlastAtBrdyGP(
    double& cROCKlast, Teuchos::ParameterList& params)
{
...
void MAT::BioChemoMechanoCellActiveFiber::cROCKusedAtBrdyGP(
    double& cROCKused, Teuchos::ParameterList& params)
{
  LINALG::Matrix<8, 1> cGP(true);
  LINALG::Matrix<8, 1> cNode(true);
...
void MAT::BioChemoMechanoCellActiveFiber::SetupRates(LINALG::Matrix<3, 3> defgrd,
 ...
 *----------------------------------------------------------------------*/
void MAT::BioChemoMechanoCellActiveFiber::CauchytoPK2(LINALG::Matrix<6, 1>& Sactive,
 ...
 *----------------------------------------------------------------------*/
void MAT::BioChemoMechanoCellActiveFiber::PK2toCauchy(LINALG::Matrix<6, 1> Sactive,
    LINALG::Matrix<3, 3>& PK2stress, LINALG::Matrix<3, 3> defgrd,
 ...
 *----------------------------------------------------------------------*/
bool MAT::BioChemoMechanoCellActiveFiber::VisData(
    const std::string& name, std::vector<double>& data, int numgp, int eleID)
{
  if (name == "Nfil")
  {
    ...
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += Nfil_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "RateAverage")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += clast_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "Drate")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += D_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "rate")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += rate_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "Nver")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += nsfver_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "Nhor")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += nsfhor_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "Ndiagup")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += nsfdiagup_()->at(iter);
    data[0] = temp / numgp;
  }
  else if (name == "Ndiagdown")
  {
    if ((int)data.size() != 1) dserror("size mismatch");
    double temp = 0.0;
    for (int iter = 0; iter < numgp; iter++) temp += nsfdiagdown_()->at(iter);
    data[0] = temp / numgp;
  }
  else
  {
