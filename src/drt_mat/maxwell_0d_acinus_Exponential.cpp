...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
MAT::Maxwell_0d_acinus_Exponential::Maxwell_0d_acinus_Exponential(MAT::PAR::Maxwell_0d_acinus* params)
...
/*----------------------------------------------------------------------*
 ...
 |                                                          roth 10/2014|
 *----------------------------------------------------------------------*/
 void MAT::Maxwell_0d_acinus_Exponential::Evaluate(
    ...
    double                                   dt)
{
  ...
  dpnpi_dt  = (e1_0_+2*e1_lin_*dvnp+e1_exp_*exp(tau_*dvnp)*(1+tau_*dvnp))*(dvnp-dvn)/dt;
  dpnpi2_dt = (2*e1_lin_+tau_*e1_exp_*exp(tau_*dvnp)*(1+tau_*dvnp) + e1_exp_*tau_*exp(tau_*dvnp))*(dvnp-dvn)/dt + (e1_0_+2*e1_lin_*dvnp+e1_exp_*exp(tau_*dvnp)*(1+tau_*dvnp))/dt;

  term_nonlin = pnpi + pnpi2*(-(dvnp) +(qn/NumOfAcini)*dt/2 + dvn);
  kq_np = kq_np + pnpi2/2*dt;
  term_nonlin = term_nonlin + dpnpi_dt*Viscosity1()/Stiffness2()  + dpnpi2_dt*Viscosity1()/Stiffness2() *(-(dvnp)+(qnp/NumOfAcini)*dt/2 + dvn);
