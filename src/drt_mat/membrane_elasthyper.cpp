...
void MAT::Membrane_ElastHyper::Evaluate(
    ...
    LINALG::Matrix<3,3>& Q_trafo,        ///< Trafo from local membrane orthonormal coordinates to global coordinates
 ...
 *----------------------------------------------------------------------*/
void MAT::Membrane_ElastHyper::EvaluateIsotropicStressCmat(
    LINALG::Matrix<3,1>& stress_iso,
    LINALG::Matrix<3,3>& cmat_iso,
    const LINALG::Matrix<3,1>& id2,
    const LINALG::Matrix<3,3>& id4sharp,
    const LINALG::Matrix<3,1>& rcg,
    const double& rcg33,
    const LINALG::Matrix<3,1>& icg,
    int eleGID
    )
{
  ...
  )
{
  ...
  prinv(1) = 0.5 * ( std::pow(prinv(0),2.0) - (std::pow(rcg(0),2.0) + std::pow(rcg(1),2.0) + std::pow(rcg33,2.0) + 2.0*std::pow(rcg(2),2.0)) );
 ...
 *----------------------------------------------------------------------*/
void MAT::Membrane_ElastHyper::EvaluateInvariantDerivatives(
    const LINALG::Matrix<3,1>& prinv,
    LINALG::Matrix<2,1>& dPI,
    LINALG::Matrix<3,1>& ddPII,
    int eleGID
    )
{
  ...
  // the derivatives dPI_full(2), ddPII_full(2), ddPII_full(3) and ddPII_full(4) are w.r.t. prinv(2) = 1.0
  // and thus not needed in this formulation
  dPI(0) = dPI_full(0);
  dPI(1) = dPI_full(1);
 ...
 *----------------------------------------------------------------------*/
void MAT::Membrane_ElastHyper::CalculateGammaDelta(
    LINALG::Matrix<3,1>& gamma,
    LINALG::Matrix<8,1>& delta,
    const LINALG::Matrix<3,1>& prinv,
    const LINALG::Matrix<2,1>& dPI,
    const LINALG::Matrix<3,1>& ddPII,
    const double rcg33
    )
{
  ...
  delta(2) = -4.0*rcg33*(ddPII(0) + prinv(0)*ddPII(2) + dPI(1) + (prinv(0)-rcg33)*(ddPII(2)+prinv(0)*ddPII(1)));
  delta(3) = 4.0*ddPII(1);
  delta(4) = 4.0*rcg33*(ddPII(2) + (prinv(0)-rcg33)*ddPII(1));
  delta(5) = -2.0*gamma(2) + 4.0*rcg33*rcg33*(ddPII(0) + 2.0*(prinv(0)-rcg33)*ddPII(2) + std::pow((prinv(0)-rcg33),2.0)*ddPII(1));
  delta(6) = -2.0*gamma(2);
  delta(7) = 2.0*gamma(1);
...
void MAT::Membrane_ElastHyper::EvaluateAnisotropicStressCmat(
    LINALG::Matrix<3,1>& stress_aniso,
    LINALG::Matrix<3,3>& cmat_aniso,
    ...
    )
{
  ...
  for (unsigned int p=0; p<potsum_.size(); ++p)
  {
    ...
    potsum_[p]->AddStressAnisoPrincipal(rcg_full,cmat_aniso_full,stress_aniso_full,params,eleGID);
    ...
    cmat_aniso_red(0,0) = cmat_aniso_full(0,0);    cmat_aniso_red(0,1) = cmat_aniso_full(0,1);    cmat_aniso_red(0,2) = cmat_aniso_full(0,3);
    cmat_aniso_red(1,0) = cmat_aniso_full(1,0);    cmat_aniso_red(1,1) = cmat_aniso_full(1,1);    cmat_aniso_red(1,2) = cmat_aniso_full(1,3);
    cmat_aniso_red(2,0) = cmat_aniso_full(3,0);    cmat_aniso_red(2,1) = cmat_aniso_full(3,1);    cmat_aniso_red(2,2) = cmat_aniso_full(3,3);
