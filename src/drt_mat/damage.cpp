...
/*----------------------------------------------------------------------*
 | evaluate material (public)                                dano 08/11 |
 *----------------------------------------------------------------------*/
void MAT::Damage::EvaluateSimplifiedLemaitre(
  const LINALG::Matrix<3,3>* defgrd,
  const LINALG::Matrix<NUM_STRESS_3D,1>* linstrain,  // linear strain vector
  Teuchos::ParameterList& params,  // parameter list for communication & HISTORY
  LINALG::Matrix<NUM_STRESS_3D,1>* stress,  // 2nd PK-stress
 ...
 *----------------------------------------------------------------------*/
void MAT::Damage::EvaluateFullLemaitre(
  const LINALG::Matrix<3,3>* defgrd,
  const LINALG::Matrix<NUM_STRESS_3D,1>* linstrain,  // linear strain vector
  Teuchos::ParameterList& params,  // parameter list for communication & HISTORY
  LINALG::Matrix<NUM_STRESS_3D,1>* stress,  // 2nd PK-stress
  ...
  int EleGID)
{
  ...
  if (Phi_trial > 1.0e-08)  // if (Phi_trial > 0.0)
  {
    ...
    while (true)
    {
      ...
      if ( (norm_k_s_tilde < params_->abstol_) and (norm_Phi < params_->abstol_)
           and (norm_k_beta < params_->abstol_) and (norm_k_D < params_->abstol_)
         )
      {
        if (omega < 0.0)
        {
      ...
      double N_ksb = 0.0;
      for (int i=0; i<6; ++i)
        N_ksb += N_tilde(i) * (k_s_tilde(i) - k_b(i));
      ...
      for (int i=0; i<6; ++i)
        dy_dstildek_NkN += dy_dstilde(i,0) * k_NkN(i,0);
      ...
      //          / (1 + 3/2 . g) . Hkin_rec / (1 + Hkin_rec . Dgamma)^2
      //          . dydsb_NbetaoldN
      h_alg = 3.0 * G + ( omega - y * Dgamma / omega + s_N )
      ...
      for (int i=0; i<6; ++i)
        dyds_matrixcD += dy_dstilde(i,0) * matrix_c_D(i,0);
