...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
MAT::ScatraMatVarChemDiffusion::ScatraMatVarChemDiffusion() : params_(NULL) {}
 ...
 *---------------------------------------------------------------------------*/
MAT::PAR::Models MAT::PAR::ScatraMatVarChemDiffusion::StringToModel(
    const std::string& modelstring) const
{
  Models modelenum(chemdiff_undefined);

  // linear mass action law + Ficks law
  if (modelstring == "Linear") modelenum = chemdiff_linear;
  ...
  switch (params_->chemdiff_model_)
  {
    ...
    case MAT::PAR::chemdiff_linear:
    {
      ...
      switch (orderderivative)
      {
        ...
        case 0:
        {
          IntEnergy = refMu * concentration +
                      (1 / 2) * (rt / refC) * (concentration - refC) * (concentration - refC);
          break;
        }
    ...
    case MAT::PAR::chemdiff_fickean:
    {
      ...
      switch (orderderivative)
      {
        ...
        case 0:
        {
          ...
          IntEnergy =
              refMu * concentration + (rt) * (concentration * log(arg_log) - concentration + refC);
          break;
        }
        // Computes internal energy first derivative
        case 1:
        {
          ...
          if (arg_log < 0)
          {
            std::cout << "Concentration value is " << concentration << std::endl;
            dserror(
                "Negative argument for logarithm while computing Internal energy first derivative");
          }
  ...
  switch (params_->chemdiff_model_)
  {
    ...
    case MAT::PAR::chemdiff_linear:
    {
      ...
      switch (orderderivative)
      {
        ...
        case 0:
        {
          DualIntEnergy = (ChemPot - refMu) * refC +
                          (1 / 2) * (refC / rt) * (ChemPot - refMu) * (ChemPot - refMu);
          break;
        }
        // Computes internal energy first derivative
        case 1:
...
{
  if (params_->chemdiff_model_ == MAT::PAR::chemdiff_linear) concentration = refC;
    ...
    ) const
{
  ...
  if (params_->chemdiff_model_ == MAT::PAR::chemdiff_linear) concentration = refC;
  for (int d = 0; d < NSD; ++d) DissipationPot2deriv(d, d) = (concentration * mobility);
  return;
}

// ComputeDissipationPot
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot<1>(
    double, const double&, const LINALG::Matrix<1, 1>&, const double&, double&) const;  //
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot<2>(
    double, const double&, const LINALG::Matrix<2, 1>&, const double&, double&) const;
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot<3>(
    double, const double&, const LINALG::Matrix<3, 1>&, const double&, double&) const;
// ComputeDissipationPot_D1
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot_D1<1>(
    double, const double&, const LINALG::Matrix<1, 1>&, const double&, LINALG::Matrix<1, 1>&) const;
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot_D1<2>(
    double, const double&, const LINALG::Matrix<2, 1>&, const double&, LINALG::Matrix<2, 1>&) const;
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot_D1<3>(
    double, const double&, const LINALG::Matrix<3, 1>&, const double&, LINALG::Matrix<3, 1>&) const;
// ComputeDissipationPot_D2
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot_D2<1>(
    double, const double&, const LINALG::Matrix<1, 1>&, const double&, LINALG::Matrix<1, 1>&) const;
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot_D2<2>(
    double, const double&, const LINALG::Matrix<2, 1>&, const double&, LINALG::Matrix<2, 2>&) const;
template void MAT::ScatraMatVarChemDiffusion::ComputeDissipationPot_D2<3>(
    double, const double&, const LINALG::Matrix<3, 1>&, const double&, LINALG::Matrix<3, 3>&) const;
