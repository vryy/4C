/*----------------------------------------------------------------------*/
/*!
\file maxwell_0d_acinus_DoubleExponential.cpp
...
{
  MAT::Maxwell_0d_acinus_DoubleExponential* mxwll_0d_acin = new MAT::Maxwell_0d_acinus_DoubleExponential();
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
MAT::Maxwell_0d_acinus_DoubleExponential::Maxwell_0d_acinus_DoubleExponential(MAT::PAR::Maxwell_0d_acinus* params)
...
/*----------------------------------------------------------------------*
 ...
 |                                                          roth 10/2014|
 *----------------------------------------------------------------------*/
 void MAT::Maxwell_0d_acinus_DoubleExponential::Evaluate(
    ...
    double                                   dt)
{
  ...
  dpnpi_dt  = (e1_01_ +2.0*e1_lin1_ *dvnp+e1_exp1_ *exp(tau1_ *dvnp)*(1.0+tau1_ *dvnp))*(dvnp-dvn)/dt;
  dpnpi_dt += (e1_02_+2.0*e1_lin2_*dvnp+e1_exp2_*exp(tau2_*dvnp)*(1.0+tau2_*dvnp))*(dvnp-dvn)/dt;

  dpnpi2_dt = (2.0*e1_lin1_ +tau1_ *e1_exp1_ *exp(tau1_ *dvnp)*(1.0+tau1_ *dvnp) + e1_exp1_ *tau1_ *exp(tau1_ *dvnp))*(dvnp-dvn)/dt + (e1_01_ +2.0*e1_lin1_ *dvnp+e1_exp1_ *exp(tau1_ *dvnp)*(1.0+tau1_ *dvnp))/dt;
  dpnpi2_dt+= (2.0*e1_lin2_+tau2_*e1_exp2_*exp(tau2_*dvnp)*(1.0+tau2_*dvnp) + e1_exp2_*tau2_*exp(tau2_*dvnp))*(dvnp-dvn)/dt + (e1_02_+2.0*e1_lin2_*dvnp+e1_exp2_*exp(tau2_*dvnp)*(1.0+tau2_*dvnp))/dt;

  // Add up the nonlinear terms
  term_nonlin = pnpi + pnpi2*(-(dvnp) +(qn/NumOfAcini)*dt/2.0 + dvn);
  kq_np = kq_np + pnpi2/2.0*dt;
  term_nonlin = term_nonlin + dpnpi_dt*Viscosity1()/Stiffness2()  + dpnpi2_dt*Viscosity1()/Stiffness2() *(-(dvnp)+(qnp/NumOfAcini)*dt/2.0 + dvn);
