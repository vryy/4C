...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
MAT::PAR::ScatraChemotaxisMat::ScatraChemotaxisMat(
  ...
  chemocoeff_(matdata->GetDouble("CHEMOCOEFF"))
{
  ...
  if (numscal_ != (int)pair_->size())
    dserror("number of materials %d does not fit to size of material vector %d", numscal_, pair_->size());
  ...
  if (numpos != 1 or numneg != 1)
    dserror("Each PAIR vector must contain exactly one '-1' (i.e. chemotractant) and exactly one '1' (i.e. attractant)!");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
MAT::ScatraChemotaxisMat::ScatraChemotaxisMat()
  : params_(NULL)
{
}
