...
//TODO: combine this with scatra_dyn and use the ScatraAlgorithm instead of the XFLUIDLEVELSET::Algorithm
...
void fluid_xfem_ls_drt(int restart)
{
  ...
  std::map<std::string, int> dofset_coupling_map; ///< key=meaning of a dofset in a dis, nodal dofset number
  ...
  // define abbreviation
   ...
   if (fluiddis->NumGlobalNodes()==0)
     dserror("Fluid discretization is empty!");
   ...
   if (fieldcoupling == INPAR::SCATRA::coupling_match)
   {
     ...
     const int nds_fluid_proxy_in_scatra = scatradis->AddDofSet(xfluiddis->GetInitialDofSetProxy(xfluid_dofset));
     ...
     dofset_coupling_map.insert(std::pair<std::string,int>("vel_fluid_proxy_in_scatra",nds_fluid_proxy_in_scatra));

     // add proxy of scatra level set degrees of freedom to fluid discretization
     const int nds_scatra_proxy_in_fluid = xfluiddis->AddDofSet(scatradis->GetDofSetProxy(cutter_nds_phi));
     if(nds_scatra_proxy_in_fluid != 1)
       dserror("Fluid discretization has illegal number of dofsets!");

     dofset_coupling_map.insert(std::pair<std::string,int>("phi_scatra_proxy_in_fluid",nds_scatra_proxy_in_fluid));
   }
   else
     dserror("not supported so far");
   ...
   // Test replacing fdyn in Algorithm with prbdyn
   Teuchos::RCP<XFLUIDLEVELSET::Algorithm> algo = Teuchos::rcp(new XFLUIDLEVELSET::Algorithm(
       comm,
       twophasedyn,
       DRT::Problem::Instance()->SolverParams(linsolvernumber),
       dofset_coupling_map
...
void DoSafetyCheck()
{
  ...
  // Safety check for xfem coupling condition and parameters set in scatra field.
  {
    ...
    // check for two-phase flow and combustion interfaces - however, we allow just one interface so far
    ...
    {
      if(std::find(names.begin(), names.end(), conditions_to_check[c]) != names.end()) // condition found
      {
        std::vector<DRT::Condition*> conditions;
        fluiddis->GetCondition( conditions_to_check[c], conditions);
        if(conditions.size()!= 1)
          dserror("number of conditions not unique, are there multiple conditions of the same type? - not allowed so far!");

        // check for unique interface
        if(condition_found == false)
          condition_found = true;
        else
          dserror("did you specify a TWOPHASE and a COMBUSTION condition at the same time? - not allowed so far!");
    ...
    if(condition_found == false)
      dserror("No DESIGN XFEM LEVELSET TWOPHASE or COMBUSTION VOL CONDITIONS provided, please add to input file.");
  }
}
