...
{
  INT        	i;		  /* a counter  		                        */
  INT      numaf;            /* actual number ale field                            */
...
/*---------------- put actual min and max stiffening factor in place ---*/
  work->min = 0.10;
  work->max = 500.0;
  work->min_stiff = work->min;
  work->max_stiff = work->max;
  ...
  )
{
  ...
  /*---------------- put actual min and max stiffening factor in place ---*/
  container.min = work->min;
  container.max = work->max;
  container.min_stiff = work->min_stiff;
  container.max_stiff = work->max_stiff;
  ...
  if (par.myrank==0)
  {
    printf("Solving ALE (in two steps)...\n");
    printf("\n");
  }
...
/*-call element routines to calculate & assemble stiffness matrix */
  *action = calc_ale_stiff_step2;
  ...
  work->min = container.min;
  work->max = container.max;
  work->min_stiff = container.min_stiff;
  work->max_stiff = container.max_stiff;
...
pseude structure. The calculation is performed in two solutions per time
step. The elemental stiffness factos used in the second step depend on
the distortion evaluated in the first one. Displacements are prescribed at
the fluid structure interface and at the free surface as Dirichlet boundary
conditions.
 ...
 *======================================================================*/
case 6:
  fsi_ale_2step_sd(work,actfield,disnum_calc,disnum_io);
  break;
