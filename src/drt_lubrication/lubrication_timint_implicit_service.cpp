/*--------------------------------------------------------------------------*/
/*!
\file lubrication_timint_implicit_service.cpp

\brief Service routines of the lubrication time integration class
 ...
 *----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> LUBRICATION::TimIntImpl::CalcFlux(const bool writetofile, const int num)
{
  switch(writeflux_)
  {
  ...
  case INPAR::SCATRA::flux_convective_boundary:
  {
    ...
    condnames.push_back("LubricationFluxCalc");
 ...
 *----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> LUBRICATION::TimIntImpl::CalcFluxInDomain
(const INPAR::SCATRA::FluxType fluxtype)
{
  ...
  return flux;
} // LUBRICATION::TimIntImpl::CalcFluxInDomain


/*----------------------------------------------------------------------*
 |  calculate mass / heat normal flux at specified boundaries  gjb 06/09|
 *----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> LUBRICATION::TimIntImpl::CalcFluxAtBoundary(
    std::vector<std::string>& condnames,
    ...
    bool fixtotflux)
{
  ...
  for (unsigned int i=0; i < condnames.size(); i++)
  {
    ...
    for (int condid = 0; condid < (int) cond.size(); condid++)
    {
      ...
      int addflux=0;
      if(condnames[i]=="LubricationFluxCalc")
      {
  ...
  return flux;
} // LUBRICATION::TimIntImpl::CalcFluxAtBoundary


/*--------------------------------------------------------------------*
 | calculate initial time derivatives of state variables   fang 03/15 |
 *--------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::CalcInitialTimeDerivative()
{
  // time measurement
  TEUCHOS_FUNC_TIME_MONITOR("LUBRICATION:       + calculate initial time derivative");

  // initial screen output
  if(myrank_ == 0)
    std::cout << "LUBRICATION: calculating initial time derivative of state variables on discretization \"" << discret_->Name().c_str() << "\" (step " << Step() <<", time " << Time() << ") ... ... ";
  ...
  // M predt^0 = f^n - K\pre^n - C(u_n)\pre^n
  ...
  solver_->Solve(sysmat_->EpetraOperator(),predtnp_,residual_,true,true);

  // copy solution
  predtn_->Update(1.0,*predtnp_,0.0);
  ...
  return;
} // LUBRICATION::TimIntImpl::CalcInitialTimeDerivative


/*---------------------------------------------------------------------------*
 | compute nodal density values from nodal concentration values   fang 12/14 |
 *---------------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::ComputeDensity()
{
  ...
  for(int lnodeid=0; lnodeid<discret_->NumMyRowNodes(); ++lnodeid)
  {
    ...
    for(int k=0; k<numscal_; ++k)
    {
      ...
      const int localdofid = Preafnp()->Map().LID(globaldofid);
      if (localdofid < 0)
        dserror("Local dof ID not found in dof map!");

      // add contribution of scalar k to nodal density value
      density += densific_[k] * ((*(Preafnp()))[localdofid] - c0_[k]);
    }
  ...
  return;
} // LUBRICATION::TimIntImpl::ComputeDensity
 ...
 *--------------------------------------------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::OutputDomainOrBoundaryIntegrals(const std::string condstring)
{
  ...
  return;
} // LUBRICATION::TimIntImpl::OutputDomainOrBoundaryIntegrals


/*----------------------------------------------------------------------*
 | Evaluate surface/interface permeability for FS3I          Thon 11/14 |
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::SurfacePermeability(
    Teuchos::RCP<LINALG::SparseOperator> matrix,
    Teuchos::RCP<Epetra_Vector>          rhs)
{
  // time measurement: evaluate condition 'SurfacePermeability'
  TEUCHOS_FUNC_TIME_MONITOR("LUBRICATION:       + evaluate condition 'LubricationCoupling'");
  ...
  //Evaluate condition
  discret_->EvaluateCondition(condparams,matrix,Teuchos::null,rhs,Teuchos::null,Teuchos::null,"LubricationCoupling");
  discret_->ClearState();

  matrix->Complete();

  return;
} // LUBRICATION::TimIntImpl::SurfacePermeability
 ...
 *----------------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::KedemKatchalsky(
    Teuchos::RCP<LINALG::SparseOperator> matrix,
    Teuchos::RCP<Epetra_Vector> rhs
    )
{
  ...
  return;
} // LUBRICATION::TimIntImpl::KedemKatchalsky
...
)
{
  ...
  for(int k=0;k<numscal_;++k)
  {
    std::ostringstream temp;
    temp << k;
    std::string name = "flux_pre_"+temp.str();
  ...
  return;
} // LUBRICATION::TimIntImpl::AddFluxApproxToParameterList


/*----------------------------------------------------------------------*
 | compute outward pointing unit normal vectors at given b.c.  gjb 01/09|
 *----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> LUBRICATION::TimIntImpl::ComputeNormalVectors(
...
)
{
  ...
  return normal;
} // LUBRICATION:: TimIntImpl::ComputeNormalVectors

/*----------------------------------------------------------------------*
 | evaluate Neumann inflow boundary condition                  vg 03/09 |
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::ComputeNeumannInflow(
    Teuchos::RCP<LINALG::SparseOperator> matrix,
    Teuchos::RCP<Epetra_Vector>          rhs)
{
  // time measurement: evaluate condition 'Neumann inflow'
  TEUCHOS_FUNC_TIME_MONITOR("LUBRICATION:       + evaluate condition 'TransportNeumannInflow'");
  ...
  return;
} // LUBRICATION::TimIntImpl::ComputeNeumannInflow

/*----------------------------------------------------------------------*
 | evaluate boundary cond. due to convective heat transfer     vg 10/11 |
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::EvaluateConvectiveHeatTransfer(
    Teuchos::RCP<LINALG::SparseOperator> matrix,
    Teuchos::RCP<Epetra_Vector>          rhs)
{
  // time measurement: evaluate condition 'ThermoConvections'
  TEUCHOS_FUNC_TIME_MONITOR("LUBRICATION:       + evaluate condition 'ThermoConvections'");
  ...
  return;
} // LUBRICATION::TimIntImpl::EvaluateConvectiveHeatTransfer
...
//    // draw scalar field 'Prendtp' for every element
...
//    // draw scalar field 'Prendtn' for every element
 ...
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::OutputFlux(Teuchos::RCP<Epetra_MultiVector> flux)
{
  //safety check
...
void LUBRICATION::TimIntImpl::OutputIntegrReac(const int num)
{
  if (outintegrreac_)
  {
    ...
    for (int k=0; k<numscal_; ++k)
      prenp_->Map().Comm().SumAll(&((*myreacnp)[k]),&intreacterm[k],1);
    discret_->ClearState();   // clean up
  ...
  return;
} // LUBRICATION::TimIntImpl::OutputIntegrReac
 ...
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::AVM3Preparation()
{
  ...
  LINALG::ApplyDirichlettoSystem(sysmat_sd_,prenp_,residual_,prenp_,*(dbcmaps_->CondMap()));
  ...
  return;
} // TimIntImpl::AVM3Preparation

/*----------------------------------------------------------------------*
 |  scaling of AVM3-based subgrid-diffusivity matrix           vg 10/08 |
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::AVM3Scaling(Teuchos::ParameterList& eleparams)
{
  // time measurement: avm3
  TEUCHOS_FUNC_TIME_MONITOR("LUBRICATION:            + avm3");
 ...
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::AccessDynSmagFilter(
...
)
{
  ...
  // by the fluid lubrication coupling algorithm
  // therefore, we only have to add some lubrication specific parameters here
  if (turbmodel_ == INPAR::FLUID::dynamic_smagorinsky)
  {
    if (myrank_ == 0)
    {
      std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
      std::cout << "Dynamic Smagorinsky model: provided access for Lubrication       " << std::endl;
      std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
    }
 ...
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::AccessVreman(
...
)
{
  ...
  if (turbmodel_ == INPAR::FLUID::dynamic_vreman)
  {
    ...
    {
      std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
      std::cout << "Dynamic Vreman model: provided access for Lubrication            " << std::endl;
      std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
    }
 ...
 *-------------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::RecomputeMeanCsgsB()
{
 ...
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::CalcIntermediateSolution()
{
  if (special_flow_ == "lubrication_forced_homogeneous_isotropic_turbulence" and
...
/*----------------------------------------------------------------------*/
/* set lubrication-fluid displacement vector due to biofilm growth          */
void LUBRICATION::TimIntImpl::SetScFldGrDisp(Teuchos::RCP<Epetra_MultiVector> lubrication_fluid_growth_disp)
{
  scfldgrdisp_= lubrication_fluid_growth_disp;

  return;
}

/*----------------------------------------------------------------------*/
/* set lubrication-structure displacement vector due to biofilm growth          */
void LUBRICATION::TimIntImpl::SetScStrGrDisp(Teuchos::RCP<Epetra_MultiVector> lubrication_struct_growth_disp)
{
  scstrgrdisp_= lubrication_struct_growth_disp;
...
/*==========================================================================*/
//! Calculate the reconstructed nodal gradient of pre
Teuchos::RCP<Epetra_MultiVector> LUBRICATION::TimIntImpl::GetSmoothedGradientAtNodes(const Teuchos::RCP<const Epetra_Vector> pre)
{
  Teuchos::RCP<Epetra_MultiVector> gradpre = Teuchos::rcp(new Epetra_MultiVector(*(discret_->DofRowMap()), 3, true));

  ReconstructGradientAtNodes(gradpre,pre);
  return gradpre;
}


/*----------------------------------------------------------------------*
 | Calculate the reconstructed nodal gradient of pre        winter 04/14|
 *----------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::ReconstructGradientAtNodes(
    Teuchos::RCP<Epetra_MultiVector> gradPre,
    const Teuchos::RCP<const Epetra_Vector> pre)
{
  // zero out matrix entries
  sysmat_->Zero();
  ...
  for (int idim=0; idim<3 ; idim++)
    solver_->Solve(sysmat_->EpetraOperator(),Teuchos::rcp((*gradPre)(idim),false),Teuchos::rcp((*rhs_vectors)(idim),false),true,true);
 ...
 *--------------------------------------------------------------------------------------------------------------------*/
bool LUBRICATION::TimIntImpl::ConvergenceCheck(int          itnum,
                                                int          itmax,
                                                const double ittol)
{
  ...
  double pre1incnorm_L2(0.0);
  double pre2incnorm_L2(0.0);
  double pre1norm_L2(0.0);
  double pre2norm_L2(0.0);
  ...
  if (numscal_ == 2)
  {
    ...
    vec1->Norm2(&pre1incnorm_L2);
    vec2->Norm2(&pre2incnorm_L2);

    vec1 = splitter_->ExtractOtherVector(prenp_);
    vec2 = splitter_->ExtractCondVector(prenp_);
    vec1->Norm2(&pre1norm_L2);
    vec2->Norm2(&pre2norm_L2);

    // check for any INF's and NaN's
    if (std::isnan(res1norm_L2) or
        std::isnan(pre1incnorm_L2) or
        std::isnan(pre1norm_L2) or
        std::isnan(res2norm_L2) or
        std::isnan(pre2incnorm_L2) or
        std::isnan(pre2norm_L2))
      dserror("At least one of the calculated vector norms is NaN.");

    if (std::isinf(res1norm_L2) or
        std::isinf(pre1incnorm_L2) or
        std::isinf(pre1norm_L2) or
        std::isinf(res2norm_L2) or
        std::isinf(pre2incnorm_L2) or
        std::isinf(pre2norm_L2))
      dserror("At least one of the calculated vector norms is INF.");

    // for scalar norm being (close to) zero, set to one
    if (pre1norm_L2 < 1e-5) pre1norm_L2 = 1.0;
    if (pre2norm_L2 < 1e-5) pre2norm_L2 = 1.0;
    ...
    if (myrank_==0)
    {
      ...
      printf("|  %3d/%3d   | %10.3E[L_2 ]  |  %10.3E   |  %10.3E   |  %10.3E   |  %10.3E   |",
           itnum,itmax,ittol,res1norm_L2,pre1incnorm_L2/pre1norm_L2,res2norm_L2,pre2incnorm_L2/pre2norm_L2);
      printf("\n");
      printf("+------------+-------------------+---------------+---------------+---------------+---------------+\n");
    }

     if ((res1norm_L2 <= ittol) and
        (pre1incnorm_L2/pre1norm_L2 <= ittol) and
         (res2norm_L2 <= ittol) and
        (pre2incnorm_L2/pre2norm_L2 <= ittol))
       stopnonliniter=true;

    // warn if itemax is reached without convergence, but proceed to next timestep
    if ((itnum == itmax) and
        ((res1norm_L2 > ittol) or (pre1incnorm_L2/pre1norm_L2 > ittol) or
         (res2norm_L2 > ittol) or (pre2incnorm_L2/pre2norm_L2 > ittol)))
    {
      stopnonliniter=true;
      if (myrank_==0)
      {
  ...
  else if (numscal_ == 1)
  {
    ...
    increment_->Norm2(&pre1incnorm_L2);
    prenp_    ->Norm2(&pre1norm_L2);

    // check for any INF's and NaN's
    if (std::isnan(res1norm_L2) or
        std::isnan(pre1incnorm_L2) or
        std::isnan(pre1norm_L2))
      dserror("At least one of the calculated vector norms is NaN.");

    if (std::isinf(res1norm_L2) or
        std::isinf(pre1incnorm_L2) or
        std::isinf(pre1norm_L2))
      dserror("At least one of the calculated vector norms is INF.");
    ...
    if (myrank_==0)
    {
      ...
      printf("|  %3d/%3d   | %10.3E[L_2 ]  | %10.3E   | %10.3E   |",
           itnum,itmax,ittol,res1norm_L2,pre1incnorm_L2/pre1norm_L2);
      printf("\n");
      printf("+------------+-------------------+--------------+--------------+\n");
    }

     if ((res1norm_L2 <= ittol) and
        (pre1incnorm_L2/pre1norm_L2 <= ittol)) stopnonliniter=true;

    // warn if itemax is reached without convergence, but proceed to next timestep
    if ((itnum == itmax) and
        ((res1norm_L2 > ittol) or (pre1incnorm_L2/pre1norm_L2 > ittol)))
    {
      stopnonliniter=true;
      if (myrank_==0)
      {
  ...
  else dserror("Lubrication convergence check for number of scalars other than one or two not yet supported!");

  return stopnonliniter;
} // LUBRICATION::TimIntImplicit::ConvergenceCheck


/*----------------------------------------------------------------------------------------------*
 | finite difference check for scalar transport system matrix (for debugging only)   fang 10/14 |
 *----------------------------------------------------------------------------------------------*/
void LUBRICATION::TimIntImpl::FDCheck()
{
  // initial screen output
  if(myrank_ == 0)
    std::cout << std::endl << "FINITE DIFFERENCE CHECK FOR LUBRICATION SYSTEM MATRIX" << std::endl;

  // make a copy of state variables to undo perturbations later
  Teuchos::RCP<Epetra_Vector> prenp_original = Teuchos::rcp(new Epetra_Vector(*prenp_));
  ...
  for (int col=0; col<sysmat_original->NumGlobalCols(); ++col)
  {
    ...
    prenp_->Update(1.,*prenp_original,0.);

    // impose perturbation
    prenp_->SumIntoGlobalValue(col,0,fdcheckeps_);
