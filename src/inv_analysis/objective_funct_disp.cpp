...
INVANA::ObjectiveFunctDisp::ObjectiveFunctDisp(Teuchos::RCP<DRT::Discretization> discret)
    : discret_(discret), dofrowmap_(discret->DofRowMap()), scalefac_(1.0)
{
  ...
  if (scaling_) scalefac_ = 1.0 / sqrt(mstate_->GlobalLength());
  ...
  for (int i = 0; i < nnodes; ++i)
  {
    ...
    for (int j = 0; j < ndofs; ++j)
    {
      ...
      if (discret_->HaveGlobalNode(nodes[i]))
      {
        ...
        if (dofrowmap_->LID(actdofs[dofs[i][j]]) != -1) mdofs.push_back(actdofs[dofs[i][j]]);
      }
...
/*----------------------------------------------------------------------*/
void INVANA::ObjectiveFunctDisp::Evaluate(
    Teuchos::RCP<Epetra_Vector> state, double time, double& val)
{
...
/*----------------------------------------------------------------------*/
void INVANA::ObjectiveFunctDisp::EvaluateGradient(
