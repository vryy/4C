...
INVANA::DcsMatrix::DcsMatrix(Teuchos::RCP<TIMINT::TimIntMStep<Epetra_Vector>> sstore,
    Teuchos::RCP<TIMINT::TimIntMStep<Epetra_Vector>> ystore, bool initscal, bool probscal,
    double probscalefac)
    : sty_(0, 0, 0.0),
      initscal_(initscal),
      init_scale_fac_(1.0),
      ...
      prob_scale_fac_(probscalefac)
{
  ...
  sstore_ =
      Teuchos::rcp(new TIMINT::TimIntMStep<Epetra_Vector>(past_, future_, rowmap_.get(), true));
  ystore_ =
      Teuchos::rcp(new TIMINT::TimIntMStep<Epetra_Vector>(past_, future_, rowmap_.get(), true));
...
/*----------------------------------------------------------------------*/
int INVANA::DcsMatrix::MaxNumEntries() const { return numentries_; }

/*----------------------------------------------------------------------*/
int INVANA::DcsMatrix::ExtractGlobalColumnCopy(
...
{
  if (not column_->Map().PointSameAs(column.Map())) dserror("maps for extraction don't agree");
...
{
  if (not column_->Map().PointSameAs(vector.Map())) dserror("maps for extraction don't agree");
...
void INVANA::DcsMatrix::twolooprecursion(Epetra_Vector& p) const
{
  ...
  if (probscal_) p.Scale(prob_scale_fac_);
...
Teuchos::RCP<Epetra_CrsMatrix> INVANA::DcsMatrix::FillMatrix() const
{
  ...
  Teuchos::RCP<Epetra_CrsMatrix> matrix =
      Teuchos::rcp(new Epetra_CrsMatrix(Copy, *rowmap_, numentries_, false));
...
{
  if (not rowmap_->PointSameAs(diagonal.Map())) dserror("Map of supplied vector doesn't fit.");
