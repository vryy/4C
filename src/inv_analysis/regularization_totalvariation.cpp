...
/*----------------------------------------------------------------------*/
void INVANA::RegularizationTotalVariation::EvaluateGradient(
    const Epetra_MultiVector& theta, Teuchos::RCP<Epetra_MultiVector> gradient)
{
  ...
  if (err) dserror("Projection to optimization parameter layout returned err=%d", err);
