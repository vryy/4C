...
*/
/*----------------------------------------------------------------------*/
#include "baci_inv_analysis_chol_factor.H"
...
int INVANA::CholFactor::Compute()
{
  ...
  if (numelements != 0)
  {
    ...
    if (err) dserror("Covariance matrix factorization failed with err %d", err);
  }
  Comm().Barrier();

  // -> CrsMatrix of the lower factor
  Epetra_SerialSymDenseMatrix* L = solver_->SymFactoredMatrix();
  Teuchos::RCP<Epetra_CrsMatrix> HSerial = Teuchos::rcp(new Epetra_CrsMatrix(Copy, *serialmap_, 0));
  for (int i = 0; i < numelements; i++)
  {
...
/*----------------------------------------------------------------------*/
int INVANA::CholFactor::ApplyInverse(const Epetra_MultiVector& X, Epetra_MultiVector& Y) const
{
  // suppose numvectors is just 1
  if (X.NumVectors() != 1 or Y.NumVectors() != 1) dserror("Use only 1 Vector per MultiVector.");

  // check maps mutually
  if (not X.Map().PointSameAs(Y.Map()))
    dserror(
        "X and Y don't share the same maps which they"
  ...
  for (int i = 0; i < numrows; i++) x(i, 0) = X0[i];
  ...
  // put result in Y
  for (int i = 0; i < numrows; i++) Y0[i] = y(i, 0);
