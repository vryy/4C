    ...
    )
{
  ...
  if (whichfssgd_ == INPAR::SCATRA::fssugrdiff_artificial)
  {
    ...
    if (timintparams_->IsIncremental()) dserror("Artificial fine-scale subgrid-diffusivity approach only in combination with non-incremental solver so far!");
  }
  else if (whichfssgd_ == INPAR::SCATRA::fssugrdiff_smagorinsky_all or whichfssgd_ == INPAR::SCATRA::fssugrdiff_smagorinsky_small)
  {
    ...
    // check for solver type
    if (not timintparams_->IsIncremental()) dserror("Fine-scale subgrid-diffusivity approach using all/small-scale Smagorinsky model only in combination with incremental solver so far!");
  }
