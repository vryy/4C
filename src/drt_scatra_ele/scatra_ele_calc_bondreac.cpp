...
DRT::ELEMENTS::ScaTraEleCalcBondReac<distype, probdim>::Instance(const int numdofpernode,
    const int numscal, const std::string& disname, const ScaTraEleCalcBondReac* delete_me)
{
  ...
  else
  {
    ...
    for (typename std::map<std::pair<std::string, int>,
             ScaTraEleCalcBondReac<distype, probdim>*>::iterator i = instances.begin();
         i != instances.end(); ++i)
...
)
{
  ...
  else if (material->MaterialType() == INPAR::MAT::m_matlist_bondreacs)
  {
    ...
    const Teuchos::RCP<MAT::MatListBondReacs> actmat =
        Teuchos::rcp_dynamic_cast<MAT::MatListBondReacs>(material);
    if (actmat->NumMat() != my::numscal_) dserror("Not enough materials in MatList.");
    ...
    for (int k = 0; k < my::numscal_; ++k)
    {
      ...
      SetBondReactionTerms(k, actmat, violation, porosity,
          advreac::GetGpCoord());  // every reaction calculation stuff happens in here!!
    }
  }
    ...
    const double* gpcoord)
{
  ...
  remanager->AddToReaBodyForce(
      matreaclist->CalcReaBodyForceTerm(k, phinp, phin, violation, porosity, gpcoord), k);

  matreaclist->CalcReaBodyForceDerivMatrix(
      k, remanager->GetReaBodyForceDerivVector(k), phinp, phin, violation, porosity, gpcoord);
}
    ...
    ) const
{
  ...
  if (dispnp == Teuchos::null) dserror("Cannot get displacement vector from scatra discretization");
  ...
  if (is_adhesion_element)
  {
    ...
    if (numNode != 4) dserror("only implemented for quad4 surface elements.");
    ...
    for (int node = 0; node < numNode; ++node)
      for (int dof = 0; dof < cell_numdofpernode; ++dof)
        adhesion_violation_nd[node * cell_numdofpernode + dof] =
            myvalues_displ[node * cell_numdofpernode + dof] + (ele->Nodes())[node]->X()[dof] -
            myadhesionfixpoints[node * cell_numdofpernode + dof];
    ...
    for (int node = 0; node < numNode; ++node)
      for (int dof = 0; dof < cell_numdofpernode; ++dof)
        adhesion_violation_gp[dof] +=
            shapefunct(node) * adhesion_violation_nd[node * cell_numdofpernode + dof];
    ...
    for (int node = 0; node < numNode; node++) traction += shapefunct(node) * traction_nd(node, 0);
