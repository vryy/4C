...
DRT::ELEMENTS::ScaTraEleCalcArtery<distype,probdim>::ScaTraEleCalcArtery(const int numdofpernode,const int numscal,const std::string& disname)
: my::ScaTraEleCalc(numdofpernode,numscal,disname)
{
  ...
  my::scatravarmanager_ = Teuchos::rcp(new ScaTraEleInternalVariableManagerArtery<my::nsd_, my::nen_>(my::numscal_));
  return;
}
 ...
 *----------------------------------------------------------------------*/
template<DRT::Element::DiscretizationType distype,int probdim>
DRT::ELEMENTS::ScaTraEleCalcArtery<distype,probdim> * DRT::ELEMENTS::ScaTraEleCalcArtery<distype,probdim>::Instance(
  const int numdofpernode,
  const int numscal,
  const std::string& disname,
  const ScaTraEleCalcArtery* delete_me )
{
  ...
  if(delete_me == NULL)
  {
    if(instances.find(disname) == instances.end())
      instances[disname] = new ScaTraEleCalcArtery<distype,probdim>(numdofpernode,numscal,disname);
  }

  else
  {
    for( typename std::map<std::string,ScaTraEleCalcArtery<distype,probdim>* >::iterator i=instances.begin(); i!=instances.end(); ++i )
...
void DRT::ELEMENTS::ScaTraEleCalcArtery<distype,probdim>::SetInternalVariablesForMatAndRHS()
{
  ...
  VarManager()->SetInternalVariablesArtery(
      my::funct_,
      my::derxy_,
      my::deriv_,
      my::xjm_,
  ...
  )
{
  ...
  const double prefac = timefacfac*VarManager()->Diam()*VarManager()->Diam()/32.0/VarManager()->Visc()*(-1.0);
  for (unsigned vi=0; vi<my::nen_; ++vi)
  {
    ...
    for (unsigned ui=0; ui<my::nen_; ++ui)
    {
      ...
      for (unsigned j = 0; j<my::nsd_; j++)
        laplawf += my::derxy_(j, ui)*gradphi(j);
      const int fui = ui ;
