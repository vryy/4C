    ...
    )
{
  ...
  if(cond == Teuchos::null)
    dserror("Cannot access condition 'ElchBoundaryKineticsPoint'!");
  ...
  if(!(params.get<bool>("calc_status",false)))
  {
    ...
    if(zerocur == 0)
    {
      EvaluateElchBoundaryKineticsPoint(
          ele,
          elemat1_epetra,
          elevec1_epetra,
          ephinp,
...
)
{
  ...
  if(my::nsd_ele_ != 1)
    dserror("Electrode kinetics point boundary conditions are applicable to one-dimensional problems only!");

  // extract global ID of conditioned node
  const int nodeid = (*nodeids)[0];

  // find out whether conditioned node is the leftmost (position 0) or rightmost (position 1) node of the current line element
  int position(-1);
  if(nodeid == ele->Nodes()[0]->Id())
    ...
    dserror("Electrode kinetics point boundary condition must be imposed either on the leftmost or on the rightmost node of a line element!");
