/*--------------------------------------------------------------------------*/
/*!
\file scatra_ele_calc_sti_diffcond.cpp
...
DRT::ELEMENTS::ScaTraEleCalcSTIDiffCond<distype>* DRT::ELEMENTS::ScaTraEleCalcSTIDiffCond<distype>::Instance(
    const int numdofpernode,
    const int numscal,
    const std::string& disname,
    const ScaTraEleCalcSTIDiffCond* delete_me
    )
{
  ...
  {
    for(typename std::map<std::string,ScaTraEleCalcSTIDiffCond<distype>* >::iterator i=instances.begin(); i!=instances.end(); ++i)
    ...
    )
{
  ...
  i.Update((1-t)*invfval*2.*R*my::scatravarmanager_->Phinp(0)/concentration,VarManager()->GradConc(),invfval*R*log(concentration),my::scatravarmanager_->GradPhi(0),-1.);
  i.Scale(kappa);
  ...
  for(int vi=0; vi<my::nen_; ++vi)
  {
    for(int ui=0; ui<my::nen_; ++ui)
    {
      ...
      emat(vi,ui) -= timefacfac*my::funct_(vi)/kappa*(di2_dgradT_gradN+2.*i.Dot(di_dT)*my::funct_(ui));
    }
    ...
    )
{
  ...
  for(int vi=0; vi<my::nen_; ++vi)
  {
    for(int ui=0; ui<my::nen_; ++ui)
    {
      ...
      const double term2 = -2.*temperature*a.Dot(gradtemp)*soret*pow(1/temperature,2)*my::funct_(ui);
    ...
    )
{
  ...
  for(int vi=0; vi<my::nen_; ++vi)
  {
    ...
    for(int ui=0; ui<my::nen_; ++ui)
    {
      ...
      emat(vi,ui) += timefacfac*diffcoeff*concentration*2.*R*soret*((term1+term2+term3)*my::funct_(vi)+term4+term5+term6);
    }

    // contributions of Soret effect term to thermo residuals
    erhs[vi] -= rhsfac*diffcoeff*2.*R*soret*(a.Dot(gradtemp)*my::funct_(vi)+temperature*laplawfrhs_a_vi);
  }
    ...
    )
{
  ...
  i.Update((1-t)*invfval*2.*R*temperature/concentration,gradconc,invfval*R*log(concentration),gradtemp,-1.);
  i.Scale(kappa);

  // derivative of current density w.r.t. concentration
  LINALG::Matrix<my::nsd_,1> di_dc = gradpot;
  di_dc.Update(kappaderiv*(1-t)*invfval*2.*R*temperature/concentration-kappa*diffmanagerdiffcond_->GetDerivTransNum(0,0)*invfval*2.*R*temperature/concentration-kappa*(1-t)*invfval*2.*R*temperature/pow(concentration,2),gradconc,kappaderiv*invfval*R*log(concentration)+kappa*invfval*R/concentration,gradtemp,-kappaderiv);
  ...
  for(int vi=0; vi<my::nen_; ++vi)
  {
    for(int ui=0; ui<my::nen_; ++ui)
    {
      ...
      // gradient of shape function times derivative of square of current density w.r.t. gradient of electric potential
    ...
    )
{
  ...
  for(int vi=0; vi<my::nen_; ++vi)
  {
    for(int ui=0; ui<my::nen_; ++ui)
    {
      ...
      // intermediate terms
      const double term1 = 2.*diffcoeff/concentration*diffmanagerdiffcond_->GetDerivIsoDiffCoef(0,0)*a2*my::funct_(ui);
      const double term2 = -pow(diffcoeff,2)/pow(concentration,2)*a2*my::funct_(ui);
      const double term3 = 2.*pow(diffcoeff,2)/concentration*a.Dot(gradtemp)*soret/temperature*my::funct_(ui);
