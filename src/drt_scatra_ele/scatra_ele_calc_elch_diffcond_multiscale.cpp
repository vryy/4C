...
DRT::ELEMENTS::ScaTraEleCalcElchDiffCondMultiScale<distype>* DRT::ELEMENTS::ScaTraEleCalcElchDiffCondMultiScale<distype>::Instance(
    ...
    )
{
  ...
  if(delete_me == NULL)
  {
    if(instances.find(disname) == instances.end())
      instances[disname] = new ScaTraEleCalcElchDiffCondMultiScale<distype>(numdofpernode,numscal,disname);
  }

  else
  {
    for(typename std::map<std::string,ScaTraEleCalcElchDiffCondMultiScale<distype>* >::iterator i=instances.begin(); i!=instances.end(); ++i)
...
DRT::ELEMENTS::ScaTraEleCalcElchDiffCondMultiScale<distype>::ScaTraEleCalcElchDiffCondMultiScale(
  ...
  : // constructor of base class
    DRT::ELEMENTS::ScaTraEleCalcElchDiffCond<distype>::ScaTraEleCalcElchDiffCond(numdofpernode,numscal,disname)
{
...
/*-----------------------------------------------------------------------------------------------------------------------*
 | macro-scale matrix and vector contributions arising from macro-micro coupling in multi-scale simulations   fang 07/17 |
 *-----------------------------------------------------------------------------------------------------------------------*/
template<DRT::Element::DiscretizationType distype>
    ...
    )
{
  ...
  // initialize variables for micro-scale coupling flux and derivatives of micro-scale coupling flux w.r.t. macro-scale state variables
  double q_micro(0.);
  ...
  //
  // 1.) additional source term on left-hand side of mass conservation equation for lithium concentration inside electrolyte:
  //   [...] - A_s * q = 0
  //
  // 2.) additional source term on left-hand side of charge conservation equation for electric potential inside electrolyte:
  //   [...] - A_s * q = 0
  //
  ...
  for(unsigned vi=0; vi<my::nen_; ++vi)
  {
    ...
    for(unsigned ui=0; ui<my::nen_; ++ui)
    {
      ...
      emat(fvi+2,fui+2) += timefacfac*mydiffcond::VarManager()->InvF()*DiffManager()->GetPhasePoroTort(0)*DiffManager()->GetSigma()*laplawf - vi_dq_dpot_ed_ui;
    }
    ...
    erhs[fvi+2] -= rhsfac*mydiffcond::VarManager()->InvF()*DiffManager()->GetPhasePoroTort(0)*DiffManager()->GetSigma()*laplawfrhs_gradpot - vi_rhsterm;
  }
    ...
    )
{
  ...
  for(int iquad=0; iquad<intpoints.IP().nquad; ++iquad)
  {
    ...
    CalcMatAndRhsMultiScale(ele,emat,erhs,0,iquad,my::scatraparatimint_->TimeFac()*fac,my::scatraparatimint_->TimeFacRhs()*fac);
  }
