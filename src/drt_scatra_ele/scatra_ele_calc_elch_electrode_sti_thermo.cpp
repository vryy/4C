...
DRT::ELEMENTS::ScaTraEleCalcElchElectrodeSTIThermo<distype>* DRT::ELEMENTS::ScaTraEleCalcElchElectrodeSTIThermo<distype>::Instance(
    ...
    )
{
  ...
  if(delete_me == NULL)
  {
    if(instances.find(disname) == instances.end())
      instances[disname] = new ScaTraEleCalcElchElectrodeSTIThermo<distype>(numdofpernode,numscal,disname);
  }

  else
  {
    for(typename std::map<std::string,ScaTraEleCalcElchElectrodeSTIThermo<distype>* >::iterator i=instances.begin(); i!=instances.end(); ++i)
    ...
    )
{
  ...
  mythermo::CalcRHSSoret(erhs,VarManager()->Phinp(0),myelectrode::DiffManager()->GetIsotropicDiff(0),rhsfac,VarManager()->Temp(),VarManager()->GradTemp(),my::derxy_);
...
void DRT::ELEMENTS::ScaTraEleCalcElchElectrodeSTIThermo<distype>::SetInternalVariablesForMatAndRHS()
{
  ...
  VarManager()->SetInternalVariables(my::funct_,my::derxy_,my::ephinp_,my::ephin_,mythermo::etempnp_,my::econvelnp_,my::ehist_);
