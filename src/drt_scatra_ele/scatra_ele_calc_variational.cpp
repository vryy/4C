 ...
 *----------------------------------------------------------------------*/
template<DRT::Element::DiscretizationType distype>
DRT::ELEMENTS::ScaTraEleCalVariational<distype>* DRT::ELEMENTS::ScaTraEleCalVariational<distype>::Instance(
    const int numdofpernode,
    const int numscal,
    const std::string& disname,
    const ScaTraEleCalVariational* delete_me
    )
{
  ...
  else
  {
    ...
    for( typename std::map<std::string,ScaTraEleCalVariational<distype>* >::iterator i=instances.begin(); i!=instances.end(); ++i )
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
DRT::ELEMENTS::ScaTraEleCalVariational<distype>::ScaTraEleCalVariational(const int numdofpernode,const int numscal,const std::string& disname) :
...
{
  // replace standard internal variable manager by internal variable manager for variational chemical diffusion
  my::scatravarmanager_ = Teuchos::rcp(new ScaTraEleInternalVariableManagerVar<my::nsd_, my::nen_>(my::numscal_));
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
int DRT::ELEMENTS::ScaTraEleCalVariational<distype>::Evaluate(
  DRT::Element*                 ele,
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::ScaTraEleCalVariational<distype>::ExtractInitialNodeValues(
  DRT::Element*                 ele,
  Teuchos::ParameterList&       params,
  DRT::Discretization&          discretization,
  DRT::Element::LocationArray&  la
  )
  {
    ...
    if (phi0==Teuchos::null)
      dserror("Cannot get the initial state vector 'phi0'");
  ...
  )
{
    ...
    for (int iquad=0; iquad<intpoints.IP().nquad; ++iquad)
    {
      ...
      // get material fields interpolated (needed for elements where the positive values of c can get lost locally)
      ...
      if((distype == DRT::Element::line3 || distype == DRT::Element::tri6 || distype == DRT::Element::quad4 || distype == DRT::Element::quad9 )&& actmat->ModelType() == MAT::PAR::chemdiff_fickean)
      ...
      for (int k=0;k<my::numscal_;++k)
      {
        ...
        CalcMatAndRhs(emat,erhs,k, intpoints.IP().qwgt[iquad],fac,timefacfac,rhsfac,taufac,timetaufac,rhstaufac,tauderpot[k],rhsint);
    ...
    )
{
  ...
  const int offsetChemPotential = my::numscal_;
    ...
    //----------------------------------------------------------------------
    // Concentration R_{c}
    CalcRhsGeneric( erhs, k,
            fac,
            DiffManager()->GetInternalEnergy1deriv(k) - VarManager()->ConjugatePhinp(k),
            my::funct_);
    ...
    //----------------------------------------------------------------------
    // K_{c , c}
    CalcMatGeneric( emat,k,k,
            fac,
            DiffManager()->GetInternalEnergy2deriv(k) ,
            my::funct_, my::funct_);

    // K_{\mu , \mu}
    CalcMatGeneric( emat,k+offsetChemPotential,k+offsetChemPotential,
            -my::scatraparatimint_->Dt() * fac,
            DiffManager()->GetDissipationPot2deriv(k),
            my::derxy_, my::derxy_);

    // K_{c, \mu}
    CalcMatGeneric( emat,k,k+offsetChemPotential,
            fac,
            -1.0,
            my::funct_, my::funct_);

    // K_{\mu , c}  //TODO see if there is a better way to copy the off diagonal tahn to re-do it
    CalcMatGeneric( emat,k+offsetChemPotential,k,
            fac,
            -1.0,
            my::funct_, my::funct_);
...
void DRT::ELEMENTS::ScaTraEleCalVariational<distype>::CalcMatGeneric(
      ...
      const int &                    col,             //!< starting column position index
      const double &                 discrFact,       //!< discrete factors(time &temporal discretization, weights, Jacobian, etc...)
      const LINALG::Matrix<my::nsd_,my::nsd_> & kernel,   //!< integral kernel (excluding shape functions)
      const LINALG::Matrix<my::nsd_,my::nen_> & sfunct,   //!< first shape function
...
//    std::cout<< "scatra_ele_calc_variaitonal.cpp(CalcMatGeneric-1)" <<std::endl;
  ...
  for (unsigned vi=0; vi<my::nen_; ++vi)
  {
    ...
    LINALG::Matrix<my::nsd_,my::nen_> temporalParam1; //TODO move the multiplications inside the for loop
  ...
  )
{
  ...
  if ( IsSemImplicitFunctional_ ) //(SCATRA::TimIntVarChemDiffusionOST::IsSemImplicitFunctional())
  {
    ...
    DiffManager()->SetDissipationPot1deriv(actmat, VarManager()->Phin(k), DiffManager()->GetRefC(k), VarManager()->ConjugateField(k), DiffManager()->GetMobility(k), k);

    // get Dissipation Potential 2nd derivative
    DiffManager()->SetDissipationPot2deriv(actmat, VarManager()->Phin(k), DiffManager()->GetRefC(k), VarManager()->ConjugateField(k) , DiffManager()->GetMobility(k), k);
  }// if: Semi-implicit evaluation
  else
  {
    // get Dissipation Potential 1st derivative
    DiffManager()->SetDissipationPot1deriv(actmat, VarManager()->Phinp(k), DiffManager()->GetRefC(k), VarManager()->ConjugateField(k), DiffManager()->GetMobility(k), k);

    // get Dissipation Potential 2nd derivative
    DiffManager()->SetDissipationPot2deriv(actmat, VarManager()->Phinp(k), DiffManager()->GetRefC(k), VarManager()->ConjugateField(k) , DiffManager()->GetMobility(k), k);
  }// if: Fully implicit evaluation

  if( (distype != DRT::Element::line3 &&  distype != DRT::Element::tri6 && distype != DRT::Element::quad4 &&  distype != DRT::Element::quad9) || actmat->ModelType() != MAT::PAR::chemdiff_fickean) // Needed when local negative concentrations may arise due to interpolation
  {
    ...
    DiffManager()->SetInternalEnergy1deriv(actmat, VarManager()->Phinp(k), DiffManager()->GetRefMu(k) , DiffManager()->GetRefC(k), DiffManager()->GetRT(), k);

     // get Internal energy 2nd derivative
    DiffManager()->SetInternalEnergy2deriv(actmat, VarManager()->Phinp(k), DiffManager()->GetRefMu(k) , DiffManager()->GetRefC(k), DiffManager()->GetRT(), k);
  }
...
void DRT::ELEMENTS::ScaTraEleCalVariational<distype>::GetMaterialInterpolatedFields(
    const DRT::Element*                         ele,      //!< the element we are dealing with
  const std::vector<LINALG::Matrix<my::nen_,1> >&     ephinp      //!< nodal state variables at t_(n+1) or t_(n+alpha_F)
    )
{
  // get actual material
  Teuchos::RCP<const MAT::Material> material = ele->Material();
  ...
  std::vector<LINALG::Matrix<my::nen_,1>> NodalValue1(my::numscal_,LINALG::Matrix<my::nen_,1>(true));
  std::vector<LINALG::Matrix<my::nen_,1>> NodalValue2(my::numscal_,LINALG::Matrix<my::nen_,1>(true));
  ...
  for (int k = 0; k < my::numscal_; ++k)
  {
    ...
    for ( unsigned j = 0; j < my::nen_ ; ++j)
    {
      if (ephinp.at(k)(j) < 0.0)
        dserror("Negative concentration at the nodes, before trying to interpolate internal energy");
      // Evaluate internal energy first derivative at the nodes
      NodalValue1.at(k)(j) = actmat->ComputeInternalEnergy( ephinp.at(k)(j) ,
                           DiffManager()->GetRefMu(k),
                           DiffManager()->GetRefC(k),
                           DiffManager()->GetRT(),
                           1);
 ...
 *----------------------------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::ScaTraEleCalVariational<distype>::FDCheck(
  DRT::Element*                              ele,
