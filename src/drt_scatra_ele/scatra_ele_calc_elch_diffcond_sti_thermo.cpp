/*--------------------------------------------------------------------------*/
/*!
\file scatra_ele_calc_elch_diffcond_sti_thermo.cpp
...
DRT::ELEMENTS::ScaTraEleCalcElchDiffCondSTIThermo<distype>* DRT::ELEMENTS::ScaTraEleCalcElchDiffCondSTIThermo<distype>::Instance(
    ...
    )
{
  ...
  if(delete_me == NULL)
  {
    if(instances.find(disname) == instances.end())
      instances[disname] = new ScaTraEleCalcElchDiffCondSTIThermo<distype>(numdofpernode,numscal,disname);
  }

  else
  {
    for(typename std::map<std::string,ScaTraEleCalcElchDiffCondSTIThermo<distype>* >::iterator i=instances.begin(); i!=instances.end(); ++i)
    ...
    )
{
  ...
  for (int vi=0; vi<my::nen_; ++vi)
  {
    ...
    for (int ui=0; ui<my::nen_; ++ui)
    {
      ...
      emat(rowconc,colconc) -= timefacfac*my::funct_(ui)*laplawfrhs_temp*pow(invfval,2.0)*(tderiv*kappa*R*log(concentration)+t*kappaderiv*R*log(concentration)+t*kappa*R/concentration);

      // linearizations of contributions for electric potential residuals w.r.t. concentration dofs
      emat(rowpot,colconc) -= timefacfac*my::funct_(ui)*laplawfrhs_temp*invffval*(kappaderiv*R*log(concentration)+kappa*R/concentration);

      // linearizations w.r.t. electric potential dofs are zero
    }

    // contribution for concentration residual
    erhs[rowconc] += rhsfac*laplawfrhs_temp*kappa*pow(invfval,2.0)*t*R*log(concentration);
  ...
  mythermo::CalcMatSoret(emat,timefacfac,VarManager()->Phinp(0),mydiffcond::DiffManager()->GetIsotropicDiff(0),mydiffcond::DiffManager()->GetDerivIsoDiffCoef(0,0),VarManager()->Temp(),VarManager()->GradTemp(),my::funct_,my::derxy_);
  mythermo::CalcRHSSoret(erhs,VarManager()->Phinp(0),mydiffcond::DiffManager()->GetIsotropicDiff(0),rhsfac,VarManager()->Temp(),VarManager()->GradTemp(),my::derxy_);
...
/*------------------------------------------------------------------------------------------------------*
 | fill element matrix with linearizations of discrete scatra residuals w.r.t. thermo dofs   fang 11/15 |
 *------------------------------------------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
  ...
  for (int iquad=0; iquad<intpoints.IP().nquad; ++iquad)
  {
    ...
    for (int vi=0; vi<my::nen_; ++vi)
    {
      ...
      for(int ui=0; ui<my::nen_; ++ui)
      {
        ...
        emat(rowconc,ui) -= timefacfac*(t*invfval*di_dT*my::funct_(ui)+t*invfval*di_dgradT*laplawf);

        // linearizations of contributions for electric potential residuals w.r.t. thermo dofs
        emat(rowpot,ui) -= timefacfac*(laplawf*kappa*invffval*R*log(concentration)+my::funct_(ui)*kappa*(1-t)*invffval*R/concentration*2*laplawfrhs_conc);
      }
    }
...
void DRT::ELEMENTS::ScaTraEleCalcElchDiffCondSTIThermo<distype>::SetInternalVariablesForMatAndRHS()
{
  ...
  VarManager()->SetInternalVariables(my::funct_,my::derxy_,mythermo::etempnp_,my::ephinp_,my::ephin_,my::econvelnp_,my::ehist_);
