...
/*------------------------------------------------------------------------------------------------------------------------------*
 | provide element matrix with linearizations of Soret effect term in discrete scatra residuals w.r.t. scatra dofs   fang 11/15 |
 *------------------------------------------------------------------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
...
)
{
  for(int vi=0; vi<nen_; ++vi)
  {
    ...
    for(int ui=0; ui<nen_; ++ui)
      ...
      emat(rowconc,ui*2) += timefacfac*funct(ui)*laplawfrhs_gradtemp/temp*diffmanagerstithermo_->GetSoret()*(diffcoeff+conc*diffcoeffderiv);

      // linearizations of Soret effect term in concentration residuals w.r.t. electric potential dofs are zero
...
/*------------------------------------------------------------------------------------------------------------------------------*
 | provide element matrix with linearizations of Soret effect term in discrete scatra residuals w.r.t. thermo dofs   fang 11/15 |
 *------------------------------------------------------------------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
    ...
    )
{
  for(int vi=0; vi<nen_; ++vi)
  {
    ...
    for(int ui=0; ui<nen_; ++ui)
    {
      ...
      emat(rowconc,ui) += timefacfac*diffcoeff*diffmanagerstithermo_->GetSoret()*concentration*(laplawf/temperature-funct(ui)*laplawfrhs_gradtemp/(pow(temperature,2)));
...
/*--------------------------------------------------------------------------------------------------------------------------*
 | provide element right-hand side vector with contributions of Soret effect term to discrete scatra residuals   fang 11/15 |
 *--------------------------------------------------------------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
    ...
    )
{
  for(int vi=0; vi<nen_; ++vi)
  {
    ...
    erhs[vi*2] -= rhsfac*laplawfrhs_gradtemp*diffcoeff*conc*diffmanagerstithermo_->GetSoret()/temp;
    ...
    diffmanagerstithermo_(Teuchos::rcp(new ScaTraEleDiffManagerSTIThermo(numscal)))
{
  // safety check
  if(numscal != 1)
    dserror("Thermodynamic scalar transport only works for exactly one transported scalar at the moment!");
