...
discret_(discret)
{
  ...
  vector<RCP<CONTACT::MtInterface> > interfaces;
  ...
  for (int i=0; i<(int)contactconditions.size(); ++i)
  {
    ...
    interfaces.push_back(rcp(new CONTACT::MtInterface(groupid1,Comm(),dim,mtparams)));
    
    // get it again
    RCP<CONTACT::MtInterface> interface = interfaces[(int)interfaces.size()-1];
    ...
    for (int j=0;j<(int)currentgroup.size();++j)
    {
      ...
      for (int k=0; k<(int)(*nodeids).size(); ++k)
      {
        ...
        // create MtNode object
        RCP<CONTACT::MtNode> mtnode = rcp(new CONTACT::MtNode(node->Id(),node->X(),
                                                              node->Owner(),
                                                              Discret().NumDof(node),
                                                              Discret().Dof(node),
                                                              isslave[j]));
    ...
    for (int j=0;j<(int)currentgroup.size();++j)
    {
      ...
      for (fool=currele.begin(); fool != currele.end(); ++fool)
      {
        ...
        RCP<CONTACT::MtElement> mtele = rcp(new CONTACT::MtElement(ele->Id()+ggsize,
