...
StruGenAlpha(params,dis,solver,output)
{
  ...
  // -------------------------------------------------------------------
  {
    ...
    /*else if (ctype==INPAR::CONTACT::contact_normal)
      cmtmanager_ = rcp(new CONTACT::Manager(discret_,alphaf));
...
void CONTACT::CmtStruGenAlpha::Update()
{
  ...
  // update contact
  cmtmanager_->GetStrategy().Update(istep);
...
void CONTACT::CmtStruGenAlpha::Output()
{
  ...
  //----------------------------- evaluate interface traction for output
    RCP<Epetra_Map> problem = (cmtmanager_->GetStrategy().ProblemRowMap());
    RCP<Epetra_Vector> traction = rcp(new Epetra_Vector(*(cmtmanager_->GetStrategy().LagrMultOld())));
  ...
  if (writeresevry and istep%writeresevry==0)
  {
    ...
    output_.WriteVector("interfacetraction",tractionexp);
