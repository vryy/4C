...
/*----------------------------------------------------------------------------*/
NLNSOL::NlnProblemNox::NlnProblemNox()
...
const bool NLNSOL::NlnProblemNox::SetModelEvaluator()
{
  ...
  {
    dserror("Parameter 'NOX Group' isn't of type "
...
const bool NLNSOL::NlnProblemNox::SetJacobianOperator()
{
  ...
  if (not Params()->isType<Teuchos::RCP<LINALG::SparseOperator> >(
      "Jacobian Operator"))
  {
    dserror("Parameter 'Jacobian Operator' isn't of type "
        "'Teuchos::RCP<LINALG::SparseOperator>'.");
    return false;
  }

  jac_ = Params()->get<Teuchos::RCP<LINALG::SparseOperator> >(
      "Jacobian Operator");
...
/*----------------------------------------------------------------------------*/
void NLNSOL::NlnProblemNox::ComputeF(const Epetra_MultiVector& x,
    Epetra_MultiVector& f) const
{
  // time measurements
  Teuchos::RCP<Teuchos::Time> time = Teuchos::TimeMonitor::getNewCounter(
      "NLNSOL::NlnProblemNox::ComputeF");
...
void NLNSOL::NlnProblemNox::ComputeJacobian() const
{
  // time measurements
  Teuchos::RCP<Teuchos::Time> time = Teuchos::TimeMonitor::getNewCounter(
      "NLNSOL::NlnProblemNox::ComputeJacobian");
  Teuchos::TimeMonitor monitor(*time);
