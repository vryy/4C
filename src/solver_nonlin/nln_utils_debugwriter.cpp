...
void NLNSOL::UTILS::DebugWriterBase::CreateLogFile()
{
  ...
  {
    (*log_) << "# " << Label() << std::endl
        << "# Encoding of auto-generated name and user-given description for"
        << std::endl << "# debug output file '" << filename << "'" << std::endl
        << "#" << std::endl << "# auto\t\t" << std::setw(16) << std::left
        << "user" << std::endl;
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void NLNSOL::UTILS::DebugWriterBase::WriteLogFileEntry(const std::string& name,
    const std::string& description) const
{
  if (Comm()->MyPID() == 0)
  {
    (*log_) << name << "\t\t" << std::setw(16) << std::left << description
        << std::endl;
  }
    ...
    const IO::VectorType vt) const
{
  ...
  {
    *getOStream() << "+++ " << LabelShort() << ": writing vector '"
        << uniquename << "' to output ... ";
  }

  // write vector to debug output
  writer_->WriteVector(uniquename,
      Teuchos::rcp_const_cast<Epetra_MultiVector>(vec), vt);
