    ...
    Teuchos::RCP<const MAT::Material> material, Teuchos::ParameterList& params, const double tempnp)
{
  ...
  if (material->MaterialType() == INPAR::MAT::m_scatra)
  {
    ...
    if (fluidele == NULL) dserror("Fluid element %i not on local processor", ele->Id());
    // get fluid material
    ...
    double& xcenter, double& ycenter, double& zcenter, const DRT::Element* ele)
{
  ...
  DRT::UTILS::ExtractMyNodeBasedValues(
    ...
    const CORE::LINALG::Matrix<nsd_, 1> convelint, const CORE::LINALG::Matrix<nsd_, 1> fsvelint)
{
  ...
  if (turbparams_->Nwl())
  {
    ...
    if (not turbparams_->Calc_N()) hk = CalcRefLength(vol, convelint);
  ...
  if (not two_ranges)  // usual case
    D_mfs = turbparams_->Csgs_SgPhi() * sqrt(kappa_phi) * pow(2.0, -gamma * Nphi / 2.0) *
  ...
  else
  {
    ...
    D_mfs = turbparams_->Csgs_SgPhi() * sqrt(kappa_phi) * pow(2.0, -gamma2 * Nphi / 2.0) *
      ...
      if (turbulencelist.get<std::string>("TURBULENCE_APPROACH", "none") == "CLASSICAL_LES")
      {
                ...
                "loma_channel_flow_of_height_2")
        {
          ...
          (*(turbulencelist.get<Teuchos::RCP<std::vector<double>>>("local_diffeff_sum")))[nlayer] +=
              diffmanager_->GetIsotropicDiff(0);
        }
      }
    }
  }
