...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
DRT::Element::DiscretizationType STK::ElementShape( const CellTopologyData & topology )
{
  switch ( topology.key )
  {
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
STK::FluidJumpIntegrator::FluidJumpIntegrator( DRT::Discretization & dis, stk::mesh::BulkData & bulk, stk::mesh::Part & active )
  : dis_( dis ),
    bulk_( bulk ),
    active_( active )
{
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidJumpIntegrator::Integrate( stk::mesh::VectorField & coords,
                                          ...
                                          Teuchos::RCP<Epetra_Vector> residual )
{
  ...
  for ( ElementIterator i( bulk_, active_ );
        not i.done();
        ++i )
  {
    FluidEvaluateHelper e( dis_, i.element(), i.topology(), coords, velnp, pressure, residual );
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
STK::FluidEvaluateHelper::FluidEvaluateHelper( DRT::Discretization & dis,
                                               stk::mesh::Entity & e,
    ...
    nu_( 0.0 )
{
  ...
  DRT::Element * ele = dis.gElement( e.key().id()-1 );
  if ( ele==NULL )
  ...
  if ( residual!=Teuchos::null )
  {
    ...
    residual_.Resize( lm.size() );
    DRT::UTILS::ExtractMyValues( *residual, residual_, lm );
  }
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidEvaluateHelper::Integrate( stk::mesh::ScalarField & error, stk::mesh::ScalarField & volume )
{
  ...
  if ( dim==2 )
  {
    ...
    for ( int intpoint=0; intpoint<gaussquadrature.nquad; ++intpoint )
    {
      ...
      {
        double f = func( i );
        Rtau_0 += f * residual_( 3*i );
        Rtau_1 += f * residual_( 3*i+1 );

        // add pressure residual here?
        Rtau_2 += f * residual_( 3*i+2 );
      }
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidEvaluateHelper::ExtractMyValues( Epetra_SerialDenseMatrix & emat,
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidEvaluateHelper::ExtractMyValues( Epetra_SerialDenseVector & evec,
                                                stk::mesh::FieldBase & f,
                                                const stk::mesh::PairIterRelation & nodes )
{
  unsigned numnodes = nodes.size();
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
double STK::FluidEvaluateHelper::Area()
{
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidEvaluateHelper::LocalCoordinates( Epetra_SerialDenseMatrix & rst, unsigned side_number )
{
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
STK::FluidSideEvaluateHelper::FluidSideEvaluateHelper( STK::FluidEvaluateHelper & e,
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidSideEvaluateHelper::ExtractMyValues( const Epetra_SerialDenseMatrix & emat,
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidSideEvaluateHelper::ExtractMyValues( const Epetra_SerialDenseVector & evec,
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void STK::FluidSideEvaluateHelper::Integrate( stk::mesh::ScalarField & error,
                                              STK::FluidEvaluateHelper & le,
                                              unsigned l_side_number,
                                              STK::FluidEvaluateHelper & re,
