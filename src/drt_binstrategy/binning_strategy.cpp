    ...
    std::vector<Teuchos::RCP<Epetra_Vector>> disnp)
{
  ...
  if (not feasibleboxinput)
  {
    ...
    ComputeMinXAABBContainingAllElementsOfInputDiscrets(
        discret, disnp, XAABB_, cutoff_radius_ < 0.0);
  }
...
void BINSTRATEGY::BinningStrategy::Setup(
    Teuchos::RCP<DRT::Discretization>& bindis, Teuchos::RCP<GEO::MESHFREE::BoundingBox> const pbb)
{
 ...
 *----------------------------------------------------------------------*/
BINSTRATEGY::BinningStrategy::BinningStrategy(
    const Epetra_Comm& comm, double cutoff_radius, LINALG::Matrix<3, 2> XAABB)
    : bindis_(Teuchos::null),
      visbindis_(Teuchos::null),
      ...
      myrank_(comm.MyPID())
{
  ...
  if (cutoff_radius_ <= 0.0) dserror("Cutoff radius cannot be zero or negative!");
...
/*----------------------------------------------------------------------*
 | Binning strategy constructor                             ghamm 11/13 |
 *----------------------------------------------------------------------*/
BINSTRATEGY::BinningStrategy::BinningStrategy(const Epetra_Comm& comm)
    : bindis_(Teuchos::null),
      visbindis_(Teuchos::null),
      cutoff_radius_(0.0),
      XAABB_(true),
      ...
      myrank_(comm.MyPID())
{
  ...
  for (int col = 0; col < 2; col++)
  {
    ...
    {
      double value = 1.0e12;
      if (xaabbstream >> value) XAABB_(row, col) = value;
    }
  }
...
void BINSTRATEGY::BinningStrategy::BuildAxisAlignedijkRangeForRigidSphere(
    Teuchos::RCP<DRT::Discretization> const& discret, DRT::Element const* const eleptr,
...
/*-----------------------------------------------------------------------------*
 *-----------------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::GetBinContent(DRT::Element* binptr,
    std::set<DRT::Element*>& eles, std::vector<BINSTRATEGY::UTILS::BinContentType> bincontent,
    bool roweles)
{
  DRT::MESHFREE::MeshfreeMultiBin* bin = static_cast<DRT::MESHFREE::MeshfreeMultiBin*>(binptr);
 ...
 *----------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::DistributeNodesToBins(Teuchos::RCP<DRT::Discretization> discret,
    std::map<int, std::vector<int>>& nodesinbin, Teuchos::RCP<Epetra_Vector> disnp) const
{
 ...
 *-------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::ExtendGhosting(Teuchos::RCP<DRT::Discretization> scatradis,
    ...
    std::map<int, std::set<int>>& myescapedpartelemap) const
{
  ...
  for (std::map<int, std::set<int>>::iterator iter = escapedpartelemap.begin();
       iter != escapedpartelemap.end(); ++iter)
  {
  ...
  Teuchos::RCP<Epetra_Map> particlecolmap = Teuchos::rcp(
      new Epetra_Map(-1, (int)myparticlecolgids.size(), &myparticlecolgids[0], 0, bindis_->Comm()));
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Map> BINSTRATEGY::BinningStrategy::ExtendGhosting(
    std::map<int, std::set<int>>& binelemap, std::map<int, std::set<int>>& ext_bintoele_ghosting,
    Teuchos::RCP<Epetra_Map> bincolmap) const
{
  ...
  for (int iproc = 0; iproc < numproc; ++iproc)
  {
    ...
    // proc i has to store the received data
    if (iproc == myrank_) ext_bintoele_ghosting = rdata;
  }
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Map> BINSTRATEGY::BinningStrategy::ExtendGhosting(
    const Epetra_Map* initial_elecolmap, std::map<int, std::set<int>>& binelemap,
    std::map<int, std::set<int>>& ext_bintoele_ghosting, Teuchos::RCP<Epetra_Map> rowbins,
    Teuchos::RCP<Epetra_Map> bincolmap) const
{
  // do communication to gather all elements for extended ghosting
  ...
  // return extended elecolmap
  return Teuchos::rcp(
      new Epetra_Map(-1, (int)colgids.size(), &colgids[0], 0, initial_elecolmap->Comm()));
}
    ...
    std::vector<Teuchos::RCP<DRT::Discretization>> dis) const
{
  for (size_t i = 0; i < dis.size(); ++i)
  {
    ...
    Teuchos::RCP<Epetra_Map> extendedelecolmap =
        ExtendGhosting(dis[i]->ElementColMap(), binelemap, dummy);
    ...
    std::vector<int> colnodes(nodes.begin(), nodes.end());
    Teuchos::RCP<Epetra_Map> nodecolmap =
        Teuchos::rcp(new Epetra_Map(-1, (int)colnodes.size(), &colnodes[0], 0, dis[i]->Comm()));
 ...
 *-------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::ExtendEleGhosting(Teuchos::RCP<DRT::Discretization> dis,
    ...
    bool assigndegreesoffreedom, bool initelements, bool doboundaryconditions) const
{
  ...
  BINSTRATEGY::UTILS::ExtendDiscretizationGhosting(
      dis, elecolmapextended, assigndegreesoffreedom, initelements, doboundaryconditions);
...
| extend ghosting according to bin distribution          ghamm 11/13 |
 *-------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::CollectInformation(Teuchos::RCP<Epetra_Map> rowbins,
    std::map<int, std::vector<int>>& nodesinbin,
    ...
    for (int i = 0; i < numbin; ++i)
    {
      ...
      if (nodesinbin.find(binids[i]) != nodesinbin.end())
        ...
        sdata[binids[i]].insert(
            sdata[binids[i]].begin(), nodesinbin[binids[i]].begin(), nodesinbin[binids[i]].end());
    }
 ...
 *----------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::CreateBinsBasedOnCutoffAndXAABB(
    Teuchos::RCP<DRT::Discretization> dis)
{
  // create XAABB for discretization
  if (dis != Teuchos::null) CreateXAABB(dis);
...
void BINSTRATEGY::BinningStrategy::CreateBinsScatra(Teuchos::RCP<DRT::Discretization> dis)
{
  ...
  for (int dim = 0; dim < 3; ++dim)
  {
    // determine number of bins per direction for prescribed cutoff radius
    // std::floor leads to bins that are at least of size cutoff_radius
    if (cutoff_radius_ > 0.0) dserror("Do not use the cutoff radius for level-set problems!");
    // bin_per_dir_[dim] = std::max(1, (int)((XAABB_(dim,1)-XAABB_(dim,0))/cutoff_radius_));
  ...
  if (myrank_ == 0)
  {
    std::cout << "Global bounding box size: " << XAABB_ << "bins per direction: "
              << "x = " << bin_per_dir_[0] << " y = " << bin_per_dir_[1]
              << " z = " << bin_per_dir_[2] << std::endl;
  }
...
void BINSTRATEGY::BinningStrategy::CreateXAABB(Teuchos::RCP<DRT::Discretization> discret,
    Teuchos::RCP<Epetra_Vector> disnp, LINALG::Matrix<3, 2>& XAABB, bool setcutoff)
{
...
void BINSTRATEGY::BinningStrategy::ComputeMinXAABBContainingAllElementsOfInputDiscrets(
    std::vector<Teuchos::RCP<DRT::Discretization>> discret,
    std::vector<Teuchos::RCP<Epetra_Vector>> disnp, LINALG::Matrix<3, 2>& XAABB, bool setmincutoff)
{
  // reset cutoff
  if (setmincutoff) cutoff_radius_ = 0.0;
 ...
 *----------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::CreateXAABB(Teuchos::RCP<DRT::Discretization> dis)
{
...
int BINSTRATEGY::BinningStrategy::ConvertPosToGid(const double* pos) const
{
  ...
  {
    deforming_simulation_domain_handler->TransformFromGlobalToUndeformedBoundingBoxSystem(
        pos, pos_ud);
    for (int dim = 0; dim < 3; ++dim)
...
void BINSTRATEGY::BinningStrategy::GetCurrentNodePos(
    Teuchos::RCP<const DRT::Discretization> const discret, DRT::Node const* node,
    Teuchos::RCP<const Epetra_Vector> const disnp, double* currpos) const
{
...
/*-----------------------------------------------------------------------------*
 *-----------------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::PeriodicBoundaryShift3D(
    LINALG::Matrix<3, 1>& d, LINALG::Matrix<3, 1> const X) const
{
  if (not havepbc_) return;
  ...
  else
  {
    for (int dim = 0; dim < 3; ++dim)
    {
      if (not pbconoff_[dim]) continue;

      while (d(dim) + X(dim) < XAABB_(dim, 0)) d(dim) += pbcdeltas_[dim];

      while (d(dim) + X(dim) > XAABB_(dim, 1)) d(dim) -= pbcdeltas_[dim];
    }
  }
  return;
}
 ...
 *-----------------------------------------------------------------------------*/
void BINSTRATEGY::BinningStrategy::CommunicateElements(Teuchos::RCP<DRT::Discretization>& discret,
