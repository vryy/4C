...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
bool XCONTACT::LEVELSET::REINIT::Elliptic::computeF(const Epetra_Vector & x,
    Epetra_Vector & F, const FillType fillFlag)
{
  dsassert(fillFlag == NOX::Epetra::Interface::Required::Residual,
      "Unsupported fill flag!" );
    ...
    const Epetra_Vector& x, Epetra_Vector& F, Epetra_Operator& jac)
{
  ...
  TEUCHOS_FUNC_TIME_MONITOR(
      "XCONTACT::LEVELSET::REINIT::Elliptic::computeFandJacobian" );
  ...
  DRT::UTILS::Evaluate( *Algorithm().Discretization(), eleparams,
      Teuchos::rcp( jac_linalg, false ), Teuchos::rcpFromRef( F ),
      & Algorithm().ActiveColEleMap() );

  if ( not jac_linalg->Filled() )
    jac_linalg->Complete();
    ...
    const Epetra_Vector& x, Epetra_Operator& jac)
{
  ...
  // time measurement: element calls
  TEUCHOS_FUNC_TIME_MONITOR(
      "XCONTACT::LEVELSET::REINIT::Elliptic::computeJacobian" );
  ...
  DRT::UTILS::Evaluate( *Algorithm().Discretization(), eleparams,
      Teuchos::rcp( jac_linalg, false ), Teuchos::null,
      & Algorithm().ActiveColEleMap() );
...
void XCONTACT::LEVELSET::REINIT::Elliptic::PreEvaluate( const Epetra_Vector & x )
{
  if ( Algorithm().UseProjection() )
  {
    ...
    Algorithm().NodalGradBasedValue() =
        Algorithm().ComputeNodalGradientL2Projection( x,
            INPAR::SCATRA::l2_proj_system_dual );
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::LEVELSET::REINIT::Elliptic::SetElementParameters(
    Teuchos::ParameterList & eleparams )
{
  ...
  if ( Algorithm().UseProjection() )
  {
    ...
    Algorithm().Discretization()->SetState( "l2_proj_system_mat_diag",
        Algorithm().L2ProjSysMatDiagonalPtr() );
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
double XCONTACT::LEVELSET::REINIT::Elliptic::GetPrimaryRHSNorms(
    ...
    const bool& isscaled ) const
{
  ...
  if ( checkquantity != NOX::NLN::StatusTest::quantity_levelset_reinit )
    dserror( "How did you come here? We support only one quantity type at this "
  ...
  const NOX::Epetra::Vector rhs_nox = NOX::Epetra::Vector( rhs_ptr,
      NOX::Epetra::Vector::CreateView );

  rhsnorm = rhs_nox.norm( type );
  // do the scaling if desired
  if (isscaled)
    rhsnorm /= static_cast<double>( rhs_nox.length() );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
double XCONTACT::LEVELSET::REINIT::Elliptic::GetPrimarySolutionUpdateRMS(
    ...
    const bool& disable_implicit_weighting) const
{
  ...
  rms = NOX::NLN::AUX::RootMeanSquareNorm( atol, rtol, Teuchos::rcpFromRef( xnew ),
      Teuchos::rcpFromRef( incr ), disable_implicit_weighting );
    ...
    const bool& isscaled) const
{
  ...
  incr.Update( 1.0, xnew, -1.0 );
  const NOX::Epetra::Vector incr_nox = NOX::Epetra::Vector(
      Teuchos::rcpFromRef( incr ), NOX::Epetra::Vector::CreateView );

  updatenorm = incr_nox.norm( type );
  // do the scaling if desired
  if ( isscaled )
    updatenorm /= static_cast<double>( incr_nox.length() );
    ...
    const bool& isscaled) const
{
  ...
  const NOX::Epetra::Vector xold_nox = NOX::Epetra::Vector(
      Teuchos::rcpFromRef( xold_mutable ), NOX::Epetra::Vector::CreateView );

  xoldnorm = xold_nox.norm( type );
  // do the scaling if desired
  if (isscaled)
    xoldnorm /= static_cast<double>( xold_nox.length() );
