...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::CutWizard::Cut_Debug( const DRT::Element * cutted_ele )
    const
{
  ...
  std::cout << "has_xfem_integration_rule = " <<
      ( has_xfem_integration_rule ? "TRUE" : "FALSE" ) << "\n";

  for ( std::vector< GEO::CUT::plain_volumecell_set >::const_iterator cit_sets =
      cell_sets.begin(); cit_sets != cell_sets.end(); ++cit_sets )
  {
    const GEO::CUT::plain_volumecell_set & cells = *cit_sets;
    for ( GEO::CUT::plain_volumecell_set::const_iterator cit = cells.begin();
          cit != cells.end(); ++cit )
    {
