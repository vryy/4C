...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::CutWizard::SetBackgroundState(
    Teuchos::RCP<const Epetra_Vector> back_disp_col,
    Teuchos::RCP<const Epetra_Vector> back_levelset_row,
    int level_set_sid )
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::CutWizard::SetOptions(
        INPAR::CUT::BoundaryCellPosition gen_bcells_position,
        ...
        bool screenoutput )
{
  ...
  GEO::CutWizard::SetOptions( nodal_dofset_strategy, VCellgausstype,
      BCellgausstype, gmsh_output, positions, tetcellsonly, screenoutput );
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
const GEO::BoundaryIntCellPtrs *
XCONTACT::CutWizard::GetBoundaryIntegrationCells( int face_ele_id ) const
{
  std::map<int, GEO::BoundaryIntCellPtrs>::const_iterator cit =
      cond_bicells_map_.find( face_ele_id );

  if ( cit == cond_bicells_map_.end() )
    return NULL;
...
{
  for ( std::vector<const DRT::Element * >::const_iterator cele =
      cut_elements.begin(); cele != cut_elements.end(); ++cele )
  {
    const DRT::Element * cut_element = ( * cele );

    GEO::CUT::ElementHandle * ehandle = GetElement( cut_element );
    if ( not ehandle )
      dserror( "Couldn't find the cut element with id = %d!", cut_element->Id() );
    ...
    for ( std::vector< GEO::CUT::plain_boundarycell_set >::const_iterator cit_bcs =
          bcellsets.begin(); cit_bcs != bcellsets.end(); ++cit_bcs )
    {
...
bool XCONTACT::CutWizard::GetNodalLocalCoordinatesInParentElement(
    const DRT::Element * parent_element,
    const Epetra_SerialDenseMatrix & xyz_cell,
    DRT::Element::DiscretizationType cell_type,
    LINALG::SerialDenseMatrix & rs_cell ) const
{
  ...
  for ( unsigned ivertex=0; ivertex<numnode_cell; ++ivertex )
  {
    ...
    if ( pxyz.M() != static_cast<unsigned>( xyz_cell.M() ) )
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::CutWizard::CreateCondBoundaryIntCells(
    const DRT::Element * cut_element,
    ...
    std::map<int, GEO::BoundaryIntCellPtrs> & cond_bicells_map ) const
{
  ...
  for ( std::vector<const DRT::FaceElement *>::const_iterator cit_fe =
      cond_face_eles.begin(); cit_fe != cond_face_eles.end(); ++cit_fe )
  {
    ...
    bool is_within_face_element = GetNodalLocalCoordinatesInParentElement(
        face_ele, xyzbc, bcell.Shape(), rs_bc );
    ...
    if ( is_within_face_element )
    {
      ...
      Teuchos::RCP<GEO::BoundaryIntCell> bicell = Teuchos::rcp(
            GEO::BoundaryIntCell::Create( bcell.Shape(), face_ele->Id(),
                rs_bc, NULL, xyzbc, false ) );
...
void XCONTACT::CutWizard::AddToCondBoundaryIntCells(
    const Teuchos::RCP<GEO::BoundaryIntCell> & bicell,
    int face_ele_id,
    std::map<int, GEO::BoundaryIntCellPtrs> & cond_bicells_map ) const
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::CutWizard::CheckBoundaryCellType(
    DRT::Element::DiscretizationType distype_bc ) const
{
  if ( distype_bc != DRT::Element::line2 and
       distype_bc != DRT::Element::quad4 and
       distype_bc != DRT::Element::tri3 )
  {
    dserror( "unexpected type of boundary cell: %s",
    ...
    const DRT::Element * ele, LINALG::SerialDenseMatrix & xyze ) const
{
  ...
  switch( distype )
  {
    ...
    default:
      dserror("Unsupported elmenet type ( type = %s )",
    ...
    ) const
{
  ...
  if ( ccond_grps.size() != 1 )
    dserror( "There has to be exactly one contact group for the moment!" );
  ...
  for ( unsigned j=0; j<desired_conds.size(); ++j )
  {
    ...
    if ( not conds[ j ] )
      dserror( "Couldn't find the \"%s\" condition!", desired_cond.c_str() );
  }
  ...
  for ( std::map<int, Teuchos::RCP<DRT::Element> >::const_iterator cit =
      cond_elements.begin(); cit!= cond_elements.end(); ++cit )
  {
    ...
    if ( parent_ele->Shape()!=DRT::Element::quad4 and
         parent_ele->Shape()!=DRT::Element::hex8 )
        ...
        cline != line_eles.end(); ++cline )
    {
      ...
      if ( common_line_node_lids.size() == 1 )
      {
        ProjectLevelSetValuesAlongLine( line_node_gids, num_line_nodes,
            common_line_node_lids[ 0 ], *expanded_level_set_values );
      }
    }
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::CutWizard::ProjectLevelSetValuesAlongLine(
    const int * line_node_gids,
    unsigned num_line_nodes,
    int  common_line_node_lid,
    Epetra_Vector & expanded_level_set_values ) const
{
  ...
  if ( ls_lid == -1 )
    dserror( "Couldn't find the node gid %d in the expanded level-set vector!",
        common_node_gid );
