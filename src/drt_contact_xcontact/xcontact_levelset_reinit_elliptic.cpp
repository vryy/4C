...
XCONTACT::LEVELSET::REINIT::Elliptic::CreateNlnSolver(
    const Epetra_Vector & phinp )
{
  ...
  return NOX::NLN::Solver::BuildSolver( grp_ptr_, ostatus_, Teuchos::null,
      nox_problem_->NlnGlobalDataPtr() );
}
...
XCONTACT::LEVELSET::REINIT::Elliptic::CreateSolutionVector(
    const Epetra_Vector & phinp_full ) const
{
  Teuchos::RCP<Epetra_Vector> soln =
      Algorithm().ActiveDofMapExtractor().ExtractVector( phinp_full,
          XCONTACT::LEVELSET::active );

  // wrap and return
  return Teuchos::rcp( new NOX::Epetra::Vector( soln,
      NOX::Epetra::Vector::CreateView ) );
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::LEVELSET::REINIT::Elliptic::SetupNox(
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::LEVELSET::REINIT::Elliptic::SetNlnSolverParams(
    Teuchos::ParameterList & p_nox ) const
{
...
void XCONTACT::LEVELSET::REINIT::Elliptic::SetLinearSolver(
    std::map<NOX::NLN::SolutionType, Teuchos::RCP<LINALG::Solver> > & linsolver,
    const Teuchos::RCP<LINALG::Solver> & lin_solver)
    const
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Teuchos::ParameterList & XCONTACT::LEVELSET::REINIT::Elliptic::NoxParams()
{
  return *nox_params_;
}
