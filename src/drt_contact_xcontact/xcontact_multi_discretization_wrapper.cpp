...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::MultiDiscretizationWrapper::AddContactIDiscret(
    const XDisPairedPtrVector & idiscrets)
{
  is_exporter_ = false;
  ...
  for (cit=idiscrets.begin(); cit!=idiscrets.end(); ++cit)
    icontact_[cit->first] = cit->second;
}
...
XCONTACT::MultiDiscretizationWrapper::GetScaTraDiscretPtr(
    const enum XFEM::FieldName& field) const
{
  XDisPairedRCPVector::const_iterator cit = iscatra_.find( field );
  if (cit==iscatra_.end())
    dserror("The given field name \"%s\" could not be found in the wrapped "
        "scatra paired vector!",XFEM::FieldName2String(field).c_str());
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
int XCONTACT::MultiDiscretizationWrapper::FillComplete(
...
{
  int ret = XSTR::MultiDiscretizationWrapper::FillComplete(assigndegreesoffreedom,
      initelements,doboundaryconditions,buildsystemmaps,setupmapextractor);
...
void XCONTACT::MultiDiscretizationWrapper::BuildSlaveMasterMaps()
{
  ...
  for ( cit = icontact_.begin(); cit != icontact_.end(); ++cit )
  {
    ...
    {
      MORTAR::MortarNode * col_mortar_node =
          dynamic_cast<MORTAR::MortarNode * >( col_nodes[ elid ] );

      if ( not col_mortar_node )
...
void XCONTACT::MultiDiscretizationWrapper::SetupContactScaTraDofExporter()
{
  ...
  if (inum != iscatra_.size())
    dserror("There was an unequal number of contact interface discretiztations and"
 ...
 *----------------------------------------------------------------------------*/
void XCONTACT::MultiDiscretizationWrapper::SetupContactScaTraDofExporter(
    const XFEM::FieldName & field,
    const DRT::Discretization & icontact,
    const DRT::Discretization & iscatra,
    const bool & resize)
{
  if ( resize )
  {
  ...
  else
  {
    ...
    const Epetra_Map * contact_slave_normal_dof_map =
        slave_ndof_row_maps_.at( field ).get();
    contact2scatra_exporter_[ field ] = Teuchos::rcp(new Epetra_Export(
        *contact_slave_normal_dof_map, *scatra_dof_map));
    scatra2contact_exporter_[ field ] = Teuchos::rcp(new Epetra_Export(
        *scatra_dof_map, *contact_slave_normal_dof_map));
  }
...
Teuchos::RCP<Epetra_Vector> XCONTACT::MultiDiscretizationWrapper::Contact2ScaTra(
    const Teuchos::RCP<Epetra_Vector> & contact_vec,
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> XCONTACT::MultiDiscretizationWrapper::Contact2ScaTra(
    const Epetra_Vector & contact_vec,
    const enum XFEM::FieldName & field,
    const bool & do_normalize ) const
{
  Teuchos::RCP<Epetra_Vector> scatra_vec = Teuchos::rcp(
      new Epetra_Vector( *iscatra_.at(field)->DofRowMap( 0 ) ) );
...
Teuchos::RCP<Epetra_MultiVector> XCONTACT::MultiDiscretizationWrapper::Contact2ScaTra(
    const Teuchos::RCP<Epetra_MultiVector> & contact_vec,
    const enum XFEM::FieldName & field,
    const bool & do_normalize ) const
{
  return Contact2ScaTra( *contact_vec, field, do_normalize );
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> XCONTACT::MultiDiscretizationWrapper::Contact2ScaTra(
    const Epetra_MultiVector & contact_vec,
    const enum XFEM::FieldName & field,
    const bool & do_normalize ) const
{
  const int num_vec = contact_vec.NumVectors();
  Teuchos::RCP<Epetra_MultiVector> scatra_vec = Teuchos::rcp(
        new Epetra_MultiVector( *iscatra_.at(field)->DofRowMap(0), num_vec ) );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::MultiDiscretizationWrapper::Contact2ScaTra(
    const Epetra_MultiVector & contact_vec,
    Epetra_MultiVector & scatra_vec,
    const enum XFEM::FieldName & field,
    const bool & do_normalize ) const
{
  if ( not IsExporter() )
    dserror("Call SetupContactScaTraDofExporter() first!");

  Epetra_Export * c2s_exporter = contact2scatra_exporter_.at( field ).get();
  if ( c2s_exporter != NULL )
  {
    const int err = scatra_vec.Export( contact_vec, *c2s_exporter,
        Insert );
    if (err)
      dserror( "Export to ScaTra distribution returned err = %d", err );
  }
  else
  {
    int err = scatra_vec.ReplaceMap( contact_vec.Map() );
    if ( err )
      dserror( "Replacement of the ScaTra vector map returned err = %d", err );
  ...
  if ( do_normalize )
    NormalizeInitialScaTraValues( scatra_vec );
}
    ...
    Epetra_MultiVector & scatra_vec ) const
{
  ...
  for ( std::vector<int>::const_iterator clid = active_lids.begin();
      clid != active_lids.end(); ++clid )
  {
    scatra_vec.Values()[ *clid ] *= gmax_value_rec;
  }

  // set the inactive values to 2.0
  for ( std::vector<int>::const_iterator clid = inactive_lids.begin();
      clid != inactive_lids.end(); ++clid )
  {
...
Teuchos::RCP<Epetra_Vector> XCONTACT::MultiDiscretizationWrapper::ScaTra2Contact(
    const Teuchos::RCP<Epetra_Vector> & scatra_vec,
    const enum XFEM::FieldName & field) const
{
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> XCONTACT::MultiDiscretizationWrapper::ScaTra2Contact(
    const Epetra_Vector & scatra_vec,
    const enum XFEM::FieldName & field) const
{
  Teuchos::RCP<Epetra_Vector> contact_vec = Teuchos::rcp(
      new Epetra_Vector( *slave_ndof_row_maps_.at( field ) ) );
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> XCONTACT::MultiDiscretizationWrapper::ScaTra2Contact(
    const Epetra_MultiVector & scatra_vec,
    const enum XFEM::FieldName & field) const
{
  const int num_vec = scatra_vec.NumVectors();
  Teuchos::RCP<Epetra_MultiVector> contact_vec = Teuchos::rcp(
      new Epetra_MultiVector( *slave_ndof_row_maps_.at( field ), num_vec) );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::MultiDiscretizationWrapper::ScaTra2Contact(
    const Epetra_MultiVector & scatra_vec,
    Epetra_MultiVector & contact_vec,
    const enum XFEM::FieldName & field) const
{
  if (not IsExporter())
    dserror("Call SetupContactScaTraDofExporter() first!");
  ...
  {
    const int err = contact_vec.Export( scatra_vec, *s2c_exporter,
        Insert );
    if ( err )
      dserror("Export to Contact distribution returned err = %d", err);
  }
  else
  {
    int err = contact_vec.ReplaceMap( scatra_vec.Map() );
    if ( err )
      dserror( "Replacement of the Contact vector map returned err = %d", err );
