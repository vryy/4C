...
void xcontact_dyn(const int& restart)
{
  ...
  if (not struct_dis_ptr->Filled())
    struct_dis_ptr->FillComplete();
  ...
  switch (coupling)
  {
    ...
    {
      dserror("The monolithic coupling scheme is currently unsupported "
    ...
    default:
    {
      dserror("The coupling scheme for the eXtended contact approach "
          "is not defined. Fix your input file! (COUPL_ALGO = %d | %s)",
          coupling,
          INPAR::XCONTACT::FieldCouplingAlgorithm2String(coupling).c_str());
      break;
    }
  }

  // initialize and setup the chosen algorithm
  xcontact_alg_ptr->Init(p_xcontact_dyn,p_structure_dyn,p_scatra_dyn,
      p_xfem_general,struct_dis_ptr);
  xcontact_alg_ptr->Setup();

  if (restart)
    xcontact_alg_ptr->ReadRestart(restart);
