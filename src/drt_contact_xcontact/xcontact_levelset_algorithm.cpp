...
XCONTACT::LEVELSET::Algorithm::Algorithm(
    ...
    : SCATRA::ScaTraTimIntImpl( dis, solver, scatradynparams, extraparams, output ),
      SCATRA::LevelSetAlgorithm( dis, solver, level_set_params, scatradynparams,
              extraparams, output ),
...
void XCONTACT::LEVELSET::Algorithm::Reinitialization()
{
  ...
  if ( reinitaction_ == INPAR::SCATRA::reinitaction_none or
       step_ % ReinitInterval() != 0)
    return;
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_MultiVector> XCONTACT::LEVELSET::Algorithm::
    ComputeNodalGradientL2Projection( const Epetra_Vector & x,
        const enum INPAR::SCATRA::L2ProjectionSystemType & l2_proj_system )
{
  ...
  Teuchos::RCP<Epetra_MultiVector> nodalgradient =
      DRT::UTILS::ComputeNodalL2Projection( *discret_, ActiveRowNodeMap(),
          ActiveColEleMap(), "phinp", dim, eleparams, l2_proj_solver,
  ...
  if ( err )
    dserror( "ReplaceMap failed! ( err = %d )", err );
...
{
  CreateActiveMaps(*Discretization(), phinp, active_row_elements_,
      active_row_nodes_, active_col_elements_, active_dof_map_extractor_ );
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::LEVELSET::Algorithm::CreateActiveMaps(
    const DRT::Discretization & dis,
    const Epetra_Vector & phinp,
    Teuchos::RCP<Epetra_Map> & active_row_elements,
    Teuchos::RCP<Epetra_Map> & active_row_nodes,
    ...
    Teuchos::RCP<LINALG::MultiMapExtractor> & active_dof_map_extractor) const
{
  ...
  for ( int elid=0; elid<num_ele; ++elid )
  {
    ...
    if (dof_lid == -1)
      dserror("The GID %d was not found in the given state vector!",
          dof_gids_per_ele[0]);
        ...
        cit != dof_gids_per_ele.end(); ++cit )
    {
      ...
      if (dof_lid == -1)
        dserror("The GID %d was not found in the given state vector!", *cit );
    ...
    if ( not ele->HasOnlyGhostNodes( dis.Comm().MyPID() ) )
    {
      ...
      for ( std::vector<int>::const_iterator cit = dof_gids_per_ele.begin();
          cit != dof_gids_per_ele.end(); ++cit )
      {
        if ( dis.DofRowMap()->LID( *cit ) > -1 )
          rdof_gids.insert( *cit );
      }
      ...
      {
        if ( dis.NodeRowMap()->LID( *cit ) > -1 )
          rnode_gids.insert( *cit );
      }
    }
  ...
  active_dof_map_extractor = Teuchos::rcp( new LINALG::MultiMapExtractor( *dis.DofRowMap(), dof_maps ) );
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XCONTACT::LEVELSET::Algorithm::CreateNewMap(
