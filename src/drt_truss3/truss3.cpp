...
{
  if ( isinit_ )
    dserror(" You are about to setup an element that has already been set up. That does not make sense.");
  ...
  lrefe_ = std::pow( ( X_(3) - X_(0) ) * ( X_(3) - X_(0) ) +
                     ( X_(4) - X_(1) ) * ( X_(4) - X_(1) ) +
                     ( X_(5) - X_(2) ) * ( X_(5) - X_(2) ) , 0.5 );
  ...
  for ( int i = 0; i< 6 ; ++i )
    abs_rotrefe += rotrefe[i] * rotrefe[i];
