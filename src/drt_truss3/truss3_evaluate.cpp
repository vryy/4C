  ...
  for (int node = 0; node < nnode; node++)
  {
    ...
    for (int k = 0; k < 3; k++)
    {
      elemat1(node * 6 + k, node * 6 + k) +=
          params.get<double>("ctransptc", 0.0) * 0.5 * jacobinode_[node];
    }
  }
    ...
    Epetra_SerialDenseMatrix& stiffmat)
{
  ...
  // computing global internal forces
  for (unsigned int i = 0; i < 6; ++i) forcevec(i) = ym * crosssec_ * epsilon * lrefeinv * aux(i);
 ...
 *-------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Truss3::CalcDeltaTheta(
    std::vector<double>& disp, LINALG::Matrix<1, 3>& thetacurr)
{
    ...
    Epetra_SerialDenseVector* force)
{
  ...
  // 1x9 force vector of the Truss at node 1. Contributions to vector {t1, d1, d2} in respective
  // order
  ...
  /*%%%%%%%%%%%%%%%%%%%% Calculate torsional stiffness matrices and forces at node 2
   * linker%%%%%%%%%%%%%%%%*/
...
/*-------------------------------------------------------------------------------------------------------------------------------*
 | Calculate torsional stiffness matrices and forces between tangent of beam and directional vector
 of truss      mukherjee 09/14|
 *-------------------------------------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Truss3::MyTorsionalStiffatNodeCos(Teuchos::ParameterList& params, double theta,
    double deltatheta, LINALG::Matrix<1, 3>& tcurr, LINALG::Matrix<1, 3>& diff_disp,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    A(i) += tcurr(i) * s / (pow(norm_t, 2) * sin(theta)) -
            diff_disp(i) / (norm_t * norm_v * sin(theta));
    B(i) += diff_disp(i) * s / (pow(norm_v, 2) * sin(theta)) -
            tcurr(i) / (norm_t * norm_v * sin(theta));
  }
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
    {
      dadt1(i, j) += (i == j) * s / (pow(norm_t, 2) * sin(theta)) -
                     2 * s * tcurr(i) * tcurr(j) / (sin(theta) * pow(norm_t, 4)) -
                     tcurr(i) * A(j) / (pow(norm_t, 2) * pow(sin(theta), 2)) +
                     diff_disp(i) * tcurr(j) / (pow(norm_t, 3) * norm_v * sin(theta)) +
                     diff_disp(i) * A(j) * s / (norm_t * norm_v * pow(sin(theta), 2));

      dadv1(i, j) += diff_disp(i) * diff_disp(j) / (norm_t * pow(norm_v, 3) * sin(theta)) +
                     diff_disp(i) * B(j) * s / (norm_t * norm_v * pow(sin(theta), 2)) -
                     (i == j) / (norm_t * norm_v * sin(theta)) -
                     tcurr(i) * B(j) / (pow(norm_t, 2) * pow(sin(theta), 2));

      dbdt1(i, j) += (i == j) / (norm_t * norm_v * sin(theta)) -
                     tcurr(i) * tcurr(j) / (pow(norm_t, 3) * norm_v * sin(theta)) -
                     tcurr(i) * A(j) * s / (norm_t * norm_v * pow(sin(theta), 2)) +
                     diff_disp(i) * A(j) / (pow(norm_v, 2) * pow(sin(theta), 2));

      dbdv1(i, j) += -(i == j) * s / (pow(norm_v, 2) * sin(theta)) +
                     2 * s * diff_disp(i) * diff_disp(j) / (sin(theta) * pow(norm_v, 4)) +
                     diff_disp(i) * B(j) / (pow(norm_v, 2) * pow(sin(theta), 2)) -
                     tcurr(i) * diff_disp(j) / (norm_t * pow(norm_v, 3) * sin(theta)) -
                     tcurr(i) * B(j) * s / (norm_t * norm_v * pow(sin(theta), 2));
    }
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
    {
      ...
      TorStiffmatrix(i, j + 3) +=
          spring * aux_c(i) * aux_c(j + 3) + spring * deltatheta * -dadv1(i, j);
      TorStiffmatrix(i, j + 6) +=
          spring * aux_c(i) * aux_c(j + 6) + spring * deltatheta * dadv1(i, j);
      TorStiffmatrix(i + 3, j) +=
          spring * aux_c(i + 3) * aux_c(j) + spring * deltatheta * dbdt1(i, j);
      TorStiffmatrix(i + 3, j + 3) +=
          spring * aux_c(i + 3) * aux_c(j + 3) + spring * deltatheta * -dbdv1(i, j);
      TorStiffmatrix(i + 3, j + 6) +=
          spring * aux_c(i + 3) * aux_c(j + 6) + spring * deltatheta * dbdv1(i, j);
      TorStiffmatrix(i + 6, j) +=
          spring * aux_c(i + 6) * aux_c(j) + spring * deltatheta * -dbdt1(i, j);
      TorStiffmatrix(i + 6, j + 3) +=
          spring * aux_c(i + 6) * aux_c(j + 3) + spring * deltatheta * dbdv1(i, j);
      TorStiffmatrix(i + 6, j + 6) +=
          spring * aux_c(i + 6) * aux_c(j + 6) + spring * deltatheta * -dbdv1(i, j);
    }
...
void DRT::ELEMENTS::Truss3::MyTorsionalStiffatNodeDot(Teuchos::ParameterList& params,
    const int node, LINALG::Matrix<1, 3>& tangentcurr, LINALG::Matrix<1, 6>& xcurr,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  ...
  LINALG::Matrix<3, 1, FAD> vcurr(true);
  // current tangent
  LINALG::Matrix<3, 1, FAD> tcurr(true);
  // reference diff_disp = d2 -d1
  LINALG::Matrix<3, 1, FAD> vref(true);
  // reference tangent
  LINALG::Matrix<3, 1, FAD> tref(true);
  // unit current diff_disp = d2 -d1
  LINALG::Matrix<3, 1, FAD> vcurr_unit(true);
  // unit current tangent
  LINALG::Matrix<3, 1, FAD> tcurr_unit(true);
  // unit reference diff_disp = d2 -d1
  LINALG::Matrix<3, 1, FAD> vref_unit(true);
  // unit reference tangent
  LINALG::Matrix<3, 1, FAD> tref_unit(true);
  ...
  FAD W = 0.5 * spring *
          pow((FADUTILS::ScalarProduct(tcurr_unit, vcurr_unit) -
                  FADUTILS::ScalarProduct(tref_unit, vref_unit)),
              2);
  ...
  // Auxiliary matrix
  LINALG::Matrix<3, 3, FAD> t_aux(true);
  LINALG::Matrix<3, 3, FAD> v_aux(true);
 ...
 *-------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Truss3::MyTorsionalStiffatNodeSin(Teuchos::ParameterList& params, double theta,
    double deltatheta, LINALG::Matrix<1, 3>& tcurr, LINALG::Matrix<1, 6>& xcurr,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  if (theta == 0.0) return;
  ...
  // diff_disp = d2 -d1
  LINALG::Matrix<1, 3, FAD> diff_disp;
  // t1 = tangent
  ...
  LINALG::Matrix<1, 3, FAD> V(true);
  LINALG::Matrix<1, 3, FAD> T(true);
  ...
  LINALG::Matrix<1, 3, FAD> A(true);  // Identical to "A" in derivation
  LINALG::Matrix<1, 3, FAD> B(true);  // Identical to "B" in derivation

  for (int i = 0; i < 3; i++)
  {
    A(i) += -tangent(i) * sin(Theta) / (pow(norm_t, 2) * cos(Theta)) +
            T(i) / (norm_t * norm_v * cos(Theta));
    B(i) += -diff_disp(i) * sin(Theta) / (pow(norm_v, 2) * cos(Theta)) +
            V(i) / (norm_t * norm_v * cos(Theta));
  }

  // Calculate auxiliary vectors for computation of forces and stiffnessmatrix
  LINALG::Matrix<1, totdof, FAD> aux_c(true);  // Identical to "c" in derivation
  ...
  for (int i = 0; i < totdof; i++)
    for (int j = 0; j < totdof; j++) TorStiffmatrix(i, j) = TorForceFAD(i).dx(j);
 ...
 *-------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Truss3::MyTorsionalStiffTangentCos(Teuchos::ParameterList& params, double theta,
    double deltatheta, LINALG::Matrix<1, 3>& tcurr1, LINALG::Matrix<1, 3>& tcurr2,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  // Norms of the tangential vectors and directional displacement vector
...
void DRT::ELEMENTS::Truss3::MyTorsionalStiffTangentDot(Teuchos::ParameterList& params,
    LINALG::Matrix<1, 3>& tangentcurr1, LINALG::Matrix<1, 3>& tangentcurr2,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  // see also so_nstet_nodalstrain.cpp, so_nstet.H, autodiff.cpp and autodiff.H
  // total no of dofs
...
/*-------------------------------------------------------------------------------------------------*
 ...
 | between tangents of beam at angle smaller than 45 degrees                         mukherjee
 09/14|
 *-------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::Truss3::MyTorsionalStiffatTangentSin(Teuchos::ParameterList& params,
    double theta, double deltatheta, LINALG::Matrix<1, 3>& tcurr1, LINALG::Matrix<1, 3>& tcurr2,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  if (theta == 0.0) return;
  ...
  LINALG::Matrix<1, 3, FAD> crossprod;
  crossprod.Clear();
  FAD Theta = 0.0;
  FAD SinTheta = 0.0;
  ...
  // Auxilary vectors for calculation of vectors
  LINALG::Matrix<1, 3, FAD> T1(true);
  LINALG::Matrix<1, 3, FAD> T2(true);
  ...
  for (int i = 0; i < 3; i++)
  {
    A(i) += -tcurr1(i) * sin(theta) / (pow(norm_t1, 2) * cos(theta)) +
            T1(i) / (norm_t1 * norm_t2 * cos(theta));
    B(i) += -tcurr2(i) * sin(theta) / (pow(norm_t2, 2) * cos(theta)) +
            T2(i) / (norm_t1 * norm_t2 * cos(theta));
  }
...
void DRT::ELEMENTS::Truss3::FADMyTorsionalStiffatNodeCos(Teuchos::ParameterList& params,
    double theta_0, LINALG::Matrix<1, 3>& tcurr, LINALG::Matrix<1, 6>& xcurr,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  // see also so_nstet_nodalstrain.cpp, so_nstet.H, autodiff.cpp and autodiff.H
  // FAD calculated stiff matrix for validation purposes
  LINALG::Matrix<9, 9, FAD> stiffmatrix_check(true);
  LINALG::Matrix<1, 9, FAD> force_check(true);
  ...
  norm_v = pow(norm_v, 0.5);
  norm_t = pow(norm_t, 0.5);
  if (norm_t == 0.0) norm_t = 1.0e-14;
  if (norm_v == 0.0) norm_v = 1.0e-14;
  ...
  for (int j = 0; j < 3; ++j) dotprod += tangent(0, j) * diff_disp(0, j);
  ...
  LINALG::Matrix<1, 9, FAD> aux_c(true);  // Identical to "c" in derivation

  for (int j = 0; j < 3; j++)
  {
    aux_a(0, j) = -diff_disp(0, j) / (norm_v * norm_t * sin(theta)) +
                  tangent(0, j) * s / (pow(norm_t, 2) * sin(theta));
    aux_b(0, j) = tangent(0, j) / (norm_v * norm_t * sin(theta)) -
                  diff_disp(0, j) * s / (pow(norm_v, 2) * sin(theta));
    aux_c(0, j) = aux_a(0, j);
  ...
  // Calculate torsional forces
  for (int j = 0; j < 9; j++) force_check(0, j) = spring * deltatheta * aux_c(0, j);
...
void DRT::ELEMENTS::Truss3::FADMyTorsionalStiffTangentCos(Teuchos::ParameterList& params,
    double theta_0, LINALG::Matrix<1, 3>& tcurrNode1, LINALG::Matrix<1, 3>& tcurrNode2,
    Epetra_SerialDenseMatrix& TorStiffmatrix, Epetra_SerialDenseVector& TorForce)
{
  // see also so_nstet_nodalstrain.cpp, so_nstet.H, autodiff.cpp and autodiff.H
  // FAD calculated stiff matrix for validation purposes
...
void DRT::ELEMENTS::Truss3::FADThetaLinearisation(LINALG::Matrix<1, 3>& diff_disp,
    LINALG::Matrix<1, 3>& tcurr, LINALG::Matrix<1, 3>& A, LINALG::Matrix<1, 3>& B)
{
