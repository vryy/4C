...
NOX::IMMERSED::AitkenRelaxationImmersedAle::AitkenRelaxationImmersedAle(const Teuchos::RCP<NOX::Utils>& utils,
...
bool NOX::IMMERSED::AitkenRelaxationImmersedAle::reset(const Teuchos::RCP<NOX::GlobalData>& gd,
                                       Teuchos::ParameterList& params)
{
  Teuchos::ParameterList& p = params.sublist("Aitken");
  ...
  // check for maxstep
  if (maxstep > 0. && maxstep < 1.-nu_ale_)
    nu_ale_ = 1.-maxstep;

  if (maxstep > 0. && maxstep < 1.-nu_immersed_)
    nu_immersed_ = 1.-maxstep;
  ...
  // check for minstep
  if (minstep < 0. && minstep > 1.-nu_ale_)
    nu_ale_ = 1.-minstep;

  if (minstep < 0. && minstep > 1.-nu_immersed_)
    nu_immersed_ = 1.-minstep;

  if (minstep < 0. && minstep > 1.-nu_)
    nu_ = 1.-minstep;
...
bool NOX::IMMERSED::AitkenRelaxationImmersedAle::compute(Abstract::Group& grp, double& step,
                                         ...
                                         const Solver::Generic& s)
{
  ...
  Teuchos::RCP<NOX::Epetra::Vector> epetra_del = Teuchos::rcp_dynamic_cast<NOX::Epetra::Vector>(del_);
  if(epetra_del == Teuchos::null)
    dserror("cast failed");

  Teuchos::RCP<NOX::Epetra::Vector> epetra_del2 = Teuchos::rcp_dynamic_cast<NOX::Epetra::Vector>(del2_);
  if(epetra_del == Teuchos::null)
    dserror("cast failed");

  // ale part
  double step_ale = -1234.0;
  Teuchos::RCP<Epetra_Vector> epetra_del2_ale = adapter_->CombinedInterface()->ExtractCondVector(epetra_del2->getEpetraVector());
  Teuchos::RCP<Epetra_Vector> epetra_del_ale = adapter_->CombinedInterface()->ExtractCondVector(epetra_del->getEpetraVector());
  ...
  Teuchos::RCP<Epetra_Vector> epetra_del2_immersed = adapter_->CombinedInterface()->ExtractOtherVector(epetra_del2->getEpetraVector());
  Teuchos::RCP<Epetra_Vector> epetra_del_immersed = adapter_->CombinedInterface()->ExtractOtherVector(epetra_del->getEpetraVector());
  ...
  utils_->out() << "          AVG. STEP =          "<<std::setprecision(13)<<((step_ale+step_immersed)/2.0) << "\n";

  // set step to 1.0 (only used as a dummy)
  step=(step_ale+step_immersed)/2.0;

  // build new dir with different relaxation for immersed and ale part here and add this step with dummy step size = 1.0 in computeX
  Teuchos::RCP<NOX::Epetra::Vector> combined_newdir = Teuchos::rcp(new NOX::Epetra::Vector(epetra_del->getEpetraVector()));
  combined_newdir->init(0.0);
  double norm = combined_newdir->norm();
  if(norm != 0.0)
    dserror("norm should be 0.0 here. Norm=%f",norm);

  // copy dir in plain dir
  Teuchos::RCP<NOX::Epetra::Vector> plain_dir = Teuchos::rcp(new NOX::Epetra::Vector(epetra_del->getEpetraVector()));
  plain_dir->init(0.0);
  norm = plain_dir->norm();
  if(norm != 0.0)
    dserror("norm should be 0.0 here. Norm=%f",norm);
  ...
  Teuchos::RCP<Epetra_Vector> dir_ale = adapter_->CombinedInterface()->ExtractCondVector(plain_dir->getEpetraVector());
  dir_ale->Scale(step_ale);

  Teuchos::RCP<Epetra_Vector> dir_immersed = adapter_->CombinedInterface()->ExtractOtherVector(plain_dir->getEpetraVector());
  dir_immersed ->Scale(step_immersed);

  // insert those two parts into combined direction vector
  adapter_->CombinedInterface()->InsertCondVector(dir_ale,Teuchos::rcpFromRef(combined_newdir->getEpetraVector()));
  adapter_->CombinedInterface()->InsertOtherVector(dir_immersed,Teuchos::rcpFromRef(combined_newdir->getEpetraVector()));
