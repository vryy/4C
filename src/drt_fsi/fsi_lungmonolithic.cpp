  ...
  : BlockMonolithic(comm,timeparams)
{
  ...
  Teuchos::RCP<ADAPTER::FluidLung> fluidfield = Teuchos::rcp_dynamic_cast<ADAPTER::FluidLung>(FluidField());
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> FSI::LungMonolithic::StructToAleOutflow(Teuchos::RCP<Epetra_Vector> iv) const
{
...
void FSI::LungMonolithic::Evaluate(Teuchos::RCP<const Epetra_Vector> x)
{
  ...
  fluidfield->EvaluateVolCon(AddFluidShapeDerivMatrix_, FluidConstrMatrix_, ConstrFluidMatrix_,
                            AleConstrMatrix_, ConstrAleMatrix_, AddFluidRHS_,
                            CurrFlowRatesRed, LagrMultVecRed, OffsetID_, dttheta);
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void FSI::LungMonolithic::ScaleSystem(LINALG::BlockSparseMatrixBase& mat,
                                      Epetra_Vector& b)
{
...
FSI::LungMonolithic::CreateStatusTest(Teuchos::ParameterList& nlParams,
