...
void FSI::DirichletNeumannVolCoupl::Setup()
{
  ...
  if(!displacementcoupling_)
    dserror("Currently only displacement coupling is supported!");
    ...
    const Epetra_Comm& comm)
{
  ...
  Teuchos::RCP< ADAPTER::FluidAle> fluidale =
      Teuchos::rcp_dynamic_cast<ADAPTER::FluidAle>( fluid_ );
  ...
  coupsa_->Init( StructureField()->Discretization(),
      fluidale->AleField()->WriteAccessDiscretization(),
      &coupleddof12,
      &coupleddof21,
      &dofsets12,
      &dofsets21,
...
FSI::DirichletNeumannVolCoupl::FluidOp(Teuchos::RCP<Epetra_Vector> idisp,
                               const FillType fillFlag)
{
  FSI::Partitioned::FluidOp(idisp,fillFlag);

  //TODO cant this be done better?
  Teuchos::RCP<Epetra_Vector> vdisp =
...
void FSI::InterfaceCorrector::SetInterfaceDisplacements(
    Teuchos::RCP<Epetra_Vector>& idisp_struct,
    ADAPTER::Coupling& icoupfs)
{
  idisp_ = idisp_struct;
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::InterfaceCorrector::CorrectInterfaceDisplacements(
    Teuchos::RCP<Epetra_Vector> disp_fluid,
    Teuchos::RCP<FLD::UTILS::MapExtractor>const& finterface)
{
  if(icoupfs_ == Teuchos::null)
    dserror("Coupling adapter not set!");
  if(idisp_ == Teuchos::null)
    dserror("Interface displacements not set!");
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::VolCorrector::CorrectVolDisplacements(
    Teuchos::RCP< ADAPTER::FluidAle> fluidale,
    Teuchos::RCP<Epetra_Vector> deltadisp,
    Teuchos::RCP<Epetra_Vector> disp_fluid,
    Teuchos::RCP<FLD::UTILS::MapExtractor>const& finterface)
{
  if(fluidale->AleField()->Discretization()->Comm().MyPID() == 0)
    std::cout<<"******************   FSI Volume Correction Step   **********************"<<std::endl;

  // correction step in parameter space
  if(true)
    CorrectVolDisplacementsParaSpace(fluidale,deltadisp,disp_fluid,finterface);
  ...
  // output
  if(fluidale->AleField()->Discretization()->Comm().MyPID() == 0)
    std::cout<<"******************FSI Volume Correction Step Done***********************"<<std::endl;
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::VolCorrector::CorrectVolDisplacementsParaSpace(
    Teuchos::RCP< ADAPTER::FluidAle> fluidale,
    Teuchos::RCP<Epetra_Vector> deltadisp,
    Teuchos::RCP<Epetra_Vector> disp_fluid,
    Teuchos::RCP<FLD::UTILS::MapExtractor>const& finterface)
{
  ...
  Teuchos::RCP<Epetra_Vector>  DofColMapDummy = Teuchos::rcp(new Epetra_Vector(*fluidale->FluidField()->Discretization()->DofColMap(),true));
  LINALG::Export(*deltadisp,*DofColMapDummy);

  const double tol = 1e-5;

  // loop over ale eles
  for (std::map<int,std::vector<int> >::iterator it=fluidalenodemap_.begin(); it!=fluidalenodemap_.end(); ++it)
  {
   ...
   for(size_t i = 0; i < it->second.size() ; ++i)
   {
     ...
     if (fluidnode->Owner() != fluidale->AleField()->Discretization()->Comm().MyPID())
       continue;
     ...
     if(aleele->Shape()==DRT::Element::quad4 or
        aleele->Shape()==DRT::Element::hex8 )
       fac = 1.0 - 0.5 * dist;
     else
       dserror("ERROR: element type not implemented!");
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::VolCorrector::CorrectVolDisplacementsPhysSpace(
    Teuchos::RCP< ADAPTER::FluidAle> fluidale,
    Teuchos::RCP<Epetra_Vector> deltadisp,
    Teuchos::RCP<Epetra_Vector> disp_fluid,
    Teuchos::RCP<FLD::UTILS::MapExtractor>const& finterface)
{
  ...
  std::map<int, LINALG::Matrix<9, 2> > CurrentDOPs = CalcBackgroundDops(
      fluidale->FluidField()->Discretization());
  ...
  for (int i = 0; i < fluidale->AleField()->Discretization()->NumMyColElements(); ++i)
  {
    ...
    if(FSIaleeles->find(aleele->Id()) == FSIaleeles->end())
      continue;
  }
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::VolCorrector::Setup(
    const int dim,
    Teuchos::RCP< ADAPTER::FluidAle> fluidale)
{
  if(fluidale->AleField()->Discretization()->Comm().MyPID() == 0)
    std::cout<<"******************FSI Volume Correction Setup***********************"<<std::endl;
  ...
  searchTree_ = Teuchos::rcp(
      new GEO::SearchTree(5));

  // find the bounding box of the elements and initialize the search tree
  const LINALG::Matrix<3, 2> rootBox = GEO::getXAABBofDis(*fluidale->FluidField()->Discretization(),
      currentpositions);
  searchTree_->initializeTree(rootBox, *fluidale->FluidField()->Discretization(), GEO::TreeType(GEO::OCTTREE));
  ...
  // loop over ale eles
  for (std::map<int,std::vector<int> >::iterator it=fluidaleelemap_.begin(); it!=fluidaleelemap_.end(); ++it)
  {
    DRT::Element* aleele = fluidale->AleField()->Discretization()->gElement(it->first);
   ...
   for(size_t i = 0; i < it->second.size() ; ++i)
   {
     ...
     for(int j=0; j<fluidele->NumNode(); ++j)
     {
       ...
       if(globalnodeids.find(nodegid) != globalnodeids.end())
         continue;
  ...
  if(fluidale->AleField()->Discretization()->Comm().MyPID() == 0)
    std::cout<<"******************FSI Volume Correction Setup Done***********************"<<std::endl;
