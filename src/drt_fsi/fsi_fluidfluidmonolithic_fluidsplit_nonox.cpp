 ...
 void FSI::FluidFluidMonolithicFluidSplitNoNOX::RecoverLagrangeMultiplier()
{
  ...
  {
    Teuchos::RCP<Epetra_Vector> fmggddg=Teuchos::rcp(new Epetra_Vector(fmggprev_->RangeMap(),true));
  ...
  {
    Teuchos::RCP<Epetra_Vector> fgidui=Teuchos::rcp(new Epetra_Vector(fgiprev_->RangeMap(),true));
  ...
  if (fmgiprev_ != Teuchos::null)
  {
    ...
    Teuchos::RCP<Epetra_Vector> fmgiddia = Teuchos::rcp(new Epetra_Vector(fmgiprev_->RangeMap(),true));
    ...
    Teuchos::RCP<const Epetra_Map> innerfluidpresmap = LINALG::MultiMapExtractor::IntersectMaps(fluidpresmaps);

    // To keep merged fluid velocity and pressure DOF apart
    Teuchos::RCP<LINALG::MapExtractor> innerfluidvelextractor = Teuchos::rcp(new LINALG::MapExtractor(
...
void FSI::FluidFluidMonolithicFluidSplitNoNOX::HandleFluidDofMapChangeInNewton()
{
  ...
  *x_sum_n = *x_sum_;
  ...
  Teuchos::RCP<Epetra_Vector> ff_stepinc = FluidField()->Interface()->ExtractOtherVector(FluidField()->Stepinc());
