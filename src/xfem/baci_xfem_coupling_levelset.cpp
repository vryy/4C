...
#include "baci_xfem_coupling_levelset.H"
    ...
    : LevelSetCoupling(bg_dis, cond_name, cond_dis, coupling_id, time, step),
      twophasedyn_(
          DRT::Problem::Instance()
              ->TwoPhaseFlowParams()),  // access parameter for two phase flow TODO: remove Problem
...
{
  // TODO: for the moment we assume matching! introduce a check here, maybe directly clone the dis
  // here, or extract one if(matching) then clone strategy, otherwise create Dis from condition...
  cutter_dis_ = cond_dis_;
  ...
  cutter_nds_vel_ =
      dofset_coupling_map_["vel_fluid_proxy_in_scatra"];  // dofset id for transport velocity
}
...
void XFEM::LevelSetCouplingTwoPhase::SetParameters_SurfaceTension()
{
  ...
  surftensapprox_ =
      DRT::INPUT::IntegralValue<INPAR::TWOPHASE::SurfaceTensionApprox>(params, "SURFTENSAPPROX");
  laplacebeltrami_ =
      DRT::INPUT::IntegralValue<INPAR::TWOPHASE::LaplaceBeltramiCalc>(params, "LAPLACE_BELTRAMI");
  smoothedgradphi_ =
      DRT::INPUT::IntegralValue<INPAR::TWOPHASE::SmoothGradPhi>(params, "SMOOTHGRADPHI");
  ...
  if (DRT::INPUT::IntegralValue<INPAR::TWOPHASE::NodalCurvatureCalc>(params, "NODAL_CURVATURE") !=
...
/*------------------------------------------------------------------------------------------------*
 *------------------------------------------------------------------------------------------------*/
void XFEM::LevelSetCouplingTwoPhase::SetFlags_InterfaceTransport(DRT::Condition* cond)
{
  // TODO: think about alternatives for twophase flow! more accurate interface transport needed
  // normal transport is implemented, however not fully tested so far! (see combustion coupling
  // object!)
  transport_direction_ = INPAR::TWOPHASE::transport_dir_all;
}
...
)
{
  ...
  for (int lnodeid = 0; lnodeid < cutter_dis_->NumMyRowNodes(); ++lnodeid)
  {
    ...
    // u_relative_flame = flvelrel  = the discontinuous relative flame velocity ( M/rho^i in Omega^i
    // and M/rho^j in Omega^j
    //                             where the mass flow rate M = -rho^i*sl with sl the laminar
    //                             flamespeed w.r.t Omega
    ...
    GetSmoothedQuantitiesAtNode(nvec, curv, cutter_node, gradphinp_smoothed_node_col_,
        curvaturenp_node_col_, cutter_dis_, cutter_nds_phi_);
    ...
    if (require_smoothedgradphi_)
    {
      ...
      {
        std::cout << "\n/!\\ phi gradient too small at node " << cutter_node->Id()
                  << " -> interface velocity is only the convective velocity" << std::endl;
      }
    ...
    if (lm_fld.size() != (nsd_ + 1)) dserror("we expect fluid and pressure dofs here!");
    ...
    {
      // ELSE: nothing to do -- // OPTION 1: use the full convective velocity (normal and tangential
      // parts) just continue in this loop
    }
    else
    ...
    // add fluid velocity (Navier Stokes solution) and relative flame velocity, in addition set the
    // pressure dof to zero!
    for (size_t icomp = 0; icomp < nsd_ + 1; ++icomp)
    {
      double tmp = 0.0;
      if (icomp != nsd_) tmp = flvelabs(icomp);
...
void XFEM::LevelSetCouplingTwoPhase::SetupConfigurationMap()
{
  if (GetAveragingStrategy() == INPAR::XFEM::Harmonic)
  {
    ...
    dserror(
        "XFEM::LevelSetCouplingTwoPhase: You want to initialize another strategy than harmonic?");
  return;
}
...
/*------------------------------------------------------------------------------------------------*
 *------------------------------------------------------------------------------------------------*/
void XFEM::LevelSetCouplingTwoPhase::GetViscositySlave(DRT::Element* coup_ele,  ///< xfluid ele
    ...
    double& kappa_m)
{
  if (GetAveragingStrategy() == INPAR::XFEM::Harmonic)
  {
    ...
    if (kappa_m > 1.0 || kappa_m < 0.0)
      dserror("Nitsche weights for inverse estimate kappa_m lies not in [0,1]: %d", kappa_m);
  }
  else
    dserror(
        "XFEM::LevelSetCouplingTwoPhase: GetCouplingSpecificAverageWeights not implemented for "
    ...
    Teuchos::RCP<DRT::Discretization>& dis, const int nds)
{
  ...
  if (require_smoothedgradphi_)
  {
    XFEM::UTILS::ExtractQuantityAtElement(
        normal, element, gradphinp_smoothed_node_col, dis, nds, nsd_);
  }
}
...
/*------------------------------------------------------------------------------------------------*
 *------------------------------------------------------------------------------------------------*/
void XFEM::LevelSetCouplingTwoPhase::ReadRestart(const int step, const int lsc_idx)
{
  //-------- boundary discretization
  IO::DiscretizationReader boundaryreader(cutter_dis_, step);
  ...
  if (myrank_ == 0)
  {
    IO::cout << "           RESTART IS PERFORMED FROM STORED VALUES!                            "
             << IO::endl;
    IO::cout << "ReadRestart for Level Set Cut in Xfluid (time=" << time << " ; step=" << step
             << ")" << IO::endl;
  }
...
void XFEM::LevelSetCouplingTwoPhase::GmshOutput(const std::string& filename_base, const int step,
    const int gmsh_step_diff, const bool gmsh_debug_out_screen)
{
  std::ostringstream filename_base_fsi;
  filename_base_fsi << filename_base << "_levelset";
...

  {
    // add 'View' to Gmsh postprocessing file
    gmshfilecontent << "View \" "
                    << "SOLcutter-smoothedcurvature \" {" << std::endl;
    IO::GMSH::ScalarFieldDofBasedToGmsh(
        cutter_dis_, curvaturenp_node_, cutter_nds_phi_, gmshfilecontent);
    gmshfilecontent << "};" << std::endl;
  }

  {
    // add 'View' to Gmsh postprocessing file
    gmshfilecontent << "View \" "
                    << "SOLcutter-transportvelocity \" {" << std::endl;
    // draw vector field, dof based (as cutter_transport_vel_, contains pressure info as well)
    IO::GMSH::VelocityPressureFieldDofBasedToGmsh(
        cutter_dis_, cutter_transport_vel_, "velocity", gmshfilecontent, cutter_nds_vel_);
    gmshfilecontent << "};" << std::endl;
  }
}
...
/*------------------------------------------------------------------------------------------------*
 *------------------------------------------------------------------------------------------------*/
void XFEM::LevelSetCouplingCombustion::SetParameters_Physical(DRT::Condition* cond)
{
...
/*------------------------------------------------------------------------------------------------*
 *------------------------------------------------------------------------------------------------*/
void XFEM::LevelSetCouplingCombustion::SetFlags_InterfaceTransport(DRT::Condition* cond)
{
  // use just the interface-normal part of the convective velocity for computing flame transport
  // velocity?
  transport_direction_ =
      static_cast<INPAR::TWOPHASE::Transport_Directions>(cond->GetInt("TRANSPORT_DIRECTIONS"));

  // account for curvature in the transport velocity
  transport_curvature_ = (bool)cond->GetInt("transport_curvature");

  if (transport_curvature_) dserror("curvature-driven transport not tested so far!");
}
...
void XFEM::LevelSetCouplingCombustion::SetFlags_GeometricQuantities()
{
  ...
  if (!transport_curvature_) require_nodalcurvature_ = false;
}
...
void XFEM::LevelSetCouplingCombustion::GetPhaseDensities(
    DRT::Element* ele, double& rhoplus, double& rhominus)
{
    ...
    const CORE::LINALG::SerialDenseVector& nvec, const double& curv)
{
  ...
  else if (position == CORE::GEO::CUT::Point::outside)
  {
    ...
    dserror("what to do now? interface is on a node!");
