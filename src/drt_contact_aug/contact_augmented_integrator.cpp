     ...
     const Teuchos::RCP<CONTACT::ParamsInterface>& cparams_ptr)
{
  ...
  if (ShapeFcn() == INPAR::MORTAR::shape_undefined)
    dserror("ERROR: IntegrateDerivCell3DAuxPlane called without specific shape "
        "function defined!");
  ...
  if ((!sele.IsSlave()) || (mele.IsSlave()))
    dserror("ERROR: IntegrateDerivCell3DAuxPlane called on a wrong type of "
        "MortarElement pair!");
     ...
     const Teuchos::RCP<CONTACT::ParamsInterface>& cparams_ptr)
{
  ...
  for (unsigned test=0;test<meles.size();++test)
  {
    if ((!sele.IsSlave()) || (meles[test]->IsSlave()))
      dserror("ERROR: IntegrateDerivCell3D called on a wrong type of "
          "MortarElement pair!");
  }
    ...
    CONTACT::CoIntegrator* wrapper )
{
  const enum INPAR::CONTACT::VariationalApproach var_type =
      cparams.GetVariationalApproachType();
...
CONTACT::AUG::Integrator<probdim,slavetype,mastertype,IntPolicy>::Integrator(
    CONTACT::ParamsInterface & cparams,
    CONTACT::CoIntegrator& wrapper )
    : IntegratorGeneric( cparams, wrapper ),
      IntPolicy()
{
  /* empty */
}
...
void CONTACT::AUG::Integrator<probdim,slavetype,mastertype,IntPolicy>::IntegrateDerivSegment2D(
       ...
       double& mxib )
{
  dserror("Deprecated method! The segmented based integration is no longer "
      "supported!");
}
...
template < unsigned probdim,
           ...
           DRT::Element::DiscretizationType mastertype,
           class IntPolicy >
void CONTACT::AUG::Integrator<probdim,slavetype,mastertype,IntPolicy>::IntegrateDerivCell3DAuxPlane(
    MORTAR::MortarElement& sele,
    MORTAR::MortarElement& mele,
    MORTAR::IntCell& cell,
    double* auxn )
...
template < unsigned probdim,
           ...
           DRT::Element::DiscretizationType mastertype,
           class IntPolicy >
void CONTACT::AUG::Integrator<probdim,slavetype,mastertype,IntPolicy>::IntegrateDerivSlaveElement(
    MORTAR::MortarElement& sele )
{
  // set evaluator
...
//      Deriv_Jacobian( sele, sxi, sderiv_, stau, derivjac_, &deriv2ndjac_ );
      ...
      // get mval
      shape_function_and_deriv1_and_deriv2<mastertype>( mele, uniqueMxi, mval_,
          mderiv_, mderiv2nd_ );
      ...
      evaluator_->Deriv_MXiGP( sele, mele, &sxi[0], uniqueMxi.A(), uniqueProjalpha,
          sval_, mval_, mderiv_, mtau );

      //**********************************************************************
      // evaluate at GP and lin char. quantities
      //**********************************************************************
      // calculate the averaged normal + derivative at gp level
      GP_Normal_DerivNormal( sele, sval_,&gpn_[0], dn_non_unit_, ddn_non_unit_,
          dn_unit_, ddn_unit_ );
      ...
      IntPolicy::Get_Deriv1st_GapN( sele, mele, sval_, mval_, &gpn_[0], mtau, dmxigp_,
          deriv_gapn_sl_, deriv_gapn_ma_ );
      ...
      IntPolicy::Get_Deriv1st_WGap(sele,lmval_,gapn_sl, gapn_ma,
          wgt, jacslave, derivjac_, deriv_gapn_sl_, deriv_gapn_ma_ );
      ...
      IntPolicy::Get_Deriv1st_WGap_Complete( linsize, sele, mele, sval_, mval_,
          lmval_, &gpn_[0], mtau, dmxigp_, gapn_sl, gapn_ma,
          wgt, jacslave, derivjac_ );

      IntPolicy::Get_Debug( sele, lmval_, gapn_sl, gapn_ma, wgt, jacslave,
          gpn_, uniqueMxi.A() );

      IntPolicy::Get_Deriv1st_Debug( sele, lmval_, sval_, sderiv_, stau, derivjac_,
          dmxigp_, dn_unit_, deriv_gapn_sl_, gapn_sl, wgt, jacslave );
      ...
      switch ( CParams().GetActionType() )
      {
        case MORTAR::eval_force_stiff:
        {
          ...
          IntPolicy::Get_Deriv2nd_WGap( sele, mele, sval_, mval_, lmval_,
              mderiv_, mderiv2nd_, mtau, gpn_, wgt, gapn_sl, gapn_ma, jacslave,
              derivjac_, deriv2ndjac_, dmxigp_, ddmxigp_, dn_unit_, ddn_unit_,
              deriv_gapn_sl_, deriv_gapn_ma_ );

          IntPolicy::Get_Deriv2nd_Debug( sele, lmval_, sval_, sderiv_, stau,
              derivjac_, deriv_gapn_sl_, deriv2ndjac_, ddmxigp_, dn_unit_,
              ddn_unit_, gapn_sl, wgt, jacslave );
...
template < unsigned probdim,
           ...
           DRT::Element::DiscretizationType mastertype,
           class IntPolicy >
void CONTACT::AUG::Integrator<probdim,slavetype,mastertype,IntPolicy>::IntegrateWeightedGapGradientError(
    MORTAR::MortarElement& sele,
    MORTAR::MortarElement& mele,
    bool boundary_ele,
    const CONTACT::INTEGRATOR::UniqueProjInfo& projInfo )
{
...

  {
    ...
    for ( my::gp_id_=0; static_cast<unsigned>(my::gp_id_)<num_gps; ++my::gp_id_ )
    {
      ...
      IntPolicy::Get_Deriv1st_WGapN_Error( sele, active_nlids, lmval_, gpn_,
          gapn_sl, gapn_ma, wgt, jacslave, derivjac_, mtau, dmxigp_,
          deriv_gapn_ma_, *grad_error_ma_ptr, *grad_error_jac_ptr );
