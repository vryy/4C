...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
CONTACT::AUG::ProjectorBase* CONTACT::AUG::ProjectorBase::Get(
    const unsigned probdim,
    DRT::Element::DiscretizationType ref_type,
    ...
    DRT::Element::DiscretizationType tar_type )
{
  switch ( ref_type )
  {
    ...
    default:
      dserror( "Unsupported reference-type %s.",
          DRT::DistypeToString( ref_type ).c_str() );
      exit( EXIT_FAILURE );
  }
}
    ...
    DRT::Element::DiscretizationType tar_type )
{
  switch ( tar_type )
  {
    ...
    default:
      dserror( "Unsupported target-type %s.",
          DRT::DistypeToString( tar_type ).c_str() );
      exit( EXIT_FAILURE );
  }
}
...
CONTACT::AUG::Projector<probdim,ref_type,tar_type>::Instance( bool delete_me )
{
  ...
  if ( not instance )
    instance = new Projector<probdim,ref_type,tar_type>;
...
bool CONTACT::AUG::Projector<probdim,ref_type,tar_type>::operator()(
    MORTAR::MortarElement& ref_ele,
    const double* ref_xi,
    MORTAR::MortarElement& target_ele,
    double* target_xi,
    double& alpha)
{
  Setup();
...
template< unsigned probdim,
          DRT::Element::DiscretizationType ref_type,
          DRT::Element::DiscretizationType tar_type >
bool CONTACT::AUG::Projector<probdim,ref_type,tar_type>::RhsGP(
        LINALG::Matrix<probdim,1>& rhs,
        const LINALG::Matrix<probdim,1>& x_ref,
        const LINALG::Matrix<probdim,1>& n_ref,
        MORTAR::MortarElement& target_ele,
        const LINALG::Matrix<probdim,TAR_NUMNODES>& tar_coords,
        const double* tar_xi,
        const double& alpha) const
{
  LINALG::Matrix<probdim,1> x_tar(false);
  const bool status = GetGlobalPosition<tar_type>( target_ele, tar_coords,
      tar_xi, x_tar );
...
bool CONTACT::AUG::Projector<probdim,ref_type,tar_type>::GetGlobalPosition(
    ...
    const LINALG::Matrix<probdim,numnodes>& coords,
    const double* xi,
...
void CONTACT::AUG::Projector<probdim,ref_type,tar_type>::LMatGP(
        LINALG::Matrix<probdim,probdim>& lmat,
        LINALG::Matrix<TAR_DIM,TAR_NUMNODES>& tar_deriv1,
        MORTAR::MortarElement& tar_ele,
        const LINALG::Matrix<probdim,TAR_NUMNODES>& tar_coords,
        const double* tar_xi,
  ...
  for ( unsigned i=0; i<TAR_NUMNODES; ++i )
    for ( unsigned j=0; j<TAR_DIM; ++j )
      for ( unsigned k=0; k<probdim; ++k )
        lmat(k,j) += tar_deriv1(j,i) * tar_coords(k,i);

  for ( unsigned i=0; i<probdim; ++i )
    lmat(i,probdim-1) = -n_ref(i,0);
}


template CONTACT::AUG::ProjectorBase* CONTACT::AUG::ProjectorBase::Get2D<DRT::Element::line2>( DRT::Element::DiscretizationType tar_type );
template CONTACT::AUG::ProjectorBase* CONTACT::AUG::ProjectorBase::Get3D<DRT::Element::quad4>( DRT::Element::DiscretizationType tar_type );
template CONTACT::AUG::ProjectorBase* CONTACT::AUG::ProjectorBase::Get3D<DRT::Element::tri3>( DRT::Element::DiscretizationType tar_type );
