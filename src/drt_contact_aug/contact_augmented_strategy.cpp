...
void CONTACT::AUG::DataContainer::InitMatrixRowColTransformer()
{
  ...
  mat_row_col_transformer_->Init( redistributed_row, redistributed_col,
      unredistributed_row, unredistributed_col );
}
        ...
        augData_( *augDataPtr_ )
{
  ...
  if ( Data().AddInactivForceContributions() and
    ...
    dserror("The \"ADD_INACTIVE_FORCE_CONTRIBUTIONS\" option is only supported "
        "by the complete variational approach.");

  Data().SetFDCheckType(
      Teuchos::getIntegralValue<INPAR::CONTACT::FDCheck>( p_aug, "FD_CHECK" ) );

  Data().SetPotential( Teuchos::rcp( new Potential( *this, *augDataPtr_ ) ) );

  Data().SetMatrixRowColTransformer( Teuchos::rcp(
      new MORTAR::MatrixRowColTransformer( 4 ) ) );
    ...
    bool init)
{
  if ( init )
  {
    ...
    if ( ParRedist() )
      Data().InitMatrixRowColTransformer();
  }
  ...
  if ( redistributed )
  {
    ...
    {
      Data().GOldActiveSlaveNodesPtr() =
          DRT::UTILS::RedistributeInAccordanceWithReference( SlRowNodes(),
              Data().GOldActiveSlaveNodes() );
    }
  }
  ...
  if ( ParRedist() )
    Data().MatrixRowColTransformer().Setup();
...
{
  if ( cn_init <= 0.0 )
    dserror( "The initial CN value must be greater than zero!" );

  if ( Data().CnPtr().is_null()     or
       Data().Cn().GlobalLength() == 0 )
...
{
  // redistribute the cn-vector
  Teuchos::RCP<Epetra_Vector> newcn =
      Teuchos::rcp( new Epetra_Vector( SlRowNodes() ) );
...
void CONTACT::AUG::Strategy::InitMortar()
{
  ...
  Data().BMatrixPtr() = Teuchos::rcp( new LINALG::SparseMatrix(
      Data().GSlNormalDofRowMap(),100,false,false,
...
void CONTACT::AUG::Strategy::SplitMortar()
{
  ...
  Data().DMatrixPtr() = ExtractMatrix( Data().BMatrix(),
      Data().GActiveNDofRowMap(), *Data().GSlDofRowMapPtr() );

  Data().MMatrixPtr() = ExtractMatrix( Data().BMatrix(),
      Data().GActiveNDofRowMap(), *Data().GMaDofRowMapPtr() );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::CreateStiffnessState( const Epetra_Map &
    gAugInactiveSlaveDofs )
{
  ...
  Data().DLmTLmTMatrixPtr()     = Teuchos::rcp(
      new LINALG::SparseMatrix(Data().GActiveTDofRowMap(),100,false,false));
  ...
  Data().InactiveDDMatrixPtr() =
      Teuchos::rcp(new LINALG::SparseMatrix(SlDoFRowMap(true),100,false,false,
          LINALG::SparseMatrix::FE_MATRIX));
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::CreateRhsState( const Epetra_Map &
    gAugInactiveSlaveDofs )
{
  ...
  Data().WGapPtr()     = Teuchos::rcp( new Epetra_Vector(
      Data().GActiveNDofRowMap(), true ) );
  Data().WGapAllSlNodesPtr() = Teuchos::rcp( new Epetra_Vector(
      Data().GSlNormalDofRowMap(), true ) );

  Data().SlForceLmPtr() = Teuchos::rcp( new Epetra_Vector(
      SlDoFRowMap(true), true ) );
  Data().SlForceLmInactivePtr() = Teuchos::rcp( new Epetra_Vector(
      SlDoFRowMap(true), true ) );
...
void CONTACT::AUG::Strategy::PostEvalForceStiff(
    CONTACT::ParamsInterface& cparams )
{
  if ( not IsInContact()  and
       not WasInContact() and
       not WasInContactLastTimeStep() )
    return;
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::EvalInterface(
    CONTACT::AUG::Interface& interface,
    const int rriter,
    const Teuchos::RCP<CONTACT::ParamsInterface>& cparams_ptr )
{
  ...
  switch (atype)
  {
    ...
    default:
    {
      dserror( "What shall be integrated? (enum=%d | \"%s\")",
          atype, MORTAR::ActionType2String( atype ).c_str() );
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::RunPostComputeX(
    ...
    const Epetra_Vector& xnew)
{
  ...
  if ( err )
    dserror("ReplaceMap failed with error code %d.", err );
...
void CONTACT::AUG::Strategy::AssembleContactStiff()
{
        ...
        cit != interface_.end(); ++cit )
  {
    ...
    interface.AssembleAugInactiveDiagMatrix(Data().InactiveDiagMatrix(),
        Data().Cn(),InactiveScaleFactor());
    ...
    interface.AssembleInactiveDDMatrix(Data().InactiveDDMatrix(),Data().Cn(),
        InactiveScaleFactor());
  }
    ...
    const CONTACT::ParamsInterface& cparams )
{
  ...
  enum class ASStatus : int { unchanged = 0, changed =1 };
  enum ASStatus lstatus = ASStatus::unchanged;
        ...
        cit != interface_.end(); ++cit )
  {
    ...
    for ( int j=0; j < num_my_slave_row_nodes; ++j )
    {
      ...
      /* Set the status to active of the node during the predictor of the
       * very first step, if the node has been set active by INITCONTACTBYGAP,
       * via the condition line or any other prescribed option. */
      if ( not cnode->Active() and
           interface.SetNodeInitiallyActive( cparams, *cnode ) )
      {
        lstatus = ( cnode->Active() ? ASStatus::changed : lstatus );
        continue;
      }
  ...
  enum ASStatus gstatus = ASStatus::unchanged;
  {
    int local = static_cast<int>(lstatus);
    int global = static_cast<int>(gstatus);
    Comm().MaxAll( &local, &global, 1 );
    gstatus = ( global>0 ? ASStatus::changed : ASStatus::unchanged );
  }
  ...
  if ( cparams.IsDefaultStep() and
       ( gstatus == ASStatus::changed ) == Data().IsActiveSetConverged() )
    dserror("The convergence state of the active set was not detected correctly!");
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::EvalStaticConstraintRHS(
    CONTACT::ParamsInterface& cparams )
{
  SetCurrentEvalState( cparams );
    ...
    CONTACT::ParamsInterface& cparams )
{
  ...
  if (Data().PrintLinearMomConservation())
  {
    ...
    // slave
    for (int i=0;i<augfs_lm->MyLength();++i)
      lssum[i%probdim] += (*augfs_lm)[i];

    Comm().SumAll(lssum,gssum,probdim);
    // master
    for (int i=0;i<augfm_lm->MyLength();++i)
      lmsum[i%probdim] += (*augfm_lm)[i];

    Comm().SumAll(lmsum,gmsum,probdim);
    // complete balance check
    for ( int i=0; i<probdim; ++i )
      gcsum[i] = gssum[i]+gmsum[i];
    ...
    for ( int i=0; i<probdim; ++i )
    {
      if ( std::abs(gcsum[i])>1.0e-11  *
          std::max( 1.0, std::max( std::abs(gssum[i]), std::abs(gmsum[i]) ) ) )
        dserror("Conservation of linear momentum is not fulfilled!");
    }

    // slave
    std::fill(lssum, lssum+probdim, 0.0);
    for (int i=0;i<augfs_g->MyLength();++i)
      lssum[i%probdim] += (*augfs_g)[i];

    Comm().SumAll(lssum,gssum,probdim);

    // master
    std::fill(lmsum, lmsum+probdim, 0.0);
    for (int i=0;i<augfm_g->MyLength();++i)
      lmsum[i%probdim] += (*augfm_g)[i];
    ...
    // slave
    std::fill(lssum, lssum+probdim, 0.0);
    for (int i=0;i<augfs.MyLength();++i)
      lssum[i%probdim] += augfs[i];

    Comm().SumAll(lssum,gssum,probdim);

    // master
    std::fill(lmsum, lmsum+probdim, 0.0);
    for (int i=0;i<augfm.MyLength();++i)
      lmsum[i%probdim] += augfm[i];
  ...
  if (Data().PrintAngularMomConservation())
  {
          ...
          cit != interface_.end(); ++cit, ++icount )
    {
      ...
      CONTACT::UTILS::WriteConservationDataToFile( Comm().MyPID(), icount,
          nln_iter, conservation_data, cparams.GetOutputFilePath(),
          "lm_terms" );
...
void CONTACT::AUG::Strategy::EvalAugmentedForces()
{
  ...
  for ( auto& inter : interface_ )
  {
    ...
    aug_inter.Assemble_SlForceLmInactive( Data().SlForceLmInactive(),
        Data().Cn(), InactiveScaleFactor() );
  }
    ...
    const CONTACT::ParamsInterface* cparams) const
{
  ...
  switch (bt)
  {
    ...
    case DRT::UTILS::block_displ_lm:
    {
      ...
      MORTAR::ReplaceColumnAndDomainMap( *kdz_ptr, *LMDoFRowMapPtr(true),
          &newcolmap );
    ...
    case DRT::UTILS::block_lm_displ:
    {
      ...
      if ( err )
        dserror( "ReplaceMap failed on kzd_ptr! (err = %d)", err );
...
/*---------------------------------------------------------------------------*
 *---------------------------------------------------------------------------*/
const Epetra_Vector& CONTACT::AUG::Strategy::GetWeightedGap(
    const enum MapType type ) const
{
  switch ( type )
  {
...
CONTACT::AUG::Strategy::GetWeightedGapGradient(
    const enum WGapGradientType grad_type,
        ...
        case MapType::active_slave_nodes:
        {
          Teuchos::RCP<LINALG::SparseMatrix> wgap_grad =
              Teuchos::rcp( new  LINALG::SparseMatrix(
                  *Data().GActiveNDofRowMapPtr(), 100 ) );

          if ( Data().DMatrixPtr().is_null() or
               Data().MMatrixPtr().is_null() )
          ...
          wgap_grad->Complete( *Data().GSlMaDofRowMapPtr(),
    ...
    case WGapGradientType::constraint_enforcement:
    {
      switch ( map_type )
      {
        case MapType::all_slave_nodes:
        {
          ...
          {
            return GetWeightedGapGradient( WGapGradientType::force_balance,
                MapType::all_slave_nodes );
          }

          Teuchos::RCP<LINALG::SparseMatrix> wgap_grad =
              Teuchos::rcp( new LINALG::SparseMatrix(
                  *Data().GSlNormalDofRowMapPtr(),100,false,false,
                  LINALG::SparseMatrix::FE_MATRIX) );
                ...
                cit != interface_.end(); ++cit )
          {
            ...
            interface.AssembleDLmNWGapLinMatrix( *wgap_grad,
                map_type );
          }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::EvalWeightedGapGradientError(
    CONTACT::ParamsInterface& cparams )
{
 ...
 Teuchos::RCP<CONTACT::ParamsInterface> cparams_ptr =
  ...
  // --- Nodal gradient error due to the convective parametric master coordinate
  {
    ...
    for ( auto& ge_per_node : grad_error_ma_per_node )
      IO::cout(IO::standard) << "GID #" << ge_per_node.first << ", e = "
          << ge_per_node.second << IO::endl;
  }
  ...
  {
    std::unordered_map<int,Deriv1stMap>* error_map_vec[2] =
        { &grad_error_ma, &grad_error_jac };
...
double CONTACT::AUG::MyTotalSquareError(
    const std::unordered_map<int,Deriv1stMap>* const * error_map_vec,
    const unsigned num_vecs )
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::L2ErrorNormPerNode(
    const std::unordered_map<int,Deriv1stMap>& error_map,
...
//        nodal_error_norm.second << std::endl;
  ...
  if ( count != error_map.size() )
    dserror("Size <--> count mismatch!");
}
    ...
    const enum NOX::NLN::MeritFunction::MeritFctName mrt_type ) const
{
  ...
  switch ( mrt_type )
  {
    ...
    {
      return Data().Potential().Get( POTENTIAL::Type::lagrangian,
          POTENTIAL::SetType::all );
    }
    case NOX::NLN::MeritFunction::mrtfct_lagrangian_active:
    {
      return Data().Potential().Get( POTENTIAL::Type::lagrangian,
          POTENTIAL::SetType::active );
    }
    case NOX::NLN::MeritFunction::mrtfct_infeasibility_two_norm:
    {
      return Data().Potential().Get( POTENTIAL::Type::infeasibility_measure,
          POTENTIAL::SetType::all );
    }
    ...
    default:
    {
      dserror( "The specified merit function type is not yet supported. "
          "type = %s | %d", NOX::NLN::MeritFunction::MeritFuncName2String(
              mrt_type ).c_str(), mrt_type );
      exit( EXIT_FAILURE );
    }
  }
}
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
double CONTACT::AUG::Strategy::GetLinearizedPotentialValueTerms(
    const Epetra_Vector& dir,
    ...
    const enum NOX::NLN::MeritFunction::LinType lintype ) const
{
  switch ( mrt_type )
  {
    case NOX::NLN::MeritFunction::mrtfct_lagrangian:
    {
      return GetLinearizedPotentialModelTerms( dir,
          POTENTIAL::Type::lagrangian, POTENTIAL::SetType::all, linorder, lintype );
    }
    case NOX::NLN::MeritFunction::mrtfct_lagrangian_active:
    {
      return GetLinearizedPotentialModelTerms( dir,
          POTENTIAL::Type::lagrangian, POTENTIAL::SetType::active, linorder, lintype );
    }
    case NOX::NLN::MeritFunction::mrtfct_infeasibility_two_norm:
    {
      return GetLinearizedPotentialModelTerms( dir,
          POTENTIAL::Type::infeasibility_measure, POTENTIAL::SetType::all, linorder,
          lintype );
    }
    case NOX::NLN::MeritFunction::mrtfct_infeasibility_two_norm_active:
    {
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
double CONTACT::AUG::Strategy::GetLinearizedPotentialModelTerms(
    const Epetra_Vector& dir,
    const enum POTENTIAL::Type pottype,
    const enum POTENTIAL::SetType potset,
    const enum NOX::NLN::MeritFunction::LinOrder linorder,
    const enum NOX::NLN::MeritFunction::LinType lintype ) const
{
  double linval = 0.0;
...
void CONTACT::AUG::Strategy::GetLinearizedPotentialModelTerms_1stOrder(
    const enum POTENTIAL::Type pottype,
    const enum POTENTIAL::SetType potset,
    const enum NOX::NLN::MeritFunction::LinType lintype,
    double& linval ) const
{
  ...
  switch ( lintype )
  {
    ...
    {
      GetLinearizedPotentialModelTerms_1stOrder( pottype, potset,
          NOX::NLN::MeritFunction::lin_wrt_primary_dofs,
          linval );
      GetLinearizedPotentialModelTerms_1stOrder( pottype, potset,
          NOX::NLN::MeritFunction::lin_wrt_lagrange_multiplier_dofs,
          linval );
    ...
    double& linval ) const
{
  ...
  switch ( lintype )
  {
    ...
    case NOX::NLN::MeritFunction::lin_wrt_all_dofs:
    {
      GetLinearizedPotentialModelTerms_2ndOrder( pottype, potset,
          NOX::NLN::MeritFunction::lin_wrt_primary_dofs,
          linval );
      GetLinearizedPotentialModelTerms_2ndOrder( pottype, potset,
          NOX::NLN::MeritFunction::lin_wrt_lagrange_multiplier_dofs,
          linval );
      GetLinearizedPotentialModelTerms_2ndOrder( pottype, potset,
          NOX::NLN::MeritFunction::lin_wrt_mixed_dofs,
          linval );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::SplitStateVector(
    const Epetra_Vector& full_state,
    ...
    Teuchos::RCP<Epetra_Vector>& z_state_inactive_ptr ) const
{
  ...
  if ( z_state_inactive_ptr.is_null() )
  {
    Teuchos::RCP<Epetra_Map> ginactivendofs = LINALG::SplitMap(
        *Data().GSlNormalDofRowMapPtr(), *Data().GActiveNDofRowMapPtr() );
    z_state_inactive_ptr = Teuchos::rcp( new Epetra_Vector( *ginactivendofs ) );
  }

  SplitStateVector( full_state, *displ_state_slma_ptr, *z_state_active_ptr,
      *z_state_inactive_ptr );
}
...
void CONTACT::AUG::Strategy::SplitStateVector(
    const Epetra_Vector& full_state,
    Epetra_Vector& displ_state_slma,
    Epetra_Vector& z_state_active,
...
CONTACT::AUG::Strategy::EnforceNodalComplementarityCondition(
    std::vector<double>& interface_cn,
    const Epetra_Vector& dincr_slma ) const
{
  ...
  for ( const Teuchos::RCP<CONTACT::CoInterface>& iptr : interface_ )
  {
    ...
    interface_cn.push_back( interface.EnforceNodalComplementarityCondition(
        dincr_slma, *Data().CnPtr(), *nodal_cn_new ) );
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Strategy::SetCurrentEvalState(
    const CONTACT::ParamsInterface& cparams )
{
