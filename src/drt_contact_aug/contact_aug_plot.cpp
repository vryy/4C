...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::Direction::ReadInput(
    const Teuchos::ParameterList& pp )
{
  type_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotDirection>(
      pp, "DIRECTION" );
    ...
    const std::string& dir_file ) const
{
  ...
  Teuchos::RCP<Epetra_Vector> direction = Teuchos::rcp( new
      Epetra_Vector( *full_map, true ) );


  if ( dir_file == "none" )
    dserror("No direction file name has been provided! Read input = \"%s\"",
        dir_file.c_str() );

  if ( plot_.strat_->Comm().NumProc() != 1 )
    dserror( "A external direction vector can currently only be considered in "
  ...
  if ( count != static_cast<unsigned>(direction->Map().NumMyElements()) )
    dserror( "Size mismatch! Did you specify the correct DIRECTION_FILE? It seems"
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::Direction::SplitIntoSlaveMasterBody(
    const Epetra_Vector& dir,
    Teuchos::RCP<Epetra_Vector>& x_dir_ptr,
    Teuchos::RCP<Epetra_Vector>& y_dir_ptr ) const
{
  if ( plot_.strat_->ParRedist() )
    dserror("Parallel redistribution is not supported!" );
  ...
  if ( slnodes.NumMyElements() > 0 )
  {
    ...
    Teuchos::RCP<Epetra_Map> slbody_dofs =
        FindConnectedDofs( snode, *plot_.discret_ );
  ...
  if ( manodes.NumMyElements() > 0 )
  {
    ...
    Teuchos::RCP<Epetra_Map> mabody_dofs =
        FindConnectedDofs( mnode, *plot_.discret_ );
  ...
  if ( x_dir_ptr->Map().NumGlobalElements() + y_dir_ptr->Map().NumGlobalElements()
    ...
    dserror( "Split into slave and master dofs failed! This function supports "
    ...
    const DRT::DiscretizationInterface& discret ) const
{
  ...
  for ( ; ; )
  {
    ...
    for ( int e=0; e<num_adj_eles; ++e )
    {
      ...
      for ( int n=0; n<ele->NumNode(); ++n )
      {
        ...
        if ( ele_node->Owner() != mypid )
          continue;

        const auto ncheck = connected_node_gids.insert( ele_node->Id() );
        if ( ncheck.second )
          connected_nodes.push_back( ele_node );
      }
    }
  ...
  Teuchos::RCP<Epetra_Map> connected_dof_map =
      Teuchos::rcp( new Epetra_Map( -1, dof_vec.size(), dof_vec.data(), 0,
          discret.Comm() ) );
    ...
    const CONTACT::CoAbstractStrategy* strat )
{
  ...
  Teuchos::RCP<NOX::Abstract::PrePostOperator> prepost_solver_ptr =
      Teuchos::rcp( new NOX::NLN::Solver::PrePostOp::CONTACT::Plot(
          contact_plot ) );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
bool CONTACT::AUG::Plot::Activated(
    const Teuchos::ParameterList& plot_params )
{
    ...
    const CONTACT::CoAbstractStrategy* strat )
{
  ...
  if ( res_x < 0 )
    dserror( "The resolution in x-direction must be positive!" );
  opt_.resolution_x_ = static_cast<unsigned>( res_x );

  const int res_y = plot_params.get<int>( "RESOLUTION_Y" );
  if ( res_y < 0 )
    dserror( "The resolution in y-direction must be positive!" );
  ...
  file_open_mode_ = Teuchos::getIntegralValue<std::ios_base::openmode>( plot_params,
      "FILE_OPEN_MODE" );

  mode_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotMode>( plot_params,
      "MODE" );

  type_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotType>( plot_params,
      "TYPE" );

  x_type_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotSupportType>( plot_params,
      "X_TYPE" );
  y_type_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotSupportType>( plot_params,
      "Y_TYPE" );

  func_type_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotFuncName>(
      plot_params, "FUNC_NAME" );

  reference_type_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotReferenceType>(
      plot_params, "REFERENCE_TYPE" );

  ReadRefPoints( plot_params );

  format_ = Teuchos::getIntegralValue<INPAR::CONTACT::PlotFileFormat>(
      plot_params, "FILE_FORMAT" );
  ...
  if ( not curr_step_np_ )
    dserror("The step pointer is NULL!");
...
{
  if ( type_ == INPAR::CONTACT::PlotType::scalar )
    opt_.resolution_x_ = 1;

  if ( type_ == INPAR::CONTACT::PlotType::line or
      type_ == INPAR::CONTACT::PlotType::scalar )
    ...
    double* coords ) const
{
  std::istringstream input_stream(
      Teuchos::getNumericStringParameter(plot_params,param_name));
  std::string word;
  char* input;
  unsigned count = 0;
...
void CONTACT::AUG::Plot::AddFileNameToPath()
{
  ...
  switch ( mode_ )
  {
    case INPAR::CONTACT::PlotMode::write_single_iteration_of_step:
    {
      filepath_ += "_step_" + std::to_string( do_plot_.step_ ) +
          "_iter_" + std::to_string( do_plot_.iter_ );
      break;
    }
    case INPAR::CONTACT::PlotMode::write_last_iteration_of_step:
    {
...
CONTACT::AUG::Plot::ConvertPlotFuncName2MeritFuncName(
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
enum CONTACT::AUG::WGapGradientType
CONTACT::AUG::Plot::ConvertPlotFuncName2WGapGradientType(
    const enum INPAR::CONTACT::PlotFuncName pfunc_name ) const
{
  switch ( pfunc_name )
  {
...
{
  if ( not strat_ )
    dserror( "No augmented strategy has been provided!" );
...
void CONTACT::AUG::Plot::LinSpace(
    ...
    const unsigned n, std::vector<double>& res ) const
{
  res.clear();
...
void CONTACT::AUG::Plot::Do( const NOX::Solver::Generic& solver )
{
  switch ( mode_ )
  {
    case INPAR::CONTACT::PlotMode::write_single_iteration_of_step:
    {
      if ( *curr_step_np_ == do_plot_.step_ and
           solver.getNumIterations() == do_plot_.iter_ )
      {
        Execute( solver );
        return;
      }
...
void CONTACT::AUG::Plot::GetSupportPoints(
    enum INPAR::CONTACT::PlotSupportType stype,
    LINALG::SerialDenseMatrix& support_mat )
{
  switch ( stype )
  {
    ...
    case INPAR::CONTACT::PlotSupportType::position_angle:
    {
      ...
      unsigned i = 0;
      for ( auto an_cit=angle_node_id_map_.begin();
            an_cit!=angle_node_id_map_.end();
            ++an_cit, ++i )
      {
        for ( unsigned j=0; j<static_cast<unsigned>(X_.N()); ++j )
          X_(i,j) = an_cit->first;
      }
...
void CONTACT::AUG::Plot::ComputeAnglePosition()
{
  ...
  for ( unsigned i=0; i<num_my_nodes; ++i)
  {
    ...
    if ( not node )
      dserror("Couldn't find the node with GID %d!", gid );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::PlotScalar(
    const NOX::NLN::CONSTRAINT::Group& ref_grp,
    const Epetra_Vector& dir,
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::PlotLine(
    const NOX::NLN::CONSTRAINT::Group& ref_grp,
    const Epetra_Vector& dir,
    NOX::NLN::CONSTRAINT::Group& plot_grp )
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::PlotSurface(
    const NOX::NLN::CONSTRAINT::Group& ref_grp,
    const Epetra_Vector& dir,
    NOX::NLN::CONSTRAINT::Group& plot_grp )
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::PlotVectorField2D(
    const NOX::NLN::CONSTRAINT::Group& ref_grp,
    const Epetra_Vector& dir,
    NOX::NLN::CONSTRAINT::Group& plot_grp )
{
  if ( x_type_ != INPAR::CONTACT::PlotSupportType::step_length or
    ...
    dserror( "PlotVectorField2D supports currently only the step_length"
        " PlotSupportType!" );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::ModifyStepLength(
    const INPAR::CONTACT::PlotSupportType stype,
    const double alpha,
    const Epetra_Vector& full_x_dir,
    Epetra_Vector& mod_step ) const
{
  switch ( stype )
  {
    case INPAR::CONTACT::PlotSupportType::step_length:
...
void CONTACT::AUG::Plot::WriteLineDataToFile() const
{
  ...
  switch ( format_ )
  {
    ...
    {
      if ( file_open_mode_ != (std::ios_base::out|std::ios_base::app) or
           nlines < 1 )
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
const NOX::NLN::CONSTRAINT::Group*
CONTACT::AUG::Plot::GetReferenceGroup( const NOX::Solver::Generic& solver ) const
{
  ...
  switch ( reference_type_ )
  {
    ...
    {
      ref_grp = dynamic_cast<const NOX::NLN::CONSTRAINT::Group*>(
  ...
  if ( not ref_grp )
    dserror( "A NOX::NLN::CONSTRAINT::Group object is expected!" );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::Direction::SplitIntoSurfaceDirections(
    const Epetra_Vector& dir,
    Teuchos::RCP<Epetra_Vector>& x_dir_ptr,
    Teuchos::RCP<Epetra_Vector>& y_dir_ptr ) const
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
double CONTACT::AUG::Plot::GetValue(
    const enum INPAR::CONTACT::PlotFuncName functype,
    NOX::NLN::CONSTRAINT::Group& plot_grp,
    const double * curr_xy,
    const Epetra_Vector* dir
    ) const
{
  ...
  else
  {
    switch ( functype )
    {
      ...
      {
        const Epetra_Vector& wgap = strat_->GetWeightedGap(
            CONTACT::AUG::MapType::all_slave_nodes );
        const int dof_gid = MapSlNodeGID2NDofGID( wgap_node_gid_ );

        const int dof_lid = wgap.Map().LID( dof_gid );
        if ( dof_lid == -1 )
          dserror( "Couldn't find the DoF with GID %d.", dof_gid );
      ...
      {
        if ( not dir )
          dserror("You have to provide a direction vector!");
...
double CONTACT::AUG::Plot::GetNodalErrorAtAngle(
    const double* angle,
    const std::vector<std::pair<int,double> >& nodal_error ) const
{
  if ( not angle )
    dserror("You have to provide the current x/y support value (a.k.a. angle "
        "in this case).");
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Plot::GetVectorValues(
    const enum INPAR::CONTACT::PlotFuncName functype,
    NOX::NLN::CONSTRAINT::Group& plot_grp,
    const std::vector<const Epetra_Vector*>& dirs,
    std::vector<double>& vec_vals ) const
{
  ...
  switch ( functype )
  {
    ...
    default:
    {
      dserror( "The function \"%s\" is not supported for the vector-field "
          "plot.", INPAR::CONTACT::PlotFuncName2String( functype ).c_str() );
      exit( EXIT_FAILURE );
    }
  }
}
...
void CONTACT::AUG::Plot::GetEnergyDirectionGradients(
    const std::vector<const Epetra_Vector*>& dirs,
    std::vector<double>& grad_vals ) const
{
    ...
    std::vector<double>& grad_vals ) const
{
  ...
  if ( rlid==-1 )
    dserror("Node to NDof mapping failed! ( %d --> %d )", dof_gid, rlid );

  for ( unsigned i=0; i<num_vecs; ++i )
    grad_vals[i] = wgap_dir_grads[i][rlid];
}
...
NOX::NLN::Solver::PrePostOp::CONTACT::Plot::Plot(
    const Teuchos::RCP<::CONTACT::AUG::Plot>& plot_ptr )
    : plot_ptr_( plot_ptr ),
      plot_( *plot_ptr_ )
{ /* empty */ }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void NOX::NLN::Solver::PrePostOp::CONTACT::Plot::runPreIterate(
    const NOX::Solver::Generic& solver )
{
