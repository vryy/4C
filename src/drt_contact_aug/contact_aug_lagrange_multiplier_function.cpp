...
void CONTACT::AUG::LagrangeMultiplierFunction::Init(
    ...
    CONTACT::AUG::DataContainer& data )
{
  issetup_ = false;

  strategy_ = strategy;
  interfaces_.reserve( strategy->ContactInterfaces().size() );
  std::copy( strategy->ContactInterfaces().begin(),
      strategy->ContactInterfaces().end(), std::back_inserter(interfaces_) );
...
void CONTACT::AUG::LagrangeMultiplierFunction::Setup()
{
  ...
  lin_solver_ = CreateLinearSolver( p_lm_func.get<int>( "LINEAR_SOLVER" ),
      strategy_->Comm(), lin_solver_type_ );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Teuchos::RCP<LINALG::Solver>
CONTACT::AUG::LagrangeMultiplierFunction::CreateLinearSolver(
    const int lin_sol_id,
    const Epetra_Comm& comm,
    enum INPAR::SOLVER::SolverType& solver_type ) const
{
  if ( lin_sol_id == -1 )
    dserror( "You must specify a meaningful LINEAR_SOLVER!" );

  // get solver parameter list of linear solver
  const Teuchos::ParameterList& solverparams =
      DRT::Problem::Instance()->SolverParams( lin_sol_id );
  solver_type =
  ...
  if( solver_type != INPAR::SOLVER::umfpack and
      solver_type != INPAR::SOLVER::superlu )
...
void CONTACT::AUG::LagrangeMultiplierFunction::LinSolve(
    ...
    Epetra_MultiVector& sol )
{
  if ( rhs.NumVectors() > 1 or sol.NumVectors() > 1 )
    dserror( "MultiVector support is not yet implemented!" );

  int err = lin_solver_->Solve( mat.EpetraOperator(), Teuchos::rcpFromRef( sol ),
      Teuchos::rcpFromRef ( rhs ), true, true );

  if ( err )
    dserror( "LinSolve failed with err = %d", err );
}
    ...
    const CONTACT::ParamsInterface& cparams )
{
  ...
  Teuchos::RCP<Epetra_Vector> lmn_vec =
      Teuchos::rcp( new Epetra_Vector( data_->GActiveNDofRowMap() ) );
...
CONTACT::AUG::LagrangeMultiplierFunction::GetStructureGradient(
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector>
CONTACT::AUG::LagrangeMultiplierFunction::FirstOrderDirection(
    const CONTACT::ParamsInterface& cparams,
    const Epetra_Vector& dincr )
{
  ...
  LINALG::SparseMatrix full_stiff( *cmodel.GetJacobianBlock( DRT::UTILS::block_displ_displ ),
      LINALG::Copy );
  ...
  // --- first summand
  Teuchos::RCP<Epetra_Vector> tmp_vec = LINALG::CreateVector(
      full_stiff.RangeMap(), true );

  int err = full_stiff.Multiply( false, dincr, *tmp_vec );
  if ( err )
    dserror( "Multiply failed with err = %d", err );

  Teuchos::RCP<Epetra_Vector> tmp_vec_exp =
      LINALG::CreateVector( *data_->GSlMaDofRowMapPtr(), true );
  ...
  Teuchos::RCP<Epetra_Vector> lmincr = LINALG::CreateVector(
      data_->GActiveNDofRowMap(), true );
...
{
  if ( not lmincr.Map().SameAs( lm.Map() ) )
    dserror( "The maps must be identical!" );

  for ( plain_interface_set::const_iterator cit = interfaces_.begin();
        cit != interfaces_.end(); ++cit )
  {
    const CONTACT::AUG::Interface& interface =
        dynamic_cast<const CONTACT::AUG::Interface&>( **cit );
...
void CONTACT::AUG::LagrangeMultiplierFunction::AssembleGradientBMatrixContribution(
    ...
    Epetra_Vector& lmincr ) const
{
  if ( not dincr.Map().SameAs( str_grad.Map() ) )
    dserror( "The maps must be identical!" );
