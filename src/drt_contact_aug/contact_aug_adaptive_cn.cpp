...
void CONTACT::AUG::AdaptiveCn::Execute(
    const AdaptiveCnType adaptive_cn_type,
...
void CONTACT::AUG::AdaptiveCn::InitNbc::operator()(
    ...
    std::ostream* os )
{
  if ( not cparams.IsPredictorState() )
    return;
  ...
  SplitStructureIntoDistinctNeumannBodies(
      neumann_bodies, *fextincr, cmodel.StrDiscret() );
  ...
  for ( Teuchos::RCP<Body>& body : neumann_bodies )
  {
    ...
    body->computeResultants( cmodel.StrDiscret(), parent_.interfaces_,
        *fextincr, *unit_gap_force );

    const double cn_tmp = body->leastSquaresForceBalance(
        unit_gap_force_scale );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::AdaptiveCn::InitNbc::PrintUpdate(
...
Teuchos::RCP<Epetra_Vector> CONTACT::AUG::AdaptiveCn::InitNbc::UnitGapForce() const
{
  ...
  for ( auto& interface_ptr : parent_.interfaces_ )
  {
    const CONTACT::AUG::Interface& interface =
        dynamic_cast<const Interface&>( *interface_ptr );

    interface.AssembleActiveUnitGap( unit_interface_gap );
  }

  Teuchos::RCP<Epetra_Vector> unit_gap_force =
      Teuchos::rcp( new Epetra_Vector( bmatrix.DomainMap() ) );
  CATCH_EPETRA_ERROR(bmatrix.Multiply( true, unit_interface_gap,
      *unit_gap_force ));
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
double CONTACT::AUG::Body::leastSquaresForceBalance(
    const double scale ) const
{
  ...
  if ( cc == 0.0 )
    dserror("The inner product of the contact forces is equal to zero!");
    ...
    const plain_interface_set& interfaces )
{
  ...
  {
    if ( interfaces[ilid].is_null() )
      dserror("There is no interface with LID %d", ilid);
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Body::computeResultants(
    const DRT::DiscretizationInterface& str_discret,
    const plain_interface_set& interfaces,
    const Epetra_Vector& fext,
    const Epetra_Vector& unit_gap_force )
{
  ...
  // compute contact forces
  {
    ...
    for ( int ilid : interface_lids_ )
      discrets.push_back( &interfaces[ilid]->Discret() );
    ...
    const DRT::DiscretizationInterface& full_discret ) const
{
  ...
  for ( unsigned b=0; b<num_bodies; ++b )
  {
    ...
    if ( fext_magnitude < std::numeric_limits<double>::epsilon() )
      continue;
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void CONTACT::AUG::Body::resultantVector(
    const Epetra_Vector& vec,
    const Epetra_Map& node_map,
    const discret_set& discrets,
    LINALG::SerialDenseVector& resultant ) const
{
  ...
  for ( int nlid = 0; nlid < num_nodes; ++nlid )
  {
    ...
    for ( int d=0; d<num_dofs_per_node_; ++d )
    {
      ...
      if ( dof_lid == -1 )
        dserror( "Couldn't find dof GID #%d.", dof_gid );
    ...
    int& ilid ) const
{
  ...
  {
    if ( discret->NodeRowMap()->MyGID( ngid ) )
      return discret->gNode( ngid );
    ++ilid;
  }
...
int CONTACT::AUG::Body::numDofsPerNode(
    ...
    int dofset_id,
    const int* num_dofs_per_node) const
{
  ...
  for ( int nlid = 1; nlid < num_rnodes; ++nlid )
  {
    ...
    if ( num_dofs != discret.NumDof( dofset_id, node ) )
      dserror( "Different number of dofs in dof-set #%d detected for node "
          "GID #%d [ndofs = %d] and node GID #%d [ndofs = %d].", dofset_id,
          first_node->Id(), first_num_dofs, node->Id(), num_dofs );
  }
  ...
  for ( int p = 0; p<comm.NumProc(); ++p )
  {
    if ( all_num_dofs[p] >= 0 and first_num_dofs != all_num_dofs[p] )
      dserror( "Different number of dofs on proc #d and proc #d.",
          comm.MyPID(), p );

    if ( first_num_dofs < 0 and all_num_dofs[p] > 0 )
      first_num_dofs = all_num_dofs[p];
  }
...
{
  return dynamic_cast<const STR::MODELEVALUATOR::Contact&>(
...
Teuchos::RCP<Epetra_Map> CONTACT::AUG::Body::extractContactInterfaceMap(
    const Epetra_Map& body_map,
    const Epetra_Map& map,
    const Epetra_Map* pmap ) const
{
  ...
  Teuchos::RCP<Epetra_Map> overlapping_map =
      LINALG::ExtractMyOverlappingSubMap( *map_ptr, body_map );

  if ( overlapping_map->NumGlobalElements() > 0 )
  {
    if ( pmap )
      overlapping_map = DRT::UTILS::RedistributeInAccordanceWithReference(
          map, *overlapping_map );
    ...
    const Epetra_Comm& comm )
{
  ...
  for ( const Teuchos::RCP<const CoInterface>& iptr : interfaces )
  {
    ...
    // --- slave maps ---------------------------------------------------------
    {
      Teuchos::RCP<Epetra_Map> overlapping_dofmap =
          extractContactInterfaceMap( *dof_map_, *interface.SlaveRowDofs(),
              interface.PSlaveRowDofs().get() );
      if ( overlapping_dofmap->NumGlobalElements() > 0 )
        contact_idofs_ = LINALG::MergeMap( *overlapping_dofmap, *contact_idofs_ );

      Teuchos::RCP<Epetra_Map> overlapping_nodemap =
          extractContactInterfaceMap( *node_map_, *interface.SlaveRowNodes(),
              interface.PSlaveRowNodes().get() );
    ...
    {
      Teuchos::RCP<Epetra_Map> overlapping_dofmap =
          extractContactInterfaceMap( *dof_map_, *interface.MasterRowDofs(),
              interface.PMasterRowDofs().get() );
      if ( overlapping_dofmap->NumGlobalElements() > 0 )
        contact_idofs_ = LINALG::MergeMap( *overlapping_dofmap, *contact_idofs_ );

      Teuchos::RCP<Epetra_Map> overlapping_nodemap =
          extractContactInterfaceMap( *node_map_, *interface.MasterRowNodes(),
              interface.PMasterRowNodes().get() );
      if ( overlapping_nodemap->NumGlobalElements() > 0 )
      {
        contact_inodes_ = LINALG::MergeMap( *overlapping_nodemap, *contact_inodes_ );
        interface_lids_.insert( i_lid );
