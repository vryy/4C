...
void CONTACT::AUG::DebugIncompleteIntPolicy<probdim,slavetype,mastertype>::Get_Deriv1st_Debug(
    ...
    const Deriv1stMap& djac,
    const Deriv1stVecMap& dmxigp,
    const Deriv1stVecMap& d_gpn,
    const Deriv1stMap& d_gap_sl,
    const double gapn_sl,
    const double wgt,
...
void CONTACT::AUG::DebugIncompleteIntPolicy<probdim,slavetype,mastertype>::Get_Deriv2nd_Debug(
    ...
    const LINALG::Matrix<3,2>& stau,
    const Deriv1stMap& djac,
    const Deriv1stMap& dgap_sl,
    const Deriv2ndMap& ddjac,
    const Deriv2ndVecMap& ddmxigp,
...
//  Debug_Deriv1st_Jacobian( sele, lmval, sderiv, stau );
  ...
  Debug_Deriv1st_Gap_Sl( sele, lmval, sval, d_gpn, d_gap_sl, djac, gapn_sl,
      wgt, jac, d_gapn_sl_complete );

  Debug_Deriv1st_WGap_Sl( sele, lmval, sval, d_gpn, d_gapn_sl_complete, djac, gapn_sl,
      wgt, jac );
...
#ifdef DD_WGAP_SL
  Deriv1stMap d_gapn_sl_complete;
  Debug_Deriv1st_Gap_Sl( sele, lmval, sval, d_n_unit, dgap_sl, djac, gapn_sl,
      wgt, jac, d_gapn_sl_complete );

  Debug_Deriv2nd_WGap_Sl( sele, lmval, sval, d_n_unit, dd_n_unit, djac,
      d_gapn_sl_complete, ddjac, gapn_sl, wgt, jac );
#endif
...
void CONTACT::AUG::DebugCompleteIntPolicy<probdim,slavetype,mastertype>::Debug_Deriv1st_Gap_Sl(
    ...
    const double wgt,
    const double jac,
    Deriv1stMap& d_gapn_sl_complete ) const
{
...
void CONTACT::AUG::DebugCompleteIntPolicy<probdim,slavetype,mastertype>::Debug_Deriv1st_WGap_Sl(
    ...
    const LINALG::Matrix<my::SLAVENUMNODE,1>& sval,
    const Deriv1stVecMap& d_gpn,
    const Deriv1stMap& d_gapn_sl_complete,
    const Deriv1stMap& d_jac,
    const double gapn_sl,
    const double wgt,
    const double jac ) const
  ...
  for ( unsigned i=0; i<my::SLAVENUMNODE; ++i )
  {
    ...
    for ( unsigned k=0; k<my::SLAVENUMNODE; ++k )
    {
      ...
      for ( unsigned d=0; d<probdim; ++d )
      {
        ...
        {
          GEN_DATA::increaseCapacity( dd_debug_var );
          dd_debug_var[ d_n_unit_lin.first ] += tmp * sval(k,0) *
              d_n_unit_lin.second;
        }
      }
    }
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype,
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::DebugCompleteIntPolicy<probdim,slavetype,mastertype>::Debug_Deriv1st_Smooth_Unit_Normal(
    MORTAR::MortarElement& sele ) const
{
  DRT::Node* const * snodes = sele.Nodes();
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype,
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::DebugCompleteIntPolicy<probdim,slavetype,mastertype>::Debug_Deriv2nd_Smooth_Unit_Normal(
    MORTAR::MortarElement& sele ) const
{
  DRT::Node* const * snodes = sele.Nodes();
    ...
    const LINALG::Matrix<3,2>& stau ) const
{
  ...
  this->Deriv1st_NonUnitSlaveElementNormal( sele, nodal_dofs, sderiv,
    ...
    const LINALG::Matrix<3,2>& stau ) const
{
  ...
  this->Get_Deriv2nd_Jacobian( sele, nodal_dofs, sderiv, unit_normal,
  ...
  for ( unsigned i=0; i<my::SLAVENUMNODE; ++i )
  {
    ...
    for ( auto& deriv2nd_jac_var : deriv2nd_jac )
    {
      ...
      {
        GEN_DATA::increaseCapacity( dd_debug_var );
        dd_debug_var[deriv2nd_jac_var_lin.first] +=
            lmval(i,0) * deriv2nd_jac_var_lin.second;
      }
    }
  }
}
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype,
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::DebugCompleteIntPolicy<probdim,slavetype,mastertype>::Debug_Deriv1st_Non_Unit_Normal(
    MORTAR::MortarElement& sele,
    const LINALG::Matrix<my::SLAVENUMNODE,1>& lmval,
    const LINALG::Matrix<my::SLAVEDIM,my::SLAVENUMNODE>& sderiv,
    const LINALG::Matrix<3,2>& stau ) const
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype,
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::DebugCompleteIntPolicy<probdim,slavetype,mastertype>::Debug_Deriv2nd_Non_Unit_Normal(
    MORTAR::MortarElement& sele,
    const LINALG::Matrix<my::SLAVENUMNODE,1>& lmval,
    const LINALG::Matrix<my::SLAVEDIM,my::SLAVENUMNODE>& sderiv ) const
{
  ...
  this->Deriv2nd_NonUnitSlaveElementNormal( sele, nodal_dofs, sderiv,
      dd_non_unit_normal );
...
template class CONTACT::AUG::DebugIncompleteIntPolicy<2,DRT::Element::nurbs2,DRT::Element::nurbs2>;
template class CONTACT::AUG::DebugIncompleteIntPolicy<2,DRT::Element::nurbs2,DRT::Element::line2>;
template class CONTACT::AUG::DebugIncompleteIntPolicy<2,DRT::Element::nurbs2,DRT::Element::nurbs3>;

template class CONTACT::AUG::DebugIncompleteIntPolicy<2,DRT::Element::nurbs3,DRT::Element::nurbs3>;
template class CONTACT::AUG::DebugIncompleteIntPolicy<2,DRT::Element::nurbs3,DRT::Element::line2>;
template class CONTACT::AUG::DebugIncompleteIntPolicy<2,DRT::Element::nurbs3,DRT::Element::nurbs2>;
...
template class CONTACT::AUG::DebugIncompleteIntPolicy<3,DRT::Element::nurbs4,DRT::Element::nurbs4>;
...
template class CONTACT::AUG::DebugIncompleteIntPolicy<3,DRT::Element::nurbs4,DRT::Element::nurbs9>;

template class CONTACT::AUG::DebugIncompleteIntPolicy<3,DRT::Element::nurbs9,DRT::Element::nurbs9>;
...
template class CONTACT::AUG::DebugIncompleteIntPolicy<3,DRT::Element::nurbs9,DRT::Element::nurbs4>;
