...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void NOX::NLN::CONTACT::LinearSystem::SetLinearProblemForSolve(
    ...
    Epetra_Vector& rhs ) const
{
  switch ( jacType_ )
  {
    case NOX::NLN::LinSystem::LinalgSparseMatrix:
      NOX::NLN::LinearSystem::SetLinearProblemForSolve( linear_problem,
          jac, lhs, rhs );
...
{
  if ( p_lhs_.is_null() or p_lhs_.get() == &glhs )
    return;
    ...
    Epetra_Vector& rhs )
{
  LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>& block_mat =
      dynamic_cast<LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>& >(
          mat );
  ...
  std::vector<std::vector<bool> > isempty( numrows,
  ...
  switch ( num_remaining_row_col )
  {
    case 0:
    {
      dserror( "You are trying to solve a pure diagonal matrix. This is currently not"
...
void NOX::NLN::CONTACT::LinearSystem::applyDiagonalInverse(
    ...
    Epetra_Vector& lhs,
    const Epetra_Vector& rhs ) const
{
  if ( mat.EpetraMatrix()->NumGlobalDiagonals() !=
      mat.EpetraMatrix()->NumGlobalNonzeros() )
  ...
  if ( err )
    dserror( "ExtractDiagonalCopy failed! (err=%d)", err );

  err = lhs_block.ReciprocalMultiply( 1.0, diag_mat, rhs_block, 0.0 );
  if ( err )
    dserror( "ReciprocalMultiply failed! (err=%d)", err );
