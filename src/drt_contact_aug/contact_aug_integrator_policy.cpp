...
void CONTACT::AUG::IncompleteIntPolicy<probdim,slavetype,mastertype>::
Get_Deriv2nd_Jacobian(
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype,
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::CompleteIntPolicy<probdim,slavetype,mastertype>::
    Get_Deriv2nd_Jacobian(
    const MORTAR::MortarElement& sele,
    const LINALG::TMatrix<int, probdim, my::SLAVENUMNODE>& nodal_dofs,
    const LINALG::Matrix<my::SLAVEDIM,my::SLAVENUMNODE>& deriv,
    ...
    Deriv2ndMap& dd_jac ) const
{
  ...
  this->Deriv2nd_NonUnitSlaveElementNormal( sele, nodal_dofs, deriv,
      dd_non_unit_normal );
  ...
  this->Deriv1st_UnitSlaveElementNormal( unit_normal,
      length_n_inv, d_non_unit_normal, d_unit_normal );
  ...
  this->Deriv2nd_Jacobian( d_unit_normal, d_non_unit_normal, unit_normal,
      dd_non_unit_normal, dd_jac );
}
    ...
    Deriv1stMap& d_jac ) const
{
  ...
  for ( auto d_jac_pair : d_jac )
    std::cout << "varied GID (" << d_jac_pair.first
        << ") = " << d_jac_pair.second << std::endl;
#endif
}
  ...
  for ( auto& dd_jac_pair_var : dd_jac )
  {
    for ( auto& dd_jac_pair_lin : dd_jac_pair_var.second )
    {
      std::cout << "varied GID (" << dd_jac_pair_var.first << "), "
          << "lin GID (" << dd_jac_pair_lin.first << ") = "
          << dd_jac_pair_lin.second << "\n";
    }
  }
#endif
}
    ...
    const bool reset ) const
{
  ...
  if ( reset )
    GEN_DATA::reset( probdim, SLAVENUMNODE*probdim, d_unit_normal );
  // safety check
  else
  {
    if ( d_unit_normal.size() < probdim )
      dserror( "Given vector has the wrong size." );
  ...
  for ( unsigned n_dof=0; n_dof<probdim; ++n_dof )
    ...
    {
      std::cout << " n-dof (" << n_dof << "), varied GID ("
    ...
    LINALG::Matrix<probdim,1>& unit_normal ) const
{
  ...
  std::copy( non_unit_normal_extended.A(),
      non_unit_normal_extended.A() + probdim, unit_normal.A() );

  double length_n_inv = unit_normal.Norm2();
  if ( length_n_inv == 0.0 )
    dserror("The length of the slave element normal is equal to 0.0!" );
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype>
void CONTACT::AUG::BaseSlaveIntPolicy<probdim,slavetype>::
      ...
      for ( unsigned j=0; j<probdim; ++j )
      {
        ...
        for ( unsigned k=0; k<3; ++k )
        {
          ...
          std::cout << "n-dof (" << n_dof << "), "
              << "varied GID (" << var_jdof << ") | "
              << "e_ijk = " << e_ijk << ", e_ikj = " << e_ikj << " = "
              << d_nn_var_jdof << std::endl;
#endif
        }
      }
    }
        ...
        for ( unsigned n_k=0; n_k<SLAVENUMNODE; ++n_k )
        {
          ...
          for ( unsigned k=0; k<probdim; ++k )
          {
            ...
            std::cout << n_dof << " | "
                ...
                << k << " (" << lin_kdof << ") : "
                << e_ijk << ", " << e_ikj << "; "
                << dd_nn_var_jdof[lin_kdof] << std::endl;
#endif
          }
        }
      }
    ...
    Deriv2ndVecMap& dd_unit_normal) const
{
  ...
  Deriv2ndMap dn_dn( 0 );
  GEN_DATA::reset( d_non_unit_normal[0].capacity(),
      d_unit_normal[0].capacity(), dn_dn );
  ...
  for ( unsigned n_dof=0; n_dof<probdim; ++n_dof )
  {
    ...
    for ( auto& d_un_ndof_var : d_un_ndof )
    {
      ...
      {
        dd_un_ndof_var.repetitive_access( dn_n_lin.first, gp_id_ ) -= length_n_inv *
            dn_n_lin.second * d_un_ndof_var.second;
      }
    }
  }
  ...
  for ( unsigned n_dof=0; n_dof<probdim; ++n_dof )
  {
    ...
    for ( auto& d_nun_ndof_var : d_nun_ndof )
    {
      ...
      for ( auto& d_un_ndof_lin : d_un_ndof )
      {
        dn_dn_var( d_un_ndof_lin.first ) += d_nun_ndof_var.second *
            d_un_ndof_lin.second;
      }
    }
  }
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype>
void CONTACT::AUG::BaseSlaveIntPolicy<probdim,slavetype>::
    InnerProductOfVectorAndDeriv1stVector(
        const LINALG::Matrix<probdim,1>& vec,
        const Deriv1stVecMap& d_vec,
        Deriv1stMap& dvec_vec ) const
{
  // loop over the normal vector components
    ...
    Deriv1stMap& d_alpha ) const
{
  ...
  for ( unsigned i=0; i<probdim; ++i )
  {
    ...
    for ( unsigned j=0; j<my::SLAVENUMNODE; ++j )
    {
      for ( unsigned k=0; k<probdim; ++k )
      {
        deriv_i.repetitive_access( snodal_dofs(k,j), my::gp_id_ ) +=
            lmat_inv(i,k) * sval(j);
      }
    }
    ...
    for ( unsigned j=0; j<MASTERNUMNODE; ++j )
    {
      for ( unsigned k=0; k<probdim; ++k )
      {
        deriv_i.repetitive_access( mnodal_dofs(k,j), my::gp_id_ ) -=
            lmat_inv(i,k) * mval(j);
      }
    }
    ...
    for ( unsigned j=0; j<my::SLAVENUMNODE; ++j )
    {
      ...
      for ( unsigned k=0; k<probdim; ++k )
      {
        ...
        for ( auto& d_n_k : d_n[k] )
          deriv_i.repetitive_access( d_n_k.first, my::gp_id_ ) +=
              tmp * d_n_k.second;
      }
    }
  }
...
void CONTACT::AUG::CompleteIntPolicy<probdim,slavetype,mastertype>::Get_Deriv2nd_MXiGP(
    ...
    const LINALG::Matrix<3,my::MASTERNUMNODE>& mderiv2,
    const LINALG::Matrix<3,2>& mtau,
    const double* mxi,
    const double alpha,
    const Deriv1stVecMap& d_mxigp,
    const Deriv1stMap& d_alpha,
    Deriv2ndVecMap& dd_mxigp ) const
{
  ...
  this->Add_Deriv1st_MaMetric( lmat_inv, mnodal_dofs, mtau, mderiv, mderiv2,
      mcoord, d_mxigp, dd_mxigp );

  // --- contributions from the slave side -------------------------------------
  this->Add_Deriv1st_Alpha_Deriv1st_Normal( lmat_inv, d_alpha, sele, sval,
      dd_mxigp );
...
void CONTACT::AUG::BaseIntPolicy<probdim,slavetype,mastertype>::Add_Deriv2nd_MaDispl(
    ...
    const LINALG::Matrix<MASTERDIM,MASTERNUMNODE>& mderiv,
    const Deriv1stVecMap& d_mxigp,
    Deriv2ndVecMap& dd_mxigp ) const
{
    ...
    for ( unsigned k=0; k<MASTERNUMNODE; ++k )
    {
      for ( unsigned j=0; j<probdim; ++j)
      {
        Deriv1stMap& dd_mxigp_i_var =
            dd_mxigp_i.repetitive_access( mnodal_dofs(j,k), my::gp_id_ );
...
void CONTACT::AUG::BaseIntPolicy<probdim,slavetype,mastertype>::Add_Deriv1st_MaMetric(
    ...
    const LINALG::Matrix<3,MASTERNUMNODE>& mcoord,
    const Deriv1stVecMap& d_mxigp,
    Deriv2ndVecMap& dd_mxigp ) const
{
    ...
    for ( unsigned j=0; j<MASTERDIM; ++j )
    {
      ...
      for ( auto& d_mxi_gp_j_pair : d_mxigp_j )
      {
        ...
        for ( unsigned k=0; k<MASTERNUMNODE; ++k )
        {
          ...
          {
            dd_mxigp_i_var.repetitive_access( mnodal_dofs(d,k), my::gp_id_ ) -=
                lmat_inv( i, d ) * tmp;
          }
        }
...
template < unsigned probdim,
           DRT::Element::DiscretizationType slavetype,
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::BaseIntPolicy<probdim,slavetype,mastertype>::
Add_Deriv1st_Alpha_Deriv1st_Normal(
    const LINALG::Matrix<probdim,probdim>& lmat_inv,
    const Deriv1stMap& d_alpha,
    const MORTAR::MortarElement& sele,
    const LINALG::Matrix<my::SLAVENUMNODE,1>& sval,
    ...
    Deriv1stMap& deriv_gapn_sl ) const
{
  for ( unsigned k=0; k<my::SLAVENUMNODE; ++k )
  {
    ...
    for ( unsigned d=0; d<probdim; ++d )
      deriv_gapn_sl( sdof[d] ) += sval(k,0) * gpn[d];
  }
}
...
void CONTACT::AUG::IncompleteIntPolicy<probdim,slavetype,mastertype>::Get_Deriv1st_WGap(
    ...
    const Deriv1stMap& d_gapn_sl,
    const Deriv1stMap& d_gapn_ma ) const
{
  // get slave element nodes
  DRT::Node** snodes = sele.Nodes();

  this->Add_Deriv1stGapNContributions( snodes, wgt*jac, lmval, d_gapn_sl,
      d_gapn_ma );
    ...
    const Deriv1stMap& d_gapn_ma ) const
{
  ...
  this->Add_Deriv1stJacobianContributions( snodes, wgt, lmval,
      gapn_sl, gapn_ma, d_jac );
}
    ...
    const Deriv1stMap& d_jac ) const
{
  ...
  complete_policy.Get_Deriv1st_GapN( sele, mele, sval, mval, gpn, mtau,
      d_mxigp, deriv_gapn_sl, deriv_gapn_ma );
  ...
  complete_policy.Get_Deriv1st_WGap( sele, lmval, gapn_sl, gapn_ma, wgt, jac,
      d_jac, deriv_gapn_sl, deriv_gapn_ma );
    ...
    const Deriv1stMap& d_jac ) const
{
  for ( unsigned i=0; i<my::SLAVENUMNODE; ++i )
  {
    ...
    for ( auto& d_jac_var : d_jac )
      d_wgap_sl( d_jac_var.first ) += tmp_sl_ma * d_jac_var.second;
  }
...
void CONTACT::AUG::IncompleteIntPolicy<probdim,slavetype,mastertype>::Get_Deriv2nd_WGap(
        ...
        const LINALG::Matrix<my::SLAVENUMNODE,1>& lmval,
        const LINALG::Matrix<my::MASTERDIM,my::MASTERNUMNODE>& mderiv,
        const LINALG::Matrix<3,my::MASTERNUMNODE>& mderiv2nd,
        const LINALG::Matrix<3,2>& mtau,
        const double* gpn,
        const double wgt,
        const double gapn_sl,
        ...
        const double jac,
        const Deriv1stMap& d_jac,
        const Deriv2ndMap& dd_jac,
        const Deriv1stVecMap& d_mxigp,
        const Deriv2ndVecMap& dd_mxigp,
        const Deriv1stVecMap& d_n_unit,
        const Deriv2ndVecMap& dd_n_unit,
        const Deriv1stMap& d_gapn_sl,
        const Deriv1stMap& d_gapn_ma ) const
{

  Add_Jac_Deriv2nd_GapN( sele, mele, sval, mval, lmval, mderiv,
      mtau, gpn, wgt, jac, d_mxigp, d_n_unit, dd_n_unit );

  Add_Deriv1st_GapN_Deriv1st_Jac( sele, lmval, wgt, d_gapn_sl,
      d_gapn_ma, d_jac );
}
    ...
    for ( unsigned k=0; k<my::SLAVENUMNODE; ++k )
    {
      ...
      for ( unsigned d=0; d<probdim; ++d )
      {
        ...
        for ( auto& d_n_unit_lin : d_n_unit[d] )
        {
          ...
          dd_wgap_sl_var( d_n_unit_lin.first ) += tmp * sval(k,0) *
              d_n_unit_lin.second;
        }
      }
    }
    ...
    for ( unsigned k=0; k<my::MASTERNUMNODE; ++k )
    {
      ...
      for ( unsigned d=0; d<probdim; ++d )
      {
        ...
        for ( auto& d_n_unit_lin : d_n_unit[d] )
        {
          ...
          dd_wgap_ma_var( d_n_unit_lin.first ) += tmp * mval(k,0) *
              d_n_unit_lin.second;
        }
    ...
    const Deriv1stMap& d_gapn_ma ) const
{
  ...
  Add_Jac_Deriv2nd_GapN( sele, mele, sval, mval, lmval, mderiv, mderiv2nd,
      mtau, gpn, wgt, jac, d_mxigp, dd_mxigp, d_n_unit, dd_n_unit );

  this->Add_Deriv1st_GapN_Deriv1st_Jac( sele, lmval, wgt, d_gapn_sl,
      d_gapn_ma, d_jac );
    ...
    for ( auto& dd_jac_var : dd_jac )
    {
      ...
      for ( auto& dd_jac_var_lin : dd_jac_var.second )
      {
        ...
        dd_wgap_sl_var.repetitive_access( gid_lin, my::gp_id_ ) +=
            tmp_sl_ma * dd_jac_var_lin.second;
      }
    }
  }
}
...
template < unsigned probdim,
           ...
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::BaseIntPolicy<probdim,slavetype,mastertype>::
Add_Var_GapN_Lin_Jac(
    MORTAR::MortarElement& sele,
    const LINALG::Matrix<my::SLAVENUMNODE,1>& lmval,
    const double wgt,
    const Deriv1stMap& d_gapn_sl,
    const Deriv1stMap& d_gapn_ma,
    const Deriv1stMap& d_jac) const
{
  DRT::Node* const * snodes = sele.Nodes();
  ...
  for ( unsigned i=0; i<my::SLAVENUMNODE; ++i )
  {
    ...
    {
      GEN_DATA::increaseCapacity( dd_wgap_sl );
      Deriv1stMap& dd_wgap_sl_var =
          dd_wgap_sl.repetitive_access( d_gapn_sl_var.first, my::gp_id_ );
      ...
      {
        GEN_DATA::increaseCapacity( dd_wgap_sl_var );
        dd_wgap_sl_var.repetitive_access( d_jac_lin.first, my::gp_id_ ) +=
            d_jac_lin.second * val;
      }
    }
    ...
    for ( auto& d_gapn_ma_var : d_gapn_ma )
    {
      ...
      Deriv1stMap& dd_wgap_ma_var =
          dd_wgap_ma.repetitive_access( d_gapn_ma_var.first, my::gp_id_ );
      ...
      {
        GEN_DATA::increaseCapacity( dd_wgap_ma_var );
        dd_wgap_ma_var.repetitive_access( d_jac_lin.first, my::gp_id_ ) +=
            d_jac_lin.second * val;
      }
    }
  }
    ...
    const Deriv1stMap& d_jac) const
{
  ...
  for ( unsigned i=0; i<my::SLAVENUMNODE; ++i )
  {
    ...
    for ( auto& d_jac_var : d_jac )
    {
      ...
      GEN_DATA::increaseCapacity( dd_wgap_sl );
      Deriv1stMap& dd_wgap_sl_var =
          dd_wgap_sl.repetitive_access( d_jac_var.first, my::gp_id_ );
      ...
      Deriv1stMap& dd_wgap_ma_var =
          dd_wgap_ma.repetitive_access( d_jac_var.first, my::gp_id_ );
...
template < unsigned probdim,
           ...
           DRT::Element::DiscretizationType mastertype >
void CONTACT::AUG::IncompleteIntPolicy<probdim,slavetype,mastertype>::
Add_Deriv1st_GapN_Deriv1st_Jac(
    MORTAR::MortarElement& sele,
    const LINALG::Matrix<my::SLAVENUMNODE,1>& lmval,
    const double wgt,
    const Deriv1stMap& d_gapn_sl,
...
//            continue;
      ...
      for ( unsigned j=0; j<my::MASTERDIM; ++j )
      {
        ...
        {
          GEN_DATA::increaseCapacity( dd_wgap_ma );
          Deriv1stMap& dd_wgap_ma_var = dd_wgap_ma.repetitive_access(
              d_mxigp_j_var.first, my::gp_id_ );
...
//            continue;
    ...
    for ( unsigned d=0; d<probdim; ++d )
    {
      for ( auto& d_n_unit_var : d_n_unit[d] )
      {
        GEN_DATA::increaseCapacity( dd_wgap_ma );
        Deriv1stMap& dd_wgap_ma_var =
            dd_wgap_ma.repetitive_access( d_n_unit_var.first, my::gp_id_ );
    ...
    for ( unsigned d=0; d<probdim; ++d )
    {
      ...
      for ( auto& dd_n_unit_var : dd_n_unit[d] )
      {
        GEN_DATA::increaseCapacity( dd_wgap_sl );
        Deriv1stMap& dd_wgap_sl_var =
            dd_wgap_sl.repetitive_access( dd_n_unit_var.first, my::gp_id_ );
    ...
    std::unordered_map<int,Deriv1stMap>& error_jac ) const
{
  ...
  IO::cout << "LINE " << __LINE__ << " -- " << __FUNCTION__ << ": "
      "There is no error for the complete variational approach." << IO::endl;
  error_ma.clear();
  error_jac.clear();
}
