...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::XFieldField::CouplingDofSet::Dof(std::vector<int>& dofs,
    const DRT::Node* node, unsigned nodal_dofset_id) const
{
    ...
    const int& node_gid) const
{
  std::map<int,int>::const_iterator pos =
      my_num_dof_per_node_.find(node_gid);
  if (pos == my_num_dof_per_node_.end())
    dserror("The given node GID %d is no coupling interface node!",
        node_gid);
