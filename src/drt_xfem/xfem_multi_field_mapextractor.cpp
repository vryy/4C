/*----------------------------------------------------------------------------*/
/**
\file xfem_multi_field_mapextractor.cpp
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::Reset(
    unsigned num_dis,
    bool full )
{
  ...
  slave_map_extractors_.resize(num_dis,
      std::vector<Teuchos::RCP<LINALG::MultiMapExtractor> >(NUM_MAP_TYPES,
          Teuchos::null));
  ...
  element_map_extractor_ = Teuchos::null;
  element_map_extractor_ = Teuchos::rcp<LINALG::MultiMapExtractor>(
      new LINALG::MultiMapExtractor());
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::Init(
    const XDisVec& dis_vec,
    int max_num_reserved_dofs_per_node)
{
  ...
  // sanity check
  if (dis_vec.size()<2)
    dserror("You gave less than 2 discretizations. What are you planning "
    ...
    for (unsigned d=0; d<SlDisVec().size(); ++d)
    {
      if (SlDisVec()[d]->NodeRowMap()->MyGID(*ngid))
      {
        std::pair<std::set<int>::iterator, bool> is_unique =
            my_coupled_sl_dis[*ngid].insert(d);
        if (not is_unique.second)
          dserror("That is impossible, since this row node cannot be part of the "
  ...
  // --------------------------------------------------------------------------
  // --- count set size
  for (cit_map=my_coupled_sl_dis.begin();cit_map!=my_coupled_sl_dis.end();
      ++cit_map)
  {
  ...
  for (int p=0;p<numprocs;++p)
  {
    ...
    if (p > 0)
    {
      ...
      // you didn't
      if (not receivedset.empty() or not receivedgid.empty()
          or not receivedsize.empty())
      ...
      if (length!=2 or tag!=frompid)
        dserror("Size information got mixed up!\n"
      ...
      if (length!=receivedsize[0] or tag!=frompid*10)
        dserror("GID information got mixed up! \n"
            "Received length = %d, Expected length = %d \n"
            "Received tag    = %d, Expected tag    = %d",
      ...
      if (length!=receivedsize[1] or tag!=frompid*100)
        dserror("Set information got mixed up! \n"
            "Received length = %d, Expected length = %d \n"
            "Received tag    = %d, Expected tag    = %d",
  ...
  std::vector<std::vector<int> >my_master_inode_gids(SlDisVec().size(),
      std::vector<int>(0));

  // loop over all (global) coupling nodes
  for (cit_map=g_coupled_sl_dis.begin();cit_map!=g_coupled_sl_dis.end();
      ++cit_map)
  {
    ...
    for (cit_set=cit_map->second.begin();cit_set!=cit_map->second.end();
        ++cit_set)
    {
     ...
     * still deterministic. */
    if (sl_dis_id_to_copy_from==-1)
      sl_dis_id_to_copy_from = *cit_map->second.begin();
  ...
  for (unsigned i=0;i<NumSlDis();++i)
  {
    ...
    interface_matrix_col_transformers_[i] =
        Teuchos::rcp(new FSI::UTILS::MatrixColTransform());
...
void XFEM::MultiFieldMapExtractor::BuildSlaveNodeMapExtractors()
{
  ...
  for (cit_dis=SlDisVec().begin();cit_dis!=SlDisVec().end();++cit_dis)
  {
    ...
    partial_maps[MULTIFIELD::block_interface] = Teuchos::rcp(new Epetra_Map(-1,
        static_cast<int>(my_interface_row_node_gids.size()),
    ...
    partial_maps[MULTIFIELD::block_non_interface] = Teuchos::rcp(new Epetra_Map(-1,
...
void XFEM::MultiFieldMapExtractor::BuildSlaveDofMapExtractors()
{
  ...
  for (cit_dis=SlDisVec().begin();cit_dis!=SlDisVec().end();++cit_dis)
  {
    ...
    partial_maps[MULTIFIELD::block_interface] =
        ...
        static_cast<int>(my_sl_interface_dofs.size()),
        &my_sl_interface_dofs[0], 0, Comm()));

    // create slave non-interface dof row map
    partial_maps[MULTIFIELD::block_non_interface] = Teuchos::null;
    partial_maps[MULTIFIELD::block_non_interface] =
        Teuchos::rcp(new Epetra_Map(-1,
        static_cast<int>(my_sl_non_interface_dofs.size()),
...
void XFEM::MultiFieldMapExtractor::BuildInterfaceCouplingDofSet()
{
  ...
  for (unsigned i=0;i<NumSlDis();++i)
  {
    ...
    DRT::Exporter export_max_dof_num(sl_inodemap,ma_inodemap,
        SlDiscret(i).Comm());
    ...
    if (g_num_std_dof==-1)
      Comm().MaxAll(&my_num_std_dof,&g_num_std_dof,1);

    if (my_num_std_dof!=-1 and g_num_std_dof!=my_num_std_dof)
      dserror("The number of standard DoF's is not equal on all procs"
          "and discretizations!");

    std::map<int,int>::const_iterator cit;
    for (cit=sl_max_num_dof_per_inode.begin();
        cit!=sl_max_num_dof_per_inode.end();
        ++cit)
    {
      std::map<int,int>::iterator ma_pos =
          ma_max_num_dof_per_inode.find(cit->first);
...
void XFEM::MultiFieldMapExtractor::BuildMasterNodeMapExtractor()
{
  ...
  // --------------------------------------------------------------------------
  for (unsigned i=0;i<NumSlDis();++i)
    partial_maps.at(i) = master_interface_node_maps_[i];
...
void XFEM::MultiFieldMapExtractor::BuildInterfaceCouplingAdapters()
{
  ...
  for (it=interface_couplings_.begin();it!=interface_couplings_.end();++it)
  {
    ...
    (*it)->SetupCoupling(IDiscret(),SlDiscret(i),
        MasterInterfaceNodeRowMap(i),SlaveNodeRowMap(i,MULTIFIELD::block_interface),
        MasterInterfaceNodeRowMap(i),-1);
...
void XFEM::MultiFieldMapExtractor::BuildElementMapExtractor()
{
  ...
  Teuchos::RCP<Epetra_Map> fullmap = Teuchos::null;
  std::vector<Teuchos::RCP<const Epetra_Map> > partial_maps(NumSlDis(),
      Teuchos::null);
  unsigned d = 0;
    ...
    enum MapType map_type) const
{
  ...
  const Teuchos::RCP<const Epetra_Map> & sl_full_map =
      SlMapExtractor(dis_id,map_type).FullMap();

  if (sl_full_map.is_null())
    dserror("null full map for field %s",FieldName2String(field).c_str());
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::ExtractVector(
    const Epetra_MultiVector & full,
    int block,
    ...
    enum MapType map_type) const
{
  ...
  SlMapExtractor(block,map_type).InsertVector(*partial_non_interface,
      MULTIFIELD::block_non_interface,partial);
  ...
  SlMapExtractor(block,map_type).InsertVector(*partial_sl_interface,
      MULTIFIELD::block_interface, partial);
...
Teuchos::RCP<Epetra_Vector> XFEM::MultiFieldMapExtractor::InsertVector(
    const Epetra_Vector& partial,
    enum FieldName field,
    enum MapType map_type) const
{
  const int dis_id = SlaveId(field);
  Teuchos::RCP<Epetra_Vector> full =
      Teuchos::rcp(new Epetra_Vector(*FullMap(map_type)));
    ...
    enum MapType map_type) const
{
  ...
  Teuchos::RCP<Epetra_MultiVector> full =
      Teuchos::rcp(new Epetra_MultiVector(*FullMap(map_type),
          partial.NumVectors()));
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::InsertVector(
    const Epetra_MultiVector & partial,
    int block,
    Epetra_MultiVector & full,
    enum MapType map_type ) const
{
  ...
  // --------------------------------------------------------------------------
  Teuchos::RCP<Epetra_MultiVector> partial_non_interface =
      SlMapExtractor(block,map_type).ExtractVector(partial,
          MULTIFIELD::block_non_interface);

  MaMapExtractor(map_type).InsertVector(*partial_non_interface,
      NumSlDis()+block,full);
  ...
  Teuchos::RCP<Epetra_MultiVector> partial_sl_interface =
      SlMapExtractor(block,map_type).ExtractVector(partial,
          MULTIFIELD::block_interface);

  Teuchos::RCP<Epetra_MultiVector> partial_ma_interface =
      ICoupling(block).SlaveToMaster(partial_sl_interface,map_type);

  MaMapExtractor(map_type).InsertVector(*partial_ma_interface,
        block,full);
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::AddVector( const Epetra_MultiVector& partial,
    int block,
    Epetra_MultiVector& full,
    double scale,
    enum MapType map_type ) const
{
  ...
  MaMapExtractor(map_type).AddVector(*partial_non_interface,
      NumSlDis()+block,full,scale);
  ...
  MaMapExtractor(map_type).AddVector(*partial_ma_interface,
        block,full,scale);
}
...
void XFEM::MultiFieldMapExtractor::BuildSlaveDiscretIdMap()
{
  ...
  for (cit_dis=SlDisVec().begin();cit_dis!=SlDisVec().end();++cit_dis)
  {
    ...
    if (boost::iequals(name,"structure"))
      ...
      dserror("Unknown field discretization name \"%s\"!",
          name.c_str());
    // increase counter
    ++dis_count;
  }
...
{
  std::map<enum FieldName, int>::const_iterator cit =
      slave_discret_id_map_.find(field);
  if (cit==slave_discret_id_map_.end())
    dserror("The slave field \"%s\" could not be found!",
        FieldName2String(field).c_str());
...
void XFEM::MultiFieldMapExtractor::BuildGlobalInterfaceNodeGidSet()
{
  ...
  for (unsigned p=0;p<static_cast<unsigned>( Comm().NumProc() );++p)
  {
    ...
    if ( p==static_cast<unsigned>( Comm().MyPID() ) )
    {
      ...
      for (unsigned j=0;j<slave_discret_vec_.size();++j)
      {
        for (unsigned i=0; i<static_cast<unsigned>( slave_discret_vec_[j]->NumMyRowNodes() );++i)
        {
          ...
          if (not is_unique.second)
            my_interface_row_node_gid_set.insert(gid);
        }
      }
      ...
      std::copy(my_unique_row_node_gid_set.begin(),
          my_unique_row_node_gid_set.end(),my_unique_row_node_gid_vec.begin());
      ...
      std::copy(my_interface_row_node_gid_set.begin(),
          my_interface_row_node_gid_set.end(),my_interface_row_node_gid_vec.begin());
    }
    ...
    if (num_my_interface_row_nodes > 0)
    {
      ...
      g_interface_node_gid_set_.insert(my_interface_row_node_gid_vec.begin(),
          my_interface_row_node_gid_vec.end());
    }
    // ------------------------------------------------------------------------
    ...
    {
      std::pair<std::set<int>::iterator, bool> is_unique =
          g_unique_row_node_gid_set.insert(*cit);
      if (not is_unique.second)
        g_interface_node_gid_set_.insert(*cit);
    }
    ...
    (const std::vector<std::vector<int> >& my_master_interface_node_gids)
{
  ...
  {
    master_interface_node_maps_.push_back(Teuchos::rcp(new Epetra_Map(-1,
    ...
    enum FieldName field, enum MULTIFIELD::BlockType block ) const
{
  switch ( block )
  {
    ...
    {
      return Teuchos::rcpFromRef<const Epetra_Map>( SlaveNodeRowMap( field,
          MULTIFIELD::block_non_interface ) );
      break;
    }
    default:
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::AddMatrix(
    ...
    double scale )
{
  ...
  if (not block_mat)
    dserror("The partial matrix must be a  LINALG::BlockSparseMatrix!");
  if (block_mat->Rows() != 2 or block_mat->Cols() != 2)
    dserror("We support only 2x2 block matrices!");

  LINALG::SparseMatrix* sp_mat =
      dynamic_cast<LINALG::SparseMatrix*>( &full_mat );
  if ( not sp_mat )
      dserror("The full matrix must be a LINALG::SparseMatrix!");
    ...
    double scale)
{
  ...
  full_mat.Add(partial_mat.Matrix(MULTIFIELD::block_non_interface,
      MULTIFIELD::block_non_interface),false,
      scale,1.0);
  ...
  const LINALG::SparseMatrix & src_ni = partial_mat.Matrix(
      MULTIFIELD::block_non_interface, MULTIFIELD::block_interface);
  IMatColTransform(block)(partial_mat.FullRowMap(),partial_mat.FullColMap(),src_ni,
      scale, ADAPTER::CouplingSlaveConverter(ICoupling(block)),full_mat,false,true);
  ...
  IMatRowTransform(block)(src_in,
      scale, ADAPTER::CouplingSlaveConverter(ICoupling(block)),full_mat,true);
  ...
  IMatRowColTransform(block)(src_ii,scale,
      ADAPTER::CouplingSlaveConverter(ICoupling(block)),
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
int XFEM::MultiFieldMapExtractor::IDof(const DRT::Node * inode,
    int dof) const
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::IDof(const DRT::Node * inode,
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::MultiFieldMapExtractor::IDof(std::vector<int>& dof,
    DRT::Node * inode,
    unsigned nodaldofset_id,
