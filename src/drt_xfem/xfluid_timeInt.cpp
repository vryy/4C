...
)
{
  ...
  if (oldRowStateVectors.size() != newRowStateVectors.size())
    dserror("TransferDofsToNewMap: not equal number of old and new vectors");
      ...
      (unique_std_uncut_np and n_old == NULL) or (unique_std_uncut_np and unique_std_uncut_n))
  {
    ...
    CopyDofs(node, nds_new, nds_old, INPAR::XFEM::Xf_TimeInt_STD_by_COPY_from_STD,
  ...
  else if (n_new == NULL or unique_std_uncut_np)
  {
    ...
    else if (numDofSets_old == 1 and !unique_std_uncut_n)
    {
      ...
      else  // case c)
      {
                 ...
                 INPAR::XFEM::Xf_TimeIntScheme_STD_by_Copy_or_Proj_AND_GHOST_by_Proj_or_Copy_or_GP)
        {
          ...
          dserror("unknwon INPAR::XFEM::Xf_TimIntScheme");
      }
    }  // some elements cut
    else if (numDofSets_old == 0)
    {
      ...
      if (timeint_scheme_ == INPAR::XFEM::Xf_TimeIntScheme_STD_by_Copy_AND_GHOST_by_Copy_or_GP)
        dserror("no dofset at t^n available, structural movement more than one element for node %d",
            gid);
      else if (timeint_scheme_ ==
                   INPAR::XFEM::Xf_TimeIntScheme_STD_by_Copy_or_SL_AND_GHOST_by_Copy_or_GP or
  ...
  else if (n_old == NULL or unique_std_uncut_n)
  {
    ...
    //-------------------------------
    // t^(n+1)
    const std::vector<Teuchos::RCP<GEO::CUT::NodalDofSet>>& dof_cellsets_new =
        n_new->NodalDofSets();
    ...
    else
    {
      ...
      // loop new dofsets
      for (std::vector<Teuchos::RCP<GEO::CUT::NodalDofSet>>::const_iterator sets =
           ...
           sets != dof_cellsets_new.end(); sets++)
      {
        ...
        if (nodaldofset_new
                ->Is_Standard_DofSet())  // first dofset (has been checked to be a std-dofset)
        {
          // copy values or SL
          if (timeint_scheme_ ==
  ...
  else
  {
    ...
    //-------------------------------
    // t^n
    const std::vector<Teuchos::RCP<GEO::CUT::NodalDofSet>>& dof_cellsets_old =
        n_old->NodalDofSets();
         ...
         sets != dof_cellsets_new.end(); sets++)
    {
      ...
      if (nds_old < 0)  // no set or not a unique set found
      {
        ...
        if (is_std_set_np)  // std at t^(n+1)
        {
          ...
          {
            MarkDofs(node, nds_new, newRowStateVectors, INPAR::XFEM::Xf_TimeInt_by_PROJ_from_DIS,
                dbcgids);
          }
          else
            dserror("unknwon INPAR::XFEM::Xf_TimIntScheme");
      ...
      else  // unique set found
      {
        ...
        else if (is_std_set_np and !is_std_set_n)  // ghost at t^n and std at t^(n+1)
        {
          ...
          else
          {
            ...
            // MarkDofs(node, nds_new, newRowStateVectors,
        ...
        else if (!is_std_set_np and is_std_set_n)  // ghost at t^(n+1)
        {
          ...
          CopyDofs(node, nds_new, nds_old, INPAR::XFEM::Xf_TimeInt_GHOST_by_COPY_from_STD,
              newRowStateVectors, oldRowStateVectors, dbcgids);
          //            MarkDofs(node, nds_new, newRowStateVectors,
        ...
        else if (!is_std_set_np and !is_std_set_n)
        {
          ...
          CopyDofs(node, nds_new, nds_old, INPAR::XFEM::Xf_TimeInt_GHOST_by_COPY_from_GHOST,
              newRowStateVectors, oldRowStateVectors, dbcgids);
          //            MarkDofs(node, nds_new, newRowStateVectors,
          //            INPAR::XFEM::Xf_TimeInt_GHOST_by_GP,dbcgids);
        }
...
bool XFEM::XFluidTimeInt::NonIntersectedElements(
    DRT::Node* n, const Teuchos::RCP<GEO::CutWizard> wizard)
{
...
{
  if (nds_new != 0)
    dserror("do you really want to find ghost dofsets surrounding a non-std node with id %i?",
        node->Id());
  ...
  for (std::set<GEO::CUT::plain_volumecell_set, GEO::CUT::Cmp>::const_iterator e_vcset =
           cellset.begin();
...
// -------------------------------------------------------------------
void XFEM::XFluidTimeInt::CopyDofs(const DRT::Node* node,  /// drt node
...
)
{
  if (method != INPAR::XFEM::Xf_TimeInt_GHOST_by_COPY_from_GHOST and
    ...
    dserror("don't call CopyDofs for non-copy reconstruction type");
  ...
  for (std::vector<Teuchos::RCP<const Epetra_Vector>>::const_iterator it =
...
// -------------------------------------------------------------------
void XFEM::XFluidTimeInt::MarkDofs(const DRT::Node* node,  /// drt node
    const int nds_new,                                     /// nodal dofset at t^(n+1)
    const std::vector<Teuchos::RCP<Epetra_Vector>>&
        newRowStateVectors,                   /// row map based state vectors at t^(n+1)
    const INPAR::XFEM::XFluidTimeInt method,  /// reconstruction method
...
)
{
  ...
  // loop vectors
  for (std::vector<Teuchos::RCP<Epetra_Vector>>::const_iterator it = newRowStateVectors.begin();
...
// -------------------------------------------------------------------
void XFEM::XFluidTimeInt::MarkDofsForExport(const int nid,  /// node id
...
// -------------------------------------------------------------------
bool XFEM::XFluidTimeInt::SetReconstrMethod(const DRT::Node* node,  /// drt node
...
// -------------------------------------------------------------------
int XFEM::XFluidTimeInt::IdentifyOldSets(const GEO::CUT::Node* n_old,  /// node w.r.t to old wizard
    const GEO::CUT::Node* n_new,                                       /// node w.r.t to new wizard
    const std::vector<Teuchos::RCP<GEO::CUT::NodalDofSet>>&
...
)
{
  ...
  for (std::vector<Teuchos::RCP<GEO::CUT::NodalDofSet>>::const_iterator old_sets =
  ...
  )
  {
    ...
    for (std::vector<Teuchos::RCP<GEO::CUT::NodalDofSet>>::const_iterator old_sets =
         ...
         old_sets != dof_cellsets_old.end(); old_sets++)
    {
      if ((*old_sets)->Is_Standard_DofSet())  // this set is a unique standard set at tn, might it
  ...
  if (identified_old_sets.size() > 1)
  {
         ...
         it != identified_old_sets.end(); it++)
    {
      ...
      if (pos_old == GEO::CUT::Point::undecided or pos_old == GEO::CUT::Point::oncutsurface)
        dserror(
...
)
{
  ...
  if ((pos_old == GEO::CUT::Point::oncutsurface and pos_new == GEO::CUT::Point::oncutsurface))
  {
    ...
    if (cut_sides_old.size() == 0 or cut_sides_new.size() == 0)
      dserror("there are no cutsides but point is 'oncutsurface' at both times");
...
)
{
  ...
  if (pos_old == GEO::CUT::Point::undecided or pos_new == GEO::CUT::Point::undecided)
    dserror("at least one node position undecided!");

  if ((pos_old == GEO::CUT::Point::outside and pos_new == GEO::CUT::Point::outside) or
      (pos_old == GEO::CUT::Point::inside and pos_new == GEO::CUT::Point::inside))
  {
  ...
  if (n_diff.Norm2() > 1e-14)
  {
    ...
    // dserror("node %i seems to move. Check the SpaceTimeChangingSide-Check for ALE!",
    // n_old->Id());
  }
