...
\brief Merge the mutual functionality of the XFLUID and XCONTACT state
  creators.
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::StateCreator::Init(
    const Teuchos::RCP<XFEM::ConditionManager> & condition_manager,
    const Teuchos::ParameterList &               params_xfem,
    int                                          numdof,
    int                                          maxnumdofsets,
    int                                          minnumdofsets,
    bool                                         include_inner)
{
  ...
  nodal_dofset_strategy_ =
      DRT::INPUT::IntegralValue<INPAR::CUT::NodalDofSetStrategy>(
          params_xfem, "NODAL_DOFSET_STRATEGY" );
  volume_cell_gauss_point_by_ =
      DRT::INPUT::IntegralValue<INPAR::CUT::VCellGaussPts>( params_xfem,
          "VOLUME_GAUSS_POINTS_BY" );
  bound_cell_gauss_point_by_ =
      DRT::INPUT::IntegralValue<INPAR::CUT::BCellGaussPts>( params_xfem,
          "BOUNDARY_GAUSS_POINTS_BY" );
    ...
    bool                                          dosetup )
{
  ...
  CompleteState( state, xfielddiscret, Teuchos::null, back_disp_col,
      solver_params, step, time, dosetup );
  ...
  bool                                             dosetup )
{
  ...
  return Create( xfielddiscret_ptr, fielddiscret_ptr, back_disp_col,
      solver_params, step, time, dosetup );
}
  ...
  bool                                          dosetup )
{
  ...
  CompleteState( state, xfielddiscret, fielddiscret, back_disp_col,
      solver_params, step, time, dosetup );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::StateCreator::CompleteState(
    const Teuchos::RCP<XFEM::XFieldState> & state,
    const Teuchos::RCP<DRT::DiscretizationInterface> & xfielddiscret,
    const Teuchos::RCP<DRT::DiscretizationInterface> & fielddiscret,
    Teuchos::RCP<const Epetra_Vector> back_disp_col,
    Teuchos::ParameterList &          solver_params,
    ...
    bool                              dosetup )
{
  ...
  // if success, do the actual function call
  CompleteState( state, xfielddiscret_ptr, fielddiscret_ptr, back_disp_col,
      solver_params, step, time, dosetup );
}
...
{
  if ( state.is_null() )
    dserror( "The state must exist at this point!" );
  ...
  CreateNewCutState( xdofset, wizard, xfielddiscret, back_disp_col,
      solver_params, step );

  //----------------------------------------------------------------------
  // Initialize the state object
  state->Init( condition_manager_, wizard, xdofset, xfielddiscret,
      fielddiscret );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::StateCreator::CreateCutWizard(
    const Teuchos::RCP<DRT::DiscretizationXFEM> & xdiscret,
    const Teuchos::RCP<const Epetra_Vector> &     back_disp_col,
    Teuchos::RCP<GEO::CutWizard> &                wizard )
{
  ...
  wizard->SetBackgroundState(
      back_disp_col,
      condition_manager_->GetLevelSetFieldCol(),
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XFEM::StateCreator::AddCutterStates(
    Teuchos::RCP<GEO::CutWizard> & wizard )
{
  // loop all mesh coupling objects
  for( int mc_idx = 0; mc_idx < condition_manager_->NumMeshCoupling();
       ++mc_idx )
  {
    Teuchos::RCP<XFEM::MeshCoupling> mc_coupl =
        condition_manager_->GetMeshCoupling( mc_idx );

    wizard->AddCutterState( mc_idx,
