    ...
    )
{
  ...
  if(nsd != 3)
    dserror("Only implemented for 3D elements. Should be simple enough to extend...");

  // DOF-COL-MAP
  const Teuchos::RCP<Epetra_Vector> phinp_col = Teuchos::rcp(new Epetra_Vector(*discret->DofColMap()));
  ...
  const Teuchos::RCP<Epetra_MultiVector> gradphirow = Teuchos::rcp(new Epetra_MultiVector(*discret->DofRowMap(),nsd));
  ...
  for(Reconstruct_iterator it_node = nodesToReconstruct.begin(); it_node!= nodesToReconstruct.end(); it_node++)
  {
    ...
    static LINALG::Matrix<nsd,1> node_gradphi_smoothed; // set whole 3D vector also for 2D examples
    ...
    // FOR OTHER TYPES OF ELEMENT THAN HEX -> One needs to change DoMeanValueAveraging - code as well.
    if(DISTYPE==DRT::Element::hex8)
      node_gradphi_smoothed = DoMeanValueAveragingOfElementGradientNode<nsd,DRT::Element::hex8>(discret,elements,phinp_col,nodegid,scatra_dofid);
    else if(DISTYPE==DRT::Element::hex27)
      node_gradphi_smoothed = DoMeanValueAveragingOfElementGradientNode<nsd,DRT::Element::hex27>(discret,elements,phinp_col,nodegid,scatra_dofid);
    else
...
SCATRA::SCATRAUTILS::ComputeGradientAtNodesMeanAverage<3>(
    Teuchos::RCP<DRT::Discretization>  discret,
    const Teuchos::RCP<const Epetra_Vector> state,
    const int scatra_dofid
    );
    ...
    const int scatra_dofid)
{
  ...
  const int numberOfElements = elements.size();
  {
    ...
    for(int ele_current=0; ele_current<numberOfElements; ele_current++)
    {
      ...
      case 2:
      {
        DRT::UTILS::shape_function_2D_deriv1(deriv3Dele,node_Xicoordinates(0),node_Xicoordinates(1),DISTYPE);
        break;
      }
      case 1:
      {
