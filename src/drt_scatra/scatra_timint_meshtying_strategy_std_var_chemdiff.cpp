 ...
 *----------------------------------------------------------------------*/
SCATRA::MeshtyingStrategyStdVar::MeshtyingStrategyStdVar(SCATRA::TimIntVariational* vartimint)
...
Teuchos::RCP<LINALG::SparseOperator> SCATRA::MeshtyingStrategyStdVar::InitSystemMatrix() const
{
  ...
  if (DRT::INPUT::IntegralValue<int>(
          (VarTimInt()->VarParameterList())->sublist("VARIATIONAL"), "BLOCKPRECOND"))
  {
    systemmatrix = Teuchos::rcp(new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(
        *scatratimint_->Splitter(), *scatratimint_->Splitter(), 27, false, true));
  }
...
void SCATRA::MeshtyingStrategyStdVar::BuildBlockNullSpaces() const
{
  ...
  for (int iblock = 0; iblock < scatratimint_->Splitter()->NumMaps(); ++iblock)
  {
    ...
    const Teuchos::RCP<std::vector<double>> ns = Teuchos::rcp(
        new std::vector<double>(scatratimint_->Discretization()->DofRowMap()->NumMyElements(), 1.));
    ...
    LINALG::Solver::FixMLNullspace("Block " + iblockstr.str(),
        *scatratimint_->Discretization()->DofRowMap(), *scatratimint_->Splitter()->Map(iblock),
        blocksmootherparams);
  }
