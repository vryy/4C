...
\brief routines to calculate forcing for homogeneous isotropic turbulence simulations with passive-scalar transport
 ...
 *--------------------------------------------------------------*/
HomIsoTurbScalarForcing::HomIsoTurbScalarForcing(
        ScaTraTimIntImpl* timeint)
        :
        forcing_type_(DRT::INPUT::IntegralValue<INPAR::FLUID::ForcingType>(timeint->extraparams_->sublist("TURBULENCE MODEL"),"FORCING_TYPE")),
        discret_(timeint->discret_),
        forcing_(timeint->forcing_),
        phinp_(timeint->phinp_),
        phiaf_(Teuchos::null),
        threshold_wavenumber_(timeint->extraparams_->sublist("TURBULENCE MODEL").get<double>("THRESHOLD_WAVENUMBER",0)),
        is_genalpha_(false),
        dt_(timeint->dta_)
{
  // set gen-alpha
...
void HomIsoTurbScalarForcing::UpdateForcing(const int step)
{
  ...
  if (activate_)
  {
    ...
    Teuchos::RCP<Teuchos::Array <std::complex<double> > > fphi_hat = Teuchos::rcp( new Teuchos::Array<std::complex<double> >(nummodes_*nummodes_*(nummodes_/2+1)));
    // where f_hat = -C(k) * u_hat according to Hickel 2007
    // C denotes a linear compensation factor

    Teuchos::RCP<Teuchos::Array <double> > fphi = Teuchos::rcp( new Teuchos::Array<double>(nummodes_*nummodes_*nummodes_));
