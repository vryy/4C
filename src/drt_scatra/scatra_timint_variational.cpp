...
void SCATRA::TimIntVariational::EvaluateErrorComparedToAnalyticalSol()
{
  ...
  case INPAR::SCATRA::calcerror_AnalyticSeries:
  {
      ...
      if(nsd_ != 1)
        dserror("The analytic solution has not been defined for this dimension");
      ...
      if(NumScal()<3)
      {
        ...
        dserror("The analytical solution of the plane sheet has only being coded so far for one species");
  ...
  )
{
  if(myrank_==0)
  {
    ...
    {
      f.open(fname.c_str());
      f << "| Step | Time | rel. L2-error concentration | rel. L2-error chemical potential |" << std::endl;
    }
    ...
    f << step_ << " " << time_ << " " << std::setprecision(6) << RelError_Conc << " " << RelError_ChemPot << std::endl;
...
void SCATRA::TimIntVariational::PostProcess_ChemPot(
    ...
    DRT::Node*    node)     //!< Node: to extract corresponding post-process relation
{
  ...
  ChemPot = actmat->ComputeInternalEnergy(conct,actmat->RefMu(), actmat->RefC(),actmat->RefTemp()*actmat->GasConstant(),1);
...
SCATRA::ScalarHandlerVar::ScalarHandlerVar():
        numscal_()
{
  return;
}
