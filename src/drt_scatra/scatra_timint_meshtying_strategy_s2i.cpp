...
couplingtype_(DRT::INPUT::IntegralValue<INPAR::S2I::CouplingType>(parameters,"COUPLINGTYPE")),
D_(Teuchos::null),
M_(Teuchos::null),
E_(Teuchos::null),
P_(Teuchos::null),
  ...
  switch(couplingtype_)
  {
    case INPAR::S2I::coupling_mortar_condensed_bubnov:
    {
      ...
      if(lmside_ == INPAR::S2I::side_slave)
      {
        ...
        if(not slaveonly_)
        {
          // replace slave-side rows of global system matrix by projected slave-side rows including interface contributions
...
void SCATRA::MeshtyingStrategyS2I::EvaluateMeshtying()
{
  ...
  case INPAR::S2I::coupling_nts_standard:
  {
    ...
    if(couplingtype_ == INPAR::S2I::coupling_mortar_standard or lmside_ == INPAR::S2I::side_slave or couplingtype_ == INPAR::S2I::coupling_nts_standard)
    {
      islavematrix_->Zero();
      islaveresidual_->PutScalar(0.);
    }
    ...
    if(couplingtype_ == INPAR::S2I::coupling_mortar_standard or lmside_ == INPAR::S2I::side_master or couplingtype_ == INPAR::S2I::coupling_nts_standard)
    {
      imastermatrix_->Zero();
    ...
    for(std::map<const int,DRT::Condition* const>::iterator islavecondition=slaveconditions_.begin(); islavecondition!=slaveconditions_.end(); ++islavecondition)
    {
      ...
      else
      {
        ...
        EvaluateNTS(
            *islavenodestomasterelements_[islavecondition->first],
            *islavenodeslumpedareas_[islavecondition->first],
            *islavenodesimpltypes_[islavecondition->first],
            idiscret,
            params,
            islavematrix_,
            INPAR::S2I::side_slave,
        ...
        if(intlayergrowth_evaluation_ == INPAR::S2I::growth_evaluation_monolithic)
        {
          ...
          switch(matrixtype_)
          {
            case INPAR::S2I::matrix_sparse:
            {
              ...
              {
                // check matrix
                const Teuchos::RCP<LINALG::SparseMatrix> scatragrowthblock = Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(scatragrowthblock_);
                if(scatragrowthblock == Teuchos::null)
                  dserror("Matrix is not a sparse matrix!");
                ...
                FSI::UTILS::MatrixRowTransform()(*islavematrix,-1.,ADAPTER::CouplingSlaveConverter(*icoup_),*scatragrowthblock,false);
                ...
                FSI::UTILS::MatrixRowTransform()(*islavematrix,-1.,ADAPTER::CouplingSlaveConverter(*icoup_),*scatragrowthblock,true);
              ...
              {
                // check matrix
                const Teuchos::RCP<LINALG::SparseMatrix> growthscatrablock = Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(growthscatrablock_);
                ...
                FSI::UTILS::MatrixColTransform()(imastermatrix->RowMap(),imastermatrix->ColMap(),*imastermatrix,1.,ADAPTER::CouplingSlaveConverter(*icoup_),*growthscatrablock,true,true);
            ...
            case INPAR::S2I::matrix_block_condition_dof:
            {
              ...
              {
                // initialize auxiliary matrix block for linearizations of slave fluxes w.r.t. scatra-scatra interface layer thicknesses
                const Teuchos::RCP<LINALG::BlockSparseMatrixBase> blockslavematrix = Teuchos::rcp(new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(
                    *blockmapgrowth_,
                    *blockmaps_slave_,
                    81,
                    false,
                ...
                // initialize auxiliary system matrix for linearizations of master fluxes associated with scatra-scatra interface coupling w.r.t. scatra-scatra interface layer thicknesses
                LINALG::SparseMatrix mastermatrix(*icoup_->MasterDofMap(),27,false,true);

                // derive linearizations of master fluxes associated with scatra-scatra interface coupling w.r.t. scatra-scatra interface layer thicknesses
                ...
                // derive linearizations of master fluxes associated with scatra-scatra interface layer growth w.r.t. scatra-scatra interface layer thicknesses
                for(int iblock=0; iblock<blockmaps_slave_->NumMaps(); ++iblock)
                  FSI::UTILS::MatrixRowTransform()(
                      blockslavematrix->Matrix(iblock,0),
                      -1.,
                ...
                const Teuchos::RCP<LINALG::BlockSparseMatrixBase> blockmastermatrix = mastermatrix.Split<LINALG::DefaultBlockMatrixStrategy>(*blockmapgrowth_,*blockmaps_);
                blockmastermatrix->Complete();
              ...
              // assemble off-diagonal growth-scatra block of global system matrix, containing derivatives of discrete scatra-scatra interface layer growth residuals w.r.t. discrete scatra degrees of freedom
              {
                ...
                const Teuchos::RCP<LINALG::SparseMatrix> imastermatrix = Teuchos::rcp(new LINALG::SparseMatrix(dofrowmap_growth,81));
                ...
                // derive linearizations of scatra-scatra interface layer growth residuals w.r.t. master-side scalar transport degrees of freedom
                FSI::UTILS::MatrixColTransform()(
                    imastermatrix->RowMap(),
                    imastermatrix->ColMap(),
                    *imastermatrix,
                    1.,
                    ADAPTER::CouplingSlaveConverter(*icoup_),
                    kgm
                ...
                const Teuchos::RCP<LINALG::BlockSparseMatrixBase> blockkgm(kgm.Split<LINALG::DefaultBlockMatrixStrategy>(*blockmaps_,*blockmapgrowth_));
                blockkgm->Complete();
    ...
    ) const
{
  ...
  SCATRA::MortarCellFactory::MortarCellCalc(impltype,slaveelement,masterelement,couplingtype_,lmside_,idiscret.Name())->Evaluate(
      idiscret,
      cell,
      slaveelement,
      masterelement,
    ...
    ) const
{
  ...
  SCATRA::MortarCellFactory::MortarCellCalc(impltype,slaveelement,masterelement,couplingtype_,lmside_,idiscret.Name())->EvaluateNTS(
      idiscret,
      slavenode,
      lumpedarea,
      slaveelement,
    ...
    ) const
{
  ...
  SCATRA::MortarCellFactory::MortarCellCalc(impltype,element,element,couplingtype_,lmside_,idiscret.Name())->EvaluateMortarElement(
      idiscret,
      element,
      la,
      params,
    ...
    ) const
{
  ...
  const std::vector<std::pair<Teuchos::RCP<MORTAR::IntCell>,INPAR::SCATRA::ImplType> >& cells = imortarcells_.at(condition->GetInt("ConditionID"));
    ...
    ) const
{
  ...
  for(int inode=0; inode<noderowmap_slave.NumMyElements(); ++inode)
  {
    ...
    MORTAR::MortarNode* const slavenode = dynamic_cast<MORTAR::MortarNode* const>(idiscret.gNode(noderowmap_slave.GID(inode)));
    if(slavenode == NULL)
      dserror("Couldn't extract slave-side node from discretization!");

    // extract first slave-side element associated with current slave-side node
    MORTAR::MortarElement* const slaveelement = dynamic_cast<MORTAR::MortarElement* const>(slavenode->Elements()[0]);
    if(!slaveelement)
      dserror("Invalid slave-side mortar element!");

    // extract master-side element associated with current slave-side node
    MORTAR::MortarElement* const masterelement = dynamic_cast<MORTAR::MortarElement* const>(idiscret.gElement(islavenodestomasterelements[inode]));
    if(!masterelement)
      dserror("Invalid master-side mortar element!");
    ...
    EvaluateSlaveNode(
        ...
        *slavenode,
        ...
        (INPAR::SCATRA::ImplType) islavenodesimpltypes[inode],
        *slaveelement,
        *masterelement,
        la_slave,
        la_master,
    ...
    ) const
{
  ...
  for(int ielement=0; ielement<ielecolmap.NumMyElements(); ++ielement)
  {
    ...
    MORTAR::MortarElement* const element = dynamic_cast<MORTAR::MortarElement* const>(idiscret.gElement(ielecolmap.GID(ielement)));
    if(!element)
      dserror("Couldn't extract mortar element from mortar discretization!");
    ...
    EvaluateMortarElement(
        idiscret,
        *element,
        (INPAR::SCATRA::ImplType) ieleimpltypes[ielement],
        la,
        params,
        strategy.CellMatrix1(),
        strategy.CellMatrix2(),
    ...
    )
{
  ...
  switch(slaveelement.Shape())
  {
    ...
    {
      return MortarCellCalc<DRT::Element::tri3>(impltype,masterelement,couplingtype,lmside,numdofpernode_slave,disname);
      break;
    }

    case DRT::Element::quad4:
    {
      return MortarCellCalc<DRT::Element::quad4>(impltype,masterelement,couplingtype,lmside,numdofpernode_slave,disname);
      break;
    }
    ...
    )
{
  ...
  switch(masterelement.Shape())
  {
    case DRT::Element::tri3:
    {
      return MortarCellCalc<distypeS,DRT::Element::tri3>(impltype,couplingtype,lmside,numdofpernode_slave,numdofpernode_master,disname);
      break;
    }

    case DRT::Element::quad4:
    {
      return MortarCellCalc<distypeS,DRT::Element::quad4>(impltype,couplingtype,lmside,numdofpernode_slave,numdofpernode_master,disname);
      break;
    }
    ...
    )
{
  ...
  switch(impltype)
  {
    ...
    {
      return SCATRA::MortarCellCalc<distypeS,distypeM>::Instance(couplingtype,lmside,numdofpernode_slave,numdofpernode_master,disname);
      break;
    }

    case INPAR::SCATRA::impltype_elch_electrode:
    {
      return SCATRA::MortarCellCalcElch<distypeS,distypeM>::Instance(couplingtype,lmside,numdofpernode_slave,numdofpernode_master,disname);
      break;
    }

    case INPAR::SCATRA::impltype_elch_electrode_thermo:
    {
      return SCATRA::MortarCellCalcElchSTIThermo<distypeS,distypeM>::Instance(couplingtype,lmside,numdofpernode_slave,numdofpernode_master,disname);
      break;
    }

    case INPAR::SCATRA::impltype_thermo_elch_electrode:
    {
      return SCATRA::MortarCellCalcSTIElch<distypeS,distypeM>::Instance(couplingtype,lmside,numdofpernode_slave,numdofpernode_master,disname);
      break;
    }
...
void SCATRA::MeshtyingStrategyS2I::SetupMeshtying()
{
  ...
  case INPAR::S2I::coupling_nts_standard:
  {
    ...
    for(std::map<const int,DRT::Condition* const>::iterator islavecondition=slaveconditions_.begin(); islavecondition!=slaveconditions_.end(); ++islavecondition)
    {
      ...
      else
      {
        ...
        {
          // extract slave-side node
          MORTAR::MortarNode* const slavenode = dynamic_cast<MORTAR::MortarNode*>(idiscret.gNode(noderowmap_slave.GID(inode)));
          if(!slavenode)
          ...
          for(unsigned imasterelement=0; imasterelement<masterelements.size(); ++imasterelement)
          {
            ...
            double dummy(0.);
            MORTAR::MortarProjector::Impl(*masterelement)->ProjectGaussPointAuxn3D(slavenode->X(),slavenode->MoData().n(),*masterelement,coordinates_master,dummy);

            // check whether projected node lies inside master-side element
            if(masterelement->Shape() == DRT::Element::quad4)
            {
              if(coordinates_master[0] < -1. - ntsprojtol_ or coordinates_master[1] < -1. - ntsprojtol_ or coordinates_master[0] > 1. + ntsprojtol_ or coordinates_master[1] > 1. + ntsprojtol_)
            ...
            {
              if(coordinates_master[0] < -ntsprojtol_ or coordinates_master[1] < -ntsprojtol_ or coordinates_master[0] + coordinates_master[1] > 1. + 2*ntsprojtol_)
                // projected node lies outside master-side element
                continue;
            }
          ...
          // determine physical implementation type of slave-side node based on first associated element
          (*islavenodesimpltypes)[inode] = dynamic_cast<DRT::ELEMENTS::Transport*>(Teuchos::rcp_dynamic_cast<DRT::FaceElement>(islavecondition->second->Geometry()[slavenode->Elements()[0]->Id()])->ParentElement())->ImplType();
        }
        ...
        for(int ielement=0; ielement<elecolmap_slave.NumMyElements(); ++ielement)
          ...
          islaveelementsimpltypes[ielement] = dynamic_cast<DRT::ELEMENTS::Transport*>(Teuchos::rcp_dynamic_cast<DRT::FaceElement>(islavecondition->second->Geometry()[elecolmap_slave.GID(ielement)])->ParentElement())->ImplType();
        ...
        Teuchos::RCP<Epetra_Vector> islavenodeslumpedareas_dofvector = LINALG::CreateVector(dofrowmap_slave);
        EvaluateMortarElements(
            elecolmap_slave,
            islaveelementsimpltypes,
            idiscret,
            eleparams,
            Teuchos::null,
            ...
            INPAR::S2I::side_undefined,
            INPAR::S2I::side_undefined,
            islavenodeslumpedareas_dofvector,
            INPAR::S2I::side_slave,
            Teuchos::null,
            INPAR::S2I::side_undefined
        );
      ...
      case INPAR::S2I::growth_evaluation_semi_implicit:
      {
        ...
        if(intlayergrowth_evaluation_ == INPAR::S2I::growth_evaluation_monolithic)
        {
          ...
          switch(matrixtype_)
          {
            ...
            case INPAR::S2I::matrix_block_condition_dof:
            {
              // initialize map extractor associated with all degrees of freedom for scatra-scatra interface layer growth
              blockmapgrowth_ = Teuchos::rcp(new LINALG::MultiMapExtractor(*dofrowmap_growth,std::vector<Teuchos::RCP<const Epetra_Map> >(1,dofrowmap_growth)));
              blockmapgrowth_->CheckForValidMapExtractor();

              // initialize extended map extractor associated with blocks of global system matrix
              const unsigned nblockmaps = blockmaps_->NumMaps();
              std::vector<Teuchos::RCP<const Epetra_Map> > extendedblockmaps(nblockmaps+1,Teuchos::null);
              ...
              extendedblockmaps_ = Teuchos::rcp(new LINALG::MultiMapExtractor(*extendedmaps_->FullMap(),extendedblockmaps));
              extendedblockmaps_->CheckForValidMapExtractor();

              scatragrowthblock_ = Teuchos::rcp(new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(*blockmapgrowth_,*blockmaps_,81,false,true));
              growthscatrablock_ = Teuchos::rcp(new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(*blockmaps_,*blockmapgrowth_,81,false,true));
          ...
          // initialize extended system matrix including rows and columns associated with scatra-scatra interface layer thickness variables
          extendedsystemmatrix_ = Teuchos::rcp(new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(*extendedblockmaps_,*extendedblockmaps_));
        }
  ...
  const Epetra_Map& dofrowmap = intlayergrowth_evaluation_ == INPAR::S2I::growth_evaluation_monolithic ? *extendedmaps_->FullMap() : *scatratimint_->Discretization()->DofRowMap();
...
void SCATRA::MeshtyingStrategyS2I::InitMeshtying()
{
  ...
  if(conditions.size())
  {
    ...
    // initialize linear solver for monolithic scatra-scatra interface coupling involving interface layer growth
    if(intlayergrowth_evaluation_ == INPAR::S2I::growth_evaluation_monolithic)
    {
      const int extendedsolver = DRT::Problem::Instance()->ScalarTransportDynamicParams().sublist("S2I COUPLING").get<int>("INTLAYERGROWTH_LINEAR_SOLVER");
      if(extendedsolver < 1)
        dserror("Invalid ID of linear solver for monolithic scatra-scatra interface coupling involving interface layer growth!");
      extendedsolver_ = Teuchos::rcp(new LINALG::Solver(DRT::Problem::Instance()->SolverParams(extendedsolver),scatratimint_->Discretization()->Comm(),DRT::Problem::Instance()->ErrorFile()->Handle()));
    }
  } // initialize scatra-scatra interface layer growth
...
void SCATRA::MeshtyingStrategyS2I::BuildBlockNullSpaces() const
{
  ...
  if(intlayergrowth_evaluation_ == INPAR::S2I::growth_evaluation_monolithic)
  {
    ...
    for(int iblock=0; iblock<blockmaps_->NumMaps(); ++iblock)
    {
      ...
      extendedsolver_->Params().sublist("Inverse"+iblockstr.str()) = scatratimint_->Solver()->Params().sublist("Inverse"+iblockstr.str());
    }
    ...
    // equip smoother for extra matrix block with null space associated with all degrees of freedom for scatra-scatra interface layer growth
    Teuchos::ParameterList& mllist = extendedsolver_->Params().sublist("Inverse"+iblockstr.str()).sublist("MueLu Parameters");
    mllist.set("PDE equations",1);
    mllist.set("null space: dimension",1);
    mllist.set("null space: type","pre-computed");
    mllist.set("null space: add default vectors",false);
    const Teuchos::RCP<std::vector<double> > ns = Teuchos::rcp(new std::vector<double>(scatratimint_->DofRowMap(2)->NumMyElements(),1.));
      ...
      switch(couplingtype_)
      {
        case INPAR::S2I::coupling_matching_nodes:
        {
          switch(matrixtype_)
          {
            ...
            {
              // check scalar transport system matrix
              const Teuchos::RCP<const LINALG::SparseMatrix> sparsematrix = Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(systemmatrix);
              if (sparsematrix == Teuchos::null)
                dserror("System matrix is not a sparse matrix!");
              ...
              extendedsystemmatrix_->Assign(0,1,LINALG::View,*Teuchos::rcp_dynamic_cast<const LINALG::SparseMatrix>(scatragrowthblock_));
              extendedsystemmatrix_->Assign(1,0,LINALG::View,*Teuchos::rcp_dynamic_cast<const LINALG::SparseMatrix>(growthscatrablock_));
            ...
            {
              // check scalar transport system matrix
              const Teuchos::RCP<const LINALG::BlockSparseMatrixBase> blocksparsematrix = Teuchos::rcp_dynamic_cast<LINALG::BlockSparseMatrixBase>(systemmatrix);
              ...
              for(unsigned iblock=0; iblock<nblockmaps; ++iblock)
              {
                ...
                extendedsystemmatrix_->Assign(
                    ...
                    nblockmaps,
                    LINALG::View,
                    Teuchos::rcp_dynamic_cast<const LINALG::BlockSparseMatrixBase>(scatragrowthblock_)->Matrix(iblock,0)
                    );
                extendedsystemmatrix_->Assign(
                    nblockmaps,
                    iblock,
                    LINALG::View,
                    Teuchos::rcp_dynamic_cast<const LINALG::BlockSparseMatrixBase>(growthscatrablock_)->Matrix(0,iblock)
                    );
              }
              extendedsystemmatrix_->Assign(nblockmaps,nblockmaps,LINALG::View,*growthgrowthblock_);
          ...
          // solve extended system of equations
          extendedsolver_->Solve(extendedsystemmatrix_->EpetraOperator(),extendedincrement,extendedresidual,true,iteration==1,projector);
...
const LINALG::Solver& SCATRA::MeshtyingStrategyS2I::Solver() const
{
  ...
  if(intlayergrowth_evaluation_ == INPAR::S2I::growth_evaluation_monolithic)
  {
    if(extendedsolver_ == Teuchos::null)
...
void SCATRA::MeshtyingStrategyS2I::EquilibrateSystem(
    const Teuchos::RCP<LINALG::SparseOperator>&   systemmatrix,   //!< system matrix
    const Teuchos::RCP<Epetra_Vector>&            residual,       //!< residual vector
    const LINALG::MultiMapExtractor&              blockmaps       //!< map extractor associated with blocks of global system matrix
    ) const
{
    ...
    )
{
  ...
  if(!delete_me)
  {
    ...
    if(instances.find(disname) == instances.end())
      instances[disname] = new MortarCellCalc<distypeS,distypeM>(couplingtype,lmside,numdofpernode_slave,numdofpernode_master);
  }
  ...
  else
  {
    ...
    for(typename std::map<std::string,MortarCellCalc<distypeS,distypeM>* >::iterator i=instances.begin(); i!=instances.end(); ++i)
    {
    ...
    ) const
{
  ...
  DRT::UTILS::ExtractMyValues<LINALG::Matrix<nen_slave_,1> >(*state,estate_slave,la_slave[nds].lm_);
    ...
    ) const
{
  ...
  // extract nodal state variables associated with slave and master elements
  DRT::UTILS::ExtractMyValues<LINALG::Matrix<nen_slave_,1> >(*state,estate_slave,la_slave[nds].lm_);
  DRT::UTILS::ExtractMyValues<LINALG::Matrix<nen_master_,1> >(*state,estate_master,la_master[nds].lm_);
...
/*--------------------------------------------------------------------------------------------------------------------------*
 | evaluate slave-side and master-side shape functions and domain integration factor at cell integration point   fang 01/16 |
 *--------------------------------------------------------------------------------------------------------------------------*/
template<DRT::Element::DiscretizationType distypeS,DRT::Element::DiscretizationType distypeM>
...
/*-------------------------------------------------------------------------------------------------------------*
 | evaluate slave-side shape functions and domain integration factor at element integration point   fang 08/16 |
 *-------------------------------------------------------------------------------------------------------------*/
template<DRT::Element::DiscretizationType distypeS,DRT::Element::DiscretizationType distypeM>
double SCATRA::MortarCellCalc<distypeS,distypeM>::EvalShapeFuncAndDomIntFacAtIntPoint(
    MORTAR::MortarElement&                               element,     //!< mortar element
    ...
    )
{
  ...
  GEO::fillInitialPositionArray<distypeS,nsd_slave_+1,LINALG::Matrix<nsd_slave_+1,nen_slave_> >(&element,coordinates_nodes);
  ...
  // return domain integration factor, i.e., Jacobian determinant times integration weight, at integration point
    ...
    )
{
  ...
  if(index == -1)
    dserror("Couldn't find out index of slave-side node w.r.t. slave-side element!");
...
/*-------------------------------------------------------------------------------------------------------------*
 | evaluate and assemble lumped interface area fractions associated with slave-side element nodes   fang 08/16 |
 *-------------------------------------------------------------------------------------------------------------*/
template<DRT::Element::DiscretizationType distypeS,DRT::Element::DiscretizationType distypeM>
    ...
    )
{
  ...
  const DRT::UTILS::IntPointsAndWeights<nsd_slave_> intpoints(SCATRA::DisTypeToOptGaussRule<distypeS>::rule);
  ...
  for(int iquad=0; iquad<intpoints.IP().nquad; ++iquad)
  {
    ...
    // compute integrals of shape functions to obtain lumped interface area fractions associated with element nodes
    ...
    ) const
{
  ...
  if(AssembleMatrix1())
    AssembleCellMatrix(systemmatrix1_,cellmatrix1_,matrix1_side_rows_,matrix1_side_cols_,la_slave,la_master,assembler_pid_master);

  // assemble cell matrix 2 into system matrix 2
  if(AssembleMatrix2())
    AssembleCellMatrix(systemmatrix2_,cellmatrix2_,matrix2_side_rows_,matrix2_side_cols_,la_slave,la_master,assembler_pid_master);

  // assemble cell matrix 3 into system matrix 3
  if(AssembleMatrix3())
    AssembleCellMatrix(systemmatrix3_,cellmatrix3_,matrix3_side_rows_,matrix3_side_cols_,la_slave,la_master,assembler_pid_master);

  // assemble cell matrix 4 into system matrix 4
  if(AssembleMatrix4())
    AssembleCellMatrix(systemmatrix4_,cellmatrix4_,matrix4_side_rows_,matrix4_side_cols_,la_slave,la_master,assembler_pid_master);

  // assemble cell vector 1 into system vector 1
  if(AssembleVector1())
    AssembleCellVector(systemvector1_,cellvector1_,vector1_side_,la_slave,la_master,assembler_pid_master);

  // assemble cell vector 2 into system vector 2
  if(AssembleVector2())
    AssembleCellVector(systemvector2_,cellvector2_,vector2_side_,la_slave,la_master,assembler_pid_master);
    ...
    ) const
{
  ...
  switch(side_rows)
  {
    case INPAR::S2I::side_slave:
    {
      systemmatrix->Assemble(-1,la_cols[nds_cols_].stride_,cellmatrix,la_slave[nds_rows_].lm_,la_slave[nds_rows_].lmowner_,la_cols[nds_cols_].lm_);
      break;
    }

    case INPAR::S2I::side_master:
    {
      Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(systemmatrix)->FEAssemble(cellmatrix,la_master[nds_rows_].lm_,std::vector<int>(la_master[nds_rows_].lmowner_.size(),assembler_pid_master),la_cols[nds_cols_].lm_);
      break;
    }
    ...
    ) const
{
  ...
  switch(side)
  {
    case INPAR::S2I::side_slave:
    {
      ...
      LINALG::Assemble(*(*systemvector)(nds_rows_),cellvector,la_slave[nds_rows_].lm_,la_slave[nds_rows_].lmowner_);
    ...
    case INPAR::S2I::side_master:
    {
      if(assembler_pid_master == systemvector->Comm().MyPID())
        if(Teuchos::rcp_dynamic_cast<Epetra_FEVector>(systemvector)->SumIntoGlobalValues(la_master[nds_rows_].lm_.size(),&la_master[nds_rows_].lm_[0],cellvector.A()))
    ...
    )
{
  ...
  if(AssembleVector1())
      InitCellVector(cellvector1_,vector1_side_,la_slave,la_master);

  // initialize system vector 2
  if(AssembleVector2())
      InitCellVector(cellvector2_,vector2_side_,la_slave,la_master);
    ...
    ) const
{
  ...
  const int nrows = side_rows == INPAR::S2I::side_slave ? la_slave[nds_rows_].Size() : la_master[nds_rows_].Size();
  const int ncols = side_cols == INPAR::S2I::side_slave ? la_slave[nds_cols_].Size() : la_master[nds_cols_].Size();
    ...
    ) const
{
  ...
  const int ndofs = side == INPAR::S2I::side_slave ? la_slave[nds_rows_].Size() : la_master[nds_rows_].Size();
