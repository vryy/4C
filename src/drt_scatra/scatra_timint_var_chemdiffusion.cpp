...
SCATRA::TimIntVarChemDiffusionOST::TimIntVarChemDiffusionOST(
  ...
  ,phi0_(Teuchos::null)
  ,semImplicitFunctional_(DRT::INPUT::IntegralValue<int>(params->sublist("VARIATIONAL"),"SEMIMPLICITFUNCTIONAL"))
{
  return;
}
...
void SCATRA::TimIntVarChemDiffusionOST::AddTimeIntegrationSpecificVectors(bool forcedincrementalsolver)
{
    ...
    case INPAR::SCATRA::initialfield_algebraic_field_dependence:
    {
        ...
        for (int n=0;n<discret_->NodeRowMap()->NumMyElements();++n)
        {
          ...
          for (int loc_id=indexOffsetIndependentVariable; loc_id<numdofs; ++loc_id)
          {
            ...
            initialval = actmat->ComputeDualInternalEnergy( initialval,
                                                                  actmat->RefMu(),
                                    actmat->RefC(),
                                    actmat->RefTemp()*actmat->GasConstant(),
                                    1);
...
)
{
  ...
  /*!
    ...
    brief: This part is needed since in the variational formulation only the boundary conditions are prescribed for the chemical potential. Based on that when the concentration is also defined at the nodes it must be used the Internal energy to relate the concentration at the dirichlet boundary to that of the chemical potential
   */
  ...
  for (int i=0;i<phinpDirichlet->MyLength(); i=i+numdofpernode)
  {
    ...
    if (lid_in_global == -1)
      dserror("didn't find gid in global vector on this proc");
