 ...
 *----------------------------------------------------------------------*/
void SCATRA::ScaTraTimIntImpl::SetWallShearStresses(Teuchos::RCP<const Epetra_Vector> wss, const int nds_wss)
{
  if (wss == Teuchos::null)
  ...
  while(true)
  {
...

    {
      ...
      if(DRT::INPUT::IntegralValue<int>(*params_,"OUTPUTLINSOLVERSTATS"))
        OutputLinSolverStats(strategy_->Solver(),dtsolve_,Step(),iternum_,strategy_->DofRowMap().NumGlobalElements());
    }
...
void SCATRA::ScaTraTimIntImpl::NonlinearMultiScaleSolve()
{
  ...
  while(true)
  {
       ...
       solvtype_ == INPAR::SCATRA::solvertype_nonlinear_multiscale_macrotomicro_aitken_dofsplit)
    {
      ...
      // replace macro-scale state vector by relaxed macro-scale state vector as input for micro scale
