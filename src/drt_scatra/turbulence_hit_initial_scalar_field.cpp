...
void HomIsoTurbInitialScalarField::CalculateInitialField()
{
  ...
  Teuchos::RCP<Teuchos::Array <double> > phi = Teuchos::rcp( new Teuchos::Array<double>(nummodes_*nummodes_*nummodes_));
...
//              random_theta = random->Uni();
  ...
  Teuchos::RCP<Teuchos::Array <std::complex<double> > > phi_hat_fftw = Teuchos::rcp( new Teuchos::Array<std::complex<double> >(nummodes_*nummodes_*(nummodes_/2+1)));
