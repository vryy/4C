    ...
    Epetra_SerialDenseVector& elevec3_epetra)
{
  ...
  else  // old structural time integration
  {
    ...
    if (action == "none")
      ...
      act = ELEMENTS::struct_calc_fluid_traction;
    else if (action == "calc_cur_normal_at_point")
      act = ELEMENTS::struct_calc_cur_normal_at_point;
    else
    {
  ...
  switch (act)
  {
    ...
    case ELEMENTS::struct_calc_fluid_traction:
    {
      ...
      // - remove the overloaded function
      // IMMERSED::ImmersedPartitionedFSIDirichletNeumannMembrane::CalcFluidTractionsOnStructure()
      ...
      std::string backgrddisname(
          params.get<std::string>("backgrddisname"));  // fluid discretization
      std::string immerseddisname(
          params.get<std::string>("immerseddisname"));  // structure discretization

      const Teuchos::RCP<DRT::Discretization> fluid_dis =
          globalproblem->GetDis(backgrddisname);  // fluid discretization
      const Teuchos::RCP<DRT::Discretization> struct_dis =
          globalproblem->GetDis(immerseddisname);  // structure discretization
      ...
      for (int gp = 0; gp < intpoints_.nquad; ++gp)
      {
        ...
        // interpolate the fluid stresses to the current gauss point position of the membrane
        // element remark: the fluid discretization is fixed to hex8
        IMMERSED::InterpolateToImmersedIntPoint<DRT::Element::hex8,  // source
            distype>                                                 // target
            (fluid_dis, struct_dis, *this, xi, mydisp, action,
    ...
    {
      // this action calculates the (non normalized) current normal vector on the membrane element
      // surface of a point given in parameter space coordinates (xi,eta) as stored in
      // elevec2_epetra
