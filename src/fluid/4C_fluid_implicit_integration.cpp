...
void FLD::FluidImplicitTimeInt::init()
{
  ...
  cfl_estimator_ = Core::UTILS::integral_value<Inpar::FLUID::AdaptiveTimeStepEstimator>(
  ...
  newton_ = Core::UTILS::get_as_enum<Inpar::FLUID::LinearisationAction>(*params_, "Linearisation");
...

  {
    ...
    if (Core::UTILS::integral_value<int>(params_->sublist("WALL MODEL"), "X_WALL"))
    {
  ...
  if (Core::UTILS::integral_value<Inpar::FLUID::StabType>(*stabparams, "STABTYPE") ==
      Inpar::FLUID::stabtype_edgebased)
  {
    create_faces_extension();
  }
  reconstructder_ = Core::UTILS::integral_value<int>(*stabparams, "Reconstruct_Sec_Der");
...
void FLD::FluidImplicitTimeInt::init_nonlinear_bc()
{
  ...
  if (impedancecond.size() > 0)
  {
    ...
    fssgv_ = Core::UTILS::integral_value<Inpar::FLUID::FineSubgridVisc>(
...
{  // forcing_->PutScalar(0.0);
  ...
  if (Core::UTILS::integral_value<Inpar::FLUID::ForcingType>(
...
void FLD::FluidImplicitTimeInt::evaluate_mat_and_rhs(Teuchos::ParameterList& eleparams)
{
  if (off_proc_assembly_)
  {
    ...
    Teuchos::RCP<Epetra_Vector> residual_col = Core::LinAlg::create_vector(*dofcolmap, true);
    ...
    //-------------------------------------------------------------------------------
    Teuchos::RCP<Epetra_Vector> tmp = Core::LinAlg::create_vector(*discret_->dof_row_map(), true);
 ...
 *----------------------------------------------------------------------*/
void FLD::FluidImplicitTimeInt::iter_update(const Teuchos::RCP<const Epetra_Vector> increment)
{
...
void FLD::FluidImplicitTimeInt::update_gridv()
{
  ...
  const int gridvel = Core::UTILS::integral_value<Inpar::FLUID::Gridvel>(fluiddynparams, "GRIDVEL");
 ...
 *----------------------------------------------------------------------*/
void FLD::FluidImplicitTimeInt::set_iter_scalar_fields(Teuchos::RCP<const Epetra_Vector> scalaraf,
...
/*----------------------------------------------------------------------*
 ...
 | overloaded in TimIntLoma                                    bk 12/13 |
 *----------------------------------------------------------------------*/
void FLD::FluidImplicitTimeInt::set_scalar_fields(Teuchos::RCP<const Epetra_Vector> scalarnp,
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
Teuchos::RCP<const Epetra_Vector> FLD::FluidImplicitTimeInt::extract_velocity_part(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
Teuchos::RCP<const Epetra_Vector> FLD::FluidImplicitTimeInt::extract_pressure_part(
    Teuchos::RCP<const Epetra_Vector> velpres)
{
...
{
  auto calcerr = Core::UTILS::get_as_enum<Inpar::FLUID::CalcError>(*params_, "calculate error");
...
void FLD::FluidImplicitTimeInt::linear_relaxation_solve(Teuchos::RCP<Epetra_Vector> relax)
{
  ...
  if (not inrelaxation_)
  {
    ...
    const Epetra_Map* dofrowmap = discret_->dof_row_map();
    Teuchos::RCP<Epetra_Vector> griddisp = Core::LinAlg::create_vector(*dofrowmap, false);
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
Teuchos::RCP<const Epetra_Vector> FLD::FluidImplicitTimeInt::inv_dirichlet()
{
  if (dbcmaps_ == Teuchos::null) FOUR_C_THROW("Dirichlet map has not been allocated");
...
/*----------------------------------------------------------------------*
 | update Newton step                                                   |
 *----------------------------------------------------------------------*/
void FLD::FluidImplicitTimeInt::update_newton(Teuchos::RCP<const Epetra_Vector> vel)
{
...
void FLD::FluidImplicitTimeInt::print_stabilization_details() const
{
  ...
  if (myrank_ == 0)
  {
    ...
    else if (Core::UTILS::integral_value<Inpar::FLUID::StabType>(*stabparams, "STABTYPE") ==
             Inpar::FLUID::stabtype_edgebased)
    {
  ...
  if (myrank_ == 0 and turbmodel_ != Inpar::FLUID::no_model)
  {
    ...
    else if (turbmodel_ == Inpar::FLUID::multifractal_subgrid_scales)
    {
      ...
      std::cout << "- Scale separation:  " << modelparams->get<std::string>("SCALE_SEPARATION")
                << "\n";
      if ((Core::UTILS::integral_value<int>(*modelparams, "CALC_N")))
      {
      ...
      std::cout << "- near-wall limit:   "
                << Core::UTILS::integral_value<int>(*modelparams, "NEAR_WALL_LIMIT") << "\n";
      ...
      if ((Core::UTILS::integral_value<int>(*modelparams, "SET_FINE_SCALE_VEL")))
...
// -------------------------------------------------------------------
Teuchos::RCP<Epetra_Vector> FLD::FluidImplicitTimeInt::extrapolate_end_point(
...
/*------------------------------------------------------------------------------------------------*
 |
 *------------------------------------------------------------------------------------------------*/
Teuchos::RCP<const Epetra_Vector> FLD::FluidImplicitTimeInt::convective_vel()
{
 ...
 *------------------------------------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> FLD::FluidImplicitTimeInt::calc_div_op()
{
...
void FLD::FluidImplicitTimeInt::set_fld_gr_disp(Teuchos::RCP<Epetra_Vector> fluid_growth_disp)
{
...
{
  msht_ = Core::UTILS::get_as_enum<Inpar::FLUID::MeshTying>(*params_, "MESHTYING");
 ...
 *                                                          wirtz 01/16 |
 *----------------------------------------------------------------------*/
void FLD::FluidImplicitTimeInt::update_slave_dof(Teuchos::RCP<Epetra_Vector>& f)
{
  if (msht_ != Inpar::FLUID::no_meshtying)
  {
