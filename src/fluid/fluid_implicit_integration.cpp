...
#include "lib_function.H"  //Todo: ager check if this header can be removed after NavierSlip is removed from BACI
...
void FLD::FluidImplicitTimeInt::CompleteGeneralInit()
{
  ...
  if (params_->get<bool>("COMPUTE_EKIN"))
  {
    ...
    massmat_ = Teuchos::rcp(new CORE::LINALG::SparseMatrix(*DofRowMap(), 108, false, true));
    EvaluateMassMatrix();
  }
}
  ...
  for (auto& cond : conds)
  {
    ...
    if (coupling == "lagrange")
    {
      ...
      for (int gIdNode : gIdNodes)
      {
        ...
        GetDofsVectorLocalIndicesforNode(gIdNode, gridv, false, &dofsLocalInd);
    ...
    else
    {
      ...
      {  // Obtain node normals from function
        ...
        for (int gIdNode : gIdNodes)
        {
          ...
          // Obtain local degree of freedom indices
          std::vector<int> dofsLocalInd;
          GetDofsVectorLocalIndicesforNode(gIdNode, nodeNormals, false, &dofsLocalInd);
    ...
    if (initfield == INPAR::FLUID::initfield_disturbed_field_from_function)
    {
      ...
      for (int lnodeid = 0; lnodeid < discret_->NumMyRowNodes(); ++lnodeid)
      {
        ...
        if (mypbc.size() > 0)
        {
          ...
          auto master = (discret_->GetAllPBCCoupledColNodes())->find(lnode->Id());
...
void FLD::FluidImplicitTimeInt::LiftDrag() const
{
  ...
  if (params_->get<bool>("LIFTDRAG"))
  {
    ...
    if (slipsuppline.size() != 0 or slipsuppsurf.size() != 0)
    {
      ...
      forces = CORE::LINALG::CreateVector(*(discret_->DofRowMap()), true);
...
const Teuchos::RCP<const Epetra_Vector> FLD::FluidImplicitTimeInt::InvDirichlet()
{
  ...
  Teuchos::RCP<Epetra_Vector> invtoggle =
      CORE::LINALG::CreateVector(*(discret_->DofRowMap()), false);
...
void FLD::FluidImplicitTimeInt::SetGeneralTurbulenceParameters()
{
          ...
          .get<std::string>("TURBULENCE_APPROACH", "DNS_OR_RESVMM_LES") == "CLASSICAL_LES")
  {
    ...
    else if (physmodel == "Multifractal_Subgrid_Scales")
    {
      ...
      fsvelaf_ = CORE::LINALG::CreateVector(*discret_->DofRowMap(), true);
      ...
      fsscaaf_ = CORE::LINALG::CreateVector(*discret_->DofRowMap(), true);
    }
...
void FLD::FluidImplicitTimeInt::SetupMeshtying()
{
  ...
  meshtying_->SetupMeshtying(coupleddof, alldofcoupled);
 ...
 *----------------------------------------------------------------------------*/
void FLD::FluidImplicitTimeInt::SetDt(const double dtnew) { dta_ = dtnew; }
   ...
   */
    if (params_->get<int>("MESHTYING") == INPAR::FLUID::no_meshtying)
    {
      ...
      couplingcontributions_ = Teuchos::rcp(new CORE::LINALG::SparseMatrix(
          *discret_->DofRowMap(), 30, true, true, CORE::LINALG::SparseMatrix::FE_MATRIX));
    }
    else
    {
...
void FLD::FluidImplicitTimeInt::InitForcing()
{
  ...
  {
    forcing_ = CORE::LINALG::CreateVector(*(discret_->DofRowMap()), true);
