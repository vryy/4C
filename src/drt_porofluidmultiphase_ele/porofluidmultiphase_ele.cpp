/*----------------------------------------------------------------------*/
/*!
 \file porofluidmultiphase_ele.cpp
...
{
  DRT::ELEMENTS::PoroFluidMultiPhase* object =
    new DRT::ELEMENTS::PoroFluidMultiPhase(-1,-1);
  object->Unpack(data);
  return object;
}
 ...
 *----------------------------------------------------------------------*/
Teuchos::RCP<DRT::Element> DRT::ELEMENTS::PoroFluidMultiPhaseType::Create( const std::string eletype,
 ...
 *----------------------------------------------------------------------*/
Teuchos::RCP<DRT::Element> DRT::ELEMENTS::PoroFluidMultiPhaseType::Create( const int id, const int owner )
{
  Teuchos::RCP<DRT::Element> ele = Teuchos::rcp(new DRT::ELEMENTS::PoroFluidMultiPhase(id,owner));
  return ele;
}
 ...
 *----------------------------------------------------------------------------*/
void DRT::ELEMENTS::PoroFluidMultiPhaseType::NodalBlockInformation( DRT::Element * dwele, int & numdf, int & dimns, int & nv, int & np )
{
 ...
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::PoroFluidMultiPhaseType::ComputeNullSpace( DRT::Discretization & dis, std::vector<double> & ns, const double * x0, int numdf, int dimns )
{
...
/*----------------------------------------------------------------------*
 ...
 |                                                           vuong 08/16 |
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::PoroFluidMultiPhaseType::SetupElementDefinition( std::map<std::string,std::map<std::string,DRT::INPUT::LineDefinition> > & definitions )
{
...
{
  DRT::ELEMENTS::PoroFluidMultiPhase* newelement =
      new DRT::ELEMENTS::PoroFluidMultiPhase(*this);
  return newelement;
}
...
std::vector<Teuchos::RCP<DRT::Element> > DRT::ELEMENTS::PoroFluidMultiPhase::Lines()
{
  ...
  if (NumLine() > 1) // 3D and 2D
    return DRT::UTILS::ElementBoundaryFactory<PoroFluidMultiPhaseBoundary,
        PoroFluidMultiPhase>(DRT::UTILS::buildLines, this);
  else
  {
...
std::vector<Teuchos::RCP<DRT::Element> > DRT::ELEMENTS::PoroFluidMultiPhase::Surfaces()
{
  ...
  if (NumSurface() > 1) // 3D
    return DRT::UTILS::ElementBoundaryFactory<PoroFluidMultiPhaseBoundary,
        PoroFluidMultiPhase>(DRT::UTILS::buildSurfaces, this);
...
DRT::ELEMENTS::PoroFluidMultiPhaseBoundary::PoroFluidMultiPhaseBoundary(
    ...
    const int* nodeids,
    DRT::Node** nodes,
    DRT::ELEMENTS::PoroFluidMultiPhase* parent,
    const int lbeleid) :
