  ...
  switch(couplingtype_)
  {
    ...
    case INPAR::STI::coupling_twoway_thermotoscatra_aitken:
    {
      ...
      if(couplingtype_ == INPAR::STI::coupling_twoway_scatratothermo_aitken or
        ...
        scatra_->PhinpIncOld() = LINALG::CreateVector(*scatra_->Discretization()->DofRowMap(),true);
      else if(couplingtype_ == INPAR::STI::coupling_twoway_thermotoscatra_aitken)
        thermo_->PhinpIncOld() = LINALG::CreateVector(*thermo_->Discretization()->DofRowMap(),true);
...
bool STI::Partitioned::ExitOuterCoupling() const
{
  ...
  if(mypid == 0)
  {
    ...
    std::cout << "|  " << std::setw(3) << iter_ << "/" << std::setw(3) << itermax_ << "   | "
              ...
              << std::setw(10) << std::setprecision(3) << std::scientific << L2_scatra_inc/L2_scatra << "   | "
              << std::setw(10) << std::setprecision(3) << std::scientific << L2_thermo_inc/L2_thermo << "   |" << std::endl;
    std::cout << "+------------+-------------------+--------------+--------------+" << std::endl;
  }
...
void STI::Partitioned::SolveTwoWay()
{
  ...
  switch(couplingtype_)
  {
    ...
    case INPAR::STI::coupling_twoway_scatratothermo_aitken_dofsplit:
    {
      ...
      const Teuchos::RCP<Epetra_Vector> scatra_relaxed = Teuchos::rcp(new Epetra_Vector(*scatra_->Phiafnp()));
      ...
      while(true)
      {
        ...
        if(thermo_->PhinpInc()->Update(1.,*thermo_->Phiafnp(),0.))
        ...
        if(thermo_->PhinpInc()->Update(1.,*thermo_->Phiafnp(),-1.))
          dserror("Update failed!");
        ...
        if(scatra_->PhinpInc()->Update(1.,*scatra_relaxed,0.))
          dserror("Update failed!");
        ...
        if(scatra_->PhinpInc()->Update(1.,*scatra_->Phiafnp(),-1.))
          dserror("Update failed!");

        // convergence check
        if(ExitOuterCoupling())
          break;
        ...
        else
        {
          ...
          if(scatra_inc_diff.Update(-1.,*scatra_->PhinpIncOld(),1.))
            dserror("Update failed!");

          if(couplingtype_ == INPAR::STI::coupling_twoway_scatratothermo_aitken)
          {
            // compute L2 norm of difference between current and previous increments of scatra state vector
            double scatra_inc_diff_L2(0.);
            scatra_inc_diff.Norm2(&scatra_inc_diff_L2);

            // compute dot product between increment of scatra state vector and difference between current and previous increments of scatra state vector
            ...
            if(iter_ > 1 and scatra_inc_diff_L2 > 1.e-12)
              scatra_->Omega()[0] *= 1 - scatra_inc_dot_scatra_inc_diff/(scatra_inc_diff_L2*scatra_inc_diff_L2);

            // restrict Aitken relaxation factor if necessary
            if(omegamax_ > 0. and scatra_->Omega()[0] > omegamax_)
              scatra_->Omega()[0] = omegamax_;
          ...
          {
            // safety check
            if(scatra_->Splitter() == Teuchos::null)
            ...
            for(int idof=0; idof<scatra_->Splitter()->NumMaps(); ++idof)
            {
              ...
              const Teuchos::RCP<const Epetra_Vector> scatra_inc_dof = scatra_->Splitter()->ExtractVector(*scatra_->PhinpInc(),idof);
              const Teuchos::RCP<const Epetra_Vector> scatra_inc_diff_dof = scatra_->Splitter()->ExtractVector(scatra_inc_diff,idof);
              ...
              if(iter_ > 1 and scatra_inc_diff_L2 > 1.e-12)
                scatra_->Omega()[idof] *= 1 - scatra_inc_dot_scatra_inc_diff/(scatra_inc_diff_L2*scatra_inc_diff_L2);
              ...
              scatra_->Splitter()->AddVector(*scatra_inc_dof,idof,*scatra_relaxed,scatra_->Omega()[idof]);
            }
          }
    ...
    case INPAR::STI::coupling_twoway_thermotoscatra_aitken:
    {
      ...
      const Teuchos::RCP<Epetra_Vector> thermo_relaxed = Teuchos::rcp(new Epetra_Vector(*thermo_->Phiafnp()));
      ...
      while(true)
      {
        ...
        // store current scatra state vector
        if(scatra_->PhinpInc()->Update(1.,*scatra_->Phiafnp(),0.))
          dserror("Update failed!");
        ...
        // store current thermo state vector
        if(thermo_->PhinpInc()->Update(1.,*thermo_relaxed,0.))
          dserror("Update failed!");
        ...
        if(couplingtype_ == INPAR::STI::coupling_twoway_thermotoscatra_aitken)
        {
          ...
          if(thermo_inc_diff.Update(-1.,*thermo_->PhinpIncOld(),1.))
            dserror("Update failed!");

          // compute L2 norm of difference between current and previous increments of thermo state vector
          double thermo_inc_diff_L2(0.);
          thermo_inc_diff.Norm2(&thermo_inc_diff_L2);

          // compute dot product between increment of thermo state vector and difference between current and previous increments of thermo state vector
          ...
          if(iter_ > 1 and thermo_inc_diff_L2 > 1.e-12)
            thermo_->Omega()[0] *= 1 - thermo_inc_dot_thermo_inc_diff/(thermo_inc_diff_L2*thermo_inc_diff_L2);

          // restrict Aitken relaxation factor if necessary
          if(omegamax_ > 0. and thermo_->Omega()[0] > omegamax_)
            thermo_->Omega()[0] = omegamax_;
