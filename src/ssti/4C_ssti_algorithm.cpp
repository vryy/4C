...
void SSTI::SSTIAlgorithm::clone_discretizations(const Epetra_Comm& comm)
{
  ...
  {
    DRT::UTILS::CloneDiscretization<SSTI::SSTIScatraStructureCloneStrategy>(
        structdis, scatradis, GLOBAL::Problem::Instance()->CloningMaterialMap());
    scatradis->fill_complete();
    DRT::UTILS::CloneDiscretization<SSTI::SSTIScatraThermoCloneStrategy>(
...
{
  StructureField()->read_restart(restart);
...
{
  StructureField()->Discretization()->set_state(1, "scalarfield", scatra_->ScaTraField()->Phinp());
...
{
  StructureField()->Discretization()->set_state(2, "tempfield", thermo_->ScaTraField()->Phinp());
...
void SSTI::SSTIAlgorithm::distribute_dt_from_sca_tra()
{
  ...
  StructureField()->set_dt(newtimestep);
