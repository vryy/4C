...
\param   *hasext        INT             (i)     flag, if there's external load
...
INT     row[MAXNOD_F3]; /* index field for free surface elements        */
   ...
   for (i=0; i<iel; i++)
   {
      for (j=0; j<iel; j++)
      {
         eiforce[i*dofpern]   += (emass[i*dim][j*dim]   * hist[0][j]
                                ...
                                + emass[i*dim][j*dim+2] * hist[2][j]);
         eiforce[i*dofpern+1] += (emass[i*dim+1][j*dim]   * hist[0][j]
                                + emass[i*dim+1][j*dim+1] * hist[1][j]
                                + emass[i*dim+1][j*dim+2] * hist[2][j]);
         eiforce[i*dofpern+2] += (emass[i*dim+2][j*dim]   * hist[0][j]
                                + emass[i*dim+2][j*dim+1] * hist[1][j]
                                + emass[i*dim+2][j*dim+2] * hist[2][j]);
         eiforce[i*dofpern+3] += (emass[iel*dim+i][j*dim]   * hist[0][j]
                                + emass[iel*dim+i][j*dim+1] * hist[1][j]
                                + emass[iel*dim+i][j*dim+2] * hist[2][j]);
      }
   }
   ...
   for (i=0; i<iel; i++)
   {
      for (j=0; j<iel; j++)
      {
         ...
         eiforce[row[i]+1] += (emass[i*dim+1][j*dim]   * hist[0][j]
                             + emass[i*dim+1][j*dim+1] * hist[1][j]
                             + emass[i*dim+1][j*dim+2] * hist[2][j]);
         eiforce[row[i]+2] += (emass[i*dim+2][j*dim]   * hist[0][j]
                             + emass[i*dim+2][j*dim+1] * hist[1][j]
                             + emass[i*dim+2][j*dim+2] * hist[2][j]);
         eiforce[row[i]+3] += (emass[iel*dim+i][j*dim]   * hist[0][j]
                             + emass[iel*dim+i][j*dim+1] * hist[1][j]
                             + emass[iel*dim+i][j*dim+2] * hist[2][j]);
      }
   }
