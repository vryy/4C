...
/*----------------------------------------------------------------------*/
INT i,j,k,l;
INT  ID;     /* Id of actual submesh ele if it's mat. law for small scale*/
/*------------------------------------------------------------------------*/
DOUBLE yip;
DOUBLE disd[5];
DOUBLE sig[4];
...
/*----------------------------------------------------------------------*/
/*     compute strains if mat-law is not called by submesh-element      */
/*----------------------------------------------------------------------*/
if(smallscale == 0)
{
  ...
  /*------------------- (for rot-symmetry:strain[3]=eps_theta_theta) ---*/
...
/*----------------------------------------------------------------------*/
/*     if mat-law is called by submesh-element: strain is calculated    */
/*----------------------------------------------------------------------*/
else
{
  for (i=0; i<4; i++)
  {
     strain[i] = strain_tot[i];
  }
}
/*--- compute strains if it mat-law is not called by submesh-element ---*/
...
/*----------------------------------------------------------------------*/
if(smallscale == 0) /* mat-law is called by (macro) element */
{
  kappa  = ele->e.w1->elewa[0].ipwa[ip].kap;
}
#ifdef D_MLSTRUCT
else /* mat-law is called by submesh-element */
{
  ID    = actsmele->Id;
  kappa = ele->e.w1->sm_eledata[ID].sm_GPdata[ip].kappa;
}
#endif /* D_MLSTRUCT */
/*----------------------------------------------------------------------*/
...
/*----- rotational symmetry-andrea: do not change d[3][3] -------------*/
/*----------------------------- put sig to stress ---------------------*/
...
/*---------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
/*                  update for converged load-step                      */
/*----------------------------------------------------------------------*/
if(istore==1)
{
  if(smallscale == 0) /* mat-law is called by (macro) element */
...
/*----------------- put damage on GP for output -----------------------*/
  ...
  {
  ID    = actsmele->Id;
  ele->e.w1->sm_eledata[ID].sm_GPdata[ip].kappa = kappa;
  ele->e.w1->sm_eledata[ID].sm_GPdata[ip].damage = damage;
  for (i=0; i<4; i++)
     ele->e.w1->sm_eledata[ID].sm_GPdata[ip].stress[i] = sig[i];
  }
#endif /* D_MLSTRUCT */
}
