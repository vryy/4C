for (vi=0; vi<iel; ++vi)
{
  for (ui=0; ui<iel; ++ui)
  {
    ...
    estif_(vi*3, ui*3 + 2)     += ttimetauM*(conv_c_(vi) + conv_g_(vi))*derxy_(0, ui) ;
    estif_(vi*3 + 1, ui*3)     += ttimetauM*funct_(ui)*gradp_(1)*derxy_(0, vi) ;
    estif_(vi*3 + 1, ui*3 + 1) += ttimetauM*funct_(ui)*gradp_(1)*derxy_(1, vi) ;
    estif_(vi*3 + 1, ui*3 + 2) += ttimetauM*(conv_c_(vi) + conv_g_(vi))*derxy_(1, ui) ;
    ...
    estif_(vi*3 + 2, ui*3)     += ttimetauMp*(conv_c_(ui)*derxy_(0, vi) + conv_g_(ui)*derxy_(0, vi) + derxy_(0, vi)*conv_r_(0, 0, ui) + derxy_(1, vi)*conv_r_(1, 0, ui)) ;
    estif_(vi*3 + 2, ui*3 + 1) += ttimetauMp*(conv_c_(ui)*derxy_(1, vi) + conv_g_(ui)*derxy_(1, vi) + derxy_(0, vi)*conv_r_(0, 1, ui) + derxy_(1, vi)*conv_r_(1, 1, ui)) ;
