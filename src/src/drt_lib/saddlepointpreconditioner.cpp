...
void LINALG::SaddlePointPreconditioner::PG_AMG(const RCP<SparseMatrix>& A, const RCP<SparseMatrix>& P_tent, const RCP<SparseMatrix>& R_tent, RCP<SparseMatrix>& P_smoothed, RCP<SparseMatrix>& R_smoothed)
{
  ...
  RCP<Epetra_Vector> ColBasedOmegas = rcp(new Epetra_Vector(*colbasedomegamap,true));
  ...
  RCP<Epetra_Vector> RowBasedOmegas = rcp(new Epetra_Vector(DinvAP0->RowMap(),true));
  ...
  {
    if((*RowBasedOmegas)[i]==-666.0/*DBL_MIN*/)
    {
...
void LINALG::SaddlePointPreconditioner::MultiplySelfAll(const RCP<SparseMatrix>& Op,RCP<Epetra_Vector>& Column2Norm)
{
#ifdef DEBUG
  /*cout << Op->DomainMap().NumGlobalElements() << " " << Column2Norm->GlobalLength() << endl;
  if(Op->DomainMap().NumGlobalElements() != Column2Norm->GlobalLength())
    dserror("SaddlePointPreconditioner::MultiplySelfAll: global Number of columns doesnt match processor replicated length of Column2Norm vector.");*/
#endif
...
void LINALG::SaddlePointPreconditioner::MultiplyAll(const RCP<SparseMatrix>& left, const RCP<SparseMatrix>& right, RCP<Epetra_Vector>& InnerProd)
{
  if(!left->DomainMap().SameAs(right->DomainMap())) dserror("domain map of left and right does not match");
  if(!left->RowMap().SameAs(right->RowMap())) dserror("row map of left and right does not match");
