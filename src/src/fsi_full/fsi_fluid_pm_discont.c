  ...
  )
{
...

  {
    ...
    work->grad.numeq_total = work->numpdof*actfield->dis[disnum_calc].numele;
    work->grad.numeq       = work->numpdof*actpart->pdis[disnum_calc].numlele;
    amdef("update",&work->grad.update,work->grad.numeq,1,"IV");
    pm_fill_gradient_update(actpart, disnum_calc, actintra, work->numpdof,
    ...
    /* Note: We replace the default values here! The original setup
     * looks at the mesh. We have special requirements because our
     * dofs belong to the elements. */
    deconstruct_trilinos_matrix(global_press);
    global_press->numeq_total = work->grad.numeq_total;
    global_press->numeq       = work->grad.numeq;
  ...
  )
{
...

  {
    ...
    /* The first time this is called the pressure matrix is not yet
     * set. Afterwards we reuse the existing matrix. */
...
nonlniter:
  ...
  *action = calc_fluid;
   ...
   *----------------------------------------------------------------------*/
    ...
    /* update pressure */
    pm_press_update(actfield, actpart, disnum_calc, actintra, ipos, work->numpdof, work->press_sol, fdyn->thsl);
