...
void FPSI::Monolithic::Evaluate(Teuchos::RCP<const Epetra_Vector> x)
{
  ...
  if (linesearch_ and islinesearch_ == false)
  {
    ...
    porointerfacedisplacementsold_ = FPSICoupl()->iPorostructToAle(PoroField() -> StructureField() -> ExtractInterfaceDispnp(true));
  }
...
void FPSI::Monolithic::FPSIFDCheck()
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> sparse_copy = Teuchos::rcp(new LINALG::SparseMatrix(sparse->EpetraMatrix(), LINALG::Copy));
  ...
  Teuchos::RCP<LINALG::SparseMatrix> temp = Teuchos::rcp(new LINALG::SparseMatrix(stiff_approx, LINALG::Copy));
...
void FPSI::Monolithic::SetConductivity(double conduct)
{
  if (FPSICoupl() != Teuchos::null)
    FPSICoupl()->SetConductivity(conduct);
