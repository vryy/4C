...
void FPSI::FPSICoupling::SetupInterfaceCoupling()
{
...

  {
    ...
    mcs.AddSelector(Teuchos::rcp(new DRT::UTILS::NDimConditionSelector( *porofluiddis,"FPSICoupling",0,ndim+1)));
...

  {
    ...
    DRT::UTILS::MultiConditionSelector mcs;
    mcs.AddSelector(Teuchos::rcp(new DRT::UTILS::NDimConditionSelector(*fluiddis,"FPSICoupling",0,ndim+1)));
...

  {
    ...
    DRT::UTILS::MultiConditionSelector mcs;
    mcs.AddSelector(Teuchos::rcp(new DRT::UTILS::NDimConditionSelector(*fluiddis,"FPSICoupling",0,ndim)));
...

  {
    ...
    DRT::UTILS::MultiConditionSelector mcs;
    mcs.AddSelector(Teuchos::rcp(new DRT::UTILS::NDimConditionSelector(*fluiddis,"FSICoupling",0,ndim)));
    mcs.AddSelector(Teuchos::rcp(new DRT::UTILS::NDimConditionSelector(*fluiddis,"FPSICoupling",0,ndim)));
...

  {
    ...
    Teuchos::RCP<const Epetra_Map> s_other_map =
        LINALG::MergeMap(
    ...
    vecSpaces.push_back(PoroField()->StructureField()->Interface()->Map(STR::AUX::MapExtractor::cond_fpsi)); //FPSICoupling
  ...
  icoup_pf_f_->SetupConditionCoupling(
      *porofluiddis,
       porofluid_extractor_->CondMap(),
      *fluiddis,
       fluidvelpres_extractor_->CondMap(),
      "FPSICoupling",
       ndim+1,
       false);
...
// porous structure to fluid
  ...
  icoup_ps_f_->SetupConditionCoupling(
      *porostructdis,
       porostruct_extractor_->CondMap(),
       *fluiddis,
       fluidvel_extractor_->CondMap(),
      "FPSICoupling",
       ndim,
       false);
...
// porous structure to ale
  ...
  icoup_ps_a_->SetupConditionCoupling(
      *porostructdis,
       porostruct_extractor_->CondMap(),
...
void FPSI::FPSICoupling::EvaluateCouplingMatrixesRHS()
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> k_fp_porofluid = Teuchos::rcp(
      new LINALG::SparseMatrix(*(PoroField()->FluidField()->DofRowMap()), 81,
          true, true));
  Teuchos::RCP<LINALG::SparseMatrix> k_pf_porofluid = Teuchos::rcp(
      new LINALG::SparseMatrix(*(FluidField()->DofRowMap()), 81, true, true));
  ...
  INPAR::FPSI::PartitionedCouplingMethod method = DRT::INPUT::IntegralValue<
      INPAR::FPSI::PartitionedCouplingMethod>(fpsidynparams, "PARTITIONED");
  ...
  if (method != INPAR::FPSI::nocoupling)
  {
    ...
    PoroField()->FluidField()->Discretization()->SetState(0, "dispnp",
        PoroField()->FluidField()->Dispnp());

    PoroField()->FluidField()->Discretization()->SetState(0, "gridv",
        PoroField()->FluidField()->GridVel());
    PoroField()->FluidField()->Discretization()->SetState(0, "dispn",
        PoroField()->FluidField()->Dispn());
    PoroField()->FluidField()->Discretization()->SetState(0, "veln",
        PoroField()->FluidField()->Veln());
    PoroField()->FluidField()->Discretization()->SetState(0, "velaf",
        PoroField()->FluidField()->Velnp());
    PoroField()->FluidField()->Discretization()->SetState(0, "velnp",
        PoroField()->FluidField()->Velnp());
    ...
    if (method == INPAR::FPSI::monolithic)
    {
      ...
      if (probtype == prb_fps3i)
      {
        ...
        {
          dserror(
              "In the case of FPS3I, a positive conductivity must be set in DESIGN SCATRA COUPLING SURF CONDITIONS");
        }
      ...
      // temp matrix with porofluid rowmap
      Teuchos::RCP<LINALG::SparseMatrix> temp2 = Teuchos::rcp(
          new LINALG::SparseMatrix(*(PoroField()->FluidField()->DofRowMap()),81, false));

      {
        TEUCHOS_FUNC_TIME_MONITOR("FPSI::Monolithic::transform");
        (*couplingrowtransform_)(
            *Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(k_pf_porofluid),
            1.0, ADAPTER::CouplingSlaveConverter(couppff_fpsi), *temp2, false);
      }
      ...
      Teuchos::RCP<LINALG::SparseOperator> temp51 =
          Teuchos::rcp(new LINALG::SparseMatrix((*PoroField()->DofRowMap()), 81, false));
      {
        TEUCHOS_FUNC_TIME_MONITOR("FPSI::Monolithic::transform");
        (*couplingrowcoltransform2_)(
            *Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(k_pf_porofluid),
            1.0,
      ...
      Teuchos::RCP<LINALG::SparseMatrix> temp =
          Teuchos::rcp(new LINALG::SparseMatrix(*FluidField()->DofRowMap(), 81, false));
      {
        TEUCHOS_FUNC_TIME_MONITOR("FPSI::Monolithic::transform");
        (*couplingrowtransform2_)(
            *Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(k_fp_porofluid),
            1.0,
      ...
      Teuchos::RCP<LINALG::SparseMatrix> k_pfs_ = Teuchos::rcp(
          new LINALG::SparseMatrix(*(PoroField()->FluidField()->Discretization()->DofRowMap()),81,true, true));
      ...
      Teuchos::RCP<LINALG::SparseMatrix> temp3 = Teuchos::rcp(
          new LINALG::SparseMatrix(*FluidField()->DofRowMap(), 81, false));
      {
        TEUCHOS_FUNC_TIME_MONITOR("FPSI::Monolithic::transform");
        (*couplingrowtransform3_)(
      ...
      k_pf_porofluid->Complete(*FluidField()->DofRowMap(),
          *FluidField()->DofRowMap());
      Teuchos::RCP<LINALG::SparseMatrix> temp32 = Teuchos::rcp(
          new LINALG::SparseMatrix(*FluidField()->DofRowMap(), 81, false));
      ...
      k_pf_porofluid->Complete(*FluidField()->DofRowMap(),*FluidField()->DofRowMap());
      Teuchos::RCP<LINALG::SparseMatrix> temp4 = Teuchos::rcp(
          new LINALG::SparseMatrix(
              (*PoroField()->StructureField()->DofRowMap()), 81, false));
      {
        TEUCHOS_FUNC_TIME_MONITOR("FPSI::Monolithic::transform");
      ...
      Teuchos::RCP<LINALG::SparseOperator> temp5 = Teuchos::rcp(
          new LINALG::SparseMatrix((*PoroField()->DofRowMap()), 81, false));
      {
        TEUCHOS_FUNC_TIME_MONITOR("FPSI::Monolithic::transform");
        (*couplingrowcoltransform_)(
            *Teuchos::rcp_dynamic_cast<LINALG::SparseMatrix>(k_pf_porofluid),
            1.0,
            ADAPTER::CouplingSlaveConverter(coupsf_fpsi), // row converter: important to use slave converter
            ADAPTER::CouplingSlaveConverter(coupsf_fpsi), // col converter: important to use slave converter
      ...
      Teuchos::RCP<LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy> > temp6 =
          Teuchos::rcp(
              new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(
                  *AleField()->Interface(), *FluidField()->Interface(), 81,
                  false, false)); //Use FluidField()->Interface =
    ...
    temprhs2 = Teuchos::rcp(new Epetra_Vector(*PoroField()->Extractor()->FullMap() , true));
    ...
    //////////////////////////////////////
    if (method == INPAR::FPSI::monolithic
        or method == INPAR::FPSI::RobinNeumann)
    {
      fparams.set<std::string>("fillblock", "NeumannIntegration");
      fparams.set("InterfaceFacingElementMap", Fluid_PoroFluid_InterfaceMap_);

      Teuchos::RCP<LINALG::BlockSparseMatrixBase> tmp_c_ff =
      ...
      c_ff_->Add(*tmp_c_ff, false, 1.0, 1.0);
      {
        ...
        Teuchos::RCP<LINALG::BlockSparseMatrixBase> tmp_c_fa = Teuchos::rcp(
            new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(
        ...
        c_fa_->Add(tmp_c_fa->Matrix(FLD::UTILS::MapExtractor::cond_other,ALE::UTILS::MapExtractor::cond_other),false,1.0,0.0);
        ...
        tmp_c_fp.Add(tmp_c_fa->Matrix(FLD::UTILS::MapExtractor::cond_other,ALE::UTILS::MapExtractor::cond_fpsi),false,1.0,0.0);
        ...
        (*couplingcoltransform2_)(
            AleField()->BlockSystemMatrix()->FullRowMap(),
