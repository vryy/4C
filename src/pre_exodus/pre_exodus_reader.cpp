...
EXODUS::Mesh::Mesh(const std::string exofilename)
{
  ...
  // print version
  std::cout<<"File "<<exofilename<<" was created with EXODUS II library version "<<exoversion<<std::endl;
  ...
  // Get all ElementBlocks
  {
    ...
    for (int i = 0; i < num_elem_blk; ++i)
    {
      ...
      if (error != 0) dserror("exo error returned");
      Teuchos::RCP<std::map<int,std::vector<int> > > eleconn = Teuchos::rcp(new std::map<int,std::vector<int> >);
  ...
  title_(newtitle.c_str())
{
  ...
  Teuchos::RCP<std::map<int,std::vector<double> > >baseNodes = Teuchos::rcp(new std::map<int,std::vector<double> >);
...
//    PrintVec(std::cout,child);
    ...
    if ((child.size() != 3) && (child.size() != 4)){
      ...
      std::cout << ShapeToString(actshape) << ",Face: " << actface << ",childsize:" << child.size() << std::endl;
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::map<int,std::vector<int> > EXODUS::Mesh::GetSideSetConn(const SideSet sideset, bool checkoutside) const
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::vector<int> EXODUS::Mesh::OutsideOrientedSide(const std::vector<int> parentele, const std::vector<int> sidemap) const
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::vector<EXODUS::ElementBlock> EXODUS::Mesh::SideSetToEBlocks(const EXODUS::SideSet& sideset, const std::map<int,std::vector<int> >& sideconn) const
{
  ...
  Teuchos::RCP<std::map<int,std::vector<int> > > quadconn = Teuchos::rcp(new std::map<int,std::vector<int> >);
  int quadcounter = 0;
  Teuchos::RCP<std::map<int,std::vector<int> > > triconn = Teuchos::rcp(new std::map<int,std::vector<int> >);
  int tricounter = 0;
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
EXODUS::NodeSet EXODUS::Mesh::SideSetToNodeSet(const EXODUS::SideSet& sideset, const std::map<int,std::vector<int> >& sideconn) const
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::set<int> EXODUS::Mesh::GetSideSetNodes(const EXODUS::SideSet& sideset, const std::map<int,std::vector<int> >& sideconn) const
{
  std::map<int,std::vector<int> >::const_iterator i_side;
  std::vector<int>::const_iterator i_node;
  std::set<int> nodes;
...
/*------------------------------------------------------------------------*
 |creates gmsh-file to visualize mesh                             MF 07/08|
 *------------------------------------------------------------------------*/
void EXODUS::Mesh::PlotElementBlocksGmsh(const std::string fname,const EXODUS::Mesh& mymesh, const std::vector<int>& ebids) const
{
  Teuchos::RCP<std::map<int,std::vector<double> > > nodes= mymesh.GetNodes();
  std::ofstream f_system(fname.c_str());
  std::stringstream gmshfilecontent;
 ...
 *------------------------------------------------------------------------*/
void EXODUS::Mesh::PlotConnGmsh(const std::string fname,const EXODUS::Mesh& mymesh, const std::map<int,std::vector<int> >& conn) const
{
  Teuchos::RCP<std::map<int,std::vector<double> > > nodes= mymesh.GetNodes();
  std::ofstream f_system(fname.c_str());
  std::stringstream gmshfilecontent;
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
EXODUS::ElementBlock::ElementBlock(ElementBlock::Shape Distype, Teuchos::RCP<std::map<int,std::vector<int> > >& eleconn, std::string name)
: distype_(Distype),
  eleconn_(eleconn),
  name_(name.c_str())
{
  // do a sanity check
  for (std::map<int,std::vector<int> >::const_iterator elem=eleconn->begin(); elem!=eleconn->end(); ++elem)
  {
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
EXODUS::NodeSet::NodeSet(const std::set<int>& nodeids, const std::string& name, const std::string& propname)
: nodeids_(nodeids),
  name_(name.c_str()),
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void EXODUS::SideSet::FillSideLists(int* elemlist, int* sidelist, const std::map<int,std::vector<int> >& sides) const
{
  std::map<int,std::vector<int> >::const_iterator it;
  int i=0;
  for (it=sides.begin(); it != sides.end(); ++it){
...
EXODUS::Mesh EXODUS::QuadtoTri(EXODUS::Mesh& basemesh)
{
  ...
  for (i_ebs = ebs.begin(); i_ebs != ebs.end(); ++i_ebs ){
    ...
    if ((quadshape != EXODUS::ElementBlock::quad4) && (quadshape != EXODUS::ElementBlock::shell4)){
      ...
      std::cout << "Warning! Only quad4 or shell4 in quad->tri conversion. Skipping EBlock" << std::endl;
    } else {
