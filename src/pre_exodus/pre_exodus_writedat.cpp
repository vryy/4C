...
int EXODUS::WriteDatFile(const std::string& datfile, const EXODUS::Mesh& mymesh,
    const std::string& headfile, const std::vector<EXODUS::elem_def>& eledefs, const std::vector<EXODUS::cond_def>& condefs,
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void EXODUS::WriteDatConditions(const std::vector<EXODUS::cond_def>& condefs,const EXODUS::Mesh& mymesh, std::ostream& dat)
{
  ...
  for (unsigned int i=0; i<(*condlist).size(); ++i)
  {
    ...
    for (i_c=(count->second).begin();i_c!=(count->second).end();++i_c)
    {
      ...
      if((name!="")){
        ...
        dat << std::endl;
      } else if (pname!="none") dat << "// " << pname << std::endl;
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void EXODUS::WriteDatDesignTopology(const std::vector<EXODUS::cond_def>& condefs, const EXODUS::Mesh& mymesh, std::ostream& dat)
{
  ...
  for (it=dpoints.begin();it!=dpoints.end();++it){
    ...
    for(i=nodes.begin();i!=nodes.end();++i){
      dat << "NODE    " << *i << " " << "DNODE " << acte.e_id << std::endl;
  ...
  for (it=dlines.begin();it!=dlines.end();++it){
    ...
    std::set<int>::const_iterator i;
    for(i=nodes.begin();i!=nodes.end();++i){
      dat << "NODE    " << *i << " " << "DLINE " << acte.e_id << std::endl;
    }
  }
  ...
  for (it=dsurfs.begin();it!=dsurfs.end();++it){
    ...
    std::set<int>::const_iterator i;
    for(i=nodes.begin();i!=nodes.end();++i){
      dat << "NODE    " << *i << " " << "DSURFACE " << acte.e_id << std::endl;
    }
  }
  ...
  for (it=dvols.begin();it!=dvols.end();++it){
    ...
    std::set<int>::const_iterator i;
    for(i=nodes.begin();i!=nodes.end();++i){
      dat << "NODE    " << *i << " " << "DVOL " << acte.e_id << std::endl;
    }
  }
    ...
    const std::map<int,std::map<int,std::vector<std::vector<double> > > >& elecenterlineinfo)
{
  ...
  for(i_et=eledefs.begin();i_et!=eledefs.end();++i_et){
    ...
    else if (acte.sec.compare("CELLSCATRA")==0) cellscatra.push_back(acte);
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void EXODUS::DatEles(Teuchos::RCP< const EXODUS::ElementBlock> eb, const EXODUS::elem_def& acte, int& startele, std::ostream& datfile,
