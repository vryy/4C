...
void stru_genalpha_drt()
{
  ...
  acttime += dt;
  //----------------------------------------------------- predicting state
  // constant predictor displacement
  // using last converged displ. in domain + prescribed new dirichlet BCs
  ...
  dism->Update(1.-alphaf,*disn,alphaf,*dis,0.0);  // dis_{n+1-alpha_f}
  velm->Update(1.-alphaf,*veln,alphaf,*vel,0.0);  // vel_{n+1-alpha_f}
  accm->Update(1.-alpham,*accn,alpham,*acc,0.0);  // acc_{n+1-alpha_m}
  ...
  //debug: while (numiter<=sdyn->maxiter)
  {
