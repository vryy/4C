...
static void inherit_dirichlet_high_to_low_entity_elements(
                RefCountPtr<CCADISCRETIZATION::DesignDiscretization> highdis,
                RefCountPtr<CCADISCRETIZATION::DesignDiscretization> lowdis);
static void inherit_dirichlet_element_to_node(
                RefCountPtr<CCADISCRETIZATION::DesignDiscretization> dis);
                ...
                RefCountPtr<CCADISCRETIZATION::DesignDiscretization> dis)
{
  ...
  for (int i=0; i<dis->NumMyColElements(); ++i)
  {
    ...
    const CCADISCRETIZATION::Condition* highcond = ele->GetCondition("Dirichlet");
    if (!highcond) continue;
    
    // get number and ptrs to its nodes
    int nnode = ele->NumNode();
    CCADISCRETIZATION::Node** nodes = ele->Nodes();
    ...
    {
      const CCADISCRETIZATION::Condition* lowcond = nodes[j]->GetCondition("Dirichlet");
      if (lowcond) continue;
      RefCountPtr<CCADISCRETIZATION::Condition> newcond = 
                                    rcp(new CCADISCRETIZATION::Condition(*highcond));
