...
*-----------------------------------------------------------------------------------------*/
XFEM::XFFCoupling_Manager::XFFCoupling_Manager(
    Teuchos::RCP<ConditionManager> condmanager,
    Teuchos::RCP<FLD::XFluid> xfluid,
    Teuchos::RCP<FLD::XFluid> fluid,
    std::vector<int> idx)
 ...
 idx_(idx)
{
  ...
  //Coupling_Comm_Manager create all Coupling Objects now with Fluid has idx = 0, Fluid has idx = 1!
  mcffi_ = Teuchos::rcp_dynamic_cast<XFEM::MeshCouplingFluidFluid>(condmanager->GetMeshCoupling(cond_name_));
...
void XFEM::XFFCoupling_Manager::SetCouplingStates()
{
  ...
  Teuchos::RCP<Epetra_Vector> tmp_diff = Teuchos::rcp(new Epetra_Vector((*mcffi_->IDispnp()).Map()));
...
*-----------------------------------------------------------------------------------------*/
void XFEM::XFFCoupling_Manager::AddCouplingMatrix(LINALG::BlockSparseMatrixBase& systemmatrix, double scaling)
{
  /*----------------------------------------------------------------------*/
...
*-----------------------------------------------------------------------------------------*/
void XFEM::XFFCoupling_Manager::AddCouplingRHS(Teuchos::RCP<Epetra_Vector> rhs,
    const LINALG::MultiMapExtractor& me, double scaling)
{
  //REMARK: Copy this vector to store the correct lambda_ in update!
  Teuchos::RCP<Epetra_Vector> coup_rhs_sum = Teuchos::rcp(new Epetra_Vector(*xfluid_->RHS_s_Vec(cond_name_)));
