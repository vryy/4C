...
/*----------------------------------------------------------------------*/
void DRT::ELEMENTS::Wall1::GetCauchyAtXi(const LINALG::Matrix<2,1>& xi,
        const std::vector<double>& disp,
        ...
        Epetra_SerialDenseMatrix& dsdd)
{
  ...
  const int nd      = numnode*numdf;
  if ((int)disp.size()!=nd)
    dserror("displacement vector size inconsistency");

  Epetra_SerialDenseMatrix xrefe(numnode,numdf);
  Epetra_SerialDenseMatrix xcurr(numnode,numdf);
  ...
  for (int k=0; k<numnode; ++k)
    for (int d=0;d<2;++d)
  {
    xrefe(k,d) = Nodes()[k]->X()[d];
    xcurr(k,d) = Nodes()[k]->X()[d] + disp[k*numdf+d];
  }

  Epetra_SerialDenseMatrix bop(3,nd);
  Epetra_SerialDenseMatrix deriv(2,numnode);
  Epetra_SerialDenseMatrix N_XY(2,numnode);

  DRT::UTILS::shape_function_2D_deriv1(deriv,xi(0),xi(1),distype_);

  Epetra_SerialDenseMatrix iJe(2,2);
  LINALG::Matrix<2,2> invJ(iJe.A(),true);
  deriv.Multiply(false,xrefe,iJe);
  invJ.Invert();

  iJe.Multiply(false,deriv,N_XY);

  Epetra_SerialDenseMatrix ft(2,2);
  N_XY.Multiply(false,xcurr,ft);

  LINALG::Matrix<3,3> defgrd3(true);
  for (int i=0;i<2;++i)
    for (int j=0;j<2;++j)
      defgrd3(i,j)=ft(j,i);
  defgrd3(2,2)=1.;
  LINALG::Matrix<3,3> b;
  b.MultiplyNT(defgrd3,defgrd3);

  LINALG::Matrix<3,3> cauchy3;
  LINALG::Matrix<6,6> cmat3;
  SolidMaterial()->EvaluateCauchy(b,cauchy3,cmat3,Id());
  for (int i=0;i<2;++i) cauchy(i,i)=cauchy3(i,i);
  cauchy(0,1)=cauchy(1,0)=cauchy3(0,1);

  dsdd.Reshape(3,nd);
  dsdd.Scale(0.);

  if (wtype_ != plane_strain)
    dserror("only plain strain yet");

  Epetra_SerialDenseMatrix cmat(3,3);
  for (int i=0;i<2;++i)
    for (int j=0;j<2;++j)
      cmat(i,j)=cmat3(i,j);
  cmat(2,2)=cmat3(3,3);
  cmat(0,2)=cmat(2,0)=cmat3(0,3);
  cmat(1,2)=cmat(2,1)=cmat3(1,3);

  for (int i=0;i<numnode;++i)
  {
    for (int x=0;x<2;++x)
      for (int g=0;g<2;++g)
        bop(x,numdf*i+x) += defgrd3(x,g)*N_XY(g,i);
    for (int g=0;g<2;++g)
    {
      bop(2,numdf*i+0) += defgrd3(1,g)*N_XY(g,i);
      bop(2,numdf*i+1) += defgrd3(0,g)*N_XY(g,i);
    }
  }
  bop.Scale(2.);
  cmat.Multiply(false,bop,dsdd);
}
