...
{
  if ( not Filled() )
    dserror("The matrix must be filled!");
  ...
  for ( int i=0; i<numdbcrows; ++i )
  {
    ...
    for ( int c=0; c<NumEntries; ++c )
      gIndices[c] = sysmat_->ColMap().GID( Indices[c] );

    // handle a diagonal block
    if ( diagonalblock )
    {
      if ( NumEntries == 0 )
        dserror("Row %d is empty and part of a diagonal block!", row);

      if ( trafo )
      {
        if ( not trafo->Filled() )
          dserror("The trafo matrix must be filled!");
        ...
        trafo->EpetraMatrix()->ExtractMyRowView( trafo_rlid, tNumEntries,
            tValues, tIndices );

        // get the global indices corresponding to the extracted local indices
        std::fill( gtIndices.begin(), gtIndices.end(), 0.0 );
        for ( int c=0; c<tNumEntries; ++c )
          gtIndices[c] = trafo->ColMap().GID( tIndices[c] );

        for ( int j=0; j<tNumEntries; ++j )
        {
          int k=-1;
          while ( ++k<NumEntries )
            if ( Indices[k]==tIndices[j] )
              break;

          if ( k==NumEntries )
            dserror("Couldn't find column index %d in row %d.",
                tIndices[j], row );
      ...
      else
      {
        ...
        {
          if ( gIndices[j]!=row and
               Values[j] > std::numeric_limits<double>::epsilon() )
          {
            isdbc = false;
            break;
          }
