      ...
      boundarynode2_(std::make_pair(0, 0))
{
  ...
  int smoothing =
      DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::Smoothing>(bcparams_, "BEAMS_SMOOTHING");
  ...
  const DRT::ELEMENTS::Beam3Base* ele1ptr =
      dynamic_cast<const DRT::ELEMENTS::Beam3Base*>(element1_);
  double l1 = ele1ptr->RefLength();
  const DRT::ELEMENTS::Beam3Base* ele2ptr =
      dynamic_cast<const DRT::ELEMENTS::Beam3Base*>(element2_);
...
#ifdef FDCHECK
  if (fdcheck == false) FDCheck(stiffmatrix, fint, pp, contactpairmap, timeintparams, fdcheck);
#endif
    ...
    const double pp)
{
  ...
  for (iter = closelargeanglesegments.begin(); iter != closelargeanglesegments.end(); ++iter)
  {
    ...
    for (int i = 0; i < (int)cpvariables_.size(); i++)
    {
      ...
      if (fabs(eta1_eval - FADUTILS::CastToDouble(closestpoint.first)) <
              XIETARESOLUTIONFAC * XIETAITERATIVEDISPTOL and
          fabs(eta2_eval - FADUTILS::CastToDouble(closestpoint.second)) <
    ...
    LINALG::SparseMatrix& stiffmatrix, Epetra_Vector& fint)
{
  for (int numcp = 0; numcp < (int)cpvariables_.size(); numcp++)
  {
    ...
    EvaluateFcContact(&fint, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi,
...
void CONTACT::Beam3contact<numnodes, numnodalvalues>::EvaluateActiveSmallAnglePairs(
    LINALG::SparseMatrix& stiffmatrix, Epetra_Vector& fint, std::pair<int, int>* iminmax,
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3contact<numnodes, numnodalvalues>::CalcPenaltyLaw(
            ...
            fabs(alpha_g_min) >= perpshiftangle1)
        {
          ...
          if (eot == DRT::ELEMENTS::Beam3ebType::Instance())
          {
            const DRT::ELEMENTS::Beam3eb* beam3ebelement1 =
                dynamic_cast<const DRT::ELEMENTS::Beam3eb*>(element1);
            double kappamax1 = beam3ebelement1->GetKappaMax();
            const DRT::ELEMENTS::Beam3eb* beam3ebelement2 =
                dynamic_cast<const DRT::ELEMENTS::Beam3eb*>(element2);
    ...
    Teuchos::RCP<Beam3contactvariables<numnodes, numnodalvalues>> variables, int contacttype)
{
  ...
  if (FADUTILS::CastToDouble(gap) < -MAXPENETRATIONSAFETYFAC * (R1_ + R2_) and numstep_ > 0)
  {
    ...
    // std::cout << "angle: " << FADUTILS::CastToDouble(BEAMCONTACT::CalcAngle(r1_xi,r2_xi) <<
    // std::endl;
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
std::vector<int> CONTACT::Beam3contact<numnodes, numnodalvalues>::GetGlobalDofs(
    const DRT::Node* node)
{
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3contact<numnodes, numnodalvalues>::SetClassVariables(
    Teuchos::ParameterList& timeintparams)
{
  iter_ = timeintparams.get<int>("iter", -10);
  numstep_ = timeintparams.get<int>("numstep", -10);

  if (iter_ == -10.0 or numstep_ == -10.0) dserror("Invalid time integration parameter!");
  ...
  if (kappa_max < 0) dserror("Maximal curvature should be a positive value!");
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3contact<numnodes, numnodalvalues>::UpdateElePos(
    ...
    Teuchos::ParameterList& timeintparams, bool fdcheck)
{
  ...
  LINALG::Matrix<2 * 3 * numnodes * numnodalvalues, 2 * 3 * numnodes * numnodalvalues>
      stiffmatrix_fd(true);

  LINALG::Matrix<2 * 3 * numnodes * numnodalvalues, 2 * 3 * numnodes * numnodalvalues> fint2_mat(
      true);
