...
#include "beamcontact_beam3tobeampotential.H"
...
template <const int numnodes, const int numnodalvalues>
CONTACT::Beam3tobeampotential<numnodes, numnodalvalues>::Beam3tobeampotential(
    const Beam3tobeampotential& old)
    ...
    const std::vector<DRT::Condition*> linechargeconds, const double k, const double m)
{
  ...
  // (Langbein) should be chosen depending on potential law
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3tobeampotential<numnodes,
    numnodalvalues>::EvaluateFpotandStiffpot_LargeSepApprox()
{
  ...
  std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>> N2_i(numgp);     // = N2_i
  std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>> N1_i_xi(numgp);  // = N1_i,xi
  std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>> N2_i_xi(numgp);  // = N2_i,eta
  ...
  if (linechargeconds_[0]->GetInt("funct") != -1 or linechargeconds_[1]->GetInt("funct") != -1)
    dserror(
        "DLINE beam potential charge condition: No functions allowed yet! Set 'funct' to '-1' -> "
        "off");
  ...
  for (int gp1 = 0; gp1 < numgp; ++gp1)
  {
    ...
    for (int gp2 = 0; gp2 < numgp; ++gp2)
    {
      ...
      double q1q2_JacFac_GaussWeights = q1 * q2 * jacobifac1 *
                                        element2_->GetJacobiFacAtXi(gausspoints.qxg[gp2][0]) *
          ...
          for (int idim = 0; idim < 3; ++idim)
          {
            ...
            for (int jdim = 0; jdim < 3; ++jdim)
            {
              stiffpot1_(3 * i + idim, 3 * (numnodes * numnodalvalues + j) + jdim) -=
                  norm_dist_exp2 * N1_i[gp1](i) * dist_dist_T(idim, jdim) * N2_i[gp2](j) *
                  q1q2_JacFac_GaussWeights;
            }
          }
        }
      ...
      for (int i = 0; i < (numnodes * numnodalvalues); ++i)
      {
        ...
        for (int j = 0; j < (numnodes * numnodalvalues); ++j)
        {
          for (int idim = 0; idim < 3; ++idim)
          {
            ...
            for (int jdim = 0; jdim < 3; ++jdim)
            {
              stiffpot2_(3 * i + idim, 3 * j + jdim) -= norm_dist_exp2 * N2_i[gp2](i) *
                                                        dist_dist_T(idim, jdim) * N1_i[gp1](j) *
                                                        q1q2_JacFac_GaussWeights;
            }
          }
          ...
          for (int idim = 0; idim < 3; ++idim)
          {
            ...
            for (int jdim = 0; jdim < 3; ++jdim)
            {
              stiffpot2_(3 * i + idim, 3 * (numnodes * numnodalvalues + j) + jdim) +=
                  norm_dist_exp2 * N2_i[gp2](i) * dist_dist_T(idim, jdim) * N2_i[gp2](j) *
                  q1q2_JacFac_GaussWeights;
            }
          }
        }
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3tobeampotential<numnodes, numnodalvalues>::AssembleStiffpot(
    CORE::LINALG::SparseMatrix& stiffmatrix)
{
  ...
  {
    for (int i = 0; i < dim1; i++)
      stiffpot1_copy(i, j) = CORE::FADUTILS::CastToDouble(stiffpot1_(i, j));
    for (int i = 0; i < dim2; i++)
      stiffpot2_copy(i, j) = CORE::FADUTILS::CastToDouble(stiffpot2_(i, j));
  }
#else  // automatic differentiation via FAD for debugging
  for (int j = 0; j < dim1 + dim2; j++)
  {
    for (int i = 0; i < dim1; i++)
      stiffpot1_copy(i, j) = CORE::FADUTILS::CastToDouble(fpot1_(i).dx(j));
    for (int i = 0; i < dim2; i++)
      stiffpot2_copy(i, j) = CORE::FADUTILS::CastToDouble(fpot2_(i).dx(j));
  }
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3tobeampotential<numnodes, numnodalvalues>::GetShapeFunctions(
    std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>>& N1_i,
    std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>>& N2_i,
    std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>>& N1_i_xi,
    std::vector<CORE::LINALG::Matrix<1, numnodes * numnodalvalues>>& N2_i_xi,
    CORE::DRT::UTILS::IntegrationPoints1D& gausspoints)
{
  ...
  if (numnodalvalues == 1)
  {
    for (int gp = 0; gp < gausspoints.nquad; ++gp)
    {
      ...
      CORE::DRT::UTILS::shape_function_1D(N2_i[gp], gausspoints.qxg[gp][0], distype2);
      CORE::DRT::UTILS::shape_function_1D_deriv1(N1_i_xi[gp], gausspoints.qxg[gp][0], distype1);
      CORE::DRT::UTILS::shape_function_1D_deriv1(N2_i_xi[gp], gausspoints.qxg[gp][0], distype2);
    }
  }
  else if (numnodalvalues == 2)
  {
    for (int gp = 0; gp < gausspoints.nquad; ++gp)
    {
      ...
      CORE::DRT::UTILS::shape_function_hermite_1D(
          N2_i[gp], gausspoints.qxg[gp][0], ele2length_, distype2);
      CORE::DRT::UTILS::shape_function_hermite_1D_deriv1(
          N1_i_xi[gp], gausspoints.qxg[gp][0], ele1length_, distype1);
      CORE::DRT::UTILS::shape_function_hermite_1D_deriv1(
          N2_i_xi[gp], gausspoints.qxg[gp][0], ele2length_, distype2);
    }
  }
  else
    dserror(
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3tobeampotential<numnodes, numnodalvalues>::ComputeCoords(
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
std::vector<int> CONTACT::Beam3tobeampotential<numnodes, numnodalvalues>::GetGlobalDofs(
    const DRT::Node* node)
{
 ...
 *----------------------------------------------------------------------*/
template <const int numnodes, const int numnodalvalues>
void CONTACT::Beam3tobeampotential<numnodes, numnodalvalues>::UpdateElePos(
...
Teuchos::RCP<CONTACT::Beam3tobeampotentialinterface> CONTACT::Beam3tobeampotentialinterface::Impl(
  ...
  switch (numnodalvalues)
  {
    case 1:
    {
      switch (numnodes)
      {
        case 2:
        {
          return Teuchos::rcp(new CONTACT::Beam3tobeampotential<2, 1>(
              pdiscret, cdiscret, dofoffsetmap, element1, element2, beampotparams));
        }
        case 3:
        {
          return Teuchos::rcp(new CONTACT::Beam3tobeampotential<3, 1>(
              pdiscret, cdiscret, dofoffsetmap, element1, element2, beampotparams));
        }
        case 4:
        {
          return Teuchos::rcp(new CONTACT::Beam3tobeampotential<4, 1>(
              pdiscret, cdiscret, dofoffsetmap, element1, element2, beampotparams));
        }
        case 5:
        {
          return Teuchos::rcp(new CONTACT::Beam3tobeampotential<5, 1>(
              pdiscret, cdiscret, dofoffsetmap, element1, element2, beampotparams));
        }
        default:
          dserror(
    ...
    case 2:
    {
      switch (numnodes)
      {
        case 2:
        {
          return Teuchos::rcp(new CONTACT::Beam3tobeampotential<2, 2>(
              pdiscret, cdiscret, dofoffsetmap, element1, element2, beampotparams));
        }
        default:
          dserror(
