/*======================================================================*/
/*!
\file
\brief Integration parameters for THERM2 element

<pre>
Maintainer: Burkhard Bornemann
            bornemann@lnm.mw.tum.de
            http://www.lnm.mw.tum.de/Members/bornemann
            089-289-15237
</pre>

\author bborn
\date 03/06
*/
/*======================================================================*/
/*!
\addtogroup THERM2
*//*! @{ (documentation module open)*/


/*----------------------------------------------------------------------*/
/* limit to D_THERM2 */
#ifdef D_THERM2


/*----------------------------------------------------------------------*/
/* header files */
#include "../headers/standardtypes.h"
#include "therm2.h"


/*----------------------------------------------------------------------*/
/* constants */
/* static DOUBLE Q12 = ONE/TWO; */
/* static DOUBLE Q13 = ONE/THREE; */
/* static DOUBLE Q16 = ONE/SIX; */
/* static DOUBLE Q23 = TWO/THREE; */


/*----------------------------------------------------------------------*/
/*!
\brief general problem data

global variable GENPROB genprob is defined in global_control.c

\author bborn
\date 03/06
*/
/*----------------------------------------------------------------------*/
extern struct _GENPROB     genprob;


/*----------------------------------------------------------------------*/
/*!
\brief Pointer to allocate dynamic variables if needed

defined in global_control.c

\author bborn
\date 03/06
*/
/*----------------------------------------------------------------------*/
extern ALLDYNA      *alldyn;


/*----------------------------------------------------------------------*/
/*!
\brief Set type, total amount of Gauss points, etc of element

\param ELEMENT    *ele      (i)   pointer to element
\parem INT         ierr     (o)   error flag: 1 ==> success 

\author bborn
\date 03/06
*/
/*----------------------------------------------------------------------*/
void th2_intg_eleinp(ELEMENT *actele, 
                     INT *ierr)
{

  INT i;
  INT actGP[2];
  THERM2 *actth2;

#ifdef DEBUG
  dstrc_enter("th2_intgeleinp");
#endif

  /*--------------------------------------------------------------------*/
  /* init error flag */
  *ierr = 1;  /* start without an error */

  /*--------------------------------------------------------------------*/
  /* get local copies / pointers */
  actth2 = actele->e.th2;
  for (i=0; i<2; i++)
  {
    actGP[i] = actth2->nGP[i];
  }

  /*--------------------------------------------------------------------*/
  /* set Gauss number type and total Gauss points */
  switch (actele->distyp)
  {
    /*------------------------------------------------------------------*/
    /* --> quad - element */
    case quad4: case quad8: case quad9:
      /* total number */
      actth2->gptotal = actGP[0] * actGP[1];
      /* type */
      if (actGP[0] == actGP[1])
      {
        actth2->gpintc = actGP[0] - 1;
        actth2->gpned = -1;  /* Gauss points on edge */
      }
      else
      {
        actth2->gpintc = -1;
        actth2->gpned = -1;  /* trouble */
      }
      break;
    /*------------------------------------------------------------------*/
    /* --> tri - element */
    case tri3: case tri6:
      /* total number */
      actth2->gptotal = actth2->nGP[0];
      /* type */
      if (actGP[0] == 1)
      {
        actth2->gpintc = 0;
        actth2->gpned = 1;
      }
      else if ((actGP[0] == 3) && (actGP[1] == 1))
      {
        actth2->gpintc = 1;
        actth2->gpned = 2;
      }
      else if ((actGP[0] == 3) && (actGP[1] == 2))
      {
        actth2->gpintc = 2;
        actth2->gpned = 2;
      }
      else if (actGP[0] == 4)
      {
        actth2->gpintc = 3;
        actth2->gpned = 2;
      }
      else if ((actGP[0] == 6) && (actGP[1] == 1))
      {
        actth2->gpintc = 4;
        actth2->gpned = 3;
      }
      else if ((actGP[0] == 6) && (actGP[1] == 2))
      {
        actth2->gpintc = 5;
        actth2->gpned = 3;
      }
      else if ((actGP[0] == 7) && (actGP[1] == 1))
      {
        actth2->gpintc = 6;
        actth2->gpned = 3;
      }
      else if ((actGP[0] == 7) && (actGP[1] == 2))
      {
        actth2->gpintc = 7;
        actth2->gpned = 3;
      }
      else if (actGP[0] == 9)
      {
        actth2->gpintc = 8;
        actth2->gpned = 4;
      }
      else if (actGP[0] == 12)
      {
        actth2->gpintc = 9;
        actth2->gpned = 4;
      }
      else if (actGP[0] == 13)
      {
        actth2->gpintc = 10;
        actth2->gpned = 5;
      }
      else
      {
        *ierr = 0;  /* error !!! */
      }
      break;
    /*------------------------------------------------------------------*/
    /* catch erroneous disc. types */
    default:
      dserror("Discretisation type is impossible!");
      break;
  }  /* end of switch (actele->distyp) */

  /*--------------------------------------------------------------------*/
  /* finish off */
#ifdef DEBUG
  dstrc_exit();
#endif

  return;
}  /* end of th2_intg_eleinp(...)  */


/*----------------------------------------------------------------------*/
/*!
\brief integration parameters for fluid2 element

this routine is a try to organise the integration parameters
different. ALL paramters are stored in FLUID_DATA, so that this
routine has to be (hopefully) called only once!!!

\param  *data   TH2_DATA     (o)  prepare quadrature data 
\return void

\author bborn
\date 03/06
*/
/*----------------------------------------------------------------------*/
void th2_intg_init(TH2_DATA *data)

{
  INT i, k, intc;  /* counters */

  DOUBLE zero = 0.0;
  DOUBLE one = 1.0;
  DOUBLE two = 2.0;
  DOUBLE three = 3.0;

  DOUBLE q12 = one/two;
  DOUBLE q13 = one/three;
  DOUBLE q16 = q13/two;
  DOUBLE q23 = two/three;

  /* HINT: GLMAXP_THERM2==6, GLINTC_THERM2==6 */
  DOUBLE  xg[GLMAXP_THERM2][GLINTC_THERM2];  /* coord. of Gauss points 
                                              * on [-1,+1] */
  DOUBLE  wgt[GLMAXP_THERM2][GLINTC_THERM2];  /* integr. weights of
                                              * Gauss points on [-1,+1] */

  /* HINT: GTMAXP_THERM2==13, GTINTC_THERM2==11 */
  DOUBLE  xgr[GTMAXP_THERM2][GTINTC_THERM2];  /* r-coord. of Gauss points
                                               * on triangle domain */
  DOUBLE  xgs[GTMAXP_THERM2][GTINTC_THERM2];  /* s-coord. of Gauss points
                                               * on triangle domain */
  DOUBLE  wgtt[GTMAXP_THERM2][GTINTC_THERM2];  /* weights of Gauss points
                                                * on triangle domain */


#ifdef DEBUG
  dstrc_enter("th2_intg");
#endif

  /*--------------------------------------------------------------------*/
  /* initialize arrays */
  /* for QUADs */
  for (i=0; i<GLMAXP_THERM2; i++)
  {
    for (k=0; k<GLINTC_THERM2; k++)
    {
       xg[i][k] = zero;
      wgt[i][k] = zero;
    }
  }
  /* for TRIs */
  for (i=0; i<GTMAXP_THERM2; i++)
  {
    for (k=0; k<GTINTC_THERM2; k++)
    {
       xgr[i][k] = zero;
       xgs[i][k] = zero;
      wgtt[i][k] = zero;
    }
  }

/*----------------------------------------------------------------------*
 |     INTEGRATION PARAMETERS FOR    R E C T A N G U L A R   ELEMENTS   |
 |     GAUSS SAMPLING POINTS  AT     R/S-COORDINATES     RESPECTIVELY   |
 |                            AND    CORRESPONDING WEIGHTING  FACTORS   |
 |     xg[i][j]                                                         |
 |    wgt[i][j]:  i+1 - actual number of gausspoint                     |
 |                j+1 - total number of gausspoints                     |
 *----------------------------------------------------------------------*/
/* ccordinates for one gauss point */
  xg[0][0]  =  zero            ;  /* identically initialised */
/* coordinates for two gauss points */
  xg[0][1]  =  -0.5773502691896;  /* -1/sqrt(3) */
  xg[1][1]  =  -xg[0][1]       ;
/* coordinates for three gauss points */
  xg[0][2]  =  -0.7745966692415;  /* -sqrt(3/5) */
  xg[1][2]  =  zero            ;  /* unnecessarily: identically initialised */
  xg[2][2]  =  -xg[0][2]       ;
/* coordinates for four gauss points */
  xg[0][3]  =  -0.8611363115941;  /* -sqrt((15+sqrt(120))/35) */
  xg[1][3]  =  -0.3399810435849;  /* -sqrt((15-sqrt(120))/35) */
  xg[2][3]  =  -xg[1][3]       ;
  xg[3][3]  =  -xg[0][3]       ;
/* coordinates for five gauss points */
  xg[0][4]  =  -0.9061798459387;
  xg[1][4]  =  -0.5384693101057;
  xg[2][4]  =  zero            ;  /* identically initialised */
  xg[3][4]  =  -xg[1][4]       ;
  xg[4][4]  =  -xg[0][4]       ;
/* coordinates for six gauss points */
  xg[0][5]  =  -0.9324695142032;
  xg[1][5]  =  -0.6612093864663;
  xg[2][5]  =  -0.2386191860832;
  xg[3][5]  =  -xg[2][5]       ;
  xg[4][5]  =  -xg[1][5]       ;
  xg[5][5]  =  -xg[0][5]       ;

/* weights for one gauss points */
  wgt[0][0] =  two             ;
/* weights for two gauss points */
  wgt[0][1] =  one             ;
  wgt[1][1] =  one             ;
/* weights for three gauss points */
  wgt[0][2] =  0.5555555555556 ;  /* 5/9 */
  wgt[1][2] =  0.8888888888889 ;  /* 8/9 */
  wgt[2][2] =  wgt[0][2]       ;
/* weights for four gauss points */
  wgt[0][3] =  0.3478548451375 ;  /* (18-sqrt(30))/36 */
  wgt[1][3] =  0.6521451548625 ;  /* (18+sqrt(30))/36 */
  wgt[2][3] =  wgt[1][3]       ;
  wgt[3][3] =  wgt[0][3]       ;
/* weights for five gauss points */
  wgt[0][4] =  0.2369268850562 ;
  wgt[1][4] =  0.4786286704994 ;
  wgt[2][4] =  0.5688888888889 ;
  wgt[3][4] =  wgt[1][4]       ;
  wgt[4][4] =  wgt[0][4]       ;
/* weights for six gauss points */
  wgt[0][5] =  0.1713244923792 ;
  wgt[1][5] =  0.3607615730481 ;
  wgt[2][5] =  0.4679139345727 ;
  wgt[3][5] =  wgt[2][5]       ;
  wgt[4][5] =  wgt[1][5]       ;
  wgt[5][5] =  wgt[0][5]       ;

/*----------------------------------------------------------------------*
 |     INTEGRATION PARAMETERS FOR    T R I A N G U L A R     ELEMENTS   |
 |     GAUSS SAMPLING POINTS  AT     R/S-COORDINATES     RESPECTIVELY   |
 |                            AND    CORRESPONDING WEIGHTING  FACTORS   |
 |     xgr[i][j]                                                        |
 |    wgts[i][j]:  i+1 - actual number of gausspoint                    |
 |                 j+1 - number for integration case (from input)       |
 *----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION         1 SAMPLING POINT, DEG.OF PRECISION 1    |
 |                              CASE 0                                  |
 *----------------------------------------------------------------------*/
  xgr[0][0]    =  q13 ;
  xgs[0][0]    =  q13 ;
  wgtt[0][0]   =  q12 ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION        3 SAMPLING POINTS, DEG.OF PRECISION 2    |
 *----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*
 |                             CASE 1                                   |
 *----------------------------------------------------------------------*/
  xgr[0][1]    =  q12  ;
  xgr[1][1]    =  q12  ;
  xgr[2][1]    =  zero ;
  xgs[0][1]    =  zero ;
  xgs[1][1]    =  q12  ;
  xgs[2][1]    =  q12  ;
  wgtt[0][1]   =  q16  ;
  wgtt[1][1]   =  q16  ;
  wgtt[2][1]   =  q16  ;
/*----------------------------------------------------------------------*
 |                             CASE 2                                   |
 *----------------------------------------------------------------------*/
  xgr[0][2]    =  q16  ;
  xgr[1][2]    =  q23  ;
  xgr[2][2]    =  q16  ;
  xgs[0][2]    =  q16  ;
  xgs[1][2]    =  q16  ;
  xgs[2][2]    =  q23  ;
  wgtt[0][2]   =  q16  ;
  wgtt[1][2]   =  q16  ;
  wgtt[2][2]   =  q16  ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION        4 SAMPLING POINTS, DEG.OF PRECISION 3    |
 |                             CASE 3                                   |
 *----------------------------------------------------------------------*/
  xgr[0][3]    =  0.2                ;
  xgr[1][3]    =  0.6                ;
  xgr[2][3]    =  0.2                ;
  xgr[3][3]    =  q13                ;
  xgs[0][3]    =  0.2                ;
  xgs[1][3]    =  0.2                ;
  xgs[2][3]    =  0.6                ;
  xgs[3][3]    =  q13                ;
  wgtt[0][3]   =  0.2604166666667    ;
  wgtt[1][3]   =  wgtt[0][3]         ;
  wgtt[2][3]   =  wgtt[0][3]         ;
  wgtt[3][3]   = -0.28125            ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION        6 SAMPLING POINTS, DEG.OF PRECISION 4    |
  *----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*
 |                             CASE 4                                   |
 *----------------------------------------------------------------------*/
  xgr[0][4]    =  0.0915762135098   ;
  xgr[1][4]    =  0.8168475729805   ;
  xgr[2][4]    =  xgr[0][4]         ;
  xgr[3][4]    =  0.4459484909160   ;
  xgr[4][4]    =  xgr[3][4]         ;
  xgr[5][4]    =  0.1081030181681   ;
  xgs[0][4]    =  xgr[0][4]         ;
  xgs[1][4]    =  xgr[0][4]         ;
  xgs[2][4]    =  xgr[1][4]         ;
  xgs[3][4]    =  xgr[5][4]         ;
  xgs[4][4]    =  xgr[3][4]         ;
  xgs[5][4]    =  xgr[3][4]         ;
  wgtt[0][4]   =  0.0549758718277   ;
  wgtt[1][4]   =  wgtt[0][4]        ;
  wgtt[2][4]   =  wgtt[0][4]        ;
  wgtt[3][4]   =  0.1116907948390   ;
  wgtt[4][4]   =  wgtt[3][4]        ;
  wgtt[5][4]   =  wgtt[3][4]        ;
/*----------------------------------------------------------------------*
 |                             CASE 5                                   |
 *----------------------------------------------------------------------*/
  xgr[0][5]    =  0.1090390090729   ;
  xgr[1][5]    =  0.2319333685530   ;
  xgr[2][5]    =  0.6590276223741   ;
  xgr[3][5]    =  xgr[2][5]         ;
  xgr[4][5]    =  xgr[1][5]         ;
  xgr[5][5]    =  xgr[0][5]         ;
  xgs[0][5]    =  xgr[1][5]         ;
  xgs[1][5]    =  xgr[0][5]         ;
  xgs[2][5]    =  xgr[0][5]         ;
  xgs[3][5]    =  xgr[1][5]         ;
  xgs[4][5]    =  xgr[2][5]         ;
  xgs[5][5]    =  xgr[2][5]         ;
  wgtt[0][5]   =  0.0833333333333   ;
  wgtt[1][5]   =  wgtt[0][5]        ;
  wgtt[2][5]   =  wgtt[0][5]        ;
  wgtt[3][5]   =  wgtt[0][5]        ;
  wgtt[4][5]   =  wgtt[0][5]        ;
  wgtt[5][5]   =  wgtt[0][5]        ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION        7 SAMPLING POINTS, DEG.OF PRECISION 5    |
 *----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*
 |                             CASE 6                                   |
 *----------------------------------------------------------------------*/
  xgr[0][6]    =  0.1012865073235 ;
  xgr[1][6]    =  0.4701420641051 ;
  xgr[2][6]    =  0.7974269853531 ;
  xgr[3][6]    =  xgr[1][6]       ;
  xgr[4][6]    =  xgr[0][6]       ;
  xgr[5][6]    =  0.0597158717898 ;
  xgr[6][6]    =  q13             ;
  xgs[0][6]    =  xgr[0][6]       ;
  xgs[1][6]    =  xgr[5][6]       ;
  xgs[2][6]    =  xgr[0][6]       ;
  xgs[3][6]    =  xgr[1][6]       ;
  xgs[4][6]    =  xgr[2][6]       ;
  xgs[5][6]    =  xgr[1][6]       ;
  xgs[6][6]    =  q13             ;
  wgtt[0][6]   =  0.0629695902724 ;
  wgtt[1][6]   =  0.0661970763943 ;
  wgtt[2][6]   =  wgtt[0][6]      ;
  wgtt[3][6]   =  wgtt[1][6]      ;
  wgtt[4][6]   =  wgtt[0][6]      ;
  wgtt[5][6]   =  wgtt[1][6]      ;
  wgtt[6][6]   =  0.1125          ;
/*----------------------------------------------------------------------*
 |                             CASE 7                                   |
 *----------------------------------------------------------------------*/
  xgr[0][7]    =  0.2379323664724 ;
  xgr[1][7]    =  0.7367124989684 ;
  xgr[2][7]    =  xgr[1][7]       ;
  xgr[3][7]    =  xgr[0][7]       ;
  xgr[4][7]    =  0.0253551345591 ;
  xgr[5][7]    =  xgr[4][7]       ;
  xgr[6][7]    =  q13             ;
  xgs[0][7]    =  xgr[4][7]       ;
  xgs[1][7]    =  xgr[4][7]       ;
  xgs[2][7]    =  xgr[0][7]       ;
  xgs[3][7]    =  xgr[1][7]       ;
  xgs[4][7]    =  xgr[1][7]       ;
  xgs[5][7]    =  xgr[0][7]       ;
  xgs[6][7]    =  q13             ;
  wgtt[0][7]   =  0.0520833333333 ;
  wgtt[1][7]   =  wgtt[0][7]      ;
  wgtt[2][7]   =  wgtt[0][7]      ;
  wgtt[3][7]   =  wgtt[0][7]      ;
  wgtt[4][7]   =  wgtt[0][7]      ;
  wgtt[5][7]   =  wgtt[0][7]      ;
  wgtt[6][7]   =  0.1875          ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION        9 SAMPLING POINTS, DEG.OF PRECISION 5    |
 |                             CASE 8                                   |
 *----------------------------------------------------------------------*/
  xgr[0][8]    =  0.1654099273898     ;
  xgr[1][8]    =  0.4375252483834     ;
  xgr[2][8]    =  0.7971126518601     ;
  xgr[3][8]    =  xgr[2][8]           ;
  xgr[4][8]    =  xgr[1][8]           ;
  xgr[5][8]    =  xgr[0][8]           ;
  xgr[6][8]    =  0.0374774207501     ;
  xgr[7][8]    =  0.1249495032332     ;
  xgr[8][8]    =  xgr[6][8]           ;
  xgs[0][8]    =  xgr[6][8]           ;
  xgs[1][8]    =  xgr[7][8]           ;
  xgs[2][8]    =  xgr[6][8]           ;
  xgs[3][8]    =  xgr[0][8]           ;
  xgs[4][8]    =  xgr[1][8]           ;
  xgs[5][8]    =  xgr[2][8]           ;
  xgs[6][8]    =  xgr[2][8]           ;
  xgs[7][8]    =  xgr[1][8]           ;
  xgs[8][8]    =  xgr[0][8]           ;
  wgtt[0][8]   =  0.0318457071431     ;
  wgtt[1][8]   =  0.1029752523804     ;
  wgtt[2][8]   =  wgtt[0][8]          ;
  wgtt[3][8]   =  wgtt[0][8]          ;
  wgtt[4][8]   =  wgtt[1][8]          ;
  wgtt[5][8]   =  wgtt[0][8]          ;
  wgtt[6][8]   =  wgtt[0][8]          ;
  wgtt[7][8]   =  wgtt[1][8]          ;
  wgtt[8][8]   =  wgtt[0][8]          ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION       12 SAMPLING POINTS, DEG.OF PRECISION 6    |
 |                            CASE 9                                    |
 *----------------------------------------------------------------------*/
  xgr[0][9]    =  0.0630890144915     ;
  xgr[1][9]    =  0.3103524510338     ;
  xgr[2][9]    =  0.6365024991214     ;
  xgr[3][9]    =  0.8738219710170     ;
  xgr[4][9]    =  xgr[2][9]           ;
  xgr[5][9]    =  xgr[1][9]           ;
  xgr[6][9]    =  xgr[0][9]           ;
  xgr[7][9]    =  0.0531450498448     ;
  xgr[8][9]    =  xgr[7][9]           ;
  xgr[9][9]    =  0.2492867451709     ;
  xgr[10][9]   =  0.5014265096582     ;
  xgr[11][9]   =  xgr[9][9]           ;
  xgs[0][9]    =  xgr[0][9]           ;
  xgs[1][9]    =  xgr[7][9]           ;
  xgs[2][9]    =  xgr[7][9]           ;
  xgs[3][9]    =  xgr[0][9]           ;
  xgs[4][9]    =  xgr[1][9]           ;
  xgs[5][9]    =  xgr[2][9]           ;
  xgs[6][9]    =  xgr[3][9]           ;
  xgs[7][9]    =  xgr[2][9]           ;
  xgs[8][9]    =  xgr[1][9]           ;
  xgs[9][9]    =  xgr[9][9]           ;
  xgs[10][9]   =  xgr[9][9]           ;
  xgs[11][9]   =  xgr[10][9]          ;
  wgtt[0][9]   =  0.0254224531851     ;
  wgtt[1][9]   =  0.0414255378092     ;
  wgtt[2][9]   =  wgtt[1][9]          ;
  wgtt[3][9]   =  wgtt[0][9]          ;
  wgtt[4][9]   =  wgtt[1][9]          ;
  wgtt[5][9]   =  wgtt[1][9]          ;
  wgtt[6][9]   =  wgtt[0][9]          ;
  wgtt[7][9]   =  wgtt[1][9]          ;
  wgtt[8][9]   =  wgtt[1][9]          ;
  wgtt[9][9]   =  0.0583931378632     ;
  wgtt[10][9]  =  wgtt[9][9]          ;
  wgtt[11][9]  =  wgtt[9][9]          ;
/*----------------------------------------------------------------------*
 |    GAUSS INTEGRATION       13 SAMPLING POINTS, DEG.OF PRECISION 7    |
 |                            CASE 10                                   |
 *----------------------------------------------------------------------*/
  xgr[0][10]    =  0.0651301029022     ;
  xgr[1][10]    =  0.3128654960049     ;
  xgr[2][10]    =  0.6384441885698     ;
  xgr[3][10]    =  0.8697397941956     ;
  xgr[4][10]    =  xgr[2][10]          ;
  xgr[5][10]    =  xgr[1][10]          ;
  xgr[6][10]    =  xgr[0][10]          ;
  xgr[7][10]    =  0.0486903154253     ;
  xgr[8][10]    =  xgr[7][10]          ;
  xgr[9][10]    =  0.2603459660790     ;
  xgr[10][10]   =  0.4793080678419     ;
  xgr[11][10]   =  xgr[9][10]          ;
  xgr[12][10]   =  q13                 ;
  xgs[0][10]    =  xgr[0][10]          ;
  xgs[1][10]    =  xgr[7][10]          ;
  xgs[2][10]    =  xgr[7][10]          ;
  xgs[3][10]    =  xgr[0][10]          ;
  xgs[4][10]    =  xgr[1][10]          ;
  xgs[5][10]    =  xgr[2][10]          ;
  xgs[6][10]    =  xgr[3][10]          ;
  xgs[7][10]    =  xgr[2][10]          ;
  xgs[8][10]    =  xgr[1][10]          ;
  xgs[9][10]    =  xgr[9][10]          ;
  xgs[10][10]   =  xgr[9][10]          ;
  xgs[11][10]   =  xgr[10][10]         ;
  xgs[12][10]   =  q13                 ;
  wgtt[0][10]   =  0.0266736178044     ;
  wgtt[1][10]   =  0.0385568804451     ;
  wgtt[2][10]   =  wgtt[1][10]         ;
  wgtt[3][10]   =  wgtt[0][10]         ;
  wgtt[4][10]   =  wgtt[1][10]         ;
  wgtt[5][10]   =  wgtt[1][10]         ;
  wgtt[6][10]   =  wgtt[0][10]         ;
  wgtt[7][10]   =  wgtt[1][10]         ;
  wgtt[8][10]   =  wgtt[1][10]         ;
  wgtt[9][10]   =  0.0878076287166     ;
  wgtt[10][10]  =  wgtt[9][10]         ;
  wgtt[11][10]  =  wgtt[9][10]         ;
  wgtt[12][10]  = -0.0747850222338     ;

  /*--------------------------------------------------------------------*/
  /* write global arrays */
  /* quadrilateral domain & edges --> line [-1,+1] */
  for (i=0; i<GLMAXP_THERM2; i++)
  {
    for (k=0; k<GLINTC_THERM2; k++)
    {
      data->gqlc[i][k] = xg[i][k];
      data->gqlw[i][k] = wgt[i][k];
    }
  }
  /* triangle domain */
  for (i=0; i<GTMAXP_THERM2; i++)
  {
    for (k=0; k<GTINTC_THERM2; k++)
    {
      data->gtdcr[i][k] = xgr[i][k];
      data->gtdcs[i][k] = xgs[i][k];
      data->gtdw[i][k] = wgtt[i][k];
    }
  }
  /* triangle edges --> line [0,1]*/
  for (i=0; i<GLMAXP_THERM2; i++)
  {
    for (k=0; k<GLINTC_THERM2; k++)
    {
      data->gtlc[i][k] = (1.0 + xg[i][k])/2.0;  /* this will give spurious
                                                 * 1/2 on lower array triangle
                                                 * however, these will never
                                                 * be called */
      data->gtlw[i][k] = wgt[i][k]/2.0;
    }
  }

  /*--------------------------------------------------------------------*/
#ifdef DEBUG
  dstrc_exit();
#endif

  return;
} /* end of th2_intg_init() */




/*----------------------------------------------------------------------*/
#endif  /* end of #ifdef D_THERM2 */

/*! @} (documentation module close)*/
