 ...
 *----------------------------------------------------------------------------------------------*/
void STR::TimIntOneStepThetaImmersed::Init
(
    const Teuchos::ParameterList& timeparams,
    const Teuchos::ParameterList& sdynparams,
    const Teuchos::ParameterList& xparams,
...
void STR::TimIntOneStepThetaImmersed::DetermineMass()
{
  ...
  Teuchos::RCP<LINALG::SparseOperator> stiff_aux = Teuchos::rcp(new LINALG::SparseMatrix(*DofRowMapView(), 81, false, true));
...
void STR::TimIntOneStepThetaImmersed::NonlinearMassSanityCheck(Teuchos::RCP<const Epetra_Vector> fext,
