          ...
          else  // filled ale discretization
          {
            if (!FSI::UTILS::FluidAleNodesDisjoint(fluiddis, aledis))
              dserror(
                  ...
                  "Use either the ALE cloning functionality or ensure non-overlapping node "
                  "numbering!");
          }
...
void CellMigrationControlAlgorithm()
{
  ...
  Teuchos::RCP<::ADAPTER::MultiphysicsStructureWrapperCellMigration> cellstructure = Teuchos::null;
  // declare model evaluator
  Teuchos::RCP<STR::MODELEVALUATOR::CellMigration> multiphysics_model_evaluator_cellmigration =
      Teuchos::null;
  ...
  const Teuchos::ParameterList& cellstructureparams =
      cellmigrationparams.sublist("STRUCTURAL DYNAMIC");
  ...
  POROELAST::UTILS::SetupPoroScatraDiscretizations<POROELAST::UTILS::PoroelastImmersedCloneStrategy,
  ...
  ///////////////////////////////////////////////////////////////////////
  {
    ...
    {
      IMMERSED::ImmersedNode* inode = dynamic_cast<IMMERSED::ImmersedNode*>(
          (problem->GetDis("porofluid")->gElement(gid))->Nodes()[0]);
  ...
  Teuchos::RCP<ADAPTER::StructureBaseAlgorithmNew> struct_adapterbase_ptr =
      ADAPTER::STR::BuildStructureAlgorithm(cellstructureparams);
  // initialize structure base algorithm
  struct_adapterbase_ptr->Init(cellmigrationparams,
      const_cast<Teuchos::ParameterList&>(cellstructureparams), problem->GetDis("cell"));
  ...
  struct_adapterbase_ptr->RegisterModelEvaluator(
      "Partitioned Coupling Model", multiphysics_model_evaluator_cellmigration);
  ...
  if (ssi_cell)
  {
    ...
    const INPAR::SSI::SolutionSchemeOverFields coupling =
        DRT::INPUT::IntegralValue<INPAR::SSI::SolutionSchemeOverFields>(
            problem->SSIControlParams(), "COUPALGO");
    ...
    switch (coupling)
    {
      case INPAR::SSI::ssi_IterStagg:
        if (simtype == INPAR::CELL::sim_type_pureProtrusionFormation)
          cellscatra_subproblem = Teuchos::rcp(
              new SSI::SSI_Part2WC_PROTRUSIONFORMATION(comm, problem->CellMigrationParams()));
        else if (simtype == INPAR::CELL::sim_type_pureAdhesion or
                 simtype == INPAR::CELL::sim_type_multiphysics)
        {
          if (problem->CellMigrationParams().get<std::string>("ADHESION_DYNAMICS") == "yes")
            cellscatra_subproblem = Teuchos::rcp(
                new SSI::SSI_Part2WC_ADHESIONDYNAMICS(comm, problem->CellMigrationParams()));
          else
            cellscatra_subproblem =
                Teuchos::rcp(new SSI::SSI_Part2WC(comm, problem->CellMigrationParams()));
        }
        else if (simtype == INPAR::CELL::sim_type_pureContraction)
        {
          cellscatra_subproblem = Teuchos::rcp(
              new SSI::SSI_Part2WC_BIOCHEMOMECHANO(comm, problem->CellMigrationParams()));
          params.set<bool>("IsPureContraction", true);
        }
        break;
      default:
        dserror(
            "unknown coupling algorithm for SSI! Only ssi_IterStagg valid. Fix your *.dat file.");
        break;
    }
    ...
    int redistribute = cellscatra_subproblem->Init(comm, problem->CellMigrationParams(),
        problem->CellMigrationParams().sublist("SCALAR TRANSPORT"),
        problem->CellMigrationParams().sublist("STRUCTURAL DYNAMIC"), "cell", "cellscatra", isale);
    ...
    if (redistribute)
    {
      if (redistribute == (int)SSI::match)
      {
        ...
        DRT::UTILS::MatchElementDistributionOfMatchingConditionedElements(
            *problem->GetDis("cellscatra"), *problem->GetDis("cellscatra"),
        ...
        DRT::UTILS::MatchElementDistributionOfMatchingDiscretizations(
            *problem->GetDis("cellscatra"), *problem->GetDis("cell"));

        // now we redistribute the ale dis to match the scatra dis
        if (simtype == INPAR::CELL::sim_type_pureProtrusionFormation)
          DRT::UTILS::MatchElementDistributionOfMatchingDiscretizations(
              *problem->GetDis("cell"), *problem->GetDis("ale"));
      }
      else
    ...
    cellstructure = Teuchos::rcp_dynamic_cast<ADAPTER::MultiphysicsStructureWrapperCellMigration>(
        struct_adapterbase_ptr->StructureField(), true);

    // set pointer to model evaluator in ssi specific wrapper
    cellstructure->GetSSIStructureWrapperPtr()->SetModelEvaluatorPtr(
        Teuchos::rcp_dynamic_cast<STR::MODELEVALUATOR::PartitionedSSI>(
            multiphysics_model_evaluator_cellmigration->GetModelEvaluatorFromMap(
                STR::MODELEVALUATOR::mt_ssi),
            true));
  ...
  ///////////////////////////////////////////////////////////////////////
  // set pointer to model evaluator in ssi specific wrapper
  cellstructure->GetFSIStructureWrapperPtr()->SetModelEvaluatorPtr(
      Teuchos::rcp_dynamic_cast<STR::MODELEVALUATOR::PartitionedFSI>(
          multiphysics_model_evaluator_cellmigration->GetModelEvaluatorFromMap(
              STR::MODELEVALUATOR::mt_fsi),
          true));
  ...
  Teuchos::RCP<POROELAST::PoroScatraBase> poroscatra_subproblem =
      POROELAST::UTILS::CreatePoroScatraAlgorithm(problem->CellMigrationParams(), comm);

  // setup of poro monolithic algorithm
  poroscatra_subproblem->SetupSystem();
  if (comm.MyPID() == 0) std::cout << " Created Field PoroScatra ... \n" << std::endl;

  // construct scatra-adapter for poro-scatra time integration
  Teuchos::RCP<ADAPTER::AdapterScatraWrapperCellMigration> poroscatrawrapper = Teuchos::rcp(
      new ADAPTER::AdapterScatraWrapperCellMigration(poroscatra_subproblem->ScaTraField()));
  ...
  LINALG::Gather<int, LINALG::Matrix<3, 1>>(
      my_currpositions_cell, currpositions_cell, comm.NumProc(), &procs[0], comm);

  // find the bounding box of the elements and initialize the search tree
  const LINALG::Matrix<3, 2> rootBox2 =
      GEO::getXAABBofDis(*(problem->GetDis("cell")), currpositions_cell);
  cell_SearchTree->initializeTree(
      rootBox2, *(problem->GetDis("cell")), GEO::TreeType(GEO::OCTTREE));
  ...
  const LINALG::Matrix<3, 2> rootBox =
      GEO::getXAABBofDis(*(problem->GetDis("porofluid")), currpositions_ECM);
  fluid_SearchTree->initializeTree(
      rootBox, *(problem->GetDis("porofluid")), GEO::TreeType(GEO::OCTTREE));
  ...
  ///////////////////////////////////////////////////////////////////////
  params.set<Teuchos::RCP<ADAPTER::MultiphysicsStructureWrapperCellMigration>>(
      "RCPToCellStructure", cellstructure);
  params.set<Teuchos::RCP<ADAPTER::AdapterScatraWrapperCellMigration>>(
      "RCPToCellScatraWrapper", cellscatra_wrapper);
  params.set<Teuchos::RCP<POROELAST::PoroScatraBase>>("RCPToPoroScatra", poroscatra_subproblem);
  params.set<Teuchos::RCP<ADAPTER::AdapterScatraWrapperCellMigration>>(
      "RCPToPoroScatraWrapper", poroscatrawrapper);
  ...
  params.set<std::map<int, LINALG::Matrix<3, 1>>*>(
      "PointerToCurrentPositionsCell", &currpositions_cell);
  params.set<std::map<int, LINALG::Matrix<3, 1>>*>(
      "PointerToCurrentPositionsECM", &currpositions_ECM);
  ...
  if (simtype == INPAR::CELL::sim_type_pureFSI)
  {
    ...
    DRT::Problem::Instance()->AddFieldTest(
        algo->PoroField()->ScaTraFieldBase()->CreateScaTraFieldTest());
  ...
  else if (simtype == INPAR::CELL::sim_type_pureAdhesion)
  {
    ...
    DRT::Problem::Instance()->AddFieldTest(
        cellscatra_subproblem->ScaTraField()->CreateScaTraFieldTest());
    DRT::Problem::Instance()->AddFieldTest(poroscatra_subproblem->FluidField()->CreateFieldTest());
    DRT::Problem::Instance()->AddFieldTest(
        poroscatra_subproblem->StructureField()->CreateFieldTest());
    DRT::Problem::Instance()->AddFieldTest(
        poroscatra_subproblem->ScaTraFieldBase()->CreateScaTraFieldTest());
  ...
  else if (simtype == INPAR::CELL::sim_type_multiphysics)
  {
    ...
    params.set<Teuchos::RCP<IMMERSED::ImmersedPartitionedFlowCellInteraction>>(
        "ImmersedPartitionedFlowCellInteraction", cfi_algo);
    params.set<Teuchos::RCP<IMMERSED::ImmersedPartitionedAdhesionTraction>>(
        "ImmersedPartitionedAdhesionTraction", adh_algo);
    ...
    dserror(
        "Unknown SIMTYPE set in ---CELL DYNAMIC section (default=pureFSI). Fix your .dat file.");
