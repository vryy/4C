  ...
  : ImmersedPartitioned(comm)
{
  ...
  dirichletcoupling_ = globalproblem_->CellMigrationParams().sublist("ADHESION MODULE").get<std::string>("COUPMETHOD") == "Dirichlet";
  if(dirichletcoupling_ and myrank_==0)
    std::cout<<" Coupling method for partitioned Cell-ECM Adhesion Dynamics scheme :  Dirichlet "<<std::endl;
  else if(!dirichletcoupling_ and myrank_==0)
      std::cout<<" Coupling method for partitioned Cell-ECM Adhesion Dynamics scheme :  Penalty "<<std::endl;
  ...
  else // violation coupling via penalty method
  {
    ...
    double scatra_tol = globalproblem_->CellMigrationParams().sublist("SCALAR TRANSPORT").sublist("NONLINEAR").get<double>("CONVTOL");
  ...
  artificial_ecm_treatment_ = DRT::INPUT::IntegralValue<int>(globalproblem_->CellMigrationParams(),"ARTIFICIAL_ECM_METHOD");
  ...
  else
  {
    ...
    if(immersed_information_invalid_)
    {
      ...
      if (artificial_ecm_treatment_)
      {
        ...
        std::vector<int> nodes = DetermineElesInFirstImmersedRow(backgroundfluiddis_, &ElesInFirstImmersedRow);
        CreateVolumeCondition(backgroundstructuredis_,nodes,DRT::Condition::CellFocalAdhesion,"ImmersedAdhesionElements",true);
    ...
    if (artificial_ecm_treatment_)
    {
      ...
      RemoveDirichlet(additional_dbc_dofmap_ecm,poroscatra_subproblem_->PoroField()->StructureField());
...
void IMMERSED::ImmersedPartitionedAdhesionTraction::PrepareBackgroundOp()
{
  ...
  Comm().Barrier();
  if(curr_subset_of_backgrounddis_.empty() == false)
    std::cout<<"  "<<curr_subset_of_backgrounddis_.begin()->second.size()<<" background elements on Proc "<<Comm().MyPID()<<std::endl;
  Comm().Barrier();
    ...
    const Teuchos::RCP<const Epetra_Vector>& force_to_distribute)
{
  ...
  if(myrank_ == 0)
    std::cout<<"###   L1-Norm of Cell Reaction Force Vector: "<<std::setprecision(11)<<freactnorm<<std::endl;
  ...
  for(int anode=0;anode<max_adhesion_node_size_over_procs;anode++)
  {
    ...
    if(not determine_adhesion_information_ and adhesion_node->Owner()==myrank_)
    {
      ...
      if(matching_id<0)
        dserror("invalid id for matching background element in map");
    }
    else
    {
        ...
        for(std::set<int>::const_iterator eleIter = (closele->second).begin(); eleIter != (closele->second).end(); eleIter++)
        {
          ...
          if(immersedele->IsBoundaryImmersed())
          {
            ...
            if (mymatch)
            {
              ...
              {
                bool owned = false;
                if(spreadnodes[snode]->Owner()==myrank_)
                  owned = true;
                ...
                if(owned)
                {
                  ...
                  for(int dim=0;dim<(3-isPseudo2D_);dim++)
                  {
                    int error = ecm_adhesion_forces_->SumIntoGlobalValue(sdofs[dim],0,0,shapefcts(snode)*anode_freact[dim]);
                    if(error != 0)
    ...
    if(not finished and adhesion_node->Owner() == myrank_)
      dserror("Adhesion node %i could not be matched to a background element on any proc.\n"
...
void IMMERSED::ImmersedPartitionedAdhesionTraction::CalcAdhesionDisplacements()
{
  ...
  for (int anode=0;anode<max_adhesionnode_size_over_procs;++anode)
  {
    ...
    if (it != adh_nod_param_coords_in_backgrd_ele_.end())
    {
      ...
      if(adhesion_node->Owner()!=myrank_)
        dserror("Only row nodes should be in map.");
    }

    // get matching background element id
    int matching_id = -1;
    if(anode_id > -1)
      matching_id = adh_nod_backgrd_ele_mapping_.at(anode_id);
      ...
      if(numproc>1)
      {
        // ---- pack data for sending -----
        {
          ...
          data.StartPacking();
          {
            ...
            {
              for(int dim=0;dim<3;++dim)
                data.AddtoPack(adhesiondisp(dim));
            }
          }
          std::swap(sdata, data());
        }
    ...
    if(not matched and anode_id > -1)
      dserror("Adhesion node %i could not be matched to a background element on any proc.",anode_id);
...
void IMMERSED::ImmersedPartitionedAdhesionTraction::BuildImmersedDirichMap(
    ...
    Teuchos::RCP<Epetra_Map>& dirichmap_new,
    ...
    if(node_ptr->Owner() == myrank_)
    {
      ...
      for (int dim=0;dim<3;++dim)
      {
        ...
        if(dirichmap_orig->LID(dofs[dim]) == -1)
          mydirichrowdofs.push_back(dofs[dim]);
      }
    } // if proc owns node
  } // for all nodes in provided map

  int nummydirichvals = mydirichrowdofs.size();
  dirichmap_new = Teuchos::rcp( new Epetra_Map(-1,nummydirichvals,&(mydirichrowdofs[0]),0,dis->Comm()) );
