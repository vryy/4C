  ...
  : ImmersedPartitioned(comm)
{
  ...
  ecm_adhesion_forces_col_ = Teuchos::rcp(new Epetra_Vector(*(backgroundstructuredis_->DofColMap()),true));
    ...
    const Teuchos::RCP<const Epetra_Vector>& force_to_distribute)
{
  ...
  // each proc needs to enter here unil the proc with the most
  ...
  for(int anode=0;anode<max_adhesion_node_size_over_procs;anode++)
  {
    ...
    if(not finished and adhesion_node->Owner() == myrank_)
      dserror("Adhesion node %i could not be matched to a background element on any proc.",anodeid);
