  ...
  : ImmersedPartitioned(comm)
{
  ...
  exchange_manager_->SetIsPureConfinementSimulation(params.get<bool>("IsPureConfinementSimulation")==true);
...
void IMMERSED::ImmersedPartitionedConfineCell::CouplingOp(const Epetra_Vector &x, Epetra_Vector &F, const FillType fillFlag)
{
  ...
  if (displacementcoupling_)
  {
    ...
    ImmersedOp(cellstructure_->Interface()->ExtractIMMERSEDCondVector(cell_penalty_traction_), fillFlag);
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IMMERSED::ImmersedPartitionedConfineCell::BackgroundOp(Teuchos::RCP<Epetra_Vector> backgrd_dirichlet_values,
...
IMMERSED::ImmersedPartitionedConfineCell::ImmersedOp(Teuchos::RCP<Epetra_Vector> bdry_traction,
                                                       const FillType fillFlag)
{
...
void IMMERSED::ImmersedPartitionedConfineCell::PrepareImmersedOp()
{
  ...
  if(myrank_==0)
    std::cout<<"   Update Immersed Information ..."<<std::endl;
...
/*----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector>
IMMERSED::ImmersedPartitionedConfineCell::InitialGuess()
{
  if(displacementcoupling_)
...
void IMMERSED::ImmersedPartitionedConfineCell::CalcECMTractionOnStructure()
{
  ...
  {
    std::cout<<"################################################################################################"<<std::endl;
    std::cout<<"###   Calculate force from ECM transmitted to cell surface due to confinement                   "<<std::endl;
  ...
  cell_penalty_traction_->Update(1.0,*cell_delta_penalty_traction_,0.0); // cell_delta_penalty_traction contains full force, thus this*0.0
  ...
  if(myrank_ == 0)
  {
    ...
    std::cout<<"###   Max gap origin:    ["<<std::setprecision(6)<<(*(exchange_manager_->GetMaxGapSpacePoint()))(0)<<" "<<(*(exchange_manager_->GetMaxGapSpacePoint()))(1)<<" "<<(*(exchange_manager_->GetMaxGapSpacePoint()))(2)<<"]"<<std::endl;
    std::cout<<"###   Max gap end point: ["<<(*(exchange_manager_->GetMaxGapSearchDirection()))(0)<<" "<<(*(exchange_manager_->GetMaxGapSearchDirection()))(1)<<" "<<(*(exchange_manager_->GetMaxGapSearchDirection()))(2)<<"]"<<std::endl;
    std::cout<<"###   Min gap origin:    ["<<std::setprecision(6)<<(*(exchange_manager_->GetMinGapSpacePoint()))(0)<<" "<<(*(exchange_manager_->GetMinGapSpacePoint()))(1)<<" "<<(*(exchange_manager_->GetMinGapSpacePoint()))(2)<<"]"<<std::endl;
    std::cout<<"###   Min gap end point: ["<<(*(exchange_manager_->GetMinGapSearchDirection()))(0)<<" "<<(*(exchange_manager_->GetMinGapSearchDirection()))(1)<<" "<<(*(exchange_manager_->GetMinGapSearchDirection()))(2)<<"]"<<std::endl;
