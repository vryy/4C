...
IMMERSED::ImmersedPartitionedProtrusionFormation::ImmersedPartitionedProtrusionFormation(
    ...
    : ImmersedPartitioned(comm)
{
  ...
  Teuchos::RCP<SSI::SSI_Part2WC> cellscatra_subproblem =
  ...
  if (displacementcoupling_ and myrank_ == 0)
    std::cout << "\n Coupling variable for partitioned protrusion formation:  Displacements "
              << std::endl;
...
void IMMERSED::ImmersedPartitionedProtrusionFormation::Setup()
{
  ...
  const Teuchos::ParameterList& noxparams =
      globalproblem_->CellMigrationParams().sublist("PROTRUSION MODULE");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> IMMERSED::ImmersedPartitionedProtrusionFormation::InitialGuess()
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IMMERSED::ImmersedPartitionedProtrusionFormation::CouplingOp(
    const Epetra_Vector& x, Epetra_Vector& F, const FillType fillFlag)
{
  ...
  if (displacementcoupling_)
  {
    ...
    ////////////////////
    PrepareImmersedOp();
    Teuchos::RCP<Epetra_Vector> idispnp = ImmersedOp(Teuchos::null, fillFlag);
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IMMERSED::ImmersedPartitionedProtrusionFormation::BackgroundOp(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> IMMERSED::ImmersedPartitionedProtrusionFormation::ImmersedOp(
