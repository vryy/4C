  ...
  for (int node = 0; node < cond_nodes_size; node++)
  {
    ...
    if (DRT::Problem::Instance()->GetDis("porofluid")->NodeColMap()->MyGID(nodeid))
    {
      ...
      if (node_ptr->Owner() == dis->Comm().MyPID())
      {
        ...
        {
          DRT::Element* iele =
              DRT::Problem::Instance()->GetDis("porofluid")->gElement(element[aele]->Id());
          DRT::ELEMENTS::FluidPoroImmersed* immersedele =
        ...
        IMMERSED::ImmersedNode* immersed_node_ptr = dynamic_cast<IMMERSED::ImmersedNode*>(node_ptr);
        if (node_ptr == NULL)
  ...
  for (int node = 0; node < cond_nodes_size; node++)
  {
    ...
    if (DRT::Problem::Instance()->GetDis("porofluid")->NodeColMap()->MyGID(nodeid))
    {
      ...
      if (node_ptr->Owner() == dis->Comm().MyPID())
      {
        ...
        // we only set Dirichlet values to fully artificial nodes,
        // which do not belong to pseudo-boundary
        if (attachedtoimmersedbdryelement or
            (attachedtoimmersedbdryelement == false and attachedtoimmersedelement == false))
        {
          // get dofs
          std::vector<int> dofs = dis->Dof(0, node_ptr);
...
std::vector<int> IMMERSED::ImmersedBase::DetermineElesInFirstImmersedRow(
    const Teuchos::RCP<const DRT::Discretization>& dis, std::vector<int>* eleids)
{
  ...
  for (int lid = 0; lid < nummyelements; lid++)
  {
    ...
    if (immersedeleptr == NULL) dserror("dynamic cast to FluidPoroImmersed failed!");
    ...
    if (immersedeleptr->IsImmersed())
    {
      ...
      for (int node = 0; node < immersedeleptr->NumNode(); node++)
      {
        ...
        if (immersednodeptr == NULL) dserror("dynamic cast to ImmersedNode failed!");
