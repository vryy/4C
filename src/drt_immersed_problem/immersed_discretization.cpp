    ...
    for (int i = 0; i < numcolele; ++i)
    {
      ...
      else
      {
        ...
        strategy.AssembleMatrix1(
            eid, la_selective[row].lm_, la[col].lm_, la_selective[row].lmowner_, la[col].stride_);
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void DRT::DiscretizationImmersed::EvaluateCondition(Teuchos::ParameterList& params,
      ...
      if (fool->first == condstring)
      {
        ...
        if (condid == -1 || condid == cond.GetInt("ConditionID"))
        {
          ...
          for (curr = geom.begin(); curr != geom.end(); ++curr)
          {
            ...
            {
              strategy.AssembleMatrix1(eid, la_selective[row].lm_, la[col].lm_,
                  la_selective[row].lmowner_, la[col].stride_);
              strategy.AssembleMatrix2(eid, la_selective[row].lm_, la[col].lm_,
                  la_selective[row].lmowner_, la[col].stride_);
              strategy.AssembleVector1(la_selective[row].lm_, la_selective[row].lmowner_);
              strategy.AssembleVector2(la_selective[row].lm_, la_selective[row].lmowner_);
              strategy.AssembleVector3(la_selective[row].lm_, la_selective[row].lmowner_);
  ...
  for (int dofset = 0; dofset < la.Size(); ++dofset)
  {
    ...
    if (nodes)
    {
      for (int i = 0; i < numnode; ++i)
      {
        ...
        dsassert(ele->NumDofPerNode(*node) <= (int)dof.size() or dofset != 0,
        ...
        immersedele = dynamic_cast<DRT::ELEMENTS::FluidPoroImmersed*>(iele);
        if (immersedele == NULL) dserror("dynamic cast to FluidPoroImmersed* failed");

        int structnodeid = node->Id();
        immersednode = dynamic_cast<IMMERSED::ImmersedNode*>(immersedinfodis->gNode(structnodeid));
        if (immersednode == NULL) dserror("dynamic cast to ImmersedNode* failed");
