...
for (vi=0; vi<iel; ++vi)
{
  for (ui=0; ui<iel; ++ui)
  {
    ...
    estif_(vi*3, ui*3) += ttimetauM*(conv_c_(ui)*conv_c_(vi) + conv_c_(vi)*conv_r_(0, 0, ui) + derxyz_(0, vi)*vconv_r_(0, ui)) ;
    ...
    estif_(vi*3 + 1, ui*3 + 1) += ttimetauM*(conv_c_(ui)*conv_c_(vi) + conv_c_(vi)*conv_r_(1, 1, ui) + derxyz_(1, vi)*vconv_r_(1, ui)) ;
    ...
    estif_(vi*3 + 2, ui*3 + 2) += ttimetauM*(conv_c_(ui)*conv_c_(vi) + conv_c_(vi)*conv_r_(2, 2, ui) + derxyz_(2, vi)*vconv_r_(2, ui)) ;
...
for (vi=0; vi<iel; ++vi)
{
    ...
    eforce_(vi*3 + 2) += -(ttimetauM*conv_c_(vi)*(velint_(0)*vderxyz_(2, 0) + velint_(1)*vderxyz_(2, 1) + velint_(2)*vderxyz_(2, 2))) ;
