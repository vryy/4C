    ...
    const DRT::UTILS::GaussIntegration& intpoints)
{
  ...
  my::ExtractValuesFromGlobalVector(
      discretization, lm, *my::rotsymmpbc_, &edispnp, &eporositynp, "dispnp");
  my::ExtractValuesFromGlobalVector(
      discretization, lm, *my::rotsymmpbc_, &egridv, &eporositydot, "gridv");
  my::ExtractValuesFromGlobalVector(
      discretization, lm, *my::rotsymmpbc_, &egridvn, &eporositydotn, "gridvn");
  ...
  int result = my::Evaluate(params, ebofoaf, elemat1, elevec1, evelaf, epreaf, evelnp, eveln,
    ...
    LINALG::Matrix<my::nen_, my::nen_>& ppmat, LINALG::Matrix<my::nen_, 1>& preforce)
{
  ...
  my::EvaluatePressureEquationNonTransient(params, timefacfacpre, rhsfac, dphi_dp, dphi_dJ,
    ...
    const DRT::UTILS::GaussIntegration& intpoints)
{
  ...
  if (my::fldparatimint_->IsGenalphaNP())
    this->ExtractValuesFromGlobalVector(
        discretization, lm, *my::rotsymmpbc_, &eveln, &epren, "veln");
    ...
    const DRT::UTILS::GaussIntegration& intpoints)
{
  ...
  static LINALG::Matrix<my::nen_ * my::nsd_, my::nen_> ecouplp1_u(
      true);  // coupling matrix for momentum equation
  static LINALG::Matrix<my::nen_, my::nen_> ecouplp1_p(
      true);  // coupling matrix for continuity equation
    ...
    const DRT::UTILS::GaussIntegration& intpoints)
{
       ...
       iquad != intpoints.end(); ++iquad)
  {
    ...
    my::ComputeJacobianDeterminantVolumeChange(my::J_, volchange, defgrd, my::N_XYZ_, edispnp);

    my::EvaluateVariablesAtGaussPointOD(params, ebofoaf, evelaf, evelnp, eveln, epreaf, eprenp,
    ...
    ComputePorosity(params, my::press_, volchange, *(iquad), my::funct_, eporositynp, my::porosity_,
    ...
    my::ComputeFDerivative(edispnp, defgrd_inv, F_x, F_X);

    // compute gradients if needed
    my::ComputeGradients(my::J_, dphi_dp, dphi_dJ, defgrd_IT_vec, F_x, my::gradp_, eporositynp,
        gradJ, my::grad_porosity_, my::refgrad_porosity_);
    ...
    if (my::porofldpara_->StabBiot() and (not my::porofldpara_->IsStationaryConti()) and
        my::structmat_->PoroLawType() != INPAR::MAT::m_poro_law_constant)
      my::ComputeMixtureStrongResidual(params, defgrd, edispnp, edispn, F_X, *iquad, true);
    ...
    my::FillMatrixMomentumOD(timefacfac, evelaf, egridv, epreaf, dgradphi_dus, dphi_dp, dphi_dJ,
        dphi_dus, refporositydot, lin_resM_Dus, lin_resM_Dus_gridvel, ecoupl_u);

    //*************************************************************************************************************
    // 2) coupling terms in continuity equation

    my::FillMatrixContiOD(timefacfacpre, dphi_dp, dphi_dJ, dphi_dJJ, dphi_dJdp, refporositydot,
      ...
      for (int ui = 0; ui < my::nen_; ++ui)
      {
        for (int idim = 0; idim < my::nsd_; ++idim)
        {
          lin_resM_Dphi(idim, ui) +=
              timefacfac * porosity_inv * (-my::reagridvel_(idim)) * my::funct_(ui);
        }  // end for (idim)
      }    // ui
    }
      ...
      for (int ui = 0; ui < my::nen_; ++ui)
      {
        for (int idim = 0; idim < my::nsd_; ++idim)
        {
          lin_resM_Dphi(idim, ui) += timefacfac * porosity_inv *
                                     (porosity_inv * viscstress_gradphi(idim) * my::funct_(ui) -
  ...
  -  |    D(phi), v                |
    ...
    for (int i = 0; i < my::nen_; i++)
      for (int j = 0; j < my::nsd_; j++) derxy_convel(i) += my::derxy_(j, i) * my::velint_(j);
    ...
    if (static_cast<DRT::ELEMENTS::FluidEleParameterPoro*>(my::fldpara_)->PoroContiPartInt() ==
        false)
    {
      /*
        /                           \     /                             \
      ...
      for (int ui = 0; ui < my::nen_; ++ui)
      {
        for (int vi = 0; vi < my::nen_; ++vi)
        {
          ecouplp1_p(vi, ui) += +timefacfacpre * my::vdiv_ * my::funct_(vi) * my::funct_(ui) +
    ...
    if (my::fldpara_->RStab() != INPAR::FLUID::reactive_stab_none)
    {
      ...
      {  // linearization of stabilization parameter w.r.t. porosity
        const double v =
            timefacfac * my::fldpara_->ViscReaStabFac() *
            (my::reacoeff_ * my::dtaudphi_(1) / my::tau_(1) + my::reacoeff_ / my::porosity_);
        for (int vi = 0; vi < my::nen_; ++vi)
        {
...
{
  my::PSPG(estif_q_u, ppmat, preforce, lin_resM_Du, lin_resMRea_Du, lin_resM_Dp, dphi_dp, fac3,
...
{
  my::ReacStab(estif_u, estif_p_v, velforce, lin_resM_Du, lin_resM_Dp, dphi_dp, timefacfac,
    ...
    Epetra_SerialDenseVector& elevec1)
{
       ...
       iquad != my::intpoints_.end(); ++iquad)
  {
    ...
    my::gridvdiv_ = 0.0;
    for (int idim = 0; idim < my::nsd_; ++idim) my::gridvdiv_ += gridvelderxy(idim, idim);
