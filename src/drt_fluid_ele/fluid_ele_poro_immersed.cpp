...
Teuchos::RCP<DRT::Element> DRT::ELEMENTS::FluidTypePoroImmersed::Create(
    const int id, const int owner)
{
  return Teuchos::rcp(new DRT::ELEMENTS::FluidPoroImmersed(id, owner));
}

void DRT::ELEMENTS::FluidTypePoroImmersed::SetupElementDefinition(
    std::map<std::string, std::map<std::string, DRT::INPUT::LineDefinition>>& definitions)
{
  std::map<std::string, DRT::INPUT::LineDefinition>& defsporoimmersed =
      definitions["FLUIDPOROIMMERSED"];

  defsporoimmersed["HEX8"].AddIntVector("HEX8", 8).AddNamedInt("MAT").AddNamedString("NA");
}
