...
DRT::ELEMENTS::FluidBoundaryParentInterface* DRT::ELEMENTS::FluidBoundaryParentInterface::Impl(DRT::ELEMENTS::FluidBoundary* ele)
{
  ...
  default:
    dserror("Element shape %d (%d nodes) not activated for boundary conditions requiring parent-element evaluations. Just do it.", ele->Shape(), ele->NumNode());
    break;
  }
  return NULL;
}
     ...
     Epetra_SerialDenseMatrix&      elemat2)
{
  ...
  case DRT::Element::tri6:
  {
    ...
    else dserror("expected combination tri6/tet10 for surface/parent pair");
    break;
  }
  case DRT::Element::quad4:
  {
     ...
     Epetra_SerialDenseVector&      elevec_epetra)
{
  ...
  for (int iquad=0; iquad<bintpoints.IP().nquad; ++iquad)
  {
    ...
    DRT::UTILS::ComputeMetricTensorForBoundaryEle<bdistype>(bxyze,deriv,metrictensor,drs_,&unitnormal);
     ...
     Epetra_SerialDenseVector&      elevec_epetra)
{
  ...
  LINALG::Matrix<nsd,nsd>  pvderxyaf(true);      // nabla*u of parent element at integration point
  LINALG::Matrix<1,1>      pressint (true);      // pressure of boundary element at integration point (N_C * p_C)
  LINALG::Matrix<nsd,1>    pvelint (true);       // velocity of parent element at integration point (N_B * U_B_i)
    ...
    for (int ui=0; ui<piel; ++ui)   // Columns
    {
      for (int vi=0; vi<piel; ++vi) // Rows
      {
        ...
        for (int idim=0;idim<nsd;++idim)
        {
          ...
          {
            if (idim==jdim) {
              elemat(vi*(nsd+1)+idim, ui*(nsd+1)+jdim) += timefacFacBeta * pfunct(vi) * pfunct(ui);
            }
          }
        }
      }
     ...
     Epetra_SerialDenseVector&      elevec_epetra)
{
  ...
  if (*active_components == "all_directions")               onlynormal = false;
    ...
    dserror("unknown definition of active components for weak DBC: %s",(*active_components).c_str());

  // optional scaling of penalty parameter
  const double scaling = (*wdbc_cond).GetDouble("TauBscaling");
  if (spalding && fabs(scaling-1.0) > 1e-9) dserror("Parameter tauB for weak DBC will be computed according to Spaldings law. Do not apply scaling factor != 1.0!\n");
  ...
  if (spalding)
  {
    for(int i=0;i<3;++i)
    {
      if ((*val)[i]*(*val)[i] > 1e-9) dserror("Applying Spaldings law to a wall with non-zero velocity!\n");
    }
  }
  ...
  else DRT::UTILS::ExtractMyValues(*velaf,mypvelnp,plm);
  ...
  if (surfele->Shape() == Fluid::nurbs4 or surfele->Shape() == Fluid::nurbs9)
  {
    ...
    for (int inode=0; inode<biel; inode++)
    {
      DRT::NURBS::ControlPoint* cp = dynamic_cast<DRT::NURBS::ControlPoint* > (surfele->Nodes()[inode]);
      bweights(inode) = cp->W();
    }

    for (int inode=0; inode<piel; inode++)
    {
      DRT::NURBS::ControlPoint* cp = dynamic_cast<DRT::NURBS::ControlPoint* > (parent->Nodes()[inode]);
      pweights(inode) = cp->W();
    }
  }*/
    ...
    if (spalding)
    {
      ...
      */
      ...
      while(res*res>1e-6)
      {
        ...
        ++count;
        if (count > 100) dserror("no convergence in 100 steps in Newton iteration during solution of Spaldings equation\n");
      }
    }
    ...
    */
    ...
    if(!onlynormal)
    {
      ...
      */
      ...
      for (int vi=0; vi<piel; ++vi)
      {
        nabla_s_w_o_n(0,0)=consistencytimefac*(unitnormal(0)*(    pderxy(0,vi))+unitnormal(1)*(0.5*pderxy(1,vi))+unitnormal(2)*(0.5*pderxy(2,vi)));
        ...
        nabla_s_w_o_n(1,1)=consistencytimefac*(unitnormal(0)*(0.5*pderxy(0,vi))+unitnormal(1)*(    pderxy(1,vi))+unitnormal(2)*(0.5*pderxy(2,vi)));
        ...
        nabla_s_w_o_n(2,2)=consistencytimefac*(unitnormal(0)*(0.5*pderxy(0,vi))+unitnormal(1)*(0.5*pderxy(1,vi))+unitnormal(2)*(    pderxy(2,vi)));
      ...
      */
      ...
      for (int vi=0; vi<piel; ++vi)
      {
        for (int ui=0; ui<piel; ++ui)
        {
          ...
          elemat(vi*4    ,ui*4 + 2) -= aux*unitnormal(0)*unitnormal(2)*pfunct(ui)*consistencytimefac;

          elemat(vi*4 + 1,ui*4    ) -= aux*unitnormal(1)*unitnormal(0)*pfunct(ui)*consistencytimefac;
          elemat(vi*4 + 1,ui*4 + 1) -= aux*unitnormal(1)*unitnormal(1)*pfunct(ui)*consistencytimefac;
          elemat(vi*4 + 1,ui*4 + 2) -= aux*unitnormal(1)*unitnormal(2)*pfunct(ui)*consistencytimefac;

          elemat(vi*4 + 2,ui*4    ) -= aux*unitnormal(2)*unitnormal(0)*pfunct(ui)*consistencytimefac;
          elemat(vi*4 + 2,ui*4 + 1) -= aux*unitnormal(2)*unitnormal(1)*pfunct(ui)*consistencytimefac;
    ...
    */
    ...
    if(!onlynormal)
    {
      ...
      */
      ...
      for (int vi=0; vi<piel; ++vi)
      {
        nabla_s_w_o_n(0,0)=consistencytimefac*(unitnormal(0)*(    pderxy(0,vi))+unitnormal(1)*(0.5*pderxy(1,vi)));
        nabla_s_w_o_n(0,1)=consistencytimefac*(unitnormal(0)*(0.5*pderxy(1,vi)));

        nabla_s_w_o_n(1,0)=consistencytimefac*(unitnormal(1)*(0.5*pderxy(0,vi)));
        nabla_s_w_o_n(1,1)=consistencytimefac*(unitnormal(0)*(0.5*pderxy(0,vi))+unitnormal(1)*(    pderxy(1,vi)));
      ...
      */
      ...
      for (int vi=0; vi<piel; ++vi)
      {
        for (int ui=0; ui<piel; ++ui)
        {
          ...
          elemat(vi*3 + 1,ui*3    ) -= aux*unitnormal(1)*unitnormal(0)*pfunct(ui)*consistencytimefac;
    ...
    for (int vi=0; vi<piel; ++vi)
    {
      ...
      for (int ui=0; ui<piel; ++ui)
      {
        ...
        Amat(ivx, iux) += fac_*(
            ...
            +1./4.*pderxy(Velz,vi)* unitnormal(Velz)*pderxy(Velz,ui)*unitnormal(Velz)//9
            +1./4.*pderxy(Vely,vi)* unitnormal(Velx)*pderxy(Vely,ui)*unitnormal(Velx)//10
            +1./4.*pderxy(Velz,vi)* unitnormal(Velx)*pderxy(Velz,ui)*unitnormal(Velx)//11
            );


        //(x,y)
        Amat(ivx, iuy) += fac_*(
             1./2.*pderxy(Velx,vi)* unitnormal(Velx)*pderxy(Velx,ui)*unitnormal(Vely)//1
        ...
        //(x,z)
        Amat(ivx, iuz) += fac_*(
             1./2.*pderxy(Velx,vi)* unitnormal(Velx)*pderxy(Velx,ui)*unitnormal(Velz)
        ...
        //(y,x)
        Amat(ivy, iux) += fac_*(
             1./2.*pderxy(Velx,vi)* unitnormal(Vely)*pderxy(Velx,ui)*unitnormal(Velx)
        ...
        //(y,y)
        Amat(ivy, iuy) += fac_*(
             1./4.*pderxy(Velx,vi)* unitnormal(Vely)*pderxy(Velx,ui)*unitnormal(Vely)//1
        ...
        //(y,z)
        Amat(ivy, iuz) += fac_*(
             1./4.*pderxy(Velx,vi)* unitnormal(Vely)*pderxy(Velx,ui)*unitnormal(Velz)//1
        ...
        //(z,x)
        Amat(ivz, iux) += fac_*(
             1./2.*pderxy(Velx,vi)* unitnormal(Velz)*pderxy(Velx,ui)*unitnormal(Velx)//1
        ...
        //(z,y)
        Amat(ivz, iuy) += fac_*(
             1./4.*pderxy(Velx,vi)* unitnormal(Velz)*pderxy(Velx,ui)*unitnormal(Vely)//1
        ...
        //(z,z)
        Amat(ivz, iuz) += fac_*(
             1./4.*pderxy(Velx,vi)* unitnormal(Velz)*pderxy(Velx,ui)*unitnormal(Velz)//1
            +1./4.*pderxy(Vely,vi)* unitnormal(Velz)*pderxy(Vely,ui)*unitnormal(Velz)//2
            +1./4.*pderxy(Velx,vi)* unitnormal(Velx)*pderxy(Velx,ui)*unitnormal(Velx)//3
            +1./4.*pderxy(Velx,vi)* unitnormal(Velx)*pderxy(Vely,ui)*unitnormal(Vely)//4
            +1./2.*pderxy(Velx,vi)* unitnormal(Velx)*pderxy(Velz,ui)*unitnormal(Velz)//5
            +1./4.*pderxy(Vely,vi)* unitnormal(Vely)*pderxy(Velx,ui)*unitnormal(Velx)//6
            +1./4.*pderxy(Vely,vi)* unitnormal(Vely)*pderxy(Vely,ui)*unitnormal(Vely)//7
            +1./2.*pderxy(Vely,vi)* unitnormal(Vely)*pderxy(Velz,ui)*unitnormal(Velz)//8
            +1./2.*pderxy(Velz,vi)* unitnormal(Velz)*pderxy(Velx,ui)*unitnormal(Velx)//9
            +1./2.*pderxy(Velz,vi)* unitnormal(Velz)*pderxy(Vely,ui)*unitnormal(Vely)//10
...
//  std::cout << "computed eigenvalue approximating C*meas(surf)/meas(vol): " << maxeigenvalue << std::endl;
//  std::cout << "meas_vol " << meas_vol << std::endl;
//  std::cout << "meas_surf " << meas_surf << std::endl;
//  std::cout << "resulting C without element-length contribution: " << maxeigenvalue *meas_vol/meas_surf << std::endl;
     ...
     Epetra_SerialDenseVector&       elevec_epetra)
{
  ...
  double normtraction=0.0;
  {
    if(nsd==3)
    {
      ...
      // Gaussian integration points
      const DRT::UTILS::IntPointsAndWeights<bnsd>
        intpoints(DRT::ELEMENTS::DisTypeToOptGaussRule<bdistype>::rule);
      ...
      for (int iquad=0; iquad<intpoints.IP().nquad; ++iquad)
      {
        ...
        */
        ...
        for(int A=0;A<piel;++A)
        {
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_  )-=fac_*pfunct(A)*unitnormal(0)*delta_vel(0);
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+1)-=fac_*pfunct(A)*unitnormal(1)*delta_vel(1);
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+2)-=fac_*pfunct(A)*unitnormal(2)*delta_vel(2);

          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+3)-=fac_*pfunct(A)*(unitnormal(1)*delta_vel(0)+unitnormal(0)*delta_vel(1));
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+4)-=fac_*pfunct(A)*(unitnormal(2)*delta_vel(0)+unitnormal(0)*delta_vel(2));
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+5)-=fac_*pfunct(A)*(unitnormal(2)*delta_vel(1)+unitnormal(1)*delta_vel(2));
        }
        ...
        for(int A=0;A<piel;++A)
        {
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_  )+=fac_*pfunct(A)*unitnormal(0)*unitnormal(0)*u_o_n;
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+1)+=fac_*pfunct(A)*unitnormal(1)*unitnormal(1)*u_o_n;
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+2)+=fac_*pfunct(A)*unitnormal(2)*unitnormal(2)*u_o_n;

          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+3)+=fac_*pfunct(A)*2*unitnormal(0)*unitnormal(1)*u_o_n;
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+4)+=fac_*pfunct(A)*2*unitnormal(0)*unitnormal(2)*u_o_n;
          vec_r_o_n_u_minus_g_SPALDING(A*numstressdof_+5)+=fac_*pfunct(A)*2*unitnormal(1)*unitnormal(2)*u_o_n;
        }
      }
      ...
      for (int iquad=0; iquad<intpoints.IP().nquad; ++iquad)
      {
        ...
        //            |
        //           / Omega
        normtraction+=fac_*sqrt(traction(0)*traction(0)+traction(1)*traction(1)+traction(2)*traction(2));
      ...
      */
      if(nsd ==2)
      {
        for(int A=0;A<piel;++A)
        {
          for(int B=0;B<piel;++B)
          {
            mat_v_sigma_o_n(A*nsd  ,B*numstressdof_  )-=fac_*pfunct(A)*pfunct(B)*unitnormal(0);
            mat_v_sigma_o_n(A*nsd+1,B*numstressdof_+1)-=fac_*pfunct(A)*pfunct(B)*unitnormal(1);

            mat_v_sigma_o_n(A*nsd  ,B*numstressdof_+2)-=fac_*pfunct(A)*pfunct(B)*unitnormal(1);
            mat_v_sigma_o_n(A*nsd+1,B*numstressdof_+2)-=fac_*pfunct(A)*pfunct(B)*unitnormal(0);
          }
        }
      }
      else if(nsd ==3)
      ...
      */
      ...
      if(nsd==2)
      {
        for(int A=0;A<piel;++A)
        {
          for(int B=0;B<piel;++B)
          {
            mat_r_o_n_u(A*numstressdof_  ,B*nsd  )-=fac_*C1*pfunct(A)*pfunct(B)*unitnormal(0);
            mat_r_o_n_u(A*numstressdof_+1,B*nsd+1)-=fac_*C1*pfunct(A)*pfunct(B)*unitnormal(1);

            mat_r_o_n_u(A*numstressdof_+2,B*nsd  )-=fac_*C1*pfunct(A)*pfunct(B)*unitnormal(1);
            mat_r_o_n_u(A*numstressdof_+2,B*nsd+1)-=fac_*C1*pfunct(A)*pfunct(B)*unitnormal(0);
          }
        }
        ...
        for(int A=0;A<piel;++A)
        {
          vec_r_o_n_u_minus_g(A*numstressdof_  )-=fac_*C1*pfunct(A)*unitnormal(0)*delta_vel(0);
          vec_r_o_n_u_minus_g(A*numstressdof_+1)-=fac_*C1*pfunct(A)*unitnormal(1)*delta_vel(1);

          vec_r_o_n_u_minus_g(A*numstressdof_+2)-=fac_*C1*pfunct(A)*(unitnormal(1)*delta_vel(0)+unitnormal(0)*delta_vel(1));
        }
      }
      else if(nsd==3)
      {
        ...
        for(int A=0;A<piel;++A)
        {
          vec_r_o_n_u_minus_g(A*numstressdof_  )-=fac_*C1*pfunct(A)*unitnormal(0)*delta_vel(0);
          vec_r_o_n_u_minus_g(A*numstressdof_+1)-=fac_*C1*pfunct(A)*unitnormal(1)*delta_vel(1);
          vec_r_o_n_u_minus_g(A*numstressdof_+2)-=fac_*C1*pfunct(A)*unitnormal(2)*delta_vel(2);

          vec_r_o_n_u_minus_g(A*numstressdof_+3)-=fac_*C1*pfunct(A)*(unitnormal(1)*delta_vel(0)+unitnormal(0)*delta_vel(1));
          vec_r_o_n_u_minus_g(A*numstressdof_+4)-=fac_*C1*pfunct(A)*(unitnormal(2)*delta_vel(0)+unitnormal(0)*delta_vel(2));
          vec_r_o_n_u_minus_g(A*numstressdof_+5)-=fac_*C1*pfunct(A)*(unitnormal(2)*delta_vel(1)+unitnormal(1)*delta_vel(2));
        }
      }
      ...
      */
      if(nsd==2)
      {
        for(int A=0;A<piel;++A)
        {
          for(int B=0;B<piel;++B)
          {
            mat_r_o_n_u(A*numstressdof_  ,B*nsd  )-=fac_*C2*pfunct(A)*unitnormal(0)*unitnormal(0)*pfunct(B)*unitnormal(0);
            mat_r_o_n_u(A*numstressdof_  ,B*nsd+1)-=fac_*C2*pfunct(A)*unitnormal(0)*unitnormal(0)*pfunct(B)*unitnormal(1);

            mat_r_o_n_u(A*numstressdof_+1,B*nsd  )-=fac_*C2*pfunct(A)*unitnormal(1)*unitnormal(1)*pfunct(B)*unitnormal(0);
            mat_r_o_n_u(A*numstressdof_+1,B*nsd+1)-=fac_*C2*pfunct(A)*unitnormal(1)*unitnormal(1)*pfunct(B)*unitnormal(1);

            mat_r_o_n_u(A*numstressdof_+2,B*nsd  )-=fac_*C2*pfunct(A)*unitnormal(0)*unitnormal(1)*pfunct(B)*unitnormal(0);
            mat_r_o_n_u(A*numstressdof_+2,B*nsd+1)-=fac_*C2*pfunct(A)*unitnormal(0)*unitnormal(1)*pfunct(B)*unitnormal(1);
          }
        }
        ...
        for(int A=0;A<piel;++A)
        {
          vec_r_o_n_u_minus_g(A*numstressdof_  )-=fac_*C2*pfunct(A)*unitnormal(0)*unitnormal(0)*u_o_n;
          vec_r_o_n_u_minus_g(A*numstressdof_+1)-=fac_*C2*pfunct(A)*unitnormal(1)*unitnormal(1)*u_o_n;

          vec_r_o_n_u_minus_g(A*numstressdof_+2)-=fac_*C2*pfunct(A)*2*unitnormal(0)*unitnormal(1)*u_o_n;
        }
      }
      else if(nsd==3)
      {
        ...
        for(int A=0;A<piel;++A)
        {
          vec_r_o_n_u_minus_g(A*numstressdof_  )-=fac_*C2*pfunct(A)*unitnormal(0)*unitnormal(0)*u_o_n;
          vec_r_o_n_u_minus_g(A*numstressdof_+1)-=fac_*C2*pfunct(A)*unitnormal(1)*unitnormal(1)*u_o_n;
          vec_r_o_n_u_minus_g(A*numstressdof_+2)-=fac_*C2*pfunct(A)*unitnormal(2)*unitnormal(2)*u_o_n;

          vec_r_o_n_u_minus_g(A*numstressdof_+3)-=fac_*C2*pfunct(A)*2*unitnormal(0)*unitnormal(1)*u_o_n;
          vec_r_o_n_u_minus_g(A*numstressdof_+4)-=fac_*C2*pfunct(A)*2*unitnormal(0)*unitnormal(2)*u_o_n;
          vec_r_o_n_u_minus_g(A*numstressdof_+5)-=fac_*C2*pfunct(A)*2*unitnormal(1)*unitnormal(2)*u_o_n;
        }
      }
...
else dserror("Material type is not supported for boundary element with parent-element evaluation!");
