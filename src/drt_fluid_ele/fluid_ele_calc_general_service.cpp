    ...
    Epetra_SerialDenseVector& elevec2, Epetra_SerialDenseVector& elevec3)
{
  ...
  switch (act)
  {
    ...
    case FLD::calc_mat_deriv_u_and_rot_u:
    {
      ...
      return CalcMatDerivAndRotU(ele, params, discretization, lm, elevec1, elevec2, elevec3);
    }
    break;
    case FLD::calc_press_grad_and_div_eps:
    {
      // calculate pressure gradient and stress term at specified element coordinates
      return CalcPressGradAndDivEps(ele, params, discretization, lm, elevec1, elevec2);
    }
    break;
    ...
    case FLD::least_squares_matrix_rhs_immersed_boundary:
    {
      ...
      return GetLeastSquaresMatrixImmersedBoundary(
          params, ele, discretization, lm, elemat1, elevec1);
    }
    break;
 ...
 *---------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype, DRT::ELEMENTS::Fluid::EnrichmentType enrtype>
int DRT::ELEMENTS::FluidEleCalc<distype, enrtype>::CalcMatDerivAndRotU(DRT::ELEMENTS::Fluid* ele,
 ...
 *---------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype, DRT::ELEMENTS::Fluid::EnrichmentType enrtype>
int DRT::ELEMENTS::FluidEleCalc<distype, enrtype>::CalcPressGradAndDivEps(DRT::ELEMENTS::Fluid* ele,
    Teuchos::ParameterList& params, DRT::Discretization& discretization, std::vector<int>& lm,
    Epetra_SerialDenseVector& elevec1, Epetra_SerialDenseVector& elevec2)
{
  //--------------------------------------------------------------------------------
  ...
  const Teuchos::RCP<Epetra_MultiVector> velocity_gradient =
      params.get<Teuchos::RCP<Epetra_MultiVector>>("velgradient");
  ...
  elevec2[0] =
      2.0 * evelgrad2(0, 0) + evelgrad2(1, 1) + evelgrad2(3, 1) + evelgrad2(2, 2) + evelgrad2(6, 2);
  elevec2[1] =
      evelgrad2(3, 0) + evelgrad2(1, 0) + 2.0 * evelgrad2(4, 1) + evelgrad2(5, 2) + evelgrad2(7, 2);
  elevec2[2] =
      evelgrad2(6, 0) + evelgrad2(2, 0) + evelgrad2(7, 1) + evelgrad2(5, 1) + 2.0 * evelgrad2(8, 2);
  elevec2.Scale(0.5);
 ...
 *---------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype, DRT::ELEMENTS::Fluid::EnrichmentType enrtype>
int DRT::ELEMENTS::FluidEleCalc<distype, enrtype>::ComputeVolumeIntegral(DRT::ELEMENTS::Fluid* ele,
    Teuchos::ParameterList& params, DRT::Discretization& discretization, std::vector<int>& lm,
    Epetra_SerialDenseVector& elevec1)
{
  ...
  for (int gp = 0; gp < intpoints.nquad; gp++)
  {
    ...
    for (int d = 0; d < nsd_; ++d)
    {
      if (abs(x_gp(d) - pos(d)) < influence)
        integrand *=
            15.0 / 16.0 *
            ((pow((x_gp(d) - pos(d)), 4.0) / pow(influence, 5.0)) -
                2.0 * (pow((x_gp(d) - pos(d)), 2.0) / pow(influence, 3.0)) + 1.0 / influence);
      else
      {
        integrand = 0.0;
        break;
...
int DRT::ELEMENTS::FluidEleCalc<distype, enrtype>::FluidFractionProjection(
    ...
    std::vector<int>& lm, Epetra_SerialDenseMatrix& elemat1, Epetra_SerialDenseVector& elevec1)
{
        ...
        if (nsd_ == 2)
        {
          u(0) =
              (2.0 * L * R * U - (3.0 * (std::pow(L, 2.0)) * (std::pow(U, 2.0)) * mu) / (K0 * R)) /
                  (2.0 * L * R) +
              (3.0 * U * std::cos((y * M_PI) / R) *
                  (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
              (3.0 * U * std::cos((2.0 * y * M_PI) / R) *
                  (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                  (4.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) +
              (U * std::cos((3.0 * y * M_PI) / R) *
                  (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                  (3.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
              (3.0 * L * (std::pow(U, 2.0)) * mu * std::sin((2.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * M_PI) -
              (3.0 * L * (std::pow(U, 2.0)) * mu * std::sin((4.0 * x * M_PI) / L)) /
                  (2.0 * K0 * (std::pow(R, 2.0)) * M_PI) -
              (L * (std::pow(U, 2.0)) * mu * std::sin((6.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * M_PI) -
              (18.0 * L * (std::pow(U, 2.0)) * mu * std::cos((y * M_PI) / R) *
                  std::sin((2.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) -
              (9.0 * L * (std::pow(U, 2.0)) * mu * std::cos((y * M_PI) / R) *
                  std::sin((4.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) +
              (9.0 * L * (std::pow(U, 2.0)) * mu * std::cos((2.0 * y * M_PI) / R) *
                  std::sin((2.0 * x * M_PI) / L)) /
                  (2.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) -
              (2.0 * L * (std::pow(U, 2.0)) * mu * std::cos((3.0 * y * M_PI) / R) *
                  std::sin((2.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) -
              (6.0 * L * (std::pow(U, 2.0)) * mu * std::cos((y * M_PI) / R) *
                  std::sin((6.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) +
              (9.0 * L * (std::pow(U, 2.0)) * mu * std::cos((2.0 * y * M_PI) / R) *
                  std::sin((4.0 * x * M_PI) / L)) /
                  (4.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) -
              (L * (std::pow(U, 2.0)) * mu * std::cos((3.0 * y * M_PI) / R) *
                  std::sin((4.0 * x * M_PI) / L)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) +
              (3.0 * L * (std::pow(U, 2.0)) * mu * std::cos((2.0 * y * M_PI) / R) *
                  std::sin((6.0 * x * M_PI) / L)) /
                  (2.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0))) -
              (2.0 * L * (std::pow(U, 2.0)) * mu * std::cos((3.0 * y * M_PI) / R) *
                  std::sin((6.0 * x * M_PI) / L)) /
                  (3.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 3.0)));
          u(1) = 0.0;
          p = (2.0 * L * R * p0 +
                  (L * (std::pow(R, 2.0)) *
                      ...
                      (K0 * (std::pow(R, 3.0)))) /
                  (2.0 * L * R) +
              (6.0 * (std::pow(U, 2.0)) * (std::pow(mu, 2.0)) * std::cos((y * M_PI) / R)) /
                  (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
              (3.0 * (std::pow(U, 2.0)) * (std::pow(mu, 2.0)) * std::cos((2.0 * y * M_PI) / R)) /
                  (2.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) +
              (2.0 * (std::pow(U, 2.0)) * (std::pow(mu, 2.0)) * std::cos((3.0 * y * M_PI) / R)) /
                  (3.0 * K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
              (9.0 * (std::pow(L, 2.0)) * (std::pow(U, 2.0)) * (std::pow(mu, 2.0)) *
                  std::cos((2.0 * x * M_PI) / L)) /
                  (2.0 * K0 * (std::pow(R, 4.0)) * (std::pow(M_PI, 2.0))) -
              (9.0 * (std::pow(L, 2.0)) * (std::pow(U, 2.0)) * (std::pow(mu, 2.0)) *
                  std::cos((4.0 * x * M_PI) / L)) /
                  (8.0 * K0 * (std::pow(R, 4.0)) * (std::pow(M_PI, 2.0))) -
              ((std::pow(L, 2.0)) * (std::pow(U, 2.0)) * (std::pow(mu, 2.0)) *
                  std::cos((6.0 * x * M_PI) / L)) /
                  (2.0 * K0 * (std::pow(R, 4.0)) * (std::pow(M_PI, 2.0))) +
              (3.0 * L * U * mu * std::sin((2.0 * x * M_PI) / L) *
                  (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                  (2.0 * K0 * (std::pow(R, 4.0)) * M_PI) +
              (3.0 * L * U * mu * std::sin((4.0 * x * M_PI) / L) *
                  (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                  (4.0 * K0 * (std::pow(R, 4.0)) * M_PI) +
              (L * U * mu * std::sin((6.0 * x * M_PI) / L) *
                  (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                  (2.0 * K0 * (std::pow(R, 4.0)) * M_PI);
          dervel(0, 0) = (9.0 * (std::pow(U, 2.0)) * mu * std::cos((2.0 * x * M_PI) / L) *
                             std::cos((2.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
                         (6.0 * (std::pow(U, 2.0)) * mu * std::cos((4.0 * x * M_PI) / L)) /
                             (K0 * (std::pow(R, 2.0))) -
                         (6.0 * (std::pow(U, 2.0)) * mu * std::cos((6.0 * x * M_PI) / L)) /
                             (K0 * (std::pow(R, 2.0))) -
                         (36.0 * (std::pow(U, 2.0)) * mu * std::cos((2.0 * x * M_PI) / L) *
                             std::cos((y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
                         (6.0 * (std::pow(U, 2.0)) * mu * std::cos((2.0 * x * M_PI) / L)) /
                             (K0 * (std::pow(R, 2.0))) -
                         (36.0 * (std::pow(U, 2.0)) * mu * std::cos((4.0 * x * M_PI) / L) *
                             std::cos((y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
                         (4.0 * (std::pow(U, 2.0)) * mu * std::cos((2.0 * x * M_PI) / L) *
                             std::cos((3.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) +
                         (9.0 * (std::pow(U, 2.0)) * mu * std::cos((4.0 * x * M_PI) / L) *
                             std::cos((2.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
                         (36.0 * (std::pow(U, 2.0)) * mu * std::cos((6.0 * x * M_PI) / L) *
                             std::cos((y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
                         (4.0 * (std::pow(U, 2.0)) * mu * std::cos((4.0 * x * M_PI) / L) *
                             std::cos((3.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) +
                         (9.0 * (std::pow(U, 2.0)) * mu * std::cos((6.0 * x * M_PI) / L) *
                             std::cos((2.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0))) -
                         (4.0 * (std::pow(U, 2.0)) * mu * std::cos((6.0 * x * M_PI) / L) *
                             std::cos((3.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 2.0)) * (std::pow(M_PI, 2.0)));
          dervel(0, 1) = (3.0 * U * std::sin((2.0 * y * M_PI) / R) *
                             (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                             (2.0 * K0 * (std::pow(R, 3.0)) * M_PI) -
                         (3.0 * U * std::sin((y * M_PI) / R) *
                             (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                             (K0 * (std::pow(R, 3.0)) * M_PI) -
                         (U * std::sin((3.0 * y * M_PI) / R) *
                             (2.0 * K0 * (std::pow(R, 2.0)) - 3.0 * L * U * mu)) /
                             (K0 * (std::pow(R, 3.0)) * M_PI) +
                         (18.0 * L * (std::pow(U, 2.0)) * mu * std::sin((2.0 * x * M_PI) / L) *
                             std::sin((y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) -
                         (9.0 * L * (std::pow(U, 2.0)) * mu * std::sin((2.0 * x * M_PI) / L) *
                             std::sin((2.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) +
                         (9.0 * L * (std::pow(U, 2.0)) * mu * std::sin((4.0 * x * M_PI) / L) *
                             std::sin((y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) +
                         (6.0 * L * (std::pow(U, 2.0)) * mu * std::sin((2.0 * x * M_PI) / L) *
                             std::sin((3.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) -
                         (9.0 * L * (std::pow(U, 2.0)) * mu * std::sin((4.0 * x * M_PI) / L) *
                             std::sin((2.0 * y * M_PI) / R)) /
                             (2.0 * K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) +
                         (6.0 * L * (std::pow(U, 2.0)) * mu * std::sin((6.0 * x * M_PI) / L) *
                             std::sin((y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) +
                         (3.0 * L * (std::pow(U, 2.0)) * mu * std::sin((4.0 * x * M_PI) / L) *
                             std::sin((3.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) -
                         (3.0 * L * (std::pow(U, 2.0)) * mu * std::sin((6.0 * x * M_PI) / L) *
                             std::sin((2.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0))) +
                         (2.0 * L * (std::pow(U, 2.0)) * mu * std::sin((6.0 * x * M_PI) / L) *
                             std::sin((3.0 * y * M_PI) / R)) /
                             (K0 * (std::pow(R, 3.0)) * (std::pow(M_PI, 2.0)));
          dervel(1, 0) = 0.0;
          dervel(1, 1) = 0.0;
    ...
    for (int node = 0; node < nen_; node++)
    {
      ...
      IMMERSED::InterpolateToBackgrdPoint<DRT::Element::hex8,  // source/structure
          ...
          (curr_subset_of_structdis,
              ...
              *ele, backgrdxi, targeteledisp, "none",
              dummy,  // result
              match, false,
...
)
{
  ...
  if (distype != DRT::Element::hex8)
    dserror(
        "immersed partitioned fsi with membrane finite element currently just implemented for hex8 "
        "fluid discretisation type!");
  ...
  // search for immersed elements within a certain radius around the searchcenter node
  curr_subset_of_structdis = struct_searchtree->searchElementsInRadius(
      *structdis, *currpositions_struct, searchcenter, radius * searchradiusfac, 0);

  // check if a structural node or gauss point lies inside the current fluid element
  // for immersed boundary fluid elements set the flags SetIsImmersed(1) and
  // SetHasProjectedDirichlet(1)
...
)
{
  ...
  // setup parameters for search of (potentially) immersed structural elements
  static double searchradiusfac_struct =
      globalproblem->ImmersedMethodParams().get<double>("FLD_SRCHRADIUS_FAC");
  ...
  // search for immersed elements within a certain radius around the searchcenter node
  curr_subset_of_structdis = struct_searchtree->searchElementsInRadius(
      *structdis, *currpositions_struct, searchcenter, radius * searchradiusfac_struct, 0);

  // fill entries of element least squares matrix and vector containing structural point position
  // and velocity
  IMMERSED::GetInFluidEleImmersedPoints<DRT::Element::quad4,  // structure
      DRT::Element::hex8>                                     // fluid
      (curr_subset_of_structdis,                              // relevant structural elements
    ...
    Epetra_SerialDenseVector& elevec1_epetra)  // vectofill
{
  ...
  if (immersedele->IsImmersed())
  {
    ...
    const Teuchos::RCP<DRT::Discretization> structuredis =
         ...
         iquad != intpoints_.end(); ++iquad)
    {
      ...
      for (int node = 0; node < nen_; ++node)
      {
        elevec1_epetra((node * numdofpernode_) + nsd_) +=
            vdiv_ * fac_ * funct_(node) * porosity * rhsfac;
      }
    }
