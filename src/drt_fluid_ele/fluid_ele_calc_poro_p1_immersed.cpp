    ...
    : DRT::ELEMENTS::FluidEleCalcPoroP1<distype>::FluidEleCalcPoroP1()
{
  ...
  gp_iquad_ = -1234.0;
  artifical_flow_mod_type = DRT::INPUT::IntegralValue<int>(
      DRT::Problem::Instance()->CellMigrationParams(), "ARTIFICIAL_FLUID_MOD");
  ...
  if (artifical_flow_mod_type == INPAR::CELL::mod_full) mod_type_multiplicator_ = 1.0;
    ...
    bool offdiag)
{
  ...
  if (poroele) my_p::kintype_ = poroele->KinematicType();
  ...
  if (not immersedele_) dserror("you shouldn't be here");

  if (immersedele_->IsBoundaryImmersed())
  {
    if (not offdiag)  // evaluate diagonal block (pure fluid block)
      return my_p1::Evaluate(ele, discretization, lm, params, mat, elemat1_epetra, elemat2_epetra,
          elevec1_epetra, elevec2_epetra, elevec3_epetra, intpoints_fluid_bound);
    else  // evaluate off diagonal block (coupling block)
      return my_p1::EvaluateOD(ele, discretization, lm, params, mat, elemat1_epetra, elemat2_epetra,
          elevec1_epetra, elevec2_epetra, elevec3_epetra, intpoints_fluid_bound);
  }
  else
  {
...
void DRT::ELEMENTS::FluidEleCalcPoroP1Immersed<distype>::ComputePorosity(
    ...
    const LINALG::Matrix<my::nen_, 1>& shapfct, const LINALG::Matrix<my::nen_, 1>* myporosity,
    ...
    double* dphi_dpp, bool save)
{
  ...
  return my_p1::ComputePorosity(params, press, J, gp, shapfct, myporosity, porosity, dphi_dp,
      dphi_dJ, dphi_dJdp, dphi_dJJ, dphi_dpp, save);
    ...
    LINALG::Matrix<my::nen_, my::nen_>& ppmat, LINALG::Matrix<my::nen_, 1>& preforce)
{
  ...
  if (my_p::porofldpara_->IsStationaryConti() == false)
  {
    if (eporositydot)
    {
      ...
      if (immersedele_->IsImmersed() and artifical_flow_mod_type == INPAR::CELL::mod_full)
        porositydot = 0.0;
      ...
      my_p::rhscon_ =
          1.0 / my_p::fldparatimint_->Dt() / my_p::fldparatimint_->Theta() * my_p::histcon_;
    ...
    dserror(
        "Stationary conti equation is not implemented and not tested for\n"
    ...
    LINALG::Matrix<my::nen_, my::nen_>& ppmat, LINALG::Matrix<my::nen_, 1>& preforce)
{
  ...
  my_p::EvaluatePressureEquationNonTransient(params, timefacfacpre, rhsfac, dphi_dp, dphi_dJ,
  ...
  if (immersedele_->IsImmersed() and exchange_manager_->IsFluidInteraction() and
    ...
    if (my_p::porofldpara_->PoroContiPartInt() == false)
    {
      ...
      for (int idim = 0; idim < my::nsd_; ++idim)
      {
        ...
        for (int ui = 0; ui < my::nen_; ++ui)
        {
          ...
          {
            estif_q_u(vi, fui + idim) -=
                timefacfacpre * my::funct_(vi) *
                (my_p::porosity_ * derxy_idim_ui);  // phi*div(v^f) (remove) Dv^f
            estif_q_u(vi, fui + idim) -= timefacfacpre * my::funct_(vi) * grad_porosity_idim *
                                         funct_ui *
                                         mod_type_multiplicator_;  // grad(phi)*v^f (remove) Dv^f
          }
        }
      }
      ...
      for (int vi = 0; vi < my::nen_; ++vi)
      {
        ...
        preforce(vi) += rhsfac * vel_grad_porosity * my::funct_(vi) *
                        mod_type_multiplicator_;  // grad(phi)*v^f (remove)
        preforce(vi) += rhsfac * my::funct_(vi) * (-grad_porosity_gridvelint) *
                        mod_type_multiplicator_;  // grad(phi)*v^s (remove)
        // preforce(vi) -=   rhsfac *my::funct_(vi) * my_p::porosity_ * my_p::gridvdiv_ *
        // mod_type_multiplicator_; // phi*div(v^s) (add)
      }  // vi
    }
    else  // my_p::porofldpara_->PoroContiPartInt() == true
      dserror(
          "No adaption of interstitial flow to compressibility of immersed body implemented\n"
          "for partially integrated poro P1 formulation!\n"
          "Set CONTIPARTINT to 'No' in ---POROELASTICITY DYNAMIC section.");
  }
  else if (immersedele_->IsBoundaryImmersed() and gp_has_projected_divergence and
           exchange_manager_->IsFluidInteraction() and
           artifical_flow_mod_type != INPAR::CELL::mod_none)
  {
    for (int vi = 0; vi < my::nen_; ++vi)
      preforce(vi) += rhsfac * immersedele_->ProjectedIntPointDivergence(gp_iquad_) *
                      my_p::porosity_ * my::funct_(vi);
...
void DRT::ELEMENTS::FluidEleCalcPoroP1Immersed<distype>::GaussPointLoopP1OD(
    ...
    LINALG::Matrix<my::nen_, my::nen_>& ecouplp1_p, Teuchos::RCP<const MAT::Material> material,
    const DRT::UTILS::GaussIntegration& intpoints)
{
  my_p1::GaussPointLoopP1OD(params, ebofoaf, evelaf, evelnp, eveln, epreaf, eprenp, epren, emhist,
  ...
  if (immersedele_->IsImmersed() and artifical_flow_mod_type != INPAR::CELL::mod_none)
  {
         ...
         iquad != intpoints.end(); ++iquad)
    {
      ...
      for (int ui = 0; ui < my_p::nen_; ++ui)
        for (int vi = 0; vi < my_p::nen_; ++vi)
          ecouplp1_p(vi, ui) -=
              my_p::fac_ * my_p::funct_(vi) * my_p::funct_(ui) * mod_type_multiplicator_;
