    ...
    PostVtkWriter(field, filename)
{
  for (size_t i = 0; i < sizeof(origin_) / sizeof(origin_[0]); ++i)
    origin_[i] = 0.0;
  for (size_t i = 0; i < sizeof(spacing_) / sizeof(spacing_[0]); ++i)
    spacing_[i] = 0.0;
  for (size_t i = 0; i < sizeof(globalextent_) / sizeof(globalextent_[0]); ++i)
    globalextent_[i] = 0;
  for (size_t i = 0; i < sizeof(localextent_) / sizeof(localextent_[0]); ++i)
    localextent_[i] = 0;
}
    ...
    const bool fillzeros)
{
  ...
  if(fillzeros)
    solution.resize(ncomponents*((localextent_[1]-localextent_[0]+1)*(localextent_[3]-localextent_[2]+1)*(localextent_[5]-localextent_[4]+1)), 0.0);
  else
...
void PostVtiWriter::WriterPrepTimestep()
{
  ...
  for (int i=0; i<3; ++i)
    spacing_[i] = (*(collected_coords[i].rbegin()) - *(collected_coords[i].begin()))/(collected_coords[i].size() - 1);
  for (int i=0; i<3; ++i) {
    int k = 0;
    for (set_tol::const_iterator it = collected_coords[i].begin();
         it != collected_coords[i].end(); ++it, ++k)
      if (*it - lorigin[i] - k * spacing_[i] >  TOL_N ||
          *it - lorigin[i] - k * spacing_[i] < -TOL_N)
        dserror("The mesh is not a uniform rectangular grid: grid coordinate[%d]: %lf, "
                "node coordinate[%d]: %lf, difference: %e", i, lorigin[i] + k * spacing_[i],
                i , *it, *it - lorigin[i] - k*spacing_[i]);
  }
