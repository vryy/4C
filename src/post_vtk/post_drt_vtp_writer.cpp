...
VtpWriter::WriteGeo()
{
  ...
  if ( not result->next_result("displacement") )
    dserror("first displacement result is missing");
  ...
  for (int n=0; n<disp->MyLength(); ++n)
    coordinates.push_back((*disp)[n]);
    ...
    const bool fillzeros)
{
  ...
  for (int lid=0; lid<data->MyLength(); ++lid)
    solution.push_back((*data)[lid]);
