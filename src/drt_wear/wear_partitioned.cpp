    ...
    Algorithm(comm)
{
  ...
  wearincr_ = Teuchos::rcp(
      new Epetra_Vector(
          *AleField().Interface()->Map(AleField().Interface()->cond_ale_wear)),
      true);
  delta_ale_ = Teuchos::rcp(
      new Epetra_Vector(AleField().Dispnp()->Map(), true));
...
bool WEAR::Partitioned::ConvergenceCheck(int iter)
{
  ...
  if (Comm().MyPID() == 0)
  {
    std::cout << "-----------------" << " Step " << iter + 1
        << " --------------------" << std::endl;
    std::cout << "Wear incr.= " << Wincr << "         ALE incr.= " << ALEincr
        << std::endl;
    std::cout << "---------------------------------------------" << std::endl;
  }
  ...
  if (iter > 50)
    dserror(
        "Staggered solution scheme for ale-wear problem unconverged within 50 nonlinear iteration steps!");
    ...
    Teuchos::RCP<Epetra_Vector>& disinterface_m)
{
  ...
  if (realwear == Teuchos::null)
    dserror("STRU_ALE::Algorithm::InterfaceDisp: realwear = Teuchos::null");
  ...
  if (interface.size() > 1)
  {
    std::cout << "*** Warning: more than 1 contact interface with wear ***"
        << std::endl;
  ...
  {
    Teuchos::RCP<CONTACT::WearInterface> winterface = Teuchos::rcp_dynamic_cast<
        CONTACT::WearInterface>(interface[m]);
    if (winterface == Teuchos::null)
      dserror("Casting to WearInterface returned null!");
    ...
    for (int i = 0; i < disinterface_s->MyLength(); ++i)
    {
      if (i % dim == 0)
      {
        if (cstrategy.ContactWear() == Teuchos::null)
          dserror("unweighted wear vector returned null-pointer");
...
void WEAR::Partitioned::ApplyMeshDisplacement(bool iterated)
{
  ...
  for (int k = 0; k < StructureField()->Discretization()->NumMyRowNodes(); ++k)
  {
    ...
    if (ndim == 2)
    {
      ...
      if (locid == -1)
        dserror("LID not found on this proc");
    }
    else
    {
