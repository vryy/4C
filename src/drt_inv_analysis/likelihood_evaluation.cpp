/*----------------------------------------------------------------------*/
/*!
 * \file likelihood_evaluation.cpp
...
/*----------------------------------------------------------------------*/
void INVANA::LogLikeMixture::Init(Teuchos::RCP<INVANA::InvanaBase> posterior,
    Teuchos::RCP<INVANA::LogLikePrior> prior)
{
  ...
  if (prob_scaling_)
    prob_scale_fac_ = eval_post_->ObjectiveFunct()->GetScaleFac();

  return;
}

/*----------------------------------------------------------------------*/
int INVANA::LogLikeMixture::EvaluateMixture(const Epetra_Vector& state,
    double& val_posterior, double& val_prior, Epetra_Vector& solution)
{
...
/*----------------------------------------------------------------------*/
int INVANA::LogLikeMixture::EvaluateMixture(const Epetra_Vector& state,
    double& val_posterior, double& val_prior)
{
...
/*----------------------------------------------------------------------*/
const Epetra_Map& INVANA::LogLikeMixture::StateMap()
{
  return *(eval_post_->VectorRowLayout());
}
...
/*----------------------------------------------------------------------*/
void INVANA::LogLikeMixture::DrawProposal(const Epetra_Vector& mean,
    const double& fac, Epetra_Vector& draw)
{
...
void INVANA::LogLikePrior::DrawfromPrior(Epetra_Vector& draw)
{
  ...
  for (int i=0; i<sample.MyLength(); i++)
    values[i] = distribution_(generator_);
...
/*----------------------------------------------------------------------*/
void INVANA::LogLikePrior::DrawProposal(const Epetra_Vector& mean,
    const double& fac, Epetra_Vector& draw)
{
