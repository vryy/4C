...
output_(Teuchos::null)
{
  ...
  params_ = Teuchos::rcp(new Epetra_MultiVector(*(matman_->GetParams())));
...
void STR::INVANA::StatInvAnalysis::EvaluateGradient(Epetra_MultiVector* objgrad)
{
  //zero out gradient vector initially
  objgrad->PutScalar(0.0);
  ...
  for (int j=0; j<msteps_; j++)
  {
    ...
    matman_->Evaluate(Teuchos::rcp(objgrad,false), Teuchos::rcp((*disdual_)(j),false));
  }
