...
void Inv_analysis::evaluate()
{
 ...
 // get_final_displacment(final_displacment0_, 0);
 // get_final_displacment(final_displacment1_, 1);
 // get_final_displacment(final_displacment2_, 2);
...
*/
 ...
 cout << "Storage Determinante: " << storage_det << endl;
	...
	storage[2][0] = storage[2][0]*J[0][0] + storage[2][1]*J[0][1] + storage[2][2]*J[0][2];
	storage[2][1] = storage[2][0]*J[1][0] + storage[2][1]*J[1][1] + storage[2][2]*J[1][2];
	storage[2][2] = storage[2][0]*J[2][0] + storage[2][1]*J[2][1] + storage[2][2]*J[2][2];
	...
	for (unsigned int i=0; i<3; i++) {
	  for (unsigned int j=0; j<3; j++) {
	    delta_p_[i] = delta_p_[i]+storage[i][j];
			//  delta_p_[i] = storage[i][j]*J[j];
		}
	  delta_p_[i] = delta_p_[i] * residual_disp_[i];
	}*/
