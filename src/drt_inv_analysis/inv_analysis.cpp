...
void Inv_analysis::get_disp_curve(const RefCountPtr<Epetra_Vector> disp,  int numb)
{
  ...
  if 	(abs(sum_x) > abs(sum_y) && abs(sum_x) > abs(sum_z)) 	disp_curve_[numb]= sum_x/final_disp_x.size();
  else if (abs(sum_y) > abs(sum_x) && abs(sum_y) > abs(sum_z)) 	disp_curve_[numb]= sum_y/final_disp_y.size();
  else 								disp_curve_[numb]= sum_z/final_disp_z.size();
...
void Inv_analysis::calculate_new_parameters()
{
  ...
  {
    p_o_[i] = p_[i];
    if          (i==0)  p_[i]   = p_[i] - (abs(residual_disp_[i])/((abs(residual_disp_[0])+abs(residual_disp_[1])+abs(residual_disp_[2]))/3))*delta_p_[i];
    else if     (i==0)  p_[i]   = p_[i] - (abs(residual_disp_[i])/((abs(residual_disp_[0])+abs(residual_disp_[1])+abs(residual_disp_[2]))/3))*delta_p_[i];
    else                p_[i]   = p_[i] - (abs(residual_disp_[i])/((abs(residual_disp_[0])+abs(residual_disp_[1])+abs(residual_disp_[2]))/3))*delta_p_[i];
  }
  ...
  cout << endl;
  cout << "NEW!!! current Parameters: \t\t\t"<< p_[0] << " \t" << p_[1] << " \t" << p_[2] << endl;

  cout << endl;

  cout << "Displacment Curve:" << endl;


  int nstep   = params_.get<int>   ("nstep",5);
  for (unsigned int i=0; i<nstep; i++)
  {
    cout << disp_curve_[i] << endl;
  }
