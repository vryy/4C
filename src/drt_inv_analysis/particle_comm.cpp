...
/*----------------------------------------------------------------------*/
INVANA::ParticleComm::ParticleComm() :
lnumparticles_(0),
group_(-1)
{}

/*----------------------------------------------------------------------*/
void INVANA::ParticleComm::Init(Teuchos::RCP<Epetra_Comm> gcomm,
    Teuchos::RCP<Epetra_Comm> lcomm, int lnumparticles, int group)
{
  gcomm_ = gcomm;
  lcomm_ = lcomm;
...
void INVANA::ParticleComm::Setup()
{
  ...
  if (icomm_->MyPID() != group_)
    dserror("Mess up in the intergroup communicator!");
