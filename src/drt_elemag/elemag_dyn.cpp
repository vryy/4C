...
void electromagnetics_drt()
{
  ...
  Teuchos::RCP<DRT::DiscretizationHDG> elemagdishdg = Teuchos::rcp_dynamic_cast<DRT::DiscretizationHDG>(problem->GetDis("elemag"));
  if (elemagdishdg == Teuchos::null) dserror("Failed to cast DRT::Discretization to DRT::DiscretizationHDG.");
  ...
  {
    std::cout << "---------------------------------------------------------------------------------" << std::endl;
    std::cout << "---------- You are now about to enter the module for electromagnetics! ----------" <<std::endl;
    std::cout << "---------------------------------------------------------------------------------" << std::endl;
  }
  ...
  // build map
  const Teuchos::RCP<Epetra_IntVector> eledofs = Teuchos::rcp(new Epetra_IntVector(*elemagdishdg->ElementColMap()));
  //for(int i=0; i<elemagdishdg->NumMyColElements(); ++i)
  //{
  //  (*eledofs)[i] = dynamic_cast<DRT::ELEMENTS::Elemag*>(elemagdishdg->lColElement(i))->NumDofPerElementAuxiliary();
  //}
  ...
  if (linsolvernumber_elemag == (-1)) dserror("There is not any linear solver defined for electromagnetic problem. Please set LINEAR_SOLVER in ELECTROMAGNETIC DYNAMIC to a valid number!");
  Teuchos::RCP<LINALG::Solver> solver = Teuchos::rcp(new LINALG::Solver(problem->SolverParams(linsolvernumber_elemag),comm,problem->ErrorFile()->Handle()));
  ...
  Teuchos::RCP<Teuchos::ParameterList> params = Teuchos::rcp(new Teuchos::ParameterList(elemagparams));
  ...
  INPAR::ELEMAG::DynamicType elemagdyna = DRT::INPUT::IntegralValue<INPAR::ELEMAG::DynamicType>(elemagparams,"TIMEINT");
