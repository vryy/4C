...
#include "opti_topopt_fluidAdjoint3_boundary.H"
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
DRT::ELEMENTS::FluidAdjoint3BoundaryImplInterface*
DRT::ELEMENTS::FluidAdjoint3BoundaryImplInterface::Impl(const DRT::Element* ele)
{
  switch (ele->Shape())
  {
    ...
    Epetra_SerialDenseVector& elevec)
{
  ...
  *----------------------------------------------------------------------*/
  for (DRT::UTILS::GaussIntegration::const_iterator iquad = intpoints_.begin();
       iquad != intpoints_.end(); ++iquad)
  {
    if (params.get<INPAR::TOPOPT::AdjointCase>("special test case") ==
    ...
    else  // special cases
    {
      ...
      INPAR::TOPOPT::AdjointCase testcase =
          params.get<INPAR::TOPOPT::AdjointCase>("special test case");
      ...
      switch (testcase)
      {
        ...
        case INPAR::TOPOPT::adjointtest_stat_quad_vel_lin_pres:
        {
          ...
          values(1) = values_old(1) =
              -3 * x * x * x + 4 * y * y * y - 6 * x * x * y + 2 * x * y * y - 6 * x;
          break;
        }
        case INPAR::TOPOPT::adjointtest_stat_all_terms_all_constants:
        {
          ...
          values(1) = values_old(1) =
              -6 * x * x * x - 12 * x * x * y + 4 * x * y * y + 8 * y * y * y - 15 * x;
          break;
        }
        ...
        {
          double t = fldAdPara_->Time();
          values(0) = 2 * x * x * x + 4 * x * x * y + 2 * x * x * y * t + 4 * x * y * y * t +
                      18 * y + 3 * y * t - 24 * y * t * t;
          values(1) = -6 * x * x * x - 12 * x * x * y + 4 * x * y * y * t * t +
                      8 * y * y * y * t * t - 18 * x + 3 * x * t;

          t += fldAdPara_->Dt();  // old time = t + dt
          values_old(0) = 2 * x * x * x + 4 * x * x * y + 2 * x * x * y * t + 4 * x * y * y * t +
                          18 * y + 3 * y * t - 24 * y * t * t;
          values_old(1) = -6 * x * x * x - 12 * x * x * y + 4 * x * y * y * t * t +
                          8 * y * y * y * t * t - 18 * x + 3 * x * t;
          break;
        }
        ...
        {
          double t = fldAdPara_->Time();
          values(0) = 3 * x * y * t * t + 3 * x * x * y * t + 5.5 * x * x + 6 * x * y * y * t +
                      6 * x * y + 2 * y * t + 4 - 1.5 * y * y + 3 * y * t * t + 5.5 * y * y * t;
          values(1) = 3 * y * t * t - 3 * x * t * t + 6 * y * y * t - 3 * x * y * t - 2 * t +
                      2 * x * t - 3 * x * x * t;

          t += fldAdPara_->Dt();  // old time = t + dt
          values_old(0) = 3 * x * y * t * t + 3 * x * x * y * t + 5.5 * x * x + 6 * x * y * y * t +
                          6 * x * y + 2 * y * t + 4 - 1.5 * y * y + 3 * y * t * t + 5.5 * y * y * t;
          values_old(1) = 3 * y * t * t - 3 * x * t * t + 6 * y * y * t - 3 * x * y * t - 2 * t +
                          2 * x * t - 3 * x * x * t;
          break;
        }
...
)
{
  ...
  DRT::UTILS::ComputeMetricTensorForBoundaryEle<distype>(
      xyze_, deriv_, metrictensor, drs_, &unitnormal_);
