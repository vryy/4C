...
#include "opti_topopt_fluidAdjointImplTimeIntegration.H"
#include "opti_topopt_fluidAdjointResulttest.H"
 ...
 *----------------------------------------------------------------------*/
TOPOPT::ADJOINT::ImplicitTimeInt::ImplicitTimeInt(Teuchos::RCP<DRT::Discretization> actdis,
...
void TOPOPT::ADJOINT::ImplicitTimeInt::SolveStationaryProblem()
{
  ...
  while (step_ < stepmax_)
  {
    if (step_ > 1)
      dserror(
          "This analysis feature with a pseudo time loop and more than one pseudo time step\n"
          "is currently not implemented to work correctly and combine the right steps for the "
          "adjoints");
...
void TOPOPT::ADJOINT::ImplicitTimeInt::PrepareTimeStep()
{
  ...
  // -------------------------------------------------------------------
  {
    ...
    nbcparams.set(
        "special test case", params_->get<INPAR::TOPOPT::AdjointCase>("special test case"));
    ...
    {
      discret_->EvaluateCondition(nbcparams, neumann_loads_, "LineNeumann");
    }
    else if (DRT::Problem::Instance()->NDim() == 3)  // 3D -> 2D (surface) neumann surface
    {
      discret_->EvaluateCondition(nbcparams, neumann_loads_, "SurfaceNeumann");
    }
    else
    ...
    // -------------------------------------------------------------------
    {
      ...
      if (itnum != itemax)
      {
        ...
        if (nonlinearbc_)
        {
          ...
          discret_->EvaluateCondition(condparams, sysmat_, Teuchos::null, residual_, Teuchos::null,
              Teuchos::null, condstring);
          discret_->ClearState();
        }
    ...
    const int step, const double time, const bool inflow) const
{
  ...
  filename << filebase << "_adjoint.solution_velpres_" << std::setw(5) << std::setfill('0') << step
           << pid_stream.str() << ".pos";

  // delete old Gmsh postprocessing file
  int step_diff = 20;  // stepdiff files are kept
  std::ostringstream filenamedel;
  filenamedel << filebase << "_adjoint_" << std::setw(5) << std::setfill('0') << step - step_diff
              << pid_stream.str() << ".pos";
    ...
    const INPAR::TOPOPT::InitialAdjointField initfield, const int startfuncno)
{
  ...
  switch (initfield)
  {
    ...
    case INPAR::TOPOPT::initadjointfield_field_by_function:  // initial field by undisturbed
    {
      ...
      // if we have no test case we set the new solution equal to the old although we have a linear
      // system
      // -> smaller numerical errors around 10^-12
      // for test cases we start with zero field so that more terms are active and tested
      //    if (params_->get<INPAR::TOPOPT::AdjointTestCases>("special test case") ==
...
void TOPOPT::ADJOINT::ImplicitTimeInt::EvaluateDirichlet()
{
  ...
  if (params_->get<INPAR::TOPOPT::AdjointCase>("special test case") ==
      INPAR::TOPOPT::adjointtest_no)
  {
    ...
    for (int inode = 0; inode < discret_->NumMyRowNodes(); inode++)
    {
      ...
      velnp_->ReplaceGlobalValues(
          nsd, (double*)values, &gdofs[0]);  // &dofs[0] gives pointer to dofs
    }
  }
  else  // special cases
  {
    INPAR::TOPOPT::AdjointCase testcase =
        params_->get<INPAR::TOPOPT::AdjointCase>("special test case");
...
TOPOPT::ADJOINT::ImplicitTimeInt::BlockSystemMatrix() const
{
...
void TOPOPT::ADJOINT::ImplicitTimeInt::SetElementGeneralAdjointParameter() const
{
  ...
  eleparams.set<INPAR::TOPOPT::ObjectiveDissipation>(
      "dissipation", params_->get<INPAR::TOPOPT::ObjectiveDissipation>("OBJECTIVE_DISSIPATION"));
  ...
  eleparams.set<INPAR::TOPOPT::AdjointCase>(
      "special test case", params_->get<INPAR::TOPOPT::AdjointCase>("special test case"));
...
void TOPOPT::ADJOINT::ImplicitTimeInt::SetElementTimeParameter() const
{
  ...
  if (step_ == 0)
    eleparams.set("initial instationary step", true);
  else
    eleparams.set("initial instationary step", false);
  // call standard loop over elements
  discret_->Evaluate(
      eleparams, Teuchos::null, Teuchos::null, Teuchos::null, Teuchos::null, Teuchos::null);
  return;
...
// ----------------------------------------------------------------------------
void TOPOPT::ADJOINT::ImplicitTimeInt::Reset(int numsteps, int iter)
{
  const Epetra_Map* dofrowmap = discret_->DofRowMap();
