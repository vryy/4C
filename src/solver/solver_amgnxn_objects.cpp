...
(double a_V, const BlockedVector& V, double a_this)
{
  for(int i=0; i<GetNumBlocks(); i++)
    GetVector(i)->Update(a_V, *(V.GetVector(i)), a_this);
}
...
Teuchos::RCP<LINALG::SOLVER::AMGNXN::BlockedVector>
LINALG::SOLVER::AMGNXN::BlockedVector::NewRCP(bool ZeroIt) const
{
  ...
  for(int i=0; i<GetNumBlocks(); i++)
  {
    ...
    out->SetVector(Teuchos::rcp( new Epetra_MultiVector(*(this->GetVector(i)))),i);//This works, I don't know why...
  }
  return out;
}
...
void LINALG::SOLVER::AMGNXN::BlockedVector::PutScalar(double a)
{
  ...
  for(int i=0; i<GetNumBlocks(); i++)
    GetVector(i)->PutScalar(a);
}

/*------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------*/

LINALG::SOLVER::AMGNXN::BlockedVector
LINALG::SOLVER::AMGNXN::BlockedVector::GetBlockedVector
(const std::vector<int>& blocks) const
{

  int NumSubBlocks = blocks.size();
  if(NumSubBlocks > GetNumBlocks())
    dserror("Input error too long");
  ...
  for(int i=0; i<NumSubBlocks; i++ )
  {
    int j = blocks[i];
    if (j>=GetNumBlocks())
      dserror("The picked block id is too large");
    if (j<0)
      dserror("The picked block id is negative");
...
Teuchos::RCP<LINALG::SOLVER::AMGNXN::BlockedVector>
LINALG::SOLVER::AMGNXN::BlockedVector::GetBlockedVectorRCP
(const std::vector<int>& blocks) const
{
...
void LINALG::SOLVER::AMGNXN::BlockedMatrix::Apply(const BlockedVector& in, BlockedVector& out) const
{
  ...
  if(in.GetNumBlocks() != GetNumCols())
    dserror("Bad number of blocks in input vector");

  if(out.GetNumBlocks() != GetNumRows())
    dserror("Bad number of blocks in output vector");
  ...
  for(int i=0; i<GetNumRows(); i++)
  {
    ...
    if(out.GetVector(i)==Teuchos::null)
      dserror("Have you set your output error?");

    Teuchos::RCP<Epetra_MultiVector> Yi = out.GetVector(i);
    Yi->PutScalar(0.0);
    Teuchos::RCP<Epetra_MultiVector> Yitmp = Teuchos::rcp(new Epetra_MultiVector(Yi->Map(),Yi->NumVectors(),true));

    for(int j=0; j<GetNumCols(); j++)
    {

      if(in.GetVector(j)==Teuchos::null)
        dserror("Have you set your input error?");

      Teuchos::RCP<Epetra_MultiVector> Xj = in.GetVector(j);
      if(GetMatrix(i,j) == Teuchos::null)
        dserror("Have you set all the blocks?");
...
LINALG::SOLVER::AMGNXN::BlockedMatrix::GetBlockSparseMatrix(DataAccess access)
{
  ...
  for(int row=0;row<rows;row++)
    for(int col=1;col<cols;col++)
      if(!((matrices_[row*cols+col]->RangeMap()).SameAs(matrices_[row*cols+0]->RangeMap())))
        dserror("The range map must be the same for all matrices_ in the same row");

  for(int col=0;col<cols;col++)
    for(int row=1;row<rows;row++)
      if(!((matrices_[row*cols+col]->DomainMap()).SameAs(matrices_[0*cols+col]->DomainMap())))
  ...
  // Assign the blocks
    for(int i=0;i<rows;i++)
    {
      for(int j=0;j<cols;j++)
      {
        ...
        if (Aij==Teuchos::null)
          dserror("We need a SparseMatrix here!");
...
LINALG::SOLVER::AMGNXN::BlockedMatrix
LINALG::SOLVER::AMGNXN::BlockedMatrix::GetBlockedMatrix(
    const std::vector<int>& row_blocks,
    const std::vector<int>& col_blocks ) const
{

  int NumSubRows = row_blocks.size();
  if(NumSubRows > GetNumRows())
    dserror("Row input too long");

  int NumSubCols = col_blocks.size();
  if(NumSubCols > GetNumCols())
    dserror("Col input too long");
  ...
  for(int i=0; i<NumSubRows; i++ )
  {
    ...
    int is = row_blocks[i];
    if (is>=GetNumRows())
      dserror("The picked row block id is too large");
    if (is<0)
      dserror("The picked row block id is negative");

    for(int j=0; j<NumSubCols; j++)
    {
      int js = col_blocks[j];
      if (js>=GetNumCols())
        dserror("The picked col block id is too large");
      if (js<0)
        dserror("The picked col block id is negative");
...
void LINALG::SOLVER::AMGNXN::DiagonalBlockedMatrix::Apply(const BlockedVector& in, BlockedVector& out) const
{
  ...
  for(int i=0; i<GetNumRows(); i++)
  {
    ...
    if(out.GetVector(i)==Teuchos::null)
      dserror("Have you set your output error?");
    if(in.GetVector(i)==Teuchos::null)
      dserror("Have you set your input error?");
...
void LINALG::SOLVER::AMGNXN::BlockedMatrix::ParseBlocks(
...
LINALG::SOLVER::AMGNXN::BlockedMatrix::NewDomainBlockedVector(int NV, bool ZeroIt) const
{
  ...
  for(int i=0;i<GetNumCols();i++)
  {
    ...
    Teuchos::RCP<Epetra_MultiVector>  Vi = Teuchos::rcp(new Epetra_MultiVector(Map,NV,ZeroIt));//This constructor seems to be buggy inside function BlockedVector::NewRCP. I don't know why here works well.
