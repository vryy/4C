...
LINALG::SOLVER::TEKO::Teko_BACIMappingStrategy::Teko_BACIMappingStrategy(const LINALG::MultiMapExtractor& rangemaps,
...
//
void LINALG::SOLVER::TEKO::Teko_BACIMappingStrategy::copyEpetraIntoThyra(const Epetra_MultiVector& X,
                                                 const Teuchos::Ptr<Thyra::MultiVectorBase<double> > & thyra_X) const
{
   ...
   for(int i=0; i<rangemaps_.NumMaps(); i++)
   {
     Teuchos::RCP<Thyra::DefaultSpmdMultiVector<double> > vec
         = Teuchos::rcp_dynamic_cast<Thyra::DefaultSpmdMultiVector<double> >(prod_X->getNonconstMultiVectorBlock(i));
...
//
void LINALG::SOLVER::TEKO::Teko_BACIMappingStrategy::copyThyraIntoEpetra(const Teuchos::RCP<const Thyra::MultiVectorBase<double> > & thyra_Y,
                                                 Epetra_MultiVector& Y) const
{
  ...
  for(int i=0; i<rangemaps_.NumMaps(); i++)
  {
    Teuchos::RCP<const Epetra_MultiVector> ep_subY = Thyra::get_Epetra_MultiVector(*(rangemaps_.Map(i)),prod_Y->getMultiVectorBlock(i));
    rangemaps_.InsertVector(*ep_subY,i,Y);
  }
}

LINALG::SOLVER::TEKO::Teko_BACIEpetraOperatorWrapper::Teko_BACIEpetraOperatorWrapper(const Teuchos::RCP<LINALG::BlockSparseMatrixBase>& mat)
  : Teko::Epetra::EpetraOperatorWrapper(),
    mat_(mat)
{
  mappingStrategy_ = Teuchos::rcp(new LINALG::SOLVER::TEKO::Teko_BACIMappingStrategy(mat->RangeExtractor(),mat->DomainExtractor()));
...
{
  /*Teuchos::RCP<const Thyra::LinearOpBase<double> > thA11 = Thyra::epetraLinearOp(mat_->Matrix(0,0).EpetraMatrix(),"A11");
  Teuchos::RCP<const Thyra::LinearOpBase<double> > thA12 = Thyra::epetraLinearOp(mat_->Matrix(0,1).EpetraMatrix(),"A12");
  Teuchos::RCP<const Thyra::LinearOpBase<double> > thA21 = Thyra::epetraLinearOp(mat_->Matrix(1,0).EpetraMatrix(),"A21");
  Teuchos::RCP<const Thyra::LinearOpBase<double> > thA22 = Thyra::epetraLinearOp(mat_->Matrix(1,1).EpetraMatrix(),"A22");*/

  Teuchos::RCP<Thyra::PhysicallyBlockedLinearOpBase<double> > blockA = Thyra::defaultBlockedLinearOp<double>();
  ...
  //Teuchos::RCP<const Thyra::LinearOpBase<double> > blockA = Thyra::block2x2(thA11,thA12,thA21,thA22,"A");
