...
for (i=0; i<fcdindex; i++)
{
   ...
   for (j=colstart; j<colend; j++)
   {
      ...
      column = ja[j];
      index  = mlpcg_getindex(column,update,numeq);
...
for (i=fcdindex; i<numeq; i++) 
{
   /* get the range of the columns in this row */
   colstart = ia[i];
   colend   = ia[i+1];
   ...
   for (j=colstart; j<colend; j++)
   {
      ...
      index = find_index(column,gdofr,ngdofr);
...
/*!---------------------------------------------------------------------
\brief update vector a by a scalar1 times vector x  + scalar2 time vector y                                            

<pre>                                                        m.gee 9/02 
update vector a by a scalar1 times vector x  + scalar2 time vector y       
a += y * facy + x * facx, if init==0
a  = y * facy + x * facx, if init==0
</pre>
\param a     double*    (o)           the updated vector length numeq                   
\param y     double*    (i/o)         the vector to update a                     
\param facy  double     (i)           scalar factor for y
\param x     double*    (i)           the vector to update a                  
\param facx  double     (i)           scalar factor for x
\param init  int        (i)           the init flag                   
\param dim   int        (i)           the dimension of vectors a, y and x                   
\return void                                         
...
#endif
return;
} /* end of mlpcg_updupdvec */



/*!---------------------------------------------------------------------
\brief update vector y by a scalar times vector x                                              

<pre>                                                        m.gee 9/02 
update vector y by a scalar times vector x 
y += x * fac, if init=0
y  = x * fac, if init=1
</pre>
\param y     double*    (i/o)         the updated vector length numeq                   
\param x     double*    (i)           the vector to update y                  
\param fac   double     (i)           the scalar factor                   
\param init  int        (i)           the init flag                   
\param dim   int        (i)           the dimension of vectors y and x                   
\return void                                         
