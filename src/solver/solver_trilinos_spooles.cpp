 ...
 *----------------------------------------------------------------------*/
void solver_trilinos_spooles(
                             struct _SOLVAR*         actsolv,
                             struct _INTRA*          actintra,
                             struct _TRILINOSMATRIX* tri,
...
#ifdef DEBUG
dstrc_enter("solver_trilinos_spooles");
#endif
if (msglvl)
{
...
case 0:
   ...
   DVzero(20,cpus);
   /*-------------------------------------------------------------------*/
   if (recv_a.Typ != ARRAY::cca_DV)
   recv = (double*)amdef("recv",&recv_a,numeq_total,1,"DV");
...
if (spo->is_factored==0)
{
   ...
   for (int i=0; i<matrix->NumMyRows(); ++i)
   {
     ...
     int I = matrix->RowMap().GID(i);
     for (int j=0; j<numentries; ++j)
     {
       int J = matrix->ColMap().GID(indices[j]);
       InpMtx_inputRealEntry(spo->mtxA,I,J,values[j]);
     }
   }
   //for (i=0; i<nnz; i++)
...
#endif
return;
} /* end of solver_trilinos_spooles */
