...
namespace MueLu
{
  ...
  SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::GetValidParameterList(
      const ParameterList &paramList) const
  {
    ...
    validParamList->set<const std::string>("Damping strategy", "Standard",
        "Damping strategy. Can be either \'Standard\' or \'User\'. (default: \'Standard\')");
    ...
    validParamList->set<Teuchos::RCP<const FactoryBase>>(
        "P", Teuchos::null, "Tentative prolongator factory");
    validParamList->set<const std::string>("NonSmoothRowMapName", "",
        "Name of row map of Dofs which corresponding transfer operator basis functions are not to "
        "be smoothed.");
    validParamList->set<Teuchos::RCP<const FactoryBase>>("NonSmoothRowMapFactory", Teuchos::null,
        "Factory for row map of Dofs which corresponding transfer operator basis functions are not "
        "to be smoothed.");
    validParamList->set<const std::string>(
        "NearZeroDiagMapName", "", "Name of row map with nearly zero entries on diagonal of A");
    validParamList->set<Teuchos::RCP<const FactoryBase>>("NearZeroDiagMapFactory", Teuchos::null,
        "Factory for row map of Dofs generating \"NearZeroDiagMapName\"");

    return validParamList;
  }

  template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
  void SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::DeclareInput(
      Level &fineLevel, Level &coarseLevel) const
  {
    Input(fineLevel, "A");

    // Get default tentative prolongator factory
    // Getting it that way ensure that the same factory instance will be used for both SaPFactory
    // and NullspaceFactory.
  ...
  template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
  void SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Build(
      Level &fineLevel, Level &coarseLevel) const
  {
  ...
  template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
  void SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::BuildP(
      Level &fineLevel, Level &coarseLevel) const
  {
    ...
    if (NearZeroDiagRowMapName != "")
    {
      ...
      {
        Teuchos::RCP<const Map> NearZeroDiagRowMap = fineLevel.Get<Teuchos::RCP<const Map>>(
            NearZeroDiagRowMapName, NearZeroDiagRowMapFact.get());
    ...
    if (bPerformSmoothing && dampingFactor != Teuchos::ScalarTraits<Scalar>::zero())
    {
      ...
      Teuchos::RCP<Matrix> AP;
      {
        ...
        // JJH -- If I switch doFillComplete to false, the resulting matrix seems weird when printed
        // with describe. JJH -- The final prolongator is wrong, to boot.  So right now, I
        // fillComplete AP, but avoid fillComplete JJH -- in the scaling.  Long story short, we're
        // doing 2 fillCompletes, where ideally we'd do just one.
      ...
      int globalbSkipDamping = 0;
      {
       ...
       // needed if we do not provide the near zero map!!!
        ...
        Utils::MyOldScaleMatrix(*AP, diag, true, doFillComplete,
            optimizeStorage);  // scale matrix with reciprocal of diag
        // Utils::MyOldScaleMatrix(AP, diag, true, doFillComplete, optimizeStorage); //scale matrix
        // with reciprocal of diag Utils::Write("DinvAP.mat", *AP);
      }
      ...
      if (globalbSkipDamping == 0)
      {
        ...
        Scalar lambdaMax;
        {
          if (DampingStrategy == "Standard")
          {
            ...
            GetOStream(Statistics1, 0) << "Damping factor = " << dampingFactor / lambdaMax << " ("
                                       << dampingFactor << " / " << lambdaMax << ")" << std::endl;
            if (lambdaMax < 0.0)
            {
              lambdaMax = 1.0;
              dampingFactor = 0.0;  // no smoothing at all
              GetOStream(Statistics1, 0)
                  << "Correct damping factor = " << dampingFactor / lambdaMax << " ("
                  << dampingFactor << " / " << lambdaMax << ")" << std::endl;
            }
          }
          else if (DampingStrategy == "User")
          {
            lambdaMax = 1.0;
            GetOStream(Statistics1, 0)
                << "Damping factor = " << dampingFactor / lambdaMax << " (user-given)" << std::endl;
          }
          else
          {
            TEUCHOS_TEST_FOR_EXCEPTION(true, Exceptions::RuntimeError,
                "MueLu::SelectiveSaFactory::Build: wrong damping strategy. allowed values are "
                "Standard and User");
          }
        }
        ...
        {
          SubFactoryMonitor m2(
              *this, "M+M: P = (Ptentative) + (D^{-1} x A x Ptentative)", coarseLevel);

          bool doTranspose = false;
          bool PtentHasFixedNnzPerRow = true;
          Utils2::TwoMatrixAdd(*Ptent, doTranspose, Teuchos::ScalarTraits<Scalar>::one(), *AP,
              doTranspose, -dampingFactor / lambdaMax, finalP, GetOStream(Statistics2, 0),
              PtentHasFixedNnzPerRow);
          // Utils2::TwoMatrixAdd(Ptent, doTranspose, Teuchos::ScalarTraits<Scalar>::one(), AP,
          // doTranspose, -dampingFactor/lambdaMax, finalP, PtentHasFixedNnzPerRow);
        }
    ...
    else
    {
      ...
      Teuchos::RCP<Matrix> R =
          Utils2::Transpose(*finalP, true);  // use Utils2 -> specialization for double
      // Teuchos::RCP<Matrix> R = Utils2::Transpose(finalP, true); // use Utils2 -> specialization
      // for double
      Set(coarseLevel, "R", R);
  ...
  SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::FixAPproduct(Level &fineLevel,
      Level &coarseLevel, Teuchos::RCP<Matrix> &A, Teuchos::RCP<Matrix> &DinvAP) const
  {
    ...
    Teuchos::RCP<const Map> NonSmoothedRowMap =
        fineLevel.Get<Teuchos::RCP<const Map>>(NonSmoothedRowMapName, NonSmoothedRowMapFact.get());
      ...
      if (NonSmoothedRowMap->isNodeGlobalElement(grid))
      {
        ...
        for (size_t i = 0; i < (size_t)indices.size(); i++)
        {
          ...
          {  // TODO this is not necessary
            GlobalOrdinal gcid =
                Ptent->getColMap()->getGlobalElement(indices[i]);  // LID -> GID (column)
    ...
    Teuchos::RCP<Export> exporter =
        ExportFactory::Build(gPtentColVec->getMap(), gPtentDomVec->getMap());
    gPtentDomVec->doExport(
        *gPtentColVec, *exporter, Xpetra::ABSMAX);  // communicate blocked gcolids to all procs
    ...
    Teuchos::RCP<Import> importer =
        ImportFactory::Build(gPtentDomVec->getMap(), gDinvAPColVec->getMap());
  ...
  template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
  Teuchos::RCP<Xpetra::Matrix<double, LocalOrdinal, GlobalOrdinal>>
  SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::MyTranspose(
      ...
      bool const &optimizeTranspose) const
  {
    ...
    if (rv != 0)
    {
      ...
      buf << rv;
      std::string msg =
          "MueLu::Utils::Transpose: Epetra::RowMatrixTransposer returned value of " + buf.str();
  ...
  // deprecated
  template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
  void SelectiveSaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::SetDampingFactor(
