...
namespace MueLu {
  ...
  void ContactTransferFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Build(Level & fineLevel, Level & coarseLevel) const {
    ...
    Teuchos::RCP<const MapExtractorClass> mapextractor = fineLevel.Get< Teuchos::RCP<const MapExtractorClass> >("SegAMapExtractor",MueLu::NoFactory::get());
    ...
    for(size_t i=0; i<mapextractor->NumMaps(); i++) {
        ...
        Teuchos::RCP<Vector> partBlockVector  = VectorFactoryClass::Build(submap);  partBlockVector->putScalar(1.0);
