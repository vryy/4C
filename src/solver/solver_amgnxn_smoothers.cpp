...
/*------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------*/
void LINALG::SOLVER::AMGNXN::GenericSmoother::Richardson(
    Teuchos::RCP<GenericSmoother> Ainv,
    const BlockedMatrix& A,
    const BlockedVector& X,
    BlockedVector& Y,
    int iters,
    double omega,
...
void LINALG::SOLVER::AMGNXN::MergeAndSolve::Setup(BlockedMatrix  matrix)
{
  ...
  x_ = Teuchos::rcp(new Epetra_MultiVector(A_->OperatorDomainMap(),1));// TODO this one might cause problems
...
(const BlockedVector& X, BlockedVector& Y, bool InitialGuessIsZero) const
{
  ...
  for(int i=0;i<X.GetNumBlocks();i++)
    range_ex.InsertVector(*(X.GetVector(i)),i,Xmv);
  ...
  for(int i=0;i<X.GetNumBlocks();i++)
    domain_ex.ExtractVector(Ymv,i,*(Y.GetVector(i)));
...
LINALG::SOLVER::AMGNXN::CoupledAmg::CoupledAmg(
...
void LINALG::SOLVER::AMGNXN::CoupledAmg::Setup()
{
  ...
  if (A_->GetMatrix(0,0)->Comm().MyPID() != 0)
    verbosity = "off";
...
(const BlockedVector& X, BlockedVector& Y, bool InitialGuessIsZero) const
{
  ...
  if(!is_setup_flag_)
    dserror("Solve cannot be called without a previous set up");
...
(const Epetra_MultiVector& X, Epetra_MultiVector& Y, bool InitialGuessIsZero) const
{
  ...
  if (InitialGuessIsZero)
    Y.PutScalar(0.0); // I am not sure what is the meaning of InitialGuessIsZero in MueLu. This is for safety
...
/*------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------*/
LINALG::SOLVER::AMGNXN::MueluAMGWrapper::MueluAMGWrapper(
...
LINALG::SOLVER::AMGNXN::SmootherManager::GetOperator(){return operator_;}

/*------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------*/

Teuchos::ParameterList
LINALG::SOLVER::AMGNXN::SmootherManager::GetParams() {return params_;};

/*------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------*/

Teuchos::ParameterList
LINALG::SOLVER::AMGNXN::SmootherManager::GetParamsSmoother(){return params_subsolver_;}

/*------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------*/

Teuchos::RCP<LINALG::SOLVER::AMGNXN::Hierarchies>
LINALG::SOLVER::AMGNXN::SmootherManager::GetHierarchies(){return hierarchies_;}
...
LINALG::SOLVER::AMGNXN::SmootherManager::GetNullSpaceAllBlocks(){return null_space_all_blocks_;}
...
LINALG::SOLVER::AMGNXN::IfpackWrapperFactory::Create()
{
  ...
  if (ifpack_type == "none")
    dserror("The ifpack type has to be given for the ifpack smoother");
...
Teuchos::RCP<LINALG::SOLVER::AMGNXN::GenericSmoother> LINALG::SOLVER::AMGNXN::CoupledAmgFactory::Create()
{
  ...
  return Teuchos::rcp(new CoupledAmg( GetOperator(), num_pdes, null_spaces_dim, null_spaces_data, amgnxn_params, smoothers_params, smoothers_params ) );
...
LINALG::SOLVER::AMGNXN::BgsSmootherFactory::Create()
{
  ...
  GetOperator()->ParseBlocks(blocks_string,GetBlocks(),SuperBlocks2Blocks,SuperBlocks2BlocksLocal);
...
LINALG::SOLVER::AMGNXN::SimpleSmootherFactory::Create()
{
  ...
  if (GetOperator()->HasOnlyOneBlock())
  ...
  Teuchos::RCP<BlockedMatrix> invApp = Teuchos::null;
  {
    ...
    if (num_rows != App->GetNumCols())
      dserror("We spect here a square matrix");
...
/*------------------------------------------------------------------------------*/
Teuchos::RCP<LINALG::SOLVER::AMGNXN::BlockedMatrix>
LINALG::SOLVER::AMGNXN::SimpleSmootherFactory::ComputeSchurComplement(
    const BlockedMatrix& invApp,
    const BlockedMatrix& Aps,
    const BlockedMatrix& Asp,
  ...
  else if (not invApp.HasOnlyOneBlock())
  {
    if (Ass.HasOnlyOneBlock())
    {
      ...
      for(int b=0;b<NumBlocks_pp;b++)
      {
        ...
        {
          Teuchos::RCP<SparseMatrix> S_sp_tmp = LINALG::MLMultiply(*temp,*(Aps.GetMatrix(b,0)),true);
...
LINALG::SOLVER::AMGNXN::DirectSolverWrapperFactory::Create()
{
  ...
  if(not GetOperator()->HasOnlyOneBlock())
    dserror("We spect here a matrix with only one block");
