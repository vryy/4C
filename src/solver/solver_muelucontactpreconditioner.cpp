...
LINALG::SOLVER::MueLuContactPreconditioner::SetupHierarchy(const Teuchos::ParameterList& params,
    ...
    const Teuchos::RCP<Xpetra::MultiVector<SC, LO, GO, NO>> nsp)
{
  ...
  // params.get<double>("aggregation: threshold");
  ...
  Finest->Set("A", A);
  Finest->Set("SlaveDofMap", Teuchos::rcp_dynamic_cast<const Xpetra::Map<LO, GO, Node>>(
                                 xSlaveDofMap));  // set map with slave dofs
  ...
  // (active?) slave dofs
  Teuchos::RCP<MueLu::FactoryBase> slaveDcAFact = Teuchos::rcp(
      new MueLu::ContactASlaveDofFilterFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>());
  ...
  Teuchos::RCP<MueLu::CoalesceDropFactory<SC, LO, GO, NO>> dropFact =
      Teuchos::rcp(new MueLu::CoalesceDropFactory<SC, LO, GO, NO>());

  // aggregation factory
  Teuchos::RCP<MueLu::UncoupledAggregationFactory<LO, GO, NO>> UCAggFact =
      Teuchos::rcp(new MueLu::UncoupledAggregationFactory<LO, GO, NO>());
  ...
  UCAggFact->SetParameter(
      "OnePt aggregate map name", Teuchos::ParameterEntry(std::string("SlaveDofMap")));
  UCAggFact->SetParameter(
  ...
  Teuchos::RCP<MueLu::PFactory> PtentFact =
  ...
  Teuchos::RCP<MueLu::NullspaceFactory<SC, LO, GO, NO>> nspFact =
      Teuchos::rcp(new MueLu::NullspaceFactory<SC, LO, GO, NO>("Nullspace"));
  ...
  // Teuchos::RCP<MueLu::MapTransferFactory<Scalar,LocalOrdinal, GlobalOrdinal, Node> > cmTransFact3
  // = Teuchos::rcp(new MueLu::MapTransferFactory<Scalar,LocalOrdinal, GlobalOrdinal,
  // Node>("SlaveDofMap", PtentFact, MueLu::NoFactory::getRCP()));
  Teuchos::RCP<MueLu::MapTransferFactory<SC, LO, GO, NO>> cmTransFact3 = Teuchos::rcp(
      new MueLu::MapTransferFactory<SC, LO, GO, NO>(/*"SlaveDofMap", MueLu::NoFactory::getRCP()*/));
  ...
  // coarsestSmooFact = MueLu::MLParameterListInterpreter<Scalar,LocalOrdinal, GlobalOrdinal,
  // Node>::GetSmootherFactory(params, "coarse");
  ...
  for (int i = 0; i < maxLevels; i++)
  {
    ...
    Teuchos::RCP<MueLu::SmootherFactory<SC, LO, GO, NO>> SmooFactFine =
        GetContactSmootherFactory(pp, i, slaveDcAFact);
  ...
  if (maxLevels == 1)
  {
    bIsLastLevel = hierarchy->Setup(
        0, Teuchos::null, vecManager[0], Teuchos::null);  // 1 level "multigrid" method
  }
  else
  {
    bIsLastLevel =
        hierarchy->Setup(0, Teuchos::null, vecManager[0], vecManager[1]);  // first (finest) level
...
LINALG::SOLVER::MueLuContactPreconditioner::GetContactSmootherFactory(
    ...
    const Teuchos::RCP<MueLu::FactoryBase>& AFact)
{
  ...
  if (type == "Jacobi")
  {
    ...
    smooProto =
        Teuchos::rcp(new MueLu::MyTrilinosSmoother<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
            "SlaveDofMap", MueLu::NoFactory::getRCP(), ifpackType, ifpackList, 0, AFact));
  }
  ...
  else if (type == "IFPACK")
  {
    ...
    if (ifpackType == "ILU")
    {
      ...
      smooProto =
          Teuchos::rcp(new MueLu::MyTrilinosSmoother<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
              "SlaveDofMap", MueLu::NoFactory::getRCP(), ifpackType, ifpackList, overlap, AFact));
    }
    else
...
//----------------------------------------------------------------------------------
Teuchos::RCP<MueLu::SmootherFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
LINALG::SOLVER::MueLuContactPreconditioner::GetContactCoarsestSolverFactory(
