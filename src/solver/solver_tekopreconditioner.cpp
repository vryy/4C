...
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
LINALG::SOLVER::TekoPreconditioner::TekoPreconditioner(
    FILE* outfile, Teuchos::ParameterList& tekolist)
    : PreconditionerType(outfile), tekolist_(tekolist), type_("undefined")
{
}

//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
void LINALG::SOLVER::TekoPreconditioner::Setup(
    bool create, Epetra_Operator* matrix, Epetra_MultiVector* x, Epetra_MultiVector* b)
{
  ...
  if (create)
  {
    ...
    if (A == NULL) dserror("BlockSparseMatrixBase expected");
    ...
    if (type_ == "BGS")
    {
      ...
      for (int i = 0; i < numBlocks; i++)
      {
        ...
        if (!Params().isSublist(ssinverse.str()))
          dserror("missing parameter sublists for inverses.");
        if (!Params().sublist(ssinverse.str()).isSublist("Stratimikos Parameters"))
          dserror(
              "Teko block preconditioners need STRATIMIKOS solver objects for the block inverses.");
        Teuchos::ParameterList invParams =
            Params().sublist(ssinverse.str()).sublist("Stratimikos Parameters");
        std::string invType = invParams.get<std::string>("Linear Solver Type");
        Teuchos::RCP<Teko::InverseFactory> inverseFact =
            Teko::invFactoryFromParamList(invParams, invType);
      ...
      Teuchos::RCP<const Teko::StaticInvDiagStrategy> diagInvStrat =
          Teuchos::rcp(new Teko::StaticInvDiagStrategy(constInverseLinearOps));

      // build 2x2 block Gauss Seidel preconditioner factory
      precFact = Teuchos::rcp(
          new Teko::GaussSeidelPreconditionerFactory(Teko::GS_UseLowerTriangle, diagInvStrat));
    }
    /////////////////////////////////////////////////////////////// TEKO SIMPLE
    else if (type_ == "SIMPLE")
    {
      if (!Params().isSublist("Inverse1")) dserror("missing parameter sublists for inverse.");
      if (!Params().sublist("Inverse1").isSublist("Stratimikos Parameters"))
        dserror(
            "Teko block preconditioners need STRATIMIKOS solver objects for the block inverses.");
      Teuchos::ParameterList invParams =
          Params().sublist("Inverse1").sublist("Stratimikos Parameters");
      std::string invType = invParams.get<std::string>("Linear Solver Type");
      ...
      {
        if (!Params().sublist("Inverse2").isSublist("Stratimikos Parameters"))
          dserror(
              "Teko block preconditioners need STRATIMIKOS solver objects for the block inverses.");
        Teuchos::ParameterList invParams2 =
            Params().sublist("Inverse2").sublist("Stratimikos Parameters");
        std::string invType2 = invParams2.get<std::string>("Linear Solver Type");
        Teuchos::RCP<Teko::InverseFactory> inverseFact2 =
            Teko::invFactoryFromParamList(invParams2, invType2);
        precFact = Teuchos::rcp(
            new Teko::NS::SIMPLEPreconditionerFactory(inverseFact, inverseFact2, alpha));
      }
      else
