...
DRT::ResultTest("POROFLUIDMULTIPHASE"),
porotimint_(porotimint)
{
  return;
}
 ...
 *----------------------------------------------------------------------*/
void POROFLUIDMULTIPHASE::ResultTest::TestNode(DRT::INPUT::LineDefinition& res, int& nerr, int& test_count)
{
  // care for the case of multiple discretizations of the same field type
  std::string dis;
  res.ExtractString("DIS",dis);
  if (dis != porotimint_.Discretization()->Name())
    return;
  ...
  {
    dserror("Node %d does not belong to discretization %s",node+1,porotimint_.Discretization()->Name().c_str());
  }
  else
  {
    if (porotimint_.Discretization()->HaveGlobalNode(node))
    {
      ...
      if (actnode->Owner() != porotimint_.Discretization()->Comm().MyPID())
        return;
    ...
    ) const
{
  ...
  // test result value of phi field
    ...
    result = (*porotimint_.Saturation())[phinpmap.LID(porotimint_.Discretization()->Dof(0,node,0))];
    ...
    ) const
{
  ...
  if(quantity == "numiterlastnewton")
    result = (double) porotimint_.IterNum();
  // catch unknown quantity strings
  else
    dserror("Quantity '%s' not supported in result test!", quantity.c_str());
