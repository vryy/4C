...
//  outmean_  (DRT::INPUT::IntegralValue<int>(*params,"OUTMEAN")),
  ...
  vectornormfres_(DRT::INPUT::IntegralValue<INPAR::POROFLUIDMULTIPHASE::VectorNorm>(poroparams_,"VECTORNORM_RESF")),
  vectornorminc_(DRT::INPUT::IntegralValue<INPAR::POROFLUIDMULTIPHASE::VectorNorm>(poroparams_,"VECTORNORM_INC")),
    ...
    int nds_scalar)
{
  ...
  dbcmaps_with_volfracpress_ = Teuchos::rcp(new LINALG::MapExtractor());
  {
    ...
    discret_->EvaluateDirichlet(eleparams, zeros_, Teuchos::null, Teuchos::null,
                                Teuchos::null, dbcmaps_with_volfracpress_);
...
void POROFLUIDMULTIPHASE::TimIntImpl::AssembleMatAndRHS()
{
  ...
  discret_->Evaluate(eleparams,sysmat_,Teuchos::null,residual_, valid_volfracpress_dofs_, Teuchos::null);
...
void POROFLUIDMULTIPHASE::TimIntImpl::NonlinearSolve()
{
  ...
  while(true)
  {
    ...
    if (AbortNonlinIter(iternum_,itemax_,abstolres,actresidual))
      break;
    ...
    double& actresidual)
{
  ...
  if (std::isnan(incprenorm) or
      std::isnan(prenorm) or
      std::isnan(preresnorm))
    dserror("calculated vector norm is NaN.");

  if (std::isinf(incprenorm) or
      std::isinf(prenorm) or
...
inline void POROFLUIDMULTIPHASE::TimIntImpl::PrintConvergenceHeader()
{
  if (myrank_ == 0)
    std::cout << "+------------+-------------------+--------------+-------------------+--------------+----------------------------+\n"
             << "|- step/max -|- tol-res   [norm]-|-- pre-res ---|- tol-relinc[norm]-|-- pre-inc ---|" << std::endl;
...
)
{
  if (myrank_ == 0)
    std::cout << "|  " << std::setw(3) << itnum << "/" << std::setw(3) << itemax << "   | "
             << std::setw(10) << std::setprecision(3) << std::scientific << ittolres_ << " [" << std::setw(3) <<  VectorNormString(vectornormfres_).c_str() << "]  | "
             << std::setw(10) << std::setprecision(3) << std::scientific << preresnorm << "   | "
             << std::setw(10) << std::setprecision(3) << std::scientific << ittolinc_ << " [" << std::setw(3) <<  VectorNormString(vectornorminc_).c_str() << "]  | "
             << "     --      | (    --   ,te="
...
)
{
  if (myrank_ == 0)
    ...
    << std::setw(10) << std::setprecision(3) << std::scientific << ittolres_ << " [" << std::setw(3) << VectorNormString(vectornormfres_).c_str() << "]  | "
             << std::setw(10) << std::setprecision(3) << std::scientific << preresnorm << "   | "
             << std::setw(10) << std::setprecision(3) << std::scientific << ittolres_ << " [" << std::setw(3) << VectorNormString(vectornorminc_).c_str() << "]  | "
...
void POROFLUIDMULTIPHASE::TimIntImpl::OutputState()
{
  ...
  if(output_porosity_)
  {
    ...
    Teuchos::RCP<Epetra_MultiVector> porosity_multi =
        POROFLUIDMULTIPHASE::UTILS::ConvertDofVectorToNodeBasedMultiVector(*discret_,*porosity_,nds_solidpressure_,1);
...
void POROFLUIDMULTIPHASE::TimIntImpl::PrepareSystemForNewtonSolve()
{
  ...
  // residual values are supposed to be zero at Dirichlet boundaries
  {
    ...
    // we identify the volume fraction pressure dofs which do not have a physical meaning and set a DBC on them
    for(int i=0; i<elecolmap->NumMyElements(); ++i)
    {
      // dynamic_cast necessary because virtual inheritance needs runtime information
      DRT::ELEMENTS::PoroFluidMultiPhase* myele = dynamic_cast<DRT::ELEMENTS::PoroFluidMultiPhase*>(discret_->gElement(elecolmap->GID(i)));
    ...
    Teuchos::RCP<Epetra_Map> dirichmap = Teuchos::rcp( new Epetra_Map(-1,nummydirichvals,&(mydirichdofs[0]),0,discret_->Comm()) );
    ...
    *dbcmaps_with_volfracpress_ = LINALG::MapExtractor(*(discret_->DofRowMap()), condmerged);
    ...
    // apply combined map
    LINALG::ApplyDirichlettoSystem(sysmat_,increment_,residual_,zeros_,*(dbcmaps_with_volfracpress_->CondMap()));
  }
...
void POROFLUIDMULTIPHASE::TimIntImpl::CalcInitialTimeDerivative()
{
  ...
  if(myrank_ == 0)
    std::cout << "POROFLUIDMULTIPHASE: calculating initial time derivative of state variables on discretization \"" << discret_->Name().c_str() << "\" (step " << Step() <<", time " << Time() << ") ... ... ";
