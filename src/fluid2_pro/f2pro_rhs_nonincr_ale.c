for (vi=0; vi<iel; ++vi)
{
    ...
    eforce_(vi*2)     += ttimetauM*(2.0*conv_c_(vi)*conv_old_(0) - 2.0*velint_(0)*derxy_(0, vi)*gridvint_(0)*vderxy_(0, 0) - velint_(0)*derxy_(0, vi)*gridvint_(1)*vderxy_(0, 1) - velint_(0)*derxy_(1, vi)*gridvint_(1)*vderxy_(0, 0) - velint_(1)*derxy_(0, vi)*gridvint_(0)*vderxy_(0, 1) - velint_(1)*derxy_(1, vi)*gridvint_(0)*vderxy_(0, 0) - 2.0*velint_(1)*derxy_(1, vi)*gridvint_(1)*vderxy_(0, 1)) ;
    eforce_(vi*2 + 1) += ttimetauM*(2.0*conv_c_(vi)*conv_old_(1) - 2.0*velint_(0)*derxy_(0, vi)*gridvint_(0)*vderxy_(1, 0) - velint_(0)*derxy_(0, vi)*gridvint_(1)*vderxy_(1, 1) - velint_(0)*derxy_(1, vi)*gridvint_(1)*vderxy_(1, 0) - velint_(1)*derxy_(0, vi)*gridvint_(0)*vderxy_(1, 1) - velint_(1)*derxy_(1, vi)*gridvint_(0)*vderxy_(1, 0) - 2.0*velint_(1)*derxy_(1, vi)*gridvint_(1)*vderxy_(1, 1)) ;
    ...
    eforce_(vi*2)     += -(ttimetauM*conv_g_(vi)*gradp_(0)) ;
    eforce_(vi*2 + 1) += -(ttimetauM*conv_g_(vi)*gradp_(1)) ;
    ...
    eforce_(vi*2)     += timetauM*conv_g_(vi)*rhsint_(0) ;
    eforce_(vi*2 + 1) += timetauM*conv_g_(vi)*rhsint_(1) ;
