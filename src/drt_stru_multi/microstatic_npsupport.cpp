...
analyses for supporting processors
...
void STRUMULTI::np_support_drt()
{
  ...
  while(true)
  {
    ...
    if(dummymaterials[eleID] == Teuchos::null)
      dummymaterials[eleID] = Teuchos::rcp_static_cast<MAT::MicroMaterial>(MAT::Material::Factory(1));
    ...
    case 0:
    {
      ...
      const Epetra_SerialDenseMatrix* defgrdcopy = condnamemap[0]->Get<Epetra_SerialDenseMatrix>("defgrd");
      LINALG::Matrix<3,3> defgrd(*(const_cast<Epetra_SerialDenseMatrix*>(defgrdcopy)), true);
      const Epetra_SerialDenseMatrix* cmatcopy = condnamemap[0]->Get<Epetra_SerialDenseMatrix>("cmat");
    ...
    case 4:
    {
      ...
      // the material is replaced in read mesh within restart on macro scale; this is done here manually one-time
      if (restart == false)
      {
        restart = true;
