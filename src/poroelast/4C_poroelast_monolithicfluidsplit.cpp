    ...
    : MonolithicSplit(comm, timeparams, porosity_splitter)
{
  ...
  fgitransform_ = Teuchos::make_rcp<Coupling::Adapter::MatrixRowTransform>();
  figtransform_ = Teuchos::make_rcp<Coupling::Adapter::MatrixColTransform>();
  cfggtransform_ = Teuchos::make_rcp<Coupling::Adapter::MatrixRowTransform>();
  csggtransform_ = Teuchos::make_rcp<Coupling::Adapter::MatrixColTransform>();
  cfgitransform_ = Teuchos::make_rcp<Coupling::Adapter::MatrixRowTransform>();
  csigtransform_ = Teuchos::make_rcp<Coupling::Adapter::MatrixColTransform>();

  // Recovering of Lagrange multiplier happens on structure field
  lambda_ =
      Teuchos::make_rcp<Core::LinAlg::Vector<double>>(*fluid_field()->interface()->fsi_cond_map());
}
...
void PoroElast::MonolithicFluidSplit::setup_rhs(bool firstcall)
{
  ...
  if (firstcall and evaluateinterface_)
  {
    ...
    Teuchos::RCP<Core::LinAlg::Vector<double>> rhs =
        Teuchos::make_rcp<Core::LinAlg::Vector<double>>(fig.row_map());
    ...
    rhs = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(fgg.row_map());
    ...
    rhs = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(kig.row_map());
    ...
    rhs = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(kgg.row_map());
...
void PoroElast::MonolithicFluidSplit::setup_system_matrix(Core::LinAlg::BlockSparseMatrixBase& mat)
{
  ...
  cgicur_ = Teuchos::make_rcp<Core::LinAlg::SparseMatrix>(k_fs->matrix(1, 0));
  cggcur_ = Teuchos::make_rcp<Core::LinAlg::SparseMatrix>(k_fs->matrix(1, 1));
}
