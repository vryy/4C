...
\brief A class for mortar coupling of ONE slave element and the master
       surface of a contact interface in 2D and 3D.
...
contactsegs_(csegs)
{
  ...
  if (Dim()!=2) dserror("Coupling not yet implemented for 3D!");
  
  // loop over the contact candidate master elements of sele_
  // use slave element's candidate list SearchElements !!!
  for (int j=0;j<SlaveElement().NumSearchElements();++j)
  {
    int gid = SlaveElement().SearchElements()[j];
    DRT::Element* ele = idiscret_.gElement(gid);
    if (!ele) dserror("ERROR: Cannot find master element with gid %",gid);
    CElement* melement = static_cast<CElement*>(ele);
    ...
    if (Dim()==2)
    {
      ...
      Project2D(*melement,hasproj,xiproj);
    
      // check for element overlap
      overlap = DetectOverlap2D(*melement,hasproj,xiproj);
    
      // integrate the element overlap
      if (overlap) IntegrateOverlap2D(*melement,xiproj);
    }
    ...
    // invalid cases for dim
      ...
      dserror("ERROR: Coupling can only be called for 2D or 3D!");
  }
 ...
 *----------------------------------------------------------------------*/
bool CONTACT::Coupling::Project2D(CONTACT::CElement& mele,
                                  vector<bool>& hasproj,
 ...
 *----------------------------------------------------------------------*/
bool CONTACT::Coupling::DetectOverlap2D(CONTACT::CElement& mele,
                                        vector<bool>& hasproj,
                                        vector<double>& xiproj)
{
  /**********************************************************************/
  ...
  else if  (s0hasproj && !s1hasproj && !m0hasproj && !m1hasproj)
  {
    if ((-1.0+CONTACTPROJTOL<=sprojxi[0]) && (sprojxi[0]<=1.0-CONTACTPROJTOL))
    {
      ...
      dserror("ERROR: IntegrateOverlap2D: Significant overlap ignored S%i M%i!", sele_.Id(), mele.Id());
    }
  }
  ...
  if (overlap)
  {
    ...
    sele_.LocalToGlobal(sxialoc,sxiaglob,true);
    sele_.LocalToGlobal(sxibloc,sxibglob,true);
 ...
 *----------------------------------------------------------------------*/
bool CONTACT::Coupling::IntegrateOverlap2D(CONTACT::CElement& mele,
                                           vector<double>& xiproj)
{
  /**********************************************************************/
  /* INTEGRATION                                                        */
  ...
  RCP<Epetra_SerialDenseMatrix> mseg = integrator.IntegrateM(sele_,sxia,sxib,mele,mxia,mxib);
  RCP<Epetra_SerialDenseVector> gseg = integrator.IntegrateG(sele_,sxia,sxib,mele,mxia,mxib);
  ...
  if (modification)
  {
    RCP<Epetra_SerialDenseMatrix> mmodseg = integrator.IntegrateMmod(sele_,sxia,sxib,mele,mxia,mxib);
    integrator.AssembleMmod(Comm(),sele_,mele,*mmodseg);
  }
  //--------------------------------------------------------------------*/
