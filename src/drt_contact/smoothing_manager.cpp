 ...
 *----------------------------------------------------------------------*/
CONTACT::SmoothingManager::SmoothingManager(
...
//      dserror("ERROR: CoManager: Contact requires redundant master storage");
    ...
    Teuchos::RCP<CONTACT::CoInterface> interface =
        cinterfaces[(int) cinterfaces.size() - 1];
...
//      dserror("ERROR: MtManager: Meshtying requires redundant master storage");
    ...
    for (int j=0;j<(int)currentgroup.size();++j)
    {
      ...
      for (fool=currele.begin(); fool != currele.end(); ++fool)
      {
        ...
        mtele->IsHermite() = DRT::INPUT::IntegralValue<int>(cparams,"HERMITE_SMOOTHING") ;
  ...
  for (int i = 0; i < (int) cinterfaces.size(); ++i)
    cinterfaces[i]->CreateSearchTree();
...
bool CONTACT::SmoothingManager::ReadAndCheckInput(Teuchos::ParameterList& cparams)
{
  ...
  if(DRT::INPUT::IntegralValue<INPAR::MORTAR::AlgorithmType>(mortar,"ALGORITHM") == INPAR::MORTAR::algorithm_mortar)
  {
    ...
    if(problemtype == prb_structure and
       ...
       DRT::INPUT::IntegralValue<INPAR::WEAR::WearType>(wearlist,"WEARTYPE") != INPAR::WEAR::wear_intstate)
