 ...
 *----------------------------------------------------------------------*/
void CONTACT::MtLagrangeStrategy::MortarCoupling(
    const Teuchos::RCP<Epetra_Vector> dis)
{
  ...
  if (Dualquadslave3d())
  {
    ...
    Teuchos::RCP<LINALG::SparseMatrix> temp1 = LINALG::MLMultiply(*dmatrix_,
        false, *invtrafo_, false, false, false, true);
    Teuchos::RCP<LINALG::SparseMatrix> temp2 = LINALG::MLMultiply(*trafo_,
        false, *invd_, false, false, false, true);
    Teuchos::RCP<LINALG::SparseMatrix> temp3 = LINALG::MLMultiply(*trafo_,
  ...
  {
    // first setup
    Teuchos::RCP<LINALG::SparseMatrix> constrmt = Teuchos::rcp(
        new LINALG::SparseMatrix(*gdisprowmap_, 100, false, true));
    ...
    Teuchos::RCP<Epetra_Vector>& feff, Teuchos::RCP<Epetra_Vector> dis)
{
  ...
  if (systype == INPAR::CONTACT::system_condensed || systype == INPAR::CONTACT::system_condensed_lagmult)
  {
    ...
    if (ParRedist())
    {
      ...
      LINALG::SplitMatrix2x2(kteffmatrix, pgsmdofrowmap_, gndofrowmap_,
          pgsmdofrowmap_, gndofrowmap_, ksmsm, ksmn, knsm, knn);
      ksmsm = MORTAR::MatrixRowColTransform(ksmsm, gsmdofrowmap_,
          gsmdofrowmap_);
    ...
    else
    {
      ...
      LINALG::SplitMatrix2x2(kteffmatrix, gsmdofrowmap_, gndofrowmap_,
          gsmdofrowmap_, gndofrowmap_, ksmsm, ksmn, knsm, knn);
    }

    // further splits into slave part + master part
    LINALG::SplitMatrix2x2(ksmsm, gsdofrowmap_, gmdofrowmap_, gsdofrowmap_,
        gmdofrowmap_, kss, ksm, kms, kmm);
    LINALG::SplitMatrix2x2(ksmn, gsdofrowmap_, gmdofrowmap_, gndofrowmap_,
        tempmap, ksn, tempmtx1, kmn, tempmtx2);
    LINALG::SplitMatrix2x2(knsm, gndofrowmap_, tempmap, gsdofrowmap_,
        gmdofrowmap_, kns, knm, tempmtx1, tempmtx2);
    ...
    LINALG::SplitVector(*ProblemDofs(), *feff, gsmdofrowmap_, fsm, gndofrowmap_,
        fn);
    ...
    LINALG::SplitVector(*gsmdofrowmap_, *fsm, gsdofrowmap_, fs, gmdofrowmap_,
        fm);
    ...
    Teuchos::RCP<LINALG::SparseMatrix> kmnadd = LINALG::MLMultiply(*mhatmatrix_,
        true, *ksn, false, false, false, true);
    ...
    Teuchos::RCP<Epetra_Vector> tempvecs = Teuchos::rcp(
...
void CONTACT::MtLagrangeStrategy::BuildSaddlePointSystem(Teuchos::RCP<LINALG::SparseOperator> kdd, Teuchos::RCP<Epetra_Vector> fd, Teuchos::RCP<Epetra_Vector> sold, Teuchos::RCP<LINALG::MapExtractor> dbcmaps, int numiter,Teuchos::RCP<Epetra_Operator>& blockMat, Teuchos::RCP<Epetra_Vector>& blocksol, Teuchos::RCP<Epetra_Vector>& blockrhs)
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> stiffmt = Teuchos::rcp_dynamic_cast<
  ...
  Teuchos::RCP<Epetra_Vector> constrrhs = Teuchos::rcp(
  ...
  if (systype == INPAR::CONTACT::system_saddlepoint)
  {
    ...
    Teuchos::RCP<LINALG::SparseMatrix> trconstrmt = Teuchos::rcp(
    ...
    Teuchos::RCP<Epetra_Vector> zeros = Teuchos::rcp(
        new Epetra_Vector(*ProblemDofs(), true));
...
void CONTACT::MtLagrangeStrategy::Recover(Teuchos::RCP<Epetra_Vector> disi)
{
  ...
  if (systype == INPAR::CONTACT::system_condensed || systype == INPAR::CONTACT::system_condensed_lagmult)
  {
    ...
    Teuchos::RCP<Epetra_Vector> disis = Teuchos::rcp(
        new Epetra_Vector(*gsdofrowmap_));
    if (gsdofrowmap_->NumGlobalElements())
      LINALG::Export(*disi, *disis);

    // extract master displacements from disi
    Teuchos::RCP<Epetra_Vector> disim = Teuchos::rcp(
        new Epetra_Vector(*gmdofrowmap_));
    if (gmdofrowmap_->NumGlobalElements())
      LINALG::Export(*disi, *disim);

    // extract other displacements from disi
    Teuchos::RCP<Epetra_Vector> disin = Teuchos::rcp(
        new Epetra_Vector(*gndofrowmap_));
    if (gndofrowmap_->NumGlobalElements())
