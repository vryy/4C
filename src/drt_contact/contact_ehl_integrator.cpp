...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void CONTACT::CoIntegratorEhl::IntegrateGP_3D(
    ...
    )
{
  ...
  GP_WeightedSurfGradAndDeriv(sele,sxi,derivsxi,lmval,lmderiv,dualmap,sval,sderiv,ssecderiv,wgt,jac,derivjac);

//  // weighted tangential velocity (average and relative)
  GP_WeightedAvRelVel(
      sele,mele,sval,lmval,mval,sderiv,mderiv,lmderiv,dualmap,wgt,
      jac,derivjac,normal,dnmap_unit,gap,deriv_gap,sxi,mxi,derivsxi,derivmxi);
}
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void CONTACT::CoIntegratorEhl::IntegrateGP_2D(
    MORTAR::MortarElement& sele,
    MORTAR::MortarElement& mele,
    LINALG::SerialDenseVector& sval,
    LINALG::SerialDenseVector& lmval,
...
void CONTACT::CoIntegratorEhl::GP_WeightedSurfGradAndDeriv(
    MORTAR::MortarElement& sele,
    const double* xi,
    const std::vector<GEN::pairedvector<int,double> >& dsxigp,
    const LINALG::SerialDenseVector& lmval,
    ...
    const LINALG::SerialDenseMatrix& sderiv2,
    const double& wgt,
    const double& jac,
...
)
{
  ...
  LINALG::Matrix<3,1> n; n.CrossProduct(t1,t2);
  n.Scale(1./n.Norm2());
  ...
  for (int i=0;i<Dim()-1;++i)
    for (int d=0;d<Dim();++d)
      gxi_contra.at(i).at(d)=contravariant_metric(i,d);
  ...
  for (int a=0;a<sele.NumNode();++a)
  {
    ...
    {
      LINALG::Matrix<3,1>& tmp = cnode->CoEhlData().GetSurfGrad()[dynamic_cast<CONTACT::CoNode*>(sele.Nodes()[c])->Dofs()[0]];
      for (int d=0;d<Dim();++d)
    ...
    for (int c=0;c<sele.NumNode();++c)
    {
      ...
      {
        LINALG::Matrix<3,1>& tmp = cnode->CoEhlData().GetSurfGradDeriv()[p->first][dynamic_cast<CONTACT::CoNode*>(sele.Nodes()[c])->Dofs()[0]];
        for (int d=0;d<Dim();++d)
          for (int al=0;al<Dim()-1;++al)
      ...
      for (int e=0;e<Dim()-1;++e)
        for (auto p=dsxigp.at(e).begin();p!=dsxigp.at(e).end();++p)
        {
          ...
          for (int d=0;d<Dim();++d)
            for (int al=0;al<Dim()-1;++al)
              tmp(d)+=wgt*jac*lmderiv(a,e)*p->second*sderiv(c,al)*gxi_contra.at(al).at(d);
        }
        ...
        for (int d=0;d<Dim();++d)
          for (int al=0;al<Dim()-1;++al)
            for (int m=0;m<sele.NumNode();++m)
              tmp(d)+=wgt*jac*p->second(a,m)*sval(m)*sderiv(c,al)*gxi_contra.at(al).at(d);
      }
    }
  }
...
void CONTACT::CoIntegratorEhl::GP_WeightedAvRelVel(
    MORTAR::MortarElement& sele,
    MORTAR::MortarElement& mele,
    const LINALG::SerialDenseVector& sval,
    const LINALG::SerialDenseVector& lmval,
...
{
  const int dim=3; if (Dim()!=dim) dserror("dimension inconsistency, or is this not implemented for all spatial dimensions?");

  LINALG::Matrix<dim,1> t1, t2;
  std::vector<GEN::pairedvector<int,double> > dt1,dt2;
  LINALG::Matrix<dim,1> relVel;
  std::vector<GEN::pairedvector<int,double> > relVel_deriv(dim,sele.NumNode()*dim+mele.NumNode()*dim+derivsxi[0].size()+derivmxi[0].size());
  double vt1,vt2;
  ...
  for (int i=0;i<sele.NumNode();++i)
  {
    ...
    for (int d=0;d<dim;++d)
      cnode->CoEhlData().GetWeightedRelTangVel()(d)+=jac*wgt*lmval(i)*(vt1*t1(d)+vt2*t2(d));
    ...
    for (auto p=dvt1.begin();p!=dvt1.end();++p)
    {
      LINALG::Matrix<3,1>& tmp = cnode->CoEhlData().GetWeightedRelTangVelDeriv()[p->first];
      for (int d=0;d<dim;++d)
        tmp(d)+=jac*wgt*lmval(i)*p->second*t1(d);
    }

    for (int d=0;d<dim;++d)
      for (auto p=dt1.at(d).begin();p!=dt1.at(d).end();++p)
        cnode->CoEhlData().GetWeightedRelTangVelDeriv()[p->first](d)+=jac*wgt*lmval(i)*vt1*p->second;

    for (auto p=dvt2.begin();p!=dvt2.end();++p)
    {
      LINALG::Matrix<3,1>& tmp = cnode->CoEhlData().GetWeightedRelTangVelDeriv()[p->first];
      for (int d=0;d<dim;++d)
        tmp(d)+=jac*wgt*lmval(i)*p->second*t2(d);
    }

    for (int d=0;d<dim;++d)
      for (auto p=dt2.at(d).begin();p!=dt2.at(d).end();++p)
        cnode->CoEhlData().GetWeightedRelTangVelDeriv()[p->first](d)+=jac*wgt*lmval(i)*vt2*p->second;
  }
  ...
  for (int i=0;i<sele.NumNode();++i)
  {
    CONTACT::CoNode* cnode = dynamic_cast<CONTACT::CoNode*>(sele.Nodes()[i]);
    for (int d=0;d<dim;++d)
      cnode->CoEhlData().GetWeightedAvTangVel()(d)-=jac*wgt*lmval(i)*(vt1*t1(d)+vt2*t2(d));
    ...
    for (int d=0;d<dim;++d)
      for (auto p=dt1.at(d).begin();p!=dt1.at(d).end();++p)
        cnode->CoEhlData().GetWeightedAvTangVelDeriv()[p->first](d)-=jac*wgt*lmval(i)*vt1*p->second;
    ...
    for (int d=0;d<dim;++d)
      for (auto p=dt2.at(d).begin();p!=dt2.at(d).end();++p)
        cnode->CoEhlData().GetWeightedAvTangVelDeriv()[p->first](d)-=jac*wgt*lmval(i)*vt2*p->second;
  }
