/*---------------------------------------------------------------------*/
/*!
\file contact_nitsche_integrator_poro.cpp
...
{
     if (fabs(theta_)>1e-16)
       dserror("Poro Contact just implemented Adjoint free ...");
}

/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void CONTACT::CoIntegratorNitschePoro::IntegrateGP_3D(
    ...
    )
{
  ...
  //if (nit_normal_==INPAR::CONTACT::NitNor_ele)
  {
    ...
    GPTS_forces<3>(sele,mele,sval,sderiv,derivsxi,mval,mderiv,derivmxi,
           jac,derivjac,wgt,gap,deriv_gap,n,dn,sxi,mxi);
  }
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void CONTACT::CoIntegratorNitschePoro::IntegrateGP_2D(
    MORTAR::MortarElement& sele,
    MORTAR::MortarElement& mele,
    LINALG::SerialDenseVector& sval,
    LINALG::SerialDenseVector& lmval,
 ...
 *----------------------------------------------------------------------*/
template <int dim>
void CONTACT::CoIntegratorNitschePoro::GPTS_forces(
    ...
    double* gpn, std::vector<GEN::pairedvector<int,double> >& dnmap_unit,double* sxi,double* mxi)
{
  ...
  // first rough check
  if (do_fast_checks)
    if (abs(theta_)<1.e-12 && gap>std::max(sele.MaxEdgeSize(),mele.MaxEdgeSize()))
      return;
  ...
  GEN::pairedvector<int,double> cauchy_nn_weighted_average_deriv_d(sele.NumNode()*3*12+sele.MoData().ParentDisp().size()+mele.MoData().ParentDisp().size());
  GEN::pairedvector<int,double> cauchy_nn_weighted_average_deriv_p(sele.MoData().ParentPFPres().size()+mele.MoData().ParentPFPres().size());
  ...
  for (_CI p=cauchy_nn_weighted_average_deriv_d.begin();p!=cauchy_nn_weighted_average_deriv_d.end();++p)
...
template <int dim>
void CONTACT::CoIntegratorNitschePoro::SoEleCauchy(
    ...
    GEN::pairedvector<int,double>& deriv_sigma_nt_p)
{
  ...
  if (!moEle.MoData().ParentPFPres().size())
  {
    dynamic_cast<DRT::ELEMENTS::So_base*>(moEle.ParentElement())->GetCauchyAtXi(
        pxsi,moEle.MoData().ParentDisp(),normal,direction,sigma_nt,&dsntdd,NULL,NULL,NULL,NULL,&dsntdn,&dsntdt,&dsntdpxi);
  }
  else
  {
    dynamic_cast< DRT::ELEMENTS::So3_Poro<DRT::ELEMENTS::So_hex8,DRT::Element::hex8>* >(moEle.ParentElement())->GetCauchyAtXi(
        pxsi,moEle.MoData().ParentDisp(),moEle.MoData().ParentPFPres(),normal,direction,sigma_nt,&dsntdd,&dsntdp,&dsntdn,&dsntdt,&dsntdpxi);
  }
...
template <int dim>
void CONTACT::CoIntegratorNitschePoro::IntegrateTest(
    const double fac,
    ...
    const double jac,const GEN::pairedvector<int,double>& jacintcellmap, const double wgt,
    const double test_val, const GEN::pairedvector<int,double>& test_deriv_d, const GEN::pairedvector<int,double>& test_deriv_p,
    ...
    )
{
  ...
  for (GEN::pairedvector<int,double>::const_iterator p=test_deriv_p.begin();p!=test_deriv_p.end();++p)
  {
...
template <int dim>
void CONTACT::CoIntegratorNitschePoro::IntegratePoroNoOutFlow(
        const double fac,
        MORTAR::MortarElement& ele,
        double* xi,
        const LINALG::SerialDenseVector& shape,
        const LINALG::SerialDenseMatrix& deriv,
        const double jac,const GEN::pairedvector<int,double>& jacintcellmap, const double wgt,
        const LINALG::Matrix<dim,1>& normal, const std::vector<GEN::pairedvector<int,double> >& normal_deriv,
        ...
        )
{
  ...
  double sweight = 1; double oweight = 0;

  double spresgp = 0; double srelveln = 0;
  for (int j=0;j<ele.NumNode();++j)
  {
    int pj = DRT::UTILS::getParentNodeNumberFromFaceNodeNumber(ele.ParentElement()->Shape(),ele.FaceParentNumber(),j);
    spresgp +=ele.MoData().ParentPFPres()[pj]*shape(j);
    for (int d = 0; d < dim; ++d)
    {
      srelveln +=  (ele.MoData().ParentPFVel()[pj*dim+d]-ele.MoData().ParentVel()[pj*dim+d])*shape(j)*normal(d);
    }
  }

  double sporosity = -1; double sJ = -1; std::map<int,double> sJLin; double sdphi_dp; double sdphi_dJ;
  ...
  {
    sweight = 0.5; oweight = 1-sweight;
    for (int j=0;j<otherele.NumNode();++j)
    {
      int pj = DRT::UTILS::getParentNodeNumberFromFaceNodeNumber(otherele.ParentElement()->Shape(),otherele.FaceParentNumber(),j);
  ...
  for (int i=0;i<ele.NumNode();++i)
  {
    ...
    for (int j=0;j<ele.NumNode();++j)
    {
      ...
      for (int d=0;d<dim;++d)
      {
        (*ele.GetNitscheContainer().rhs_p(pi*(dim+1)+d)) += shape(i)*ele.MoData().ParentPFPres()[pj]*shape(j)*normal(d)*val*sweight; //(v,k1 p1 n)
        ele.GetNitscheContainer().k_pp(ele.MoData().ParentPFDof()[pj*(dim+1)+dim])[(pi*(dim+1)+d)] -= shape(i)*shape(j)*normal(d)*val*sweight; //(v,k1 dp1/dp n)
        for (GEN::pairedvector<int,double>::const_iterator p=normal_deriv[d].begin();p!=normal_deriv[d].end();++p)
        {
          ele.GetNitscheContainer().k_pd(p->first)[(pi*(dim+1)+d)] -=
              shape(i)*shape(j)*ele.MoData().ParentPFPres()[pj]*p->second*val*sweight; //(v,k1 p1 dn/dd)

          ele.GetNitscheContainer().k_pd(p->first)[(pi*(dim+1)+dim)] +=
              shape(i)*sporosity*p->second*(ele.MoData().ParentPFVel()[pj*dim+d]-ele.MoData().ParentVel()[pj*dim+d])*shape(j)*val*sweight; // (k1 q1, phi (vF-vS) dn/dd)
        }
        ele.GetNitscheContainer().k_pd(ele.MoData().ParentDof()[pj*dim+d])[(pi*(dim+1)+dim)] -= // (k1 q1, phi (vF-dvS/dd) n)
            shape(i)*sporosity*normal(d)*shape(j)*val*dv_dd_*sweight;

        ele.GetNitscheContainer().k_pp(ele.MoData().ParentPFDof()[pj*(dim+1)+d])[(pi*(dim+1)+dim)] += // (k1 q1, phi (dvF/dvF-vS) n)
            shape(i)*sporosity*normal(d)*shape(j)*val*sweight;

        (*ele.GetNitscheContainer().rhs_p(pi*(dim+1)+(dim))) -=
            shape(i)*sporosity*normal(d)*(ele.MoData().ParentPFVel()[pj*dim+d]-ele.MoData().ParentVel()[pj*dim+d])*shape(j)*val*sweight; // (k1 q1, phi (vF-vS) n)
      }
      ele.GetNitscheContainer().k_pp(ele.MoData().ParentPFDof()[pj*(dim+1)+dim])[(pi*(dim+1)+dim)] += val*srelveln*shape(i)*sdphi_dp*shape(j)*sweight; // (k1 q1, phi/dp (vF-vS) n)
    }

    for (std::map<int,double>::iterator dJit = sJLin.begin(); dJit != sJLin.end(); ++dJit)
      ele.GetNitscheContainer().k_pd(dJit->first)[(pi*(dim+1)+dim)] += val*srelveln*shape(i)*sdphi_dJ*dJit->second*sweight; // (k1 q1, phi/dd (vF-vS) n)
  }
  ...
  if (oweight > 1e-16)
  {
    for (int i=0;i<ele.NumNode();++i)
    {
      ...
      for (int j=0;j<otherele.NumNode();++j)
      {
        ...
        for (int d=0;d<dim;++d)
        {
            (*ele.GetNitscheContainer().rhs_p(pi*(dim+1)+d)) += shape(i)*otherele.MoData().ParentPFPres()[pj]*othershape(j)*normal(d)*val*oweight; //(v,k2 p2 n)
            ele.GetNitscheContainer().k_pp(otherele.MoData().ParentPFDof()[pj*(dim+1)+dim])[(pi*(dim+1)+d)] -= shape(i)*othershape(j)*normal(d)*val*oweight; //(v,k2 dp2/dp n)
          for (GEN::pairedvector<int,double>::const_iterator p=normal_deriv[d].begin();p!=normal_deriv[d].end();++p)
          {
            ele.GetNitscheContainer().k_pd(p->first)[(pi*(dim+1)+d)] -=
                shape(i)*othershape(j)*otherele.MoData().ParentPFPres()[pj]*p->second*val*oweight; //(v,k2 p2 dn/dd)

            otherele.GetNitscheContainer().k_pd(p->first)[(pj*(dim+1)+dim)] +=
                othershape(j)*sporosity*p->second*(ele.MoData().ParentPFVel()[pi*dim+d]-ele.MoData().ParentVel()[pi*dim+d])*shape(i)*val*oweight; // (k2 q2, phi (vF-vS) dn/dd)
          }

          otherele.GetNitscheContainer().k_pd(ele.MoData().ParentDof()[pi*dim+d])[(pj*(dim+1)+dim)] -= // (k2 q2, phi (vF-dvS/dd) n)
              othershape(j)*sporosity*normal(d)*shape(i)*val*dv_dd_*oweight;

          otherele.GetNitscheContainer().k_pp(ele.MoData().ParentPFDof()[pi*(dim+1)+d])[(pj*(dim+1)+dim)] += // (k2 q2, phi (dvF/dvF-vS) n)
              othershape(j)*sporosity*normal(d)*shape(i)*val*oweight;

          (*otherele.GetNitscheContainer().rhs_p(pj*(dim+1)+(dim))) -=
              othershape(j)*sporosity*normal(d)*(ele.MoData().ParentPFVel()[pi*dim+d]-ele.MoData().ParentVel()[pi*dim+d])*shape(i)*val*oweight; // (k2 q2, phi (vF-vS) n)
        }
        otherele.GetNitscheContainer().k_pp(ele.MoData().ParentPFDof()[pi*(dim+1)+dim])[(pj*(dim+1)+dim)] += val*srelveln*othershape(j)*sdphi_dp*shape(i)*oweight; // (k2 q2, phi/dp (vF-vS) n)
      }
    }
    for (int j=0;j<otherele.NumNode();++j)
    {
      int pj = DRT::UTILS::getParentNodeNumberFromFaceNodeNumber(otherele.ParentElement()->Shape(),otherele.FaceParentNumber(),j);
      for (std::map<int,double>::iterator dJit = sJLin.begin(); dJit != sJLin.end(); ++dJit)
        otherele.GetNitscheContainer().k_pd(dJit->first)[(pj*(dim+1)+dim)] += val*srelveln*othershape(j)*sdphi_dJ*dJit->second*oweight; // (k2 q2, phi/dd (vF-vS) n)
    }
  }
}

bool CONTACT::CoIntegratorNitschePoro::GetPoroPressure(
...
void CONTACT::CoIntegratorNitschePoro::GetPoroQuantitiesatGP(
     MORTAR::MortarElement& ele,double* xi,double& spresgp, //(in)
     double& sJ,std::map<int,double>& sJLin,double& sporosity, double& sdphi_dp, double& sdphi_dJ) //out
{
...
template
void CONTACT::CoIntegratorNitschePoro::IntegrateTest<2>(
    const double,
    ...
    const double , const GEN::pairedvector<int,double>& , const GEN::pairedvector<int,double>&,
    const LINALG::Matrix<2,1>& test_dir, const std::vector<GEN::pairedvector<int,double> >& test_dir_deriv
    );
template
void CONTACT::CoIntegratorNitschePoro::IntegrateTest<3>(
    const double,
    MORTAR::MortarElement& ,
    const LINALG::SerialDenseVector& ,
    const LINALG::SerialDenseMatrix& ,
