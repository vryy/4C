...
void CONTACT::CoTSIInterface::AssembleLinStick(
    LINALG::SparseMatrix& linstickLMglobal,
    LINALG::SparseMatrix& linstickDISglobal,
    LINALG::SparseMatrix& linstickTEMPglobal,
    Epetra_Vector& linstickRHSglobal)
{
  ...
  if (ftype!=INPAR::CONTACT::friction_coulomb)
    dserror("only coulomb friction for CTSI");
  ...
  for (int i=0;i<sticknodes->NumMyElements();++i)
  {
    ...
    for (_CI p= dfrdT.begin();p!=dfrdT.end();++p)
    {
      if (constr_direction_==INPAR::CONTACT::constr_xyz)
        for (int j=0; j<Dim(); j++)
        {
          linstickTEMPglobal.Assemble(fac*jump_txi *p->second*txi (j),cnode->Dofs()[j],p->first);
          linstickTEMPglobal.Assemble(fac*jump_teta*p->second*teta(j),cnode->Dofs()[j],p->first);
        }
      else
      {
    ...
    for (_CI p= dfrdD.begin();p!=dfrdD.end();++p)
    {
      if (constr_direction_==INPAR::CONTACT::constr_xyz)
        for (int j=0; j<Dim(); j++)
        {
          linstickDISglobal.Assemble(fac*jump_txi *p->second*txi (j),cnode->Dofs()[j],p->first);
          linstickDISglobal.Assemble(fac*jump_teta*p->second*teta(j),cnode->Dofs()[j],p->first);
        }
      else
      {
...
void CONTACT::CoTSIInterface::AssembleLinSlip(
    LINALG::SparseMatrix& linslipLMglobal,
    LINALG::SparseMatrix& linslipDISglobal,
    LINALG::SparseMatrix& linslipTEMPglobal,
    Epetra_Vector& linslipRHSglobal)
{
  ...
  for (int i=0;i<slipnodes->NumMyElements();++i)
  {
    ...
    {
      row[0] = slipt->GID(2*i);   if (slipt->GID(2*i)   != cnode->Dofs()[1]) dserror("why");
      row[1] = slipt->GID(2*i)+1; if (slipt->GID(2*i)+1 != cnode->Dofs()[2]) dserror("why2");
    }
    else
      dserror("ERROR: AssemblelinStick: Dimension not correct");
    ...
    for (_CI p= dfrdT.begin();p!=dfrdT.end();++p)
    {
      if (constr_direction_==INPAR::CONTACT::constr_xyz)
        for (int j=0;j<Dim();++j)
        {
          linslipTEMPglobal.Assemble(-fac*(lm_txi +ct*jump_txi )*p->second*txi (j),cnode->Dofs()[j],p->first);
          linslipTEMPglobal.Assemble(-fac*(lm_teta+ct*jump_teta)*p->second*teta(j),cnode->Dofs()[j],p->first);
        }
      else
      {
    ...
    for (_CI p= dfrdD.begin();p!=dfrdD.end();++p)
    {
      if (constr_direction_==INPAR::CONTACT::constr_xyz)
        for (int j=0;j<Dim();++j)
        {
          linslipDISglobal.Assemble(-fac*(lm_txi +ct*jump_txi )*p->second*txi (j),cnode->Dofs()[j],p->first);
          linslipDISglobal.Assemble(-fac*(lm_teta+ct*jump_teta)*p->second*teta(j),cnode->Dofs()[j],p->first);
        }
      else
      {
...
void CONTACT::CoTSIInterface::AssembleLinConduct(
      LINALG::SparseMatrix& linConductDISglobal,
      LINALG::SparseMatrix& linConductTEMPglobal,
      LINALG::SparseMatrix& linConductThermoLMglobal,
      LINALG::SparseMatrix& linConductContactLMglobal)
{
  ...
  if (activenodes_->NumMyElements()==0)
    return;

  // heat transfer factors
  const double alpha_s = IParams().get<double>("HEATTRANSSLAVE");
  const double alpha_m = IParams().get<double>("HEATTRANSMASTER");
...
void CONTACT::CoTSIInterface::AssembleDualMassLumped(
    LINALG::SparseMatrix& dualMassGlobal,
    LINALG::SparseMatrix& linDualMassGlobal)
{
  // get out of here if not participating in interface
  if (!lComm())
    return;
    ...
    const Teuchos::RCP<Epetra_Map> node_rowmap)
{
  ...
  for (int j=0;j<node_rowmap->NumMyElements();++j)
  {
    ...
    switch (mode)
    {
    case LinDM_ThermoLM: lm = cnode->CoTSIData().ThermoLM(); break;
    case LinDM_Diss:
    {
...
void CONTACT::CoTSIInterface::AssembleDM_linDiss(
    LINALG::SparseMatrix* d_LinDissDISP,
    LINALG::SparseMatrix* m_LinDissDISP,
    LINALG::SparseMatrix* d_LinDissContactLM,
    ...
    const double fac)
{
  ...
  for (int j=0;j<activenodes_->NumMyElements();++j)
  {
    ...
    LINALG::Matrix<3,1> jump_tan; jump_tan.Multiply(tang_proj,jump);
...
void CONTACT::CoTSIInterface::AssembleLinLMnDM_Temp(const double fac,
    LINALG::SparseMatrix* lin_disp,
    LINALG::SparseMatrix* lin_lm)
{
  // get out of here if not participating in interface
  if (!lComm())
    return;
    ...
    for (_cip k = cnode->MoData().GetD().begin(); k!=cnode->MoData().GetD().end(); ++k)
    {
      ...
      for (int d=0;d<3;++d)
        for (_cip l=cnode->CoData().GetDerivN()[d].begin();l!=cnode->CoData().GetDerivN()[d].end();++l)
          if (abs(l->second)>1.e-12)
            lin_disp->FEAssemble(lm(d)*k->second*l->second*fac*temp_k,cnode->Dofs()[0],l->first);
...
void CONTACT::CoTSIInterface::AssembleInactive(LINALG::SparseMatrix* linConductThermoLM)
{
  ...
  for (int j=0;j<inactivenodes->NumMyElements();++j)
  {
    ...
    if (cnode->Active())
      dserror("something with active and inactive maps is not in order");
