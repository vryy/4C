    ...
    const Teuchos::RCP<const STR::MODELEVALUATOR::Data>& str_data_ptr)
{
  ...
  {
      std::istringstream input_file_linecontent(
          Teuchos::getNumericStringParameter(
              browndyn_params_list,
              "BEAMS_DAMPING_COEFF_PER_UNITLENGTH") );
    ...
    if ( not beams_damping_coefficient_prefactors_perunitlength_.empty() )
    {
      if ( beams_damping_coefficient_prefactors_perunitlength_.size() != 3 )
      {
        ...
        for (unsigned int i = 0; i < beams_damping_coefficient_prefactors_perunitlength_.size(); ++i )
          std::cout << beams_damping_coefficient_prefactors_perunitlength_[i] << " ";


        dserror("Expected 3 values for beam damping coefficients if specified via input file "
