    ...
    const std::map<int, LINALG::Matrix<3, 1> >& currentpositions)
{
  ...
  for (elemiter = elements.begin(); elemiter != elements.end(); ++elemiter)
  {
    ...
    const LINALG::Matrix<3, 2> xaabbEle = GEO::computeFastXAABB(
        currelement->Shape(), xyze_element, eleGeoType);
    XAABB = mergeAABB(XAABB, xaabbEle);
  }
...
>  initcurrentpos = currentpositions.begin()->second;
  for (int dim = 0; dim < 3; ++dim)
  {
    ...
    const std::map<int, std::set<int> >& elementList)
{
  ...
  for (std::map<int, std::set<int> >::const_iterator labelIter =
      elementList.begin(); labelIter != elementList.end(); labelIter++)
  {
        ...
        eleIter != (labelIter->second).end(); eleIter++)
    {
      ...
      GEO::EleGeoType eleGeoType(GEO::HIGHERORDER);
      GEO::checkRoughGeoType(element, xyze_element, eleGeoType);
      LINALG::Matrix<3, 2> xaabbEle = GEO::computeFastXAABB(element->Shape(),
          xyze_element, eleGeoType);
    ...
    const std::map<int, std::set<int> >& elementList)
{
  ...
  int label = nearestObjectInNode(dis, currentpositions, elementList,
      querypoint, minDistanceVec, nearestObject);

  // compute normal in the point found on or in the object
  LINALG::Matrix<3, 1> normal = getNormalAtSurfacePoint(dis, currentpositions,
      nearestObject);

  // compare normals and set label
  const double scalarproduct = minDistanceVec(0) * normal(0)
      + minDistanceVec(1) * normal(1) + minDistanceVec(2) * normal(2);
    ...
    LINALG::Matrix<3, 1>& minDistanceVec)
{
  ...
  // compute normal in the point found on or in the object
  LINALG::Matrix<3, 1> normal = getNormalAtSurfacePoint(triangleList, pointList,
      nearestObject);
    ...
    const int projectiontype)
{
  ...
  fillPotObjectsInNode(dis, currentpositions, elementList, gaussPoints,
      potObjects, cutoff_radius, label, projectiontype);
  return;
}
    ...
    std::map<int, std::set<int> >& elementList)
{
      ...
      elementList.begin(); labelIter != elementList.end(); labelIter++)
  {
    if (label != labelIter->first) // don't collect nodes which belong to the same label
    {
          ...
          eleIter != (labelIter->second).end(); eleIter++)
      {
        ...
        for (int i = 0;
            i < DRT::UTILS::getNumberOfElementCornerNodes(element->Shape());
            i++)
  ...
  for (std::map<int, std::set<int> >::const_iterator labelIter =
      nodeList.begin(); labelIter != nodeList.end(); labelIter++)
        ...
        nodeIter != (labelIter->second).end(); nodeIter++)
    {
      ...
      if (distance < (radius + GEO::TOL7))
      {
        for (int i = 0; i < dis.gNode(*nodeIter)->NumElement(); i++)
          elementMap[labelIter->first].insert(
              dis.gNode(*nodeIter)->Elements()[i]->Id());
      }
    }
    ...
    std::map<int, std::set<int> >& elementList)
{
  ...
  for (std::map<int, std::set<int> >::const_iterator labelIter =
    ...
    {
      const DRT::Node* node = dis.gNode(*nodeIter);
      const LINALG::Matrix<3, 1> x_node =
          currentpositions.find(node->Id())->second;
    ...
    std::map<int, std::set<int> >& elementList)
{
  ...
  for (std::set<int>::const_iterator pointIter = pointlist.begin();
      pointIter != pointlist.end(); pointIter++)
  {
    ...
    const LINALG::SerialDenseMatrix& xyze)
{
  ...
  CONTACT::CoElement* selement = dynamic_cast<CONTACT::CoElement*>(element);
    ...
    GEO::NearestObject& nearestObject)
{
  ...
  for (std::map<int, std::set<int> >::const_iterator labelIter =
        ...
        eleIter != (labelIter->second).end(); eleIter++)
    {
      ...
      DRT::Element* element = dis.gElement(*eleIter);
      pointFound = GEO::getDistanceToSurface(element, currentpositions, point,
          x_surface, distance);
      ...
      const std::vector<Teuchos::RCP<DRT::Element> > eleLines =
          element->Lines();
      for (int i = 0; i < element->NumLine(); i++)
      {
        pointFound = GEO::getDistanceToLine(eleLines[i].get(), currentpositions,
            point, x_surface, distance);
        if (pointFound && distance < min_distance)
        {
          pointFound = false;
          min_distance = distance;
          nearestObject.setLineObjectType(i, *eleIter, labelIter->first,
              x_surface);
        }
      }
      // collect nodes
      for (int i = 0;
          i < DRT::UTILS::getNumberOfElementCornerNodes(element->Shape()); i++)
        nodeList[labelIter->first].insert(element->NodeIds()[i]);
    }
    ...
    LINALG::Matrix<3, 1>& minDistCoords)
{
  ...
  for (std::map<int, std::set<int> >::const_iterator labelIter =
        ...
        eleIter != (labelIter->second).end(); eleIter++)
    {
      ...
      pointFound = GEO::getDistanceToLine(element, currentpositions, point,
          x_surface, distance);
      if (pointFound && distance < min_distance)
      {
        pointFound = false;
        nearestObject.setLineObjectType(1, *eleIter, labelIter->first,
            x_surface);
        min_distance = distance;
      }
    ...
    LINALG::Matrix<3, 1>& minDistCoords)
{
  ...
  pointFound = GEO::getDistanceToSurface(surfaceelement, currentpositions,
      point, x_surface, distance);
  if (pointFound && distance < min_distance)
  {
    pointFound = false;
  ...
  // run over all nodes
  for (std::map<int, LINALG::Matrix<3, 1> >::const_iterator nodeIter =
      currentpositions.begin(); nodeIter != currentpositions.end(); nodeIter++)
  {
    ...
    DRT::Node& nearnode)
{
  ...
  for (std::set<int>::const_iterator nodeIter = nodeList.begin();
  ...
  for (unsigned int i_gp = 0; i_gp < gaussPoints.size(); i_gp++)
  {
        ...
        elementList.begin(); labelIter != elementList.end(); labelIter++)
    {
      if (labelIter->first != label)
      {
        ...
        for (std::set<int>::const_iterator eleIter =
            (labelIter->second).begin(); eleIter != (labelIter->second).end();
            eleIter++)
        {
          // not const because otherwise no lines can be obtained
          DRT::Element* element = dis.gElement(*eleIter);
          pointFound = GEO::getDistanceToSurface(element, currentpositions,
              gaussPoints[i_gp], x_surface, distance);
          ...
          {
            pointFound = GEO::getDistanceToLine(eleLines[i_lines].get(),
                currentpositions, gaussPoints[i_gp], x_surface, distance);
            if (pointFound && distance < cutoff_radius
                && distance < min_distance)
            {
              pointFound = false;
              min_distance = distance;
              nearestObject.setLineObjectType(i_lines, *eleIter,
                  labelIter->first, x_surface);
            }
          }
          // collect nodes
          for (int i_node = 0;
              i_node
                  < DRT::UTILS::getNumberOfElementCornerNodes(element->Shape());
              i_node++)
        ...
        {
          const DRT::Node* node = dis.gNode(*nodeIter);
          GEO::getDistanceToPoint(node, currentpositions, gaussPoints[i_gp],
              distance);
    ...
    GEO::NearestObject& nearestObject)
{
  ...
  if (elementList.empty()
      || (elementList.size() == 1 && elementList.begin()->first == pointLabel))
      ...
      elementList.begin(); labelIter != elementList.end(); labelIter++)
  {
    if (labelIter->first != pointLabel)
    {
      ...
      {
        // not const because otherwise no lines can be obtained
        pointFound = GEO::getDistanceToSurface(triangleList[*eleIter],
            pointList, point, x_surface, distance);
        if (pointFound && distance < min_distance)
        {
          pointFound = false;
          min_distance = distance;
        ...
        for (int i = 0; i < 3; i++)
        {
          ...
          pointFound = GEO::getDistanceToLine(linePoints, pointList, point,
              x_surface, distance);
          if (pointFound && distance < min_distance)
          {
            pointFound = false;
        ...
        // collect nodes
        for (int i = 0;
            i < DRT::UTILS::getNumberOfElementCornerNodes(DRT::Element::tri3);
            i++)
  ...
  std::cout << "nearestObject.getPhysCoord() = " << nearestObject.getPhysCoord()
      << std::endl;
    ...
    double& distance)
{
  ...
  GEO::CurrentToSurfaceElementCoordinates(surfaceElement->Shape(),
      xyze_surfaceElement, point, elecoord);

  if (GEO::checkPositionWithinElementParameterSpace(elecoord,
      surfaceElement->Shape()))
  {
    GEO::elementToCurrentCoordinates(surfaceElement->Shape(),
        xyze_surfaceElement, elecoord, x_surface_phys);
  ...
  if (eleGeoType == GEO::HIGHERORDER)
  {
    ...
    for (int i = 0; i < surfaceElement->NumNode(); i++)
    {
          ...
          surfaceElement->Shape()))
      {
        ...
        GEO::elementToCurrentCoordinates(surfaceElement->Shape(),
            xyze_surfaceElement, elecoord, physcoord);
        // normal pointing away from the surface towards point
    ...
    double& distance)
{
  ...
  GEO::CurrentToSurfaceElementCoordinates(DRT::Element::tri3, xyze_triElement,
      point, elecoord);

  if (GEO::checkPositionWithinElementParameterSpace(elecoord,
      DRT::Element::tri3))
  {
    GEO::elementToCurrentCoordinates(DRT::Element::tri3, xyze_triElement,
        elecoord, x_surface_phys);
    // normal pointing away from the surface towards point
    distance_vector.Update(1.0, point, -1.0, x_surface_phys);
    distance = distance_vector.Norm2();
    pointFound = true;
    ...
    double& distance)
{
  ...
  GEO::CurrentToLineElementCoordinates(lineElement->Shape(), xyze_lineElement,
      point, elecoord);

  if (GEO::checkPositionWithinElementParameterSpace(elecoord,
      lineElement->Shape()))
  {
    GEO::elementToCurrentCoordinates(lineElement->Shape(), xyze_lineElement,
        elecoord, x_line_phys);
  ...
  if (eleGeoType == GEO::HIGHERORDER)
  {
    ...
    for (int i = 0; i < 2; i++)
    {
      ...
      GEO::CurrentToLineElementCoordinates(lineElement->Shape(),
          xyze_lineElement, point, elecoord);
    ...
    double& distance)
{
  ...
  GEO::CurrentToLineElementCoordinates(DRT::Element::line2, xyze_lineElement,
      point, elecoord);

  if (GEO::checkPositionWithinElementParameterSpace(elecoord,
      DRT::Element::line2))
  {
    GEO::elementToCurrentCoordinates(DRT::Element::line2, xyze_lineElement,
        elecoord, x_line_phys);
    // normal pointing away from the line towards point
    distance_vector.Update(1.0, point, -1.0, x_line_phys);
    distance = distance_vector.Norm2();
...
/*----------------------------------------------------------------------*
 ...
 |  of an element                                                       |
 *----------------------------------------------------------------------*/
void GEO::getDistanceToPoint(const int nodeId,
    const std::vector<GEO::InterfacePoint>& pointList,
    const LINALG::Matrix<3, 1>& point,
    double& distance)
{
 ...
 *----------------------------------------------------------------------*/
std::vector<int> GEO::getAdjacentSurfaceElementsToLine(const DRT::Node* node1,
    const DRT::Node* node2)
{
  ...
  if (adjacentElements.size() > 2)
    dserror(
        "more than two surfaces adjacent to a line - xfem coupling conditions might be not correct");
...
std::vector<int> GEO::getAdjacentTriElementsToLine(
    const std::vector<std::vector<int> >& triangleList, const int triangleId,
    ...
    GEO::NearestObject& nearestObject)
{
  ...
  case GEO::SURFACE_OBJECT:
  {
    ...
    const DRT::Element* surfaceElement = dis.gElement(
        nearestObject.getSurfaceId());
    const LINALG::SerialDenseMatrix xyze_surfaceElement =
        GEO::getCurrentNodalPositions(surfaceElement, currentpositions);
    GEO::CurrentToSurfaceElementCoordinates(surfaceElement->Shape(),
        xyze_surfaceElement, nearestObject.getPhysCoord(), elecoord);
    GEO::computeNormalToSurfaceElement(surfaceElement->Shape(),
        xyze_surfaceElement, elecoord, normal);
    break;
  }
  case GEO::LINE_OBJECT:
  {
    ...
    std::vector<int> adjacentElements = getAdjacentSurfaceElementsToLine(
        lineElement->Nodes()[0], lineElement->Nodes()[1]);
        ...
        eleIter != adjacentElements.end(); ++eleIter)
    {
      ...
      GEO::CurrentToSurfaceElementCoordinates(surfaceElement->Shape(),
          xyze_surfaceElement, nearestObject.getPhysCoord(), eleCoord);
      GEO::computeNormalToSurfaceElement(surfaceElement->Shape(),
          xyze_surfaceElement, eleCoord, surface_normal);
    ...
    const GEO::NearestObject& nearestObject)
{
  ...
  case GEO::SURFACE_OBJECT:
  {
    ...
    GEO::CurrentToSurfaceElementCoordinates(DRT::Element::tri3, xyze_triElement,
        nearestObject.getPhysCoord(), elecoord);
    GEO::computeNormalToSurfaceElement(DRT::Element::tri3, xyze_triElement,
        elecoord, normal);
    break;
  }
  case GEO::LINE_OBJECT:
  {
    ...
    std::vector<int> adjacentElements = getAdjacentTriElementsToLine(
        triangleList, nearestObject.getSurfaceId(), node1, node2);
    // run over all elements adjacent ot a line
    for (std::vector<int>::const_iterator eleIter = adjacentElements.begin();
        eleIter != adjacentElements.end(); ++eleIter)
    {
      ...
      {
        LINALG::Matrix<3, 1> node =
            pointList[triangleList[*eleIter][i]].getCoord();
        for (int j = 0; j < 3; j++)
          xyze_triElement(i, j) = node(j);
      }
      GEO::CurrentToSurfaceElementCoordinates(DRT::Element::tri3,
          xyze_triElement, nearestObject.getPhysCoord(), elecoord);
      GEO::computeNormalToSurfaceElement(DRT::Element::tri3, xyze_triElement,
          elecoord, tri_normal);
  ...
  {
    // run over all elements adjacent ot a node
    std::vector<int> adjacentElements = getAdjacentTriElementsToNode(
        triangleList, nearestObject.getNodeId());
 ...
 *----------------------------------------------------------------------*/
bool GEO::pointInTreeNode(const LINALG::Matrix<3, 1>& point,
 ...
 *----------------------------------------------------------------------*/
LINALG::Matrix<3, 2> GEO::mergeAABB(const LINALG::Matrix<3, 2>& AABB1,
 ...
 *----------------------------------------------------------------------*/
bool GEO::inSameNodeBox(const LINALG::Matrix<3, 2>& AABB_old,
    ...
    std::set<int>& intersectionCandidateIds)
{
  ...
  for (std::set<int>::const_iterator elementIter =
      (elementList.begin()->second).begin();
      elementIter != (elementList.begin()->second).end(); elementIter++)
  {
    if (intersectionOfXAABB<3>(currentXAABBs.find(*elementIter)->second,
        xfemXAABB))
    ...
    const int label)
{
      ...
      elementList.begin(); labelIter != elementList.end(); labelIter++)
  {
    if (label != labelIter->first) // don't collect nodes which belong to the same label
    {
      ...
      for (std::set<int>::const_iterator elementIter =
          (labelIter->second).begin(); elementIter != (labelIter->second).end();
          elementIter++)
      {
        if (intersectionOfXAABB<3>(currentXAABBs.find(*elementIter)->second,
            xfemXAABB))
