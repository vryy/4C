...
radius2_(0.0)
{
  ...
  if(DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::Damping>(beamcontactparams,"BEAMS_DAMPING")!= INPAR::BEAMCONTACT::bd_no)
  {
    if(beamcontactparams.get<double>("BEAMS_DAMPINGPARAM",-1.0)==-1.0 or beamcontactparams.get<double>("BEAMS_DAMPREGPARAM1",-1.0)==-1.0 or beamcontactparams.get<double>("BEAMS_DAMPREGPARAM2",-1.0)==-1.0)
                                                                   ...
                                                                   bool fdcheck)
{
  ...
  //If the contact opens once at a boundary element, the contactflag_ is set to false for the whole Newton iteration
  ...
  if (FADUTILS::CastToDouble(FADUTILS::Norm(xi1_)) < (1.0 + XIETATOL) && FADUTILS::CastToDouble(FADUTILS::Norm(xi2_)) < (1.0 + XIETATOL))
  {
    ...
    tangentproduct_ = FADUTILS::Norm(FADUTILS::ScalarProduct(r1_xi,r2_xi))/(FADUTILS::VectorNorm<3>(r1_xi)*FADUTILS::VectorNorm<3>(r2_xi));
    ...
    if (contactflag_ or dampingcontactflag_)
    {
      ...
      {
        //For very small tangent angles the contact is not evaluated. This would lead to a bad conditioned problem.
        //Therefore, the problem of almost parallel beams have to be modeled by an alternative contact approach
        beamsalmostparallel_ = true;
      }
      if (tangentproduct_<PARALLEL_ACTIVATION_VAL)
      {
        //In order to avoid an oscillation between the status "beamsalmostparallel_ = true" and the status "beamsalmostparallel_ = false"
        //during the Newton iterations we have introduced a gap between the value that deactivates contact and the value that activates contact.
        beamsalmostparallel_ = false;
      }
    }
...
//  std::cout << "iter_: " << iter_ << std::endl;
  ...
  //Apply algorithmic contact forces and stiffnesses that may improve the convergence behavior but will not change the physical results!
  #if defined(ALGORITHMICDAMP) or defined(BEAMCONTACTPTC) or defined(BASICSTIFFWEIGHT)
    EvaluateAlgorithmicForce(pp, &fint, N1, N2);
    EvaluateAlgorithmicStiff(pp,norm_delta_r,delta_r,stiffmatrix,r1,r2,r1_xi,r2_xi,r1_xixi,r2_xixi,N1,N2,N1_xi,N2_xi,N1_xixi,N2_xixi);
  #endif
                                                                            ...
                                                                            LINALG::TMatrix<TYPE, 3*numnodes*numnodalvalues, 1>* fc2_FAD)
{
  ...
  //**********************************************************************
  if (DRT::INPUT::IntegralValue<int>(bcparams_,"BEAMS_DAMPING")!=INPAR::BEAMCONTACT::bd_no and dampingcontactflag_)
  {
    DoNotAssemble=false;
    //********************************************************************
                                                                               ...
                                                                               const LINALG::TMatrix<TYPE, 3, 3*numnodes*numnodalvalues>& N2_xixi)
{
  ...
  //if the bool inactivestiff is true, the contact stiffness will always be applied in the first Newton steps for
  //pair which have been active in the last time step (even when they are currentely not active) -> This makes the algorithm more robust.
...
//      FADCheckLinXiAndLinEta(delta_r,r1_xi,r2_xi,r1_xixi,r2_xixi,N1,N2,N1_xi,N2_xi);
    ...
    // linearization of gap function which is equal to delta d
    ComputeLinGapt(delta_gap_t,delta_xi,delta_eta,delta_n,r1_xi,r2_xi,N1,N2,r1_old_,r2_old_,r1_xi_old_,r2_xi_old_);
    ...
    if (contactflag_ or (iter_==0 and inactivestiff and oldcontactflag_ and !beamendcontactopened_ and !beamsalmostparallel_))
    {
      DoNotAssemble = false;
      ...
      //The geoemtric part is only applied for gap_ < -BEAMS_BASICSTIFFGAP and when the contact is really active
      //(not for the "inactivestiff"-case)
                                                                                  ...
                                                                                  LINALG::TMatrix<TYPE, 3*numnodes*numnodalvalues, 1>* fc2_FAD)
{
  ...
  if (firstcallofstep_)
  {
    ...
    //velocities. No damping forces can be applied then. However, it is assumed that in the first time
    //step the pair is found by the contact search, it will not be active immediately and thus no damping
    //force is needed. If this happens anyway, an error is thrown in the method CheckContactStatus().
    for(int i=0;i<3;i++)
    {
      vc1(i)= 0.0;
      vc2(i)= 0.0;
  ...
  {
    //Attention: The velocities vc1 and vc2 are not the total contact point velocities, since they do not contain
    //the velocity contribution due to the change in xi and eta. However, since these contributions are perpendicular
    //on the normal_ vector, they are not needed in order to calculate g_t (this case is similar to the calculation
                                                                                   ...
                                                                                   const LINALG::TMatrix<TYPE, 3, 3*numnodes*numnodalvalues>& N2_xixi)
{
  ...
  if (currentlyincontact_ and iter_ > ITERMAX)
  {
    ...
    // linearization of gap function which is equal to delta d
    ComputeLinGapt(delta_gap_t,delta_xi,delta_eta,delta_n,r1_xi,r2_xi,N1,N2,r1_lastiter,r2_lastiter,r1_xi_lastiter,r2_xi_lastiter);
    ...
    for (int i=0;i<dim1;i++)
    {
      for (int j=0;j<dim1+dim2;j++)
      {
        stiffc1(i,j) += algdampbasicstifffac*N1T_normal(i) * (d*dfd*delta_gap_t(j)+dd*fd*delta_gap(j));
      }
    }
    ...
    for (int i=0;i<dim1;i++)
    {
      for (int j=0;j<dim1+dim2;j++)
      {
        stiffc2(i,j) += -algdampbasicstifffac*N2T_normal(i)*(d*dfd*delta_gap_t(j)+dd*fd*delta_gap(j));
      }
    }
...
//          for (int i=0;i<3;i++)
//          {
//            stiffc1(i+j*3*numnodalvalues,i+j*3*numnodalvalues) += 0.5 * BEAMCONTACTPTC * jacobi1;
//            stiffc1(i+j*3*numnodalvalues+3,i+j*3*numnodalvalues+3) += 0.5 * BEAMCONTACTPTCROT * jacobi1;
//          }
//        }
//        //********************************************************************
...
//          for (int i=0;i<3;i++)
//          {
//            stiffc2(i+j*3*numnodalvalues,dim1+i+j*3*numnodalvalues) += 0.5 * BEAMCONTACTPTC * jacobi2;
//            stiffc2(i+j*3*numnodalvalues+3,dim1+i+j*3*numnodalvalues+3) += 0.5 * BEAMCONTACTPTCROT * jacobi2;
//          }
//        }
        //2)*******************standard ptc diagonal terms********************
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ComputeLinXiAndLinEta( LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>&  delta_xi,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ComputeLinGap(LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>& delta_gap,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ComputeLinGapt(LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>& delta_gap_t,
                                                                       const LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>&  delta_xi,
                                                                       const LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>& delta_eta,
                                                                       ...
                                                                       const LINALG::TMatrix<TYPE, 3, 3*numnodes*numnodalvalues>& N1,
                                                                       const LINALG::TMatrix<TYPE, 3, 3*numnodes*numnodalvalues>& N2,
                                                                       const LINALG::TMatrix<TYPE, 3, 1>& r1_old,
                                                                       const LINALG::TMatrix<TYPE, 3, 1>& r2_old,
                                                                       const LINALG::TMatrix<TYPE, 3, 1>& r1_xi_old,
                                                                       const LINALG::TMatrix<TYPE, 3, 1>& r2_xi_old)
{
  ...
  for (int i=0;i<3;i++)
  {
    for (int j=0;j<dim1+dim2;j++)
    {
      delta_gap_t(j)+=(vc1(i)-vc2(i))*delta_n(i,j)+normal_(i)*(delta_vc1(i,j)-delta_vc2(i,j));
    }
  }
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ComputeLinNormal(LINALG::TMatrix<TYPE, 3, 2*3*numnodes*numnodalvalues>& delta_normal,
                                                                          const LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>&  delta_xi,
                                                                          const LINALG::TMatrix<TYPE, 2*3*numnodes*numnodalvalues, 1>& delta_eta,
                                                                          const TYPE& norm_delta_r,
                                                                          const LINALG::TMatrix<TYPE, 3, 1>& r1_xi,
...
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ClosestPointProjection()
{
  ...
  TYPE denom=((FADUTILS::ScalarProduct(t_2,t_2)*FADUTILS::ScalarProduct(t_1,t_1)-FADUTILS::ScalarProduct(t_2,t_1)*FADUTILS::ScalarProduct(t_2,t_1))/(FADUTILS::ScalarProduct(t_2,t_2)*FADUTILS::ScalarProduct(t_1,t_1)));
  ...
  if(denom>PARALLELTOL)
  {
    ...
    eta1 = (aux1+aux2)/(FADUTILS::ScalarProduct(t_2,t_2)*FADUTILS::ScalarProduct(t_1,t_1)-FADUTILS::ScalarProduct(t_2,t_1)*FADUTILS::ScalarProduct(t_2,t_1));
    ...
    eta2 = (aux1+aux2)/(FADUTILS::ScalarProduct(t_2,t_2)*FADUTILS::ScalarProduct(t_1,t_1)-FADUTILS::ScalarProduct(t_2,t_1)*FADUTILS::ScalarProduct(t_2,t_1));
  }
...
//    #endif
  ...
  {
    if(residual/residual0 < 1.0e-08 and residual0 > 10*BEAMCONTACTTOL and fabs(eta1)< 1.0+XIETATOL and fabs(eta2)< 1.0+XIETATOL)
    {
      ...
      dserror("Relative CPP residual norm is smaller than 1.0e-08 but Newton is not converged. Adapt your absolut CPP residual norm!");
    }
...
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::CalcPenaltyLaw()
{
    ...
    switch (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::PenaltyLaw>(bcparams_,"BEAMS_PENALTYLAW"))
    {
      ...
      case INPAR::BEAMCONTACT::pl_lpep:             //exponential regularization for positiv gaps. Here g0 represents the cut off radius!
      {
        ...
        if(gap_>0)
        {
          ...
          {
            std::cout << "Warning - g0: " << g0 << " f0*exp(-pp*g0/f0): " << f0*exp(-pp_*g0/f0) << "-> Choose higher cut-off radius g0!"<< std::endl;
          }
        }
        else
        {
  ...
  #ifdef MAXFORCE
    //If a maximum penalty force is defined, we regularize the penalty force and apply the original / a secant penalty parameter
    if(fp_ > MAXFORCE)
    {
...
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::CalcDampingLaw()
{
  ...
  if(gd1<gd2)
    dserror("BEAMS_DAMPREGPARAM1 has to be larger or equal to BEAMS_DAMPREGPARAM2!");
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::GetShapeFunctions( LINALG::TMatrix<TYPE, 3, 3*numnodes*numnodalvalues>& N1,
 ...
 *-----------------------------------------------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::AssembleShapefunctions(const LINALG::TMatrix<TYPE,1,numnodes*numnodalvalues>& N_i,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ComputeCoordsAndDerivs(LINALG::TMatrix<TYPE,3,1>& r1,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::ComputeOldCoordsAndDerivs(LINALG::TMatrix<TYPE,3,1>& r1_old,
                                                                                ...
                                                                                const LINALG::TMatrix<TYPE,3,3*numnodes*numnodalvalues>& N2_xi)
{
  ...
  //important: in order to compute the derivatives correctely, the current parameter coordinates xi and eta of the contact points have to be applied!
  for (int i=0;i<3;i++)
  {
    for (int j=0;j<3*numnodes*numnodalvalues;j++)
    {
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::EvaluateOrthogonalityCondition(LINALG::TMatrix<TYPE,2,1>& f,
                                                                                        const LINALG::TMatrix<TYPE,3,1>& delta_r,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::EvaluateLinOrthogonalityCondition( LINALG::TMatrix<TYPE,2,2>& df,
                                                                       ...
                                                                       std::map<std::pair<int,int>, Teuchos::RCP<Beam3contactinterface > >& contactpairmap)
{
  ...
  //Initialize normal_old_ in the first step with valid closest point projection (in this case the vector normal_old_ is zero, since no
  //valid normal vector was available in the last time step). In case of "sliding contact", i.e. when normal_old_ has already been calculated
  //out of the neighbor element (GetNeighborNormalOld), this is not allowed. Otherwise we would overwrite the vector normal_old_ which has already
  //been calcualted via the neighbor element. (For this reason we check with the norm of normal_old_ and not with the variable firstcall_).
  if (FADUTILS::CastToDouble(FADUTILS::Norm(FADUTILS::ScalarProduct(normal_old_, normal_old_))) < NORMALTOL)
  {
    for (int i=0;i<3;i++)
  ...
  {
    if (FADUTILS::CastToDouble(FADUTILS::Norm(FADUTILS::ScalarProduct(normal_, normal_old_))) < NORMALTOL)
      dserror("ERROR: Rotation too large! --> Choose smaller Time step!");

    gap = FADUTILS::Signum(FADUTILS::ScalarProduct(normal_, normal_old_))*norm_delta_r - radius1_ - radius2_;
  ...
  // thus gap_original==gap_ if the origianl gap function definition is applied
...
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::CheckContactStatus(const double& pp)
{
  ...
  //Contact is not allowed to happen in the first time step a pair was found by the contact search. An exception
  //is the first time step of a simulation, where no history is available.
  if((contactflag_ or dampingcontactflag_) and firsttimestep_ and numstep_>1)
    dserror("Contact is not allowed to happen in the first time step a pair was found by the contact search! Choose larger search radius or smaller time step!");
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
std::vector<int> CONTACT::Beam3contactnew<numnodes, numnodalvalues>::GetGlobalDofs(const DRT::Node* node)
{
...
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::UpdateClassVariablesStep()
{
  ...
  //This method is called at the end of the time step for all element pairs found by the contact search, therefore all updates are done here!
  ...
  for (int i=0;i<3;i++)
  {
    ele1_delta_pos1+=(ele1pos_old_(i)-FADUTILS::CastToDouble(ele1pos_(i)))*(ele1pos_old_(i)-FADUTILS::CastToDouble(ele1pos_(i)));
    ele1_delta_pos2+=(ele1pos_old_(3*numnodalvalues+i)-FADUTILS::CastToDouble(ele1pos_(3*numnodalvalues+i)))*(ele1pos_old_(3*numnodalvalues+i)-FADUTILS::CastToDouble(ele1pos_(3*numnodalvalues+i)));
    ele2_delta_pos1+=(ele2pos_old_(i)-FADUTILS::CastToDouble(ele2pos_(i)))*(ele2pos_old_(i)-FADUTILS::CastToDouble(ele2pos_(i)));
    ele2_delta_pos2+=(ele2pos_old_(3*numnodalvalues+i)-FADUTILS::CastToDouble(ele2pos_(3*numnodalvalues+i)))*(ele2pos_old_(3*numnodalvalues+i)-FADUTILS::CastToDouble(ele2pos_(3*numnodalvalues+i)));
  }
  ...
  //Change of nodal positions is not allowed to be larger than MAXDELTADFAC*searchboxinc_ (except for the first time step where elepos_old_=0)
  //TODO: Also Check element midpoint!
  if ((ele1_delta_pos1 > MAXDELTADFAC*searchboxinc_ or ele1_delta_pos2 > MAXDELTADFAC*searchboxinc_ or ele2_delta_pos1 > MAXDELTADFAC*searchboxinc_ or ele2_delta_pos2 > MAXDELTADFAC*searchboxinc_) and !firsttimestep_)
  {
    ...
    dserror("Change in nodal positions per time step is larger than prescribed maximum MAXDELTADFAC*searchboxinc_! Choose smaller time step or larger search radius!");
  }

  //No contact should happen in the first time step an element has been found by the search algorithm (since we need history
  //variables like normal_old_ in order to detect contact). However, there is the following dilemma when this criterion has to be checked:
  //On the one hand, the vector normal_old is needed in order to decide, if we have contact in the first step or not. On the other hand,
  //the vector normal_old_ can only be calculated correctly, when we guarantee, that no contact has happened in the first step. Thus,
  //we choose a heuristic criterion: when at the end of the first time step the distance between the two elements is larger than 2*MAXDELTADFAC*searchboxinc_
  //no contact has happened. The reason for this choice is as follows: If the elements had crossed in the current time step (contact active)
  //and the distance at the end of the time step is 2*MAXDELTADFAC*searchboxinc_, the distance in the beginning of this time step (=distance at the end of last time step)
  //would have been zero in the worst case. In this case, the contact pair would have already been found in the last time step (2*searchboxinc_ > 2*MAXDELTADFAC*searchboxinc_).
  //These assumption do not hold for the first time step in the simulation, where pairs can be found that already penetrate each other!
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::SetClassVariables(const double& pp, Teuchos::ParameterList timeintparams)
{
  pp_=pp;
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::UpdateElePos(Epetra_SerialDenseMatrix& newele1pos,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::UpdateEleSmoothTangents(std::map<int,LINALG::Matrix<3,1> >& currentpositions)
{
...
{
  //In this method we calculate an approximation for the vector normal_old_ based on the neighbor element pair.
  int id1 = -1.0;
  int id2 = -1.0;
  ...
  //compute non-unit normal
  for (int i=0;i<3;i++)
    delta_r(i) = FADUTILS::CastToDouble(r1_(i)-r2_(i));

  bool beamsclose = false;

  if(delta_r.Norm2()<(radius1_ + radius2_ + 2*MAXDELTADFAC*searchboxinc_))
    beamsclose = true;
  ...
  //If the considered pair had no valid closest point pair in the last time step (see ClosestPointProjection: eta1=eta2=1e+12),
  //we can not find the correct neighbor element pair! If the coordinate values of xi1 and xi2 are larger than NEIGHBORNORMALTOL,
  //the normal of the direct neighbor does not provide a good approximation for the own normal_old_ vector. In both cases, we exit here
  //and calculate no approximation for normal_old_;
  if(fabs(xi1_old_) < NEIGHBORNORMALTOL and fabs(xi2_old_) < NEIGHBORNORMALTOL and beamsclose and tangentproduct_ < PARALLEL_DEACTIVATION_VAL)
  {
    ...
    else if (xi2_old_ > 1.0)
    {
      if (neighbors2_->GetRightNeighbor()!=NULL)
        id2 = neighbors2_->GetRightNeighbor()->Id();
    }
    else
    {
      id2=element2_->Id();
    ...
    //One ID being -1 means, that no neighbor element has been found. Consequently, the considered beam element is a boundary element
    //and no sensible information for normal_old is available/needed.
    if (id1 == -1 or id2 == -1)
    {
    ...
    else if (id1<id2)
    {
      ...
      {
        //It is already tested in GetNormalOld() that the corresponding pair had a valid closest point solution in the last time step if &normal_old_!=NULL!
        if(contactpairmap[std::make_pair(id1,id2)]->GetNormalOld()!=NULL)
        {
          normal_old_=*(contactpairmap[std::make_pair(id1,id2)]->GetNormalOld());

          if(contactpairmap[std::make_pair(id1,id2)]->FirstTimeStep()==true and beamsclose==true)
            dserror("Vector normal_old_ requested but not available in the first time step the pair has been found: Choose larger search radius!!!");

          normaloldset=true;

          //If the neighbor pair had a valid cpp solution at last time step the value ||normal_old_=0|| should not be possible!
          if (FADUTILS::CastToDouble(FADUTILS::Norm(FADUTILS::ScalarProduct(normal_old_, normal_old_))) < NORMALTOL and beamsclose)
          {
            std::cout << "pair: " << element1_->Id() << " / " << element2_->Id() << ":" << std::endl;
            std::cout << "neighbor pair: " << id1 << " / " << id2 << ":" << std::endl;
            dserror("The vector normal_old_ is not allowed to be zero when taken from neighbor element pair!");
          }
        }
        else
        {
          std::cout << "Warning: No valid vector normal_old_ of neighbor pair " << id1 << " / " << id2 << " available in order to calculate normal_old_ for pair " << element1_->Id() << " / " << element2_->Id() << "!" << std::endl;
        }
      }
      else
      {
        if(beamsclose)
        {
          std::cout <<"Warning: Neighbor pair "<< id1 << " / " << id2 << " not found in order to calculate normal_old_ for pair " << element1_->Id() << " / " << element2_->Id() << "! Choose larger search radius!" << std::endl;
      ...
      if(contactpairmap.find(std::make_pair(id2,id1))!=contactpairmap.end())
      {
        //It is already tested in GetNormalOld() that the corresponding pair had a valid closest point solution in the last time step if &normal_old_!=NULL!
         if(contactpairmap[std::make_pair(id2,id1)]->GetNormalOld()!=NULL)
         {
           ...
           if(contactpairmap[std::make_pair(id2,id1)]->FirstTimeStep()==true and beamsclose==true)
             dserror("Vector normal_old_requested but not available in the first time step the pair has been found: Choose larger search radius!!!");
         ...
         {
           std::cout << "Warning: No valid vector normal_old_ of neighbor pair " << id2 << " / "<< id1 << " available in order to calculate normal_old_ for pair " << element1_->Id() << " / " << element2_->Id() << "!" << std::endl;
         }
      }
      else
      {
        if(beamsclose)
        {
          std::cout <<"Warning: Neighbor pair "<< id2 << " / " << id1 << " not found in order to calculate normal_old_ for pair " << element1_->Id() << " / " << element2_->Id() << "! Choose larger search radius!" << std::endl;
          dserror("Stopped due to Warning above!");
        }
      }
    }
  ...
  //If no valid vector normal_old_ has been delivered from the neighbor element pair we set it to zero. In this case normal_old_ is initialized with normal_ later
  //on in the method ComputeNormal().
  if (normaloldset==false)
    for (int i=0;i<3;i++)
      normal_old_=0.0;

  if(beamsclose and (FADUTILS::Norm(xi1_-xi1_old_)>MAXDELTAXIETA or FADUTILS::Norm(xi2_-xi2_old_)>MAXDELTAXIETA) and tangentproduct_ < PARALLEL_DEACTIVATION_VAL)
  {
    std::cout << "Pair: " << element1_->Id() << " / " << element2_->Id() << std::endl;
    std::cout << "xi1: " << xi1_ << "xi2: " << xi2_ << std::endl;
    ...
    std::cout << "Warning: Neighbor normal required for an element with |xi1_-xi1_old_|>MAXDELTAXIETA or |xi2_-xi2_old_|>MAXDELTAXIETA. Choose smaller time step or larger element size!" << std::endl;
  }
...
double CONTACT::Beam3contactnew<numnodes, numnodalvalues>::GetJacobi(DRT::Element* element1)
{
   ...
   *----------------------------------------------------------------------*/
  template<const int numnodes , const int numnodalvalues>
  void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::FADCheckLinXiAndLinEta(const LINALG::TMatrix<TYPE, 3, 1>& delta_r,
                                                                                  const LINALG::TMatrix<TYPE, 3, 1>& r1_xi,
                                                                                  const LINALG::TMatrix<TYPE, 3, 1>& r2_xi,
                                                                                  const LINALG::TMatrix<TYPE, 3, 1>& r1_xixi,
                                                                                  const LINALG::TMatrix<TYPE, 3, 1>& r2_xixi,
   ...
   *----------------------------------------------------------------------*/
  template<const int numnodes , const int numnodalvalues>
  void CONTACT::Beam3contactnew<numnodes, numnodalvalues>::FADCheckLinOrthogonalityCondition(const LINALG::TMatrix<TYPE, 3, 1>& delta_r,
                                                                                             const LINALG::TMatrix<TYPE, 3, 1>& r1_xi,
                                                                                             const LINALG::TMatrix<TYPE, 3, 1>& r2_xi)
  {
    LINALG::TMatrix<TYPE,2,1>f(true);
