                                      ...
                                      const Epetra_Vector& disrow, double alphaf)
{
  ...
  // intialize global contact force vectors
  ...
  for (int i=0;i<(int)pairs_.size();++i)
  {
    ...
    // least one node of one of the two elements of the pair	
  	...
  	if (firstisincolmap || secondisincolmap) 
  		pairs_[i]->Evaluate(stiffmatrix,*fc_,currentpp_);
  }
...
{
  //**********************************************************************
	// get positions off all nodes (fully overlapping map) and store
...
{
  // TODO more sophisticated criteria moght be added later
