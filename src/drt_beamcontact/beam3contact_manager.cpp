      ...
      step_(0)
{
  ...
  if (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::OctreeType>(sbeamcontact_, "BEAMS_OCTREE") !=
  ...
  else
  {
    ...
    ComputeSearchRadius();
    tree_ = Teuchos::null;
    if (!pdiscret_.Comm().MyPID()) std::cout << "\nBrute Force Search" << std::endl;
  }

  if (!pdiscret_.Comm().MyPID())
  {
    if (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::Strategy>(sbeamcontact_, "BEAMS_STRATEGY") ==
        INPAR::BEAMCONTACT::bstr_penalty)
      std::cout << "Strategy                 Penalty" << std::endl;
    else if (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::Strategy>(
    ...
    switch (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::PenaltyLaw>(
    ...
    if (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::PenaltyLaw>(
    ...
    if (DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::Damping>(sbeamcontact_, "BEAMS_DAMPING") ==
    ...
    const Epetra_Vector& disrow, Teuchos::ParameterList timeintparams, bool newsti, double time)
{
  ...
  // SetCurrentPositions) and current tangents (will be calculated in SetState)
  ...
  if (DRT::INPUT::IntegralValue<INPAR::STR::MassLin>(sstructdynamic_, "MASSLIN") !=
...
void CONTACT::Beam3cmanager::InitBeamContactDiscret()
{
  ...
  if (noderowmap_->NumMyElements()) stproc.push_back(BTSolDiscret().Comm().MyPID());
  if (elerowmap_->NumMyElements()) estproc.push_back(BTSolDiscret().Comm().MyPID());
    ...
    std::map<int, LINALG::Matrix<3, 1>>& currentpositions, const Epetra_Vector& disccol)
{
  ...
  for (int i = 0; i < FullNodes()->NumMyElements(); ++i)
  {
    ...
    if (BEAMCONTACT::BeamNode(*node) and !BEAMCONTACT::BeamCenterlineNode(*node)) continue;
    ...
    std::map<int, LINALG::Matrix<3, 1>>& currentpositions, const Epetra_Vector& disccol)
{
  ...
  // SetCurrentPositions loop over all beam contact nodes
  for (int i = 0; i < FullNodes()->NumMyElements(); ++i)
  {
    // get node pointer
    DRT::Node* node = BTSolDiscret().lColNode(i);
    ...
    if (numnodalvalues_ == 2 and BEAMCONTACT::BeamNode(*node))
    {
                 ...
                 // nodes the function call node->Elements()[0]->NumNode() would tell you)
      {
        if (node->Elements()[0]->Nodes()[i]->Id() == node->Id() and
            node->Elements()[0]->ElementType() == DRT::ELEMENTS::Beam3ebType::Instance())
        {
          const DRT::ELEMENTS::Beam3eb* ele =
          ...
          currtan(2) = ((ele->Tref())[i])(2) + disccol[BTSolDiscret().DofColMap()->LID(dofnode[5])];
                 ...
                 node->Elements()[0]->ElementType() == DRT::ELEMENTS::Beam3kType::Instance())
        {
          const DRT::ELEMENTS::Beam3k* ele =
              dynamic_cast<const DRT::ELEMENTS::Beam3k*>(node->Elements()[0]);
          currtan(0) = ((ele->Tref())[i])(0) + disccol[BTSolDiscret().DofColMap()->LID(dofnode[3])];
          currtan(1) = ((ele->Tref())[i])(1) + disccol[BTSolDiscret().DofColMap()->LID(dofnode[4])];
          currtan(2) = ((ele->Tref())[i])(2) + disccol[BTSolDiscret().DofColMap()->LID(dofnode[5])];
        }
        else if (node->Elements()[0]->Nodes()[i]->Id() == node->Id() and
                 node->Elements()[0]->ElementType() == DRT::ELEMENTS::Beam3rType::Instance())
        {
          const DRT::ELEMENTS::Beam3r* ele =
              dynamic_cast<const DRT::ELEMENTS::Beam3r*>(node->Elements()[0]);
  ...
  int smoothing =
      DRT::INPUT::IntegralValue<INPAR::BEAMCONTACT::Smoothing>(sbeamcontact_, "BEAMS_SMOOTHING");
  ...
  for (int i = 0; i < ProblemDiscret().NumMyColElements(); i++)
  {
    ...
    if (eot == DRT::ELEMENTS::Beam3ebType::Instance())
    {
      const DRT::ELEMENTS::Beam3eb* beam3ebelement =
          dynamic_cast<const DRT::ELEMENTS::Beam3eb*>(element);
    ...
    {
      const DRT::ELEMENTS::Beam3r* beam3relement =
      ...
      for (unsigned int k = 0; k < conds1.size(); ++k)
      {
        ...
        for (unsigned int j = 0; j < conds2.size(); ++j)
        {
          ...
          if (npotlaw1 == npotlaw2 and npotlaw1 > 0)
          {
            ...
            // be careful here, as npotlaw =1 corresponds to first entry of ki_/mi_, therefore index
            // 0
            if (npotlaw1 > (int)ki_->size())
              dserror(
                  "number of potential law specified in line charge condition exceeds number of "
                  "defined potential laws!");
            btbpotpairs_[i]->Evaluate(
                *stiffc_, *fc_, currconds, ki_->at(npotlaw1 - 1), mi_->at(npotlaw1 - 1));
          }
        }
      }
    }
  ...
  if ((int)formattedelementpairs.size() > 0)
  {
    ...
    for (int k = 0; k < (int)formattedelementpairs.size(); ++k)
    {
      ...
      if (ele1_type != pair1_ele1_type or
          (BEAMCONTACT::BeamElement(*(formattedelementpairs[k])[1]) and
    ...
    else if (BEAMCONTACT::SolidContactElement(*(formattedelementpairs[k])[1]))
    {
      ...
      if (!isalreadyinpairs)
      {
        ...
        btsolpairs_.push_back(CONTACT::Beam3tosolidcontactinterface::Impl(
            (formattedelementpairs[k])[1]->NumNode(), numnodes_, numnodalvalues_, ProblemDiscret(),
            BTSolDiscret(), dofoffsetmap_, ele1, ele2, sbeamcontact_));
        if (currid1 <= currid2)
          btsolpairmap_[std::make_pair(currid1, currid2)] = btsolpairs_[btsolpairs_.size() - 1];
        else
          dserror("Element 1 has to have the smaller element-ID. Adapt your contact search!");
  ...
  for (int k = 0; k < (int)formattedelementpairs.size(); k++)
  {
    ...
    if (validinteraction)
    {
      ...
      if (BEAMCONTACT::BeamElement(*(formattedelementpairs[k])[1]))
      {
        ...
        {
          btbpotpairs_.push_back(
              CONTACT::Beam3tobeampotentialinterface::Impl(numnodes_, numnodalvalues_,
                  ProblemDiscret(), BTSolDiscret(), dofoffsetmap_, ele1, ele2, sbeampotential_));
      ...
      else if (BEAMCONTACT::RigidsphereElement(*(formattedelementpairs[k])[1]) and potbtsph_)
      {
        ...
        {
          btsphpotpairs_.push_back(
              CONTACT::Beam3tospherepotentialinterface::Impl(numnodes_, numnodalvalues_,
                  ProblemDiscret(), BTSolDiscret(), dofoffsetmap_, ele1, ele2, sbeampotential_));
          btsphpotpairmap_[std::make_pair(currid1, currid2)] =
              btsphpotpairs_[btsphpotpairs_.size() - 1];
        }
      }
 ...
 *----------------------------------------------------------------------*/
void CONTACT::Beam3cmanager::GmshOutput(
              ...
              for (int i = 0; i < 3; ++i)
              {
                for (int j = 0; j < nnodescl; ++j)
                {
                  ...
                  nodaltangents(i, j) = ((ele->Tref())[j])(i) +
                                        disccol[BTSolDiscret().DofColMap()->LID(dofnode[6 + i])];
                }
              }
        ...
        else if (eot == DRT::ELEMENTS::Beam3ebType::Instance())
        {
          ...
          for (int i = 0; i < 3; ++i)
          {
            for (int j = 0; j < element->NumNode(); ++j)
            {
              ...
              nodalcoords(i, j) = referenceposition + displacement;
              nodaltangents(i, j) =
                  ((ele->Tref())[j])(i) + disccol[BTSolDiscret().DofColMap()->LID(dofnode[3 + i])];
            }
          }
        ...
        else if (eot == DRT::ELEMENTS::Beam3kType::Instance())
        {
          ...
          for (int i = 0; i < 3; ++i)
          {
            for (int j = 0; j < element->NumNode(); ++j)
            {
              ...
              {
                if (ele->RotVec())
                  nodaltangents(i, j) = ((ele->Theta0())[j])(i) +
                                        disccol[BTSolDiscret().DofColMap()->LID(dofnode[3 + i])];
                else
                  nodaltangents(i, j) = ((ele->Tref())[j])(i) +
                                        disccol[BTSolDiscret().DofColMap()->LID(dofnode[3 + i])];
              }
 ...
 *----------------------------------------------------------------------*/
void CONTACT::Beam3cmanager::Reactions(
    ...
    const DRT::Element* thisele, std::stringstream& gmshfilecontent)
{
  ...
  {
    const DRT::ELEMENTS::Rigidsphere* thisparticle =
        static_cast<const DRT::ELEMENTS::Rigidsphere*>(thisele);
 ...
 *----------------------------------------------------------------------*/
void CONTACT::Beam3cmanager::GMSH_GetSurfColor(const DRT::Element* element, const int& n_surfNodes,
