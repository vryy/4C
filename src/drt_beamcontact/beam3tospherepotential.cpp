 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
CONTACT::Beam3tospherepotential<numnodes, numnodalvalues>::Beam3tospherepotential(const Beam3tospherepotential& old):
...
void CONTACT::Beam3tospherepotential<numnodes, numnodalvalues>::EvaluateFpotandStiffpot_LargeSepApprox()
{
  ...
  for (int gp1=0; gp1 < numgp; ++gp1)
  {
    ...
    for (int i=0; i<(numnodes*numnodalvalues); ++i)
    {
      ...
      for (int idim=0; idim<3; ++idim)
      {
        stiffpot1_(3*i+idim,3*(numnodes*numnodalvalues)+idim) += norm_dist_exp1 * N1_i[gp1](i) *  q1q2_JacFac_GaussWeights;

        for (int jdim=0; jdim<3; ++jdim)
        {
          stiffpot1_(3*i+idim,3*(numnodes*numnodalvalues)+jdim) -= norm_dist_exp2 * N1_i[gp1](i) * dist_dist_T(idim,jdim) *  q1q2_JacFac_GaussWeights;
        }
      }
    ...
    for (int j=0; j<(numnodes*numnodalvalues); ++j)
    {
      ...
      for (int idim=0; idim<3; ++idim)
      {
        ...
        for (int jdim=0; jdim<3; ++jdim)
        {
          stiffpot2_(idim,3*j+jdim) -= norm_dist_exp2 * dist_dist_T(idim,jdim) * N1_i[gp1](j) *  q1q2_JacFac_GaussWeights;
        }
      }
    }
    ...
    for (int idim=0; idim<3; ++idim)
    {
      stiffpot2_(idim,3*(numnodes*numnodalvalues)+idim) -= norm_dist_exp1 * q1q2_JacFac_GaussWeights;

      for (int jdim=0; jdim<3; ++jdim)
      {
        stiffpot2_(idim,3*(numnodes*numnodalvalues)+jdim) += norm_dist_exp2 * dist_dist_T(idim,jdim) * q1q2_JacFac_GaussWeights;
      }
    }
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3tospherepotential<numnodes,numnodalvalues>::AssembleStiffpot(LINALG::SparseMatrix& stiffmatrix)
{
  // get dimensions for vectors fpot1 and fpot2
  const int dim1 = 3*numnodes*numnodalvalues;
  const int dim2 = 3;
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3tospherepotential<numnodes, numnodalvalues>::GetShapeFunctions( std::vector<LINALG::Matrix<1, numnodes*numnodalvalues> >& N1_i,
...
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3tospherepotential<numnodes, numnodalvalues>::ComputeCoords(LINALG::TMatrix<TYPE,3,1>& r,
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
std::vector<int> CONTACT::Beam3tospherepotential<numnodes, numnodalvalues>::GetGlobalDofs(const DRT::Node* node)
{
 ...
 *----------------------------------------------------------------------*/
template<const int numnodes , const int numnodalvalues>
void CONTACT::Beam3tospherepotential<numnodes, numnodalvalues>::UpdateElePos(Epetra_SerialDenseMatrix& newele1pos,
...
Teuchos::RCP<CONTACT::Beam3tospherepotentialinterface> CONTACT::Beam3tospherepotentialinterface::Impl( const int numnodes,
