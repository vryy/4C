...
EHL::Base::Base(const Epetra_Comm& comm,
    ...
    dry_contact_(DRT::INPUT::IntegralValue<bool>(DRT::Problem::Instance()->ElastoHydroDynamicParams(),"DRY_CONTACT_MODEL"))
{
...
//  Teuchos::RCP<Epetra_Vector> idisp = LINALG::CreateVector(*(mergedrowmapextr_->Map(0)), true);//Structure displacement at the lubricated interface
//  mergedrowmapextr_->ExtractVector(disp,0,idisp);
  ...
  //Calculate the tangential fractions of the structure velocities at the interface and provide them to the lubrication field
 ...
 *----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> EHL::Base::EvaluateFluidForce(Teuchos::RCP<const Epetra_Vector> pressure)
{
  ...
  if (inf_gap_toggle_lub_!=Teuchos::null)
    for (int i=0;i<lubrication_->LubricationField()->Prenp()->Map().NumMyElements();++i)
    {
      if (abs(inf_gap_toggle_lub_->operator [](
          inf_gap_toggle_lub_->Map().LID(lubrication_->LubricationField()->Prenp()->Map().GID(i)))-1)<1.e-2)
        lubrication_->LubricationField()->Prenp()->operator [](i)=0.;
    }

  //Forces on the interfaces due to the fluid traction
  Teuchos::RCP<Epetra_Vector>  slaveiforce = Teuchos::rcp(new Epetra_Vector(mortaradapter_->GetDMatrix()->DomainMap()));
  Teuchos::RCP<Epetra_Vector>  masteriforce = Teuchos::rcp(new Epetra_Vector(mortaradapter_->GetMMatrix()->DomainMap()));
  ...
  Teuchos::RCP<Epetra_Vector> strforce = Teuchos::rcp(new Epetra_Vector(*(structure_->DofRowMap())));
...
void EHL::Base::SetLubricationSolution( Teuchos::RCP<const Epetra_Vector> pressure)
{
  ...
  //Note that the mid-point values (gen-alpha) of the interface forces are evaluated in STR::TimIntGenAlpha::EvaluateForceResidual()
    ...
    )
{
  ...
  Teuchos::RCP<Epetra_Vector> poiseuille_force = Teuchos::rcp(new Epetra_Vector(*mortaradapter_->SlaveDofMap()));
  m.Apply(*p_int_full,*poiseuille_force);

  Teuchos::RCP<Epetra_Vector>  slave_psl  = Teuchos::rcp(new Epetra_Vector(mortaradapter_->GetDMatrix()->DomainMap()));
  Teuchos::RCP<Epetra_Vector>  master_psl = Teuchos::rcp(new Epetra_Vector(mortaradapter_->GetMMatrix()->DomainMap()));
  ...
  //f_master = +M^T*t // attention: no minus sign here: poiseuille points in same direction on slave and master side
  ...
  if (slaveiforce ->Update(1.,*slave_psl ,1.))
    dserror("Update failed");
  if (masteriforce->Update(1.,*master_psl,1.))
    dserror("Update failed");
}
    ...
    )
{
  ...
  Teuchos::RCP<Epetra_Vector> couette_force = Teuchos::rcp(new Epetra_Vector(*mortaradapter_->SlaveDofMap()));
  couette_force->Multiply(-1.,*visc_vec_str,*hinv_relV,0.);

  Teuchos::RCP<Epetra_Vector>  slave_cou  = Teuchos::rcp(new Epetra_Vector(mortaradapter_->GetDMatrix()->DomainMap()));
  Teuchos::RCP<Epetra_Vector>  master_cou = Teuchos::rcp(new Epetra_Vector(mortaradapter_->GetMMatrix()->DomainMap()));
  //f_slave = D^T*t
  ...
  // add the contribution
  if (slaveiforce ->Update(1.,*slave_cou ,1.))
    dserror("Update failed");
  if (masteriforce->Update(-1.,*master_cou,1.))
    dserror("Update failed");
}
...
{
  Teuchos::RCP<Epetra_Vector> avVelLub = ada_strDisp_to_lubDisp_->MasterToSlave(mortaradapter_->AvTangVel());
...
//  const Teuchos::RCP<LINALG::SparseMatrix> mortardinv = mortaradapter_->GetDinvMatrix();
  ...
  //get the weighted gap and store it in slave dof map (for each node, the scalar value is stored in the 0th dof)
...
void EHL::Base::SetMeshDisp( Teuchos::RCP<const Epetra_Vector> disp )
{
  ...
  Teuchos::RCP<Epetra_Vector> slaveidisp = LINALG::CreateVector(*(slaverowmapextr_->Map(0)), true);//Structure displacement at the lubricated interface
...
{
  if(not DRT::INPUT::IntegralValue<int>(((DRT::Problem::Instance()->ElastoHydroDynamicParams())),"UNPROJ_ZERO_DBC"))
    return;

  Teuchos::RCP<Epetra_FEVector> inf_gap_toggle = Teuchos::rcp(new Epetra_FEVector(*mortaradapter_->SlaveDofMap(),true));
  for (int i=0;i<mortaradapter_->Interface()->SlaveRowNodes()->NumMyElements();++i)
  {
    DRT::Node* node = mortaradapter_->Interface()->Discret().gNode(mortaradapter_->Interface()->SlaveRowNodes()->GID(i));
  ...
  Teuchos::RCP<Epetra_Vector> exp = Teuchos::rcp(new Epetra_Vector(*ada_strDisp_to_lubPres_->MasterDofMap()));
  ...
  lubrication_->LubricationField()->InfGapToggle()=Teuchos::rcp(new Epetra_Vector(*inf_gap_toggle_lub_));
}
...
void EHL::Base::SetupFieldCoupling(const std::string struct_disname, const std::string lubrication_disname)
{
  ...
  if (structdis.is_null())
    dserror("structure dis does not exist");
  if ( lubricationdis.is_null())
    dserror("lubrication dis does not exist");
  ...
  //A mortar coupling adapter, using the "EHL Coupling Condition" is set up. The Coupling is between
  //the slave- and the master-side interface of the structure. Dofs, which, need to be transfered from
  //the master-side to the lubrication field, need to be mortar-projected to the slave-side interface
  //and then transfered by a matching-node coupling,  and vice versa. The matching node coupling is
  //defined below.

  std::vector<int> coupleddof (ndim,1);
  mortaradapter_ = Teuchos::rcp(new ADAPTER::CouplingEhlMortar);
  mortaradapter_ ->Setup(structdis,
                          structdis,
                         coupleddof,
                        "EHLCoupling");

  if (DRT::INPUT::IntegralValue<INPAR::CONTACT::SolvingStrategy>(
        mortaradapter_->Interface()->IParams(),"STRATEGY")
      !=INPAR::CONTACT::solution_ehl)
    dserror("you need to set ---CONTACT DYNAMIC: STRATEGY   Ehl");

  Teuchos::RCP<Epetra_Vector> idisp = LINALG::CreateVector(*(structdis->DofRowMap()), true);//Structure displacement at the lubricated interface
  ...
  Teuchos::RCP<Epetra_Map> mergeddofrowmap = LINALG::MergeMap(masterdofrowmap,slavedofrowmap, false);

  //Map extractors with the structure dofs as full maps and local interface maps
  slaverowmapextr_ =  Teuchos::rcp(new LINALG::MapExtractor(*(structdis->DofRowMap()),slavedofrowmap,false));
  masterrowmapextr_ =  Teuchos::rcp(new LINALG::MapExtractor(*(structdis->DofRowMap()),masterdofrowmap,false));
  mergedrowmapextr_ =  Teuchos::rcp(new LINALG::MapExtractor(*(structdis->DofRowMap()),mergeddofrowmap,false));
  ...
  ada_strDisp_to_lubDisp_->SetupCoupling(*structdis,
      *lubricationdis,
      *strucnodes,
      *lubrinodes,
      ndim,
  ...
  ada_lubPres_to_lubDisp_->SetupCoupling(
      *lubricationdis,
      *lubricationdis,
  ...
  ada_strDisp_to_lubPres_->SetupCoupling(
      mortaradapter_->Interface()->Discret(),
      *lubricationdis,
      *mortaradapter_->Interface()->SlaveRowNodes(),
      *lubricationdis->NodeRowMap(),
      1,
      true,
      1.e-3);
  ...
  lubrimaptransform_ = Teuchos::rcp(new LINALG::SparseMatrix(*(lubricationdis->DofRowMap(1)),81,false,false));
...
//  ApplyLubricationCouplingState(lubrication_->LubricationField()->Prenp());
  ...
  if (dry_contact_)
  {
    ...
    Teuchos::RCP<Epetra_Vector> active = Teuchos::rcp(new Epetra_Vector(*StructureField()->Discretization()->NodeRowMap()));
    Teuchos::RCP<Epetra_Vector> slip   = Teuchos::rcp(new Epetra_Vector(*StructureField()->Discretization()->NodeRowMap()));
  ...
  if (dry_contact_)
  {
    ...
    Teuchos::RCP<Epetra_Vector> ne= Teuchos::rcp(new Epetra_Vector(*StructureField()->Discretization()->DofRowMap()));
    Teuchos::RCP<Epetra_Vector> te   = Teuchos::rcp(new Epetra_Vector(*StructureField()->Discretization()->DofRowMap()));
    LINALG::Export(*n,*ne);
    LINALG::Export(*t,*te);
  ...
  // Addtitional output on the lubrication field
  {
    ...
    Teuchos::RCP<Epetra_Vector> height_ex= Teuchos::rcp(new Epetra_Vector(*ada_lubPres_to_lubDisp_->SlaveDofMap()));
    ...
    if (inf_gap_toggle_lub_!=Teuchos::null)
      lubrication_->LubricationField()->DiscWriter()->WriteVector("no_gap_DBC",inf_gap_toggle_lub_,IO::dofvector);
  }
