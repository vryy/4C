...
// | (gamma_T_growth*(phi1-w_nl_crit)/(w_nl_env-w_nl_crit)*heaviside((phi1-w_nl_crit)/(w_nl_env-w_nl_crit))*heaviside(p_t_crit-p2))*(1-phi2)*porosity*S2 - lambda*phi2*porosity*S2 |
...
// |                                                                                                                                                                               |
// | INPUT DEFINITION:                                                                                                                                                             |
// | POROMULTIPHASESCATRA_FUNCTION TUMOR_GROWTH_LAW_HEAVISIDE NUMPARAMS 5 PARAMS gamma_T_growth 9.6e-6 w_nl_crit 2.0e-6 w_nl_env 4.2e-6 lambda 0.0 p_t_crit 1.0e9                  |
// +-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
    ...
    PoroMultiPhaseScaTraFunction()
{
  ...
  if(funct_params.size()!=5)
    dserror("Wrong size of funct_params for TUMOR_GROWTH_LAW_HEAVISIDE, it should have exactly\n"
    ...
    const std::vector<std::pair<std::string,double> >& constants)
{
  ...
  // they should have been added in exactly the same way in fluidporo_multiphase_singlereaction, but order
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
double POROMULTIPHASESCATRA::TumorGrowthLawHeaviside::Evaluate(
    const int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
  // Check order (only once since it does not change)
  if(not order_checked_)
  ...
  // evaluate function
  const double functval = (macaulay*heaviside_pres)*(1-necr_frac)*porosity*S2-lambda*porosity*oxy_mass_frac*S2;
  return functval;
}

/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::vector<double> POROMULTIPHASESCATRA::TumorGrowthLawHeaviside::EvaluateDerivative(
    int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
  ...
  const double saturationderiv = (macaulay*heaviside_pres)*(1-necr_frac)*porosity-lambda*necr_frac*porosity;
  deriv[4] = saturationderiv;

  // set porosity derivs
  const double porosityderiv = (macaulay*heaviside_pres)*(1-necr_frac)*S2-lambda*necr_frac*S2;
  deriv[6] = porosityderiv;

  // set scalar derivs w.r.t. phi1
  const double oxygenderiv = gamma_T_growth*heaviside_oxy*heaviside_pres*1.0/(w_nl_env-w_nl_crit)*(1-necr_frac)*porosity*S2;
  deriv[7] = oxygenderiv;

  // set scalar derivs w.r.t. phi2
  const double necroticderiv = - (macaulay*heaviside_pres)*porosity*S2-lambda*porosity*S2;
...
// | POROMULTIPHASESCATRA_FUNCTION NECROSIS_LAW_HEAVISIDE NUMPARAMS 5 PARAMS gamma_t_necr 9.6e-6 w_nl_crit 2.0e-6 w_nl_env 4.2e-6 delta_a_t 0.0 p_t_crit 1.0e9                     |
// +-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
    ...
    PoroMultiPhaseScaTraFunction()
{
  // Check size
  if(funct_params.size()!=5)
    dserror("Wrong size of funct_params for NECROSIS_LAW_HEAVISIDE, it should have exactly\n"
...
{
  // safety check for correct ordering of variables and constants
  // they should have been added in exactly the same way in scatra_ele_calc_multiporo_reaction, but order
  // might be different if we do not use exactly three fluid phases
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
double POROMULTIPHASESCATRA::NecrosisLawHeaviside::Evaluate(
    const int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::vector<double> POROMULTIPHASESCATRA::NecrosisLawHeaviside::EvaluateDerivative(
    int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
    ...
    PoroMultiPhaseScaTraFunction()
{
  ...
  if(funct_params.size()!=5)
    dserror("Wrong size of funct_params for NECROSIS_LAW_HEAVISIDE, it should have exactly\n"
            "5 funct_params (in this order) gamma_nl_growth, gamma_0_nl, w_nl_crit, w_nl_env and p_t_crit");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
double POROMULTIPHASESCATRA::OxygenConsumptionLawHeaviside::Evaluate(
    const int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
  ...
  // evaluate heaviside
  const double heaviside_oxy((oxy_mass_frac-w_nl_crit) > 0. ? 1. : 0.);
  const double macaulay = gamma_nl_growth*(oxy_mass_frac-w_nl_crit)/(w_nl_env-w_nl_crit)*heaviside_oxy;
  const double heaviside_pres((p_t_crit-p2) > 0. ? 1. : 0.);
...
// | POROMULTIPHASESCATRA_FUNCTION TUMOR_GROWTH_LAW_HEAVISIDE_OXY NUMPARAMS 5 PARAMS gamma_T_growth 9.6e-6 w_nl_crit 2.0e-6 w_nl_env 4.2e-6 lambda 0.0 p_t_crit 1.0e9              |
// +-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
    ...
    PoroMultiPhaseScaTraFunction()
{
  // Check size
  if(funct_params.size()!=5)
    dserror("Wrong size of funct_params for TUMOR_GROWTH_LAW_HEAVISIDE_OXY, it should have exactly\n"
            "5 funct_params (in this order) gamma_T_growth, w_nl_crit, w_nl_env, lambda and p_t_crit");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
double POROMULTIPHASESCATRA::TumorGrowthLawHeavisideOxy::Evaluate(
    const int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
std::vector<double> POROMULTIPHASESCATRA::TumorGrowthLawHeavisideOxy::EvaluateDerivative(
    int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
double POROMULTIPHASESCATRA::TumorGrowthLawHeavisideNecro::Evaluate(
    const int index,
    const std::vector<std::pair<std::string,double> >& variables,
    const std::vector<std::pair<std::string,double> >& constants)
{
