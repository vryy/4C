...
POROMULTIPHASESCATRA::PoroMultiPhaseScaTraArtCouplBase::PoroMultiPhaseScaTraArtCouplBase(
    ...
    evaluate_in_ref_config_(DRT::INPUT::IntegralValue<int>(meshtyingparams,"EVALUATE_IN_REF_CONFIG")),
    comm_(arterydis->Comm())
{
  ...
  if(coupleddofs_cont_.size() == 1 and coupleddofs_art_.size() == 1 and coupleddofs_cont_[0] < 0 and coupleddofs_art_[0] < 0)
  {
  ...
  if(myrank_ == 0)
  {
    ...
    if(num_coupled_dofs_ > 0)
    {
      ...
      for (int i = 0; i < num_coupled_dofs_; i++)
        std::cout << "<                        " << coupleddofs_cont_[i]+1 << "   <------>   " << coupleddofs_art_[i]+1 << "          >" << std::endl;
    }
    else
 ...
 *----------------------------------------------------------------------*/
const Teuchos::RCP<const Epetra_Map>& POROMULTIPHASESCATRA::PoroMultiPhaseScaTraArtCouplBase::FullMap() const
{
 ...
 *----------------------------------------------------------------------*/
const Teuchos::RCP<LINALG::MultiMapExtractor>&  POROMULTIPHASESCATRA::PoroMultiPhaseScaTraArtCouplBase::GlobalExtractor() const
{
