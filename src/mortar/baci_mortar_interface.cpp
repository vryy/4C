    ...
    for (int i = 0; i < Discret().NodeRowMap()->NumMyElements(); ++i)
    {
      ...
      if (node->IsSlave())
      {
        ...
        switch (shape)
        {
          ...
          default:
          {
            dserror("Lin/Lin interpolation of LM only for line3/tri6/quad8/quad9 mortar elements");
            break;
          }
        }  // switch(Shape)
      }    // if (IsSlave())
...
void MORTAR::MortarInterface::InitializeDataContainer()
{
  ...
  for (int i = 0; i < numMySlaveColumnElements; ++i)
  {
    ...
    auto* mele = dynamic_cast<MortarElement*>(ele);
    ...
    const double meanVelocity)
{
  ...
  for (int lid = 0; lid < SlaveColElements()->NumMyElements(); ++lid)
  {
    ...
    auto* mtrele = dynamic_cast<MORTAR::MortarElement*>(ele);
...
void MORTAR::MortarInterface::Redistribute()
{
...

  {
    ...
    Teuchos::RCP<const Epetra_CrsGraph> snodegraph = REBALANCE::BuildGraph(idiscret_, sroweles);
    ...
    std::tie(srownodes, scolnodes) = REBALANCE::RebalanceNodeMaps(snodegraph, rebalanceParams);
  }
    ...
    const Teuchos::RCP<Epetra_Comm>& comm, const int parts, const double imbalance) const
{
  if (not HasMaSharingRefInterface())
  {
    ...
    Teuchos::RCP<const Epetra_CrsGraph> nodegraph = REBALANCE::BuildGraph(idiscret_, roweles);
    ...
    std::tie(rownodes, colnodes) = REBALANCE::RebalanceNodeMaps(nodegraph, rebalanceParams);
  }
  else
  {
    ...
    const bool isFinalParallelDistribution, const double meanVelocity)
{
  ...
  {
    dserror("Invalid redundancy type.");
  }
}
...
void MORTAR::MortarInterface::CreateSearchTree()
{
  ...
  if (SearchAlg() == INPAR::MORTAR::search_binarytree)
  {
    ...
    auto updatetype = DRT::INPUT::IntegralValue<INPAR::MORTAR::BinaryTreeUpdateType>(
...
void MORTAR::MortarInterface::EvaluateGeometry(std::vector<Teuchos::RCP<MORTAR::IntCell>>& intcells)
{
  ...
  auto algo = DRT::INPUT::IntegralValue<INPAR::MORTAR::AlgorithmType>(imortar_, "ALGORITHM");
  ...
  for (int i = 0; i < selecolmap_->NumMyElements(); ++i)
  {
    ...
    auto* selement = dynamic_cast<MortarElement*>(ele1);
    ...
    for (int j = 0; j < selement->MoData().NumSearchElements(); ++j)
    {
      ...
      auto* melement = dynamic_cast<MortarElement*>(ele2);
      ...
      if (selement->IsQuad())
      {
        dserror("Geometry evaluation only implemented for first order elements!");
      }
      // noquad!
      else
      {
    ...
    const Epetra_Map* snoderowmap, const Teuchos::RCP<MORTAR::ParamsInterface>& mparams_ptr)
{
  ...
  switch (algo)
  {
    ...
    default:
    {
      dserror("Unknown discretization type for constraints!");
      break;
    }
  }
}
    ...
    const Epetra_Map& selecolmap, const Teuchos::RCP<MORTAR::ParamsInterface>& mparams_ptr)
{
  ...
  for (int i = 0; i < selecolmap.NumMyElements(); ++i)
  {
    ...
    DRT::Element* ele1 = idiscret_->gElement(gid1);
    if (!ele1) dserror("Cannot find slave element with gid %d", gid1);
    ...
    for (int j = 0; j < selement->MoData().NumSearchElements(); ++j)
    {
      int gid2 = selement->MoData().SearchElements()[j];
      DRT::Element* ele2 = idiscret_->gElement(gid2);
      if (!ele2) dserror("Cannot find master element with gid %d", gid2);
      auto* melement = dynamic_cast<MortarElement*>(ele2);
    ...
    MortarNode& mrtrnode, std::vector<MortarElement*>& meles) const
{
  ...
  {
    auto* adjcele = dynamic_cast<MortarElement*>(adjeles[j]);
    ...
    for (int k = 0; k < adjcele->MoData().NumSearchElements(); ++k)
    {
      ...
      if (!mele) dserror("Cannot find master element with gid %", gid2);
      auto* melement = dynamic_cast<MortarElement*>(mele);
...
void MORTAR::MortarInterface::EvaluateSearchBruteForce(const double& eps)
{
  ...
  for (int i = 0; i < selecolmap_->NumMyElements(); ++i)
  {
    ...
    auto* mrtrelement = dynamic_cast<MortarElement*>(element);
  ...
  else if (dim_ == 3)
  {
    ...
    dserror("Problem dimension must be either 2D or 3D.");
  ...
  for (int i = 0; i < selecolmap_->NumMyElements(); i++)
  {
    ...
    for (int j = 0; j < melefullmap->NumMyElements(); j++)
    {
      ...
      if (nintercepts == kdop / 2)
      {
        ...
        DRT::Element* element = idiscret_->gElement(sgid);
        auto* selement = dynamic_cast<MORTAR::MortarElement*>(element);
    ...
    const Teuchos::RCP<MORTAR::ParamsInterface>& mparams_ptr)
{
  ...
  else if (Dim() == 3)
  {
    ...
    dserror("Dimension for Mortar coupling must be either 2D or 3D.");
  // *********************************************************************
        ...
        for (int j = 0; j < rowsize; ++j)
        {
                                 ...
                                 shapefcn_ == INPAR::MORTAR::shape_petrovgalerkin))
          {
            ...
            else
            {
              ...
              if (row != col && abs(val) > 1.0e-12) dserror("D-Matrix is not diagonal!");
    ...
    CORE::LINALG::SparseMatrix& invtrafo, std::set<int>& donebefore)
{
  ...
  for (int i = 0; i < snoderowmap_->NumMyElements(); ++i)
  {
    ...
    auto* mrtrele = dynamic_cast<MortarElement*>(mrtrnode->Elements()[0]);
    ...
    switch (shape)
    {
      ...
      default:
      {
        dserror("Trafo matrix only for line3/tri6/quad8/quad9 contact elements");
        break;
      }
    }  // switch(Shape)
    ...
    {
      dserror("Undefined node type (corner, edge, center)");
    }
  }
...
void MORTAR::MortarInterface::DetectTiedSlaveNodes(int& founduntied)
{
  ...
  for (int i = 0; i < snoderowmap_->NumMyElements(); ++i)
  {
    ...
    {
      dserror("Inconsistency in tied/untied node detection");
    }
  }
    ...
    const Teuchos::ParameterList& outParams, const std::vector<std::string>& requiredEntries) const
{
  ...
  for (const auto& requiredEntry : requiredEntries)
  {
    ...
    {
      dserror("Parameter list is missing the required entry '%s'.", (requiredEntry).c_str());
      return false;
    }
  }
