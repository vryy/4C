...
*----------------------------------------------------------------------*/
DRT::ELEMENTS::XDiff3::ActionType DRT::ELEMENTS::XDiff3::convertStringToActionType(
              const string& action) const
{
  ...
  if (action == "calc_fluid_systemmat_and_residual")
    ...
    act = XDiff3::store_xfem_info;
  else if (action == "get_density")
    act = XDiff3::get_density;
  else if (action == "reset")
    act = XDiff3::reset;
  else if (action == "set_output_mode")
    act = XDiff3::set_output_mode;
  else
    dserror("Unknown type of action for XDiff3");
  return act;
}
...
/*----------------------------------------------------------------------*
 ...
 //                                                          gammi 08/07
 *----------------------------------------------------------------------*/
DRT::ELEMENTS::XDiff3::StabilisationAction DRT::ELEMENTS::XDiff3::ConvertStringToStabAction(
  const string& action) const
{
  DRT::ELEMENTS::XDiff3::StabilisationAction act = stabaction_unspecified;
 ...
 *----------------------------------------------------------------------*/
int DRT::ELEMENTS::XDiff3::Evaluate(ParameterList& params,
                                     ...
                                     Epetra_SerialDenseVector&)
{
  ...
  const DRT::ELEMENTS::XDiff3::ActionType act = convertStringToActionType(action);
...
//          dserror("this happens, when element is intersected, but we skip the stress unknown due to small surface integral");
    ...
    case calc_fluid_systemmat_and_residual:
    {
      ...
      DRT::ELEMENTS::XDiff3::MyState mystate(discretization,lm,true);
...
//            }
    ...
    case calc_fluid_stationary_systemmat_and_residual:
    {
      ...
      DRT::ELEMENTS::XDiff3::MyState mystate(discretization,lm,false);
 ...
 *----------------------------------------------------------------------*/
int DRT::ELEMENTS::XDiff3::EvaluateNeumann(ParameterList& params,
                                            DRT::Discretization&      discretization,
...
DRT::UTILS::GaussRule3D DRT::ELEMENTS::XDiff3::getOptimalGaussrule(const DiscretizationType& distype)
{
 ...
 *---------------------------------------------------------------------*/
void DRT::ELEMENTS::XDiff3::f3_int_beltrami_err(
...
/*---------------------------------------------------------------------*
 *---------------------------------------------------------------------*/
void DRT::ELEMENTS::XDiff3::UpdateOldDLMAndDLMRHS(
...
/*---------------------------------------------------------------------*
 *---------------------------------------------------------------------*/
void DRT::ELEMENTS::XDiff3::CondenseDLMAndStoreOldIterationStep(
