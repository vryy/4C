...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
UQ::RandomVariable::RandomVariable(const Teuchos::ParameterList& rfp,
    int random_variable_id, int seed) :
    pdf_(none),
    param_1_(0),
    param_2_(0),
    is_bounded_(false),
    ...
    myrank_(-1)
{
 // initialize parameters
  ...
  // init pdf
  INPAR::MLMC::MarginalPdf mpdf = DRT::INPUT::IntegralValue<
  INPAR::MLMC::MarginalPdf>(rfp, "PDF");
  switch (mpdf)
  {
  ...
  normalgen_ = Teuchos::rcp(
      new boost::variate_generator<randnumgen&, boost::normal_distribution<> >(
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
double UQ::RandomVariable::EvalVariable( const double paracont_parameter,
     const bool writetofile, const bool output)
{
  ...
  if (is_bounded_)
  {
    if (temp_rv_val > upper_bound_)
      temp_rv_val = upper_bound_;
    if (temp_rv_val < lower_bound_)
      temp_rv_val = lower_bound_;
  }
  if(writetofile)
  {
    if (myrank_ == 0)
