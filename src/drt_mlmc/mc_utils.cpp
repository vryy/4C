...
void UQ::ComputePeakAndQuantile(
    Teuchos::RCP<std::vector <double > > values1,
    Teuchos::RCP<std::vector <double > > values2,
    Teuchos::RCP<std::vector <int> > ele_ids,
    Teuchos::RCP<std::pair <double , std::pair <int , double> > >  peak,
    Teuchos::RCP<std::pair<double,std::pair <int , double> > > quantile99,
    int tot_num_elements,
    std::string mode,
    const Epetra_Comm& mycomm)
{
  ...
  for (std::list< std::pair<double, std::pair< int, double > > >::iterator it = my_quantity.begin(); it != my_quantity.end(); it++)
  {
  ...
  LINALG::Gather<int>(my_quantity_id_vec,my_quantity_id_gathered_vec,my_numprocs,&allproc[0], mycomm);
  LINALG::Gather<double>(my_quantity_value1_vec,my_quantity_values1_gathered_vec,my_numprocs,&allproc[0], mycomm);
  LINALG::Gather<double>(my_quantity_value2_vec,my_quantity_values2_gathered_vec,my_numprocs,&allproc[0], mycomm);
