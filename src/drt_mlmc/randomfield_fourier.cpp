...
UQ::RandomFieldFourier::RandomFieldFourier(unsigned int seed,
    ...
    RandomField(discret, rfp)
{
  ...
  if (dim_ != 3)
    dserror(
        "Dimension of random field for fourier series representation must be 3, fix your input file");
  ...
  if (periodicity_ > 2.0 * largestlength_)
    dserror(
        "Periodic length of random field is very large compared to your discretization, you should consider a smaller value");
  ...
  if (d_ > 0.35 * periodicity_)
    dserror(
        "correlation length is to large, choose a smaller correlation length or another representatoin method ");
  ...
  INPAR::MLMC::CorrStruct cstruct = DRT::INPUT::IntegralValue<
      INPAR::MLMC::CorrStruct>(rfp, "CORRSTRUCT");
  switch (cstruct)
  {
  ...
  INPAR::MLMC::MarginalPdf mpdf = DRT::INPUT::IntegralValue<
      INPAR::MLMC::MarginalPdf>(rfp, "MARGINALPDF");
  switch (mpdf)
  {
    ...
    for (int k2 = 0; k2 <= m_; k2++)
    {
      for (int k3 = 0; k3 <= m_; k3++)
      {
        if (((k1 * k1) + (k2 * k2) + (k3 * k3)) < k_trunk_threshold_)
        {
          sum_ck =
              sum_ck
                  + (ComputeLambda3D(k1) * ComputeLambda3D(k2)
                      * ComputeLambda3D(k3));
          num_k_ = num_k_ + 1;
        }
      }
  ...
  kb_ = Teuchos::rcp(
      new std::vector<std::vector<int> >(num_k_, std::vector<int>(3, 0)));
  int index = 0;
  for (int k1 = 0; k1 < m_; k1++)
  {
    for (int k2 = 0; k2 < m_; k2++)
  ...
  xi_ = Teuchos::rcp(
      new std::vector<std::vector<double> >(num_k_,
...
/*----------------------------------------------------------------------------*/
double UQ::RandomFieldFourier::EvalFieldAtLocation(std::vector<double> location,
    double local_mean, double paracont_parameter, bool writetofile, bool output)
{
  // safety check
  if(mean_!=0)
  ...
  if (marginal_pdf_ == normal)
    temp_rf_val = det_value_paracont_ * (1.0 - paracont_parameter)
        + (paracont_parameter) * (local_mean + sigma_0_ * tempgp);
  else if (marginal_pdf_ == lognormal)
    temp_rf_val = det_value_paracont_ * (1.0 - paracont_parameter)
        + (paracont_parameter) * exp((local_mean + sigma_0_ * tempgp));
  else
  {
    dserror("unknown marginal density ");
  }
...
double UQ::RandomFieldFourier::ComputeLambda3D(int k)
{
  ...
  {
    lambda = d_ * sqrt(pi_) / (periodicity_)
        * exp(-(pow(k * pi_ * d_, 2) / (4 * pow(periodicity_, 2.0))));
