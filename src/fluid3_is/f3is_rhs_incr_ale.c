...
#ifdef FLUID3_IS_TERM4
    /* Stabilisierung der Konvektion ( L_pres_p) */
    eforce_(vi*3 + 8) += -(ttimetauM*(conv_c_(vi) + conv_g_(vi))*gradp_(0)) ;
    eforce_(vi*3 + 9) += -(ttimetauM*(conv_c_(vi) + conv_g_(vi))*gradp_(1)) ;
    eforce_(vi*3 + 10) += -(ttimetauM*(conv_c_(vi) + conv_g_(vi))*gradp_(2)) ;
#endif
