    ...
    : beam_radius_extension_factor_(-1), sphere_radius_extension_factor_(-1)
{
  ...
  dsassert(std::signbit(beam_bounding_volume_scaling_), "Scaling parameter needs to be positiv!");

  sphere_radius_extension_factor_ = params_list.get<double>("SPHERE_RADIUS_EXTENSION_FACTOR");
  dsassert(std::signbit(sphere_bounding_volume_scaling_), "Scaling parameter needs to be positiv!");
