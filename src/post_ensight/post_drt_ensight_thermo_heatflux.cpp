/*!
  ...
  Maintainer: Caroline Danowski
              ...
              http://www.lnm.mw.tum.de/Members/danowski
  </pre>
...
/*
  else if (heatfluxtype == "nd123")
  {
    WriteNodalEigenHeatflux(groupname, result);
  }

  else if (heatfluxtype == "c123")
  {
    WriteElementCenterEigenHeatflux(groupname, result);
  }

  else if (heatfluxtype == "c123_nd123")
  {
    WriteNodalEigenHeatflux(groupname, result);
    ...
    PostResult resulteleheatflux = PostResult(field_);
    resulteleheatflux.next_result();
    WriteElementCenterEigenHeatflux(groupname,resulteleheatflux);
  }
*/
  else
  {
    ...
    PostResult& result)
{
  ...
  std::map<std::string, vector<ofstream::pos_type> > resultfilepos;
  int stepsize = 0;
...
/*----------------------------------------------------------------------*
 |   check this part because of the distinction between      dano 11/09 |
 |   normal_ and shear_stresses                                         |
 *----------------------------------------------------------------------*/
...
//
//  Teuchos::RCP<Epetra_Vector> normal_stresses = LINALG::CreateVector(*(dis->DofRowMap()),true);
//  Teuchos::RCP<Epetra_Vector> shear_stresses = LINALG::CreateVector(*(dis->DofRowMap()),true);
//  dis->Evaluate(p,Teuchos::null,Teuchos::null,normal_stresses,shear_stresses,Teuchos::null);
// 06/11/2009:
  // create heatfluxes (in temperature no distinction between entry in diagonal and secondary diagonal)
  Teuchos::RCP<Epetra_Vector> heatflux_x = LINALG::CreateVector(*(dis->DofRowMap()),true);
  Teuchos::RCP<Epetra_Vector> heatflux_y = LINALG::CreateVector(*(dis->DofRowMap()),true);
  Teuchos::RCP<Epetra_Vector> heatflux_z = LINALG::CreateVector(*(dis->DofRowMap()),true);
  dis->Evaluate(p,Teuchos::null,Teuchos::null,heatflux_x,heatflux_y,heatflux_z);
  ...
  if (numdf==3) // 3 heatflux terms per node in 3D
  {
    for (int i=0;i<numnodes;++i)
    {
      ...
      (*((*nodal_heatfluxes)(0)))[i] = (*heatflux_x)[dis->DofRowMap()->LID(lnodedofs[0])]/adjele;
      (*((*nodal_heatfluxes)(1)))[i] = (*heatflux_y)[dis->DofRowMap()->LID(lnodedofs[0])]/adjele;
      (*((*nodal_heatfluxes)(2)))[i] = (*heatflux_z)[dis->DofRowMap()->LID(lnodedofs[0])]/adjele;
    }
  }
  ...
  else if (numdf==1) // 1 heatflux entries per node  in 1D
  {
    for (int i=0;i<numnodes;++i)
    {
      const DRT::Node* lnode = dis->lRowNode(i);
    ...
    PostResult& result)
{
  ...
  // 3 entries of heatflux in 3D, 2 entries in 2D 06/11/09
  int numdf = -1;
  if (field_->problem()->num_dim()==3) numdf = 3;
  else if (field_->problem()->num_dim()==2) numdf = 2;
  else if (field_->problem()->num_dim()==1) numdf = 1;
    ...
    const int numdf) const
{
  ...
  //--------------------------------------------------------------------
  const Teuchos::RefCountPtr<DRT::Discretization> dis = field_->discretization();
  const Teuchos::RefCountPtr<std::map<int,Teuchos::RefCountPtr<Epetra_SerialDenseMatrix> > > data =
    result.read_result_serialdensematrix(groupname);
  // create the parameters for the discretization
  Teuchos::ParameterList p;
  // other parameters that might be needed by the elements
  ...
  Teuchos::RefCountPtr<Epetra_Map> epetradatamap;
  ...
  Teuchos::RefCountPtr<Epetra_Map> proc0datamap;
  ...
  // contract Epetra_MultiVector on proc0 (proc0 gets everything, other procs empty)
  Teuchos::RefCountPtr<Epetra_MultiVector> data_proc0 = Teuchos::rcp(new Epetra_MultiVector(*proc0datamap,numdf));
...
// all the eigenvalue stuff is not important in the context of heatfluxes in comparison with stresses 10.11.09
