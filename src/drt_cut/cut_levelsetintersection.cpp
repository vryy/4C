...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
GEO::CUT::LevelSetIntersection::LevelSetIntersection( const Epetra_Comm & comm,
    bool create_side )
    : ParentIntersection( comm.MyPID() ),
      side_( Teuchos::null ),
      comm_( & comm )
{
  if ( create_side )
...
{
  if(!side_.is_null())
    dserror("currently only one levelset-side is supported");
...
)
{
  ...
  if ( numnode != xyz.N() )
    dserror( "node coordinate number mismatch" );
...
void GEO::CUT::LevelSetIntersection::Cut( bool include_inner, bool screenoutput,
    INPAR::CUT::VCellGaussPts VCellGP )
{
  //###########################################################################
