                                                      ...
                                                      INPAR::CUT::BCellGaussPts BCellgausstype )
{
  ...
  if (BCellgausstype!= INPAR::CUT::BCellGaussPts_Tessellation)
    dserror("DirectDivergenceGaussRule: just INPAR::CUT::BCellGaussPts_Tessellation supported at the moment!");
  ...
  //as this vc will never be computed in xfem algorithm
  if( Position() == Point::inside and include_inner==false )
    return;
...
{
  if (Position() != Point::undecided)
    dserror("Do not call FindPositionCutSideBased() if Position for volumecell is already set (%d)!", (int)Position());
  ...
  for ( plain_facet_set::const_iterator i=facets.begin(); i!=facets.end(); ++i )
  {
   ...
   if (!f->OnCutSide())
   {
     ...
     // This reference vector points outside the background element! (for LOCAL this is a unit normal vector)
     //-----
     LINALG::Matrix<3,1> ref_vec;
     ref_vec.Update( 1.0, facecen, -1.0, elecen );

     //-----
     // STEP 4: get point from parent side as the normal orientation should be calculated from the side!!!
     ...
     double dotProduct = ref_vec.Dot( norm_fac ); // > 0 ...Side Normal is pointing outside of the Element //< 0 Side Normal points inside the element!
     if (abs(dotProduct) < BASICTOL)
     {
       std::cout << "Reference Vector: (" <<  ref_vec(0,0) << "," <<  ref_vec(1,0) << "," <<  ref_vec(2,0) << ")" << std::endl;
       std::cout << "Facet Vector: (" <<  norm_fac(0,0) << "," <<  norm_fac(1,0) << "," <<  norm_fac(2,0) << ")" << std::endl;
  ...
  if (iter == 1000)
  {
    throw std::runtime_error("SetPositionCutSideBased failed: too many iterations (theoretically a facet with many points could also lead to this)!");
    return false;
  }
      ...
      double prod = 0.0;
        std::vector<double> eqn_plane_facet = KERNEL::EqnPlaneOfPolygon( on->first->CornerPoints() );
         ...
         for (unsigned int dim = 0; dim < 3; ++dim)
           prod += eqn_plane_facet[dim]*eqn_plane_side[dim];

      if ((on->second && prod > 0) || (!on->second && prod < 0)) //this means that the
      {
        if (posi != Point::undecided && posi != Point::inside)
          throw std::runtime_error("SetPositionCutSideBased: posi != Point::undecided && posi != Point::inside (Are all you Cut Sides oriented correct?)");
          //dserror("SetPositionCutSideBased: posi != Point::undecided && posi != Point::inside (Are all you Cut Sides oriented correct?)");
        posi = Point::inside;
      }
      else
      {
        if (posi != Point::undecided && posi != Point::outside)
          throw std::runtime_error("SetPositionCutSideBased: posi != Point::undecided && posi != Point::outside (Are all you Cut Sides oriented correct?)");
          //dserror("SetPositionCutSideBased: posi != Point::undecided && posi != Point::outside (Are all you Cut Sides oriented correct?)");
        posi = Point::outside;
      }
    }
  }
