...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
GEO::CUT::IMPL::SimplePointGraph_1D::SimplePointGraph_1D(
    Mesh & mesh,
    Element * element,
    Side * side,
    PointGraph::Location location,
    PointGraph::Strategy strategy )
    : PointGraph::PointGraph( 1 ) /* explicit call of the protected base
                                     class constructor */
{
  if ( element->Dim() != 1 )
    dserror( "This class is only meaningful for 1-D elements!" );
    ...
    Cycle & cycle) const
{
  ...
  for ( std::vector<Point*>::const_iterator cit=edge_points.begin();
      cit!=edge_points.end(); ++cit )
  {
    Point * p = *cit;
    cycle.push_back( p );
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void GEO::CUT::IMPL::SimplePointGraph_1D::AddCutLinesToGraph(Element * element,
    Side * side, Strategy strategy, Cycle & cycle)
{
  // this is completely unneccesary for the element_side
  if ( not side->IsCutSide() )
    return;
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void GEO::CUT::IMPL::SimplePointGraph_1D::AddCutPointsToCycle( Element * element,
    Side * side, Cycle & cycle )
{
  std::vector<Side * > ele_sides = element->Sides();
  if (ele_sides.size() != 1)
    dserror("There should be only one side in the 1-D case!");
  ...
  for (PointSet::const_iterator cit = cuts.begin(); cit != cuts.end(); ++cit)
    cycle.push_back( *cit );
...
void GEO::CUT::IMPL::SimplePointGraph_1D::Graph::FindCycles(
    Element * element, Side * side, Cycle & cycle, Location location,
    Strategy strategy )
{
  ...
  for (std::vector<Point * >::const_iterator cit = cycle().begin();
      cit != cycle().end(); ++cit)
  {
    Point * p = *cit;
    double t = p->t( edge );
...
{
  if ( element->Dim() != 2 )
      dserror( "This class is only meaningful for 2-D elements!" );
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void GEO::CUT::IMPL::SimplePointGraph_2D::Graph::FindCycles( Element * element,
    Side * side, Cycle & cycle, Location location, Strategy strategy )
{
  if ( location == element_side )
  {
    GEO::CUT::IMPL::PointGraph::Graph::FindCycles( element, side, cycle, location,
        strategy );

    if ( correct_rotation_direction_ )
      CorrectRotationDirection( element->Sides()[ 0 ], main_cycles_ );
  ...
  else
  {
    if ( all_points_.size() > 2 )
      dserror( "A cut_side point cycle with more than 2 points is currently not "
    ...
    for ( std::map<int, Point * >::const_iterator cit = all_points_.begin();
        cit != all_points_.end(); ++cit )
    {
...
void GEO::CUT::IMPL::SimplePointGraph_2D::Graph::SplitMainCyclesIntoLineCycles()
{
  ...
  for ( std::vector<Cycle>::const_iterator ic = main_cycles_.begin();
        ic != main_cycles_.end(); ++ic )
  {
...
void GEO::CUT::IMPL::SimplePointGraph_2D::FindLineFacetCycles(
    const plain_facet_set & line_facets,
    ...
    const plain_facet_set & line_facets, Cycle & cycle )
{
  ...
  for ( plain_facet_set::const_iterator cit = line_facets.begin();
        cit != line_facets.end(); ++cit )
  {
    Facet * f = *cit;

    if ( not f->Equals( DRT::Element::line2 ) )
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void GEO::CUT::IMPL::SimplePointGraph_2D::FindCycles( Element * element,
    Cycle & cycle )
{
  Side * side = element->Sides()[ 0 ];
  GetGraph().FindCycles( element, side, cycle, PointGraph::element_side,
      PointGraph::all_lines );
}
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
    ...
    const Side * side, std::vector<Cycle> & cycles )
{
  ...
  for ( std::vector<Cycle>::iterator it = cycles.begin();
        it != cycles.end(); ++it )
  {
    Cycle & cycle = *it;
    ...
    if ( cycle_size < 3 )
      dserror( "The cycle needs at least three points!" );
    ...
    if ( normal_cycle.Dot( normal_side ) > 0.0 )
      continue;
