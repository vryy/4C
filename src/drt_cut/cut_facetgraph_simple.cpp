    ...
    : FacetGraph()                       /* call empty base class constructor */
{
  ...
  for ( plain_facet_set::const_iterator cit=facets.begin(); cit!=facets.end(); ++cit )
  {
    ...
    if ( f->HasHoles() )
      run_time_error("Holes are not yet considered for the simple case!");
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void GEO::CUT::SimpleFacetGraph_1D::CreateVolumeCells( Mesh & mesh,
    Element * element, plain_volumecell_set & cells )
{
  if ( all_facets_.size() < 1 )
    return;
  ...
  for (std::vector<plain_facet_set>::iterator it = volumes.begin();
      it != volumes.end(); ++it)
  {
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void GEO::CUT::SimpleFacetGraph_1D::SortFacets( const Element * element,
    std::map<double, Facet * > & sorted_facets ) const
{
...
void GEO::CUT::SimpleFacetGraph_1D::CombineFacetsToLineVolumes(
    const std::map<double, Facet * > & sorted_facets,
    std::vector<plain_facet_set> & volumes ) const
{
  std::map<double, Facet * >::const_iterator end_cit = sorted_facets.end();
  --end_cit;
  for ( std::map<double, Facet * >::const_iterator cit = sorted_facets.begin();
        cit != end_cit; )
  {
    ...
    Mesh & mesh, Element * element, plain_volumecell_set & cells )
{
  ...
  if ( sides.size() != 1 )
    dserror( "A 2-D element is supposed to contain exactly one side!" );

  Side * side = sides[ 0 ];

  IMPL::SimplePointGraph_2D pg_2d( mesh, element, side,
      IMPL::PointGraph::element_side, IMPL::PointGraph::all_lines );

  volumes.reserve( pg_2d.NumSurfaces() ) ;

  for ( IMPL::SimplePointGraph_2D::surface_const_iterator sit =
      pg_2d.sbegin(); sit != pg_2d.send(); ++sit )
  {
