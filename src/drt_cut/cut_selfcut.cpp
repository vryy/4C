...
void GEO::CUT::SelfCut::GetSelfCutObjects()
{
      ...
      selfcut_sides_.begin(); i != selfcut_sides_.end(); ++i)
  {
    ...
    for (std::vector<Node*>::const_iterator i = cutsidenodes.begin();
...
void GEO::CUT::SelfCut::CreateSelfCutNodes()
{
      ...
      selfcut_sides_.begin(); i != selfcut_sides_.end(); ++i)
  {
    ...
    for (PointSet::const_iterator i = cutsideselfcutpoints.begin();
...
void GEO::CUT::SelfCut::CreateSelfCutEdges()
{
      ...
      selfcut_sides_.begin(); i != selfcut_sides_.end(); ++i)
  {
    ...
    for (plain_side_set::const_iterator i = cuttingsides.begin();
        i != cuttingsides.end(); ++i)
    {
      ...
      for (plain_node_set::const_iterator i = cutside_nodes.begin();
...
void GEO::CUT::SelfCut::CreateSelfCutSides()
{
      ...
      selfcut_sides_.begin(); i != selfcut_sides_.end(); ++i)
  {
    ...
    const std::vector<std::vector<Point*> > & selfcuttriangles =
        cutside->SelfCutTriangles();

    for (std::vector<std::vector<Point*> >::const_iterator i =
...
//        Output(*cutside, "CreateSelfCutSides");
      ...
      Side * selfcutside = mesh_.CreateSide(cutside->Id(), selfcutsidenodeids,
          DRT::Element::tri3);
      const std::map<plain_int_set, Teuchos::RCP<Side> > & cutsideedgercp =
          mesh_.Sides();
...
void GEO::CUT::SelfCut::PropagateSelfCutPosition()
{
      ...
      selfcut_sides_.begin(); i != selfcut_sides_.end(); ++i)
  {
    ...
    if (cutside->SelfCutPosition() == Point::undecided)
    {
      ...
      for (std::vector<Edge*>::const_iterator i = cutsideedges.begin();
        ...
        ui != undecidedsides.end();)
    {
      ...
      if (undecidedside->SelfCutPosition() == Point::undecided)
      {
            ...
            i != undecidedcutsideedges.end(); ++i)
        {
          ...
          const plain_side_set & undecidedcutsideedgesides =
...
void GEO::CUT::SelfCut::EraseInsideEdges()
{
  ...
  std::vector<plain_int_set> cutsideedgeids;
  for (std::map<plain_int_set, Teuchos::RCP<Edge> >::const_iterator i =
...
void GEO::CUT::SelfCut::NextNode(Node * node, plain_node_set & remainingnodes, int count)
{
  ...
  for (plain_edge_set::const_iterator i = nodeedges.begin();
      i != nodeedges.end(); ++i)
  {
    Edge * nodeedge = *i;
    const std::vector<Node*> & nodeedgenodes =
        nodeedge->Nodes();
    for (std::vector<Node*>::const_iterator i = nodeedgenodes.begin();
        i != nodeedgenodes.end(); ++i)
    {
      Node * nodeedgenode = *i;
      plain_node_set::iterator remainingnodesiter = remainingnodes.find(
          nodeedgenode);
...
void GEO::CUT::SelfCut::FindIslands()
{
  ...
  if (selfcut_connectivity_.size() > 1)
  {
    ...
    for (std::map<int, plain_node_set >::iterator i = selfcut_connectivity_.begin();
        i != selfcut_connectivity_.end(); ++i)
    {
      int count = i->first;
      plain_node_set selfcutconnectivity = i->second;
      for (plain_node_set::iterator i = selfcutconnectivity.begin();
        ...
        i != selfcutconnectivitybbs.end(); ++i)
    {
      ...
      {
        volmax = selfcutconnectivitybbvols[count];
      }
    }

    for (std::map<int, double >::iterator i = selfcutconnectivitybbvols.begin();
        i != selfcutconnectivitybbvols.end(); ++i)
    {
      int count = i->first;
      double vol = i->second;
      if (vol < 0.1*volmax)
      {
        plain_node_set selfcutconnectivity = selfcut_connectivity_[count];
        Node * node = *selfcutconnectivity.begin();
        node->ChangeSelfCutPosition(Point::inside);
        // numislands++;
      }
    }
    // std::cout << "******************************************************************************************************************" << std::endl;
    // std::cout << "SelfCut -- MeshCorrection: " << selfcut_connectivity_.size() << " separated parts of the cut mesh" << std::endl;
    // std::cout << "if the vol of the bb of one part is 10 times smaller than the vol of the biggest bb, it is identified as a islands" << std::endl;
    // std::cout << numislands << " parts of the cut mesh identified as islands and erased from the cut mesh" << std::endl;
    // std::cout << "******************************************************************************************************************" << std::endl;
  }
...
void GEO::CUT::SelfCut::CuttedSideStatusText()
{
      ...
      selfcut_sides_.begin(); i != selfcut_sides_.end(); ++i)
  {
    ...
    std::cout << "\nSelfcutpoints:\n";
    PointPlotHead();
    for (PointSet::const_iterator i = selfcutpoints.begin(); i != selfcutpoints.end();
        ++i)
    {
    ...
    std::cout << "\nSelfcutnodes:\n";
    NodePlotHead();
    for (plain_node_set::const_iterator i = selfcutnodes.begin();
    ...
    std::cout << "\nSelfcutedges:\n";
    EdgePlotHead();
    for (plain_edge_set::const_iterator i = selfcutedges.begin();
    ...
    const std::vector<std::vector<GEO::CUT::Point*> > & selfcuttriangles =
        cutside->SelfCutTriangles();
    std::cout << "\nSelfcuttriangles:\n";
    for (std::vector<std::vector<GEO::CUT::Point*> >::const_iterator i =
        selfcuttriangles.begin(); i != selfcuttriangles.end(); ++i)
    {
...
void GEO::CUT::SelfCut::WallGmsh(const std::string & name)
{
      ...
      cutsides.begin(); i != cutsides.end(); ++i)
  {
    ...
    for (std::vector<Edge*>::const_iterator i = cutedges.begin(); i != cutedges.end();
        ++i)
    {
      Edge * cutsideedge = *i;
      ...
      cutsides.begin(); i != cutsides.end(); ++i)
  {
    ...
    for (std::vector<Node*>::const_iterator i = cutnodes.begin(); i != cutnodes.end();
        ++i)
    {
      Node * cutsidenode = *i;
      ...
      cutsides.begin(); i != cutsides.end(); ++i)
  {
    ...
    for (plain_side_set::const_iterator i = selfcutsides.begin();
        i != selfcutsides.end(); ++i)
    {
      Side * cuttingside = *i;
      ...
      cutsides.begin(); i != cutsides.end(); ++i)
  {
    ...
    for (plain_node_set::const_iterator i = selfnodes.begin(); i != selfnodes.end();
        ++i)
    {
      Node * selfcutnode = *i;
      ...
      cutsides.begin(); i != cutsides.end(); ++i)
  {
    ...
    for (plain_edge_set::const_iterator i = selfedges.begin(); i != selfedges.end();
        ++i)
    {
      Edge * selfcutedge = *i;
  ...
  {
    Side * cutside = &*i->second;
    const std::vector<std::vector<Point*> > & selftriangles =
        cutside->SelfCutTriangles();
...
void GEO::CUT::SelfCut::SCObjectsGmsh(const std::string & name)
{
      ...
      i != selfcut_edges_.end(); ++i)
  {
    ...
    for (int i = 0; i < 2; ++i)
    {
      ...
      file << selfcutedgecoordinates(0, i) << ","
          << selfcutedgecoordinates(1, i) << ","
          << selfcutedgecoordinates(2, i);
    }
    file << "){";
    Point::PointPosition selfcutedgeposition =
        selfcutedge->SelfCutPosition();
    for (int i = 0; i < 2; ++i)
    {
      if (i > 0)
      ...
      i != selfcut_nodes_.end(); ++i)
  {
    ...
    file << "){";
    Point::PointPosition selfcutnodeposition =
        selfcutnode->SelfCutPosition();
...
void GEO::CUT::SelfCut::EraseSidePointer(Side & cutside)
{
  ...
  for (PointSet::const_iterator i = cutsidepoints.begin(); i != cutsidepoints.end();
      ++i)
  {
...
void GEO::CUT::SelfCut::EraseEdgePointer(Edge & cutsideedge)
{
  ...
  for (PointPositionSet::const_iterator i = cutsideedgepoints.begin();
...
void GEO::CUT::SelfCut::SideNodePlot(Side & side)
{
  ...
  const std::vector<Node*> & sidenodes = side.Nodes();
  NodePlotHead();
  for (std::vector<Node*>::const_iterator i = sidenodes.begin(); i != sidenodes.end();
      ++i)
  {
    Node * sidenode = *i;
    NodePlot(*sidenode);
