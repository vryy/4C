...
std::vector<double> GEO::CUT::DirectDivergenceGlobalRefplane::GetReferencePlane()
{
  ...
  if (not comp_ref_plane) dserror("Proper reference plane not found");
...
bool GEO::CUT::DirectDivergenceGlobalRefplane::DiagonalBasedRef(std::vector<double>& RefPlaneEqn)
{
  ...
  // that again!
  if (!found_refplane)
    dserror("Couldn't find a diagonal reference plane with all gausspoints inside!!!");
    ...
    std::vector<double>& RefPlaneEqn)
{
  ...
  for (std::vector<Point*>::iterator it = corners.begin(); it != corners.end(); it++)
  {
    ...
    if (std::abs(rst_proj(0, 0)) > 1.0 + 1e-8 or std::abs(rst_proj(1, 0)) > 1.0 + 1e-8 or
        std::abs(rst_proj(2, 0)) > 1.0 + 1e-8 or std::isnan(rst_proj(0, 0)) or
