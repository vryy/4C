...
// Quadrature and the base quadrature rule has a very good point distribution (obtained from direct divergence), the problem
// due to matrix conditioning does not appear
//#define CHECBYSHEV
 ...
 *---------------------------------------------------------------------------------------------------------------*/
bool GEO::CUT::QuadratureCompression::PerformCompressionOfQuadrature( DRT::UTILS::GaussPointsComposite& gin,
    GEO::CUT::VolumeCell * vc )
{
  const double t_start = Teuchos::Time::wallTime();

  Teuchos::RCP<Teuchos::SerialDenseMatrix<int,double> > vander = Teuchos::rcp(new Teuchos::SerialDenseMatrix<int,double> );
  Teuchos::RCP<Teuchos::SerialDenseVector<int,double> > x = Teuchos::rcp(new Teuchos::SerialDenseVector<int,double> );
  Teuchos::RCP<Teuchos::SerialDenseVector<int,double> > rhs = Teuchos::rcp(new Teuchos::SerialDenseVector<int,double> );
 ...
 * 1. Find discrete Leja points by LU factorizing the Vandermonde matrix --> this will give us the number of quadrature
 *    points equal to the number of base functions
 * 2. Solve the matrix system constructing Vandermonde matrix at these Leja points and obtain quadrature weights
 *
 * Read the following paper for the details of the method:
 * L. Bos, S. De Marchi, A. Sommariva, and M. Vianello. Computing multivariate Fekete and Leja points by numerical
 * linear algebra. SIAM J Numer Anal. 48:1984--1999, 2010.
 *-----------------------------------------------------------------------------------------------------------------*/
    ...
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& sol )
{
  ...
  Teuchos::RCP<Teuchos::SerialDenseMatrix<int,double> > matTemp = Teuchos::rcp(new Teuchos::SerialDenseMatrix<int,double> );
  matTemp->shape( ma, na );
  ...
  Teuchos::RCP<Teuchos::SerialDenseVector<int,int> > work_temp = Teuchos::rcp(new Teuchos::SerialDenseVector<int,int> );
  ...
  Teuchos::RCP<Teuchos::SerialDenseMatrix<int,double> > sqrmat = Teuchos::rcp(new Teuchos::SerialDenseMatrix<int,double> );
  sqrmat->shape( na,na );
  ...
  if( isSolved != 0 )
    dserror("SerialDenseSolver failed\n");
...
/*-------------------------------------------------------------------------------------------------------------------------*
 * Form new quadrature rule from the quadrature points and weights obtained from compression                       sudhakar 08/15
 * (just creating the required data structure)
 *-------------------------------------------------------------------------------------------------------------------------*/
Teuchos::RCP<DRT::UTILS::GaussPoints> GEO::CUT::QuadratureCompression::FormNewQuadratureRule( DRT::UTILS::GaussPointsComposite& gin,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& sol,
    std::vector<int>& work,
    int & na )
{
...
void GEO::CUT::QuadratureCompression::GetPivotalRows( Teuchos::RCP<Teuchos::SerialDenseVector<int,int> >& work_temp,
    std::vector<int>& work )
{
...
bool GEO::CUT::QuadratureCompression::isThisValueAlreadyInDenseVector( int & input,
    std::vector<int>& vec,
    int upper_range,
    int & index )
{
...
int GEO::CUT::QuadratureCompression::GetCorrectIndex( int & input,
    std::vector<int>& vec,
    int upper_range )
{
  int index = 0;
...
/*--------------------------------------------------------------------------------------------------------------------*
 * Compute error between original and compressed quadrature points to evalute defined      sudhakar 08/15
 * base functions (either Chebyshev or monomial) and print the maximum absolute error
 *--------------------------------------------------------------------------------------------------------------------*/
...
void GEO::CUT::QuadratureCompression::Teuchos_GELS( Teuchos::RCP<Teuchos::SerialDenseMatrix<int,double> >& mat,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& rhs,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& sol )
{
  mat->shape( 3, 4 );
  ...
  (*mat)(0,0) = 5.0; (*mat)(0,1) = 7.0; (*mat)(0,2) = 4.0; (*mat)(0,3) = 1.0;
  (*mat)(1,0) = 7.0; (*mat)(1,1) = 9.0; (*mat)(1,2) = 8.0; (*mat)(2,3) = 3.0;
  (*mat)(2,0) = 2.0; (*mat)(2,1) = 4.0; (*mat)(2,2) = 10.0; (*mat)(2,3) = 6.0;

  (*rhs)(0) = 17.0; (*rhs)(1) = 27.0; (*rhs)(2) = 22.0;
  (*sol)(0) = 0.0; (*sol)(1) = 0.0; (*sol)(2) = 0.0; (*sol)(3) = 0.0;
  ...
  Teuchos::RCP<Teuchos::SerialDenseVector<int,double> > work = Teuchos::rcp(new Teuchos::SerialDenseVector<int,double> );
  work->shape( na,1 );

  Teuchos::LAPACK<int,double> ll;

  int info = 0;
  ll.GELS( 'N', ma, na, nb, mat->values(), mat->stride(), rhs->values(), 1, work->values(), lwork, &info );
...
void GEO::CUT::QuadratureCompression::QR_decomposition_Teuchos( Teuchos::RCP<Teuchos::SerialDenseMatrix<int,double> >& mat,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& rhs,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& sol )
{
  ...
  if( NotSolved != 0 )
    dserror( "QR-factorization using Trilinos not successful\n" );

}

void GEO::CUT::QuadratureCompression::QR_decomposition_LAPACK( Teuchos::RCP<Teuchos::SerialDenseMatrix<int,double> >& mat,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& rhs,
    Teuchos::RCP<Teuchos::SerialDenseVector<int,double> >& sol )
{
  ...
  Teuchos::RCP<Teuchos::SerialDenseVector<int,int> > jpvt = Teuchos::rcp(new Teuchos::SerialDenseVector<int,int> );
  Teuchos::RCP<Teuchos::SerialDenseVector<int,double> > tau = Teuchos::rcp(new Teuchos::SerialDenseVector<int,double> );
  Teuchos::RCP<Teuchos::SerialDenseVector<int,double> > work = Teuchos::rcp(new Teuchos::SerialDenseVector<int,double> );
  Teuchos::RCP<Teuchos::SerialDenseVector<int,double> > rwork = Teuchos::rcp(new Teuchos::SerialDenseVector<int,double> );
  jpvt->shape( na,1 );
  tau->shape( na,1 );
  work->shape( na,1 );
  rwork->shape( na,1 );
  ...
  Teuchos::LAPACK<int,double> ll;
  ll.GEQP3( ma, na, mat->values(), mat->stride(), jpvt->values(), tau->values(), work->values(), lwork, rwork->values(), &info );
  if( info )
    dserror( "QR-decomposition with column pivoting failed\n" );

  ll.ORMQR( 'L', 'T', ma, nb, na, mat->values(), mat->stride(), tau->values(),
      rhs->values(), rhs->stride(), work->values(), lwork, &info  );

  int k = 0;
  for( k=0; k < na; k++  )
  {
    if( (*mat)(k,k) < 0.01*(*mat)(0,0) )
      break;
  }
 ...
 * into GMSH output
 *------------------------------------------------------------------------------------------------------------------------------------*/
void GEO::CUT::QuadratureCompression::WriteCompressedQuadratureGMSH( DRT::UTILS::GaussPointsComposite& gin, GEO::CUT::VolumeCell * vc )
{
  static int sideno = 0;
  sideno++;
  ...
  for ( int npt = 0; npt < numpts; npt++ )
  {
    ...
    file<<"SP("<<loc[0]<<","<<loc[1]<<","<<loc[2]<<","<<"1"<<"){0.0};"<<std::endl;
...
/*---------------------------------------------------------------------------------------------------------------------*
 * Integrate predefined polynomial functions using original and compressed quadrature and compute error         sudhakar 08/15
 * between these values
 *---------------------------------------------------------------------------------------------------------------------*/
void GEO::CUT::QuadratureCompression::IntegratePredefinedPolynomials( DRT::UTILS::GaussPointsComposite& gin )
{
  ...
  for ( int npt = 0; npt < numpts; npt++ )
  {
    ...
    intOri[5] += (pow(x,5)+5.0*x*y*pow(z,3)-10.0*x*pow(y,3)*z+5.0*pow(x,3)*y*z+pow(y,5)+pow(z,5))*wei;
  }
  ...
  for ( int npt = 0; npt < num; npt++ )
  {
    ...
    // integrate x^5+5*xyz^3-10*xy^3z+5x^3yz+y^5+z^5
    intCom[5] += (pow(x,5)+5.0*x*y*pow(z,3)-10.0*x*pow(y,3)*z+5.0*pow(x,3)*y*z+pow(y,5)+pow(z,5))*wei;
  }

  // Compute error
  for( int ii=0; ii<6; ii++ )
    err[ii] = fabs(intOri[ii]-intCom[ii]);
