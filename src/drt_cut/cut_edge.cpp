 ...
 *-----------------------------------------------------------------------------*/
bool GEO::CUT::Edge::FindCutPoints(
    Mesh& mesh, Element* element, Side& side, Side& other, int recursion)
{
  bool cut = false;
...
void GEO::CUT::Edge::CutPointsIncluding(Point* begin, Point* end, std::vector<Point*>& line)
{
  ...
  if (*bi != begin) throw std::runtime_error("begin point not on edge");

  if (*ei != end) throw std::runtime_error("end point not on edge");
...
bool GEO::CUT::ConcreteEdge<probDim, edgeType, dimEdge, numNodesEdge>::ComputeCut(
    Mesh* mesh, Edge* other, PointSet* cut_points, double& tolerance)
{
  ...
  const enum IntersectionStatus istatus = inter_ptr->ComputeEdgeSideIntersection(tolerance);
