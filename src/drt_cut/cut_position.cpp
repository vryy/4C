    ...
    const LINALG::Matrix<rdim, 1> & xyz )
{
  ...
  if ( rdim < factory.ProbDim() )
    dserror("The given point has the wrong row dimension!\n"
        "rdim < prodbim <--> %d < %d", rdim, factory.ProbDim());
    ...
    const DRT::Element::DiscretizationType & distype )
{
  ...
  if ( rdim < probdim or cdim != num_nodes_ele )
    dserror("Dimension mismatch of xyze! \n"
        "expected input: %d x %d (rows x cols)\n"
        "received input : %d x %d (rows x cols)", probdim,num_nodes_ele,
        xyze.M(), xyze.N());
  ...
  if ( rdim_2 < probdim )
    dserror("Dimension mismatch of xyz! \n"
        "expected input: %d x 1 (rows x cols)\n"
        "received input : %d x 1 (rows x cols)", probdim,rdim_2);
    ...
    const DRT::Element::DiscretizationType & distype )
{
  ...
  if ( static_cast<unsigned>( xyze.M() ) < probdim or
       static_cast<unsigned>( xyze.N() ) != num_nodes_ele )
    dserror("Dimension mismatch of xyze! \n"
        "expected input: %d x %d (rows x cols)\n"
        "received input : %d x %d (rows x cols)", probdim,num_nodes_ele,
        xyze.M(), xyze.N());
  ...
  if ( xyz.M() < probdim )
    dserror("Dimension mismatch of xyz! \n"
        "expected input: %d x 1 (rows x cols)\n"
        "received input : %d x 1 (rows x cols)", probdim, xyz.M());
...
template <unsigned probdim,
      ...
      unsigned dim>
bool GEO::CUT::ComputeEmbeddedPosition<probdim,eletype,numNodesElement,dim>::
     Compute( const double & Tol, const bool & allow_dist )
{
  xsi_aug_ = 0.0;
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
GEO::CUT::PositionFactory::PositionFactory()
 ...
 *----------------------------------------------------------------------------*/
Teuchos::RCP<GEO::CUT::Position> GEO::CUT::PositionFactory::CreatePosition(
    const std::vector<GEO::CUT::Node*> nodes,
    const double * xyz,
...
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<2>( const Element & element, const LINALG::Matrix<2, 1> & xyz );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3>( const Element & element, const LINALG::Matrix<3, 1> & xyz );

template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3,3,3>( const LINALG::Matrix< 3, 3 > & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3,6,3>( const LINALG::Matrix< 3, 6 > & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3,4,3>( const LINALG::Matrix< 3, 4 > & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3,8,3>( const LINALG::Matrix< 3, 8 > & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3,9,3>( const LINALG::Matrix< 3, 9 > & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3,2,3>( const LINALG::Matrix< 3, 2 > & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<2,2,2>( const LINALG::Matrix< 2, 2 > & xyze, const LINALG::Matrix< 2, 1 > & xyz, const DRT::Element::DiscretizationType & distype );

template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<3>( const Epetra_SerialDenseMatrix & xyze, const LINALG::Matrix< 3, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
template Teuchos::RCP<GEO::CUT::Position> GEO::CUT::Position::Create<2>( const Epetra_SerialDenseMatrix & xyze, const LINALG::Matrix< 2, 1 > & xyz, const DRT::Element::DiscretizationType & distype );
