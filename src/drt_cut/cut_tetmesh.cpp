    ...
    facets_( facets )
{
  ...
  for ( std::vector<std::vector<int> >::iterator i=original_tets.begin(); i!=original_tets.end(); ++i )
  {
    ...
    {
      std::cout << "IsValidTet, threw away tet with nodes: " << t[0] << ", " << t[1] << ", "<< t[2] << ", "<< t[3] << std::endl;
...
/* Loop over the facets of the tetmesh. Can every facet (triangulated at this stage) be associated with an unique
   TET from the delauney-cell decomposition?

   @Magnus:
   WARNING: For a LevelSet-cut with many cut-points this might lead to a failure to fill the FacetMesh as no info about
            the triangulated point is provided. This is however taken care of in the TetMeshIntersection, (the TETs are not cut)
            It could however be dealt with if the triangulated point is added to the cut information. Might have to look
            into this....
...
/* From the input create TETs.
   1) First remove the TETs which are too small for arithmetic precision from the "accepted_tet_set" of TETs.
      However they might be needed later (for boundary-cell creation) so don't throw them away.
   2) Try to "FillFacetMesh(...)", this means that for the given given element (either a tet or a hex8)
      find the facets on the boundaries and find if they are associated with an unique TET.

    2.1) If NOT assume the tesselation (i.e. on TET or more) is cut, and call the Tesselation recursively.
    2.2) If YES fill the inner part of the element with accepted tets and finish the tesselation for this element.
      ...
      if ( cell_domain.Empty() )
      {
        ...
        if ( cell_domain.Empty() )
        {
                ...
                ++i )
          {
            ...
            std::cout << "f->ShareSameCutSide(*facets.begin()): " << f->ShareSameCutSide(*facets.begin())<< std::endl;

#ifdef DEBUGCUTLIBRARY
            std::cout << "num_facet_counter: " << num_facet_counter << std::endl;
            std::cout << "f->OnCutSide(): " << f->OnCutSide() << std::endl;
            std::cout << "f->ParentSide()->Id(): " << f->ParentSide()->Id() << std::endl;
            std::cout << "f->IsPlanar(mesh,f->CornerPoints()): " << f->IsPlanar(mesh,f->CornerPoints()) << std::endl;
            f->PrintPointIds();
            num_facet_counter++;
#endif
            ...
            {
              std::cout << "Warning: This volume cell has no integration cells AND not all on same cut-side." << std::endl;
          ...
          if(not volume_cell_on_facet and not does_not_share_same_cutside)
            throw std::runtime_error("cell_domain.Empty() and facets not on same cut-side and not planar");
...
//      std::cout << ", cell_domain.Border().size(): " << cell_domain.Border().size() << std::endl;
//      std::cout << "Tet-IDs: ";
//      for(unsigned j=0; j<cell_domain.Members().size(); j++)
//      {
...
//#endif
  ...
  else
  {
    //if ( count <= 3 )
    {
      ...
      intersection.Cut( mesh, element, cells, count ,tetcellsonly); //writes tetmesh output in debug mode!!!
    }
  }
}

/* Initialize the data-structures needed for the TetMesh. The call to Qhull has to be performed before.
*/
...
/* Call up QHull and let it create the delauney-cells. There are different options for how QHull can be called.
   3 options are tested in order, if the previous option failed.
   Fill the std::vector tets with tetrahedrons provided from the delauney triangulization of the "convex"-hull.
...
//   }
  ...
  if ( project ) //Never used... Not working properly either, I think...
  {
    ...
    double length = 0;
    LINALG::Matrix<3,1> l;
    for ( int i=0; i<n; ++i ) // Find the distance to the point furthest away from the mid-point (length)
    {
      Point * p = points[i];
      LINALG::Matrix<3,1> x( p->X() );
      l = m;
...
/* First check if all the points of a tet share a cut-side, i.e. does the tet lie on a cut-side?
  ...
  + This was the way envisioned by Kuettler. However, it makes little sense accepting a TET completely
     on a cut-side. A simplification by me (Magnus) is to remove TETS whos points are all on a cut-side or
     more than one cut-side (A case not uncommon for MeshIntersection, however this works? Would it imply a TET is on a line?).
  + The change proposed does not change any existing test-case or any cut-test. However, the algorithm implemented,
     by Kuettler, is probably not there unnecessarily (hopefully?). Thus keep the code for reference in the future.
 !+ For now use the old way of Kuettler as after further investigation it seems to be somewhat more exact...
...
bool GEO::CUT::TetMesh::IsValidTet( const std::vector<Point*> & t )
{
  ...
  if ( sides.size()==0 )
  {
    ...
    //  as the points know what sides it cuts.
#ifdef DEBUGCUTLIBRARY
...
#endif
    dserror("You have encountered a case where the a tet is not on a cut-side BUT is on a facet!!! CHECK IT!");
  ...
  if ( sides.size()==1 )
  {
    ...
    if ( dynamic_cast<LevelSetSide*>( *sides.begin() ) != NULL )
    {
      ...
      {
#ifdef TETMESH_EXTENDED_DEBUG_OUTPUT
      std::cout << "side.size()==1 and LEVELSetSide with facets.size(): " << facets.size() << "!"<< std::endl;
      std::cout << "This is most likely a degenerate cut case for the level-set. Something should have been done before this stage...." << std::endl;
#endif
...
//          }
//        }
        //throw std::runtime_error("A LevelSetSide should BE associated to ONE facet. CHECK THIS!");
      }
  ...
  {
#ifdef TETMESH_EXTENDED_DEBUG_OUTPUT
    std::cout << "One TET lies on MORE THAN one cut-side. CUT-SIDES OVERLAPPING! Discard this tet." << std::endl;
#endif
    return false;
  }
#else
...
#ifdef TETMESH_EXTENDED_DEBUG_OUTPUT
  std::cout << "sides.size()==" << sides.size() <<", and facets.size(): " << facets.size()<< std::endl;
...
{
  if(t.size()!=4)
    dserror("Expected a tet. Size of vector is not 4.");
