...
bool GEO::CUT::LevelSetSide<probdim>::Cut( Mesh & mesh, Edge & edge,
    PointSet & cut_points )
{
 ...
 *----------------------------------------------------------------------------*/
template < int probdim >
void GEO::CUT::LevelSetSide<probdim>::MakeInternalFacets( Mesh & mesh,
    Element * element, plain_facet_set & facets )
{
 ...
 *----------------------------------------------------------------------------*/
template < int probdim >
bool GEO::CUT::LevelSetSide<probdim>::FindAmbiguousCutLines( Mesh & mesh,
    Element * element, Side & side, const PointSet & cut )
{
  ...
  switch ( side.Shape() )
  {
    ...
    default:
      dserror( "Unsupported side shape! (shape = %d | %s )",
          side.Shape(),DRT::DistypeToString(side.Shape()).c_str());
      break;
  }
  return false;
}
