...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
MAT::ELASTIC::CoupTransverselyIsotropic::CoupTransverselyIsotropic(
    my_params* params )
    : params_( params )
{
  /* empty */
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void MAT::ELASTIC::CoupTransverselyIsotropic::AddStrainEnergy(
    double& psi,
  ...
  // (3) contribution
  {
    ...
    for ( unsigned a=0; a<6; ++a )
    {
      for ( unsigned b=0; b<6; ++b )
      {
        ...
        cmat(a,b) += delta * ( rcg_inv_s( MAT::IMap::second_[i][k] ) *
            rcg_inv_s( MAT::IMap::second_[j][l] ) +
            rcg_inv_s( MAT::IMap::second_[i][l] ) *
            rcg_inv_s( MAT::IMap::second_[j][k] ) );
      }
    }
  }
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void MAT::ELASTIC::CoupTransverselyIsotropic::ResetInvariants(
    const LINALG::Matrix<6,1>& rcg )
{
  ...
  if ( I3 < 0.0 )
    dserror( "I3 is negative!" );

  // jacobian determinant
  j_ = std::sqrt(I3);

  // calculate pseudo invariant I4 ( strain measure in fiber direction )
  I4_ = AA_(0)*rcg(0) + AA_(1)*rcg(1) + AA_(2)*rcg(2)
      + AA_(3)*rcg(3) + AA_(4)*rcg(4) + AA_(5)*rcg(5);
  ...
  I5_ = AA_(0)*( rcg_quad(0) ) + AA_(1)*( rcg_quad(1) )
      + AA_(2)*( rcg_quad(2) ) + AA_(3)*( rcg_quad(3) )
      + AA_(4)*( rcg_quad(4) ) + AA_(5)*( rcg_quad(5) );
}
