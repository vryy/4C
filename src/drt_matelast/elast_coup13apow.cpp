 ...
 *----------------------------------------------------------------------*/
MAT::ELASTIC::PAR::Coup13aPow::Coup13aPow(
 ...
 *----------------------------------------------------------------------*/
MAT::ELASTIC::Coup13aPow::Coup13aPow(MAT::ELASTIC::PAR::Coup13aPow* params)
  : params_(params)
{
}
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void MAT::ELASTIC::Coup13aPow::AddStrainEnergy(
    double& psi,
 ...
 *                                                       birzle 11/2016 */
/*----------------------------------------------------------------------*/
void MAT::ELASTIC::Coup13aPow::AddDerivativesPrincipal(
    ...
    const int eleGID )
{
  ...
  if (d<2)
    dserror("The Elast_Coup13aPow - material only works for positive integer exponents, which are larger than two.");
  // a==0 not necessary to implement extra as equal to coup1pow
  if (a==0)
    dserror("Use Elast_Coup1Pow.");
  ...
  if (d==2)
  {
    ...
    ddPII(2) += a*(a+1.)*c*d*prinv(0)*pow(prinv(2),-a-2.)*pow(I1I3a3,d-1.) + a*a*c*d*(d-1.)*prinv(0)*prinv(0)*pow(prinv(2),(-2.*a-2.));
    ddPII(4) += -a*c*d*pow(prinv(2),-a-1.)*pow(I1I3a3,d-1.) - a*c*d*(d-1.)*prinv(0)*pow(prinv(2),(-2.*a-1.));
  }
  else
  {
    ddPII(0) += c*d*(d-1.)*pow(prinv(2),-2.*a)*pow(I1I3a3,d-2.);
    ddPII(2) += a*(a+1.)*c*d*prinv(0)*pow(prinv(2),-a-2.)*pow(I1I3a3,d-1.) + a*a*c*d*(d-1.)*prinv(0)*prinv(0)*pow(prinv(2),(-2.*a-2.))*pow(I1I3a3,d-2.);
    ddPII(4) += -a*c*d*pow(prinv(2),-a-1.)*pow(I1I3a3,d-1.) - a*c*d*(d-1.)*prinv(0)*pow(prinv(2),(-2.*a-1.))*pow(I1I3a3,d-2.);
  }
