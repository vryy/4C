...
/*----------------------------------------------------------------------*/
template< typename T >
void MAT::ELASTIC::CoupAnisoExpoActive::EvaluateFunc(T& psi,
...
/*----------------------------------------------------------------------*/
template < typename T >
void MAT::ELASTIC::CoupAnisoExpoActive::EvaluateActiveStressCmatAniso(LINALG::TMatrix<T,3,3> const& CM,
                                                                      LINALG::TMatrix<T,6,6> & cmat,
                                                                      LINALG::TMatrix<T,6,1> & stress,
                                                                      const int eleGID) const
{
  T lambda_sq = 0.0;
  static LINALG::TMatrix<T,6,1> Av_T(true);
  for(int i=0;i<6;++i) Av_T(i) = A_(i);
  lambda_sq =  Av_T(0)*CM(0,0) + Av_T(1)*CM(1,1) + Av_T(2)*CM(2,2)
      + Av_T(3)*CM(0,1) + Av_T(4)*CM(1,2) + Av_T(5)*CM(0,2)
      + Av_T(3)*CM(1,0) + Av_T(4)*CM(2,1) + Av_T(5)*CM(2,0);
...
template void MAT::ELASTIC::CoupAnisoExpoActive::GetDerivativesAniso<double>(LINALG::TMatrix<double,2,1>&,
                                                                             LINALG::TMatrix<double,3,1>&,
                                                                             LINALG::TMatrix<double,4,1>&,
                                                                             LINALG::TMatrix<double,3,3> const&,
                                                                             const int) const;
template void MAT::ELASTIC::CoupAnisoExpoActive::GetDerivativesAniso<FAD>(LINALG::TMatrix<FAD,2,1>&,
                                                                          LINALG::TMatrix<FAD,3,1>&,
                                                                          LINALG::TMatrix<FAD,4,1>&,
                                                                          LINALG::TMatrix<FAD,3,3> const&,
                                                                          const int) const;
template void MAT::ELASTIC::CoupAnisoExpoActive::EvaluateActiveStressCmatAniso<double>(LINALG::TMatrix<double,3,3> const&,
                                                                                       LINALG::TMatrix<double,6,6> &,
                                                                                       LINALG::TMatrix<double,6,1> &,
                                                                                       const int) const;
template void MAT::ELASTIC::CoupAnisoExpoActive::EvaluateActiveStressCmatAniso<FAD>(LINALG::TMatrix<FAD,3,3> const&,
                                                                                    LINALG::TMatrix<FAD,6,6> &,
                                                                                    LINALG::TMatrix<FAD,6,1> &,
                                                                                    const int) const;
template void MAT::ELASTIC::CoupAnisoExpoActive::EvaluateFunc<double>(double&,
                                                                      LINALG::TMatrix<double,3,3> const&,
                                                                      const int) const;
template void MAT::ELASTIC::CoupAnisoExpoActive::EvaluateFunc<FAD>(FAD&,
                                                                   LINALG::TMatrix<FAD,3,3> const&,
                                                                   const int) const;
