...
void DRT::ELEMENTS::NStet5Type::NodalIntegration(
                                                ...
                                                std::vector<DRT::ELEMENTS::NStet5*>& adjele,
                                                std::map<int,std::vector<int> >&          adjsubele,
                                                std::vector<int>&               lm,
                                                std::vector<std::vector<std::vector<int> > >&  lmlm,
...
//  Teuchos::SerialDenseMatrix<int,FAD> tbopbar(6,ndofinpatch,true);
  ...
  {
     bopbar(0,j) = Fnode(0,0)*nxyzbar(0,j)+Fnode(1,0)*nxyzbar(1,j)+Fnode(2,0)*nxyzbar(2,j);
     bopbar(1,j) = Fnode(0,1)*nxyzbar(3,j)+Fnode(1,1)*nxyzbar(4,j)+Fnode(2,1)*nxyzbar(5,j);
