...

{
  ...
  LINALG::Matrix<3, 3, double> F_tmatrix(F, true);
  LINALG::Inverse(F_tmatrix);
    ...
    for (int m = 0; m < 3; ++m)
    {
      for (int p = 0; p < 3; ++p)
        for (int q = 0; q < 3; ++q)
        {
          ...
          fperm(p, q) += eps;
          LINALG::Matrix<3, 3, double> fperm_tmatrix(fperm, true);
          LINALG::Inverse(fperm_tmatrix);
...

{
  ...
  Epetra_SerialDenseVector cstress(MAT::NUM_STRESS_3D);
  {
    ...
    LINALG::SerialDenseMatrix F(f);
    LINALG::Matrix<3, 3, double> F_tmatrix(F, true);
    const double detf = LINALG::Determinant(F_tmatrix);
    LINALG::Inverse(F_tmatrix);
...

{
  ...
  double detf = 0.0;
  {
    ...
    LINALG::SerialDenseMatrix F(f);
    LINALG::Matrix<3, 3, double> F_tmatrix(F, true);
    double detf = LINALG::Determinant(F_tmatrix);
    LINALG::Inverse(F_tmatrix);
  }
  ...
  for (int j = 0; j < NUMDOF_SOH8; ++j)
  {
    ...
    LINALG::SerialDenseMatrix F(f);
    LINALG::Matrix<3, 3, double> F_tmatrix(F, true);
    const double detfperm = LINALG::Determinant(F_tmatrix);
    LINALG::Inverse(F_tmatrix);
...

{
  ...
  double detf;
  {
    ...
    LINALG::SerialDenseMatrix F(f);
    LINALG::Matrix<3, 3, double> F_tmatrix(F, true);
    detf = LINALG::Determinant(F_tmatrix);
    LINALG::Inverse(F_tmatrix);
    // const double detF = 1.0/detf;
