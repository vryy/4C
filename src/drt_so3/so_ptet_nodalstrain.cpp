                                          ...
                                          const int                   gp)
{
  ...
  for (node=noderids_.begin(); node != noderids_.end(); ++node)
  {
    ...
    if (action != "calc_struct_stress")
    {
      ...
      NodalIntegration(&stiff,&force1,nodepatch,adjele,NULL,NULL,INPAR::STR::stress_none,INPAR::STR::strain_none,gp,p); // p = params
    }
    else
    {
      ...
      NodalIntegration(NULL,NULL,nodepatch,adjele,&nodalstress,&nodalstrain,iostress,iostrain,gp,p); // p = params
                                               ...
                                               Teuchos::ParameterList&       params)
{
  ...
  if (matequal) // element patch has single material
  {
    ...
    SelectMaterial(mat,stress,cmat,density,glstrain,FnodeL,0,params);
  }
  else
  {
    double density; // just a dummy density
    ...
    for (int ele=0; ele<neleinpatch; ++ele)
    {
      ...
      SelectMaterial(mat,stressele,cmatele,density,glstrain,FnodeL,0,params);
