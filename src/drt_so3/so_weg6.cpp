...
Teuchos::RCP<DRT::Element> DRT::ELEMENTS::So_weg6Type::Create( const std::string eletype,
...
(
    LINALG::Matrix<NUMGPT_WEG6,MAT::NUM_STRESS_3D>& stresses,
...
bool DRT::ELEMENTS::So_weg6::VisData(const std::string& name, std::vector<double>& data)
{
  ...
  Teuchos::RCP<MAT::So3Material> so3mat = Teuchos::rcp_dynamic_cast<MAT::So3Material>(Material());
  return so3mat->VisData(name, data, NUMGPT_WEG6);
}
