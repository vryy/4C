                                                ...
                                                Epetra_SerialDenseVector& elevec1)
{
  ...
  */
  ...
  Epetra_SerialDenseMatrix xsrefe(numnode,NUMDIM_DISP);  // material coord. of element
  Epetra_SerialDenseMatrix xscurr(numnode,NUMDIM_DISP);  // material coord. of element
  for (int i=0; i<numnode; ++i){
    xsrefe(i,0) = Nodes()[i]->X()[0];
    xsrefe(i,1) = Nodes()[i]->X()[1];
    xsrefe(i,2) = Nodes()[i]->X()[2];
    
    xscurr(i,0) = xsrefe(i,0) + mydisp[i*NODDOF_DISP+0];
    xscurr(i,1) = xsrefe(i,1) + mydisp[i*NODDOF_DISP+1];
  ...
  *----------------------------------------------------------------------*/
  ...
  for (int gpid=0; gpid<intpoints.nquad; gpid++)
  {
    ...
    sodisp_surface_integ(&drs,&xsrefe,deriv);
    ...
    {
      for(int dim=0 ; dim<NUMDIM_DISP; dim++)
      {
