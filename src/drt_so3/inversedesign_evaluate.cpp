...
{
  // compute Lambda4[k][m][p][q] = F(k,p)*F(q,m)
...
{
  // compute IF4[k][l][m][n] = 0.5 * ( F(k,m)*F(l,n) + F(k,n)*F(l,m) )
...
{
  // compute Theta4 = 0.5 * ( K(i,l)*F(k,j) + K(j,l)*F(k,i) ) with the identity matrix K_ii
  Theta(0, 0) = (F(0, 0));
  Theta(0, 1) = (F(1, 0));
  Theta(0, 2) = (F(2, 0));
  Theta(0, 3) = 0.0;
...
{
  // compute Y4[k][l][p][q] with the identity matrix K_ii:
  // Y4[k][l][p][q] = 0.5*(K(k,p)*(K(m,q)*F(l,n) + K(n,q)*F(l,m)) + K(l,p)*(K(n,q)*F(k,m) +
  // K(m,q)*F(k,n))) * S(m,n)
