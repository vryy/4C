...
#ifdef PRINT_NSTET
        std::cout << "Proc " << myrank << " MIS    " << actnodeid
             << " numele " << count[n].first << std::endl;
#endif
...
void DRT::ELEMENTS::NStetType::InitMISpatchesGreedyI(
                           ...
                           std::map<int,DRT::ELEMENTS::NStet*>&          elecids,
                           std::map<int,DRT::ELEMENTS::NStet*>&          elecids_full,
                           std::map<int,DRT::Node*>&                     noderids,
                           const int                                myrank,
                           const int                                numproc,
                           DRT::Discretization&                     dis)
...
#ifdef PRINT_NSTET
        std::cout << "Proc " << myrank << " MIS node " << mis->first << " patchsize " << misnode->NumElement() << std::endl;
#endif
...
#ifdef PRINT_NSTET
    std::cout << "Proc " << myrank << " LEFTOVER " << *ele << " " << std::endl; fflush(stdout);
#endif
    ...
    if (proc==myrank)
    {
      ...
      for (ele=eleonpatches.begin(); ele != eleonpatches.end(); ++ele)
      {
        ...
        for (p=patchweights.begin(); p!=patchweights.end(); ++p)
        {
          ...
          std::map<int,std::vector<DRT::ELEMENTS::NStet*> >::iterator patch = pstab_adjele.find(p->first);
...
#ifdef PRINT_NSTET
        std::cout << "Proc " << myrank << " NStet " << eleid
...
void DRT::ELEMENTS::NStetType::InitMISAdjacency(
                        ...
                        std::map<int,std::map<int,DRT::Node*> >&           adjnode,
                        std::map<int,std::vector<int> >&                   pstab_adjlm,
                        std::map<int,std::vector<int> >&                   adjlm,
                        const int                                myrank,
                        const int                                numproc,
                        DRT::Discretization&                     dis)
...
int DRT::ELEMENTS::NStetType::Initialize(DRT::Discretization& dis)
{
  ...
  {
#ifdef PRINT_NSTET
    if (!myrank) std::cout << "Greedy II round " << count << std::endl; fflush(stdout);
    dis.Comm().Barrier();
#endif
  ...
  if ((int)elecids.size() != 0)
  {
    ...
    {
      std::cout << "Proc " << myrank <<
              " leftover NStet with no MIS node " << *ele->second << std::endl;
    }
