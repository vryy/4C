                                    ...
                                    Epetra_SerialDenseVector& elevec3_epetra)
{
  ...
  else if (action=="calc_struct_nlnstiff")             act = So_tet4av::calc_struct_nlnstiff;
  else if (action=="calc_struct_internalforce")        act = So_tet4av::calc_struct_internalforce;
  else if (action=="calc_struct_nlnstiffmass")         act = So_tet4av::calc_struct_nlnstiffmass;
  else if (action=="calc_struct_stress")               act = So_tet4av::calc_struct_stress;
  else if (action=="postprocess_stress")               act = So_tet4av::postprocess_stress;
  ...
  switch(act)
  {
    ...
    case calc_struct_internalforce:
    {
      ...
      std::vector<double> mydisp(lm.size());
      DRT::UTILS::ExtractMyValues(*disp,mydisp,lm);
      nlnstiffmass(lm,mydisp,NULL,NULL,&elevec1,NULL,NULL,params,
        INPAR::STR::stress_none,INPAR::STR::strain_none);
    }
    break;
    ...
    case calc_struct_stress:
    {
      ...
      if (discretization.Comm().MyPID()==Owner())
      {
        ...
        INPAR::STR::StrainType iostrain = DRT::INPUT::get<INPAR::STR::StrainType>(params, "iostrain", INPAR::STR::strain_none);
          ...
          nlnstiffmass(lm,mydisp,NULL,NULL,NULL,&stress,&strain,params,
            iostress,iostrain);
    ...
    case postprocess_stress:
    {
      ...
      int gid = Id();
      LINALG::Matrix<NUMGPT_SOTET4av,MAT::NUM_STRESS_3D> gpstress(((*gpstressmap)[gid])->A(),true);
                                           ...
                                           Epetra_SerialDenseMatrix* elemat1)
{
  ...
  */
  ...
  std::vector<double> curvefacs(NUMDIM_SOTET4av, 1.0);
  for (int i=0; i < NUMDIM_SOTET4av; ++i)
  {
 ...
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::So_tet4av::nlnstiffmass(
    ...
    if (stiffmatrix != NULL)
    {
      ...
      if (gp==0)
        for (int inod=0;inod<4;++inod)
        {
          ...
          for (int n=0; n<4*3; n++)
            for (int i=0; i<3; i++)
              (*stiffmatrix)(inod*4+3,n+n/3) -= detF * invdefgrd(i,n%3) * N_XYZ(i,n/3);
        }
    }
  } // end gp loop
