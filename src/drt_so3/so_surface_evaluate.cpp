    ...
    Epetra_SerialDenseVector& elevector2, Epetra_SerialDenseVector& elevector3)
{
  ...
  else if (action == "calc_ecm_traction")
    act = StructuralSurface::calc_ecm_traction;
  ...
  switch (act)
  {
    ...
    case calc_ecm_traction:
    {
      ...
      // Teuchos::RCP<Epetra_Vector> cell_penalty_traction =
      // immersedmanager->GetPointerToECMPenaltyTraction(); Teuchos::RCP<Epetra_Vector>
      // cell_current_nodal_normals = immersedmanager->GetPointerToCurrentNodalNormals();
      std::map<int, std::vector<LINALG::Matrix<3, 1>>>* penalty_traction_at_gp =
          immersedmanager->GetPointerToPenaltyTractionAtGPMap();

      int isdummycall = params.get<int>("dummy_call");

      std::map<int, std::vector<double>>* porosity_at_gp_ =
          immersedmanager->GetPointerToPorosityAtGP();
      std::map<int, std::vector<double>>* porosity_at_gp_old =
          immersedmanager->GetPointerToPorosityAtGPOldTimestep();
      ...
      if (not during_init)
      {
        ...
        static std::multimap<std::string, Teuchos::RCP<DRT::Condition>> conditions =
            immerseddis->GetAllConditions();
        DRT::Node** bdrynodes = Nodes();

        for (condition_iterator = conditions.begin(); condition_iterator != conditions.end();
             ++condition_iterator)
        {
      ...
      for (int gp = 0; gp < intpoints.IP().nquad; gp++)
      {
        ...
        else
        {
          ...
          IMMERSED::InterpolateToImmersedIntPoint<DRT::Element::hex8,  // source
              ...
              (scatradis,  // experimental pre cmbe // backgrddis
                  immerseddis, *this, bdryxi, bdryeledisp, action,
                  porosity_gp  // result
              );
        }

        if (porosity_gp[0] == -1234.0) dserror("interpolation of porosity failed");
        // DEBUG output
        ...
        if (abs(delta_porosity_at_gp) < tol) delta_porosity_at_gp = 0.0;
        ...
        // fill map of porosities (will be used to update porosity of old timestep after convergence
        // in global algorithm)
        ...
        // compute dXYZ / drs
        LINALG::Matrix<2, 3> dxyzdrs;
        dxyzdrs.MultiplyNT(deriv, xrefe);  // to calculate unitnormal in current config. argument
                                           // must be xcurr (xrefe instead)
        ...
        //    <r^2>=
        //    R^2*alpha*((1.0-eta)/eta)^2*(1.0-exp(alpha)*sqrt(pi*alpha)*(1.0-Erf(sqrt(alpha))))
        //
        ///////////////////////////////////////////////////////////////
        static double R = DRT::Problem::Instance()
                              ->CellMigrationParams()
                              .sublist("CONFINEMENT MODULE")
                              .get<double>("ECM_FIBER_RADIUS");
        double penalty = params.get<double>("penalty", -1234.0);
        if (penalty == -1234.0) dserror("could not get penalty parameter from parameter list");
        ...
        */
        ...
        // searchdirection is showing inwards the discretization (inward normal in !!! current !!!
        // configuration)
        ...
        {
#endif
          // std::cout<<"PROC "<<backgrddis->Comm().MyPID()<<": Evaluate cell diameter from point
          // ["<<spatial_coord_gp(0)<<" "<<spatial_coord_gp(1)<<" "<<spatial_coord_gp(2)<<"] on ele
          // with GID="<<Id()<<std::endl;
          // get intersection point of cell interface and line: x(xi_gp) - unitnormal * lambda (i.e.
          // 'space point' + 'direction' * 'free parameter')
          celldiameter = IMMERSED::IntersectLineWithBoundary<DRT::Element::quad4,  // source
              DRT::Element::quad4>                                                 // target
              (immerseddis, dispnp, params, searchdirection, spatial_coord_gp, "IMMERSEDCoupling",
                  -1);

          // DEBUG safety check
          if (celldiameter == -1234.0)
          {
            std::cout << "PROC " << backgrddis->Comm().MyPID()
                      << ": Warning! Evaluation of cell diameter failed!\n"
                         "        Pertubate space point by very small value and repeat evaluation."
                      << std::endl;
            // dserror("evaluation of cell diameter failed");
            spatial_coord_gp(0) += eps;
            spatial_coord_gp(1) += eps;
            spatial_coord_gp(2) += eps;
            // get intersection point of cell interface and line: x(xi_gp) - unitnormal * lambda
            // (i.e. 'space point' + 'direction' * 'free parameter') returns cell diameter
            // perpendicular to spatial_coord_gp
            celldiameter = IMMERSED::IntersectLineWithBoundary<DRT::Element::quad4,  // source
                DRT::Element::quad4>                                                 // target
                (immerseddis, dispnp, params, searchdirection, spatial_coord_gp, "IMMERSEDCoupling",
                    -1);
            if (celldiameter == -1234.0)
              std::cout << "Warning! Found no intersection for evaluation of cell diameter!"
                        << std::endl;
        ...
        if (not during_init)
        {
          ...
          scalarproduct_disp = unit_inward_normal.Dot(
              dispnp_gp);  // component of displacement in direction inward normal (positive)
          ...
          if (scalarproduct < -1.0e-06 and abs(delta_porosity_at_gp) > 0.0)
          {
            multiplicator = 0.0;
            // effective gap
            gap = ((voiddiameter - voiddiameter_old) / 2.0) +
                  scalarproduct_disp;  // deltaD_void/2.0 + scalarproduct_disp (zero if change in
                                       // void space is equal to displacement)
            std::cout << "Id()=" << Id() << "  voiddiameter=" << voiddiameter
                      << "  voiddiameter_old=" << voiddiameter_old
                      << "  scalarprod=" << scalarproduct_disp << std::endl;
          }
          else
            multiplicator = 1.0;
        }
        ...
        if (multiplicator == -1234.0) dserror("determination of variable multiplicator failed");
...
#define LOG_LAW
        ...
        for (int node = 0; node < nen; node++)
        {
          for (int dof = 0; dof < globdim; dof++)
          {
            if (gap < 0.0)
              elevector1(node * numdofpernode + dof) +=
                  (gpweight * detA) * J * penalty * gap * tempvec(dof) * funct(node);
            else
...
#ifdef LOG_LAW
        // logarithmic law
        // std::cout<<"PROC "<<backgrddis->Comm().MyPID()<<" gap_max =
        // "<<std::setprecision(6)<<gap_max<<"for ele with GID="<<Id()<<std::endl;
        delta_penalty_force =
            log((1.0 / gap_max) * gap + 1.0) * penalty;  // < 0.0 if celldiameter > voiddiameter
#endif

#ifdef POLYNOMIAL_LAW
        // polynomial (gap^3 + gap) law
        delta_penalty_force =
            ((1.0 / (2.0 * gap_max)) * (pow(gap, 3)) + ((1.0 / (gap_max)) * gap)) *
            penalty;  // < 0.0 if celldiameter > voiddiameter
#endif
        ...
        if (not is_leadingedge_or_rear and (not during_init))
        {
          if (gap < 0.0 and delta_porosity_at_gp < 0.0)        // cell > void ; new_void < old_void
            ...
            delta_penalty_force =
                0.0;  // no change in porosity -> already equilibrium force on surface
          else if (gap > 0.0 and delta_porosity_at_gp == 0.0)  // cell < void ; new_void = old_void
            delta_penalty_force =
                0.0;  // no change in porosity -> already equilibrium force on surface
          else if (gap < 0.0 and delta_porosity_at_gp > 0.0)  // cell > void ; new_void > old_void
        ...
        if (during_init)
        {
          ...
          if ((-norm_previous_traction_gp + delta_penalty_force) < 1.0e-12 and
              not isdummycall)  // traction equal or less than zero (good case)
            current_traction_gp.Update(delta_penalty_force, tempvec, 1.0);
          else if ((-norm_previous_traction_gp + delta_penalty_force) >
                   1.0e-12)  // new traction would be pulling traction (bad case)
        ...
        for (int node = 0; node < nen; node++)
        {
          ...
          {
            elevector1(node * numdofpernode + dof) +=
                (gpweight * detA) * J * current_traction_gp(dof) * funct(node);  // compress
          ...
          if (not is_leadingedge_or_rear)
          {
            ...
            elevector2(node * numdofpernode + 1) +=
                (gpweight * detA) * J * gap * funct(node) * multiplicator;
        ...
        if (during_init)
        {
          ...
          ((penalty_traction_at_gp->at(Id())).at(gp))
              .Update((1.0 / norm_tempvec),
                  current_traction_gp);  // curr_tract_gp contains old force in new normal direction
                                         // + delta force -> total force
    ...
    Epetra_SerialDenseVector& elevector2, Epetra_SerialDenseVector& elevector3)
{
  ...
  if (action == "none")
    ...
    act = StructuralSurface::calc_ref_nodal_normals;
  else if (action == "calc_cell_growth")
    act = StructuralSurface::calc_cell_growth;
  else if (action == "calc_cell_adhesion_forces")
    act = StructuralSurface::calc_cell_adhesion_forces;
  else
    dserror("Unknown type of action for StructuralSurface");
  ...
  switch (act)
  {
    ...
    case calc_cell_growth:
    {
      ...
      static int numdof_barbedends = globalproblem->CellMigrationParams()
                                         .sublist("PROTRUSION MODULE")
                                         .get<int>("NUMDOF_BARBEDENDS");
      // dof number of actin monomer concentration
      static int numdof_actin = globalproblem->CellMigrationParams()
                                    .sublist("PROTRUSION MODULE")
                                    .get<int>("NUMDOF_ACTIN");
      if (numdof_barbedends == -1 or numdof_actin == -1)
        dserror(
            "provide the dof numbers for pointed ends and actin monomers in section ---CELL "
            "DYNAMIC/PROTRUSION MODULE !");
      // frequently used constants
      static const double delta =
          globalproblem->CellMigrationParams()
              .sublist("PROTRUSION MODULE")
              .get<double>("ACTIN_MONOMER_SIZE");  //< actin monomer size [micrometer]
      if (delta <= 0.0)
        dserror(
            "Parameter ACTIN_MONOMER_SIZE in --CELL DYNAMIC/PROTRUSION FORMATION is invalid: delta "
            "= %f",
            delta);
      ...
      const double beta =
          0.6109;  //< 35° in rad --> half branching angle, for calc of growth value to gp normal
      const int numNode = this->NumNode();  //< number of nodes of FaceElement
      const int numdofpernode =
          this->NumDofPerNode(*this->Nodes()[0]);  //< numdof of FaceElemet nodes

      // get displacement state from structure discretization
      Teuchos::RCP<const Epetra_Vector> dispnp = discretization.GetState("displacement");
      if (dispnp == Teuchos::null) dserror("Cannot get displacement vector of structure");
      ...
      DRT::UTILS::BoundaryGPToParentGP<3>(pqxg, derivtrafo, intpoints, parentele->Shape(),
          DRT::Element::quad4, this->FaceMasterNumber());
      ...
      Teuchos::RCP<MAT::So3Material> mat =
          Teuchos::rcp_dynamic_cast<MAT::So3Material>(parentele->Material(0));
      if (mat == Teuchos::null) dserror("cast to MAT::So3Material failed");
      ...
      for (int iquad = 0; iquad < intpoints.IP().nquad; iquad++)
      {
        ...
        {
          pxsi(idim) = pqxg(iquad,
              idim);  //< pxsi := coordinates of current gauss point for parent element (HEX8) [3x1]
        }
        ...
        // compute derivatives N_XYZ at gp with respect to material coordinates by N_XYZ =
        // invJ*N_rst
        ...
        LINALG::Matrix<2, 3> dxyzdrs;
        dxyzdrs.MultiplyNT(
            deriv, xcurre);  // to calculate unitnormal in current config. argument must be xcurr

        normal(0) = dxyzdrs(0, 1) * dxyzdrs(1, 2) - dxyzdrs(0, 2) * dxyzdrs(1, 1);
        normal(1) = dxyzdrs(0, 2) * dxyzdrs(1, 0) - dxyzdrs(0, 0) * dxyzdrs(1, 2);
        normal(2) = dxyzdrs(0, 0) * dxyzdrs(1, 1) - dxyzdrs(0, 1) * dxyzdrs(1, 0);

        for (int i = 0; i < 3; ++i) unitnormal(i, 0) = normal(i);
        const double norm2 = unitnormal.Norm2();
        unitnormal.Scale(1.0 / norm2);
        ...
        LINALG::Matrix<4, 1> rate_be_nd(
            true);  //< nodal barbed end rate (equal to nodal rate of change of branching points)
        ...
        if ((conc_be_n_gp > 0.0) and (camsnp_gp > 0.0))
        {
          ...
          growth = -1.0 * ((rate_am_gp - (2.0 * rate_be_gp)) * dt * delta * cos(beta)) /
                   (2.0 * conc_be_n_gp);
        ...
        {
          std::cout << "WARNING!!! "
                    << "conc_be_n_gp=" << conc_be_n_gp << "   camsnp_gp=" << camsnp_gp
                    << "   -> growth=" << 0.0 << std::endl;
          growth = 0.0;
        }
    ...
    case calc_cell_adhesion_forces:
    {
      ...
      if (num_bound_species < 0) dserror("No valid number of bound species given in .dat file!");

      // get adhesion fixpoints
      Teuchos::RCP<const Epetra_Vector> adhesion_fixpoints =
          discretization.GetState(0, "adh_fixpoint_coords");
      if (adhesion_fixpoints == Teuchos::null) dserror("Cannot get adhesion_fixpoints vector");
      ...
      for (int node = 0; node < numnodes; ++node)
        for (int dof = 0; dof < numdofpernode; ++dof)
          adhesion_nd_coord[node * numdofpernode + dof] =
              myvalues_displ[node * numdofpernode + dof] + (this->Nodes())[node]->X()[dof];
      ...
      for (int node = 0; node < numnodes; ++node)
        for (int dof = 0; dof < numdofpernode; ++dof)
          adhesion_fixpoint_nd_coord[node * numdofpernode + dof] =
              myvalues_fixpoint_coords[node * numdofpernode + dof];
      ...
      for (int node = 0; node < numnodes; ++node)
        for (int dof = 0; dof < numscatradofpernode; ++dof)
          adhesioneleconc_nd[node * numscatradofpernode + dof] =
              myvalues_conc[node * numscatradofpernode + dof];
      ...
      for (int iquad = 0; iquad < intpoints.IP().nquad; ++iquad)
      {
        ...
        for (int dof = 0; dof < numdofpernode; ++dof) adhesion_penalty_distance_gp[dof] = 0;

        for (int node = 0; node < numnodes; ++node)
          for (int dof = 0; dof < numdofpernode; ++dof)
            adhesion_penalty_distance_gp[dof] +=
                funct[node] * (adhesion_nd_coord[node * numdofpernode + dof] -
                                  adhesion_fixpoint_nd_coord[node * numdofpernode + dof]);
        ...
        if (adhesion_conc_gp < 1e-12) adhesion_conc_gp = 0.0;
