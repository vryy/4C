...
//    cout << "hths=" << *hths << ", hthr=" << *hthr << endl;
  ...
  LINALG::Matrix<NUMPRES_,NUMPRESBRO_> stabHA(true); // integral of pressure shape functions across element
  LINALG::Matrix<NUMPRES_,NUMPRES_> stabHH(true);  // mass-like matrix
  std::vector<LINALG::Matrix<NUMPRESBRO_,NUMPRESBRO_> > stabA(NUMGPT_);  // shape functions for projected Q0/constant pressure
...
//        cout << "atas_B="  << atas_B << ", atas_D" << atas_D << ", atar_A=" << atar_A << "atar_C=" << atar_C << endl;
    ...
    LINALG::Matrix<NUMDIM_,NUMDIM_> invdefgrad; // inverse of deformation gradient and its derivative
    ...
    Teuchos::RCP<MAT::So3Material> so3mat = Teuchos::rcp_dynamic_cast<MAT::So3Material>(Material());
    so3mat->Evaluate(&defgrad,&glstrain,params,&stress,&cmat);
    ...
    if (stiffmatrix != NULL)
    {
      ...
      // Ke = Keu + Kg - Kp;
      {
        ...
        if (ans_ == ans_none) {
          ...
          LINALG::Matrix<MAT::NUM_STRESS_3D,MAT::NUM_STRESS_3D> invrgtstrDbyrgtstrD; // ...U^{d;-1}_{,U}
          LINALG::Matrix<MAT::NUM_STRESS_3D,MAT::NUM_STRESS_3D> rcgDbyrgtstrD; // ...(C^d_{,U^d})^{-1}
          LINALG::Matrix<MAT::NUM_STRESS_3D,NUMDISP_> rgtstrDbydisp; // ...U^d_{,d}
          {
            ...
            {
              LINALG::FixedSizeSerialDenseSolver<MAT::NUM_STRESS_3D,MAT::NUM_STRESS_3D,NUMDISP_> rcgDbyrgtstrDsolver;
          ...
          // U^{ass}_{,d} = (C^{ass}_{,U^{ass}})^{-1} . C^{ass}_{,d}
          {
            ...
            LINALG::FixedSizeSerialDenseSolver<MAT::NUM_STRESS_3D,MAT::NUM_STRESS_3D,NUMDISP_> rcgbyrgtstrsolver;
            {
              ...
              for (int k=0; k<NUMDISP_; ++k) {
                ...
                for (int aB=0; aB<NUMDFGR_; ++aB) {
                  ...
                  for (int C=0; C<NUMDIM_; ++C) {
                    ...
                    defgradbydisp_aBk
                      += defgradD(a,C) * pseudoidenity(CB,k);
                  }
                  for (int C=0; C<NUMDIM_; ++C) {
            ...
            if (lin_ > lin_half) {
                    ...
                    for (int EF=0; EF<MAT::NUM_STRESS_3D; ++EF) {
                      ...
                      for (int GHIJ=0; GHIJ<6; ++GHIJ) {
                        if (ircgbybyrgtstr[MAT::NUM_STRESS_3D*EF+GHIJ] != -1) {
                          const int GH = ircgbybyrgtstr[MAT::NUM_STRESS_3D*EF+GHIJ] / MAT::NUM_STRESS_3D;
                          const int IJ = ircgbybyrgtstr[MAT::NUM_STRESS_3D*EF+GHIJ] % MAT::NUM_STRESS_3D;
                          // C^{ass}_{,UU} . U^{ass}_{,d} . U^{ass}_{,d}
              ...
              if (lin_ >= lin_one) {
                ...
                // U^{d-1}_{,U^d U^d}
                LINALG::Matrix<MAT::NUM_STRESS_3D,MAT::NUM_STRESS_3D*MAT::NUM_STRESS_3D> invrgtstrDbybyrgtstrD;
                ...
                LINALG::Matrix<MAT::NUM_STRESS_3D,NUMDISPSQSYM_> invrgtstrDbybydisp; // ... U^{d-1}_{DB,dk}
                for (int d=0; d<NUMDISP_; ++d) {
                  for (int k=d; k<NUMDISP_; ++k) {  // symmetric in d,k
                    const int dk = NUMDISP_*d - d*(d+1)/2 + k;
                    ...
                    for (int B=0; B<NUMDIM_; ++B) {
                      for (int D=0; D<NUMDIM_; ++D) {
                        ...
                        for (int C=0; C<NUMDIM_; ++C) {
                          ...
                          defgradbybydisp_dk
                            += invdefgradtimesboplin(BC,k) * CDfact*invrgtstrDbydisp(CD,d) * rgtstr(D,B)  // (3)
                            + invdefgradtimesboplin(BC,k) * invrgtstrD(C,D) * DBfact*rgtstrbydisp(DB,d);  // (4)
                          defgradbybydisp_dk
                            += invdefgradtimesdefgradD(B,C) * CDfact*invrgtstrDbydisp(CD,d) * DBfact*rgtstrbydisp(DB,k)  // (5)
                            + invdefgradtimesdefgradD(B,C) * CDfact*invrgtstrDbydisp(CD,k) * DBfact*rgtstrbydisp(DB,d);  // (6)
                          defgradbybydisp_dk
                            += invdefgradtimesdefgradD(B,C) * CDfact*invrgtstrDbybydisp(CD,dk) * rgtstr(D,B);  // (7)
                        }
                        defgradbybydisp_dk
...
void DRT::ELEMENTS::So_sh8p8::Stress(
  LINALG::Matrix<NUMGPT_,MAT::NUM_STRESS_3D>* elestress,
  ...
  case INPAR::STR::strain_gl:  // Green-Lagrange strain
    {
      ...
      for (int i=NUMDIM_; i<MAT::NUM_STRESS_3D; ++i)
...
void DRT::ELEMENTS::So_sh8p8::CalcSTCMatrix
(
...
int DRT::ELEMENTS::So_sh8p8Type::Initialize(DRT::Discretization& dis)
{
  ...
  for (int i=0; i<dis.NumMyColElements(); ++i)
  {
    ...
    if (dis.lColElement(i)->ElementType() != *this)
      continue;
