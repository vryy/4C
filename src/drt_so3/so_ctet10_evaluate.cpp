 ...
 *----------------------------------------------------------------------*/
 #if 0
double LINALG::SerialDenseMatrix::det()
{
	if (N()==1)
	{
		return (*this)(0,0);	
	...
	else if (N()>2)
	{
		...
		for (int i_col=0;i_col < N();i_col++)
		{
			...
			out_det=out_det+(sign* in_matrix(0,i_col)*temp_matrix.det);
			sign*=-1;
		}
		return out_det;
	}	
 ...
 */
  Epetra_SerialDenseVector* shapefct; //[NUMGPT_SOTET10](NUMNOD_SOTET10)
...
DRT::Elements::So_tet10::Tet_integrator_4point::Tet_integrator_4point(void)
{
 ...
 DRT::Elements::So_tet10::Tet_integrator_14point::Tet_integrator_14point(void)
{
  // forward initialization of necessary attributes
  num_gp = NUMGPT_SOTET10;
  num_nodes = NUMNOD_SOTET10 ;
...
/*----------------------------------------------------------------------*
 ...
 | destroyed															|				
 *----------------------------------------------------------------------*/
DRT::Elements::So_tet10::Discrete_integrator::~Discrete_integrator()
{
	delete[] shapefct_gp;
	delete[] deriv_gp;	
}
