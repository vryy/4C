                                    ...
                                    Epetra_SerialDenseVector& elevec3)
{
  ...
  switch(act) {
    ...
    case calc_struct_linstiff: {
      ...
      soh8_nlnstiffmass(lm,mydisp,myres,&elemat1,NULL,&elevec1,NULL,NULL,time);
    }
    break;
    ...
    */
    ...
    if (elestrain != NULL){
      if (!euler_almansi) {
        ...
        LINALG::SerialDenseMatrix invdefgrd(NUMDIM_SOH8,NUMDIM_SOH8);
        ...
        LINALG::SerialDenseMatrix transdefgrd(NUMDIM_SOH8,NUMDIM_SOH8);
        for (int i=0;i<NUMDIM_SOH8;++i)
        {
          for (int j=0;j<NUMDIM_SOH8;++j)
            transdefgrd(i,j)=defgrd(j,i);
        }
        LINALG::SerialDenseMatrix invtransdefgrd(NUMDIM_SOH8,NUMDIM_SOH8);
