/*-----------------------------------------------------------------------------------------------*/
/*!
\file beam_to_sphere_contact_pair.cpp
...
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::Setup()
{
  ...
  if (beam_element_ == NULL)
    dserror("cast to Beam3Base failed! first element in BeamToSphereContactPair pair"
  ...
  if (sphere_element_ == NULL)
    dserror("cast to Rigidsphere failed! second element in BeamToSphereContactPair pair"
    ...
    LINALG::SerialDenseMatrix* stiffmat22)
{
  ...
  if (std::abs(FADUTILS::CastToDouble(xicontact_)) < (1.0 + XIETATOL))    // ToDo when to reset nodalcontactflag_?
  {
  ...
  else    // Fixme do this only for the nodes (physical) end points of beams in case of C1-continuous beam centerline representation
  {
    if (numnodes != 2)
    dserror("BeamToSphereContactPair: Check for nodal contact only implemented for 2-noded beam elements!");
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::EvaluateOrthogonalityCondition(TYPE& f, const LINALG::TMatrix<TYPE,3,1>& delta_x, const double norm_delta_x, const LINALG::TMatrix<TYPE,3,1>& dx1)
{
  // reset f
  f=0.0;
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::EvaluateLinOrthogonalityCondition(TYPE& df,
                                                               ...
                                                               const bool contactactive)
{
  ...
  if (!DoNotAssemble)
  {
    for (unsigned int i=0;i<dim1;++i)
      forcevec1(i) += FADUTILS::CastToDouble(fc1_(i));

    for (unsigned int i=0;i<dim2;++i)
      forcevec2(i) += FADUTILS::CastToDouble(fc2_(i));
  }
                                                            ...
                                                            bool linxi)
{
  ...
  if (not DoNotAssemble)
  {
    ...
    std::cout << "BTSPH Contact Pair: " << Element1()->Id() << " / " << Element2()->Id() << std::endl;
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::ComputeNormal(LINALG::TMatrix<TYPE,3,1>& normal,
                                                          TYPE& gap,
                                                          TYPE& norm,
                                                          const LINALG::TMatrix<TYPE,3,1>& x1,
                                                          const LINALG::TMatrix<TYPE,3,1>& x2)
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::ComputeGap(TYPE& gap,
                                                                         const TYPE& norm)
{
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::GetShapeFunctions(LINALG::TMatrix<TYPE,1,numnodes*numnodalvalues>& N1_i,
                                                      LINALG::TMatrix<TYPE,1,numnodes*numnodalvalues>& N1_i_xi,
                                                      LINALG::TMatrix<TYPE,1,numnodes*numnodalvalues>& N1_i_xixi,
                                                      const TYPE& eta)
{
  ...
  {
     // get values and derivatives of shape functions
     DRT::UTILS::shape_function_hermite_1D(N1_i, eta, beamele_reflength_, distype1);
     DRT::UTILS::shape_function_hermite_1D_deriv1(N1_i_xi, eta, beamele_reflength_, distype1);
     DRT::UTILS::shape_function_hermite_1D_deriv2(N1_i_xixi, eta, beamele_reflength_, distype1);
  }
  else
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::ComputeLinXi(LINALG::TMatrix<TYPE,3*numnodes*numnodalvalues+3, 1>& delta_xi,
                                                          const LINALG::TMatrix<TYPE,3,1>& x1,
                                                          const LINALG::TMatrix<TYPE,3,1>& x2,
                                                          const LINALG::TMatrix<TYPE,3,1>& dx1,
                                                          const LINALG::TMatrix<TYPE,3,1>& ddx1,
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::ComputeDistance(LINALG::TMatrix<TYPE,3,1>& distance,
                                                                 TYPE& normdist,
                                                                 const LINALG::TMatrix<TYPE,3,1>& normal,
                                                                 const TYPE& norm)
{
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::ComputeLinGap(LINALG::TMatrix<TYPE,3*numnodes*numnodalvalues+3, 1>& delta_gap,
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::ComputeLinNormal(LINALG::TMatrix<TYPE,3, 3*numnodes*numnodalvalues+3>& delta_normal,
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::Print(std::ostream& out) const
{
  // ToDo add further information here
  out << "\nInstance of BeamToSphereContactPair: element GIDs "
      << Element1()->Id() << " (beam) and " << Element2()->Id() << " (sphere)";
 ...
 *-----------------------------------------------------------------------------------------------*/
template<unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSphereContactPair<numnodes, numnodalvalues>::PrintSummaryOneLinePerActiveSegmentPair(
