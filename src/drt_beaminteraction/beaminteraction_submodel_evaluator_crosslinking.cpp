...
void BEAMINTERACTION::SUBMODELEVALUATOR::Crosslinking::Reset()
{
  ...
  for ( iter = doublebondcl_.begin(); iter != doublebondcl_.end(); ++iter )
  {
    ...
    CROSSLINKING::CrosslinkerNode *crosslinker_i =dynamic_cast< CROSSLINKING::CrosslinkerNode* >( ParticleHandlerPtr()->BinStrategy()->BinDiscret()->gNode( elepairptr->Id() ) );
    ...
    IO::DiscretizationWriter& iowriter) const
{
  ...
  Teuchos::RCP<DRT::Discretization> bindis = ParticleHandler().BinStrategy().BinDiscret();

  // mesh is not written to disc, only maximum node id is important for output
  bindis->Writer()->ParticleOutput(GState().GetStepN(), GState().GetTimeN(), false);
    ...
    AddBinsWithRelevantContentForIaDiscretColMap( std::set< int >& colbins ) const
{
  ...
  {
    // get current bin
    DRT::Element* currbin = ParticleHandler().BinStrategy().BinDiscret()->gElement( *iter );
...
/*  std::set< int > colbinsext(colbins.begin(),colbins.end());
  ...
  {
    std::vector<int> binvec;
    particlehandler_->BinStrategy()->GetNeighborAndOwnBinIds( *biniter, binvec );
    colbinsext.insert( binvec.begin(), binvec.end() );
  }
      ...
      case 1:
      {
        ...
        if( colelelid < 0 )
          dserror("Crosslinker has %i bonds but his binding partner with gid %i "
                  ...
                  cldata_i.clnumbond,elegid,GState().GetMyRank());
#endif
    ...
    std::map<int, std::vector<BindEventData> >& undecidedbonds)
{
  ...
  for( icl = rordercolcl.begin(); icl != rordercolcl.end(); ++icl )
  {
    ...
    ParticleHandlerPtr()->BinStrategy()->GetBinContent( neighboring_beams, bc,
    ...
    std::map<int, NewDoubleBonds> const& mynewdbondcl)
{
  ...
  for ( iter = mynewdbondcl.begin(); iter != mynewdbondcl.end(); ++iter )
  {
    ...
    CROSSLINKING::CrosslinkerNode *crosslinker_i =dynamic_cast< CROSSLINKING::CrosslinkerNode* >( ParticleHandlerPtr()->BinStrategy()->BinDiscret()->gNode( linkelepairptr->Id() ) );

    if (crosslinker_i->ClData()->GetNumberOfBonds() != 2)
     dserror("Setup: Cl with gid %i Owner %i on myrank %i and numbonds %i",linkelepairptr->Id(),crosslinker_i->Owner(), GStatePtr()->GetMyRank(), crosslinker_i->ClData()->GetNumberOfBonds() );
  }
    ...
    switch(cldata_i.clnumbond)
    {
      ...
      case 2:
      {
        ...
        if( not doublebondcl_.count( crosslinker_i->Id() ) )
          dserror("double bonded crosslinker %i is not in double bonded map of rank %i, something"
                  "went wrong ... ", crosslinker_i->Id(), GStatePtr()->GetMyRank() );
        ...
        if( abs( crosslinking_params_ptr_->DeltaBellEq() ) > 1.0e-8 )
          CalcBellsForceDependentUnbindProbability( doublebondcl_[crosslinker_i->Id()], koff, p_unlink_db );
        else
        ...
        for( clbspotiter = ro.begin(); clbspotiter != ro.end(); ++clbspotiter )
        {
          ...
          // erase crosslinker out double bonded crosslinker list
  ...
  for( int rec = 0; rec < summedtargets[GState().GetMyRank()]; ++rec )
  {
    ...
    while ( position < rdata.size() )
    {
      ...
      Teuchos::RCP<BEAMINTERACTION::BeamToBeamLinkage> beamtobeamlink =
          Teuchos::rcp_dynamic_cast<BEAMINTERACTION::Beam3rLin2Linkage>(object);
