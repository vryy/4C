...
void BEAMINTERACTION::BeamToBeamContactPair<numnodes, numnodalvalues>::Setup()
{
  ...
  const DRT::ELEMENTS::Beam3Base* ele1ptr = dynamic_cast<const DRT::ELEMENTS::Beam3Base*>(Element1());
  double l1 = ele1ptr->RefLength();
  const DRT::ELEMENTS::Beam3Base* ele2ptr = dynamic_cast<const DRT::ELEMENTS::Beam3Base*>(Element2());
  ...
  std::map<std::pair<int,int>,LINALG::TMatrix<double,3,1> >::iterator iter;
    ...
    for (iter=closelargeanglesegments.begin(); iter != closelargeanglesegments.end(); ++iter)
    {
      ...
      for(unsigned int i=0;i<cpvariables_.size();i++)
      {
        ...
        if(std::fabs(eta1_eval-FADUTILS::CastToDouble(closestpoint.first))<XIETARESOLUTIONFAC*XIETAITERATIVEDISPTOL and std::fabs(eta2_eval-FADUTILS::CastToDouble(closestpoint.second))<XIETARESOLUTIONFAC*XIETAITERATIVEDISPTOL)
          allready_found=true;
      }
      ...
      if(validpairfound and !allready_found)
      {
        ...
        cpvariables_.push_back(Teuchos::rcp (new BeamToBeamContactVariables<numnodes,numnodalvalues>(closestpoint,leftpoint_ids,integration_ids,pp,1.0)));
      }
    }
}
/*----------------------------------------------------------------------*
...
//    std::cout << "cpvariables_[numcp]->Getfp(): " << cpvariables_[numcp]->Getfp() << std::endl;
    ...
    if (forcevec1!=NULL and forcevec2!=NULL)
      EvaluateFcContact(*forcevec1, *forcevec2, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi, cpvariables_[numcp], 1.0,true,false,false,false);
    ...
    if (stiffmat11 != NULL and stiffmat12!=NULL and stiffmat21!=NULL and stiffmat22!=NULL)
      EvaluateStiffcContact(*stiffmat11, *stiffmat12, *stiffmat21, *stiffmat22, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi, N1_xixi, N2_xixi,cpvariables_[numcp],1.0,true,false,false,false);
          ...
          if(BEAMCONTACT::WithinInterval(eta1_slave,eta1_segleft,eta1_segright))
          {
            ...
            if(solutionwithinsegment)
            {
              ...
              if(pairactive)
              {
                ...
                gpvariables_.push_back(Teuchos::rcp (new BeamToBeamContactVariables<numnodes,numnodalvalues>(closestpoint,leftpoint_ids,integration_ids,parallel_pp,jacobi)));
              }
...
//    std::cout << "gpvariables_[numgptot]->Getfp(): " << gpvariables_[numgp]->Getfp() << std::endl;
    ...
    // call function to compute contact contribution to residual vector
    if (forcevec1!=NULL and forcevec2!=NULL)
      EvaluateFcContact(*forcevec1, *forcevec2, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi, gpvariables_[numgptot],intfac,false,true,false,false);

    #ifndef ENDPOINTSEGMENTATION
    // call function to compute contact contribution to stiffness matrix
    if (stiffmat11 != NULL and stiffmat12!=NULL and stiffmat21!=NULL and stiffmat22!=NULL)
      EvaluateStiffcContact(*stiffmat11, *stiffmat12, *stiffmat21, *stiffmat22, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi, N1_xixi, N2_xixi,gpvariables_[numgptot],intfac,false,true,false,false);
    #else
  ...
  for(int i=0;i<(int)closeendpointsegments.size();i++)
  {
    ...
    if(segid1==0 and boundarynode1_.first)
    {
      ...
      if(solutionwithinsegment and pairactive)
      {
          ...
          epvariables_.push_back(Teuchos::rcp (new BeamToBeamContactVariables<numnodes,numnodalvalues>(closestpoint,leftpoint_ids,integration_ids,pp,1.0)));
      }
    }
...
//    std::cout << "epvariables_[numep]->Getfp(): " << epvariables_[numep]->Getfp() << std::endl;
    ...
    // call function to compute contact contribution to residual vector
    if (forcevec1!=NULL and forcevec2!=NULL)
      EvaluateFcContact(*forcevec1, *forcevec2, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi, epvariables_[numep],1.0,false,false,fixedendpointxi,fixedendpointeta);

    // call function to compute contact contribution to stiffness matrix
    if (stiffmat11 != NULL and stiffmat12!=NULL and stiffmat21!=NULL and stiffmat22!=NULL)
      EvaluateStiffcContact(*stiffmat11, *stiffmat12, *stiffmat21, *stiffmat22, r1, r2, r1_xi, r2_xi, r1_xixi, r2_xixi, N1, N2, N1_xi, N2_xi, N1_xixi, N2_xixi,epvariables_[numep],1.0,false,false,fixedendpointxi,fixedendpointeta);
  }
}
/*----------------------------------------------------------------------*
...
//      std::cout << "angle: " << BEAMCONTACT::CalcAngle(FADUTILS::CastToDouble<TYPE,3,1>(r1_xi),FADUTILS::CastToDouble<TYPE,3,1>(r2_xi))/M_PI*180.0 << std::endl;
    ...
    else
    {
      ...
      if(eta_left2 - XIETAITERATIVEDISPTOL <= eta2 and eta2 <= eta_right2 + XIETAITERATIVEDISPTOL)
      {
        ...
        if(CheckContactStatus(gap_test) and relevant_angle)
          pairactive=true;
