    ...
    LINALG::Matrix<3, 1, scalar_type_rot_vec>& psi_solid, const DRT::Element* element) const
{
...
#if 0
  // Get beam e1 vector in reference configuration.
  LINALG::Matrix<3, 3, double> triad_beam_ref(true);
  LARGEROTATIONS::quaterniontotriad(quaternion_beam_ref, triad_beam_ref);
  LINALG::Matrix<3, 1, double> dr_beam_ref(true);
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++) dr_beam_ref(i_dir) = triad_beam_ref(i_dir, 0);

  // Calculate surface basis vectors in ref configuration.
  LINALG::Matrix<3, 2, double> dr_surf_ref(true);
  GEOMETRYPAIR::EvaluatePositionDerivative1<surface>(xi, q_solid_ref, dr_surf_ref, element);

  // Calculate basis vectors in the surface coordinate system.
  LINALG::Matrix<3, 1, double> dr_surf_ref_0;
  LINALG::Matrix<3, 1, double> dr_surf_ref_1;
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++)
  {
    dr_surf_ref_0(i_dir) = dr_surf_ref(i_dir, 0);
    dr_surf_ref_1(i_dir) = dr_surf_ref(i_dir, 1);
  }

  // Calculate normal on the basis vectors.
  LINALG::Matrix<3, 1, double> element_surface_normal_ref;
  element_surface_normal_ref.CrossProduct(dr_surf_ref_0, dr_surf_ref_1);

  // Put the new basis vectors in a matrix, and perform a basis transformation. The first two
  // coordinates in the transformed basis are the projected coordinates into the surface
  // plane.
  LINALG::Matrix<3, 3, double> surface_basis_inverse;
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++)
  {
    surface_basis_inverse(i_dir, 0) = dr_surf_ref(i_dir, 0);
    surface_basis_inverse(i_dir, 1) = dr_surf_ref(i_dir, 1);
    surface_basis_inverse(i_dir, 2) = element_surface_normal_ref(i_dir);
  }
  LINALG::Inverse(surface_basis_inverse);
  LINALG::Matrix<3, 1, double> new_basis;
  new_basis.Multiply(surface_basis_inverse, dr_beam_ref);

  // Calculate the reference surface triad.
  LINALG::Matrix<3, 1, double> beam_director_ref(true);
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++)
  {
    beam_director_ref(i_dir) += new_basis(0) * dr_surf_ref(i_dir, 0);
    beam_director_ref(i_dir) += new_basis(1) * dr_surf_ref(i_dir, 1);
  }
  beam_director_ref.Scale(1.0 / FADUTILS::VectorNorm(beam_director_ref));
  element_surface_normal_ref.Scale(1.0 / FADUTILS::VectorNorm(element_surface_normal_ref));
  LINALG::Matrix<3, 1, double> third_basis_vector_ref(true);
  third_basis_vector_ref.CrossProduct(beam_director_ref, element_surface_normal_ref);
  LINALG::Matrix<3, 3, double> surface_triad_ref;
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++)
  {
    surface_triad_ref(i_dir, 0) = beam_director_ref(i_dir);
    surface_triad_ref(i_dir, 1) = element_surface_normal_ref(i_dir);
    surface_triad_ref(i_dir, 2) = third_basis_vector_ref(i_dir);
  }

  // Get the relative triad in the reference configuration.
  LINALG::Matrix<3, 3, double> surface_triad_rel;
  surface_triad_rel.MultiplyTN(surface_triad_ref, triad_beam_ref);
  LINALG::Matrix<3, 3, scalar_type_rot_vec> surface_triad_rel_scalar_type;
  for (unsigned int i_row = 0; i_row < 3; i_row++)
    for (unsigned int i_col = 0; i_col < 3; i_col++)
      surface_triad_rel_scalar_type(i_row, i_col) = surface_triad_rel(i_row, i_col);

  // Calculate the current surface basis vectors.
  LINALG::Matrix<3, 2, scalar_type_rot_vec> dr_surf(true);
  GEOMETRYPAIR::EvaluatePositionDerivative1<surface>(xi, q_solid, dr_surf, element);
  LINALG::Matrix<3, 1, scalar_type_rot_vec> dr_surf_0;
  LINALG::Matrix<3, 1, scalar_type_rot_vec> dr_surf_1;
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++)
  {
    dr_surf_0(i_dir) = dr_surf(i_dir, 0);
    dr_surf_1(i_dir) = dr_surf(i_dir, 1);
  ...
  // Normal in current configuration.
  LINALG::Matrix<3, 1, scalar_type_rot_vec> element_surface_normal;
  element_surface_normal.CrossProduct(dr_surf_0, dr_surf_1);

  // Director in current configuration.
  dr_surf_0.Scale(new_basis(0));
  dr_surf_1.Scale(new_basis(1));
  LINALG::Matrix<3, 1, scalar_type_rot_vec> beam_director(true);
  beam_director = dr_surf_0;
  beam_director += dr_surf_1;

  // Normalize the first two basis vectors.
  beam_director.Scale(1.0 / FADUTILS::VectorNorm(beam_director));
  element_surface_normal.Scale(1.0 / FADUTILS::VectorNorm(element_surface_normal));

  // Calculate third basis vector.
  LINALG::Matrix<3, 1, scalar_type_rot_vec> third_basis_vector(true);
  third_basis_vector.CrossProduct(beam_director, element_surface_normal);

  // Assemble the basis vectors into the current surface triad.
  LINALG::Matrix<3, 3, scalar_type_rot_vec> surface_triad;
  for (unsigned int i_dir = 0; i_dir < 3; i_dir++)
  {
    surface_triad(i_dir, 0) = beam_director(i_dir);
    surface_triad(i_dir, 1) = element_surface_normal(i_dir);
    surface_triad(i_dir, 2) = third_basis_vector(i_dir);
  }
  LINALG::Matrix<3, 3, scalar_type_rot_vec> surface_triad_relative_to_beam;
  surface_triad_relative_to_beam.MultiplyNN(surface_triad, surface_triad_rel_scalar_type);

  // Get the rotation angle.
  LINALG::Matrix<4, 1, scalar_type_rot_vec> rot_quat;
  LARGEROTATIONS::triadtoquaternion(surface_triad_relative_to_beam, rot_quat);
