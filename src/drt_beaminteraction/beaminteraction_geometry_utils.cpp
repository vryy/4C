...
bool BEAMINTERACTION::GEO::PointToCurveProjection(
    ...
    double master_ele_ref_length)
{
  ...
  for (unsigned int iter=0; iter<POINT_TO_CURVE_PROJECTION_MAX_NUM_ITER; ++iter)
  {
    ...
    EvaluatePointToCurveOrthogonalityCondition(
        f, delta_r, norm_delta_r, r_xi_master);
    ...
    bool validlinearization =
        EvaluateLinearizationPointToCurveOrthogonalityCondition(
            df, delta_r, norm_delta_r, r_xi_master, r_xixi_master);

    if (not validlinearization)
      dserror("Linearization of point to line projection is zero, i.e. the minimal distance "
  ...
  IO::cout(IO::debug) << "\n\nWARNING Point-to-Curve projection: local Newton loop " <<
      "unconverged after " << POINT_TO_CURVE_PROJECTION_MAX_NUM_ITER << " iterations!" << IO::endl;
  ...
  IO::cout(IO::debug) << "xi_master_previous_iteration: " << xi_master_previous_iteration << IO::endl;
 ...
 *-----------------------------------------------------------------------------------------------*/
template<typename T>
void BEAMINTERACTION::GEO::EvaluatePointToCurveOrthogonalityCondition(
    T& f,
 ...
 *-----------------------------------------------------------------------------------------------*/
template<typename T>
bool BEAMINTERACTION::GEO::EvaluateLinearizationPointToCurveOrthogonalityCondition(
    T& df,
    const LINALG::TMatrix<T,3,1>& delta_r,
    const double norm_delta_r,
    const LINALG::TMatrix<T,3,1>& r_xi_master,
    ...
    const LINALG::TMatrix<T,3,3*numnodes*numnodalvalues>& N_xi_master)
{
  ...
  CalcPTCProjectionOrthogonalityConditionPartialDerivParameterCoordMaster(
      orthogon_condition_partial_xi_master,
      delta_r,
      r_xi_master,
      r_xixi_master);
  ...
  CalcPTCProjectionOrthogonalityConditionPartialDerivClPosSlave(
      orthogon_condition_partial_r_slave,
      r_xi_master);
  ...
  CalcPTCProjectionOrthogonalityConditionPartialDerivClPosMaster(
      orthogon_condition_partial_r_master,
      r_xi_master);
  ...
  CalcPTCProjectionOrthogonalityConditionPartialDerivClTangentMaster(
      orthogon_condition_partial_r_xi_master,
      delta_r);
  ...
  for (unsigned int idim=0; idim<3; ++idim)
    ...
    {
      lin_xi_master_slaveDofs(jdof) += orthogon_condition_partial_r_slave(idim) * N_slave(idim,jdof);
    }

  for (unsigned int idim=0; idim<3; ++idim)
    for (unsigned int jdof=0; jdof<dim2; ++jdof)
    {
      lin_xi_master_masterDofs(jdof) += orthogon_condition_partial_r_master(idim) * N_master(idim,jdof)
    ...
    const LINALG::TMatrix<T,3,1>& r_xixi_master)
{
  ...
  xi_master_partial_r_slave.Update( -1.0/orthogon_condition_partial_xi_master,
      orthogon_condition_partial_r_slave );

  xi_master_partial_r_master.Update( -1.0/orthogon_condition_partial_xi_master,
      orthogon_condition_partial_r_master );

  xi_master_partial_r_xi_master.Update( -1.0/orthogon_condition_partial_xi_master,
      orthogon_condition_partial_r_xi_master );
}
...
{
  orthogon_condition_partial_xi_master =
        -r_xi_master.Dot(r_xi_master) + delta_r.Dot(r_xixi_master);
...
template bool BEAMINTERACTION::GEO::PointToCurveProjection<2,1,double>(
    LINALG::TMatrix<double,3,1> const&,
    double&,
    double const&,
...
template bool BEAMINTERACTION::GEO::PointToCurveProjection<2,1,Sacado::Fad::DFad<double> >(
    LINALG::TMatrix<Sacado::Fad::DFad<double>,3,1> const&,
...
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<2,1,double>(
    LINALG::TMatrix<double,1,6>&,
    LINALG::TMatrix<double,1,6>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,6>&,
    const LINALG::TMatrix<double,3,6>&,
    const LINALG::TMatrix<double,3,6>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<3,1,double>(
    LINALG::TMatrix<double,1,9>&,
    LINALG::TMatrix<double,1,9>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,9>&,
    const LINALG::TMatrix<double,3,9>&,
    const LINALG::TMatrix<double,3,9>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<4,1,double>(
    LINALG::TMatrix<double,1,12>&,
    LINALG::TMatrix<double,1,12>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,12>&,
    const LINALG::TMatrix<double,3,12>&,
    const LINALG::TMatrix<double,3,12>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<5,1,double>(
    LINALG::TMatrix<double,1,15>&,
    LINALG::TMatrix<double,1,15>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,15>&,
    const LINALG::TMatrix<double,3,15>&,
    const LINALG::TMatrix<double,3,15>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<2,2,double>(
    LINALG::TMatrix<double,1,12>&,
    LINALG::TMatrix<double,1,12>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    ...
    const LINALG::TMatrix<double,3,12>&,
    const LINALG::TMatrix<double,3,12>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<2,1,Sacado::Fad::DFad<double> >(
    ...
    const LINALG::TMatrix<Sacado::Fad::DFad<double>,3,6>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<3,1,Sacado::Fad::DFad<double> >(
    ...
    const LINALG::TMatrix<Sacado::Fad::DFad<double>,3,9>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<4,1,Sacado::Fad::DFad<double> >(
    ...
    const LINALG::TMatrix<Sacado::Fad::DFad<double>,3,12>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<5,1,Sacado::Fad::DFad<double> >(
    ...
    const LINALG::TMatrix<Sacado::Fad::DFad<double>,3,15>&);
template void BEAMINTERACTION::GEO::CalcLinearizationPointToCurveProjectionParameterCoordMaster<2,2,Sacado::Fad::DFad<double> >(
    LINALG::TMatrix<Sacado::Fad::DFad<double>,1,12>&,
    LINALG::TMatrix<Sacado::Fad::DFad<double>,1,12>&,
    const LINALG::TMatrix<Sacado::Fad::DFad<double>,3,1>&,
    const LINALG::TMatrix<Sacado::Fad::DFad<double>,3,1>&,
...
template void BEAMINTERACTION::GEO::CalcPointToCurveProjectionParameterCoordMasterPartialDerivs<double>(
    LINALG::TMatrix<double,1,3>&,
    LINALG::TMatrix<double,1,3>&,
    LINALG::TMatrix<double,1,3>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&,
    const LINALG::TMatrix<double,3,1>&);
template void BEAMINTERACTION::GEO::CalcPointToCurveProjectionParameterCoordMasterPartialDerivs<Sacado::Fad::DFad<double> >(
