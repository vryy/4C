...
void BEAMINTERACTION::SphereBeamLinkingParams::Init( STR::TIMINT::BaseDataGlobalState const & gstate )
{
  ...
  // number of linker in simulation volume
  {
    ...
    char* input;
    while (PL >> word)
      maxnumlinkerpertype_.push_back( std::strtod( word.c_str(), &input ) );

    // safety check
    for( unsigned int i = 0; i < maxnumlinkerpertype_.size(); ++i )
      if ( maxnumlinkerpertype_[i] < 0 )
        dserror(" negative number of linker does not make sense.");
  }
  ...
  // material numbers for crosslinker types
  {
    ...
    char* input;
    while (PL >> word)
      matlinkerpertype_.push_back( std::strtod( word.c_str(), &input ) );

    for( unsigned int i = 0; i < matlinkerpertype_.size(); ++i )
    {
      // safety check
      if ( matlinkerpertype_[i] < 0 )
        dserror(" negative material number does not make sense.");

      // store materials
      mat_.push_back(Teuchos::rcp_dynamic_cast<MAT::CrosslinkerMat>( MAT::Material::Factory( matlinkerpertype_[i] ) ) );
  ...
  {
    if ( not ( std::find( linkertypes_.begin(), linkertypes_.end(), matlinkerpertype_[type_i]) != linkertypes_.end() ) )
      linkertypes_.push_back( Teuchos::rcp_dynamic_cast< MAT::CrosslinkerMat >(
          MAT::Material::Factory( matlinkerpertype_[type_i] ) )->LinkerType() );
  }
  ...
  // distance between the two binding spots on each filament the same
  {
    filamentbspotintervalglobal_.clear();
    std::istringstream PL(
        Teuchos::getNumericStringParameter( spherebeamlink_params_list, "FILAMENTBSPOTINTERVALGLOBAL") );
    std::string word;
    char * input;
    int count = 0;
    while ( PL >> word )
  ...
  // distance between the two binding spots on a filament as percentage of current filament reference length
  {
    filamentbspotintervallocal_.clear();
    std::istringstream PL(
        Teuchos::getNumericStringParameter( spherebeamlink_params_list, "FILAMENTBSPOTINTERVALLOCAL" ) );
    std::string word;
    char * input;
    int count = 0;
    while ( PL >> word )
