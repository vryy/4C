...
BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    ...
    : BeamContactPair(), atleastone_(false), n_gp_(0), ele1gid_(0), contactflag_(false)
{
  // empty constructor
}
...
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes, numnodalvalues>::Setup()
{
  ...
  // Initialize the vector that keeps track of which Gauss points have a valid projection in this
  // pair get number of Gaus points of the beam element
  ...
  std::map<int, std::vector<bool>>* GaussPointEvaluateTracker =
      EvaluationData()
          ...
          ->BTSVOLMTGaussPointEvaluateTracker();
  std::map<int, std::vector<bool>>* GaussPointProjectionTracker =
      EvaluationData()
          ->BeamToSolidVolumeMeshtyingEvaluationData()
          ->BTSVOLMTGaussPointProjectionTracker();
    ...
    if ((*GaussPointProjectionTracker)[ele1gid_][i_gp] == false)
    {
      ...
      if (proj)
      {
        if (output)
        {
          ...
          std::cout << "Gauss Point " << i_gp << " with eta = " << x_gp
                    << " was projected at xi1 = " << xi1 << " xi2 = " << xi2 << " xi3 = " << xi3
                    << std::endl;
        }
    ...
    LINALG::SerialDenseMatrix* stiffmat22)
{
  ...
  double pp =
      Params()->BeamToSolidVolumeMeshtyingParams()->BeamToSolidVolumeMeshtyingPenaltyParam();
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::EvaluateStiffmtMeshtying(LINALG::SerialDenseMatrix& stiffmat11,
    LINALG::SerialDenseMatrix& stiffmat12, LINALG::SerialDenseMatrix& stiffmat21,
    ...
    const LINALG::TMatrix<TYPEBTS, 3, 3 * numnodessol>& N2, TYPEBTS& jacobi, const double& pp)
{
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < dim1; j++)
      for (int k = 0; k < dim1; k++) stiffmat11(j, k) += pp * jacobi * w_gp * N1(i, j) * N1(i, k);
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < dim2; j++)
      for (int k = 0; k < dim2; k++) stiffmat22(k, j) += pp * jacobi * w_gp * N2(i, k) * N2(i, j);
}
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::EvaluateFmtMeshtying(LINALG::SerialDenseVector& forcevec1,
    LINALG::SerialDenseVector& forcevec2, LINALG::SerialDenseMatrix& stiffmat11,
    LINALG::SerialDenseMatrix& stiffmat12, LINALG::SerialDenseMatrix& stiffmat21,
    LINALG::SerialDenseMatrix& stiffmat22)
{
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::FillElementStiffmtMeshtying(LINALG::SerialDenseMatrix& local_stiffmat11,
    ...
    LINALG::SerialDenseMatrix& stiffmat22)
{
  ...
  for (int i = 0; i < dim1; i++)
    for (int j = 0; j < dim1; j++) stiffmat11(i, j) += local_stiffmat11(i, j);
  ...
  for (int i = 0; i < dim2; i++)
    for (int j = 0; j < dim2; j++) stiffmat22(i, j) += local_stiffmat22(i, j);
}
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::EvaluateBoundarySegmentation()
{
  /******************************************************************************************
   Initialize some data
   ******************************************************************************************/
  ...
  if (startBool)
  {
    ...
    if (output)
    {
      std::cout << "BOUNDARY SEGMENTATION for GP no. " << change << ", eta = " << compareEta
                << " of Beam element " << ele1gid_ << " and solid element " << Element2()->Id()
                << std::endl;
    }
  }
    ...
    if (proj)
    {
      ...
      if (fabs(eta - compareEta) < fabs(curreta - compareEta))
      {
        ...
        if (output)
        {
          std::cout << "FOUND new point for beam element " << ele1gid_
                    << " projecting on solid element " << Element2()->Id() << std::endl;
          std::cout << "Gauss point " << compareEta << ", Found eta = " << eta << ", xi1 = " << xi1
                    << ", xi2 = " << xi2 << ", xi3 = " << xi3 << std::endl;
        }
      }
    }
  ...
  for (int i_gp = 0; i_gp < n_gp_; i_gp++)
  {
    ...
    if (proj)
    {
      ...
      if (output)
      {
        std::cout << "BOUNDARY SEGMENTATION FINISHED for GP no. " << i_gp << " of beam element "
                  << ele1gid_ << std::endl;
        if (localGaussPointTracker_[i_gp] == true)
        {
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::SurfProjection(TYPEBTS& xi1, TYPEBTS& xi2, TYPEBTS& xi3, TYPEBTS& eta,
    const int fixedPar, double fixedAt, bool& proj_allowed)
{
  proj_allowed = true;
  bool parallel = false;
  ...
  if (numnodessol == 8 || numnodessol == 20 || numnodessol == 27)
  {
    ...
    {
      proj_allowed = false;
      if (output) std::cout << "Projection not allowed" << std::endl;
    }
  }
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::Projection(TYPEBTS& xi1, TYPEBTS& xi2, TYPEBTS& xi3, TYPEBTS& eta,
    bool& proj_allowed)
{
  proj_allowed = true;
  bool parallel = false;
  ...
  {
    std::cout << "Projection output:" << std::endl;
    std::cout << "Start parameters xi1: " << xi1 << ", xi2: " << xi2 << ", xi3: " << xi3
              << std::endl;
  }
  ...
  if (numnodessol == 8 || numnodessol == 27 || numnodessol == 20)
  {
    ...
    {
      if (output) std::cout << "Projection allowed" << std::endl;
    }
  }
  // TET4, TET10 --> xi1 > 0; xi2 > 0; xi3 > 0 and xi1 + xi2 + xi3 < 1 for valid projection
  else if (numnodessol == 4 || numnodessol == 10)
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::GetSolShapeFunctions(LINALG::TMatrix<TYPEBTS, 3, 3 * numnodessol>& N,
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::AssembleSolShapefunctions(const LINALG::TMatrix<TYPEBTS, 1, numnodessol>& N_i,
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::ComputeSolCoordsAndDerivsRef(LINALG::TMatrix<TYPEBTS, 3, 1>& r,
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::ComputeSolCoordsAndDerivsMom(LINALG::TMatrix<TYPEBTS, 3, 1>& r,
    LINALG::TMatrix<TYPEBTS, 3, 1>& r_xi1, LINALG::TMatrix<TYPEBTS, 3, 1>& r_xi2,
    LINALG::TMatrix<TYPEBTS, 3, 1>& r_xi3, LINALG::TMatrix<TYPEBTS, 3, 3 * numnodessol>& N,
    LINALG::TMatrix<TYPEBTS, 3, 3 * numnodessol>& N_xi1,
    LINALG::TMatrix<TYPEBTS, 3, 3 * numnodessol>& N_xi2,
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::GetBeamJacobi(const TYPEBTS& eta, TYPEBTS& jac)
{
...
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes, numnodalvalues>::
    GetBeamShapeFunctions(LINALG::TMatrix<TYPEBTS, 3, 3 * numnodes * numnodalvalues>& N,
        ...
        LINALG::TMatrix<TYPEBTS, 3, 3 * numnodes * numnodalvalues>& N_etaeta, const TYPEBTS& eta)
{
  ...
  else if (numnodalvalues == 2)
  {
    ...
    dserror(
        "Only beam elements with one (nodal positions) or two "
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::ComputeBeamCoordsAndDerivsRef(LINALG::TMatrix<TYPEBTS, 3, 1>& r,
...
template <unsigned int numnodessol, unsigned int numnodes, unsigned int numnodalvalues>
void BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<numnodessol, numnodes,
    numnodalvalues>::ComputeBeamCoordsAndDerivsMom(LINALG::TMatrix<TYPEBTS, 3, 1>& r,
    LINALG::TMatrix<TYPEBTS, 3, 1>& r_eta, LINALG::TMatrix<TYPEBTS, 3, 1>& r_etaeta,
    const LINALG::TMatrix<TYPEBTS, 3, 3 * numnodes * numnodalvalues>& N,
    const LINALG::TMatrix<TYPEBTS, 3, 3 * numnodes * numnodalvalues>& N_eta,
    const LINALG::TMatrix<TYPEBTS, 3, 3 * numnodes * numnodalvalues>& N_etaeta)
...
template class BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<8, 2,
    2>;  // Hermite beam element, hex8 solid element
template class BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<20, 2,
    2>;  // Hermite beam element, hex20 solid element
template class BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<27, 2,
    2>;  // Hermite beam element, hex27 solid element
template class BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<4, 2,
    2>;  // Hermite beam element, tet4 solid element
template class BEAMINTERACTION::BeamToSolidVolumeMeshtyingPair<10, 2,
    2>;  // Hermite beam element, tet10 solid element
