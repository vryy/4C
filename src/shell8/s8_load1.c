...
/*---------------------------------- FSI loads are treated seperately ! */
foundsurface=0;
if (!(ele->g.gsurf->neum)) goto endsurface;
if (ele->g.gsurf->neum->neum_type==neum_FSI) goto endsurface;
foundsurface=1;
/*---------------------------------------------------- initialize eload */
...
/* condition off, for a combination of live and FSI loads keep it       */
if (ele->g.gsurf->neum->neum_type != neum_live_FSI &&
    ele->g.gsurf->neum->neum_type != neum_opres_FSI)
...
{
case neum_live: case neum_live_FSI:
/*----------------------------------------------------------------------*/
/*                                                    uniform live load */
/*----------------------------------------------------------------------*/
/*
...
case neum_orthopressure: case neum_opres_FSI:
/*----------------------------------------------------------------------*/
...
DOUBLE       nsigma[6][MAXNOD_SHELL8]; /* nodal fluid stresses       */
DOUBLE       sigmaint[6];   /* fluid stresses at integration point    */
DOUBLE      *hte;
DOUBLE       hhi;
DOUBLE       e1,e2,e3;
DOUBLE       facr,facs,wgt;
...
/*--------------------------------- get nodal stresses for fluid sol_mf */
for (j=0;j<iel;j++)
{
   actsgnode = ele->g.gsurf->gnode[j];
...
/* the surface load of this element has been done, so swhich the neumann */
/* condition off */
ele->g.gsurf->neum=NULL;
/*----------------------------------------------------------------------*/
endsurface:
