    ...
    bool offdiag)
{
  ...
  switch (action)
  {
    ...
    case ACOU::interpolate_psi_to_node:
    {
      ...
      dyna_ = params.get<INPAR::ACOU::DynamicType>("dynamic type");
      if (dyna_ != INPAR::ACOU::acou_impleuler)  // explicit time integration does not need the
                                                 // scaling with c!
      {
        c = 1.0;
        rho = 1.0;
    ...
    case ACOU::calc_systemmat_and_residual:
    {
      ...
      if (!resonly) localSolver_->CondenseLocalPart(elemat1);
    ...
    case ACOU::update_secondary_solution_and_calc_residual:
    {
      ...
      zeroMatrix(elevec1);
      if (!allelesequal) localSolver_->ComputeMatrices(discretization, mat, *ele, dt, dyna_);

      // this happens for DIRK time integration and is disadvantageous, since history variables are
      // shaped according to elevec size in UpdateInteriorVariablesAndComputeResidual
    ...
    case ACOU::calc_ader_sysmat_and_residual:
    {
      ...
      double rho = actmat->Density(ele->Id());
      double c = actmat->SpeedofSound(ele->Id());
      localSolver_->ComputeADERResidual(
          ele, discretization, params, elevec1, interiorVelnp_, interiorPressnp_, c, rho);
    ...
    case ACOU::ader_postpro_gradanddiv:
    {
      ...
      for (int i = 0; i < onfdofs; ++i) traceVal_SDV(i) = traceVal_[i];
...
void DRT::ELEMENTS::AcouEleCalc<distype>::ReadGlobalVectors(DRT::Element* ele,
    DRT::Discretization& discretization, const std::vector<int>& lm, const bool padaptivity)
{
  ...
  if (trac_with_pml) interiorauxiliaryPML_ = acouele->eleinteriorAuxiliaryPML_;
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::FillRestartVectors(
    DRT::Element* ele, DRT::Discretization& discretization)
{
  ...
  if (trac_with_pml) size = shapes_->ndofs_ * (nsd_ + nsd_ + 1);
  ...
  for (unsigned int i = 0; i < interiorValnp.size(); ++i)
  {
    ...
    else
    {
      ...
      else if ((i + 1) % (nsd_ + nsd_ + 1) <= nsd_)
      {
        ...
        acouele->eleinteriorVelnp_(xyz * shapes_->ndofs_ + i / (nsd_ + nsd_ + 1)) =
            interiorValnp[i];
      }
      else
      {
        int xyz = i % (nsd_ + nsd_ + 1) - nsd_;  // 0 for x, 1 for y and 2 for z (for 3D)
        acouele->eleinteriorAuxiliaryPML_(xyz * shapes_->ndofs_ + i / (nsd_ + nsd_ + 1)) =
            interiorValnp[i];
      }
    }
  }
...
void DRT::ELEMENTS::AcouEleCalc<distype>::ComputeError(DRT::ELEMENTS::Acou* ele,
    Teuchos::ParameterList& params, Epetra_SerialDenseVector& elevec, double rho, double dt)
{
  ...
  for (int q = 0; q < highquad->NumPoints(); ++q)
  {
    ...
    numerical = 0.0;
    exact = 0.0;
    for (unsigned int i = 0; i < shapes_->ndofs_; ++i) numerical += values(i) * interiorPressnp_(i);
    ...
    Teuchos::ParameterList& params, Epetra_SerialDenseVector& elevec, double rho, double dt)
{
  ...
  for (int q = 0; q < highquad->NumPoints(); ++q)
  {
    ...
    for (int j = 0; j < ndofspost; ++j) h(0, j) += values(j) * jfac;
    ...
    for (int i = 1; i < ndofspost; ++i)
      for (unsigned int d = 0; d < nsd_; ++d) rhs(i) += ugrad[d] * derivs(d, i) * jfac;
  ...
  for (int q = 0; q < highquad->NumPoints(); ++q)
  {
    ...
    numerical_post = 0.0;
    exact = 0.0;
    for (int i = 0; i < ndofspost; ++i) numerical_post += values(i) * rhs(i);
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::ElementInit(
    DRT::ELEMENTS::Acou* ele, Teuchos::ParameterList& params)
{
  ...
  if (withpmls) ele->eleinteriorAuxiliaryPML_.Shape(shapes_->ndofs_ * nsd_, 1);

  ele->eleinteriorVelnp_.Shape(shapes_->ndofs_ * nsd_, 1);
  ele->eleinteriorPressnp_.Shape(shapes_->ndofs_, 1);

  if (dyna_ == INPAR::ACOU::acou_ader || dyna_ == INPAR::ACOU::acou_ader_lts ||
      dyna_ == INPAR::ACOU::acou_ader_tritet)
  {
    ele->eleADERimprovedGrad_.Shape(shapes_->ndofs_ * nsd_, 1);
    ele->eleADERimprovedDiv_.Shape(shapes_->ndofs_, 1);
  }

  if (params.get<bool>("padaptivity")) ele->elenodeTrace_.Shape(nen_, 1);
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
int DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ProjectField(DRT::ELEMENTS::Acou* ele,
    ...
    Epetra_SerialDenseVector& elevec2)
{
  ...
  dsassert(elevec2.M() == 0 || unsigned(elevec2.M()) == (nsd_ + 1) * shapes_.ndofs_,
  ...
  if (elevec2.M() > 0)
  {
    ...
    if (trac_with_pml) ele->eleinteriorAuxiliaryPML_.Scale(0.0);

    // projection according to Cockburn, formula 2.8 in "uniform-in-time superconvergence of the hdg
    // methods for the acosutic wave equation" */
    /*
    // need smaller polynomial space
    DRT::UTILS::PolynomialSpace<nsd_> poly_kmin1(distype, ele->Degree()-1,
    ele->UsesCompletePolynomialSpace()); unsigned int ndofskmin1 = poly_kmin1.Size();
    {
      DRT::UTILS::ShapeValuesFaceParams
    svfparams(ele->Faces()[0]->Degree(),shapes_.usescompletepoly_, 2 * ele->Faces()[0]->Degree());
    ...
    // fill system matrix and rhs vector with correspondent values (this quadrature should be
    enough) for (unsigned int q = 0; q < shapes_.nqpoints_; ++q)
    {
    ...
    {
      DRT::UTILS::ShapeValuesFaceParams
    svfparams(ele->Faces()[f]->Degree(),shapes_.usescompletepoly_, 2 * ele->Faces()[f]->Degree());
      shapesface_ = DRT::UTILS::ShapeValuesFaceCache<distype>::Instance().Create(svfparams);
    ...
    coordinates double p; double gradient[nsd_]; EvaluateAll(*start_func, xyz, p, gradient);
    ...
    coordinates double p; double gradient[nsd_]; EvaluateAll(*start_func, xyz, p, gradient);
        ...
        for (unsigned int i = 0; i < shapesface_->nfdofs_; ++i)
          for(unsigned int d=0; d<nsd_; ++d)
            rhs((nsd_+1)*ndofskmin1+i+f*shapesface_->nfdofs_) += shapesface_->jfac(q) * gradient[d]*
    shapesface_->normals(d,q) * shapesface_->shfunct(i,q);
      }
    }
    ...
    DRT::ELEMENTS::Acou* ele, Teuchos::ParameterList& params, Epetra_SerialDenseVector& elevec1)
{
  ...
  if (params.isParameter("faceconsider"))
  {
    ...
    for (unsigned int i = 0; i < shapesface_->nfdofs_; ++i) elevec1(i) = trVec(i);
  }
    ...
    DRT::ELEMENTS::Acou* ele, Teuchos::ParameterList& params, Epetra_SerialDenseVector& elevec2)
{
  ...
  if (meshconform)
  {
    ...
    Teuchos::RCP<std::vector<double>> nodevals =
        params.get<Teuchos::RCP<std::vector<double>>>("nodevals");
    ...
    {
      for (unsigned int j = 0; j < nsd_; ++j) lightxyz[i][j] = (*nodevals)[i * (nsd_ + 1) + j];
  ...
  else
  {
    ...
    Teuchos::RCP<const Epetra_Vector> matrix_state =
        params.get<Teuchos::RCP<Epetra_Vector>>("pressurenode");
    ...
    for (int i = 0; i < numlightnode; ++i)
    {
      for (unsigned int j = 0; j < nsd_; ++j) lightxyz[i][j] = shapes_.xyze(j, i);
      values[i] = nodevals[i];
    }
  }
  ...
  if (elevec2.M() > 0)
  {
    ...
    for (unsigned int q = 0; q < shapes_.nqpoints_; ++q)
    {
      ...
      for (unsigned int d = 0; d < nsd_; ++d)
        xyz[d] = shapes_.xyzreal(d, q); // coordinates of quadrature point in real coordinates
      double p = 0.0;
      EvaluateLight(lightxyz, values, numlightnode, xyz, p, absorptioncoeff); // p at quadrature
    point
    ...
    */
    ...
    {
      double xyz[nsd_];
      for (unsigned int d = 0; d < nsd_; ++d) xyz[d] = shapes_.nodexyzreal(d, r);
      double p = 0.0;
      EvaluateLight(
          lightxyz, values, numlightnode, xyz, p, absorptioncoeff);  // p at quadrature point
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::EvaluateLight(double lightxyz[][nsd_],
    double values[], int numnode, const double (&xyz)[nsd_], double& p,
    double absorptioncoeff) const
{
  ...
  if (distype == DRT::Element::quad4)
  {
    ...
    p = ( coeff_N(0,0) + coeff_N(1,0) * xyz[0] + coeff_N(2,0) * xyz[1] + coeff_N(3,0) * xyz[0] *
    xyz[1] ) * values[0]
      + ( coeff_N(0,1) + coeff_N(1,1) * xyz[0] + coeff_N(2,1) * xyz[1] + coeff_N(3,1) * xyz[0] *
    xyz[1] ) * values[1]
      + ( coeff_N(0,2) + coeff_N(1,2) * xyz[0] + coeff_N(2,2) * xyz[1] + coeff_N(3,2) * xyz[0] *
    xyz[1] ) * values[2]
      + ( coeff_N(0,3) + coeff_N(1,3) * xyz[0] + coeff_N(2,3) * xyz[1] + coeff_N(3,3) * xyz[0] *
    xyz[1] ) * values[3];
     */
  ...
  else if (distype == DRT::Element::hex8)
  {
    ...
    for (int i = 0; i < 8; ++i) coeff_N(i, i) = 1.0;

    {
      ...
      int err = inverseCoeff.Solve();
      if (err != 0)
        dserror("Inversion of matrix in light evaluation failed with errorcode %d", err);
    }

    p = (coeff_N(0, 0) + coeff_N(1, 0) * xyz[0] + coeff_N(2, 0) * xyz[1] + coeff_N(3, 0) * xyz[2] +
            coeff_N(4, 0) * xyz[0] * xyz[1] + coeff_N(5, 0) * xyz[0] * xyz[2] +
            coeff_N(6, 0) * xyz[1] * xyz[2] + coeff_N(7, 0) * xyz[0] * xyz[1] * xyz[2]) *
            values[0] +
        (coeff_N(0, 1) + coeff_N(1, 1) * xyz[0] + coeff_N(2, 1) * xyz[1] + coeff_N(3, 1) * xyz[2] +
            coeff_N(4, 1) * xyz[0] * xyz[1] + coeff_N(5, 1) * xyz[0] * xyz[2] +
            coeff_N(6, 1) * xyz[1] * xyz[2] + coeff_N(7, 1) * xyz[0] * xyz[1] * xyz[2]) *
            values[1] +
        (coeff_N(0, 2) + coeff_N(1, 2) * xyz[0] + coeff_N(2, 2) * xyz[1] + coeff_N(3, 2) * xyz[2] +
            coeff_N(4, 2) * xyz[0] * xyz[1] + coeff_N(5, 2) * xyz[0] * xyz[2] +
            coeff_N(6, 2) * xyz[1] * xyz[2] + coeff_N(7, 2) * xyz[0] * xyz[1] * xyz[2]) *
            values[2] +
        (coeff_N(0, 3) + coeff_N(1, 3) * xyz[0] + coeff_N(2, 3) * xyz[1] + coeff_N(3, 3) * xyz[2] +
            coeff_N(4, 3) * xyz[0] * xyz[1] + coeff_N(5, 3) * xyz[0] * xyz[2] +
            coeff_N(6, 3) * xyz[1] * xyz[2] + coeff_N(7, 3) * xyz[0] * xyz[1] * xyz[2]) *
            values[3] +
        (coeff_N(0, 4) + coeff_N(1, 4) * xyz[0] + coeff_N(2, 4) * xyz[1] + coeff_N(3, 4) * xyz[2] +
            coeff_N(4, 4) * xyz[0] * xyz[1] + coeff_N(5, 4) * xyz[0] * xyz[2] +
            coeff_N(6, 4) * xyz[1] * xyz[2] + coeff_N(7, 4) * xyz[0] * xyz[1] * xyz[2]) *
            values[4] +
        (coeff_N(0, 5) + coeff_N(1, 5) * xyz[0] + coeff_N(2, 5) * xyz[1] + coeff_N(3, 5) * xyz[2] +
            coeff_N(4, 5) * xyz[0] * xyz[1] + coeff_N(5, 5) * xyz[0] * xyz[2] +
            coeff_N(6, 5) * xyz[1] * xyz[2] + coeff_N(7, 5) * xyz[0] * xyz[1] * xyz[2]) *
            values[5] +
        (coeff_N(0, 6) + coeff_N(1, 6) * xyz[0] + coeff_N(2, 6) * xyz[1] + coeff_N(3, 6) * xyz[2] +
            coeff_N(4, 6) * xyz[0] * xyz[1] + coeff_N(5, 6) * xyz[0] * xyz[2] +
            coeff_N(6, 6) * xyz[1] * xyz[2] + coeff_N(7, 6) * xyz[0] * xyz[1] * xyz[2]) *
            values[6] +
        (coeff_N(0, 7) + coeff_N(1, 7) * xyz[0] + coeff_N(2, 7) * xyz[1] + coeff_N(3, 7) * xyz[2] +
            coeff_N(4, 7) * xyz[0] * xyz[1] + coeff_N(5, 7) * xyz[0] * xyz[2] +
            coeff_N(6, 7) * xyz[1] * xyz[2] + coeff_N(7, 7) * xyz[0] * xyz[1] * xyz[2]) *
            values[7];
    p *= -absorptioncoeff;
  }
  else
    ...
    const int start_func, const double (&xyz)[nsd_], double& p, double (&v)[nsd_]) const
{
  ...
  if (DRT::Problem::Instance()->Funct(start_func - 1).NumberComponents() == nsd_ + 1)
  {
    ...
    dserror(
        "Supply ONE component for your start function or NUMDIM+1, not anything else! The first "
        "component is for the pressure, the others for the velocity.");
  else
  {
...
void DRT::ELEMENTS::AcouEleCalc<distype>::NodeBasedValues(
    DRT::ELEMENTS::Acou* ele, Epetra_SerialDenseVector& elevec1, bool padaptivity)
{
  ...
  if ((!padaptivity && (traceVal_.size() > 0)) &&
      dyna_ ==
          INPAR::ACOU::acou_impleuler)  // (trace val size is zero for explicit time integration)
  {
    ...
    for (unsigned int face = 0; face < nfaces_; ++face)
    {
      ...
      localSolver_->shapesface_ =
          DRT::UTILS::ShapeValuesFaceCache<distype>::Instance().Create(svfparams);
      ...
      for (int i = 0; i < DRT::UTILS::DisTypeToNumNodePerFace<distype>::numNodePerFace; ++i)
      {
        ...
        localSolver_->shapesface_->polySpace_->Evaluate(
            localSolver_->shapesface_->xsi, fvalues);  // TODO: fix face orientation here
    ...
    for (unsigned int i = 0; i < nen_; ++i) elevec1((nsd_ + 1) * nen_ + i) /= touchcount(i);
  }
  else if (padaptivity)
    for (unsigned int i = 0; i < nen_; ++i) elevec1((nsd_ + 1) * nen_ + i) = ele->elenodeTrace_(i);
    ...
    Epetra_SerialDenseVector& elevec1, double dt, double fac)
{
  ...
  for (unsigned int i = 0; i < nen_; ++i)
  {
    ...
    // compute values for velocity and pressure by summing over all basis functions
    double sum = 0;
    for (unsigned int k = 0; k < shapes_->ndofs_; ++k) sum += values(k) * interiorPressnp_(k);
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
DRT::ELEMENTS::AcouEleCalc<distype>* DRT::ELEMENTS::AcouEleCalc<distype>::Instance(bool create)
{
  static AcouEleCalc<distype>* instance;
  if (create)
  {
    if (instance == NULL) instance = new AcouEleCalc<distype>();
  }
  else
  {
...
DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::LocalSolver(
    DRT::UTILS::ShapeValues<distype>& shapeValues, INPAR::ACOU::DynamicType& dyna)
    : ndofs_(shapeValues.ndofs_), shapes_(shapeValues), dyna_(dyna)
{
  // shape all matrices
    ...
    const DRT::ELEMENTS::Acou* ele, bool completepoly)
{
  ...
  {
    DRT::UTILS::PolynomialSpaceParams params(DRT::UTILS::DisTypeToFaceShapeType<distype>::shape,
        ele->Faces()[i]->Degree(), completepoly);
...
void DRT::ELEMENTS::AcouEleCalc<distype>::UpdateInteriorVariablesAndComputeResidual(
    Teuchos::ParameterList& params, DRT::ELEMENTS::Acou& ele,
    const Teuchos::RCP<MAT::Material>& mat, Epetra_SerialDenseVector& elevec, double dt,
    bool errormaps, bool updateonly)
{
  TEUCHOS_FUNC_TIME_MONITOR(
      "DRT::ELEMENTS::AcouEleCalc::UpdateInteriorVariablesAndComputeResidual");
  ...
  // now, we have to do the Schur complement thing, just as in "CondenseLocalPart" but without C and
  // E
  ...
  if (errormaps)
  {
    ...
    if (padaptivity)
    {
      ...
      double delta_k = 0.0;
      if (err_p != 0.0) delta_k = std::ceil(std::log10(err_p / padaptivitytol));
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::ProjectPadapField(DRT::ELEMENTS::Acou& ele, int newdeg)
{
  // create shape values for projection
  Teuchos::RCP<DRT::UTILS::ShapeValues<distype>> proj_shapes =
      Teuchos::rcp(new DRT::UTILS::ShapeValues<distype>(newdeg, usescompletepoly_, 2 * newdeg));
  ...
  Epetra_SerialDenseMatrix rectMat3(proj_shapes->ndofs_ * nsd_, shapes_->ndofs_ * nsd_);
  for (unsigned d = 0; d < nsd_; ++d)
    for (unsigned int j = 0; j < proj_shapes->ndofs_; ++j)
      for (unsigned int i = 0; i < shapes_->ndofs_; ++i)
        rectMat3(j + proj_shapes->ndofs_ * d, i + shapes_->ndofs_ * d) = rectMat2(j, i);
  ...
  // calculation of node based trace values, since this is no longer possible afterwards!!
  {
    ...
    for (unsigned int i = 0; i < nen_; ++i) ele.elenodeTrace_(i) /= touchcount(i);
  }
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::ComputePMonNodeVals(DRT::ELEMENTS::Acou* ele,
    Teuchos::ParameterList& params, Teuchos::RCP<MAT::Material>& mat, int face,
    Epetra_SerialDenseMatrix& elemat, Epetra_SerialDenseVector& elevec, int indexstart)
{
  localSolver_->shapesface_->EvaluateFace(*ele, face);
  ...
  {
    // evaluate shape polynomials in node
    for (unsigned int idim = 0; idim < nsd_ - 1; idim++) xsiFl(idim) = locations(idim, i);
    localSolver_->shapesface_->polySpace_->Evaluate(
        xsiFl, fvalues);  // TODO: fix face orientation here
 ...
 *----------------------------------------------------------------------*/
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ComputeSource(Teuchos::ParameterList& params,
    Epetra_SerialDenseVector& interiorSourcen, Epetra_SerialDenseVector& interiorSourcenp)
{
  ...
  if (DRT::Problem::Instance()->Funct(funcno).NumberComponents() > 1)
    dserror("for standard acou, the source term has to be scalar, i.e. only 1 component");
...
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ComputeAbsorbingBC(
    DRT::Discretization& discretization, DRT::ELEMENTS::Acou* ele, Teuchos::ParameterList& params,
...
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ComputeBoundaryIntegral(
    DRT::ELEMENTS::Acou* ele, Teuchos::ParameterList& params, int face)
{
  DRT::UTILS::ShapeValuesFaceParams svfparams(
  ...
  for (unsigned int i = 0; i < shapesface_->nqpoints_; ++i) boundaryint += shapesface_->jfac(i);
    ...
    double dt, double rho, double c)
{
  ...
  Mmat.Multiply('N', 'T', 1.0 / c / c / rho / dt, massPart, massPartW,
      0.0);  // Mmat.Multiply('N', 'T', 1.0 / c / c / dt, massPart, massPartW, 0.0);
  Dmat.Multiply(
      'N', 'T', rho / dt, massPart, massPartW, 0.0);  // only temporary for smaller inversion
  ...
  for (unsigned int j = 0; j < ndofs_; ++j)
    for (unsigned int i = 0; i < ndofs_; ++i)
    {
      for (unsigned int d = 0; d < nsd_; ++d)
      {
        Amat(d * ndofs_ + i, d * ndofs_ + j) =
            rho * rho * c * c * Mmat(i, j);  // rho * c * c * Mmat(i, j);
        Hmat(j, d * ndofs_ + i) =
            -1.0 * Bmat(d * ndofs_ + i, j);  // -rho * Bmat(d * ndofs_ + i, j);
      }
    }
...
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ComputeResidual(
    ...
    Epetra_SerialDenseVector& interiorVeln, Epetra_SerialDenseVector& interiorPressn)
{
  ...
  /*
   ...
   -1
                   +---------+    +----------------------------------------------------------+
                   |         |    |    n-1                     n-1          n-1              |
    n              | A    B  |    | A V     - (1 - theta) ( B P   + C Lambda    )            | n-1
   n-1           n-1 R  = - [ I  J ] |         |    |    n-1                     n-1       n-1 n-1
   | - (1 - theta) ( I V   + J P    + G Lambda    ) | H   D+M |    | M P     - (1 - theta) ( H V   +
   DmM P    + E Lambda    ) |
                   +---------+    +----------------------------------------------------------+
   */
  ...
  tempVec2.Multiply(
      'N', 'N', 1.0, tempMat2, tempVec1, 0.0);  // = w = ( D + M - H A^{-1} B )^{-1} ( ... )
...
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ComputeFaceMatrices(
    const int face, double dt, int indexstart, double c)
{
    ...
    if (shapesface_->shfunctI.NonzeroOnFace(q))
    {
      ...
      for (unsigned int p = 0; p < shapesface_->nfdofs_; ++p)
      {
        ...
        for (unsigned int i = 0; i < shapesface_->nqpoints_; ++i)
        {
          ...
          tempE += temp;
          for (unsigned int j = 0; j < nsd_; ++j) temp_d[j] += temp * shapesface_->normals(j, i);
        }
        for (unsigned int j = 0; j < nsd_; ++j)
        {
...
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::LocalSolver::ComputeADERResidual(DRT::ELEMENTS::Acou* ele,
    ...
    Epetra_SerialDenseVector& interiorPressn, double c, double rho)
{
  ...
  for (unsigned int q = 0; q < n_q_points; ++q)
  {
    ...
    for (unsigned int i = 0; i < shapes_.ndofs_; ++i)
    {
      for (unsigned int d = 0; d < nsd_; ++d)
      {
        ...
        for (unsigned int e = 0; e < nsd_; ++e)
          v_and_p_grad(e, d) +=
              shapes_.shderxy(i * nsd_ + d, q) * interiorVeln(i + e * shapes_.ndofs_);
      }
    }
    ...
    for (unsigned int i = 0; i < shapes_.ndofs_; ++i)
    {
      ...
      for (unsigned int d = 0; d < nsd_; ++d)
        postpressure_val[d] +=
            shapes_.shfunct(i, q) * ele->eleADERimprovedGrad_(i + d * shapes_.ndofs_);
    }


    // contribution from k=0
    for (unsigned int d = 0; d < nsd_; ++d) vcontrib[q][d] = dt * v_and_p[d];
    ...
    for (unsigned int d = 0; d < nsd_ + 1; ++d) tempcontrib[q][d] = postpressure_val(d);
  }
    ...
    Epetra_SerialDenseVector temp_p(shapes_.ndofs_);
    {
      for (unsigned int i = 0; i < shapes_.ndofs_; ++i)
      {
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          for (unsigned int d = 0; d < nsd_; ++d)
            temp_v(i + d * shapes_.ndofs_) +=
                shapes_.shfunct(i, q) * tempcontrib[q][d] * shapes_.jfac(q);
        }
      }
      for (unsigned int s = 0; s < tempcontrib.size(); ++s)
        for (unsigned int e = 0; e < nsd_ + 1; ++e) tempcontrib[s][e] = 0.0;
...
template <DRT::Element::DiscretizationType distype>
void DRT::ELEMENTS::AcouEleCalc<distype>::UpdateADERSolution(DRT::Discretization& discretization,
    Teuchos::ParameterList& params, DRT::ELEMENTS::Acou* ele,
    const Teuchos::RCP<MAT::Material>& mat, Epetra_SerialDenseVector& elevec, double dt)
{
  // we will need the inverse of M
