...
/*----------------------------------------------------------------------*/
ACOU::PatImageReconstruction::PatImageReconstruction(Teuchos::RCP<DRT::Discretization> scatradis,
    Teuchos::RCP<DRT::DiscretizationHDG> acoudis, Teuchos::RCP<Teuchos::ParameterList> scatrapara,
    Teuchos::RCP<Teuchos::ParameterList> acoupara, Teuchos::RCP<LINALG::Solver> scatrasolv,
    Teuchos::RCP<LINALG::Solver> acousolv, Teuchos::RCP<IO::DiscretizationWriter> scatraout,
    ...
    : scatra_discret_(scatradis),
      ...
      timereversal_(DRT::INPUT::IntegralValue<bool>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "TIMEREVERSAL")),
      patchtype_(DRT::INPUT::IntegralValue<INPAR::ACOU::PatchType>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "PATCHTYPE")),
      fdcheck_(DRT::INPUT::IntegralValue<bool>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), ("FDCHECK"))),
      J_(0.0),
      J_start_(0.0),
      error_(0.0),
      error_start_(0.0),
      overwrite_output_(DRT::INPUT::IntegralValue<bool>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), ("OVERWRITEOUTPUT"))),
      write_baci_acou_output_(DRT::INPUT::IntegralValue<bool>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), ("ACOUOUTPUT"))),
      illusetup_(0),
      currentsequenzeiter_(0)
{
  ...
  reac_searchdirection_ =
      Teuchos::rcp(new PATSearchDirection(DRT::INPUT::IntegralValue<INPAR::ACOU::OptimizationType>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "OPTIMIZATION")));
  ...
  std::string word2;
  std::istringstream pstream(Teuchos::getNumericStringParameter(
      acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "OPTIPARAMLIST"));
  char* pEnd;
  while (pstream >> word2)
  {
    int id = std::strtol(&word2[0], &pEnd, 10);
    if (*pEnd == '\0') opti_matids_.push_back(id);
  }
  ...
  for (int e = 0; e < scatra_discret_->NumMyRowElements(); ++e)
  {
    ...
    int elematid =
        sca_ele->Material()->Parameter()->Id();  // check if element has material that is optimized
  ...
  if (DRT::INPUT::IntegralValue<INPAR::ACOU::RegulaType>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "REGULATYPE") !=
      INPAR::ACOU::pat_regula_none)
    reac_regula_ = Teuchos::rcp(
        new PATRegula(DRT::INPUT::IntegralValue<INPAR::ACOU::RegulaType>(
                          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "REGULATYPE"),
...
void ACOU::PatImageReconstruction::ReplaceParams(Teuchos::RCP<Epetra_Vector> params)
{
  ...
  Teuchos::RCP<Epetra_Vector> paramscol =
      Teuchos::rcp(new Epetra_Vector(*(scatra_discret_->ElementColMap()), false));
...
void ACOU::PatImageReconstruction::FDCheck()
{
  ...
  if (scatra_discret_->Comm().NumProc() > 1) dserror("FDCHECK only implemented for one processor");
  ...
  // reaction part
  {
    ...
    Teuchos::RCP<Epetra_Vector> perturb_reac_vals =
        Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap(), false));
    Teuchos::RCP<Epetra_Vector> reac_vals_before =
        Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap(), false));
    ...
    for (int i = 0; i < reac_vals_->MyLength(); ++i)
    {
      ...
      dp = (J_before - J_) / (p - pn);
      std::cout << "J_before - J_ " << J_before - J_ << " p-pn " << p - pn << " val " << dp
                << std::endl;
...
double ACOU::PatImageReconstruction::EvalulateObjectiveFunction()
{
  ...
  if (reac_regula_ != Teuchos::null) reac_regula_->Evaluate(reac_vals_, &J_);

  // output
  if (!myrank_)
    std::cout << "objective function value " << J_ << " error value " << error_
              << " regularization " << J_ - error_ << std::endl;
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstruction::CalculateGradDirNorm(
    const Epetra_Vector& bvector, const Epetra_Map& uniquemap, double* result)
{
...
{
  linesearch_->Init(J_, reac_objgrad_,
      reac_searchdirection_->ComputeDirection(reac_objgrad_, reac_vals_, iter_), reac_vals_,
      scatra_discret_->ElementRowMap());
...
void ACOU::PatImageReconstruction::Optimize()
{
  ...
  if (timereversal_) TimeReversalEstimate();
  ...
  do
  {
    if (!myrank_)
    {
      ...
      std::cout
          << "*********************************************************************************"
          << std::endl;
      std::cout << "iteration " << iter_ + 1 << " of maximal " << maxiter_ << " iterations "
                << std::endl;
      std::cout
          << "*********************************************************************************"
          << std::endl;
...
void ACOU::PatImageReconstruction::EvaluateReacGrad()
{
  ...
  for (int i = 0; i < scatra_discret_->NumMyRowElements(); i++)
  {
    ...
    actele->Evaluate(
        p, *scatra_discret_, la, elematrix1, elematrix2, elevector1, elevector2, elevector3);
  ...
  if (reac_regula_ != Teuchos::null) reac_regula_->EvaluateGradient(reac_vals_, reac_objgrad_);
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstruction::CheckNeighborsReacGrad(DRT::Element* actele, int owner,
    Teuchos::RCP<Epetra_Vector> setsids, double set, double reacval, double interval,
    Teuchos::RCP<Epetra_Vector> auxvals)
{
  ...
  if (owner == myrank_)
  {
    for (int n = 0; n < numnode; ++n) lactelenodeids[n] = actele->NodeIds()[n];
  }
  ...
  for (int n = 0; n < numnode; ++n)
  {
    ...
    if (scatra_discret_->HaveGlobalNode(gactelenodeids[n]))
    {
      ...
      for (int e = 0; e < node->NumElement(); ++e)
      {
        ...
        if (neighborele == NULL) dserror("hupsi");
        ...
        for (int a = 0; a < numnode; ++a)
          ...
          {
            if (gactelenodeids[a] == neighborele->NodeIds()[b]) share++;
          }
        ...
        else if (share == numnode / 2)  // neighbor element
        {
          ...
          if (scatra_discret_->ElementRowMap()->LID(neighborele->Id()) < 0) continue;
          if (setsids->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id())) <=
                  set &&
              setsids->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id())) >= 0)
            continue;

          // determine reaction coefficient
          double neighborreac =
              reac_objgrad_->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id()));
          if (abs(neighborreac - reacval) <= interval)
          {
            setsids->ReplaceMyValue(
                scatra_discret_->ElementRowMap()->LID(neighborele->Id()), 0, set);
            auxvals->ReplaceMyValue(scatra_discret_->ElementRowMap()->LID(neighborele->Id()), 0,
    ...
    if (toevaluate.size() != unsigned(size)) toevaluate.resize(size, 0);
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstruction::ConvertGradient(
    Teuchos::RCP<DRT::Discretization> discret, Teuchos::RCP<Epetra_Vector> gradient)
{
  double L = 8.0;
  ...
  if (0)
  {
    ...
    // to calculate the gradient with respect to the values scaling the basis function, we need to
    // scale each element gradient value with the value of the basis function at that point
    ...
    for (int i = 0; i < basis_size; ++i)
    {
      for (int j = 0; j < basis_size; ++j)
      {
        ...
        for (int e = 0; e < discret->NumMyRowElements(); ++e)
        {
          ...
          double N_ij_at_xy_cc =
              std::cos(double(i) * PI * xyz[0] / L) * std::cos(double(j) * PI * xyz[1] / L);
          double N_ij_at_xy_cs =
              std::cos(double(i) * PI * xyz[0] / L) * std::sin(double(j + 1) * PI * xyz[1] / L);
          double N_ij_at_xy_ss =
              std::sin(double(i + 1) * PI * xyz[0] / L) * std::sin(double(j + 1) * PI * xyz[1] / L);
    ...
    for (int e = 0; e < discret->NumMyRowElements(); ++e)
    {
      ...
      for (int i = 0; i < basis_size; ++i)
      {
        for (int j = 0; j < basis_size; ++j)
        {
          ...
          new_grad_ele +=
              N_ij_at_xy_cc * newgrad[i * basis_size + j] +
      ...
      for (int j = 0; j < basis_size; ++j)
      {
        ...
        for (int e = 0; e < discret->NumMyRowElements(); ++e)
        {
          ...
          for (int k = 0; k < basis_size; ++k)
          {
            if (k != i)
              N_i_at_x *=
                  (xyz[0] - (-L / 2.0 + k * L / (basis_size - 1))) /
                  (-L / 2.0 + i * L / (basis_size - 1) - (-L / 2.0 + k * L / (basis_size - 1)));
            if (k != j)
              N_j_at_y *=
                  (xyz[1] - (-L / 2.0 + k * L / (basis_size - 1))) /
                  (L / 2.0 + j * L / (basis_size - 1) - (L / 2.0 + k * L / (basis_size - 1)));
          }
...
ACOU::PatImageReconstructionOptiSplit::PatImageReconstructionOptiSplit(
    Teuchos::RCP<DRT::Discretization> scatradis, Teuchos::RCP<DRT::DiscretizationHDG> acoudis,
    Teuchos::RCP<Teuchos::ParameterList> scatrapara, Teuchos::RCP<Teuchos::ParameterList> acoupara,
    Teuchos::RCP<LINALG::Solver> scatrasolv, Teuchos::RCP<LINALG::Solver> acousolv,
    Teuchos::RCP<IO::DiscretizationWriter> scatraout,
    Teuchos::RCP<IO::DiscretizationWriter> acouout)
    : PatImageReconstruction(
          scatradis, acoudis, scatrapara, acoupara, scatrasolv, acousolv, scatraout, acouout),
      sequenzeiter_(acouparams_->sublist("PA IMAGE RECONSTRUCTION").get<int>("SEQUENZE"))
{
  ...
  if (DRT::INPUT::IntegralValue<INPAR::ACOU::RegulaType>(
    ...
    diff_regula_ = Teuchos::rcp(new PATRegula(
        DRT::INPUT::IntegralValue<INPAR::ACOU::RegulaType>(
            acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "REGULATYPE"),
        ...
        acouparams_->sublist("PA IMAGE RECONSTRUCTION").get<double>("TVDEPS_D"), scatra_discret_));
...
void ACOU::PatImageReconstructionOptiSplit::ReplaceParams(Teuchos::RCP<Epetra_Vector> params)
{
  ...
  // update node based vector
  if (reacordifforcorrho_ == 0) ComputeNodeBasedReactionCoefficient();

  return;
}

/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplit::SetRestartParameters(Teuchos::RCP<Epetra_Vector> reacs,
    Teuchos::RCP<Epetra_Vector> diffs, Teuchos::RCP<Epetra_Vector> cs,
    Teuchos::RCP<Epetra_Vector> rhos)
{
...
double ACOU::PatImageReconstructionOptiSplit::EvalulateObjectiveFunction()
{
  ...
  if (reac_regula_ != Teuchos::null) reac_regula_->Evaluate(reac_vals_, &J_);
  if (diff_regula_ != Teuchos::null) diff_regula_->Evaluate(diff_vals_, &J_);
...
void ACOU::PatImageReconstructionOptiSplit::EvaluateDiffGrad()
{
  ...
  if (diff_regula_ != Teuchos::null) diff_regula_->EvaluateGradient(diff_vals_, diff_objgrad_);
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplit::CheckNeighborsDiffGrad(DRT::Element* actele, int owner,
    Teuchos::RCP<Epetra_Vector> setsids, double set, double diffval, double interval,
    Teuchos::RCP<Epetra_Vector> auxvals)
{
  int numnode = 4 * (DRT::Problem::Instance()->NDim() - 1);
    ...
    if (scatra_discret_->HaveGlobalNode(gactelenodeids[n]))
    {
      ...
      for (int e = 0; e < node->NumElement(); ++e)
      {
        ...
        else if (share == numnode / 2)  // neighbor element
        {
          ...
          if (setsids->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id())) <=
                  set &&
              setsids->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id())) >= 0.0)
            continue;

          // determine reaction coefficient
          double neighbordiff =
              diff_objgrad_->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id()));
...
bool ACOU::PatImageReconstructionOptiSplit::PerformIteration()
{
  ...
  if (iter_ == 0) reacseq *= 5;
  for (int i = 0; i < reacseq; ++i)
  {
    if (!myrank_) std::cout << "ITERATION " << i << std::endl;
    linesearch_->Init(J_, reac_objgrad_,
        reac_searchdirection_->ComputeDirection(reac_objgrad_, reac_vals_, iter_), reac_vals_,
        scatra_discret_->ElementRowMap());
  ...
  {
    if (!myrank_) std::cout << "ITERATION " << i << std::endl;
    linesearch_->Init(J_, diff_objgrad_,
        diff_searchdirection_->ComputeDirection(diff_objgrad_, diff_vals_, iter_), diff_vals_,
        scatra_discret_->ElementRowMap());
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplit::CalculateGradDirNorm(
    const Epetra_Vector& bvector, const Epetra_Map& uniquemap, double* result)
{
  if (reacordifforcorrho_ == 0)
    reac_objgrad_->Dot(bvector, result);
...
void ACOU::PatImageReconstructionOptiSplit::FDCheck()
{
  ...
  // diffusion part
  {
    ...
    Teuchos::RCP<Epetra_Vector> perturb_diff_vals =
        Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap(), false));
    Teuchos::RCP<Epetra_Vector> diff_vals_before =
        Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap(), false));
          ...
          scatradis, acoudis, scatrapara, acoupara, scatrasolv, acousolv, scatraout, acouout)
{
  ...
  std::string word2;
  std::istringstream pstream(Teuchos::getNumericStringParameter(
      acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "ACOUPARAMLIST"));
  char* pEnd;
  while (pstream >> word2)
  {
    int id = std::strtol(&word2[0], &pEnd, 10);
    if (*pEnd == '\0') acou_matids_.push_back(id);
  }
  ...
  for (int e = 0; e < acou_discret_->NumMyRowElements(); ++e)
  {
    ...
    int elematid =
        acou_ele->Material()->Parameter()->Id();  // check if element has material that is optimized
      ...
      INPAR::ACOU::pat_regula_none)
  {
    c_regula_ = Teuchos::rcp(new PATRegula(
        ...
        acouparams_->sublist("PA IMAGE RECONSTRUCTION").get<double>("TVDEPS_C"), acou_discret_));
    rho_regula_ = Teuchos::rcp(new PATRegula(
        ...
        acouparams_->sublist("PA IMAGE RECONSTRUCTION").get<double>("TVDEPS_RHO"), acou_discret_));
  }
  ...
  if (0)
  {
    ...
    for (int i = scatra_discret_->ElementRowMap()->MinAllGID();
         i <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++i)
    {
      // local id
      ...
      std::vector<double> xyz =
          DRT::UTILS::ElementCenterRefeCoords(scatra_discret_->lRowElement(lid));
  ...
  if (0)
  {
    ...
    */
    ...
    Teuchos::RCP<Epetra_Vector> c_tatparams =
        Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), true));
    Teuchos::RCP<Epetra_Vector> rho_tatparams =
        Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), true));
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::ReplaceParams(
    Teuchos::RCP<Epetra_Vector> params)
{
  if (reacordifforcorrho_ == 0)
    for (int i = 0; i < params->MyLength(); ++i)
    {
  ...
  {
    Teuchos::RCP<Epetra_Vector> paramscol =
        Teuchos::rcp(new Epetra_Vector(*(acou_discret_->ElementColMap()), false));
    LINALG::Export(*params, *paramscol);
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::SetRestartParameters(
    Teuchos::RCP<Epetra_Vector> reacs, Teuchos::RCP<Epetra_Vector> diffs,
    Teuchos::RCP<Epetra_Vector> cs, Teuchos::RCP<Epetra_Vector> rhos)
{
  reacordifforcorrho_ = 0;
  ReplaceParams(reacs);
...
double ACOU::PatImageReconstructionOptiSplitAcouSplit::EvalulateObjectiveFunction()
{
  ...
  if (c_regula_ != Teuchos::null) c_regula_->Evaluate(c_vals_, &J_);
  if (rho_regula_ != Teuchos::null) rho_regula_->Evaluate(rho_vals_, &J_);
...
void ACOU::PatImageReconstructionOptiSplitAcouSplit::ReduceBasis()
{
  ...
  if (patchtype_ == INPAR::ACOU::pat_patch_none) return;
  ...
  Teuchos::RCP<Epetra_Vector> setids =
      Teuchos::rcp(new Epetra_Vector(*(scatra_discret_->ElementRowMap()), false));
  if (patchtype_ != INPAR::ACOU::pat_patch_mixed)
  {
    ...
    int loc_owner = -1;
    if (minid == global_minid) loc_owner = myrank_;
    int owner = -1;
    ...
    while (minvalsetids < 0.0)
    {
      ...
      if (global_minid < 0)  // it could not find another value which is small enough - safe failure
                             // - all remaining elements are done
      {
        for (int e = 0; e < scatra_discret_->NumMyRowElements(); ++e)
        {
    ...
    if (!myrank_)
      std::cout << "identified " << i << " sets using the reaction gradient for the reaction basis"
                << std::endl;
    ...
    for (int j = 0; j < i; ++j)  // i holds the number of sets right now
    {
      ...
      if (gnumsetval != 0.0) gsetvalreac /= gnumsetval;
      ...
      for (int g = 0; g < reac_objgrad_->MyLength(); ++g)
      {
        ...
        {
          int set = setids->operator[](g);
          if (set == j) reac_objgrad_->ReplaceMyValue(g, 0, gsetvalreac);
        }
      }
  ...
  // if(0)
  {
    ...
    {
      // this is the easy case, because all other gradient just use the same patch as the absorption
      // coefficient but this makes only sense for sequenze_=1
    ...
    {
      // diffusion coefficient with reaction gradient basis and speed of sound and mass density with
      // reaction values
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::ReducedGradientsCalculation(
    Teuchos::RCP<Epetra_Vector> setids, bool acouonly)
{
  // create a vector to store the set ids
  Teuchos::RCP<Epetra_Vector> setidsacou =
      Teuchos::rcp(new Epetra_Vector(*(acou_discret_->ElementRowMap()), true));
       ...
       i <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++i)
  {
    ...
    int agid = i - scatra_discret_->ElementRowMap()->MinAllGID() +
               acou_discret_->ElementRowMap()->MinAllGID();
    int alid = acou_discret_->ElementRowMap()->LID(agid);
    if (alid >= 0) setidsacou->ReplaceMyValue(alid, 0, setval);
  }
  ...
  for (int j = 0; j <= int(doublemaxset); ++j)  // i holds the number of sets right now
  {
    ...
    for (int g = 0; g < c_objgrad_->MyLength(); ++g)
      if (acou_opt_ind_->operator[](g) !=
          0.0)  // only for those who play a role (not the water region)
      {
    ...
    if (gnumsetvaldiff != 0.0) gsetvaldiff /= gnumsetvaldiff;
    ...
    if (acouonly == false)
      for (int g = 0; g < diff_objgrad_->MyLength(); ++g)
        ...
        {
          int set = setids->operator[](g);
          if (set == j) diff_objgrad_->ReplaceMyValue(g, 0, gsetvaldiff);
        }
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::ReducePatchSelf(
    Teuchos::RCP<DRT::Discretization> discret, Teuchos::RCP<Epetra_Vector> patchvec,
    Teuchos::RCP<Epetra_Vector> opt_ind)
{
  // for reduced basis, the diffusion coefficient has to build patches according to the absorption
  ...
  double rangeval = std::abs(maxval - minval);
  if (rangeval == 0.0)
    if (!myrank_) dserror("rangeval is zero");

  // create a helper vector
  Teuchos::RCP<Epetra_Vector> auxvals =
      Teuchos::rcp(new Epetra_Vector(*(discret->ElementRowMap()), false));
  ...
  for (int e = 0; e < discret->NumMyRowElements(); ++e)
  {
    if (patchvec->operator[](e) <= minval + 1.0e-10 && opt_ind->operator[](e) != 0.0) minid = e;
  }
  int global_minid = -1;
  ...
  while (minvalsetids < 0.0)
  {
    ...
    CheckNeighborsPatchSelf(
        discret, patchvec, actele, owner, setids, set, minval, 2.0 / 3.0 * rangeval, auxvals);
    ...
    for (int e = 0; e < discret->NumMyRowElements(); ++e)
    {
      if (auxvals->operator[](e) <= minval + 1.0e-6 && opt_ind->operator[](e) != 0.0) minid = e;
    }
    global_minid = -1;
    discret->Comm().MaxAll(&minid, &global_minid, 1);
    loc_owner = -1;
  ...
  for (int j = 0; j < i; ++j)  // i holds the number of sets right now
  {
    ...
    if (gnumsetval != 0.0) gsetval /= gnumsetval;
    ...
    for (int g = 0; g < patchvec->MyLength(); ++g)
    {
      ...
      {
        int set = setids->operator[](g);
        if (set == j) patchvec->ReplaceMyValue(g, 0, gsetval);
      }
    }
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::CheckNeighborsPatchSelf(
    Teuchos::RCP<DRT::Discretization> discret, Teuchos::RCP<Epetra_Vector> patchvec,
    DRT::Element* actele, int owner, Teuchos::RCP<Epetra_Vector> setsids, double set, double val,
    double interval, Teuchos::RCP<Epetra_Vector> auxvals)
{
  int numnode = 4 * (DRT::Problem::Instance()->NDim() - 1);
  ...
  for (int n = 0; n < numnode; ++n)
  {
    ...
    if (discret->HaveGlobalNode(gactelenodeids[n]))
    {
      ...
      for (int e = 0; e < node->NumElement(); ++e)
      {
        ...
        {
          // if already evaluated, skip
          if (discret->ElementRowMap()->LID(neighborele->Id()) < 0) continue;
          if (setsids->operator[](discret->ElementRowMap()->LID(neighborele->Id())) <= set &&
              setsids->operator[](discret->ElementRowMap()->LID(neighborele->Id())) >= 0.0)
            continue;

          // determine reaction coefficient
          double neighborval =
              patchvec->operator[](discret->ElementRowMap()->LID(neighborele->Id()));
          if (abs(neighborval - val) <= interval)
          {
            setsids->ReplaceMyValue(discret->ElementRowMap()->LID(neighborele->Id()), 0, set);
            auxvals->ReplaceMyValue(discret->ElementRowMap()->LID(neighborele->Id()), 0,
                std::numeric_limits<double>::max());
    ...
    for (int s = 0; s < size; ++s)
    {
      ...
      CheckNeighborsPatchSelf(
          discret, patchvec, neighborele, nbowner, setsids, set, val, interval, auxvals);
    }
  }
  return;
}
...
void ACOU::PatImageReconstructionOptiSplitAcouSplit::ReducePatchReacVals()
{
  ...
  int loc_owner = -1;
  if (maxid == global_maxid) loc_owner = myrank_;
  int owner = -1;
  scatra_discret_->Comm().MaxAll(&loc_owner, &owner, 1);
  ...
  if (!myrank_) std::cout << "identified " << i << " sets using the reaction values" << std::endl;
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::CheckNeighborsPatchReacVals(
    DRT::Element* actele, int owner, Teuchos::RCP<Epetra_Vector> setsids, double set,
    double reacval, double interval, Teuchos::RCP<Epetra_Vector> auxvals)
{
  int numnode = 4 * (DRT::Problem::Instance()->NDim() - 1);
    ...
    if (scatra_discret_->HaveGlobalNode(gactelenodeids[n]))
    {
      ...
      for (int e = 0; e < node->NumElement(); ++e)
      {
        ...
        else if (share == numnode / 2)  // neighbor element
        {
          ...
          // determine reaction coefficient
          double neighborreac =
              reac_vals_->operator[](scatra_discret_->ElementRowMap()->LID(neighborele->Id()));
          if (abs(neighborreac - reacval) <= interval)
          {
            setsids->ReplaceMyValue(
                scatra_discret_->ElementRowMap()->LID(neighborele->Id()), 0, set);
...
void ACOU::PatImageReconstructionOptiSplitAcouSplit::EvaluateCGrad()
{
  ...
  for (int i = 0; i < acou_discret_->NumMyColElements(); i++)
  {
    ...
    if (acou_discret_->ElementRowMap()->LID(acou_discret_->ElementColMap()->GID(i)) >= 0)
      c_objgrad_->ReplaceMyValue(
          acou_discret_->ElementRowMap()->LID(acou_discret_->ElementColMap()->GID(i)), 0, val);
  }
  ...
  // regularization
  if (c_regula_ != Teuchos::null) c_regula_->EvaluateGradient(c_vals_, c_objgrad_);
...
void ACOU::PatImageReconstructionOptiSplitAcouSplit::EvaluateRhoGrad()
{
  ...
  // regularization
  if (rho_regula_ != Teuchos::null) rho_regula_->EvaluateGradient(rho_vals_, rho_objgrad_);
...
bool ACOU::PatImageReconstructionOptiSplitAcouSplit::PerformIteration()
{
  ...
  //  Teuchos::RCP<Epetra_Vector> accumulated_c_grad = Teuchos::rcp(new
  //  Epetra_Vector(*acou_discret_->ElementRowMap(),true)); Teuchos::RCP<Epetra_Vector>
  //  accumulated_rho_grad = Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(),true));
  //
  ...
  for (int i = 0; i < reacseq; ++i)
  {
    ...
    illusetup_ = i;
    acouparams_->set<Teuchos::RCP<PATMonitorManager>>(
        "monitormanager", monitor_manager_[0]);  //[i]);
    if (!myrank_) std::cout << "REACTION ITERATION " << i << std::endl;
    linesearch_->Init(J_, reac_objgrad_,
        reac_searchdirection_->ComputeDirection(reac_objgrad_, reac_vals_, iter_), reac_vals_,
        scatra_discret_->ElementRowMap());
    reacsucc = linesearch_->Run();
  ...
  for (int i = 0; i < cseq; ++i)
  {
    ...
    if (!myrank_) std::cout << "SOUND SPEED ITERATION " << i << " OF " << cseq << std::endl;
    linesearch_->Init(J_, c_objgrad_,
        c_searchdirection_->ComputeDirection(c_objgrad_, c_vals_, iter_), c_vals_,
        acou_discret_->ElementRowMap());
  ...
  for (int i = 0; i < rhoseq; ++i)
  {
    ...
    if (!myrank_) std::cout << "DENSITY ITERATION " << i << " OF " << rhoseq << std::endl;
    linesearch_->Init(J_, rho_objgrad_,
        rho_searchdirection_->ComputeDirection(rho_objgrad_, rho_vals_, iter_), rho_vals_,
        acou_discret_->ElementRowMap());
    bool newsucc = linesearch_->Run();
  ...
  {
    if (!myrank_) std::cout << "DIFFUSION ITERATION " << i << std::endl;
    linesearch_->Init(J_, diff_objgrad_,
        diff_searchdirection_->ComputeDirection(diff_objgrad_, diff_vals_, iter_), diff_vals_,
        scatra_discret_->ElementRowMap());
    diffsucc = linesearch_->Run();
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouSplit::CalculateGradDirNorm(
    const Epetra_Vector& bvector, const Epetra_Map& uniquemap, double* result)
{
  if (reacordifforcorrho_ == 0)
    reac_objgrad_->Dot(bvector, result);
  ...
  */
  ...
  for (int s = 0; s <= smax + 2; ++s)
    ...
    // for(int r=0; r<=rmax; ++r)
    {
      ...
      std::cout << "run r " << r << " run s " << s << " ratiocorrect r " << ratiocorrect_r
                << " ratiocorrect s " << ratiocorrect_s << std::endl;
...
void ACOU::PatImageReconstructionOptiSplitAcouSplit::FDCheck()
{
  ...
  // sos part
  {
    ...
    Teuchos::RCP<Epetra_Vector> perturb_c_vals =
        Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), false));
    Teuchos::RCP<Epetra_Vector> c_vals_before =
        Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), false));
  ...
  // density part
  {
    ...
    Teuchos::RCP<Epetra_Vector> perturb_rho_vals =
        Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), false));
    Teuchos::RCP<Epetra_Vector> rho_vals_before =
        Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), false));
...
void ACOU::PatImageReconstructionOptiSplitAcouSplit::ComputeParameterError()
{
  ...
  if (acou_discret_->Comm().NumProc() > 1)
  {
    if (!myrank_)
      std::cout
          << "Function ComputeParameterError() skipped! Not yet implemented for parallel usage";
    return;
  }
       ...
       i <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++i)
  {
    ...
    if (xyz[0] < 3.5 && xyz[0] > -3.5 && xyz[1] < 0.75 &&
        xyz[1] > -0.75)  // check if the center of this element is in the rectangular region
    {
      reac_val = 0.1;
      D_val = 0.5;
      c_val = 2.0;
    ...
    if (xyz[0] < 3.5 + h_halbe && xyz[0] > 3.5 - h_halbe && xyz[0] > -3.5 - h_halbe &&
        xyz[0] < -3.5 + h_halbe && xyz[1] < 0.75 + h_halbe && xyz[1] > 0.75 - h_halbe &&
        xyz[1] > -0.75 - h_halbe && xyz[1] < -0.75 + h_halbe)
    {
      skip++;
    }
    else if (std::sqrt(xyz[0] * xyz[0] + (xyz[1] - 3.0) * (xyz[1] - 3.0)) < 1.0 + h_halbe &&
             std::sqrt(xyz[0] * xyz[0] + (xyz[1] - 3.0) * (xyz[1] - 3.0)) > 1.0 - h_halbe)
    {
      skip++;
    }
    else
    {
      reac_error +=
          (reac_vals_->operator[](lid) - reac_val) * (reac_vals_->operator[](lid) - reac_val);
  ...
  DRT::UTILS::ElementCenterRefeCoords(scatra_discret_->lRowElement(lid));
    ...
    double g1 =
  (1.90184747127+0.49231421291)/(3.032389891147+3.54545535887)*(xyz[0]+3.54545535887)-0.49231421291;
    double g2 =
  (1.90184747127+0.49231421291)/(3.032389891147+3.54545535887)*(xyz[0]+3.0322954880291)-1.90173750140464;
    double g3 =
  (-1.90173750140464+0.49231421291)/(-3.0322954880291+3.54545535887)*(xyz[0]+3.54545535887)-0.49231421291;
    double g4 =
  (-1.90173750140464+0.49231421291)/(-3.0322954880291+3.54545535887)*(xyz[0]-3.03238981147)+1.90184747127;
...
*/
  std::cout << std::endl
            << "AFTER " << skip << " SKIPS OF "
            << scatra_discret_->ElementRowMap()->MaxAllGID() -
                   scatra_discret_->ElementRowMap()->MinAllGID() + 1
            << " ERRORS(order:muDcrho): " << reac_error << " " << diff_error << " " << c_error
            << " " << rho_error << std::endl
            << std::endl;
...
ACOU::PatImageReconstructionOptiSplitAcouIdent::PatImageReconstructionOptiSplitAcouIdent(
    ...
    : PatImageReconstructionOptiSplit(
          scatradis, acoudis, scatrapara, acoupara, scatrasolv, acousolv, scatraout, acouout),
      sequenzeiter_(acouparams_->sublist("PA IMAGE RECONSTRUCTION").get<int>("SEQUENZE")),
      acouident_avg_(DRT::INPUT::IntegralValue<bool>(
          acouparams_->sublist("PA IMAGE RECONSTRUCTION"), "ACOUIDENT_AVG"))
{
  c_vals_ = Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), false));
  rho_vals_ = Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), false));
  ...
  // read materials
  ReadMaterials(
      acouparams_->sublist("PA IMAGE RECONSTRUCTION").get<std::string>("SEGMENTATIONMATS"));
}
          ...
          scatradis, acoudis, scatrapara, acoupara, scatrasolv, acousolv, scatraout, acouout)
{
  ...
  acou_discret_->Comm().Barrier();
  acou_discret_->Comm().Barrier();
  if (!myrank_) std::cout << "constructor finished" << std::endl;
  ...
  acou_discret_->Comm().Barrier();
  acou_discret_->Comm().Barrier();
  if (!myrank_) std::cout << "Read materials finished" << std::endl;
}
...
void ACOU::PatImageReconstructionOptiSplitAcouIdentSmart::UpdateAcousticalParameters()
{
  ...
  Teuchos::RCP<Epetra_Vector> c_p =
      Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), true));
  Teuchos::RCP<Epetra_Vector> rho_p =
      Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap(), true));
  ...
  for (int i = 0; i < c_objgrad_->MyLength(); ++i)
  {
    ...
    if ((c_objgrad_->operator[](i)) < 0.0)  // increase
      current_c_step_length =
          std::abs((max_all_c - (c_vals_->operator[](i))) / (c_objgrad_->operator[](i)));
    else if ((c_objgrad_->operator[](i)) > 0.0)  // decrease
      current_c_step_length =
          std::abs((min_all_c - (c_vals_->operator[](i))) / (c_objgrad_->operator[](i)));
    ...
    if ((rho_objgrad_->operator[](i)) < 0.0)  // increase
      current_rho_step_length =
          std::abs((max_all_rho - (rho_vals_->operator[](i))) / (rho_objgrad_->operator[](i)));
    else if ((rho_objgrad_->operator[](i)) > 0.0)  // decrease
      current_rho_step_length =
          std::abs((min_all_rho - (rho_vals_->operator[](i))) / (rho_objgrad_->operator[](i)));
  ...
  // std::cout<<"minimal step length for c "<<glob_c_step_length<<" and rho
  // "<<glob_rho_step_length<<std::endl;
       ...
       i <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++i)
  {
    ...
    {
      if (reac >= materialtable_[m][0] && reac <= materialtable_[m][1]) possiblemats.push_back(m);
    }
    ...
    {
      c_p->ReplaceMyValue(
          alid, 0, (materialtable_[possiblemats[0]][4] + materialtable_[possiblemats[0]][5]) / 2.0);
      rho_p->ReplaceMyValue(
          alid, 0, (materialtable_[possiblemats[0]][6] + materialtable_[possiblemats[0]][7]) / 2.0);
      numelesofmaterial[possiblemats[0]]++;
      // std::cout<<"case 1: element "<<i<<" with reaction coefficient "<<reac<<" is identified to
      // be of material "<<possiblemats[0]<<std::endl;
    }
    else  // several materials are in question. use acoustical gradients to determine which fits
          // best
    {
      ...
      {
        double c_possib =
            (materialtable_[possiblemats[m]][4] + materialtable_[possiblemats[m]][5]) / 2.0;
        double rho_possib =
            (materialtable_[possiblemats[m]][6] + materialtable_[possiblemats[m]][7]) / 2.0;
      ...
      int bestfit = -1;
      if (materials_with_correct_direction.size() ==
          0)  // directions are weird, determine from possiblemats with distance
      {
        ...
        for (unsigned int m = 0; m < possiblemats.size(); ++m)
        {
          ...
          double c_update =
              (c_vals_->operator[](alid)) - glob_c_step_length * (c_objgrad_->operator[](alid));
          double rho_update = (rho_vals_->operator[](alid)) -
                              glob_rho_step_length * (rho_objgrad_->operator[](alid));

          double distance = sqrt((c_possib - c_update) * (c_possib - c_update) +
                                 (rho_possib - rho_update) * (rho_possib - rho_update));

          // std::cout<<"case 2: reac "<<reac<<" m in question "<<possiblemats[m]<<" c_objgrad
          // "<<(c_objgrad_->operator [](alid))<<" c_update "<<c_update<<" c_possib "<<c_possib<<"
          // distance "<<distance<<std::endl; std::cout<<"case 2: reac "<<reac<<" m in question
          // "<<possiblemats[m]<<" r_objgrad "<<(rho_objgrad_->operator [](alid))<<" r_update
          // "<<rho_update<<" r_possib "<<rho_possib<<" distance "<<distance<<std::endl;
        ...
        if (bestfit < 0) dserror("material identification failed in case no direction applies");
      }
      else if (materials_with_correct_direction.size() ==
               1)  // take the one material with correct direction
      {
        // std::cout<<"case 3: one material with correct direction is set, namely
        // "<<materials_with_correct_direction[0]<<std::endl;
      ...
      else  // take the material with correct direction and smallest distance
      {
        ...
        for (unsigned int m = 0; m < possiblemats.size(); ++m)
        {
          ...
          // std::cout<<"case 4: reac "<<reac<<" m in question "<<possiblemats[m]<<" c_objgrad
          // "<<(c_objgrad_->operator [](alid))<<" c_update "<<c_update<<" c_possib "<<c_possib<<"
          // distance "<<distance<<std::endl; std::cout<<"case 4: reac "<<reac<<" m in question
          // "<<possiblemats[m]<<" r_objgrad "<<(rho_objgrad_->operator [](alid))<<" r_update
          // "<<rho_update<<" r_possib "<<rho_possib<<" distance "<<distance<<std::endl;
          // std::cout<<prev_distance<<std::endl;
          if (distance < prev_distance)
        ...
        if (bestfit < 0) dserror("material identification failed in case several directions apply");
      }
      ...
      rho_p->ReplaceMyValue(
          alid, 0, (materialtable_[bestfit][6] + materialtable_[bestfit][7]) / 2.0);
    }
  }

  std::vector<int> glo_numelesofmaterial(materialtable_.size(), 0);
  acou_discret_->Comm().SumAll(
      &numelesofmaterial[0], &glo_numelesofmaterial[0], materialtable_.size());
  ...
  Teuchos::RCP<Epetra_Vector> c_paramscol =
      Teuchos::rcp(new Epetra_Vector(*(acou_discret_->ElementColMap()), false));
  LINALG::Export(*c_p, *c_paramscol);
  Teuchos::RCP<Epetra_Vector> rho_paramscol =
      Teuchos::rcp(new Epetra_Vector(*(acou_discret_->ElementColMap()), false));
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouIdentSmart::ReplaceParams(
    Teuchos::RCP<Epetra_Vector> params)
{
  // correct for negative absorption coefficients
  for (int i = 0; i < params->MyLength(); ++i)
  {
    if (params->operator[](i) < 0.0) params->ReplaceMyValue(i, 0, 0.0);
  }
  ...
  if (!ACOUUPDATEAFTERWARDS)
  {
         ...
         i <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++i)
    {
      ...
      if (possiblemats.size() == 1)
      {
        c_p->ReplaceMyValue(alid, 0,
            (materialtable_[possiblemats[0]][4] + materialtable_[possiblemats[0]][5]) / 2.0);
        rho_p->ReplaceMyValue(alid, 0,
            (materialtable_[possiblemats[0]][6] + materialtable_[possiblemats[0]][7]) / 2.0);
        numelesofmaterial[possiblemats[0]]++;
        // std::cout<<"case 1: element "<<i<<" with reaction coefficient "<<reac<<" is identified to
        // be of material "<<possiblemats[0]<<std::endl;
      }
            ...
            // best
      {
        ...
        numelesofmaterial[bestfit]++;
        c_p->ReplaceMyValue(
            alid, 0, (materialtable_[bestfit][4] + materialtable_[bestfit][5]) / 2.0);
        rho_p->ReplaceMyValue(
            alid, 0, (materialtable_[bestfit][6] + materialtable_[bestfit][7]) / 2.0);
      }
    }
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouIdent::ReplaceParams(
    Teuchos::RCP<Epetra_Vector> params)
{
  if (reacordifforcorrho_ == 0)
    for (int i = 0; i < params->MyLength(); ++i)
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouIdent::SetRestartParameters(
    Teuchos::RCP<Epetra_Vector> reacs, Teuchos::RCP<Epetra_Vector> diffs,
    Teuchos::RCP<Epetra_Vector> cs, Teuchos::RCP<Epetra_Vector> rhos)
{
  reacordifforcorrho_ = 0;
...
void ACOU::PatImageReconstructionOptiSplitAcouIdent::ReadMaterials(std::string materialfilename)
{
  ...
  for (unsigned i = 0; i < nummats_; ++i) materialtable_[i].resize(4);  // mu_a, D, c, rho
  ...
  for (unsigned int m = 0; m < nummats_; ++m)
    std::cout << materialtable_[m][0] << " " << materialtable_[m][1] << " " << materialtable_[m][2]
              << " " << materialtable_[m][3] << " " << std::endl;
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstructionOptiSplitAcouIdentSmart::ReadMaterials(
    std::string materialfilename)
{
  ...
  for (unsigned i = 0; i < nummats_; ++i)
    materialtable_[i].resize(
        8);  // mu_a_min, mu_a_max, D_min, D_max, c_min, c_max, rho_min, rho_max
  ...
  if (!myrank_)
  {
    ...
    for (unsigned int m = 0; m < nummats_; ++m)
    {
      ...
      std::cout << "mua_min " << materialtable_[m][0] << " mua_max " << materialtable_[m][1]
                << std::endl;
      std::cout << "D_min   " << materialtable_[m][2] << " D_max   " << materialtable_[m][3]
                << std::endl;
      std::cout << "c_min   " << materialtable_[m][4] << " c_max   " << materialtable_[m][5]
                << std::endl;
      std::cout << "rho_min " << materialtable_[m][6] << " rho_max " << materialtable_[m][7]
                << std::endl;
    }
  }
  ...
  for (unsigned int m = 0; m < nummats_; ++m)
  {
    if (min_all_c > materialtable_[m][4]) min_all_c = materialtable_[m][4];
    if (max_all_c < materialtable_[m][5]) max_all_c = materialtable_[m][5];

    if (min_all_rho > materialtable_[m][6]) min_all_rho = materialtable_[m][6];
    if (max_all_rho < materialtable_[m][7]) max_all_rho = materialtable_[m][7];
  }
  return;
}
...
void ACOU::PatImageReconstructionOptiSplitAcouIdent::UpdateAcousticalParameters()
{
       ...
       i <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++i)
  {
    ...
    if (!acouident_avg_)
    {
      ...
      for (unsigned m = 0; m < nummats_; ++m)
      {
        double abstm = sqrt(0.85 * (reac - materialtable_[m][0]) * (reac - materialtable_[m][0]) +
                            0.15 * (D - materialtable_[m][1]) * (D - materialtable_[m][1]));
        if (abstm < abst)
        {
          abst = abstm;
          mat = m;
    ...
    else  // second possibility: average from two closest
    {
      ...
      for (unsigned int m = 0; m < nummats_; ++m)
      {
        double currentdistance =
            sqrt(0.85 * (reac - materialtable_[m][0]) * (reac - materialtable_[m][0]) +
                 0.15 * (D - materialtable_[m][1]) * (D - materialtable_[m][1]));
...
ACOU::PatImageReconstructionReduction::PatImageReconstructionReduction(
    ...
    : PatImageReconstruction(
          scatradis, acoudis, scatrapara, acoupara, scatrasolv, acousolv, scatraout, acouout),
      reductioncuttime_(
          acoupara->sublist("PA IMAGE RECONSTRUCTION").get<double>("REDUCTIONCUTTIME"))
{
  if (reductioncuttime_ == 0.0)
    dserror(
        "if you want to reduce domain and choose patreduction, set REDUCTIONCUTTIME to a "
        "reasonable non-zero value, thanks");
}
...
void ACOU::PatImageReconstructionReduction::Optimize()
{
  ...
  acouparams_->set<Teuchos::RCP<PATMonitorManager>>(
      "monitormanager", monitor_manager_[currentsequenzeiter_]);
  ...
  switch (dyna_)
  {
    ...
    {
      acoualgo_ = Teuchos::rcp(
          new ACOU::TimIntImplEuler(acou_discret_, acousolver_, acouparams_, acououtput_));
      break;
    }
    ...
    {
      acoualgo_ = Teuchos::rcp(
          new ACOU::AcouExplicitTimeInt(acou_discret_, acousolver_, acouparams_, acououtput_));
      break;
    }
    default:
  ...
  // now read the values from the newly written monitor file, return them in time and overwrite the
  // monitor file!
  if (myrank_ == 0)
  {
...
{
  if (PARTIALILLU) scatraparams_->set<unsigned int>("illusetup", illusetup_);
  ...
  const INPAR::SCATRA::VelocityField veltype =
      DRT::INPUT::IntegralValue<INPAR::SCATRA::VelocityField>(*scatraparams_, "VELOCITYFIELD");
  switch (veltype)
  {
    ...
    case INPAR::SCATRA::velocity_function:  // function
    {
      ...
      scatraalgo_ = Teuchos::rcp(new SCATRA::TimIntStationary(
          scatra_discret_, scatrasolver_, scatraparams_, scatraextraparams_, scatraoutput_));
    ...
    default:
      dserror(
          "unknown velocity field type for transport of passive scalar in problem type Acoustics");
      break;
  }
...
void ACOU::PatImageReconstruction::SolveStandardAcou()
{
  ...
  if (overwrite_output_)
  {
    if (write_baci_acou_output_) acououtput_->NewResultFile(outname, 0);
    last_acou_fw_output_count_ = 0;
  }
  else
  {
    if (write_baci_acou_output_) acououtput_->NewResultFile(outname, output_count_);
...
void ACOU::PatImageReconstruction::SolveAdjointScatra()
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> sysmatscatra =
      scatraalgo_->SystemMatrix();  // this matrix and the algorithm should still exist
  ...
  scatra_discret_->Evaluate(
      eleparams, Teuchos::null, Teuchos::null, b, Teuchos::null, Teuchos::null);
...
void ACOU::PatImageReconstruction::UpdateAbsorptionCoefficientFromTimeReversal()
{
  ...
  Teuchos::RCP<Epetra_Vector> trparams =
      Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap(), true));

  // do the business
  if (meshconform_)
  {
    for (int e = scatra_discret_->ElementRowMap()->MinAllGID();
         e <= scatra_discret_->ElementRowMap()->MaxAllGID(); ++e)
    {
      ...
      if (scatra_discret_->HaveGlobalElement(e))
      {
        ...
        if (myopteleowner != scatra_discret_->Comm().MyPID()) myopteleowner = -1;
      }
      ...
      if (acou_discret_->HaveGlobalElement(e - scatra_discret_->ElementRowMap()->MinAllGID() +
                                           acou_discret_->ElementRowMap()->MinAllGID()))
      {
        acou_ele = acou_discret_->gElement(e - scatra_discret_->ElementRowMap()->MinAllGID() +
                                           acou_discret_->ElementRowMap()->MinAllGID());
        myacoueleowner = acou_ele->Owner();
        if (myacoueleowner != myrank_) myacoueleowner = -1;
      }
      ...
      if (acoueleowner == opteleowner)
      {
        ...
        if (opteleowner == myrank_)
        {
          ...
          for (int i = 0; i < opti_ele->NumNode(); ++i)
            phi += phi_->operator[](
                scatra_discret_->DofRowMap()->LID(scatra_discret_->Dof((opti_ele->Nodes()[i]), 0)));
          ...
          acou_ele->Evaluate(
              para, *acou_discret_, la[0].lm_, elemat, elemat, elevec, elevec, elevec);
          double pressure = elevec[0];

          // compute absorption coefficient
          double reac = -pressure / gamma / phi;
          if (reac < 0.0) reac = 0.0;
      ...
      {
        // optical and acoustical element are not owned by the same processor -> communicate
        // acoustical values optical owner does most of the business
...
void ACOU::PatImageReconstruction::OutputReactionAndDiffusion()
{
  ...
  Teuchos::RCP<Epetra_Vector> reacvec =
      Teuchos::rcp(new Epetra_Vector(*(scatra_discret_->ElementRowMap()), false));
  Teuchos::RCP<Epetra_Vector> diffvec =
      Teuchos::rcp(new Epetra_Vector(*(scatra_discret_->ElementRowMap()), false));
...
void ACOU::PatImageReconstruction::ComputeNodeBasedReactionCoefficient()
{
  ...
  for (int nd = 0; nd < acou_discret_->NumGlobalNodes();
       ++nd)  // cannot loop scatra nodes, because they don't necessarily start with gid 0
  {
    // get node and owner
    ...
    if (scatra_discret_->HaveGlobalNode(nd + minnodeidscatra))
    {
      ...
      if (myoptnodeowner != scatra_discret_->Comm().MyPID())
        myoptnodeowner = -1;  // cannot use myrank_ because that is acou_discret_->Comm().MyPID()
    }
    ...
    // we have to do the same procedure for the absorption coefficient with the scatra
    // discretization
    ...
    for (int roel = 0; roel < scatra_discret_->NumMyRowElements(); ++roel)
    {
      ...
      for (int i = 0; i < numnode; ++i)
        ...
        {
          const MAT::ScatraMat* actmat =
              static_cast<const MAT::ScatraMat*>(roptele->Material().get());
...
/*----------------------------------------------------------------------*/
Teuchos::RCP<Epetra_Vector> ACOU::PatImageReconstruction::CalculateAdjointOptiRhsvec(
    Teuchos::RCP<Epetra_Vector> acounodevec)
{
  // this function is similar to the mapping in void
  // ACOU::AcouImplicitTimeInt::SetInitialPhotoAcousticField just the other way round
  Teuchos::RCP<Epetra_Vector> rhsvec = LINALG::CreateVector(*(scatra_discret_->DofRowMap()), true);
  ...
  if (meshconform_)
  {
    ...
    for (int nd = 0; nd < acou_discret_->NumGlobalNodes(); ++nd)
    {
      ...
      scatra_discret_->Comm().MaxAll(&myoptnodeowner, &optnodeowner, 1);
      if (optnodeowner ==
          -1)  // in this case, this node does not exist in the scatra discretization
        continue;
      ...
      // ok, we got the value, we still need c, rho and mu_a, but they are stored on the nodemap of
      // the scatra dis, so this should not be a problem
  ...
  else
  {
    ...
    Teuchos::RCP<Epetra_Vector> acounodeveccol =
        Teuchos::rcp(new Epetra_Vector(*(acou_discret_->NodeColMap())), true);
    ...
    for (int optnd = 0; optnd < scatra_discret_->NumGlobalNodes(); ++optnd)
    {
      ...
      {
        for (int d = 0; d < numdim; ++d) optnodecoords[d] = optnode->X()[d];
      }
      ...
      for (int acouel = 0; acouel < acou_discret_->NumMyRowElements(); ++acouel)
      {
        ...
        if (ele->Shape() == DRT::Element::quad4)
        {
          ...
          for (int nd = 0; nd < 4; ++nd)  // quad4 has 4 nodes
            for (int d = 0; d < numdim; ++d)
            {
              ...
              if (acounodecoords[nd][d] < minmaxvals[0][d])
                minmaxvals[0][d] = acounodecoords[nd][d];
              if (acounodecoords[nd][d] > minmaxvals[1][d])
                minmaxvals[1][d] = acounodecoords[nd][d];
            }
          // check, if acoustical node is in bounding box
          bool inside = true;
          for (int d = 0; d < numdim; ++d)
            if (optnodecoords[d] > minmaxvals[1][d] + 5.0e-5 ||
                optnodecoords[d] < minmaxvals[0][d] - 5.0e-5)
              inside = false;
          if (inside)
          {
        ...
        else if (ele->Shape() == DRT::Element::hex8)
        {
          ...
          if (inside)
          {
            ...
            for (int nd = 0; nd < 8; ++nd) r += values[nd];
            r /= 8.0;
          }
        }
        else
          dserror("up to now only implemented for quad4 and hex8");
      }  // for(int acouel=0; acouel<acou_discret_->NumMyRowElements(); ++acouel)
...
void ACOU::PatImageReconstruction::OutputStats()
{
  if (!myrank_)
  {
    ...
    std::cout << "*** relative error value:                 " << error_ / error_start_ << std::endl;
    std::cout << "*** output count:                         " << output_count_ << std::endl;
    std::cout << "*** simulation time since start [h]:      "
              << (Teuchos::Time::wallTime() - tstart_) / (60.0 * 60.0) << std::endl;
...
void ACOU::PatImageReconstruction::ReadRestart(int restartoutputcount)
{
  ...
  if (!myrank_)
  {
    std::cout << "READING RESTART FROM OUTPUTCOUNT " << restartoutputcount
              << " FROM FILES:" << std::endl;
  ...
  Teuchos::RCP<IO::InputControl> scainputfile =
      Teuchos::rcp(new IO::InputControl(scainputfilename, scatra_discret_->Comm()));
  Teuchos::RCP<IO::InputControl> acouinputfile =
      Teuchos::rcp(new IO::InputControl(acouinputfilename, acou_discret_->Comm()));
  ...
  // vectors
  Teuchos::RCP<Epetra_Vector> reacs =
      Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap()));
  Teuchos::RCP<Epetra_Vector> diffs =
      Teuchos::rcp(new Epetra_Vector(*scatra_discret_->ElementRowMap()));
  Teuchos::RCP<Epetra_Vector> cs = Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap()));
  Teuchos::RCP<Epetra_Vector> rhos =
      Teuchos::rcp(new Epetra_Vector(*acou_discret_->ElementRowMap()));
...
/*----------------------------------------------------------------------*/
void ACOU::PatImageReconstruction::SetRestartParameters(Teuchos::RCP<Epetra_Vector> reacs,
    Teuchos::RCP<Epetra_Vector> diffs, Teuchos::RCP<Epetra_Vector> cs,
    Teuchos::RCP<Epetra_Vector> rhos)
{
