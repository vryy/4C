...
inline double InterfaceDataPML<dim,Number>::sigma_fct(double distance, double width, int fct_choice) const
{
 ...
 */
template <int dim, typename Number>
inline double InterfaceDataPML<dim,Number>::sigma_fct_integral(double distance, double width, int fct_choice) const
{
  double return_value = 0.0;
  double argument = width > 1e-3 ? distance / width : 1e3 * distance;
  if (distance >= 0)
 ...
 */
template <int dim, typename Number>
AttenuationPML<dim,Number>::AttenuationPML()
:
attenuation_function(quadratic),
n_layer(0)
{}
...
{
  if (layer > n_layer)
    dserror("layer number is not valid!");
...
void AttenuationPML<dim,Number>::read_pml_definition(std::string filename)
{
  if (filename != std::string("none"))
  {
    ...
    if (!inputfile)
    {
      std::cout << (std::string("file \"")+filename+std::string("\" could not be opened")).c_str() << std::endl;
      dserror("");
      return;
    }
    ...
    while ((!inputfile.eof()) && (error_flag == 0))
    {
      ...
      if (word == std::string("PML"))
      {
        ...
        if ((word == std::string("N_VEC")) ||
            (word == std::string("R_SPH")) ||
            (word == std::string("R_CYL")) ||
            (word == std::string("R_INV")))
        {
          if (word == std::string("N_VEC"))
          {
    ...
    if ((!inputfile.eof()) || (error_flag != 0))
    {
      ...
      std::cout << std::endl;
      std::cout << "the input for the pml definition should look something like this: " << std::endl;
    ...
    for (unsigned int i = 0; i < n_layer; ++i)
    {
      ...
      for (unsigned int j = i+1; j < n_layer; ++j)
      {
        ...
        {
          double
          scalar_product = interface_pml[i].get_normal_vector() * interface_pml[j].get_normal_vector();
          if (std::abs(scalar_product) < 1e-14)
            orthogonality[i][j] = 0;
        }
                ...
                ((interface_pml[i].get_geometry() & hyper_cylinder) != 0)))
        {
          int flag = 0;
          for (unsigned int i = 0; i < 2; ++i)
            if (std::abs(interface_pml[i].get_normal_vector()[i]) > 1e-14)
              flag |= 1;
          if (flag)
            orthogonality[i][j] = 1;
          else
...
void AttenuationPML<dim,Number>::get_matrix (const std::vector<int> &layer_reference,
  ...
  for (unsigned int i = 0; i < n_layer; ++i)
    for (unsigned int j = i+1; j < n_layer; ++j)
      if (orthogonality[layer_reference[i]][layer_reference[j]])
        ortho_flag |= 1;
  ...
  else if (dim == 2)
  {
    ...
    for (unsigned int n = 0; n < dim; ++n)
      for (unsigned int i = 0; i < dim; ++i)
        for (unsigned int j = 0; j < dim; ++j)
          eigen_tensors[n][i][j][vec_array_no] = eigenvalues[n][vec_array_no] * eigenvectors[n][i] * eigenvectors[n][j];


    for (unsigned int i = 0; i < dim; ++i)
      sigma_values[i][vec_array_no] = 1.0;
  }
