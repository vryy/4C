...
namespace ACOU
{
  namespace internal
  {
    ...
    MatrixFree<dim, Number> create_matrix_free(const DoFHandler<dim> &dof_handler,
        const unsigned int fe_degree, const Epetra_Comm &comm, const bool extendedghosting)
    {
  ...
  //  this->data()
  {
    ...
    mass_matrix_data.reset(
        new internal::InverseMassMatrixData<dim, fe_degree, dim + 1, value_type>(this->data));
    mass_matrix_data_solid.reset(
        new internal::InverseMassMatrixData<dim, fe_degree, dim * dim + dim + 1, value_type>(
            this->data));
    mass_matrix_data_pml.reset(
        new internal::InverseMassMatrixData<dim, fe_degree, 2 * dim + 1, value_type>(this->data));
    ...
    if (monitormanager != Teuchos::null)
    {
      ...
      for (unsigned int i = 0; i < this->data.n_macro_cells() + this->data.n_macro_ghost_cells();
           ++i)
      {
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::read_initial_conditions(
      Teuchos::RCP<DRT::DiscretizationHDG> &discret,
      std::vector<parallel::distributed::Vector<value_type>> &dst)
  {
    ...
    for (unsigned int i = 0; i < this->data.n_macro_cells() + this->data.n_macro_ghost_cells(); ++i)
    {
      ...
      for (unsigned int v = 0; v < this->data.n_components_filled(i); ++v)
      {
        ...
        DRT::ELEMENTS::Acou *acouele =
            dynamic_cast<DRT::ELEMENTS::Acou *>(discret->lColElement(element_index));
        if (acouele == NULL) dserror("No acoustic element given!");
        ...
        switch (acouele->Shape())
        {
          ...
          case DRT::Element::hex8:
          {
            ...
            {
              std::cout << "d " << deal_vals_loc[0](0) << " " << deal_vals_loc[0](1) << " "
                        << deal_vals_loc[0](2) << " b " << baci_vals_loc[0](0) << " "
                        << baci_vals_loc[0](1) << " " << baci_vals_loc[0](2) << std::endl;
              std::cout << "d " << deal_vals_loc[1](0) << " " << deal_vals_loc[1](1) << " "
                        << deal_vals_loc[1](2) << " b " << baci_vals_loc[1](0) << " "
                        << baci_vals_loc[1](1) << " " << baci_vals_loc[1](2) << std::endl;
              std::cout << "d " << deal_vals_loc[2](0) << " " << deal_vals_loc[2](1) << " "
                        << deal_vals_loc[2](2) << " b " << baci_vals_loc[2](0) << " "
                        << baci_vals_loc[2](1) << " " << baci_vals_loc[2](2) << std::endl;
              std::cout << "d " << deal_vals_loc[3](0) << " " << deal_vals_loc[3](1) << " "
                        << deal_vals_loc[3](2) << " b " << baci_vals_loc[3](0) << " "
                        << baci_vals_loc[3](1) << " " << baci_vals_loc[3](2) << std::endl;
              std::cout << "d " << deal_vals_loc[4](0) << " " << deal_vals_loc[4](1) << " "
                        << deal_vals_loc[4](2) << " b " << baci_vals_loc[4](0) << " "
                        << baci_vals_loc[4](1) << " " << baci_vals_loc[4](2) << std::endl;
              std::cout << "d " << deal_vals_loc[5](0) << " " << deal_vals_loc[5](1) << " "
                        << deal_vals_loc[5](2) << " b " << baci_vals_loc[5](0) << " "
                        << baci_vals_loc[5](1) << " " << baci_vals_loc[5](2) << std::endl;
              std::cout << "d " << deal_vals_loc[6](0) << " " << deal_vals_loc[6](1) << " "
                        << deal_vals_loc[6](2) << " b " << baci_vals_loc[6](0) << " "
                        << baci_vals_loc[6](1) << " " << baci_vals_loc[6](2) << std::endl;
              std::cout << "d " << deal_vals_loc[7](0) << " " << deal_vals_loc[7](1) << " "
                        << deal_vals_loc[7](2) << " b " << baci_vals_loc[7](0) << " "
                        << baci_vals_loc[7](1) << " " << baci_vals_loc[7](2) << std::endl;
              dserror("unknown permutation");
            }
            break;
          }
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::write_deal_cell_values(
      Teuchos::RCP<DRT::DiscretizationHDG> &discret,
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWavePML<dim, fe_degree, Number>::write_deal_cell_values(
      Teuchos::RCP<DRT::DiscretizationHDG> &discret,
      const std::vector<parallel::distributed::Vector<value_type>> &src) const
  {
    const unsigned dofs_per_cell = this->data.get_dof_handler().get_fe().dofs_per_cell;
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::compute_gradient_contributions(
  ...
  {
    // we need the derivative of the mass matrix with respect to density and sound speed and have to
    // buuild the scalar product with corresponding pressure and velocity values
    ...
    ad.resize(adnp.size());
    for (unsigned int i = 0; i < adnp.size(); ++i) ad[i] = adnp[i];

    for (unsigned int cell = 0;
         cell < this->data.n_macro_cells() + this->data.n_macro_ghost_cells(); ++cell)
    {
      ...
      {
        const Tensor<1, dim, VectorizedArray<value_type>> adjoint_velocity_value =
            adjoint_velocity.get_value(q);
      ...
      const VectorizedArray<value_type> rho_fac =
          1. / densities[cell];  //-1./densities[cell]/densities[cell]/speeds[cell]/speeds[cell];
      const VectorizedArray<value_type> c_fac =
          2. / speeds[cell];  //-2./speeds[cell]/speeds[cell]/speeds[cell]/densities[cell];

      // get the dof values of integrated adjoint solution and multiply with correspondent forward
      // solutions
      ...
      {
        for (unsigned int d = 0; d < dim; ++d)
          densities_grad[cell] += 1. / (densities[cell]) * adjoint_velocity.get_dof_value(dof)[d] *
                                  forward_velocity.get_dof_value(dof)[d];  // factor is 1
        pressure_mass_mat_contrib +=
            adjoint_pressure.get_dof_value(dof) * forward_pressure.get_dof_value(dof);
      }
  ...
  template <int dim, int fe_degree, typename Number>
  double WaveEquationOperation<dim, fe_degree, Number>::get_SoS_gradient(int colid) const
  {
    for (unsigned int i = 0; i < this->data.n_macro_cells() + this->data.n_macro_ghost_cells(); ++i)
      for (unsigned int v = 0; v < this->data.n_components_filled(i); ++v)
      {
  ...
  template <int dim, int fe_degree, typename Number>
  double WaveEquationOperation<dim, fe_degree, Number>::get_density_gradient(int colid) const
  {
    for (unsigned int i = 0; i < this->data.n_macro_cells() + this->data.n_macro_ghost_cells(); ++i)
      for (unsigned int v = 0; v < this->data.n_components_filled(i); ++v)
        if (this->data.get_cell_iterator(i, v)->index() == colid)
  ...
  template <int dim, int fe_degree, typename Number>
  typename WaveEquationOperationBase<dim, Number>::value_type
  WaveEquationOperation<dim, fe_degree, Number>::evaluate_source_adjoint(const Point<dim> &p,
      const std::vector<std::vector<value_type>> nodes, std::vector<value_type> values) const
  {
    ...
    {
      value_type node_distance =
          std::sqrt((nodes[0][0] - nodes[1][0]) * (nodes[0][0] - nodes[1][0]) +
      ...
      result = quad_distance / node_distance * values[1] +
               (node_distance - quad_distance) / node_distance * values[0];
      result *= -1.0;
      result *= 2.0 / node_distance;

      //    if(node_distance<quad_distance)
      //    {
      //      std::cout<<"punkt "<<xyz[0]<<" "<<xyz[1]<<" nodecoords "<<nodes[0][0]<<"
      //      "<<nodes[0][1]
      //                                              <<" "           <<nodes[1][0]<<"
      //                                              "<<nodes[1][1]<<" "
      //                                              <<" nodevals  " <<values[0]  <<"
    ...
    else if (dim == 3 && nodes.size() == 4)  // hex8 with quad4 face element
    {
      ...
      result = (entiredistance - distance1) * values[0] + (entiredistance - distance2) * values[1] +
               (entiredistance - distance3) * values[2] + (entiredistance - distance4) * values[3];
      result *= -1.0;
  ...
  template <int dim, int fe_degree, typename Number>
  typename WaveEquationOperationBase<dim, Number>::value_type
  WaveEquationOperation<dim, fe_degree, Number>::evaluate_source_timereversal(const Point<dim> &p,
      const std::vector<std::vector<value_type>> nodes, std::vector<value_type> values) const
  {
    value_type result = 0.0;
    value_type xyz[dim];
  ...
  template <int dim, int fe_degree, typename Number>
  WaveEquationOperationAcousticWave<dim, fe_degree, Number>::WaveEquationOperationAcousticWave(
      const std::vector<const DoFHandler<dim> *> &dof_handlers,
      Teuchos::RCP<DRT::DiscretizationHDG> &discret,
      Teuchos::RCP<Function<dim>> boundary_conditions, Teuchos::RCP<Function<dim>> source_term,
      value_type time_step_in, int sourceno, Teuchos::RCP<PATMonitorManager> monitormanagerin)
            ...
            source_term, time_step_in, sourceno, monitormanagerin)
  {
    for (unsigned int i = 0; i < this->data.n_macro_cells() + this->data.n_macro_ghost_cells(); ++i)
    {
      ...
      for (unsigned int v = 0; v < this->data.n_components_filled(i); ++v)
      {
        ...
        this->densities[i][v] = discret->lColElement(element_index)
                                    ->Material()
                                    ->Parameter()
                                    ->GetParameter(0, discret->lColElement(element_index)->Id());
        this->speeds[i][v] = discret->lColElement(element_index)
                                 ->Material()
                                 ->Parameter()
                                 ->GetParameter(1, discret->lColElement(element_index)->Id());
      }
    }
  }

  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::local_apply_domain(
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::evaluate_cell(
      ...
      const unsigned int cell) const
  {
    ...
    {
      const Tensor<1, dim, VectorizedArray<value_type>> pressure_gradient = phi_p.get_gradient(q);
      const Tensor<1, dim, VectorizedArray<value_type>> velocity_value = phi_v.get_value(q);
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWavePML<dim, fe_degree, Number>::local_apply_domain(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      for (unsigned int q = 0; q < velocity.n_q_points; ++q)
      {
        ...
        const Tensor<2, dim, VectorizedArray<value_type>> velocity_gradient =
        ...
        for (unsigned int n = 0; n < this->data.n_components_filled(cell); ++n)
        {
          ...
          if (layer_reference[cell][n].size())
            sigma_pml->get_matrix(layer_reference[cell][n], n, q_point, sigma_values, eigen_values,
                Matrix_A, eigen_tensors);
        }
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::local_apply_face(
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::evaluate_inner_face(
      FEFaceEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi,
      FEFaceEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi_neighbor,
      ...
      value_type boundary_fac) const
  {
    ...
    for (unsigned int q = 0; q < phi.n_q_points; ++q)
    {
      ...
      if (this->adjoint_eval == false)
      {
        lambda = tau_inv * (normal_v_plus + normal_v_minus + tau_plus * val_plus[dim] +
      ...
      {
        lambda = tau_inv * (rho_inv_plus * normal_v_plus + rho_inv_minus * normal_v_minus -
                               tau_plus * rho_plus * c_sq_plus * val_plus[dim] -
                               tau_minus * rho_minus * c_sq_minus * val_minus[dim]);
      ...
      {
        val_plus[dim] =
            -boundary_fac * (-rho_inv_plus * normal_v_plus +
                                tau_plus * (c_sq_plus * rho_plus * val_plus[dim] + lambda));
        val_minus[dim] =
            -boundary_fac * (-rho_inv_minus * normal_v_minus +
                                tau_minus * (c_sq_minus * rho_minus * val_minus[dim] + lambda));
      }
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWavePML<dim, fe_degree, Number>::local_apply_face(
      const MatrixFree<dim, value_type> &,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &face_range) const
  {
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++)
    {
      ...
      for (unsigned int q = 0; q < phi.n_q_points; ++q)
      {
          ...
          if (this->reduction)
          {
            if (inner_face_monitored[face].any())
            {
              for (unsigned int v = 0;
                   v < VectorizedArray<value_type>::n_array_elements &&
                   this->data.faces[face].left_cell[v] != numbers::invalid_unsigned_int;
                   ++v)
              {
                if (inner_face_monitored[face][v])
                {
                  ...
                  double value = 0.0;
                  for (unsigned int d = 0; d < dim; ++d) vecpoint[d] = q_point[d][v];
        ...
        {
          val_plus[dim] =
              -c_sq_plus * rho_plus * (normal_v_plus + tau_plus * (val_plus[dim] - lambda));
          val_minus[dim] =
              -c_sq_minus * rho_minus * (normal_v_minus + tau_minus * (val_minus[dim] - lambda));
        }
        else
        {
          val_plus[dim] = -(-rho_inv_plus * normal_v_plus +
                            tau_plus * (c_sq_plus * rho_plus * val_plus[dim] + lambda));
          val_minus[dim] = -(-rho_inv_minus * normal_v_minus +
                             tau_minus * (c_sq_minus * rho_minus * val_minus[dim] + lambda));
        }
        phi.submit_value(val_plus, q);
        phi_neighbor.submit_value(val_minus, q);
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_domain(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    ACOU::WaveEquationOperation<dim, fe_degree, Number>::local_apply_domain(
        data, dst, src, cell_range);
  }
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_face(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &face_range) const
  {
    ACOU::WaveEquationOperation<dim, fe_degree, Number>::local_apply_face(
        data, dst, src, face_range);
  }
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_boundary_face(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &face_range) const
  {
    ACOU::WaveEquationOperation<dim, fe_degree, Number>::local_apply_boundary_face(
        data, dst, src, face_range);
  }
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_mass_matrix(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    ACOU::WaveEquationOperation<dim, fe_degree, Number>::local_apply_mass_matrix(
        data, dst, src, cell_range);
  }


  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::local_apply_boundary_face(
      const MatrixFree<dim, value_type> &,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &face_range) const
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::evaluate_boundary_face(
      FEFaceEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi,
      const std::vector<parallel::distributed::Vector<value_type>> &src, unsigned int face,
      value_type boundary_fac) const
  {
    ...
    for (unsigned int q = 0; q < phi.n_q_points; ++q)
    {
      ...
      VectorizedArray<value_type> normal_v_plus = val_plus[0] * normal[0];
      for (unsigned int d = 1; d < dim; ++d) normal_v_plus += val_plus[d] * normal[d];
      Point<dim, VectorizedArray<value_type>> q_point = phi.quadrature_point(q);
      VectorizedArray<value_type> lambda;
      if (int_boundary_id == 0)  // absorbing boundary
        if (this->adjoint_eval == false)
          ...
          lambda = 1. / (tau + 1. / c / rho) * rho_inv * normal_v_plus -
                   tau * rho * c_sq / (tau + 1. / c / rho) * p_plus;
      ...
      else if (int_boundary_id >= 5)  // dbcs
      {
        if (this->adjoint_eval == false)
          for (unsigned int v = 0; v < VectorizedArray<value_type>::n_array_elements; ++v)
          {
            ...
            lambda[v] =
                this->dirichlet_boundary_conditions->value(point, (int_boundary_id - 5) * dim);
          }
        else
      ...
      else
      {
        ...
        val_plus[dim] =
            -boundary_fac * (-rho_inv * normal_v_plus + tau * (lambda + rho * c_sq * p_plus));
      }
      phi.submit_value(val_plus, q);
    }
    phi.integrate(true, false);
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::local_apply_mass_matrix(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      mass_matrix_data->inverse.apply(mass_matrix_data->coefficients, dim + 1,
          mass_matrix_data->phi[0].begin_dof_values(), mass_matrix_data->phi[0].begin_dof_values());
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWavePML<dim, fe_degree, Number>::local_apply_mass_matrix(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      this->mass_matrix_data_pml->inverse.fill_inverse_JxW_values(
          this->mass_matrix_data_pml->coefficients);
      this->mass_matrix_data_pml->inverse.apply(this->mass_matrix_data_pml->coefficients,
          dim + dim + 1, this->mass_matrix_data_pml->phi[0].begin_dof_values(),
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::apply(
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      std::vector<parallel::distributed::Vector<value_type>> &dst, const double &cur_time,
      const double &dt) const
  {
    for (unsigned int d = 0; d <= dim; ++d) dst[d] = 0.0;
    ...
    this->data.loop(&WaveEquationOperation<dim, fe_degree, Number>::local_apply_domain,
        ...
        &WaveEquationOperation<dim, fe_degree, Number>::local_apply_boundary_face, this, dst, src);

    this->computing_times[0] += timer.wall_time();
    timer.restart();

    this->data.cell_loop(
        &WaveEquationOperation<dim, fe_degree, Number>::local_apply_mass_matrix, this, dst, dst);
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWave<dim, fe_degree, Number>::apply(
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      std::vector<parallel::distributed::Vector<value_type>> &dst, const double &cur_time,
      const double &dt) const
  {
    ...
    this->data.loop(&WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_domain,
        ...
        &WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_boundary_face, this,
        dst, src);
    ...
    this->data.cell_loop(
        &WaveEquationOperationAcousticWave<dim, fe_degree, Number>::local_apply_mass_matrix, this,
        dst, dst);
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperation<dim, fe_degree, Number>::compute_post_gradient(
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      std::vector<parallel::distributed::Vector<value_type>> &dst, const double &cur_time,
      const double &dt) const
  {
  ...
  template <int dim, int fe_degree, typename Number>
  WaveEquationOperationAcousticWavePML<dim, fe_degree,
      Number>::WaveEquationOperationAcousticWavePML(const std::vector<const DoFHandler<dim> *>
                                                        &dof_handlers,
      Teuchos::RCP<DRT::DiscretizationHDG> &discret,
      Teuchos::RCP<Function<dim>> boundary_conditions, Teuchos::RCP<Function<dim>> source_term,
      Teuchos::RCP<AttenuationPML<dim, Number>> sigma_fct, value_type time_step_in, int sourceno,
      Teuchos::RCP<PATMonitorManager> monitormanagerin, bool adjoint, bool redin)
      : WaveEquationOperationAcousticWave<dim, fe_degree, Number>(dof_handlers, discret,
        ...
        for (unsigned int layer = 0; layer < sigma_fct->get_n_layer(); ++layer)
        {
          ...
          for (unsigned int k = 0; k < GeometryInfo<dim>::vertices_per_cell; ++k)
            if (sigma_fct->is_layer_active(layer, this->data.get_cell_iterator(i, v)->vertex(k)))
              layer_flag = true;
          if (layer_flag) layer_reference[i][v].push_back(layer);
        }
      }
    }
    ...
    if (adjoint || redin)
    {
      ...
      for (unsigned int f = 0; f < this->data.n_macro_inner_faces(); ++f)
      {
        for (unsigned int v = 0; v < VectorizedArray<value_type>::n_array_elements &&
                                 this->data.faces[f].left_cell[v] != numbers::invalid_unsigned_int;
             ++v)
        {
          const unsigned int left_cell_index_non_vectorized = this->data.faces[f].left_cell[v];
          const int left_element_index =
              this->data
                  .get_cell_iterator(left_cell_index_non_vectorized /
                                         VectorizedArray<value_type>::n_array_elements,
                      left_cell_index_non_vectorized %
                          VectorizedArray<value_type>::n_array_elements)
                  ->index();
          const unsigned int right_cell_index_non_vectorized = this->data.faces[f].right_cell[v];
          const int right_element_index =
              this->data
                  .get_cell_iterator(right_cell_index_non_vectorized /
                                         VectorizedArray<value_type>::n_array_elements,
                      right_cell_index_non_vectorized %
                          VectorizedArray<value_type>::n_array_elements)
                  ->index();
          ...
          for (int n = 0; n < leftele->NumNode(); ++n)
          {
            ...
            {
              if (nodeid == rightele->NodeIds()[m]) sharednodes.push_back(nodeid);
            }
          }
          ...
          for (unsigned int i = 0; i < sharednodes.size(); ++i)
            for (unsigned int j = 0; j < pressmonBC.size(); ++j)
              if (pressmonBC[j]->ContainsNode(int(sharednodes[i]))) nodepartofmon++;
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWavePML<dim, fe_degree, Number>::read_initial_conditions(
      Teuchos::RCP<DRT::DiscretizationHDG> &discret,
      std::vector<parallel::distributed::Vector<value_type>> &dst)
  {
                ...
                deal_vals_loc[3].distance(baci_vals_loc[2]) < 1e-10)
            {
              ...
              for (unsigned j = 0; j < dofs_per_cell; ++j)
              {
                ...
                phi.begin_dof_values()[dim * dofs_per_cell + j][v] =
                    acouele->eleinteriorPressnp_(j);
                for (unsigned int d = 0; d < dim; ++d)
                  phi.begin_dof_values()[(dim + 1 + d) * dofs_per_cell + j][v] =
                      acouele->eleinteriorAuxiliaryPML_(d * dofs_per_cell + j);
              }
            }
            else if (deal_vals_loc[0].distance(baci_vals_loc[3]) < 1e-10 &&
                     deal_vals_loc[1].distance(baci_vals_loc[0]) < 1e-10 &&
              ...
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  phi.begin_dof_values()[d * dofs_per_cell + i][v] =
                      acouele->eleinteriorVelnp_(d * dofs_per_cell + permute);
                for (unsigned int d = 0; d < dim; ++d)
                  phi.begin_dof_values()[(dim + 1 + d) * dofs_per_cell + i][v] =
                      acouele->eleinteriorAuxiliaryPML_(d * dofs_per_cell + permute);
              }
            }
            else if (deal_vals_loc[0].distance(baci_vals_loc[2]) < 1e-10 &&
                     deal_vals_loc[1].distance(baci_vals_loc[3]) < 1e-10 &&
