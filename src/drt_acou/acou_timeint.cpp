 ...
 *----------------------------------------------------------------------*/
ACOU::AcouTimeInt::AcouTimeInt(const Teuchos::RCP<DRT::DiscretizationHDG>& actdis,
      ...
      dtp_(params_->get<double>("TIMESTEP"))
{
  ...
  if (invana_ && (DRT::INPUT::IntegralValue<INPAR::ACOU::InvAnalysisType>(
                      *params_, "INV_ANALYSIS") == INPAR::ACOU::pat_reduction))
    reduction_ = true;
  else
    reduction_ = false;
  ...
  if (!invana_) params_->set<bool>("timereversal", false);
...
{
  // we have to call an init for the elements (for inverse analysis, otherwise adjoint run starts
  // from values of previous forward run)
    ...
    Teuchos::RCP<DRT::Discretization> scatradis, bool meshconform)
{
  ...
  if (meshconform)
  {
    ...
    for (int optel = 0; optel < numoptele; ++optel)
    {
      ...
      if (scatradis->HaveGlobalElement(optel + minoptelegid))
      {
        ...
        if (myopteleowner != myrank_)
          myopteleowner = -1;  // do not want to consider ghosted elements
      }
      ...
      if (acoueleowner == opteleowner)
      {
        ...
        if (opteleowner == myrank_)
        {
          ...
          double absorptioncoeff =
              static_cast<MAT::ScatraMat*>((optele->Material()).get())->ReaCoeff(optele->Id());
      ...
      else  // the other case: the acoustical element and the optical element are owned by different
            // procs
      {
        ...
        if (myrank_ != opteleowner) (*nodevals).resize(size);
    ...
    for (unsigned int i = 0; i < relevantghostelements.size(); ++i)
    {
      ...
      if (discret_->HaveGlobalElement(
              relevantghostelements[i]))  // true for owner and the proc with ghost
      {
      ...
      if (myrank_ != actualowner) pressvals.resize(size);
  ...
  else
  {
    ...
    if (!myrank_)
      std::cout << "Welcome to nonconform mapping, this might take a while! Please have a little "
                   "patience."
                << std::endl;
    ...
    // this is necessary, otherwise we get problems with the interpolation, especially for
    // acoustical elements which are bigger than optical elements!

    /*************************** STEP 1 - COMPUTE NODE BASED PRESSURE FIELD
     * ***************************/
    Teuchos::RCP<Epetra_Vector> pressurenode =
        Teuchos::rcp(new Epetra_Vector(*(discret_->NodeRowMap())));
    ...
    for (int acound = 0; acound < discret_->NumGlobalNodes(); ++acound)
    {
      if ((int)((acound * 100) / discret_->NumGlobalNodes()) > (int)(10 * percent_counter))
      {
        ...
        {
          std::cout << "---------------------------------------" << std::endl;
          std::cout << (int)((acound * 100) / discret_->NumGlobalNodes()) - 1
      ...
      {
        for (int d = 0; d < numdim_; ++d) acounodecoords[d] = acounode->X()[d];
      }
      ...
      for (int optel = 0; optel < scatradis->NumMyRowElements(); ++optel)
      {
        ...
        if (ele->Shape() == DRT::Element::quad4)
        {
          ...
          bool inside = true;
          for (int d = 0; d < numdim_; ++d)
            if (acounodecoords[d] > minmaxvals[1][d] + 1.0e-5 ||
                acounodecoords[d] < minmaxvals[0][d] - 1.0e-5)
              inside = false;
          ...
          if (inside)
          {
            ...
            {
              double absorptioncoeff =
                  static_cast<MAT::ScatraMat*>((ele->Material()).get())->ReaCoeff(ele->Id());
              // get the values!
              double values[4] = {0};
              for (int nd = 0; nd < 4; ++nd)  // quad4 has 4 nodes
              {
          ...
          dserror("up to now only implemented for quad4");
      }  // for(int optel=0; optel<scatradis->NumMyRowElements(); ++optel)
    ...
    Teuchos::RCP<Epetra_Vector> pressurenodecol =
        Teuchos::rcp(new Epetra_Vector(*(discret_->NodeColMap())));
    ...
    {
      std::cout << "---------------------------------------" << std::endl;
      std::cout << "100% of Coupling Evaluations are done! It took "
                << Teuchos::Time::wallTime() - tcpumap << " seconds!" << std::endl;
      std::cout << "---------------------------------------" << std::endl;
    }
  }  // else ** if(meshconform)
...
/*----------------------------------------------------------------------*
 | Initialization by given scatra solution vector (pub)  schoeder 04/14 |
 *----------------------------------------------------------------------*/
// void ACOU::AcouTimeInt::SetInitialPhotoAcousticField(Teuchos::RCP<SCATRA::TimIntStationary>
// scatraalgo)
//{
//  // we have to call an init for the elements first!
//  Teuchos::ParameterList initParams;
...
//  int minacouelegid = params_->get<int>("MINPATELEGID");
//  if(minacouelegid<0)
//    minacouelegid = discret_->ElementRowMap()->MinAllGID();
//  int maxacouelegid = params_->get<int>("MAXPATELEGID");
//  if(maxacouelegid<0)
//    maxacouelegid = discret_->ElementRowMap()->MaxAllGID();
//
...
//    gausspointsforscatra(gsize/numdim_,std::vector<double>(numdim_)); for(int gp=0;
//    gp<gsize/numdim_; ++gp)
//      for(int d=0; d<numdim_; ++d)
//      {
...
void ACOU::AcouTimeInt::OutputDensityAndSpeedOfSound()
{
  ...
  Teuchos::RCP<Epetra_Vector> densvec =
      Teuchos::rcp(new Epetra_Vector(*(discret_->ElementRowMap()), false));
  Teuchos::RCP<Epetra_Vector> cvec =
      Teuchos::rcp(new Epetra_Vector(*(discret_->ElementRowMap()), false));
...
namespace
{
  void getNodeVectorsHDG(DRT::Discretization& dis, const Teuchos::RCP<Epetra_Vector>& traceValues,
      const int ndim, Teuchos::RCP<Epetra_MultiVector>& velocity,
      Teuchos::RCP<Epetra_Vector>& pressure, Teuchos::RCP<Epetra_Vector>& tracevel,
      Teuchos::RCP<Epetra_Vector>& cellPres, INPAR::ACOU::PhysicalType phys, bool padapt)
  {
    ...
    for (int el = 0; el < dis.NumMyColElements(); ++el)
    {
      ...
      if (interpolVec.M() == 0) interpolVec.Resize(ele->NumNode() * (ndim + 2) + 1);
      ...
      if (eleIndex >= 0) (*cellPres)[eleIndex] += interpolVec((ndim + 2) * ele->NumNode());
    }
  ...
  void getNodeVectorsHDGSolid(DRT::Discretization& dis,
      ...
      Teuchos::RCP<Epetra_MultiVector>& tracevelocity, Teuchos::RCP<Epetra_Vector>& cellPres,
      INPAR::ACOU::PhysicalType phys, bool writestress)
  {
    {
      const Epetra_Map* nodemap = dis.NodeRowMap();
      velocity.reset(new Epetra_MultiVector(*nodemap, 3));
    ...
    for (int el = 0; el < dis.NumMyColElements(); ++el)
    {
      ...
      for (int i = 0; i < ele->NumNode(); ++i)
      {
        ...
        for (int d = 0; d < ndim; ++d)
        {
          ...
          tracevelocity->SumIntoMyValue(
              localIndex, d, interpolVec((d + ndim) * ele->NumNode() + i));
        }
        for (int d = 0; d < ndim * ndim; ++d)
          velocitygradient->SumIntoMyValue(
              localIndex, d, interpolVec(ele->NumNode() * (2 * ndim + 2 + d) + i + 2));
    ...
    for (int i = 0; i < pressure->MyLength(); ++i)
    {
      ...
      for (int d = 0; d < ndim * ndim; ++d) (*velocitygradient)[d][i] /= touchCount[i];
    }
    dis.ClearState(true);
...
void ACOU::AcouTimeInt::Output()
{
  ...
  if (step_ % upres_ == 0)
  {
    ...
    {
      getNodeVectorsHDG(*discret_, velnp_, numdim_, interpolatedVelocity, interpolatedPressure,
          traceVel, cellPres, phys_, padaptivity_);
    }
    else  // if(phys_ == INPAR::ACOU::acou_solid)
    {
      getNodeVectorsHDGSolid(*discret_, velnp_, numdim_, interpolatedVelocityGradient,
          interpolatedVelocity, interpolatedPressure, traceVelocity, cellPres, phys_, writestress_);
    }

    // fill in pressure values into monitor file, if required
    if (!invana_ || reduction_) FillMonitorFile(interpolatedPressure);
    ...
    if (step_ == 0)
    {
      ...
      if (phys_ != INPAR::ACOU::acou_solid) OutputDensityAndSpeedOfSound();
    }
    ...
    else  // (phys_ == INPAR::ACOU::acou_solid)
    {
      ...
      if (numdim_ == 2)
      {
        Teuchos::RCP<Epetra_Vector> stress =
            Teuchos::rcp(new Epetra_Vector(*discret_->NodeRowMap()));
      ...
      {
        output_->WriteVector(
            "stress_xx", Teuchos::rcp(interpolatedVelocityGradient->operator()(0)));
        output_->WriteVector(
            "stress_xy", Teuchos::rcp(interpolatedVelocityGradient->operator()(1)));
        output_->WriteVector(
            "stress_xz", Teuchos::rcp(interpolatedVelocityGradient->operator()(2)));
        output_->WriteVector(
            "stress_yx", Teuchos::rcp(interpolatedVelocityGradient->operator()(3)));
        output_->WriteVector(
            "stress_yy", Teuchos::rcp(interpolatedVelocityGradient->operator()(4)));
        output_->WriteVector(
            "stress_yz", Teuchos::rcp(interpolatedVelocityGradient->operator()(5)));
        output_->WriteVector(
            "stress_zx", Teuchos::rcp(interpolatedVelocityGradient->operator()(6)));
        output_->WriteVector(
            "stress_zy", Teuchos::rcp(interpolatedVelocityGradient->operator()(7)));
        output_->WriteVector(
            "stress_zz", Teuchos::rcp(interpolatedVelocityGradient->operator()(8)));
      }
    }
...
void ACOU::AcouTimeInt::NodalPsiField(Teuchos::RCP<Epetra_Vector> outvec)
{
  ...
  for (int el = 0; el < discret_->NumMyColElements(); ++el)
  {
    ...
    if (interpolVec.M() == 0) interpolVec.Resize(ele->NumNode());

    ele->Evaluate(
        params, *discret_, la[0].lm_, dummyMat, dummyMat, interpolVec, dummyVec, dummyVec);
  ...
  for (int i = 0; i < outvec->MyLength(); ++i) (*outvec)[i] /= touchCount[i];
...
void ACOU::AcouTimeInt::InitMonitorFile()
{
  if (writemonitor_)
  {
    ...
    // if(pressuremon.size()>1) dserror("write of monitor file only implemented for one pressure
    // monitor condition");
    ...
    for (unsigned int n = 0; n < pressuremonmics.size(); ++n)
    {
      ...
      if (myrank_ == 0) fprintf(fp, "%e %e %e\n", coords[0], coords[1], coords[2]);
    }
    if (myrank_ == 0)
    {
      fprintf(fp, "#\n#\n#\n");
      fprintf(fp, "%e", 0.0);  // first time
      for (int m = 0; m < mics; ++m) fprintf(fp, " %e", 0.0);
      fprintf(fp, "\n");
      fclose(fp);
    }
  }
