...
{
  template <int dim>
  DistributedTriangulation<dim>::DistributedTriangulation(Teuchos::RCP<DRT::DiscretizationHDG> discret)
  :
  discret(discret)
  {
    ...
    Teuchos::RCP<Epetra_MultiVector> elenodeids = Teuchos::rcp(new Epetra_MultiVector(*discret->ElementRowMap(),GeometryInfo<dim>::vertices_per_cell));
    for(int i=0; i<discret->NumMyRowElements(); ++i)
      for(unsigned int j=0; j<GeometryInfo<dim>::vertices_per_cell; ++j)
        elenodeids->operator ()(j)->operator [](i) = discret->lRowElement(i)->NodeIds()[j];
    Teuchos::RCP<Epetra_MultiVector> proc0elenodeids = Teuchos::rcp(new Epetra_MultiVector(*proc0elemap,GeometryInfo<dim>::vertices_per_cell));
    ...
    Teuchos::RCP<Epetra_MultiVector> nodecoords = Teuchos::rcp(new Epetra_MultiVector(*discret->NodeRowMap(),dim));
    for(int i=0; i<discret->NumMyRowNodes(); ++i)
      for(unsigned int j=0; j<dim; ++j)
        nodecoords->operator ()(j)->operator [](i) = discret->lRowNode(i)->X()[j];
    Teuchos::RCP<Epetra_MultiVector> proc0nodecoords = Teuchos::rcp(new Epetra_MultiVector(*proc0nodemap,dim));
    ...
    // 5. delete unused vertices (necessary since all vertices are stored on every processor) but only col elements
  ...
  void DoFHandler<dim,spacedim>::distribute_dofs (const FiniteElement<dim,spacedim> &ff)
  {
    ...
    for (unsigned int i=0; i<tria->n_levels(); ++i)
      {
        ...
        .resize (tria->n_raw_cells(i) *
                 dim == 2 ? selected_fe->dofs_per_quad :
    ...
    if (tria->n_cells() > 0)
      ...
      if (dim > 1)
        ...
        .resize (tria->n_raw_lines() *
                 ...
                 DoFHandler<dim,spacedim>::invalid_dof_index);
      if (dim == 3)
        reinterpret_cast<dealii::internal::DoFHandler::DoFFaces<3> *>(faces)->quads.dofs
        .resize (tria->n_raw_quads() *
    ...
    for (typename DoFHandler<dim>::active_cell_iterator cell=begin_active();
         cell != end(); ++cell)
    {
      const types::global_dof_index index_base = selected_fe->dofs_per_cell *
    ...
    number_cache.n_global_dofs        = types::global_dof_index(rowmap->NumGlobalElements()) *
        selected_fe->dofs_per_cell;
    number_cache.n_locally_owned_dofs = types::global_dof_index(rowmap->NumMyElements()) *
        selected_fe->dofs_per_cell;

    if (!rowmap->LinearMap())
      dserror("Only linear maps are supported");

    number_cache.locally_owned_dofs
      = IndexSet (number_cache.n_global_dofs);
    number_cache.locally_owned_dofs.add_range (types::global_dof_index(rowmap->MinMyGID())*
                                               selected_fe->dofs_per_cell,
                                               types::global_dof_index(rowmap->MaxMyGID())*
                                               selected_fe->dofs_per_cell+1);
    number_cache.locally_owned_dofs.compress();

    // TODO: fill in correct info here!
    number_cache.n_locally_owned_dofs_per_processor
      = std::vector<types::global_dof_index> (rowmap->Comm().NumProc(),
                                              -1);

    number_cache.locally_owned_dofs_per_processor
      = std::vector<IndexSet> (rowmap->Comm().NumProc(),
                               IndexSet (number_cache.n_global_dofs));
                      ...
                      DoFHandler<dim>          &dof_handler)
  {
    ...
    int shift = 0;
    for (int i=0; i<rowmap->Comm().MyPID(); ++i)
      shift += ids_per_proc[i];
    ...
    {
      const types::global_dof_index index_base = fe.dofs_per_cell *
          types::global_dof_index(ghosted[ghosted.Map().LID(tria->discret->lColElement(cell->index())->Id())]);
    ...
    dof_handler.number_cache.n_global_dofs        = types::global_dof_index(rowmap->NumGlobalElements()) *
        fe.dofs_per_cell;
    dof_handler.number_cache.n_locally_owned_dofs = types::global_dof_index(rowmap->NumMyElements()) *
        fe.dofs_per_cell;

    dof_handler.number_cache.locally_owned_dofs
      = IndexSet (dof_handler.number_cache.n_global_dofs);
    dof_handler.number_cache.locally_owned_dofs.add_range (types::global_dof_index(shift)*
                                                           fe.dofs_per_cell,
                                                           types::global_dof_index(shift+nids)*
                                                           fe.dofs_per_cell);
    ...
    dof_handler.number_cache.locally_owned_dofs_per_processor
      = std::vector<IndexSet> (rowmap->Comm().NumProc(),
                               IndexSet (dof_handler.number_cache.n_global_dofs));
  ...
  template void assign_dg_dofs(const FiniteElement<2> &,
                               DoFHandler<2>          &);
  template void assign_dg_dofs(const FiniteElement<3> &,
                               DoFHandler<3>          &);
}
