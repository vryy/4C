                                   ...
                                   Number cfl_number_in)
{
  ...
  n_cells_with_ghosts = op.get_matrix_free().n_macro_cells()+op.get_matrix_free().n_macro_ghost_cells();
  ...
  for (unsigned int i=0; i<n_cells_with_ghosts; ++i)
    {
      ...
      for (unsigned int v=0; v<op.get_matrix_free().n_components_filled(i); ++v)
        {
          //std::cout<<"p "<<Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)<<" i "<<i<<" v "<<v<<" center "<<op.get_matrix_free().get_cell_iterator(i,v)->center()<<std::endl;
          ...
          if (acttimestep<vectcelltimestep)
            vectcelltimestep = acttimestep;
        }
      ...
      if (cluster_id<smallest_cluster_id)
  ...
  if (n_clusters>max_clusters)
    {
      ...
      cluster_diff = std::min(cluster_diff,max_diff); // but this difference is also restricted by user input
  ...
  distr_cluster_ids.compress(VectorOperation::insert); // TODO: update deal in lnm folder to allow for min evaluation
  ...
  while (faster_and_slower_neighbor && count<100)
    {
                          ...
                          if (op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->has_children())
                            {
                              for (unsigned int subfaces = 0; subfaces < GeometryInfo<Operator::dimension>::max_children_per_face; ++subfaces)
                                {
                                  if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]]<c)
                                    is_anyone_faster = true;
                                }
                            }
                          else if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]]<c)
                            is_anyone_faster = true;
                        }
                  for (unsigned int v=0; v<op.get_matrix_free().n_components_filled(i); ++v)
                    {
                                  ...
                                  for (unsigned int subfaces = 0; subfaces < GeometryInfo<Operator::dimension>::max_children_per_face; ++subfaces)
                                    {
                                      if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]]>c)
                                        {
                                          if (is_anyone_faster)
                                            cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]] = c;
                                          else
                                            cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]] = c+1;
                                        }
                                    }
                                }
                              else
                                {
                                  if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]]>c)
                                    {
                                      if (is_anyone_faster)
                                        cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]] = c;
                                      else
                                        cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]] = c+1;
                                    }
                                }
                            }
                        }
      ...
      do{
        ...
        commcount++;
      } while(Utilities::MPI::max(int(cell_cluster_ids_old!=cell_cluster_ids),MPI_COMM_WORLD) && commcount<100);
      if (commcount==100)
        Assert(false,ExcMessage("clustering communication is too bad"));
                      ...
                      if (op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->has_children())
                        {
                          for (unsigned int subfaces = 0; subfaces < GeometryInfo<Operator::dimension>::max_children_per_face; ++subfaces)
                            {
                              if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]]<current_c)
                                {
                                  is_anyone_faster = true;
                                  cell_have_faster_neighbor[i] = true;
                                }
                              else if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]]>current_c)
                                {
                                  is_anyone_slower = true;
                                  cell_have_slower_neighbor[i] = true;
                                }
                              if (int(cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]])<int(current_c-1)
                                  ||  cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor_child_on_subface(n,subfaces)->active_cell_index()]]>current_c+1)
                                {
                        ...
                        {
                          if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]]<current_c)
                            {
                              is_anyone_faster = true;
                              cell_have_faster_neighbor[i] = true;
                            }
                          else if (cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]]>current_c)
                            {
                              is_anyone_slower = true;
                              cell_have_slower_neighbor[i] = true;
                            }
                          if (int(cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]])<int(current_c-1)
                              ||  cell_cluster_ids[mf_index[op.get_matrix_free().get_cell_iterator(i,v)->neighbor(n)->active_cell_index()]]>current_c+1)
                            {
                              faster_and_slower_neighbor = true;
                            }
                        }
  ...
  // communicate the cell_have_faster_neighbor and cell_have_slower_neighbor vector to a vector with ghost entries
  //{
  distr_cluster_ids = 0.;
  for (unsigned int i=0; i<n_cells; ++i)
    for (unsigned int v=0; v<op.get_matrix_free().n_components_filled(i); ++v)
                                               ...
                                               std::vector<parallel::distributed::Vector<Number> >        &dst) const
{
  ...
  for (unsigned int cycle = 0; cycle<n_updates; ++cycle)
    {
      ...
      t2sa = std::min(Number(actual_cluster_time+cluster_timestepmultiples[actual_cluster]*fastest_time_step),Number(op.time));
                    ...
                    for (unsigned int v=0; v<op.get_matrix_free().n_components_filled(e); ++v)
                      for (unsigned int n=0; n<GeometryInfo<Operator::dimension>::faces_per_cell; ++n)
                        if (op.get_matrix_free().get_cell_iterator(e,v)->neighbor_index(n)>=0)
                          {
                            if (temp_faster[op.get_matrix_free().get_cell_iterator(e,v)->neighbor_index(n)/Operator::n_vect] && cell_cluster_ids[e]<actual_cluster)
                              update_cell[e]=true;
                            else
                              update_cell[e]=false;
                          }
                ...
                for (unsigned int e=0; e<n_cells; ++e)
                  {
                    for (unsigned int v=0; v<op.get_matrix_free().n_components_filled(e); ++v)
                      for (unsigned int n=0; n<GeometryInfo<Operator::dimension>::faces_per_cell; ++n)
                        if (op.get_matrix_free().get_cell_iterator(e,v)->neighbor_index(n)>=0)
                          if (temp_slower[op.get_matrix_free().get_cell_iterator(e,v)->neighbor_index(n)/Operator::n_vect] && cell_cluster_ids[e]>actual_cluster)
                            {
                              update_cell[e]=true;
                            }
                  }
              ...
              if (f<op.get_matrix_free().n_macro_inner_faces())
                {
                  for (unsigned int v=0; v<Operator::n_vect; ++v)
                    {
                      if (op.get_matrix_free().faces[f].left_cell[v]!=numbers::invalid_unsigned_int && op.get_matrix_free().faces[f].right_cell[v]!=numbers::invalid_unsigned_int)
  ...
  for (unsigned c=0; c<cluster_update_times.size(); ++c)
    for (unsigned int n=0; n<2 ; ++n)
      cluster_update_times[c][n] += op.time_step;
               ...
               if (op.get_matrix_free().get_cell_iterator(e,v)->neighbor_index(n)>=0)
                 {
                   if (op.get_matrix_free().get_cell_iterator(e,v)->neighbor(n)->has_children())
                     ...
                     {
                       if (update_cell[mf_index[op.get_matrix_free().get_cell_iterator(e,v)->neighbor(n)->active_cell_index()]])
                         is_neighbor_of_update_cell[e] = true;
                     }
                 }
         }
       ...
       if (std::abs(t2-t1)>relative_tolerance*fastest_time_step) // only do this if necessary
         {
           ...
           for (unsigned int f=0; f<op.get_matrix_free().faces.size(); ++f)
           {
             ...
             if (f<op.get_matrix_free().n_macro_inner_faces())
             {
               for (unsigned int v=0; v<Operator::n_vect; ++v)
                 if (op.get_matrix_free().faces[f].left_cell[v]!=numbers::invalid_unsigned_int && op.get_matrix_free().faces[f].right_cell[v]!=numbers::invalid_unsigned_int)
                   if (evaluate_cell[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect] && evaluate_cell[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect])
                   {
                     // set face evaluation
                     if (  ( cell_cluster_ids[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect]==actual_cluster
                         && cell_cluster_ids[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect]!=actual_cluster
                         && is_neighbor_of_update_cell[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect] )
                         ||
                         ( cell_cluster_ids[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect]!=actual_cluster
                             && cell_cluster_ids[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect]==actual_cluster
                             && is_neighbor_of_update_cell[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect] ) )
                     {
                       evaluate_face[f] = true;
                       // set masks
                       if (cell_cluster_ids[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect]==actual_cluster)
         ...
         if (f<op.get_matrix_free().n_macro_inner_faces())
           {
             for (unsigned int v=0; v<Operator::n_vect; ++v)
               if (op.get_matrix_free().faces[f].left_cell[v]!=numbers::invalid_unsigned_int && op.get_matrix_free().faces[f].right_cell[v]!=numbers::invalid_unsigned_int)
                 {
                   if ( (update_cell[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect] && update_cell[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect])
                        || (update_cell[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect] && evaluate_cell[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect] && cell_cluster_ids[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect] == actual_cluster)
                        || (evaluate_cell[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect] && update_cell[op.get_matrix_free().faces[f].right_cell[v]/Operator::n_vect] && cell_cluster_ids[op.get_matrix_free().faces[f].left_cell[v]/Operator::n_vect] == actual_cluster) )
                     {
                       evaluate_face[f] = true;
                       // set masks
                       phi_to_dst[f][v] = true;
...
WaveEquationOperationAcousticWaveADERLTS(const std::vector<const DoFHandler<dim> *> &dof_handlers,
  ...
  :
  WaveEquationOperationAcousticWaveADER<dim,fe_degree,Number>(dof_handlers,discret,boundary_conditions,source_term,time_step_in,sourceno,monitormanagerin)
{
      ...
      if (cluster_manager.is_evaluate_cell(cell))
        {
          ...
          this->integrate_taylor_cauchykovalewski(cell,phi_eval,phi_spectral,src,t2,t1,te,cluster_manager.improvedgraddiv);
      ...
      if (cluster_manager.is_update_cell(cell))
        {
          ...
          this->integrate_taylor_cauchykovalewski(cell,phi_eval,phi_spectral,src,dt,0.,0.,cluster_manager.improvedgraddiv);
...
template<int dim, int fe_degree, typename Number>
void WaveEquationOperationAcousticWaveADERLTS<dim,fe_degree,Number>::evaluate_cells_and_faces_first_ader(
  const std::vector<parallel::distributed::Vector<value_type> >  &src,
  std::vector<parallel::distributed::Vector<value_type> >        &dst) const
{
  ...
  this->data.cell_loop (&WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_firstader_domain,
                        this, this->tempsrc, src);

  // evaluate faces for these cells
  this->data.loop (&WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_dummy_domain,
...
void WaveEquationOperationAcousticWaveADERLTS<dim,fe_degree,Number>::evaluate_cells_second_ader(
  ...
  std::vector<parallel::distributed::Vector<value_type> >        &dst) const
{
  this->data.cell_loop (&WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_secondader_domain,
                        this, dst, src);
}
...
template <int dim, int fe_degree, typename Number>
void WaveEquationOperationAcousticWaveADERLTS<dim,fe_degree,Number>::reconstruct_div_grad(const std::vector<parallel::distributed::Vector<value_type> >  &src,
    std::vector<parallel::distributed::Vector<value_type> >        &dst) const
{
  this->data.loop (&WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_postprocessing_domain,
                   &WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_postprocessing_face,
                   &WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_postprocessing_boundary_face,
                   this, dst, src);

  this->data.cell_loop(&WaveEquationOperationAcousticWaveADERLTS<dim, fe_degree, Number>::local_apply_postprocessing_mass_matrix,
                       this, dst, dst);
}
