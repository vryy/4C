...
namespace ACOU
{
  ...
  class ExplicitIntegrator
  {
   public:
    ...
    ExplicitIntegrator(const Operator &operation) : operation(operation) {}

    ~ExplicitIntegrator() {}

    virtual void do_time_step(std::vector<vector_type> &vec_n, std::vector<vector_type> &vec_np,
        const double current_time, const double time_step) = 0;
  ...
  class ArbitraryHighOrderDG : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    ArbitraryHighOrderDG(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};
        ...
        const double current_time, const double time_step)
    {
      ...
      // add
      for (unsigned int d = 0; d < vec_n.size(); ++d) vec_np[d].sadd(-1.0, 1.0, vec_n[d]);
    }
  };
  ...
  class ArbitraryHighOrderDGLTS : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    ArbitraryHighOrderDGLTS(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};
  ...
  class ExplicitEuler : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    ExplicitEuler(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};

    virtual void do_time_step(std::vector<vector_type> &vec_n, std::vector<vector_type> &vec_np,
        const double current_time, const double time_step)
    {
      for (unsigned int d = 0; d < vec_n.size(); ++d) vec_np[d] = 0;
  ...
  class ClassicalRK4 : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    ClassicalRK4(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};
        ...
        const double current_time, const double time_step)
    {
      ...
      if (vec_tmp1.empty())
      {
        ...
        for (unsigned int d = 0; d < n_components; ++d) vec_tmp1[d].reinit(vec_np[d]);
        for (unsigned int d = 0; d < n_components; ++d) vec_tmp2[d].reinit(vec_np[d], true);
      }
  ...
  namespace
  {
    ...
    {
      RKVectorUpdater(const Number factor1, const Number factor2, const bool is_last,
      ...
      void apply_to_subrange(const std::size_t begin, const std::size_t end) const
      {
    ...
    {
      RKVectorUpdatesRange(const double factor1, const double factor2, const bool is_last,
          std::vector<parallel::distributed::Vector<Number>> &vector1,
          std::vector<parallel::distributed::Vector<Number>> &vector2,
          std::vector<parallel::distributed::Vector<Number>> &vector3)
          : updater(factor1, factor2, is_last, vector1, vector2, vector3)
      {
        ...
        if (size < dealii::internal::Vector::minimum_parallel_grain_size)
          ...
          apply_parallel(0, size, dealii::internal::Vector::minimum_parallel_grain_size);
      }
  ...
  class LowStorageRK33Reg2 : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    LowStorageRK33Reg2(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};
        ...
        const double current_time, const double time_step)
    {
      ...
      // stage 1
      this->operation.apply(vec_n, vec_tmp1, current_time);
      RKVectorUpdatesRange<value_type>(
          a21 * time_step, (b1 - a21) * time_step, false, vec_tmp1, vec_n, vec_np);
      // stage 2
      this->operation.apply(vec_n, vec_tmp1, current_time + a21 * time_step);
      RKVectorUpdatesRange<value_type>(
          a32 * time_step, (b2 - a32) * time_step, false, vec_tmp1, vec_np, vec_n);
      // stage 3
      this->operation.apply(vec_np, vec_tmp1, current_time + (b1 + a32) * time_step);
      RKVectorUpdatesRange<value_type>(b3 * time_step, 0, true, vec_tmp1, vec_np, vec_n);
    }
  ...
  class LowStorageRK45Reg2 : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    LowStorageRK45Reg2(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};
        ...
        const double current_time, const double time_step)
    {
      ...
      this->operation.apply(vec_np, vec_tmp1, current_time + (b1 + a32) * time_step);
      RKVectorUpdatesRange<value_type>(
          a43 * time_step, (b3 - a43) * time_step, false, vec_tmp1, vec_n, vec_np);
      // stage 4
      this->operation.apply(vec_n, vec_tmp1, current_time + (b1 + b2 + a43) * time_step);
      RKVectorUpdatesRange<value_type>(
          a54 * time_step, (b4 - a54) * time_step, false, vec_tmp1, vec_np, vec_n);
      // stage 5
      this->operation.apply(vec_np, vec_tmp1, current_time + (b1 + b2 + b3 + a54) * time_step);
      RKVectorUpdatesRange<value_type>(b5 * time_step, 0, true, vec_tmp1, vec_np, vec_n);
    }
  ...
  class LowStorageRK45Reg3 : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    LowStorageRK45Reg3(const Operator &operation) : ExplicitIntegrator<Operator>(operation){};
        ...
        const double current_time, const double time_step)
    {
      ...
      for (unsigned int d = 0; d < n_components; ++d) vec_np[d] = 0;
      ...
      for (unsigned int d = 0; d < n_components; ++d) vec_np[d].add(b5 * time_step, vec_tmp1[d]);
      // stage 5 -- end
    }
  ...
  class StrongStabilityPreservingRK : public ExplicitIntegrator<Operator>
  {
   public:
    ...
    StrongStabilityPreservingRK(
        const Operator &operation, const unsigned int order, const unsigned int stages)
        : ExplicitIntegrator<Operator>(operation), coeffs_are_initialized(false), order(order)
    {
  ...
  template <typename Operator>
  void StrongStabilityPreservingRK<Operator>::do_time_step(std::vector<vector_type> &vec_n,
      std::vector<vector_type> &vec_np, const double current_time, const double time_step)
  {
    ...
    for (unsigned int s = 1; s < stages + 1; ++s)
    {
      ...
      for (unsigned int l = 0; l < s; ++l)
      {
        for (unsigned int d = 0; d < n_components; ++d)
        {
          vec_tmp2[d].add(A[s - 1][l], vec_tmp1[l * n_components + d], B[s - 1][l] * time_step,
              vec_tmp3[l * n_components + d]);
        }
        if (s > 1 && A[s - 2][l] > 0) c += B[s - 2][l] / A[s - 2][l];
      }
      for (unsigned int d = 0; d < n_components; ++d) vec_np[d] = 0;
    ...
    for (unsigned int d = 0; d < n_components; ++d) vec_np[d] = vec_tmp1[stages * n_components + d];
  }

  template <typename Operator>
  void StrongStabilityPreservingRK<Operator>::initialize_coefficients(const unsigned int stages)
  {
