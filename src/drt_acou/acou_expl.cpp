...
namespace ACOU
{
   ...
   *----------------------------------------------------------------------*/
  AcouExplicitTimeInt::AcouExplicitTimeInt(const Teuchos::RCP<DRT::DiscretizationHDG> &actdis,
        ...
        writebacioutput_(params_->get<bool>("writeacououtput"))
  {
    ...
    if (numdim_ == 2 && doubleorfloat_ == true)
      wave2dd_ = Teuchos::rcp(
          new WaveEquationProblem<2, double>(actdis, params, Teuchos::rcp(this, false)));
    else if (numdim_ == 3 && doubleorfloat_ == true)
      wave3dd_ = Teuchos::rcp(
          new WaveEquationProblem<3, double>(actdis, params, Teuchos::rcp(this, false)));
    else if (numdim_ == 2 && doubleorfloat_ == false)
      wave2df_ = Teuchos::rcp(
          new WaveEquationProblem<2, float>(actdis, params, Teuchos::rcp(this, false)));
    else if (numdim_ == 3 && doubleorfloat_ == false)
      wave3df_ = Teuchos::rcp(
          new WaveEquationProblem<3, float>(actdis, params, Teuchos::rcp(this, false)));
    else
      dserror(
          "number of dimensions must be 2 or 3 for explicit time integration of acoustic problems");

    // in case time step is reduced through deal cfl, tell baci
    UpdateTimeStepSize();

    if (writebacioutput_) output_->WriteMesh(0, 0.0);
  ...
  double AcouExplicitTimeInt::GetSoSGradient(int colid)
  {
    ...
    //        std::cout<<p<<" "<<i<<" "<< wave2dd_->get_SoS_gradient(i)<<"
    //        "<<discret_->ElementRowMap()->GID(i)<<" col
    //        "<<discret_->ElementColMap()->LID(discret_->ElementRowMap()->GID(i))<<std::endl;
  ...
  {
    // first: get the deal values and put them in the baci elements (output is probably a bit more
  ...
  {
   public:
    ExactSolution(const unsigned int numcomponent, const double time, const unsigned int functno)
        : Function<dim>(numcomponent, time), numcomponent(numcomponent), functno(functno)
    {
    }
  ...
  {
    double xyz[dim];
    for (unsigned d = 0; d < dim; ++d) xyz[d] = p(d);
  ...
  {
   public:
    RightHandSide(const unsigned int n_components, const double time, const unsigned int functno)
        : Function<dim>(n_components, time), functno(functno)
    {
    }
  ...
  {
   public:
    DirichletBoundaryFunction(const unsigned int n_components, const double time,
  ...
  WaveEquationProblem<dim, Number>::WaveEquationProblem(
      Teuchos::RCP<DRT::DiscretizationHDG> discretin,
      const Teuchos::RCP<Teuchos::ParameterList> params,
      ...
      : pcout(std::cout, Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0),
        ...
        time(0.0),
        step(0),
        cfl_number(params->get<double>("COURANTNUMBER") / fe.degree /
                   dim),  // cfl_number (0.1/fe.degree/dim),
        discret(discretin),
        ...
        solid((DRT::INPUT::IntegralValue<INPAR::ACOU::PhysicalType>(*params, "PHYSICAL_TYPE")) ==
        ...
        writebacioutput(params->get<bool>("writeacououtput"))
  {
    ...
    for (; cell != endc; ++cell)
    {
      ...
      if (diameter < min_cell_diameter) min_cell_diameter = diameter;
    }
    ...
    {
      pcout << "WARNING: Time step size set to " << time_step << " to prevent stability problems"
            << std::endl;
      pcout << "   finest cell / speed of sound " << glob_min_cell_diameter << std::endl
            << std::endl;
      if (invana)
      {
        time_step = 0.0;
    ...
    else
    {
      ...
      pcout << "Time step size: " << time_step << " (smaller than critical timestep) " << std::endl
            << std::endl;
    }
    ...
    for (unsigned int d = 1; d < solutions.size(); ++d) solutions[d] = solutions[0];
    for (unsigned int d = 0; d < solutions.size(); ++d) previous_solutions[d] = solutions[0];
    ...
    if (bacitimeint->MonitorManager() != Teuchos::null)
    {
      if (bacitimeint->MonitorManager()->CellIdsRequired())
      {
        ...
        if (bacitimeint->iswithPMLS()) discret->GetCondition("PressureMonitor", pressmonBC);
        ...
        for (; cell != endc; ++cell)
        {
          ...
          for (unsigned f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
          {
            if (cell->face(f)->at_boundary())
            {
              ...
              if (reduction)
              {
                if (int_boundary_id == 4)
                {
                  ...
                  if (facemeasure != facemeasure)
                  {
                    ...
                    //    " rowlid
                    //    "<<discret->ElementRowMap()->LID(discret->ElementColMap()->GID(cell->index()))<<std::endl;
              ...
              else if (int_boundary_id == 1 ||
                       int_boundary_id == 2)  // monitored or monitored and absorbing
              {
                cellhasmonitoredface = true;
                facemeasure = cell->face(f)->measure();
                fullfacemeasure += facemeasure;
            ...
            else
            {
              ...
              if (bacitimeint->iswithPMLS())
              {
                ...
                for (int n = 0; n < ele->NumNode(); ++n)
                  if (pressmonBC[0]->ContainsNode(ele->NodeIds()[n])) count++;
                ...
                if (count >= 2)  // dim) //if(count>1)
                {
                  ...
                  if (facemeasure == facemeasure)
                    fullfacemeasure +=
                        facemeasure / 2.0;  // an inner face is touched two times by this
                }
              }
            }
          }
          if (cellhasmonitoredface) countcell++;

          if (cellhasmonitoredface)
            if (discret->ElementColMap()->GID(cell->index()) >= 0)
            {
              // if( (discret->gElement(discret->ElementColMap()->GID(cell->index())))->Owner() ==
              // discret->Comm().MyPID()) // exclude column elements
              for (int det = 0; det < numdetec; ++det)
              {
                ...
                if (cell->point_inside(position))
                {
                  ...
                  if (discret->ElementColMap()->GID(cell->index()) >= 0)
                    if ((discret->gElement(discret->ElementColMap()->GID(cell->index())))
                            ->Owner() == discret->Comm().MyPID())
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::set_evaluator(
      const Teuchos::RCP<Teuchos::ParameterList> params, const int fe_degree)
  {
    if (fe_degree > 6) dserror("Only degrees between 1 and 6 are implemented!");
    ...
    Teuchos::RCP<Function<dim>> diriboundary(
        new DirichletBoundaryFunction<dim>(comps, 0.0, discret));
    ...
    {
      if (fe_degree == 1)
        evaluator.reset(new WaveEquationOperationElasticWave<dim, 1, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 2)
        evaluator.reset(new WaveEquationOperationElasticWave<dim, 2, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 3)
        evaluator.reset(new WaveEquationOperationElasticWave<dim, 3, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 4)
        evaluator.reset(new WaveEquationOperationElasticWave<dim, 4, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 5)
        evaluator.reset(new WaveEquationOperationElasticWave<dim, 5, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 6)
        evaluator.reset(new WaveEquationOperationElasticWave<dim, 6, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
    }
    else if (dyna == INPAR::ACOU::acou_ader_lts)
    {
      if (fe_degree == 1)
        evaluator.reset(new WaveEquationOperationAcousticWaveADERLTS<dim, 1, Number>(dof_handlers,
            discret, diriboundary, rhs, time_step, cfl_number * fe.degree * dim, sourcetermfuncno,
            bacitimeint->MonitorManager()));
      else if (fe_degree == 2)
        evaluator.reset(new WaveEquationOperationAcousticWaveADERLTS<dim, 2, Number>(dof_handlers,
            discret, diriboundary, rhs, time_step, cfl_number * fe.degree * dim, sourcetermfuncno,
            bacitimeint->MonitorManager()));
      else if (fe_degree == 3)
        evaluator.reset(new WaveEquationOperationAcousticWaveADERLTS<dim, 3, Number>(dof_handlers,
            discret, diriboundary, rhs, time_step, cfl_number * fe.degree * dim, sourcetermfuncno,
            bacitimeint->MonitorManager()));
      else if (fe_degree == 4)
        evaluator.reset(new WaveEquationOperationAcousticWaveADERLTS<dim, 4, Number>(dof_handlers,
            discret, diriboundary, rhs, time_step, cfl_number * fe.degree * dim, sourcetermfuncno,
            bacitimeint->MonitorManager()));
      else if (fe_degree == 5)
        evaluator.reset(new WaveEquationOperationAcousticWaveADERLTS<dim, 5, Number>(dof_handlers,
            discret, diriboundary, rhs, time_step, cfl_number * fe.degree * dim, sourcetermfuncno,
            bacitimeint->MonitorManager()));
      else if (fe_degree == 6)
        evaluator.reset(new WaveEquationOperationAcousticWaveADERLTS<dim, 6, Number>(dof_handlers,
            discret, diriboundary, rhs, time_step, cfl_number * fe.degree * dim, sourcetermfuncno,
            bacitimeint->MonitorManager()));
    ...
    else if (dyna == INPAR::ACOU::acou_ader)
    {
      if (fe_degree == 1)
        evaluator.reset(
            new WaveEquationOperationAcousticWaveADER<dim, 1, Number>(dof_handlers, discret,
                diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 2)
        evaluator.reset(
            new WaveEquationOperationAcousticWaveADER<dim, 2, Number>(dof_handlers, discret,
                diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 3)
        evaluator.reset(
            new WaveEquationOperationAcousticWaveADER<dim, 3, Number>(dof_handlers, discret,
                diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 4)
        evaluator.reset(
            new WaveEquationOperationAcousticWaveADER<dim, 4, Number>(dof_handlers, discret,
                diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 5)
        evaluator.reset(
            new WaveEquationOperationAcousticWaveADER<dim, 5, Number>(dof_handlers, discret,
                diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 6)
        evaluator.reset(
            new WaveEquationOperationAcousticWaveADER<dim, 6, Number>(dof_handlers, discret,
                diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
    }
    else if (filename_pml != std::string("none.txt"))
    {
      Teuchos::RCP<AttenuationPML<dim, Number>> sigma =
          Teuchos::rcp(new AttenuationPML<dim, Number>());
      sigma->read_pml_definition(filename_pml);

      if (fe_degree == 1)
        evaluator.reset(new WaveEquationOperationAcousticWavePML<dim, 1, Number>(dof_handlers,
            discret, diriboundary, rhs, sigma, time_step, sourcetermfuncno,
            bacitimeint->MonitorManager(), invana && adjoint, reduction));
      else if (fe_degree == 2)
        evaluator.reset(new WaveEquationOperationAcousticWavePML<dim, 2, Number>(dof_handlers,
            discret, diriboundary, rhs, sigma, time_step, sourcetermfuncno,
            bacitimeint->MonitorManager(), invana && adjoint, reduction));
      else if (fe_degree == 3)
        evaluator.reset(new WaveEquationOperationAcousticWavePML<dim, 3, Number>(dof_handlers,
            discret, diriboundary, rhs, sigma, time_step, sourcetermfuncno,
            bacitimeint->MonitorManager(), invana && adjoint, reduction));
      else if (fe_degree == 4)
        evaluator.reset(new WaveEquationOperationAcousticWavePML<dim, 4, Number>(dof_handlers,
            discret, diriboundary, rhs, sigma, time_step, sourcetermfuncno,
            bacitimeint->MonitorManager(), invana && adjoint, reduction));
      else if (fe_degree == 5)
        evaluator.reset(new WaveEquationOperationAcousticWavePML<dim, 5, Number>(dof_handlers,
            discret, diriboundary, rhs, sigma, time_step, sourcetermfuncno,
            bacitimeint->MonitorManager(), invana && adjoint, reduction));
      else if (fe_degree == 6)
        evaluator.reset(new WaveEquationOperationAcousticWavePML<dim, 6, Number>(dof_handlers,
            discret, diriboundary, rhs, sigma, time_step, sourcetermfuncno,
            bacitimeint->MonitorManager(), invana && adjoint, reduction));
    }
    else
    {
      if (fe_degree == 1)
        evaluator.reset(new WaveEquationOperationAcousticWave<dim, 1, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 2)
        evaluator.reset(new WaveEquationOperationAcousticWave<dim, 2, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 3)
        evaluator.reset(new WaveEquationOperationAcousticWave<dim, 3, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 4)
        evaluator.reset(new WaveEquationOperationAcousticWave<dim, 4, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 5)
        evaluator.reset(new WaveEquationOperationAcousticWave<dim, 5, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
      else if (fe_degree == 6)
        evaluator.reset(new WaveEquationOperationAcousticWave<dim, 6, Number>(dof_handlers, discret,
            diriboundary, rhs, time_step, sourcetermfuncno, bacitimeint->MonitorManager()));
    }
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::make_grid_and_dofs(
      Teuchos::RCP<DRT::DiscretizationHDG> discret)
  {
    if (!invana)
    {
      ...
      pcout << "Number of global active faces: " << triangulation.n_active_faces() << std::endl;
    }
    ...
    if (!invana)
    {
      pcout << "Number of degrees of freedom \n"
            << "from discontinuous velocitites and fluxes: " << (dim + 1) * dof_handler.n_dofs()
            << std::endl;
    }
    typename Triangulation<dim>::active_cell_iterator cell = triangulation.begin_active(),
                                                      endc = triangulation.end();
      ...
      for (unsigned f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
      {
        if (cell->face(f)->at_boundary())
        {
          ...
          if (is_diri < GeometryInfo<dim>::vertices_per_face)
          {
            ...
            if (is_abc >= GeometryInfo<dim>::vertices_per_face) cell->face(f)->set_boundary_id(0);
          }

          if (is_diri < GeometryInfo<dim>::vertices_per_face)  // monitored cannot be where
                                                               // Dirichlet is prescribed but it can
                                                               // be absorbing and monitored
          {
            ...
            if (reduction)
            {
              ...
              if (is_pmon1)
              {
                ...
                cell->face(f)->set_boundary_id(
                    2);  // second pmon condition is at the inner ring (write into new monitor file)
              }
            }
  ...
  {
    template <int dim>
    void get_relevant_set(const DoFHandler<dim> &dof_handler, IndexSet &relevant_set)
    {
      ...
      for (; cell != endc; ++cell)
      {
        ...
        for (std::vector<types::global_dof_index>::iterator it = dof_indices.begin();
             it != dof_indices.end(); ++it)
          if (!relevant_set.is_element(*it)) global_dof_indices.insert(*it);
      }
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::compute_post_pressure()
  {
    ...
    // evaluate error in gradient
    {
      for (int d = 0; d < dim; ++d)
      {
        ...
        VectorTools::integrate_difference(dof_handler, temp_post_solutions[d], ZeroFunction<dim>(1),
            norm_per_cell_p, QGauss<dim>(fe.degree + 2), VectorTools::L2_norm);
        double solution_mag =
        ...
        norm_per_cell_p = 0;
        VectorTools::integrate_difference(dof_handler, temp_post_solutions[d],
            ExactSolution<dim>(dim, time, exactsolutionfuncno + d + dim + 1), norm_per_cell_p,
        ...
        VectorTools::integrate_difference (dof_handler,
            ...
            VectorTools::L2_norm);*/
        double exact_sol_mag =
            0.0;  // std::sqrt(Utilities::MPI::sum (norm_per_cell_p.norm_sqr(), MPI_COMM_WORLD));

        pcout << "gradient, component " << d << " absolute error " << std::sqrt(error_mag)
              << " exact solution norm " << std::sqrt(exact_sol_mag) << " solution norm "
              << std::sqrt(solution_mag) << " at time " << time << std::endl;
      }
    }
    ...
    Vector<double> rhs_vector(dofs_per_cell_post), replace_vector(dofs_per_cell_post);
    ...
    for (; cellpost != endcpost; ++cell, ++cellpost)
    {
      if (cell->is_locally_owned())
      {
        ...
        for (unsigned int q_index = 0; q_index < n_q_points; ++q_index)
        {
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            replace_value += fe_values.shape_value(i, q_index) * solution_vector[dim][i] *
          ...
          for (unsigned int i = 0; i < dofs_per_cell_post; ++i)
          {
            ...
            rhs_vector(i) -= rho * flux_values * fe_values_post.shape_grad(i, q_index) *
                             fe_values_post.JxW(q_index);
            replace_vector(i) +=
                fe_values_post.shape_value(i, q_index) * 1.0 * fe_values_post.JxW(q_index);
          }
        }
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::compute_post_velocity()
  {
    evaluator->set_adjoint_eval(false);
    for (unsigned int d = 0; d < dim * dim + dim + 1; ++d) previous_solutions[d] = 0;
    ...
    for (unsigned int vcomp = 0; vcomp < dim; ++vcomp)
    {
      ...
      typename DoFHandler<dim>::active_cell_iterator cell = dof_handler.begin_active();
      typename DoFHandler<dim>::active_cell_iterator cellpost =
                                                         dof_handler_post_disp.begin_active(),
                                                     endcpost = dof_handler_post_disp.end();
      ...
      for (; cellpost != endcpost; ++cell, ++cellpost)
      {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          for (unsigned int d = 0; d < dim; ++d)
            solution_vector[d][i] =
                previous_solutions[dim + 1 + vcomp * dim + d](local_dof_indices[i]);
        ...
        for (unsigned int q_index = 0; q_index < n_q_points; ++q_index)
        {
          ...
          for (unsigned int i = 0; i < dofs_per_cell_post; ++i)
          {
            ...
            rhs_vector(i) -=
                flux_values * fe_values_post.shape_grad(i, q_index) * fe_values_post.JxW(q_index);
            replace_vector(i) +=
                fe_values_post.shape_value(i, q_index) * 1.0 * fe_values_post.JxW(q_index);
          }
        }
  ...
  template <int dim, typename Number>
  Number WaveEquationProblem<dim, Number>::determine_pressure_value(
      Point<dim> point, unsigned int cellindex)
  {
    ...
    typename DoFHandler<dim>::active_cell_iterator cell(
        &dof_handler.get_triangulation(), 0, cellindex, &dof_handler);
    ...
    bool inrange = true;
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
      if (solutions[dim].get_partitioner()->in_local_range(local_dof_indices[i]) == false &&
          solutions[dim].get_partitioner()->is_ghost_entry(local_dof_indices[i]) == false)
        inrange = false;
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
      pressure_value += dof_handler.get_fe().shape_value(i, referencepoint) *
                        solutions[dim](local_dof_indices[i]);

    return pressure_value;
  }

  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::output_results(const unsigned int timestep_number)
  {
    if (step % up_res == 0)
    {
      if (invana && !adjoint && acouopt) write_restart(timestep_number);
    }
    ...
    if (invana && !adjoint && !reduction)
    {
      ...
      for (int det = 0; det < numdetec; ++det)
      {
        Point<dim> position;
        for (unsigned int d = 0; d < dim; ++d)
          position[d] = positions[det * dim + d] *
                        (1.0 - eps);  // we don't want errors from points which are on the boundary
    ...
    if (step % up_res == 0 && exactsolutionfuncno >= 0 && this->solid == false)
    {
      for (int d = dim; d < dim + 1; ++d)  // for(int d=0; d<dim+1; ++d)
      {
        ...
        // calculate norm of field
        VectorTools::integrate_difference(dof_handler, solutions[d], ZeroFunction<dim>(1),
            norm_per_cell_p, QGauss<dim>(fe.degree + 1), VectorTools::L2_norm);
        double solution_mag =
            std::sqrt(Utilities::MPI::sum(norm_per_cell_p.norm_sqr(), MPI_COMM_WORLD));

        IndexSet relevant_set;
        get_relevant_set(dof_handler, relevant_set);
        parallel::distributed::Vector<double> ghosted_sol(
            dof_handler.locally_owned_dofs(), relevant_set, solutions[d].get_mpi_communicator());
        ...
        VectorTools::integrate_difference(dof_handler,
            ghosted_sol,  // solutions[d],
            ExactSolution<dim>(dim, time, exactsolutionfuncno + d), norm_per_cell_p,
        ...
        norm_per_cell_p = 0;
        ghosted_sol = 0.0;
        VectorTools::integrate_difference(dof_handler, ghosted_sol,
            ExactSolution<dim>(dim, time, exactsolutionfuncno + d), norm_per_cell_p,
            QGauss<dim>(fe.degree + 3), VectorTools::L2_norm);
        double exact_sol_mag =
            std::sqrt(Utilities::MPI::sum(norm_per_cell_p.norm_sqr(), MPI_COMM_WORLD));

        pcout << "d " << d << " absolute error " << (error_mag) << " exact solution norm "
              << (exact_sol_mag) << " solution norm " << (solution_mag) << " at time " << time
              << std::endl;
      }
      ...
      //                                       QGauss<dim>(fe.degree+3),
      //                                       VectorTools::L2_norm);
      //    double error_post_mag = std::sqrt(Utilities::MPI::sum (norm_per_cell_p.norm_sqr(),
      //    MPI_COMM_WORLD)); pcout<<"post absolute error "<<(error_post_mag)<<std::endl;
    }
    else if (exactsolutionfuncno >= 0 && this->solid == true)
    {
      ...
      for (int d = 0; d < dim + 1 + dim * dim; ++d)
      {
        ...
        VectorTools::integrate_difference(dof_handler, ghosted_sol, ZeroFunction<dim>(1),
            norm_per_cell_v, QGauss<dim>(fe.degree + 3), VectorTools::L2_norm);
        ...
        norm_per_cell_v = 0;
        VectorTools::integrate_difference(dof_handler, ghosted_sol,
            ExactSolution<dim>(dim, time, exactsolutionfuncno + d), norm_per_cell_v,
            QGauss<dim>(fe.degree + 3), VectorTools::L2_norm);
        ...
        if (d < dim)
          pcout << "d " << d << " absolute error in v " << std::sqrt(v_error_mag)
                << " exakt solution norm " << std::sqrt(v_exactsol_mag) << std::endl;
        else if (d == dim)
          pcout << "d " << d << " absolute error in p " << std::sqrt(v_error_mag)
                << " exakt solution norm " << std::sqrt(v_exactsol_mag) << std::endl;
        else
          pcout << "d " << d << " absolute error in H " << std::sqrt(v_error_mag)
                << " exakt solution norm " << std::sqrt(v_exactsol_mag) << " solution norm "
                << std::sqrt(v_solution_mag) << std::endl;
      }
      ...
      // postprocessed solution
      {
        ...
        for (int d = 0; d < dim; ++d)
        {
          ...
          parallel::distributed::Vector<double> ghosted_sol(
              dof_handler_post_disp.locally_owned_dofs(), relevant_set,
          ...
          // calculate norm of difference between velocity and analytic solution
          norm_per_cell_v = 0;
          VectorTools::integrate_difference(dof_handler_post_disp, ghosted_sol,
              ExactSolution<dim>(dim, time, exactsolutionfuncno + d), norm_per_cell_v,
              QGauss<dim>(fe.degree + 3), VectorTools::L2_norm);
          v_error_mag = Utilities::MPI::sum(norm_per_cell_v.norm_sqr(), MPI_COMM_WORLD);
      ...
      ghosted_sol(dof_handler.locally_owned_dofs(),relevant_set,
       ...
       pcout<<"stress! absolute error in stressxy "<<std::sqrt(v_error_mag)<<" solution norm "<<
      std::sqrt(v_solution_mag)<<" exakt solution norm "<< std::sqrt(v_exactsol_mag) <<std::endl;
    ...
    {
      Vector<double> clusterids(triangulation.n_active_cells()),
      ...
      for (unsigned int i = 0; i < evaluator->get_matrix_free().n_macro_cells(); ++i)
        for (unsigned int v = 0; v < evaluator->get_matrix_free().n_components_filled(i); ++v)
        {
          typename Triangulation<dim>::cell_iterator cell =
              evaluator->get_matrix_free().get_cell_iterator(i, v);
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::write_restart(const unsigned int timestep_number)
  {
    ...
    {
      VectorView<value_type> tmp(solutions[0].local_size(), solutions[0].begin());
      oa << tmp;
      for (unsigned int i = 1; i < solutions.size(); i++)
      {
        tmp.reinit(solutions[i].local_size(), solutions[i].begin());
        oa << tmp;
      }
    }
    ...
    // write restart file
    {
      ...
      // if all procs write their data at the same time, this causes severe problems when going
      // above 1000 cores so the data is not written at once
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::read_restart(
      const unsigned int timestep_number, double &restarttime, bool fw)
  {
    std::string filename = bacitimeint->Params()->get<std::string>("name");
    if (!fw)
      filename.append("_adjo_");
    ...
    if (!in) dserror("file %s cannot be read for restart", filename.c_str());
    ...
    {
      ia >> tmp;
      std::copy(tmp.begin(), tmp.end(), solutions[i].begin());
    }
  ...
  void WaveEquationProblem<dim, Number>::run()
  {
    ...
    if (!invana || reduction) bacitimeint->InitMonitorFile();
    ...
    switch (dyna)
    {
      ...
      {
        integrator.reset(
            new LowStorageRK45Reg2<WaveEquationOperationBase<dim, Number>>(*evaluator));
        break;
      }
      case INPAR::ACOU::acou_lsrk33reg2:
      {
        integrator.reset(
            new LowStorageRK33Reg2<WaveEquationOperationBase<dim, Number>>(*evaluator));
        break;
      }
      case INPAR::ACOU::acou_lsrk45reg3:
      {
        integrator.reset(
            new LowStorageRK45Reg3<WaveEquationOperationBase<dim, Number>>(*evaluator));
        break;
      }
      case INPAR::ACOU::acou_ssprk:
      {
        integrator.reset(new StrongStabilityPreservingRK<WaveEquationOperationBase<dim, Number>>(
            *evaluator, 4, 8));
        break;
      }
      case INPAR::ACOU::acou_ader:
      {
        integrator.reset(
            new ArbitraryHighOrderDG<WaveEquationOperationBase<dim, Number>>(*evaluator));
        break;
      }
      case INPAR::ACOU::acou_ader_lts:
      {
        integrator.reset(
            new ArbitraryHighOrderDGLTS<WaveEquationOperationBase<dim, Number>>(*evaluator));
        break;
      }
      default:
    ...
    if (int(final_time / time_step) < maxstep) maxstep = final_time / time_step;
    int percent_count = 0;

    // the time loop
    for (time += time_step; time <= final_time + time_step / 100.0 && step < step_max;
         time += time_step, ++step)
    {
      ...
      // in case of adjoint run with acoustic parameter optimization, calculate the gradient
      // contributions
      if (invana && adjoint && acouopt)
        evaluator->compute_gradient_contributions(stored_forward_solutions[(step - 1) % up_res],
            stored_forward_solutions[(step - 1) % up_res + 1], solutions);
      ...
      {
        pcout << "TIME: " << std::setw(10) << std::setprecision(5) << time << "/" << std::setw(10)
              << std::setprecision(5) << final_time << " DT " << std::setw(10)
              << std::setprecision(5) << time_step << " STEP " << std::setw(6) << step << "/"
              << std::setw(6) << step_max << std::endl;
      }
      pcout << std::flush;
      timer.restart();
    ...
    if (invana && !adjoint && !reduction) bacitimeint->MonitorManager()->ConvolveSignals();

    pcout << std::endl << "   Performed " << step << " time steps." << std::endl;

    pcout << "   Average wallclock time per time step: " << wtime / step
          << "s, time per element: " << wtime / step / triangulation.n_active_cells() << "s"
          << std::endl;
  ...
  template <int dim, typename Number>
  void WaveEquationProblem<dim, Number>::intermediate_integrate(
      Teuchos::RCP<ExplicitIntegrator<WaveEquationOperationBase<dim, Number>>> integrator)
  {
    // only do this all in case of adjoint run and acoustical parameter optimization
    if (acouopt == false || adjoint == false) return;

    // only do this if we are at the right step
    if ((step) % up_res != 0) return;

    // save the adjoint solution and previous solution vector (such that the adjoint run can
    // continue afterwards without recognizing anything)
    std::vector<parallel::distributed::Vector<value_type>> adjoint_solutions(solutions.size());
    std::vector<parallel::distributed::Vector<value_type>> adjoint_previous_solutions(
        solutions.size());
    ...
    read_restart(restartstep / up_res, restarttime,
        true);  // sets restarttime to correct value and writes into solutions
