 ...
 *----------------------------------------------------------------------*/
ACOU::AcouImplicitTimeInt::AcouImplicitTimeInt(const Teuchos::RCP<DRT::DiscretizationHDG>& actdis,
      ...
      allelesequal_(DRT::INPUT::IntegralValue<bool>(*params_, "ALLELESEQUAL"))
{
  ...
  // service
  if (params_->get<int>("CALCERRORFUNCNO") > 0) calcerr_ = true;
  if (acouopt_ && adjoint_) outputcount_ = params_->get<int>("outputcount");

  // get the dof map
  const Epetra_Map* dofrowmap = discret_->DofRowMap();

  // create vector containing element based error values
  if (errormaps_) error_ = LINALG::CreateVector(*(discret_->ElementRowMap()), true);
  ...
  if (errormaps_ && !myrank_)
    std::cout << "Local postprocessing is only effective when temporal accuracy is of order k+2. "
                 "Did you choose your time integrator accordingly?"
              << std::endl;
...
void ACOU::AcouImplicitTimeInt::Integrate()
{
  ...
  // output of initial field (given by function for purely acoustic simulation or given by optics
  // for PAT simulation)
  Output();
...
void ACOU::AcouImplicitTimeInt::ApplyDirichletToSystem()
{
  ...
  LINALG::ApplyDirichlettoSystem(
      sysmat_, velnp_, residual_, Teuchos::null, zeros_, *(dbcmaps_->CondMap()));
  return;
}  // ApplyDirichletToSystem
...
void ACOU::AcouImplicitTimeInt::AssembleMatAndRHS()
{
  ...
  if (!padaptivity_) discret_->SetState("trace", velnp_);
  ...
  if (adjoint_ &&
      phys_ == INPAR::ACOU::acou_lossless)  // only needed for fluid, since the source term for the
                                            // solid is calculated directly in the update routine
  {
...
void ACOU::AcouImplicitTimeInt::UpdatePolyAndState()
{
   ...
   * is the following:
   * 1.) Do the local postprocessing, calculate delta_k (this is the amount the polynomial shape
   * function needs to change) 2.) Several things
   *     - Update the degree
   *     - Map / project the values
   *     - Rebuild the distributed vectors
   *     - Fill the distributed vectors
   * 3.) Do the next time step
   * UpdateInteriorVariables and ComputeResidual have to be separated (or not, if the element are
   * samrt)
   */
  ...
  for (int i = 0; i < discret_->NumMyColElements(); ++i)
    dynamic_cast<DRT::ELEMENTS::Acou*>(discret_->lColElement(i))
        ->SetDegree(int(error_->operator[](i)));
  ...
  if (!myrank_)
  {
    if (invana_)
    {
      ...
      printf("TIME: %11.4E/%11.4E  DT = %11.4E %s STEP = %4d/%4d, ts=%10.3E, te=%10.3E \n", time_,
          maxtime_, dtp_, Name().c_str(), step_, stepmax_, dtsolve_, dtele_);
  }
...
void ACOU::AcouImplicitTimeInt::NodalPressureField(Teuchos::RCP<Epetra_Vector> outvec)
{
  ...
  for (int i = 0; i < pressure->MyLength(); ++i) (*pressure)[i] /= touchCount[i];
...
void ACOU::AcouImplicitTimeInt::EvaluateErrorComparedToAnalyticalSol()
{
  if (calcerr_)
  {
    ...
    if (!myrank_)
    {
      // std::cout<<"time "<<time_<<" relative L2 pressure error "<<(*relerror)[0]<<" absolute L2
      // pressure error "<<sqrt((*errors)[0])<< " L2 pressure norm "<<sqrt((*errors)[1])<<std::endl;
      std::cout << "time " << time_ << " absolute L2 pressure error " << sqrt((*errors)[0])
                << " absolute L2 postprocessed pressure error " << sqrt((*errors)[2])
                << " L2 pressure norm " << sqrt((*errors)[1]) << std::endl;
      if (phys_ == INPAR::ACOU::acou_solid)
      {
        std::cout << "time " << time_ << " relative L2 velocity error " << (*relerror)[1]
                  << " absolute L2 velocity error " << sqrt((*errors)[2]) << " L2 velocity norm "
                  << sqrt((*errors)[3]) << std::endl;
        std::cout << "time " << time_ << " relative L2 velgradi error " << (*relerror)[2]
                  << " absolute L2 velgradi error " << sqrt((*errors)[4]) << " L2 velgradi norm "
                  << sqrt((*errors)[5]) << std::endl;
      }
    }
  }
  return;
...
ACOU::AcouTimeIntAderTriTet::AcouTimeIntAderTriTet(
    const Teuchos::RCP<DRT::DiscretizationHDG>& actdis, const Teuchos::RCP<LINALG::Solver>& solver,
...
void ACOU::AcouTimeIntAderTriTet::EvaluateErrorComparedToAnalyticalSol()
{
  if (calcerr_)
  {
    ...
    if (1)
    {
      ...
      residual_->Scale(0.0);
      discret_->Evaluate(
          params, Teuchos::null, Teuchos::null, residual_, Teuchos::null, Teuchos::null);
      ApplyDirichletToSystem();
      ...
      *error_post = 0.0;
      ...
      std::cout << "time " << time_ << " absolute L2 postprocessed pressure error "
                << sqrt(*error_post) << std::endl;
    }
  }
  return;
}
