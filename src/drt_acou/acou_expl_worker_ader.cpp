...
namespace ACOU
{
  ...
  WaveEquationOperationAcousticWaveADER<dim, fe_degree,
      Number>::WaveEquationOperationAcousticWaveADER(const std::vector<const DoFHandler<dim> *>
                                                         &dof_handlers,
            ...
            source_term, time_step_in, sourceno, monitormanagerin)
  {
    ...
    for (unsigned int d = 1; d < tempsrc.size(); ++d) tempsrc[d] = tempsrc[0];
  }

  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWaveADER<dim, fe_degree, Number>::local_apply_firstader_domain(
      ...
      const std::pair<unsigned int, unsigned int> &cell_range) const
  {
    ...
    FEEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> phi_eval(
        this->data);  // this->mass_matrix_data->phi[0];
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      integrate_taylor_cauchykovalewski(
          cell, phi_eval, phi_spectral, src, this->time_step, 0.0, 0.0, dst);
  ...
  template <int dim, int fe_degree, typename Number>
  void
  WaveEquationOperationAcousticWaveADER<dim, fe_degree, Number>::integrate_taylor_cauchykovalewski(
      ...
      const std::vector<parallel::distributed::Vector<value_type>> &recongraddiv) const
  {
    ...
    for (unsigned int q = 0; q < n_q_points; ++q)
    {
      ...
      for (unsigned int d = 0; d < dim; ++d) vcontrib[q][d] = (t2 - t1) * v_and_p[d];
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        if (spectral_evaluation)
        {
          ...
          for (unsigned int d = 1; d < dim; ++d)
            phi_spectral.begin_dof_values()[q + dim * n_q_points] +=
                c_sq * rho * phi_gradient[d][d];
        }
        else
        {
    ...
    for (unsigned int q = 0; q < n_q_points; ++q)
    {
      ...
      for (unsigned int d = 0; d < dim; ++d) v_and_p[d] = vcontrib[q][d];
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWaveADER<dim, fe_degree, Number>::local_apply_ader_face(
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWaveADER<dim, fe_degree, Number>::local_apply_secondader_domain(
      const MatrixFree<dim, value_type> &data,
      std::vector<parallel::distributed::Vector<value_type>> &dst,
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
  ...
  template <int dim, int fe_degree, typename Number>
  void WaveEquationOperationAcousticWaveADER<dim, fe_degree, Number>::apply_ader(
      const std::vector<parallel::distributed::Vector<value_type>> &src,
      ...
      const double &dt) const
  {
    ...
    // first ader step
    timer.restart();
    this->data.cell_loop(&WaveEquationOperationAcousticWaveADER<dim, fe_degree,
                             Number>::local_apply_firstader_domain,
        this, tempsrc, src);
    this->computing_times[4] += timer.wall_time();
    timer.restart();
    this->data.loop(&WaveEquationOperationAcousticWaveADER<dim, fe_degree,
