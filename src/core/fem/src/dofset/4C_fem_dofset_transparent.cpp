    ...
    const int start)
{
  ...
  dofrowmap_ =
      std::make_shared<Epetra_Map>(-1, dofrowvec.size(), dofrowvec.data(), 0, newdis.get_comm());
  ...
  dofcolmap_ =
      std::make_shared<Epetra_Map>(-1, dofcolvec.size(), dofcolvec.data(), 0, newdis.get_comm());
}
