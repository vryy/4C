...
std::shared_ptr<Core::Elements::Element> Core::Communication::factory(
    const std::string& eletype, const Core::FE::CellType celltype, const int id, const int owner)
{
  return ParObjectFactory::instance().create(eletype, celltype, id, owner);
}
