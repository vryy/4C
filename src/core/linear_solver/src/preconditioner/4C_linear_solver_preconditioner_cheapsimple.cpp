    ...
    const Teuchos::ParameterList& origvlist, const Teuchos::ParameterList& origplist)
{
  ...
  //-------------------------------------------------------------------------
  {
    ...
    diag_ainv_ = Teuchos::make_rcp<Core::LinAlg::SparseMatrix>(diag);
...
#if CHEAPSIMPLE_ALGORITHM
  {
    ...
    if (visml)
    {
      ...
      ppredict_ = Teuchos::make_rcp<ML_Epetra::MultiLevelPreconditioner>(
    ...
    if (pisml)
    {
      ...
      pschur_ = Teuchos::make_rcp<ML_Epetra::MultiLevelPreconditioner>(
  ...
  //-------------------------------------------------------------------------
  {
    Teuchos::RCP<Teuchos::ParameterList> vrcplist = Teuchos::rcpFromRef(predictSolver_list_);
    vsolver_ = Teuchos::make_rcp<Core::LinAlg::Solver>(vrcplist, A_->Comm(), outfile_);
    Teuchos::RCP<Teuchos::ParameterList> prcplist = Teuchos::rcpFromRef(schurSolver_list_);
    psolver_ = Teuchos::make_rcp<Core::LinAlg::Solver>(prcplist, A_->Comm(), outfile_);
  }
#endif
  ...
  //-------------------------------------------------------------------------
  vx_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(0), false);
  vb_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(0), false);
  px_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(1), false);
  pb_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(1), false);

  //-------------------------------------------------------------------------
  // Allocate working vectors for velocity and pressure
  //-------------------------------------------------------------------------
  vwork1_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(0), false);
  vwork2_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(0), false);
  pwork1_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(1), false);
  pwork2_ = Teuchos::make_rcp<Core::LinAlg::Ana::Vector>(*mmex_.Map(1), false);
