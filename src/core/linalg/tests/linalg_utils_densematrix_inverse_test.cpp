...
namespace
{
  TEST(LinalgDenseMatrixInverseTest, 2x2InverseReorder)
  {
    ...
    LINALG::InverseReorderMatrixEntries(A);
  ...
  TEST(LinalgDenseMatrixInverseTest, 2x2InverseSingular)
  {
    ...
    A_singular(0, 0) = 1.0;
    EXPECT_ANY_THROW(LINALG::Inverse(A_singular));
  }
  ...
  TEST(LinalgDenseMatrixInverseTest, 2x2InverseDoNotThrowErrorOnZeroDeterminant)
  {
    ...
    EXPECT_TRUE(LINALG::InverseDoNotThrowErrorOnZeroDeterminant(A, 1e-12));
  ...
  TEST(LinalgDenseMatrixInverseTest, 2x2InverseDoNotThrowErrorOnZeroDeterminantSingular)
  {
    LINALG::Matrix<2, 2, double> A_singular(true);
    A_singular(0, 0) = 1.0;
    EXPECT_TRUE(not LINALG::InverseDoNotThrowErrorOnZeroDeterminant(A_singular, 1e-12));
  }
  ...
  {
    LINALG::Matrix<4, 4, double> A(true);
    LINALG::Matrix<4, 1, double> b(true);
    LINALG::Matrix<4, 1, double> x(true);
    ...
    EXPECT_TRUE(LINALG::SolveLinearSystemDoNotThrowErrorOnZeroDeterminantScaled(A, b, x, eps_det));
