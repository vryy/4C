 ...
 *----------------------------------------------------------------------*/
int DRT::UTILS::getNumberOfElementNodes(const DRT::Element::DiscretizationType& distype)
{
  int numnodes = 0;
...
/*----------------------------------------------------------------------*
 ...
 |  for each discretization type                                        |
 *----------------------------------------------------------------------*/
int DRT::UTILS::getNumberOfElementCornerNodes(const DRT::Element::DiscretizationType& distype)
{
    ...
    const DRT::Element::DiscretizationType& distype)
{
  ...
  switch (distype)
  {
    ...
    case DRT::Element::hex27:
    {
      ...
      for (int i = 0; i < nSurf; i++) surfNodeMap.push_back(nCornerNode);
      break;
    }
    case DRT::Element::tet4:
    case DRT::Element::tet10:
...
/*----------------------------------------------------------------------*
 ...
 |  for each discretization type                                        |
 *----------------------------------------------------------------------*/
int DRT::UTILS::getNumberOfElementLines(const DRT::Element::DiscretizationType& distype)
{
  int numLines = 0;
  switch (distype)
  {
...
/*----------------------------------------------------------------------*
 ...
 |  for each discretization type                                        |
 *----------------------------------------------------------------------*/
int DRT::UTILS::getNumberOfElementSurfaces(const DRT::Element::DiscretizationType& distype)
{
  int numSurf = 0;
  switch (distype)
  {
...
/*----------------------------------------------------------------------*
 ...
 |  for each discretization type                                        |
 *----------------------------------------------------------------------*/
int DRT::UTILS::getNumberOfElementVolumes(const DRT::Element::DiscretizationType& distype)
{
  int numVol = 0;
  switch (distype)
  {
...
/*----------------------------------------------------------------------*
 ...
 |  for each discretization type                                        |
 *----------------------------------------------------------------------*/
int DRT::UTILS::getNumberOfElementFaces(const DRT::Element::DiscretizationType& distype)
{
 ...
 *----------------------------------------------------------------------*/
DRT::Element::DiscretizationType DRT::UTILS::getEleFaceShapeType(
    ...
    const DRT::Element::DiscretizationType distype)
{
      ...
      distype == DRT::Element::hex27)
  {
    ...
    {
      map.push_back(submap);
      for (int j = 0; j < nSurf; j++) map[i][j] = eleNodeNumbering_hex27_nodes_surfaces[i][j];
    }
  }
  else if (distype == DRT::Element::tet4 || distype == DRT::Element::tet10)
  {
    ...
    {
      map.push_back(submap);
      for (int j = 0; j < nSurf; j++) map[i][j] = eleNodeNumbering_tet10_nodes_surfaces[i][j];
    }
  }
  else
    dserror("discretization type %s not yet implemented", (DRT::DistypeToString(distype)).c_str());
    ...
    const LINALG::Matrix<3, 1>& rst, const DRT::Element::DiscretizationType distype)
{
  ...
  switch (distype)
  {
    ...
    case DRT::Element::hex27:
    {
      if (fabs(rst(0) + 1.0) < TOL && fabs(rst(1) + 1.0) < TOL && fabs(rst(2) + 1.0) < TOL)
        node = 0;  // -r -s -t
      else if (fabs(rst(0) - 1.0) < TOL && fabs(rst(1) + 1.0) < TOL && fabs(rst(2) + 1.0) < TOL)
        node = 1;  // +r -s -t
      else if (fabs(rst(0) - 1.0) < TOL && fabs(rst(1) - 1.0) < TOL && fabs(rst(2) + 1.0) < TOL)
        node = 2;  // +r +s -t
      else if (fabs(rst(0) + 1.0) < TOL && fabs(rst(1) - 1.0) < TOL && fabs(rst(2) + 1.0) < TOL)
    ...
    case DRT::Element::tet10:
    {
      if (fabs(rst(0)) < TOL && fabs(rst(1)) < TOL && fabs(rst(2)) < TOL)
        node = 0;  // 0 0 0
      else if (fabs(rst(0) - 1.0) < TOL && fabs(rst(1)) < TOL && fabs(rst(2)) < TOL)
        node = 1;  // 1 0 0
      else if (fabs(rst(0)) < TOL && fabs(rst(1) - 1.0) < TOL && fabs(rst(2)) < TOL)
        node = 2;  // 0 1 0
      else if (fabs(rst(0)) < TOL && fabs(rst(1)) < TOL && fabs(rst(2) - 1.0) < TOL)
        node = 3;  // 0 0 1
 ...
 *----------------------------------------------------------------------*/
LINALG::Matrix<3, 1> DRT::UTILS::getNodeCoordinates(
...
int DRT::UTILS::getHigherOrderIndex(
    const int index1, const int index2, const DRT::Element::DiscretizationType distype)
{
  ...
  switch (distype)
  {
    case DRT::Element::tet10:
    {
      if ((index1 == 0 && index2 == 1) || (index1 == 1 && index2 == 0))
        ...
        dserror("no valid tet10 edge found");
      break;
    }
    case DRT::Element::quad9:
    {
      if ((index1 == 0 && index2 == 1) || (index1 == 1 && index2 == 0))
        ...
        higherOrderIndex = 6;
      else if ((index1 == 3 && index2 == 0) || (index1 == 0 && index2 == 3))
        higherOrderIndex = 7;
      else
        dserror("no valid quad9 edge found");
      break;
    }
    case DRT::Element::tri6:
    {
...
int DRT::UTILS::getDimension(const DRT::Element::DiscretizationType distype)
{
  ...
  {
    case DRT::Element::line2:
      dim = DisTypeToDim<DRT::Element::line2>::dim;
      break;
    case DRT::Element::line3:
      dim = DisTypeToDim<DRT::Element::line3>::dim;
      break;
    case DRT::Element::nurbs2:
      dim = DisTypeToDim<DRT::Element::nurbs2>::dim;
      break;
    case DRT::Element::nurbs3:
      dim = DisTypeToDim<DRT::Element::nurbs3>::dim;
      break;
    case DRT::Element::quad4:
      dim = DisTypeToDim<DRT::Element::quad4>::dim;
      break;
    case DRT::Element::quad8:
      dim = DisTypeToDim<DRT::Element::quad8>::dim;
      break;
    case DRT::Element::quad9:
      dim = DisTypeToDim<DRT::Element::quad9>::dim;
      break;
    case DRT::Element::tri3:
      dim = DisTypeToDim<DRT::Element::tri3>::dim;
      break;
    case DRT::Element::tri6:
      dim = DisTypeToDim<DRT::Element::tri6>::dim;
      break;
    case DRT::Element::nurbs4:
      dim = DisTypeToDim<DRT::Element::nurbs4>::dim;
      break;
    case DRT::Element::nurbs9:
      dim = DisTypeToDim<DRT::Element::nurbs9>::dim;
      break;
    case DRT::Element::hex8:
      dim = DisTypeToDim<DRT::Element::hex8>::dim;
      break;
    case DRT::Element::nurbs8:
      dim = DisTypeToDim<DRT::Element::nurbs8>::dim;
      break;
    case DRT::Element::hex20:
      dim = DisTypeToDim<DRT::Element::hex20>::dim;
      break;
    case DRT::Element::hex27:
      dim = DisTypeToDim<DRT::Element::hex27>::dim;
      break;
    case DRT::Element::nurbs27:
      dim = DisTypeToDim<DRT::Element::nurbs27>::dim;
      break;
    case DRT::Element::tet4:
      dim = DisTypeToDim<DRT::Element::tet4>::dim;
      break;
    case DRT::Element::tet10:
      dim = DisTypeToDim<DRT::Element::tet10>::dim;
      break;
    case DRT::Element::wedge6:
      dim = DisTypeToDim<DRT::Element::wedge6>::dim;
      break;
    case DRT::Element::wedge15:
      dim = DisTypeToDim<DRT::Element::wedge15>::dim;
      break;
    case DRT::Element::pyramid5:
      dim = DisTypeToDim<DRT::Element::pyramid5>::dim;
      break;
    case DRT::Element::point1:
      dim = DisTypeToDim<DRT::Element::point1>::dim;
      break;
    default:
      dserror(
          "discretization type %s not yet implemented", (DRT::DistypeToString(distype)).c_str());
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
double DRT::UTILS::getSizeInLocalCoordinates(const DRT::Element::DiscretizationType distype)
{
  double size = 0.0;
  switch (distype)
  {
