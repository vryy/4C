...
/*----------------------------------------------------------------------*
 |  Fills a vector< vector<int> > with all nodes for         gammi 08/08|
 |  every volume for each discretization type                           |
 *----------------------------------------------------------------------*/
vector< vector<int> > DRT::UTILS::getEleNodeNumberingVolumes(
    const DRT::Element::DiscretizationType&     distype)
{

  // volumeid -> node numbering of volume compared to parent
  // 
  // at the moment we normally have one volume per element, and we
  // use the identical node numbering for volume element and element
  // (basically, the volume is a copy of the geometry of the element)
  vector< vector<int> >   map;
