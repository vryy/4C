...
namespace Adapter
{
  class FBIStructureWrapper : public FSIStructureWrapper
  {
   public:
    ...
    virtual Teuchos::RCP<Core::LinAlg::Vector> extract_interface_veln();

    /// extracts interface velocities at \f$t_{n+1}\f$
    virtual Teuchos::RCP<Core::LinAlg::Vector> extract_interface_velnp();

    /// Predictor for interface velocities
    virtual Teuchos::RCP<Core::LinAlg::Vector> predict_interface_velnp();
    ...
    Teuchos::RCP<Core::LinAlg::Vector> extract_interface_dispn() override;

    /// extract interface displacements at \f$t_{n+1}\f$
    Teuchos::RCP<Core::LinAlg::Vector> extract_interface_dispnp() override;

    /// Predictor for interface displacements
    Teuchos::RCP<Core::LinAlg::Vector> predict_interface_dispnp() override;
     ...
     */
    void apply_interface_forces(Teuchos::RCP<Core::LinAlg::Vector> iforce) override;
