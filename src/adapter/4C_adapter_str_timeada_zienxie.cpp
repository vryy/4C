...
void Adapter::StructureTimeAdaZienXie::integrate_step_auxiliar()
{
  ...
  Teuchos::RCP<const Core::LinAlg::Vector> dis = gstate.get_dis_n();    // D_{n}^{A2}
  Teuchos::RCP<const Core::LinAlg::Vector> vel = gstate.get_vel_n();    // V_{n}^{A2}
  Teuchos::RCP<const Core::LinAlg::Vector> acc = gstate.get_acc_n();    // A_{n}^{A2}
  Teuchos::RCP<const Core::LinAlg::Vector> accn = gstate.get_acc_np();  // A_{n+1}^{A2}
