...
void Adapter::StructureTimeAdaZienXie::integrate_step_auxiliar()
{
  ...
  std::shared_ptr<const Core::LinAlg::Vector<double>> dis = gstate.get_dis_n();    // D_{n}^{A2}
  std::shared_ptr<const Core::LinAlg::Vector<double>> vel = gstate.get_vel_n();    // V_{n}^{A2}
  std::shared_ptr<const Core::LinAlg::Vector<double>> acc = gstate.get_acc_n();    // A_{n}^{A2}
  std::shared_ptr<const Core::LinAlg::Vector<double>> accn = gstate.get_acc_np();  // A_{n+1}^{A2}
