...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
IO::DiscretizationReader::DiscretizationReader(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IO::DiscretizationReader::ReadCharVector(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
IO::DiscretizationWriter::DiscretizationWriter(const IO::DiscretizationWriter& writer,
...
IO::DiscretizationWriter::~DiscretizationWriter()
{
  ...
  if (resultfile_ != -1)
  {
    ...
    for (int i = 0; i < num_og; i++)
    {
      ...
      if (status_g < 0) dserror("Failed to close HDF-group in resultfile");
    }
...
void IO::DiscretizationWriter::CreateMeshFile(const int step)
{
  if (binio_)
  {
    ...
    if (meshfile_ < 0) dserror("Failed to open file %s", meshname.str().c_str());
...
void IO::DiscretizationWriter::CreateResultFile(const int step)
{
  if (binio_)
  {
    ...
    if (resultfile_ < 0) dserror("Failed to open file %s", resultname.str().c_str());
...
void IO::DiscretizationWriter::NewStep(const int step, const double time)
{
  if (binio_)
  {
    ...
    if (resultgroup_ < 0) dserror("Failed to write HDF-group in resultfile");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IO::DiscretizationWriter::WriteVector(
  ...
  if (binio_)
  {
    ...
    else
    {
      ...
      if (size != 0)
      {
        ...
        if (make_status < 0) dserror("Failed to create dataset in HDF-resultfile");
      }
      else
      {
        const herr_t make_status =
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IO::DiscretizationWriter::WriteVector(const std::string name, const std::vector<char>& vec,
    const Epetra_Map& elemap, IO::VectorType vt)
{
  if (binio_)
  {
    std::string valuename = name + ".values";
...
void IO::DiscretizationWriter::WriteMesh(const int step, const double time)
{
  if (binio_)
  {
    ...
    if (meshgroup_ < 0) dserror("Failed to write group in HDF-meshfile");
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void IO::DiscretizationWriter::WriteMesh(
...
void IO::DiscretizationWriter::WriteNodeData(bool writeowner)
{
  if (binio_)
  {
    ...
    for (fool = names.begin(); fool != names.end(); ++fool)
    {
      ...
      WriteVector(fool->first, Teuchos::rcp(&sysdata, false), IO::nodevector);
    ...
    const std::string name, Teuchos::RCP<std::vector<char>> charvec)
{
  if (binio_)
  {
    ...
    const herr_t flush_status = H5Fflush(resultgroup_, H5F_SCOPE_LOCAL);
    if (flush_status < 0) dserror("Failed to flush HDF file %s", resultfilename_.c_str());
  }
}
