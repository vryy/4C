...
void FSI::SlidingMonolithicFluidSplit::setup_rhs_firstiter(Epetra_Vector& f)
{
  ...
  duiinc_ = Core::LinAlg::CreateVector(*fsi_fluid_field()->fsi_interface()->other_map(), true);
    ...
    Teuchos::RCP<Core::FE::Discretization> fluiddis, const Inpar::FSI::Redistribute domain)
{
  ...
  Teuchos::RCP<Core::LinAlg::SparseMatrix> P =
      coupsfm_->get_mortar_matrix_p();  // P matrix that couples fluid dofs with structure dofs
