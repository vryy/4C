...
namespace FSI
{
  ...
  class SlidingMonolithicFluidSplit : public BlockMonolithic
  {
    ...
    */
    ...
    std::shared_ptr<Adapter::FluidFSIMsht> fsi_fluid_field()
    {
      return std::static_pointer_cast<Adapter::FluidFSIMsht>(fluid_field());
    }

    std::shared_ptr<Adapter::AleFsiMshtWrapper> fsi_ale_field()
    {
      return std::static_pointer_cast<Adapter::AleFsiMshtWrapper>(ale_field());
    }
