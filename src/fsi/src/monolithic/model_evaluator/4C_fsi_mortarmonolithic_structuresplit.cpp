...
void FSI::MortarMonolithicStructureSplit::setup_system()
{
  if (notsetup_)
  {
    ...
    if (aleproj_ != Inpar::FSI::ALEprojection_none)
    {
      ...
      iprojdisp_ =
          Teuchos::make_rcp<Core::LinAlg::Vector<double>>(*coupsfm_->master_dof_map(), true);
      iprojdispinc_ =
          Teuchos::make_rcp<Core::LinAlg::Vector<double>>(*coupsfm_->master_dof_map(), true);
    }
    notsetup_ = false;
  ...
  if (restart)
  {
    ...
    if (restartfrompartfsi)  // restart from part. fsi
    {
      ...
      //      Teuchos::RCP<Core::LinAlg::Vector<double>> lambdafullfluid = Teuchos::rcp(new
      ...
      //
      //      Teuchos::RCP<Core::LinAlg::Vector<double>> lambdafluid = Teuchos::rcp(new
...
void FSI::MortarMonolithicStructureSplit::setup_rhs_firstiter(Core::LinAlg::Vector<double>& f)
{
   ...
   */
  // ----------addressing term 1
  rhs = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(sig.range_map(), true);
  ...
  if (mmm != Teuchos::null)
  {
    ...
    rhs = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(fmgg.range_map(), true);
  ...
  auxvec = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(sgg.range_map(), true);
...
void FSI::MortarMonolithicStructureSplit::update()
{
  ...
  if (aleproj_ != Inpar::FSI::ALEprojection_none)
  {
    iprojdisp_ = Teuchos::make_rcp<Core::LinAlg::Vector<double>>(*coupsfm_->master_dof_map(), true);
