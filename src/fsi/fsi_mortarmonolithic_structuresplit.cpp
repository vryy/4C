...
void FSI::MortarMonolithicStructureSplit::SetupSystem()
{
  if (notsetup_)
  {
    ...
    coupsfm_->Setup(FluidField()->Discretization(), StructureField()->Discretization(),
        AleField()->WriteAccessDiscretization(), coupleddof, "FSICoupling", comm_, false);
    ...
    {
      // MeshInit possibly modifies reference configuration of slave side --> recompute element
      // volume in InitializeElements()
  ...
  if (restart)
  {
    ...
    if (restartfrompartfsi)  // restart from part. fsi
    {
      ...
      //      Epetra_Vector(*FluidField()->DofRowMap(),true)); IO::DiscretizationReader reader =
      ...
      //      Epetra_Vector(*FluidField()->Interface()->FullMap(),true));
      //
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::SetupDBCMapExtractor()
{
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
Teuchos::RCP<LINALG::BlockSparseMatrixBase> FSI::MortarMonolithicStructureSplit::SystemMatrix()
    const
{
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::SetupRHSResidual(Epetra_Vector& f)
{
  ...
  const Teuchos::RCP<LINALG::SparseMatrix> mortarp = coupsfm_->GetPMatrix();
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::SetupRHSLambda(Epetra_Vector& f)
{
  if (lambdaold_ != Teuchos::null)
  {
    // get time integration parameters of structure and fluid time integrators
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::SetupRHSFirstiter(Epetra_Vector& f)
{
  // get time integration parameters of structure and fluid time integrators
  // to enable consistent time integration among the fields
  const double stiparam = StructureField()->TimIntParam();
  ...
  const Teuchos::RCP<const LINALG::SparseMatrix> mortarp = coupsfm_->GetPMatrix();
...
void FSI::MortarMonolithicStructureSplit::SetupSystemMatrix(LINALG::BlockSparseMatrixBase& mat)
{
  ...
  // get the Mortar projection matrix P = D^{-1} * M
  Teuchos::RCP<LINALG::SparseMatrix> mortarp = coupsfm_->GetPMatrix();
  ...
  if (!s->Matrix(0, 1).DomainMap().PointSameAs(mortarp->RangeMap())) dserror("Maps do not match.");
  if (!s->Matrix(1, 0).RangeMap().PointSameAs(mortarp->RangeMap())) dserror("Maps do not match.");
  if (!s->Matrix(1, 1).DomainMap().PointSameAs(mortarp->RangeMap())) dserror("Maps do not match.");
#endif
  ...
  (*aigtransform_)(a->FullRowMap(), a->FullColMap(), aig, 1. / timescale,
      ADAPTER::CouplingSlaveConverter(InterfaceFluidAleCoupling()), mat.Matrix(2, 1));
...
void FSI::MortarMonolithicStructureSplit::Update()
{
  ...
  if (aleproj_ != INPAR::FSI::ALEprojection_none)
  {
    ...
    slideale_->Remeshing(
        *StructureField(), FluidField()->Discretization(), idispale, iprojdisp_, *coupsfm_, Comm());
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::ScaleSystem(
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::UnscaleSolution(
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
Teuchos::RCP<NOX::StatusTest::Combo> FSI::MortarMonolithicStructureSplit::CreateStatusTest(
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::CalculateInterfaceEnergyIncrement()
{
  // get time integration parameters of structure and fluid time integrators
  // to enable consistent time integration among the fields
  const double stiparam = StructureField()->TimIntParam();
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::CombineFieldVectors(Epetra_Vector& v,
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
double FSI::MortarMonolithicStructureSplit::SelectDtErrorBased() const
{
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
bool FSI::MortarMonolithicStructureSplit::SetAccepted() const
{
  // get error norms
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void FSI::MortarMonolithicStructureSplit::CreateNodeOwnerRelationship(std::map<int, int>* nodeOwner,
    std::map<int, std::list<int>>* inverseNodeOwner, std::map<int, DRT::Node*>* structurenodesPtr,
    std::map<int, DRT::Node*>* fluidgnodesPtr, Teuchos::RCP<DRT::Discretization> structuredis,
    Teuchos::RCP<DRT::Discretization> fluiddis, const INPAR::FSI::Redistribute domain)
{
  ...
  Teuchos::RCP<LINALG::SparseMatrix> P =
      coupsfm_->GetPMatrix();  // P matrix that couples structure dofs with fluid dofs
    ...
    for (int j = 0; j < maxNumEntries; ++j)
    {
      ...
      for (int proc = 0; proc < numproc; ++proc)
      {
        ...
        if (copy == -2)
        {
          ...
          FindNodeRelatedToDof(fluidgnodesPtr, dofid, fluiddis,
