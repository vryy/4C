...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
NOX::NLN::StatusTest::NormF::NormF(
    ...
    Teuchos::RCP<const NOX::Abstract::Group>& initialGuess)
{
  ...
  if (initialTolerance_.is_null())
  {
    utils_.err() << "NOX::NLN::StatusTest::NormF::NormF - F was not computed for the"
        " given nox_abstract_group!" << std::endl;
    throw "NOX Error";
  }
    ...
    NOX::StatusTest::CheckType checkType)
{
  ...
  else
  {
    ...
    for (std::size_t i=0;i<normF_->size();++i)
    {
      ...
      if (status_[i] == NOX::StatusTest::Unconverged)
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
NOX::StatusTest::StatusType NOX::NLN::StatusTest::NormF::getStatus() const
{
  return gStatus_;
}
