    ...
    Teuchos::ParameterList& params) const
{
  ...
  if (method == "Full Step")
    line_search = Teuchos::make_rcp<::NOX::LineSearch::FullStep>(gd, params);
  else if (method == "Backtrack")
  {
    line_search =
        Teuchos::make_rcp<NOX::Nln::LineSearch::Backtrack>(gd, outerTests, innerTests, params);
  }
  else
  {
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Teuchos::RCP<::NOX::LineSearch::Generic> NOX::Nln::LineSearch::build_line_search(
    const Teuchos::RCP<::NOX::GlobalData>& gd,
    const Teuchos::RCP<::NOX::StatusTest::Generic> outerTests,
    const Teuchos::RCP<NOX::Nln::Inner::StatusTest::Generic> innerTests,
