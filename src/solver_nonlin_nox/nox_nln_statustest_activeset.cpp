  ...
  else
  {
    ...
    else
    {
      ...
      if (max_cycle_size_>0)
      {
        ...
        Teuchos::RCP<const Epetra_Map> activeset =
            cnlngrp->GetCurrentActiveSetMap(qtype_);
    ...
    int indent) const
{
  ...
  // optional output
  if (cycle_size_>0)
    stream << indent_string << std::setw(13) << " "<< "WARNING: "
        "The active set cycles between iteration (k) and (k-" << cycle_size_ << ")!" << std::endl;
