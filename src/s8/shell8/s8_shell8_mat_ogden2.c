    ...
    DOUBLE **gmkovr, DOUBLE **gmkonr, DOUBLE **gmkovc, DOUBLE **gmkonc)
{
  ...
  */
  ...
  if (fabs(lambda[0] - lambda[1]) < 1e-9) printf("lambda[0]==lambda[1]\n");
  if (fabs(lambda[0] - lambda[2]) < 1e-9) printf("lambda[0]==lambda[2]\n");
  if (fabs(lambda[1] - lambda[2]) < 1e-9) printf("lambda[1]==lambda[2]\n");
  /* test orthogonality of eigenvectors */
  /* N0 * N1 = 0.0 */
  work = N[0][0] * N[1][0] + N[0][1] * N[1][1] + N[0][2] * N[1][2];
  if (fabs(work) > 1e-10) printf("eigenvectors N0 * N1 = %f\n", work);
  /* N0 * N2 = 0.0 */
  work = N[0][0] * N[2][0] + N[0][1] * N[2][1] + N[0][2] * N[2][2];
  if (fabs(work) > 1e-10) printf("eigenvectors N0 * N2 = %f\n", work);
  /* N1 * N2 = 0.0 */
  work = N[1][0] * N[2][0] + N[1][1] * N[2][1] + N[1][2] * N[2][2];
  if (fabs(work) > 1e-10) printf("eigenvectors N1 * N2 = %f\n", work);
  /* test length==1 of eigenvectors */
  work = N[0][0] * N[0][0] + N[0][1] * N[0][1] + N[0][2] * N[0][2];
  work = sqrt(work);
  if (fabs(work - 1.0) > 1e-10) printf("eigenvectors N0 not unit length %f\n", work);
  work = N[1][0] * N[1][0] + N[1][1] * N[1][1] + N[1][2] * N[1][2];
  work = sqrt(work);
  if (fabs(work - 1.0) > 1e-10) printf("eigenvectors N1 not unit length %f\n", work);
  work = N[2][0] * N[2][0] + N[2][1] * N[2][1] + N[2][2] * N[2][2];
  work = sqrt(work);
  if (fabs(work - 1.0) > 1e-10) printf("eigenvectors N2 not unit length %f\n", work);
