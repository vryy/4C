...
void *amdef(char *namstr, ARRAY *a, int fdim, int sdim, char typstr[])
{
  ...
  if (strncmp("IA", typstr, 2) == 0)
  {
    a->Typ = cca_IA;
    goto next;
  }
  if (strncmp("DV", typstr, 2) == 0)
  {
    a->Typ = cca_DV;
    goto next;
  }
  if (strncmp("IV", typstr, 2) == 0)
  {
    a->Typ = cca_IV;
    goto next;
  }
...
void *amredef(ARRAY *a, int newfdim, int newsdim, char newtypstr[])
{
  ...
  if (strncmp("IV", newtypstr, 2) == 0)
  {
    newtyp = newIV;
    goto next;
  }
  if (strncmp("IA", newtypstr, 2) == 0)
  {
    newtyp = newIA;
    goto next;
  }
  if (strncmp("DV", newtypstr, 2) == 0)
  {
    newtyp = newDV;
    goto next;
  }
  if (strncmp("DA", newtypstr, 2) == 0)
  {
    newtyp = newDA;
    goto next;
  }
    ...
    case newDV:       /*---------------------------------------------new typ is DV */
      switch (a->Typ) /*------------------------- what is the old array typ */
      {
        case cca_DV: /*---------------------------------------conversion DV to DV */
          ...
          amzero(a);
          size1 = IMIN(newfdim * newsdim, copyarray.fdim * copyarray.sdim);
          for (i = 0; i < size1; ++i) a->a.dv[i] = copyarray.a.dv[i];
          amdel(&copyarray);
          goto end;
        ...
        case cca_DA: /*---------------------------------------conversion DA to DV */
          ...
          {
            size1 = IMIN(newsdim, copyarray.sdim);
            for (i = 0; i < size1; ++i) a->a.dv[i] = copyarray.a.da[0][i];
          }
          else
          {
            size1 = IMIN(newfdim, copyarray.fdim);
            for (i = 0; i < size1; ++i) a->a.dv[i] = copyarray.a.da[i][0];
          }
          amdel(&copyarray);
          goto end;
        default:
    ...
    case newDA:       /*---------------------------------------------new typ is DA */
      switch (a->Typ) /*------------------------- what is the old array typ */
      {
        case cca_DV: /*---------------------------------------conversion DV to DA */
          ...
          {
            size1 = IMIN(newsdim, copyarray.sdim);
            for (i = 0; i < size1; ++i) a->a.da[0][i] = copyarray.a.dv[i];
          }
          else
          {
            size1 = IMIN(newfdim, copyarray.fdim);
            for (i = 0; i < size1; ++i) a->a.da[i][0] = copyarray.a.dv[i];
          }
          amdel(&copyarray);
          goto end;
...
void *amcopy(ARRAY *array_from, ARRAY *array_to)
{
  ...
  if (array_from->Typ != array_to->Typ) dserror("mismatching typ of ARRAYs, cannot copy ARRAYs");
  /*----------------------------------------------------------------------*/
  switch (array_from->Typ)
  {
    case cca_DA:
...
void amadd(ARRAY *array_to, ARRAY *array_from, double factor, int init)
{
  ...
  switch (array_from->Typ)
  {
    case cca_DA:
      ...
      dato = array_to->a.da;
      for (i = 0; i < fdim; i++)
        for (j = 0; j < sdim; j++) dato[i][j] += dafrom[i][j] * factor;
      break;
    case cca_DV:
      ...
      dvto = array_to->a.dv;
      for (i = 0; i < fdim; i++) dvto[i] += dvfrom[i] * factor;
      break;
    case cca_IA:
      fdim = IMIN(array_to->fdim, array_from->fdim);
      sdim = IMIN(array_to->sdim, array_from->sdim);
...
void *am4def(char *namstr, ARRAY4D *a, int fdim, int sdim, int tdim, int fodim, char typstr[])
{
  ...
  a->tdim = tdim;
  a->fodim = fodim;
  if (strncmp("D3", typstr, 2) == 0)
  {
    a->Typ = cca_D3;
    goto next;
  }
  if (strncmp("D4", typstr, 2) == 0)
  {
    a->Typ = cca_D4;
    goto next;
  }
  if (strncmp("I3", typstr, 2) == 0)
  {
    a->Typ = cca_I3;
    goto next;
  }
  if (strncmp("I4", typstr, 2) == 0)
  {
    a->Typ = cca_I4;
    goto next;
  }
...
void *am4_alloc_copy(ARRAY4D *array_from, ARRAY4D *array_to)
{
  ...
  switch (array_from->Typ)
  {
    case cca_D3:
      ...
      am4def(array_from->name, array_to, array_from->fdim, array_from->sdim, array_from->tdim, 0,
          "D3");
    ...
    case cca_D4:
      ...
      am4def(array_from->name, array_to, array_from->fdim, array_from->sdim, array_from->tdim,
          array_from->fodim, "D4");
...
void *am4redef(ARRAY4D *array, int newfdim, int newsdim, int newtdim, int newfodim)
{
  ...
  switch (array->Typ)
  {
    case cca_D3:
      ...
      for (i = 0; i < size1; i++)
        for (j = 0; j < size2; j++)
          for (k = 0; k < size3; k++) array->a.d3[i][j][k] = copyarray.a.d3[i][j][k];
      am4del(&copyarray);
      break;
    case cca_D4:
      am4_alloc_copy(array, &copyarray);
      ...
      for (i = 0; i < size1; i++)
        for (j = 0; j < size2; j++)
          for (k = 0; k < size3; k++)
            for (l = 0; l < size4; l++) array->a.d4[i][j][k][l] = copyarray.a.d4[i][j][k][l];
      am4del(&copyarray);
      break;
    case cca_I3:
      am4_alloc_copy(array, &copyarray);
      ...
      for (i = 0; i < size1; i++)
        for (j = 0; j < size2; j++)
          for (k = 0; k < size3; k++) array->a.i3[i][j][k] = copyarray.a.i3[i][j][k];
      am4del(&copyarray);
      break;
    case cca_I4:
      am4_alloc_copy(array, &copyarray);
      ...
      for (i = 0; i < size1; i++)
        for (j = 0; j < size2; j++)
          for (k = 0; k < size3; k++)
            for (l = 0; l < size4; l++) array->a.i4[i][j][k][l] = copyarray.a.i4[i][j][k][l];
      am4del(&copyarray);
      break;
    default:
      dserror("Unknown type of array given");
