...
/*----------------------------------------------------------------------*
 |  Integrate a Line Neumann boundary condition (public) mukherjee 08/15|
 *----------------------------------------------------------------------*/
int DRT::ELEMENTS::DiscSh3Line::EvaluateNeumann(
  ...
  )
{
  ...
  FAD TotEnergy= 0.5 * curvature_penalty *ThetaCurr *ThetaCurr * (this->GetRefEdgeLength()/Span_neighbours);
  ...
  for (int column=0; column<12; column++)
    for (int row=0; row<3; row++)
    {
      DcosThetaDx(column)+=DnDx_master(row,column)*NormalSlaveCurr(row)+DnDx_neighbour(row,column)*NormalMasterCurr(row);
    }
  ...
  for (int row=0; row<3; row++)
    for (int column=0; column<12; column++)
      for (int k=0; k<3; k++)
        DCrossProdDx(row,column)+=SkewSymmTensNormalMaster(row,k)*DnDx_neighbour(k,column)-SkewSymmTensNormalNeighbour(row,k)*DnDx_master(k,column);

  /* Calculate dsinTheta/dx with sinTheta = abs(n1xn2) */
  LINALG::TMatrix<FAD,1,12> DsinThetaDx(true);
  for (int column=0; column<12; column++)
    for (int row=0; row<3; row++)
      DsinThetaDx(column)+=DCrossProdDx(row,column)*crossprod_normal(row)/(pow((crossprod_normal.Dot(crossprod_normal)),0.5));
  ...
  {
    ana_force_aux(i) = curvature_penalty* (ThetaCurr-ThetaRef)*DThetaDx(i)*this->GetRefEdgeLength()/Span_neighbours;

    // Behavior function
    BehaviorFunctCurvature_dx(i) =  DThetaDx(i)*pow((this->GetRefEdgeLength()/Span_neighbours),0.5);
  }
...
if(master_ele->IfHaveDamping()&& slave_ele->IfHaveDamping())
{
  ...
  for(int i = 0; i < NUMDOF_DISCSH3; i++)
  {
    ...
    {
      FAD_force_viscous(i) += gamma*BehaviorFunctCurvature.dx(i)*BehaviorFunctCurvature.dx(j)*v_FAD_curr[j];
    }
  ...
  //LengthConstrtStiffmass(params, edge, x_FAD_curr, discretization, ana_force_aux,stiffmatrix_dummy);
 ...
 if(master_ele->IfHaveDamping()&& slave_ele->IfHaveDamping())
 {
   ...
   for (int i=0; i<NUMDOF_DISCSH3; i++)
     for (int j=0; j<NUMDOF_DISCSH3; j++)
     {
       ...
       {
         stiffmat_viscous(i,j)+=  gamma* BehaviorFunctCurvature_dx(i).dx(j)*BehaviorFunctCurvature_dx(k)*v_FAD_curr[k]
                                + gamma* BehaviorFunctCurvature_dx(i)*BehaviorFunctCurvature_dx(k).dx(j)*v_FAD_curr[k]
                                + gamma* BehaviorFunctCurvature_dx(i)*BehaviorFunctCurvature_dx(k)*(j==k)/dt;
       }
...
//   stiffmatrix_Epetra.
 ...
 if(edge->Id()==EdgeID)
 {
  ...
  RemapForceVectoMasterSlave(ana_force_aux,elevector_m,elevector_s,connectivity,lm_slaveNodeToPatch);


  // element matrices in block structure master vs. slave
  LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>  elematrix_mm(true);         // element matrix master-master block
  LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>  elematrix_ms(true);         // element matrix master-slave block
  LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>  elematrix_sm(true);         // element matrix slave-master block
  LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>  elematrix_ss(true);         // element matrix slave-slave block

  RemapStiffmattoMasterSlave(stiffmatrix_dummy,elematrix_mm,elematrix_ms,elematrix_sm,elematrix_ss,
      lm_masterNodeToPatch,lm_slaveNodeToPatch,connectivity);
    ...
    for(int idim = 0; idim < NODDOF_DISCSH3; idim++)
    {
      ...
      for(int jdim=0; jdim < NODDOF_DISCSH3; jdim++)
      {
        ReassembleMATBlock(idim,
                           jdim,
                           elemat_blocks[idim*NODDOF_DISCSH3+jdim],
                           elematrix_mm,
                           elematrix_ms,
                           elematrix_sm,
                           elematrix_ss,
...
//  if(!stab_required) return;
  ...
  std::vector<Epetra_SerialDenseVector> elevec_blocks(NODDOF_DISCSH3); // 3D: 4 vectors for u,v,w,p components, 2D: 3 vectors for u,v,p
  ...
  int err = EvaluateEdges( params,
                                  intface, discretization,
                                   lm_patch,
                                   lm_masterToPatch,lm_slaveToPatch,lm_faceToPatch,
                                   lm_masterNodeToPatch, lm_slaveNodeToPatch,
...
void DRT::ELEMENTS::DiscSh3Line::RemapForceVectoMasterSlave(
    ...
    LINALG::Matrix<NUMDOF_DISCSH3, 1> & elevector_s,
    ...
    )
{
    ...
    for (int j=0; j<NODDOF_DISCSH3; j++)
  {
    ...
    // rows corresponding to second node of master element
      ...
      elematrix_mm(i+3,j+3)=stiffmatrix(3*connectivity[1]+i,3*connectivity[1]+j).val();
      elematrix_mm(i+3,j+6)=stiffmatrix(3*connectivity[1]+i,3*connectivity[2]+j).val();
    // rows corresponding to third node of master element
      ...
      elematrix_mm(i+6,j+3)=stiffmatrix(3*connectivity[2]+i,3*connectivity[1]+j).val();
      elematrix_mm(i+6,j+6)=stiffmatrix(3*connectivity[2]+i,3*connectivity[2]+j).val();
  }
    ...
    for (int col_node=0; col_node<NUMNOD_DISCSH3; col_node++)
  {
    ...
    if (rpatch!=3 && cpatch==3)
    {
      ...
      for (int i=0; i<NODDOF_DISCSH3; i++)
        for (int j=0; j<NODDOF_DISCSH3; j++)
          elematrix_ms(3*row_node+i,3*col_node+j)=stiffmatrix(i+3*rindex,j+cpatch).val();
    }
  }
    ...
    for (int col_node=0; col_node<NUMNOD_DISCSH3; col_node++)
  {
    ...
    if (rpatch==3 && cpatch==3) // 3 because of the slave DOFs
    {
      ...
      for (int i=0; i<NODDOF_DISCSH3; i++)
        for (int j=0; j<NODDOF_DISCSH3; j++)
          elematrix_ss(3*row_node+i,3*col_node+j)=stiffmatrix(i+rpatch,j+cpatch).val();
    }
  }
    ...
    for (int col_node=0; col_node<NUMNOD_DISCSH3; col_node++)
  {
    ...
    if (rpatch==3 && cpatch!=3)
    {
      ...
      for (int i=0; i<NODDOF_DISCSH3; i++)
        for (int j=0; j<NODDOF_DISCSH3; j++)
          elematrix_sm(3*row_node+i,3*col_node+j)=stiffmatrix(i+rpatch,j+3*cindex).val();
    }
  }
...
//  std::vector<FAD> x_FAD(12,0.0);
    ...
    for(int i = 0; i < 3; i++)
    {
      Ana_force(i)   = -line_penalty*BehaviorFunctLine*(x_FAD_line_curr[i]-x_FAD_line_curr[i+3])/(Edge_length_curr*std::pow(Edge_length_ref,0.5));
      Ana_force(i+3) =  line_penalty*BehaviorFunctLine*(x_FAD_line_curr[i]-x_FAD_line_curr[i+3])/(Edge_length_curr*std::pow(Edge_length_ref,0.5));
...
//      tol(i+3)=Ana_force(i+9).val()-FAD_force(i+3).val();
    ...
    for (int i=0; i<6; i++)
    {
      for (int j=0; j<6; j++)
      {
        ...
        FAD_stiff_alt(i,j) = line_penalty*grad_edge(i)*grad_edge(j)/Edge_length_ref - line_penalty*(1-Edge_length_curr/Edge_length_ref)*grad2_edge(i,j);
