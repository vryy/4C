...
DRT::ELEMENTS::DiscSh3Type& DRT::ELEMENTS::DiscSh3Type::Instance()
{
  return instance_;
}
...
Teuchos::RCP<DRT::Element> DRT::ELEMENTS::DiscSh3Type::Create( const std::string eletype,
...
void DRT::ELEMENTS::DiscSh3Type::NodalBlockInformation( DRT::Element * dwele, int & numdf, int & dimns, int & nv, int & np )
{
...
void DRT::ELEMENTS::DiscSh3Type::ComputeNullSpace( DRT::Discretization & dis, std::vector<double> & ns, const double * x0, int numdf, int dimns )
{
  DRT::UTILS::ComputeStructure3DNullSpace( dis, ns, x0, numdf, dimns );
}

void DRT::ELEMENTS::DiscSh3Type::SetupElementDefinition( std::map<std::string,std::map<std::string,DRT::INPUT::LineDefinition> > & definitions )
{
 ...
 *----------------------------------------------------------------------------*/
LINALG::Matrix<1,9> DRT::ELEMENTS::DiscSh3::SpatialConfiguration(const std::vector<double>&  disp) const
{
...
/*----------------------------------------------------------------------------*
 |  Calculate spatial configuration of an element (private)    mukherjee 07/15|
 *----------------------------------------------------------------------------*/
LINALG::Matrix<1,9> DRT::ELEMENTS::DiscSh3::SpatialConfiguration(DRT::Discretization & dis,
                                                         const Epetra_Vector & discol) const
{
  LINALG::Matrix <1,9> coord(true);
 ...
 *--------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::AddPrimaryDOFsMaster(DRT::Element & master,
                                                  DRT::Element & neighbour,
                                                  ...
                                                  bool refconfig)
{
  ...
  else
  {
    ...
    {
      if((master.NodeIds()[j])!=(neighbour.NodeIds()[0]) && (master.NodeIds()[j])!=(neighbour.NodeIds()[1]) && (master.NodeIds()[j])!=(neighbour.NodeIds()[2]) )
      {
      ...
      else if((master.NodeIds()[j])==(neighbour.NodeIds()[0]) || (master.NodeIds()[j])==(neighbour.NodeIds()[1]) || (master.NodeIds()[j])==(neighbour.NodeIds()[2]) )
      {
        if(count==0)
        {
          for (int dof=0; dof<3; dof++)
 ...
 *-------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::AddPrimaryDOFsNeighbour(DRT::Element & master,
                                                  DRT::Element & neighbour,
                                                  std::vector<FAD> & x_FAD,
                                                  DRT::Discretization & dis,
                                                  bool refconfig)
{
  ...
  else
  {
    ...
    {
      if((neighbour.NodeIds()[j])!=(master.NodeIds()[0]) && (neighbour.NodeIds()[j])!=(master.NodeIds()[1]) && (neighbour.NodeIds()[j])!=(master.NodeIds()[2]) )
      {
        for (int dof=0; dof<3; dof++)
        {
 ...
 *---------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::SortPrimaryDOFs(DRT::Element & master,
                                             DRT::Element & neighbour,
                                             std::vector<FAD> & x_FAD,
                                             std::vector<FAD> & x_FAD_master,
                                             std::vector<FAD> & x_FAD_neighbour)
{
 ...
 *-----------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::CheckIfOutwardsNormal(Teuchos::ParameterList& params,
 ...
 * ---------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::ComputeAreaDeriv(const LINALG::SerialDenseMatrix& x,
                                                        const int numnode,
                                                        const int ndof,
                                                        double& A,
 ...
 * ---------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::ComputeAreaRef(const LINALG::SerialDenseMatrix& x0,
                                                        const int numnode,
                                                        const int ndof,
                                                        double& A)
{
 ...
 * ---------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::SurfaceIntegration(double& detA,
 ...
 *-----------------------------------------------------------------------*/
Teuchos::RCP<DRT::Element> DRT::ELEMENTS::DiscSh3::CreateFaceElement( DRT::Element* parent_slave,           //!< parent slave fluid3 element
