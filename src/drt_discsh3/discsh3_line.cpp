...
DRT::ELEMENTS::DiscSh3LineType& DRT::ELEMENTS::DiscSh3LineType::Instance()
{
  return instance_;
}
 ...
 *----------------------------------------------------------------------*/
DRT::ELEMENTS::DiscSh3Line::DiscSh3Line(int id, int owner,
                              int nnode, const int* nodeids,
                              DRT::Node** nodes,
                              DRT::ELEMENTS::DiscSh3* parent,
                              const int lline) :
DRT::FaceElement(id,owner)
{
  SetNodeIds(nnode,nodeids);
 ...
 *----------------------------------------------------------------------*/
DRT::ELEMENTS::DiscSh3Line::~DiscSh3Line()
{
  return;
}
 ...
 *-------------------------------------------------------------------------------*/
FAD DRT::ELEMENTS::DiscSh3Line::CalcTheta(LINALG::TMatrix<FAD,1,3> & vector1,
                                          LINALG::TMatrix<FAD,1,3> & vector2)
{
    FAD theta=0;
    ...
    if(CosTheta>1.0 && fabs(CosTheta-1.0)<1e-12)
      CosTheta = 1.0;
...
FAD DRT::ELEMENTS::DiscSh3Line::GetCurrEdgeLength(DRT::Discretization & dis)
{
  ...
  FAD Edge_length_curr=  std::pow((std::pow((x_FAD[0]-x_FAD[3]),2)+
                                   std::pow((x_FAD[1]-x_FAD[4]),2)+
                                   std::pow((x_FAD[2]-x_FAD[5]),2)), 0.5);
...
/*------------------------------------------------------------------------------------*
 | Calculate change in angle from reference configuration              mukherjee 09/14|
 *------------------------------------------------------------------------------------*/
double DRT::ELEMENTS::DiscSh3Line::GetCurrEdgeLength(DRT::Discretization & dis,
...
FAD DRT::ELEMENTS::DiscSh3Line::GetEdgeLengthPrevTimeStep()
{
  ...
  // Edge length at curr configuration
  FAD Edge_length_prevstep=  std::pow((std::pow((x_FAD[6]-x_FAD[9]),2)+
                             std::pow((x_FAD[7]-x_FAD[10]),2)+
                             std::pow((x_FAD[8]-x_FAD[11]),2)), 0.5);
                                                      ...
                                                      bool refconfig)
{
  ...
  {
    if((master.NodeIds()[j])!=(slave.NodeIds()[0]) && (master.NodeIds()[j])!=(slave.NodeIds()[1]) && (master.NodeIds()[j])!=(slave.NodeIds()[2]) )
    {
      ...
      if (!refconfig)
        connectivity.push_back(0);    // Node 1
    }
    else if((master.NodeIds()[j])==(slave.NodeIds()[0]) || (master.NodeIds()[j])==(slave.NodeIds()[1]) || (master.NodeIds()[j])==(slave.NodeIds()[2]) )
    {
      if(count==0)
      {
        for (int dof=0; dof<3; dof++)
        ...
        if (!refconfig)
          connectivity.push_back(2); // Node 3
      ...
      else if(count==1)
      {
        for (int dof=0; dof<3; dof++)
        {
          ...
          if (!refconfig)
            connectivity.push_back(3);  // Node 4
      }
      else if (count==2)
                                                  ...
                                                  bool refconfig)
{
  ...
  {
    if((slave.NodeIds()[j])!=(master.NodeIds()[0]) && (slave.NodeIds()[j])!=(master.NodeIds()[1]) && (slave.NodeIds()[j])!=(master.NodeIds()[2]) )
    {
      for (int dof=0; dof<3; dof++)
      {
 ...
 *---------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3Line::SortPrimaryDOFs(DRT::Element & master,
                                             DRT::Element & slave,
                                             std::vector<FAD> & x_FAD,
                                             std::vector<FAD> & x_FAD_master,
                                             std::vector<FAD> & x_FAD_slave)
{
 ...
 *-----------------------------------------------------------------------------*/
LINALG::TMatrix<FAD,1,3> DRT::ELEMENTS::DiscSh3Line::CalcSurfaceNormalMaster(std::vector<FAD> & x_FAD)
{
 ...
 *-----------------------------------------------------------------------------*/
LINALG::TMatrix<FAD,1,3> DRT::ELEMENTS::DiscSh3Line::CalcSurfaceNormalSlave(std::vector<FAD> & x_FAD)
{
 ...
 *-----------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3Line::ReassembleMATBlock(
     const int                                          row_block,            ///< row block
     const int                                          col_block,            ///< column block
     Epetra_SerialDenseMatrix&                          mat_block,            ///< matrix block
     LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>&    elematrix_mm,         ///< element matrix master-master block
     LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>&    elematrix_ms,         ///< element matrix master-slave block
     LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>&    elematrix_sm,         ///< element matrix slave-master block
     LINALG::Matrix<NUMDOF_DISCSH3, NUMDOF_DISCSH3>&    elematrix_ss,         ///< element matrix slave-slave block
 ...
 *-----------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3Line::ReassembleRHSBlock(
    const int                              row_block,            ///< row block
...
void DRT::ELEMENTS::DiscSh3Line::CalcGradienNormal(std::vector<FAD> & x_FAD,
                                               LINALG::TMatrix<FAD,3,3> & DnDx1,
                                               LINALG::TMatrix<FAD,3,3> & DnDx2,
 ...
 *----------------------------------------------------------------------------*/
LINALG::Matrix<1,6> DRT::ELEMENTS::DiscSh3Line::SpatialConfiguration(DRT::Discretization & dis) const
{
...
/*----------------------------------------------------------------------------*
 |  Calculate spatial configuration of an element (private)    mukherjee 07/15|
 *----------------------------------------------------------------------------*/
LINALG::Matrix<1,6> DRT::ELEMENTS::DiscSh3Line::SpatialConfiguration(DRT::Discretization & dis,
