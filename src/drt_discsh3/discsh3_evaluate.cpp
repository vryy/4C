                                    ...
                                    Epetra_SerialDenseVector& elevec3)
{
  ...
  else if (action=="calc_struct_linstiff")                        act = DiscSh3::calc_struct_linstiff;
  else if (action=="calc_struct_nlnstiff")                        act = DiscSh3::calc_struct_nlnstiff;
  else if (action=="calc_struct_internalforce")                   act = DiscSh3::calc_struct_internalforce;
  else if (action=="calc_struct_linstiffmass")                    act = DiscSh3::calc_struct_linstiffmass;
  else if (action=="calc_struct_nlnstiffmass")                    act = DiscSh3::calc_struct_nlnstiffmass;
  else if (action=="calc_struct_nlnstifflmass")                   act = DiscSh3::calc_struct_nlnstifflmass;
  else if (action=="calc_struct_ptcstiff")                        act = DiscSh3::calc_struct_ptcstiff;
  else if (action=="calc_struct_stress")                          act = DiscSh3::calc_struct_stress;
  else if (action=="calc_struct_eleload")                         act = DiscSh3::calc_struct_eleload;
  else if (action=="calc_struct_fsiload")                         act = DiscSh3::calc_struct_fsiload;
  else if (action=="calc_struct_refvol")                          act = DiscSh3::calc_struct_refvol;
  else if (action=="calc_struct_currvol")                         act = DiscSh3::calc_struct_currvol;
  else if (action=="calc_struct_refCG")                           act = DiscSh3::calc_struct_refCG;
  else if (action=="calc_struct_currCG")                          act = DiscSh3::calc_struct_currCG;
  else if (action=="calc_struct_refarea")                         act = DiscSh3::calc_struct_refarea;
  else if (action=="calc_struct_currarea")                        act = DiscSh3::calc_struct_currarea;
  else if (action=="calc_struct_update_istep")                    act = DiscSh3::calc_struct_update_istep;
  else if (action=="calc_struct_reset_istep")                     act = DiscSh3::calc_struct_reset_istep;
  else dserror("Unknown type of action for DiscSh3: %s", action.c_str());
  ...
  break;
    ...
    case calc_struct_refvol:
    {
      ...
      double vol_ref = (-(v3(0)*v2(1)*v1(2))+(v2(0)*v3(1)*v1(2))+(v3(0)*v1(1)*v2(2))
                        -(v1(0)*v3(1)*v2(2))-(v2(0)*v1(1)*v3(2))+(v1(0)*v2(1)*v3(2)))/6;
    ...
    case calc_struct_currvol:
    {
      ...
      double vol_curr = (-(v3(0)*v2(1)*v1(2))+(v2(0)*v3(1)*v1(2))+(v3(0)*v1(1)*v2(2))
                         -(v1(0)*v3(1)*v2(2))-(v2(0)*v1(1)*v3(2))+(v1(0)*v2(1)*v3(2)))/6;
    ...
    case calc_struct_currCG:
    {
      ...
      for(int i=0; i<3; i++)
        bary_curr(i)= (x_curr(i)+x_curr(i+3)+x_curr(i+6))/3;
 ...
 *----------------------------------------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::EvaluatePTC(Teuchos::ParameterList& params,
                                        Epetra_SerialDenseMatrix& elemat1)
{

    //PTC for translational degrees of freedom; the Lobatto integration weight is 0.5 for 3-noded elements
    for(int i=0; i<9; i++)
    {
...
/*-----------------------------------------------------------------------------------------------------------*
 ...
 | theorem                                                                          (private) Mukherjee 08/15|
 *----------------------------------------------------------------------------------------------------------*/
 ...
 *----------------------------------------------------------------------------------------------------------*/
inline void DRT::ELEMENTS::DiscSh3::MyTranslationalDamping(Teuchos::ParameterList& params,  //!<parameter list
                                                  ...
                                                  Epetra_SerialDenseVector* force)//!< element internal force vector
{
  ...
  LINALG::TMatrix <LINALG::TMatrix<FAD,NUMDOF_DISCSH3,NUMDOF_DISCSH3>, 1, NUMDOF_DISCSH3 > Grad_DampingMat(true);
  for (int i=0; i<NUMDOF_DISCSH3; i++)
    for (int j=0; j<NUMDOF_DISCSH3; j++)
      for (int k=0; k<NUMDOF_DISCSH3; k++)
        Grad_DampingMat(k)(i,j) =Damping_mat(i,j).dx(k);
  ...
  for(int i=0; i<NUMDOF_DISCSH3; i++)
    for(int j=0; j<NUMDOF_DISCSH3; j++)
    {
      ...
      if(stiffmatrix != NULL)
        for (int k=0; k<NUMDOF_DISCSH3; k++)
        {
          FAD_StiffMat(i,j) += Grad_DampingMat(k)(i,j)*vel[k]/3+ Damping_mat(i,j)/(3*dt);
        }
    }
  ...
  for(int i=0; i<NUMDOF_DISCSH3; i++)
  {
    ...
    {
      if(stiffmatrix != NULL)
        (*stiffmatrix)(i,j)+=FAD_StiffMat(i,j).val();
    }
  }
 ...
 *----------------------------------------------------------------------------------------------------------*/
inline void DRT::ELEMENTS::DiscSh3::MyStochasticForces(Teuchos::ParameterList& params,  //!<parameter list
                                              const std::vector<double>& vel,  //!< element velocity vector
                                              ...
                                              Epetra_SerialDenseVector* force)//!< element internal force vector
{
  ...
  for(int i=0; i<3; i++)
    for(int j=0; j<3; j++)
    {
      Stoch_mat_small(i,j) = (i==j)* std::sqrt(gamma(1)) +  (std::sqrt(gamma(0)) - std::sqrt(gamma(1)))* normal(i)*normal(j);
  ...
  LINALG::TMatrix <LINALG::TMatrix<FAD,NUMDOF_DISCSH3,NUMDOF_DISCSH3>, 1, NUMDOF_DISCSH3 > Grad_StochMat(true);
  for (int i=0; i<NUMDOF_DISCSH3; i++)
    for (int j=0; j<NUMDOF_DISCSH3; j++)
      for (int k=0; k<NUMDOF_DISCSH3; k++)
        Grad_StochMat(k)(i,j) =Stoch_mat(i,j).dx(k);
...
/*-----------------------------------------------------------------------------------------------------------*
 ...
 | rotation around filament axis                                             (private)       Mukherjee  10/15|
 *----------------------------------------------------------------------------------------------------------*/
inline void DRT::ELEMENTS::DiscSh3::MyDampingConstants(Teuchos::ParameterList& params,LINALG::Matrix<3,1>& gamma)
{
 ...
 *----------------------------------------------------------------------------------------------------------*/
inline void DRT::ELEMENTS::DiscSh3::MyDampingForces(Teuchos::ParameterList& params,  //!<parameter list
                                                  const std::vector<double>& vel,  //!< vector containing first order time derivative of nodal positions and nodal tangents of an element
                                                  const std::vector<double>& disp, //!< vector containing change in nodal positions and nodal tangents of an element w.r.t. inital config.
                                                  Epetra_SerialDenseMatrix* stiffmatrix,  //!< element stiffness matrix
                                                  Epetra_SerialDenseVector* force)//!< element internal force vector
 ...
 *----------------------------------------------------------------------------------------------------------*/
inline void DRT::ELEMENTS::DiscSh3::MyDampingForcesArea(Teuchos::ParameterList& params,  //!<parameter list
                                                  const std::vector<double>& vel,  //!< vector containing first order time derivative of nodal positions and nodal tangents of an element
                                                  const std::vector<double>& disp, //!< vector containing change in nodal positions and nodal tangents of an element w.r.t. inital config.
                                                  Epetra_SerialDenseMatrix* stiffmatrix,  //!< element stiffness matrix
                                                  Epetra_SerialDenseVector* force)//!< element internal force vector
{
  ...
  for (int j=0; j<NUMNOD_DISCSH3; j++)
  {
    ...
    for(int i=0; i<3; i++)
      crossprod2(3*j+i)=crossprod_aux(i);
  }
  ...
  for (int i=0; i<NUMDOF_DISCSH3; i++)
    for (int j=0; j<NUMDOF_DISCSH3; j++)
    {
      for (int k=0; k<NUMDOF_DISCSH3; k++)
      {
        stiffmat_viscous(i,j)+=  gamma* BehaviorFunctArea_dx(i).dx(j)*BehaviorFunctArea_dx(k)*vel[k]
...
/*-----------------------------------------------------------------------------------------------------------*
 | computes damping forces arising from strectch (area) (private                           Mukherjee   10/15|
 *----------------------------------------------------------------------------------------------------------*/
inline void DRT::ELEMENTS::DiscSh3::MyDampingForcesVol(Teuchos::ParameterList& params,  //!<parameter list
                                                  const std::vector<double>& vel,  //!< vector containing first order time derivative of nodal positions and nodal tangents of an element
                                                  const std::vector<double>& disp, //!< vector containing change in nodal positions and nodal tangents of an element w.r.t. inital config.
                                                  Epetra_SerialDenseMatrix* stiffmatrix,  //!< element stiffness matrix
                                                  Epetra_SerialDenseVector* force)//!< element internal force vector
{
  ...
  std::vector<FAD> x_FAD(NUMDOF_DISCSH3,0.0);
    ...
    for (int j=0; j<NUMNOD_DISCSH3; j++)
    {
      ...
      for(int i=0; i<3; i++)
        crossprod(3*j+i)=crossprod_aux(i);
    ...
    for (int i=0; i<NUMDOF_DISCSH3; i++)
      for (int j=0; j<NUMDOF_DISCSH3; j++)
      {
        for (int k=0; k<NUMDOF_DISCSH3; k++)
        {
          stiffmat_viscous(i,j)+=  gamma* BehaviorFunctVol_dx(i).dx(j)*BehaviorFunctVol_dx(k)*vel[k]
...
//    dserror("Please enter a non-zero CG_PENALTY");
    ...
    for(int i=0; i<3; i++)
    {
      EnergyFuctional += cg_penalty*(bary_curr(i)-bary_ref(i))*(bary_curr(i)-bary_ref(i))/(2*NumGlobalEles);
...
//    dserror("Please enter a non-zero CG_PENALTY");
  ...
  for(int i=0; i<3; i++)
  {
   ...
   {
     for(int j=0; j<9; j++)
       ana_force(i)= cg_penalty*aux_vect(i);
   }
...
//    dserror("Please enter a non-zero BARY_PENALTY");
  ...
  FAD bary_penalty = 0.0;
    ...
    for(int i=0; i<3; i++)
    {
      EnergyFuctional += bary_penalty*(bary_curr(i)-bary_ref(i))*(bary_curr(i)-bary_ref(i))/2;
      aux_vect(i) = (bary_curr(i)-bary_ref(i))/3;
      aux_vect(i+3) = (bary_curr(i)-bary_ref(i))/3;
      aux_vect(i+6) = (bary_curr(i)-bary_ref(i))/3;
    }
...
//    }
   ...
   for (int i=0; i<NUMDOF_DISCSH3; i++)
   {
     ana_force(i)= -area_penalty*BehaviorFunctArea*crossprod2(i)/(4*std::pow(area_ref,0.5)*area_curr);
//     tol(i)= ana_force(i).val()-FAD_force(i).val();
   }
...
//  // element geometry update
  ...
  for (int i=0;i<ndof;++i)
  {
    ...
    {
      (*stiffmatrix)(i,j) += (area_penalty*(*Adiff)[i]*(*Adiff)[j]/A0-area_penalty*(1-A/A0)*(*Adiff2)(i,j));
    }
  }
...
//  }
  ...
  for (int i=0; i<NUMDOF_DISCSH3; i++)
  {
    ana_force(i)= vol_penalty*1/std::pow(abs(vol_ref),0.5)*BehaviorFunctVol*crossprod(i)*sign(i)/6;
//    tol(i)=ana_force(i).val()-FAD_force(i).val();
  }
...
//  }
  ...
  for (int i=0; i<NUMDOF_DISCSH3; i++)
    for (int j=0; j<NUMDOF_DISCSH3; j++)
    {
      ...
      stiff_vol(i,j) = vol_penalty*grad_vol(i)*grad_vol(j)/vol_ref - vol_penalty*(1-vol_curr/vol_ref)*grad2_vol(i,j);
...
//      grad2_area(i,j)= crossprod2(i).dx(j)/(4*area_curr)- crossprod2(i)*crossprod2(j)/(16*std::pow(area_curr,3));
      stiff_area(i,j)= area_penalty*grad_area(i)*grad_area(j)/area_ref - area_penalty*(1-area_curr/area_ref)*grad2_area(i,j);
 ...
 *--------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::CalcBehaviorFunctArea(Teuchos::ParameterList&   params,
                                                    const std::vector<double>& disp,
                                                   FAD & BehaviorFunctArea)
{
//  Teuchos::ParameterList StatMechParams = DRT::Problem::Instance()->StatisticalMechanicsParams();
...
/*----------------------------------------------------------------------*
 ...
 |of the element (private)                               mukherjee 07/15|
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::CalcBehaviorFunctVol(Teuchos::ParameterList&   params,
                                                   const std::vector<double>& disp,
                                                  FAD & BehaviorFunctVol)
{
 ...
 *------------------------------------------------------------------------*/
FAD DRT::ELEMENTS::DiscSh3::CalcSurfaceArea(const std::vector<double>&  disp,
                                             bool refconfig)
{
  FAD area;
  std::vector<FAD> x_FAD(NUMDOF_DISCSH3,0.0);
...
/*------------------------------------------------------------------------*
 |  Calculate surface area of the element (private)        mukherjee 04/15|
 *------------------------------------------------------------------------*/
FAD DRT::ELEMENTS::DiscSh3::CalcSurfaceAreaFAD(const std::vector<double>&  disp,
                                             bool refconfig)
{
  FAD area;
  ...
  for (int j=0; j<NUMNOD_DISCSH3; j++)
   {
     ...
     for (int k=0; k<3; k++)
       FAD_force_vector(k+3*j)=area.dx(k+3*j);
   }
 ...
 *------------------------------------------------------------------------*/
FAD DRT::ELEMENTS::DiscSh3::CalcSurfaceArea(DRT::Discretization&      discretization,
                                             bool refconfig)
{
  FAD area;
  std::vector<FAD> x_FAD(NUMDOF_DISCSH3,0.0);
 ...
 *------------------------------------------------------------------------*/
double DRT::ELEMENTS::DiscSh3::CalcSurfArea(DRT::Discretization & dis,
...
FAD DRT::ELEMENTS::DiscSh3::CalcSurfaceArea(LINALG::TMatrix<FAD,1,3>& vertex1,
                                            LINALG::TMatrix<FAD,1,3>& vertex2,
                                            LINALG::TMatrix<FAD,1,3>& vertex3)
{
  FAD area;
...
/*------------------------------------------------------------------------*
 |  Calculate surface area of a triangle (private)        mukherjee 04/15|
 *------------------------------------------------------------------------*/
double DRT::ELEMENTS::DiscSh3::CalcSurfaceArea(LINALG::Matrix<1,3>& vertex1,
                                            LINALG::Matrix<1,3>& vertex2,
 ...
 *----------------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::CalcVolume(FAD & volume,
                                        const std::vector<double>& disp,
                                        bool refconfig)
{
  std::vector<FAD> x_FAD(NUMDOF_DISCSH3,0.0);
 ...
 *----------------------------------------------------------------------------*/
double DRT::ELEMENTS::DiscSh3::CalcVolume(DRT::Discretization & dis,
                                          const Epetra_Vector & discol,
                                          bool refconfig)
{
  FAD volume=0;
 ...
 *----------------------------------------------------------------------*/
void DRT::ELEMENTS::DiscSh3::sh3_lumpmass(const std::vector<double>&      disp,
                                          Epetra_SerialDenseMatrix* massmatrix)
{
