...
void CONTACT::MtAbstractStrategy::RestrictMeshtyingZone()
{
  ...
  if (quadratic) dserror("RestrictMeshtyingZone only implemented for first-order elements");
    ...
    IO::DiscretizationReader& reader, Teuchos::RCP<const Epetra_Vector> dis)
{
  ...
  bool restartwithmeshtying = INPUT::IntegralValue<int>(Params(), "RESTART_WITH_MESHTYING");
  ...
  for (int i = 0; i < (int)interface_.size(); ++i)
  {
    ...
    for (int j = 0; j < interface_[i]->SlaveRowNodes()->NumMyElements(); ++j)
    {
      ...
      for (int d = 0; d < Dim(); ++d)
      {
        ...
        if (dofid < 0) dserror("InterfaceForces: Did not find slave dof in map");
    ...
    for (int j = 0; j < interface_[i]->MasterRowNodes()->NumMyElements(); ++j)
    {
      ...
      for (int d = 0; d < Dim(); ++d)
      {
        ...
        if (dofid < 0) dserror("InterfaceForces: Did not find master dof in map");
