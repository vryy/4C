/*!----------------------------------------------------------------------
\file
\brief contains wall_contact_detection routine used for 2-D contact
interfaces
...
/*----------------------------------------------------------------------*/
/*Contact flag of all the contact nodes are assigned to contact_off*/
...
/*Current positions of the contact nodes (both master and slave) are updated.*/ 
  ...
  for(t=0; t<3; t++)
   ...
   /*Closest node is the projection of the slave node*/
    ...
    T[0] =  tangent[0];
    T[1] =  tangent[1];
    T[2] = -0.5*(1.0 - local_coordinate)*tangent[0];
