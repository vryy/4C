...
void INPAR::PATSPEC::SetValidParameters(Teuchos::RCP<Teuchos::ParameterList> list)
{
  ...
  BoolParameter(
      "CALCINNERRADIUS", "No", "Compute inner radius for pure structural wall shear stress", &ps);
  BoolParameter("LINEARCENTERLINE", "No",
      "Is the centerline linear? Only important when CALCINNERRADIUS is turned on", &ps);
  ...
  // not constant e.g. during UQ analysis. Therefore, a new more accurate method was added which
...
void INPAR::PATSPEC::SetValidConditions(
    std::vector<Teuchos::RCP<DRT::INPUT::ConditionDefinition>>& condlist)
{
  using namespace DRT::INPUT;
  ...
  Teuchos::RCP<ConditionDefinition> aaasurfcond =
      Teuchos::rcp(new ConditionDefinition("DESIGN AAA SURFACE CONDITION", "AAASurface",
          "AAA surface", DRT::Condition::AAASurface, true, DRT::Condition::Surface));
