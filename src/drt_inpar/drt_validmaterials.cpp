...
Teuchos::RCP<std::vector<Teuchos::RCP<DRT::INPUT::MaterialDefinition> > > DRT::INPUT::ValidMaterials()
{
  ...
  /*----------------------------------------------------------------------*/
   // growth and remodeling (homogenized constrained mixture model)
   {
     ...
     AddNamedIntVector(m,"MATIDSEL2D","the list 2d elastin matrix material/potential IDs","NUMMATEL2D",false);
     AddNamedInt(m,"MATIDELPENALTY","penalty material ID",-1,true);
     AddNamedReal(m,"ELMASSFRAC","initial mass fraction of elastin matrix in constraint mixture",false);
     ...
     AddNamedReal(m,"THICKNESS","reference wall thickness of the idealized cylindrical aneurysm [m]",-1,true);
...
//      AddNamedReal(mm,"ALPHA","size of ",-1.0,true);
  ...
  // material for a crosslinker in a biopolymer simulation
  {
    ...
    AddNamedReal( matdef, "NOBONDDISTSPHERE","distance to sphere elements in which no double bonded linker is allowed", 0.0, true );
