...
void INPAR::CELL::SetValidParameters(Teuchos::RCP<Teuchos::ParameterList> list)
{
  ...
  setStringToIntegralParameter<int>("INT_STRATEGY", "Old",
      "global type of the used integration strategy", tuple<std::string>("Old", "Standard", "LOCA"),
      tuple<int>(int_old, int_standard, int_loca), &cellstructdyn);

  setStringToIntegralParameter<int>("DYNAMICTYP", "OneStepTheta",
      "type of the specific dynamic time integration scheme",
      tuple<std::string>("Statics", "GenAlpha", "OneStepTheta", "OneStepThetaImmersed", "GEMM",
          "ExplicitEuler", "CentrDiff", "AdamsBashforth2", "EulerMaruyama", "EulerImpStoch"),
      tuple<int>(dyna_statics, dyna_genalpha, dyna_onesteptheta, dyna_onesteptheta_immersed,
  ...
  DoubleParameter(
      "PRESTRESSTIME", 0.0, "time to switch from pre to post stressing", &cellstructdyn);
  ...
  IntParameter(
      "RESTARTEVRY", 1, "write restart possibility every RESTARTEVRY steps", &cellstructdyn);
  // Time loop control
  ...
  setStringToIntegralParameter<int>("DAMPING", "No",
      "type of damping: (1) Rayleigh damping matrix and use it from M_DAMP x M + K_DAMP x K, (2) "
      "Material based and calculated in elements",
      tuple<std::string>(
  ...
  DoubleParameter("TOLDISP", 1.0E-10, "tolerance in the displacement norm for the newton iteration",
      &cellstructdyn);
  setStringToIntegralParameter<int>("NORM_DISP", "Abs",
      "type of norm for displacement convergence check", tuple<std::string>("Abs", "Rel", "Mix"),
      tuple<int>(convnorm_abs, convnorm_rel, convnorm_mix), &cellstructdyn);

  DoubleParameter(
      "TOLRES", 1.0E-08, "tolerance in the residual norm for the newton iteration", &cellstructdyn);
  setStringToIntegralParameter<int>("NORM_RESF", "Abs",
      "type of norm for residual convergence check", tuple<std::string>("Abs", "Rel", "Mix"),
      tuple<int>(convnorm_abs, convnorm_rel, convnorm_mix), &cellstructdyn);

  DoubleParameter(
      "TOLPRE", 1.0E-08, "tolerance in pressure norm for the newton iteration", &cellstructdyn);
  setStringToIntegralParameter<int>("NORM_PRES", "Abs",
      "type of norm for pressure convergence check", tuple<std::string>("Abs"),
      tuple<int>(convnorm_abs), &cellstructdyn);

  DoubleParameter("TOLINCO", 1.0E-08,
      "tolerance in the incompressible residual norm for the newton iteration", &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("NORMCOMBI_DISPPRES", "And",
      ...
      tuple<std::string>("And", "Or"), tuple<int>(bop_and, bop_or), &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("STC_SCALING", "no",
      ...
      tuple<int>(stc_none, stc_none, stc_none, stc_currsym, stc_curr), &cellstructdyn);

  IntParameter("STC_LAYER", 1, "number of STC layers for multilayer case", &cellstructdyn);
  ...
  DoubleParameter("TOLCONSTR", 1.0E-08,
      "tolerance in the constr error norm for the newton iteration", &cellstructdyn);

  DoubleParameter("TOLCONSTRINCR", 1.0E-08,
      "tolerance in the constr lm incr norm for the newton iteration", &cellstructdyn);

  IntParameter("MAXITER", 50,
      "maximum number of iterations allowed for Newton-Raphson iteration before failure",
      &cellstructdyn);
  IntParameter("MINITER", 0, "minimum number of iterations to be done within Newton-Raphson loop",
      &cellstructdyn);
  setStringToIntegralParameter<int>("ITERNORM", "L2", "type of norm to be applied to residuals",
      tuple<std::string>("L1", "L2", "Rms", "Inf"),
      tuple<int>(norm_l1, norm_l2, norm_rms, norm_inf), &cellstructdyn);
  ...
  IntParameter("MAXDIVCONREFINEMENTLEVEL", 10,
      "number of times timestep is halved in case nonlinear solver diverges", &cellstructdyn);

  setStringToIntegralParameter<int>("NLNSOL", "fullnewton", "Nonlinear solution technique",
      tuple<std::string>("vague", "fullnewton", "modnewton", "lsnewton", "ptc", "newtonlinuzawa",
          "augmentedlagrange", "NoxNewtonLineSearch", "noxgeneral", "noxnln", "NLNSOL"),
  ...
  IntParameter("LSMAXITER", 30, "maximum number of line search steps", &cellstructdyn);
  DoubleParameter("ALPHA_LS", 0.5, "step reduction factor alpha in (Newton) line search scheme",
      &cellstructdyn);
  DoubleParameter("SIGMA_LS", 1.e-4, "sufficient descent factor in (Newton) line search scheme",
      &cellstructdyn);

  setStringToIntegralParameter<int>("MATERIALTANGENT", "analytical",
      "way of evaluating the constitutive matrix",
      tuple<std::string>("analytical", "finitedifferences"), tuple<int>(0, 1), &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("CONTROLTYPE", "load", "load, disp, arc1, arc2 control",
      ...
      tuple<int>(control_load, control_load, control_disp, control_disp, control_disp, control_arc1,
          control_arc1, control_arc2, control_arc2),
      &cellstructdyn);
  // Currently not used, but structure will be kept if someone wants to reimplement
  // AN 2013_05
  ...
  setStringToIntegralParameter<int>("LOADLIN", "No",
      "Use linearization of external follower load in Newton", yesnotuple, yesnovalue,
      &cellstructdyn);
  ...
  setStringToIntegralParameter<int>(
      "NEGLECTINERTIA", "No", "Neglect inertia", yesnotuple, yesnovalue, &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("PREDICT", "ConstDis", "Type of predictor",
      tuple<std::string>("Vague", "ConstDis", "ConstVel", "ConstAcc", "ConstDisVelAcc", "TangDis",
          "ConstDisPres", "ConstDisVelAccPres"),
      tuple<int>(pred_vague, pred_constdis, pred_constvel, pred_constacc, pred_constdisvelacc,
          pred_tangdis, pred_constdispres, pred_constdisvelaccpres),
      &cellstructdyn);

  // Uzawa iteration for constraint systems
  DoubleParameter("UZAWAPARAM", 1.0,
      "Parameter for Uzawa algorithm dealing with lagrange multipliers", &cellstructdyn);
  DoubleParameter(
      "UZAWATOL", 1.0E-8, "Tolerance for iterative solve with Uzawa algorithm", &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("UZAWAALGO", "direct", "",
      ...
      tuple<int>(consolve_uzawa, consolve_simple, consolve_direct), &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("LUMPMASS", "No",
      "Lump the mass matrix for explicit time integration", yesnotuple, yesnovalue, &cellstructdyn);

  setStringToIntegralParameter<int>("MODIFIEDEXPLEULER", "Yes",
      "Use the modified explicit Euler time integration scheme", yesnotuple, yesnovalue,
      &cellstructdyn);

  // linear solver id used for structural problems
  IntParameter(
      "LINEAR_SOLVER", -1, "number of linear solver used for structural problems", &cellstructdyn);
  ...
  setStringToIntegralParameter<int>("YOUNG_IS_TEMP_DEPENDENT", "No",
      "Use temperature-dependent Young's modulus", yesnotuple, yesnovalue, &cellstructdyn);

  // where the geometry comes from
  setStringToIntegralParameter<int>("GEOMETRY", "full", "How the geometry is specified",
      tuple<std::string>("full", "box", "file"),
      tuple<int>(INPAR::geometry_full, INPAR::geometry_box, INPAR::geometry_file), &cellstructdyn);
  ...
  DoubleParameter("OUTSYSPERIOD", 0.0,
      ...
      &cellstructtap);
  DoubleParameter(
      "OUTSTRPERIOD", 0.0, "Write stress/strain every given period of time", &cellstructtap);
  DoubleParameter("OUTENEPERIOD", 0.0, "Write energy every given period of time", &cellstructtap);
  DoubleParameter(
      "OUTRESTPERIOD", 0.0, "Write restart data every given period of time", &cellstructtap);
  IntParameter("OUTSIZEEVERY", 0, "Write step size every given time step", &cellstructtap);

  DoubleParameter(
      "STEPSIZEMAX", 0.0, "Limit maximally permitted time step size (>0)", &cellstructtap);
  DoubleParameter(
      "STEPSIZEMIN", 0.0, "Limit minimally allowed time step size (>0)", &cellstructtap);
  DoubleParameter("SIZERATIOMAX", 0.0,
      "Limit maximally permitted change of time step size compared to previous size, important for "
      "multi-step schemes (>0)",
      &cellstructtap);
  DoubleParameter("SIZERATIOMIN", 0.0,
      "Limit minimally permitted change of time step size compared to previous size, important for "
      "multi-step schemes (>0)",
      &cellstructtap);
  DoubleParameter("SIZERATIOSCALE", 0.9,
      "This is a safety factor to scale theoretical optimal step size, should be lower than 1 and "
      "must be larger than 0",
      &cellstructtap);

  setStringToIntegralParameter<int>("LOCERRNORM", "Vague", "Vector norm to treat error vector with",
  ...
  IntParameter("ADAPTSTEPMAX", 0, "Limit maximally allowed step size reduction attempts (>0)",
      &cellstructtap);
