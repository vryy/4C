...
void allgather( stk::ParallelMachine c ,
...
void verify_node_coordinate_field( const stk::mesh::FieldBase & f )
{
...
public:
  ...
  const stk::mesh::EntityRank     m_entity_type ;
  const CellTopologyData * m_topology ;
  const int            m_number_attr ;

  FilePart( stk::mesh::Part      & arg_part ,
            int         arg_id ,
            stk::mesh::EntityRank  arg_entity_type ,
            const CellTopologyData * arg_topology ,
...
const FileSchema::IndexField & exo_index( stk::mesh::MetaData & arg_mesh_meta_data )
{
  typedef FileSchema::IndexField IndexField ;

  static const char name[] = "exodusII_local_index" ;

  const stk::mesh::Part & upart = arg_mesh_meta_data.universal_part();

  IndexField & f = arg_mesh_meta_data.declare_field< IndexField >( std::string(name) );

  stk::mesh::put_field( f , stk::mesh::Node ,    upart , 2 );
  stk::mesh::put_field( f , stk::mesh::Edge ,    upart , 2 );
  stk::mesh::put_field( f , stk::mesh::Face ,    upart , 2 );
  stk::mesh::put_field( f , stk::mesh::Element , upart , 2 );
...
void map_to_exodus( const CellTopologyData & top ,
...
const FilePart * internal_declare_part(
  stk::mesh::MetaData & arg_mesh_meta_data ,
  stk::mesh::Part         & arg_part ,
  int            arg_id ,
  stk::mesh::EntityRank     arg_type ,
  const CellTopologyData * arg_topology ,
  int            arg_space_dim ,
  int            arg_number_attr ,
  ...
  if ( ! file_part ) {
    ...
    if ( arg_number_attr ) {
      stk::mesh::put_field(
        ...
        stk::mesh::Element, arg_part, arg_number_attr );
    }
...
void FileSchema::declare_part( stk::mesh::Part & arg_part , int arg_id )
{
  std::vector< const FilePart * >::iterator
    i = std::lower_bound( m_parts[stk::mesh::Element].begin() ,
                          m_parts[stk::mesh::Element].end() ,
                          arg_id , FilePartLess() );

  if ( i != m_parts[ stk::mesh::Element ].end() && (*i)->m_identifier == arg_id ) {
    std::ostringstream msg ;
  ...
  const CellTopologyData * const top = stk::mesh::get_cell_topology( arg_part );
  ...
  const FilePart * const fp =
    internal_declare_part( m_schema , arg_part , arg_id ,
                           stk::mesh::Element , top , m_dimension ,
                           0 , m_field_elem_attr );

  m_parts[ stk::mesh::Element ].insert( i , fp );
}
  ...
  stk::mesh::EntityRank arg_type )
{
  ...
  if ( i != m_parts[ arg_type ].end() && (*i)->m_identifier == arg_id ) {
    ...
    msg << arg_id ;
    msg << " , " ;
    msg << stk::mesh::fem_entity_rank_names()[ arg_type ];
    msg << " ) FAILED, already called declare_part( " ;
    msg << (*i)->m_part.name() ;
    msg << " , " ;
    msg << arg_id ;
...
struct LessIndices {
  ...
  bool operator()( const std::pair<int,const stk::mesh::Entity*> & L ,
                   const std::pair<int,const stk::mesh::Entity*> & R ) const
  {
...
void assign_contiguous_indices(
  ...
  const std::vector< std::pair<int,const stk::mesh::Entity*> > & entities ,
  const FileSchema::IndexField & f )
{
  ...
  const stk::ParallelMachine p_comm = M.parallel();

  std::vector< std::pair<int,const stk::mesh::Entity*> >::const_iterator i ;

  unsigned unique_count = 0 ;

  // Get the total count, split evenly among processors, and then map

  stk::mesh::EntityId end_id = 1 ;
#if 0
  {
    const stk::mesh::EntitySet & es = M.entities( entity_type );
    if ( ! es.empty() ) {
...
#else
  {
    const std::vector<stk::mesh::Bucket*> & buckets = M.buckets( entity_type );
    for (std::vector<stk::mesh::Bucket*>::const_iterator i=buckets.begin();
         i!=buckets.end();
         ++i)
    {
      for (stk::mesh::Bucket::iterator ib=(*i)->begin();
           ib!=(*i)->end();
           ++ib)
      {
        stk::mesh::Entity & e = *ib;
        stk::mesh::EntityId id = e.identifier();
  ...
  for ( unsigned p = 0 ; p < p_size ; ++p ) {
    ...
    while ( buf.remaining() ) {
      stk::mesh::EntityId data[2] ;
      buf.unpack<stk::mesh::EntityId>( data , 2 );
      BlockEntityProc tmp ;
      tmp.block_id  = data[0] ;
  ...
  stk::mesh::EntityId count = 0 ;

  for ( unsigned p = 0 ; p < p_rank ; ++p ) { count += global_count[p] ; }

  stk::mesh::EntityId global_index = count ;
...

  {
    ...
    for ( std::vector<BlockEntityProc>::iterator
      ...
      data[0] = global_index ;
      data[1] = id ;
      all.send_buffer( j->processor ).pack<stk::mesh::EntityId>( data , 2 );
    }
  }

  all.communicate();

  stk::mesh::EntityId local_index = 0 ;

  for ( i = entities.begin() ; i != entities.end() ; ++i ) {
    const stk::mesh::EntityId id = i->second->identifier();

    const unsigned p = (unsigned)( p_map * id );
    stk::CommBuffer & buf = all.recv_buffer( p );
    stk::mesh::EntityId tmp[2] ;
    buf.unpack<stk::mesh::EntityId>( tmp , 2 );
...
void assign_contiguous_indices( stk::mesh::BulkData & M , unsigned entity_type ,
                                const FileSchema::IndexField & f )
{
  //const EntitySet & es = M.entities( entity_type );

  std::vector< std::pair<int,const stk::mesh::Entity*> > entities ;

  //entities.reserve( es.size() );

  std::pair<int,const stk::mesh::Entity*> tmp ; tmp.first = 0 ;
  ...
  for (std::vector<stk::mesh::Bucket*>::const_iterator i=buckets.begin();
       ...
       ++i)
  {
    entities.reserve(entities.size() + (*i)->size());
    for (stk::mesh::Bucket::iterator ib=(*i)->begin();
         ib!=(*i)->end();
         ++ib)
    {
...
void FileSchema::assign_indices( stk::mesh::BulkData & arg_mesh ) const
{
  ...
  assign_contiguous_indices( M , stk::mesh::Node , m_field_index );
  assign_contiguous_indices( M , stk::mesh::Edge , m_field_index );
  assign_contiguous_indices( M , stk::mesh::Face , m_field_index );
  ...
  const std::vector<const FilePart*> & elem_parts = m_parts[ stk::mesh::Element ];

  const unsigned num_elem_blk = elem_parts.size();

  //const KernelSet & ks = M.kernels( Element );
  const std::vector<stk::mesh::Bucket*> & ks = M.buckets(stk::mesh::Element);
  ...
  for ( std::vector<stk::mesh::Bucket*>::const_iterator ik = ks.begin() ; ik != ks.end() ; ++ik ) {
    ...
    for ( unsigned i = 0 ; i < num_elem_blk ; ++i ) {
      ...
      if ( stk::mesh::has_superset( k, m_active_part ) &&
           stk::mesh::has_superset( k, elem_parts[i]->m_part ) ) {
        ...
        std::pair<int,const stk::mesh::Entity*> tmp ;
  ...
  assign_contiguous_indices( M , stk::mesh::Element , entities , m_field_index );
}
...
void broadcast( stk::ParallelMachine c , int p_root , int * value , int n )
{ MPI_Bcast( value , n , MPI_INT , p_root , c ); }

void broadcast( stk::ParallelMachine c , int p_root , char * value , int n )
{ MPI_Bcast( value , n , MPI_BYTE , p_root , c ); }

void scatter( stk::ParallelMachine c ,
              const unsigned p_root ,
              const unsigned * const send_size ,
              const unsigned recv_size ,
              void * const data )
{
  const unsigned p_rank = stk::parallel_machine_rank( c );
  const unsigned p_size = stk::parallel_machine_size( c );
...
template<class Op>
void chunk( const std::vector<const stk::mesh::Entity*> & entities ,
            ...
            Op & op )
{
  ...
  const std::vector<const stk::mesh::Entity *>::const_iterator i_end = entities.end();
        std::vector<const stk::mesh::Entity *>::const_iterator i = entities.begin();
  ...
  while ( index < index_end ) {
    ...
    const std::vector<const stk::mesh::Entity *>::const_iterator i_beg = i ;
...
struct WriteNodeIndexCoord {
  ...
  void operator()( const int , const int ,
                   const std::vector<const stk::mesh::Entity *>::const_iterator ,
                   const std::vector<const stk::mesh::Entity *>::const_iterator );
...
WriteNodeIndexCoord::WriteNodeIndexCoord(
  ...
  : output( arg_output ),
    ...
    coord_x_y_z()
{
  const stk::mesh::BulkData       & M  = output.m_mesh ;
...
void WriteNodeIndexCoord::operator()(
  const int index_begin ,
  const int index_end ,
  const std::vector<const stk::mesh::Entity *>::const_iterator send_begin ,
  const std::vector<const stk::mesh::Entity *>::const_iterator send_end )
{
  const stk::mesh::BulkData & M  = output.m_mesh ;
  const FileSchema & SF = output.m_schema ;
  ...
  stk::CommGather gather( p_comm , p_write , send_size );

  {
    stk::CommBuffer & buf = gather.send_buffer();

    for ( std::vector<const stk::mesh::Entity *>::const_iterator
          i = send_begin ; i != send_end ; ++i ) {
      const stk::mesh::Entity & node = **i ;
      int ids[2] ;
  ...
  const std::vector<const stk::mesh::Entity *>::const_iterator send_end )
{
...

  {
          ...
          i = send_begin ; i != send_end ; ++i )
    {
      ...
      stk::mesh::PairIterRelation con = elem.relations( stk::mesh::Node );
      ...
      for ( unsigned j = 0 ; j < num_nodes_per_elem ; ++j , ++con ) {
        stk::mesh::Entity & node = * con->entity();
...
unsigned count( const std::vector<stk::mesh::Bucket*> & ks , const stk::mesh::PartVector & ps )
{
  unsigned n = 0 ;

  std::vector<stk::mesh::Bucket*>::const_iterator i ;
  for ( i = ks.begin() ; i != ks.end() ; ++i ) {
    if ( has_superset( **i,ps ) ) {
      n += (*i)->size();
    }
  }
  return n ;
...
std::string variable_name( stk::mesh::EntityRank        r ,
                           const stk::mesh::Part      & p ,
                           const stk::mesh::FieldBase & f ,
                           const shards::array_traits::int_t    k )
{
  const shards::array_traits::int_t f_num_dim = f.rank();

  std::string name( f.name() );

  if ( f_num_dim ) {
    shards::array_traits::int_t dims[    stk::mesh::MaximumFieldDimension ];
    shards::array_traits::int_t indices[ stk::mesh::MaximumFieldDimension ];

    const stk::mesh::FieldBase::Restriction & dim = f.restriction( r , p );
    ...
    shards::array_traits::stride_to_natural_dimensions( f_num_dim , dim.stride , dims );
    shards::array_traits::stride_to_natural_indices( f_num_dim , dim.stride , k , indices );

    for ( int i = 0 ; i < f_num_dim ; ++i ) {

      // Natural, as opposed to Fortran, ordering of dimensions:
      const shards::ArrayDimTag & tag = * f.dimension_tags()[ ( f_num_dim - 1 ) - i ];
...
void variable_add( stk::mesh::EntityRank entity_type ,
                   ...
                   std::vector< FieldIO >              & spec )
{
  ...
  const shards::array_traits::int_t field_num_dim = field.rank();

  for ( unsigned j = 0 ; j < parts.size() ; ++j ) {
    stk::mesh::Part & p = parts[j]->m_part ;

    const stk::mesh::FieldBase::Restriction & d = field.restriction( entity_type , p );

    if ( d.stride[0] ) { // Exists
      const unsigned n = shards::array_traits::stride_size( field_num_dim , d.stride );
...
struct EntityLess {
  bool operator()( const stk::mesh::Entity * const lhs , const stk::mesh::Entity * const rhs )
    { return lhs->identifier() < rhs->identifier(); }
};

void get_entities(
  std::vector< const stk::mesh::Entity * > & tmp ,
  ...
  stk::mesh::Part & part )
{
  ...
  for ( std::vector<stk::mesh::Bucket*>::const_iterator ik = ks.begin() ; ik != ks.end() ; ++ik ) {
    if ( has_superset( **ik, part ) ) { count += (*ik)->size(); }
  }

  {
    //const stk::mesh::Entity * const en = NULL ;
    //tmp.assign( count , en );
    tmp.reserve( count );
  }

  for ( std::vector<stk::mesh::Bucket*>::const_iterator ik = ks.begin() ; ik != ks.end() ; ++ik ) {
    if ( has_superset( **ik, part ) ) {
      for ( unsigned i = 0 ; i < (*ik)->size() ; ++i ) {
        stk::mesh::Entity * e = &(**ik)[i];
//         if (type==stk::mesh::Element and not active_element(e))
//           continue;
        tmp.push_back( e );
      }
    }
  ...
  stk::mesh::Part & part3 )
{
  ...
  for ( std::vector<stk::mesh::Bucket*>::const_iterator ik = ks.begin() ; ik != ks.end() ; ++ik ) {
    if ( has_superset( **ik, part1 ) &&
         has_superset( **ik, part2 ) &&
         has_superset( **ik, part3 ) ) { count += (*ik)->size(); }
  }
...
FileOutput::FileOutput(
  ...
  const stk::mesh::BulkData & arg_mesh ,
  stk::mesh::Part & active_part ,
    ...
    m_max_buffer( 0x0200000 )
{
  ...
  const stk::mesh::BulkData       & M  = arg_mesh ;
  ...
  const std::vector<const FilePart *> & node_parts = FS.parts( stk::mesh::Node );
  const std::vector<const FilePart *> & edge_parts = FS.parts( stk::mesh::Edge );
  const std::vector<const FilePart *> & face_parts = FS.parts( stk::mesh::Face );
  ...
  for ( std::vector< const stk::mesh::FieldBase * >::const_iterator
    ...
    if ( d.stride[0] ) {
      ...
      if ( ! f_num_dim ) {
        ...
        const unsigned n = shards::array_traits::stride_size( f_num_dim , d.stride );
  ...
  for ( std::vector< const stk::mesh::FieldBase * >::const_iterator
    ...
    for ( std::vector<stk::mesh::FieldBase::Restriction>::const_iterator
          ...
          j != f.restrictions().end() ; ++j ) {

      if ( entity_rank( j->key ) == stk::mesh::Element )
      {
        const stk::mesh::FieldBase::Restriction & d = *j;
        const shards::array_traits::int_t f_num_dim = f.rank();
        ...
        if ( ! f_num_dim ) {
          name_elem_var.push_back( f.name() );
          tmp.m_var_index = name_elem_var.size();
          m_field_elem.push_back( tmp );
        }
        else {
          const unsigned n = shards::array_traits::stride_size( f_num_dim , d.stride );
          for ( unsigned k = 0 ; k < n ; ++k ) {
            stk::mesh::Part & p = active_part; // d.ordinal();
            name_elem_var.push_back( variable_name(stk::mesh::Element, p, f, k) );
            tmp.m_offset    = k ;
            tmp.m_var_index = name_elem_var.size();
            m_field_elem.push_back( tmp );
          }
...

  {
    ...
    insert( ps , owns_part );
    insert( ps , active_part );

    part_count_local[ 0 ] = count( M.buckets( stk::mesh::Node ) , ps );
    part_count_local[ 1 ] = count( M.buckets( stk::mesh::Edge ) , ps );
    part_count_local[ 2 ] = count( M.buckets( stk::mesh::Face ) , ps );
    part_count_local[ 3 ] = count( M.buckets( stk::mesh::Element ) , ps );
  }

  unsigned n = 4 ;
  for ( int i = 0 ; i < num_node_sets ; ++i , ++n ) {
    stk::mesh::Part & part = node_parts[i]->m_part ;
    stk::mesh::PartVector ps ;
    insert( ps , part );
    insert( ps , owns_part );
    insert( ps , active_part );
    part_count_local[ n ] = count( M.buckets( stk::mesh::Node ) , ps );
  }

  for ( int i = 0 ; i < num_edge_sets ; ++i , ++n ) {
    stk::mesh::Part & part = edge_parts[i]->m_part ;
    stk::mesh::PartVector ps ;
    insert( ps , part );
    insert( ps , owns_part );
    insert( ps , active_part );
    part_count_local[ n ] = count( M.buckets( stk::mesh::Edge ) , ps );
  }

  for ( int i = 0 ; i < num_face_sets ; ++i , ++n ) {
    stk::mesh::Part & part = face_parts[i]->m_part ;
    stk::mesh::PartVector ps ;
    insert( ps , part );
    insert( ps , owns_part );
    insert( ps , active_part );
    part_count_local[ n ] = count( M.buckets( stk::mesh::Face ) , ps );
  }

  for ( int i = 0 ; i < num_elem_blk ; ++i , ++n ) {
    stk::mesh::Part & part = elem_parts[i]->m_part ;
    stk::mesh::PartVector ps ;
    insert( ps , part );
    insert( ps , owns_part );
    insert( ps , active_part );
    part_count_local[ n ] = count( M.buckets( stk::mesh::Element ) , ps );
  }

  stk::all_reduce_sum( p_comm , & part_count_local[0] , & m_global_counts[0] , n );
  ...
  if ( writer ) {
    ...
    if ( ! exo_error ) {
      ...
      const shards::ArrayDimTag & dim_tag = *FS.m_field_node_coord.dimension_tags()[0];
  ...
  if ( ! exo_error ) {
    ...
    std::vector< const stk::mesh::Entity * > tmp ;
    get_entities( tmp , M , stk::mesh::Node , owns_part );
  ...
  if ( ! exo_error ) {
    ...
    for ( int j = 0 ; j < num_elem_blk ; ++j ) {
      ...
      std::vector<const stk::mesh::Entity *> tmp ;
      get_entities( tmp , M , stk::mesh::Element , owns_part , active_part , elem_parts[j]->m_part );
...
void pack_data_2( const double * const b , stk::CommBuffer & buf )
{ buf.pack<double>( b , 2 ); }

template<typename T>
void pack_entity_data(
  const std::vector<const stk::mesh::Entity *>::const_iterator ib ,
  const std::vector<const stk::mesh::Entity *>::const_iterator ie ,
  const FileSchema::IndexField      & f_index ,
  const stk::mesh::FieldBase                   & f_data ,
...
void FileOutput::write( int step, double time_value )
{
  static const char method[] = "phdmesh::exodus::FieldIO::write" ;
  ...
  {
    const int num_nodes_global = m_global_counts[ stk::mesh::Node ] ;
    ...
    for ( std::vector<FieldIO>::iterator
      ...
      for ( unsigned j = 0 ; j < num_elem_parts ; ++j ) {
        ...
        if ( part == NULL || part == elem_parts[j] ) {
          ...
          std::vector<const stk::mesh::Entity *> tmp ;
          get_entities( tmp , M , stk::mesh::Element, owns_part , m_active_part , elem_parts[j]->m_part );
