...
#include "structure_xstructure_xstr_multi_io.H"
...
XSTR::MultiDiscretizationWrapper::MultiDiscretizationWrapper()
    : isinit_(false), issetup_(false), isfilled_(false), max_num_reserved_dofs_per_node_(-1)
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::Init(const std::string& name,
...
void XSTR::MultiDiscretizationWrapper::Init(
    const std::string& name, const XDisVec& discret_vec, const int& max_num_reserved_dofs_per_node)
{
  issetup_ = false;
...
void XSTR::MultiDiscretizationWrapper::Setup()
{
   ...
   * call Setup(). */
  writers_wrapper_ = Teuchos::rcp<XSTR::IO::MultiDiscretizationWriter>(
      new XSTR::IO::MultiDiscretizationWriter(Teuchos::rcp(this, false)));
    ...
    const Teuchos::RCP<DRT::DiscretizationInterface>& discret_ptr, const bool& check_insertion)
{
  ...
  const enum XFEM::FieldName fieldname = XFEM::String2FieldName(discret_ptr->Name());

  if (check_insertion)
  {
    std::pair<XDisMap::iterator, bool> ret = discret_map_.insert(XDisPair(fieldname, discret_ptr));

    if (not ret.second)
      dserror("The discretization \"%s\" was already inserted!", discret_ptr->Name().c_str());
  }
  else
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::ExtractVector(const Epetra_MultiVector& full,
    enum XFEM::FieldName field, Epetra_MultiVector& partial, enum ::IO::VectorType vt) const
{
  switch (vt)
  {
    case ::IO::dofvector:
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::InsertVector(const Epetra_MultiVector& partial,
    enum XFEM::FieldName field, Epetra_MultiVector& full, enum ::IO::VectorType vt) const
{
  switch (vt)
  {
    case ::IO::dofvector:
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::AddVector(const Epetra_MultiVector& partial,
    enum XFEM::FieldName field, Epetra_MultiVector& full, double scale,
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Teuchos::RCP<IO::DiscretizationWriter> XSTR::MultiDiscretizationWrapper::Writer() const
{
  CheckInitSetup();
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
bool XSTR::MultiDiscretizationWrapper::IsXFEM(enum XFEM::FieldName field) const
{
...
void XSTR::MultiDiscretizationWrapper::Print(std::ostream& os) const
{
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    {
      os << "\n\n::::::::::::::::::::::::::::::::::::::::::::::::::::::::::\n";
      os << "            " << XFEM::FieldName2String(cit->first) << "-DISCRETIZATION";
      os << "\n::::::::::::::::::::::::::::::::::::::::::::::::::::::::::\n\n";
    }
    cit->second->Print(os);
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
int XSTR::MultiDiscretizationWrapper::FillComplete(bool assigndegreesoffreedom, bool initelements,
    bool doboundaryconditions, bool buildsystemmaps, bool setupmapextractor)
{
  CheckInit();
  Reset();
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ret = cit->second->FillComplete(assigndegreesoffreedom, initelements, doboundaryconditions);
    if (ret != 0)
      dserror(
          "FillComplete failed on discretization %s with the "
          "error code %d!",
          cit->second->Name().c_str(), ret);
  }

  if (setupmapextractor) map_extractor_->Setup();
...
void XSTR::MultiDiscretizationWrapper::BuildMergedNodes()
{
  ...
  for (int nlid = 0; nlid < my_num_nodes; ++nlid)
  {
    std::pair<std::map<int, Teuchos::RCP<DRT::Node>>::iterator, bool> check =
        merged_node_.insert(std::pair<int, Teuchos::RCP<DRT::Node>>(
            ngids[nlid], Teuchos::rcp<DRT::Node>(map_extractor_->gINode(ngids[nlid]), false)));

    if (not check.second)
      dserror(
          "The interface row node with GID %d could not be inserted, because "
          "the node GID is not unique!",
          ngids[nlid]);
  }
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ngids = map_extractor_->NodeRowMap(cit->first, XFEM::MULTIFIELD::block_non_interface)
                ->MyGlobalElements();
    my_num_nodes = map_extractor_->NodeRowMap(cit->first, XFEM::MULTIFIELD::block_non_interface)
                       ->NumMyElements();
    for (int nlid = 0; nlid < my_num_nodes; ++nlid)
    {
      std::pair<std::map<int, Teuchos::RCP<DRT::Node>>::iterator, bool> check =
          merged_node_.insert(std::pair<int, Teuchos::RCP<DRT::Node>>(
              ngids[nlid], Teuchos::rcp<DRT::Node>(cit->second->gNode(ngids[nlid]), false)));

      if (not check.second)
        dserror(
            "The non-interface row node with GID %d could not be inserted, "
            "because the node GID is not unique!",
            ngids[nlid]);
    }
  }
}
...
void XSTR::MultiDiscretizationWrapper::BuildMergedNodeRowMap()
{
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    merged_node_row_map_ = LINALG::MergeMap(*merged_node_row_map_,
        *map_extractor_->NodeRowMap(cit->first, XFEM::MULTIFIELD::block_non_interface), false);
  }
}
    ...
    for (int lid = 0; lid < num_my_row_eles; ++lid)
    {
      std::pair<std::map<int, Teuchos::RCP<DRT::Element>>::iterator, bool> check =
          merged_element_.insert(std::pair<int, Teuchos::RCP<DRT::Element>>(
              egids[lid], Teuchos::rcp<DRT::Element>(cit->second->gElement(egids[lid]), false)));

      if (not check.second)
        dserror(
            "The row element with GID %d could not be inserted, because the"
            "element GID is not unique!",
            egids[lid]);
    }
  }
}
...
void XSTR::MultiDiscretizationWrapper::BuildMergedElementRowMap()
{
  ...
  for (cit = merged_element_.begin(); cit != merged_element_.end(); ++cit)
  {
    ...
    if (cit->second->Owner() == Comm().MyPID())
      ...
      dserror(
          "The merged_element map is supposed to contain only "
          "row elements (MultiDiscretizationWrapper). Something went "
          "wrong at element GID = %d!",
          cit->first);
  }
...
{
  if (not Filled()) dserror("FillComplete() must be called before call to NodeRowMap()");
...
{
  if (not Filled()) dserror("FillComplete() must be called before call to ElementRowMap()");
...
{
  if (not Filled()) dserror("FillComplete() must be called before call to NumMyRowNodes()");
...
{
  std::map<int, Teuchos::RCP<DRT::Node>>::const_iterator curr = merged_node_.find(gid);
...
{
  std::map<int, Teuchos::RCP<DRT::Node>>::const_iterator cit = merged_node_.find(gid);

  if (cit == merged_node_.end())
    dserror(
        "No direct access to the node with GID %d. The merged_node_ map "
...
{
  if (not Filled()) dserror("FillComplete() must be called before call to lRowNode()");
...
int XSTR::MultiDiscretizationWrapper::NumMyRowElements() const
{
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    if (mynum == -1)
      dserror(
          "The discretization %s returned a number of my row elements of"
          " -1!",
...
{
  if (not Filled()) dserror("FillComplete() must be called before call to lRowElement()");
...
{
  std::map<int, Teuchos::RCP<DRT::Element>>::const_iterator cit = merged_element_.find(gid);

  if (cit == merged_element_.end())
    dserror(
        "No direct access to the element with GID %d. The merged_element_ "
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
const Epetra_Map* XSTR::MultiDiscretizationWrapper::DofRowMap(unsigned nds) const
{
  CheckInit();

  if (nds > 0)
    dserror(
        "The MultiDiscretizationWrapper provides only the combined \n"
...
bool XSTR::MultiDiscretizationWrapper::HaveDofs() const
{
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    if (not cit->second->HaveDofs()) have_dofs = false;
  }
  return have_dofs;
}
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
int XSTR::MultiDiscretizationWrapper::NumDof(unsigned dofset_id, const DRT::Node* node) const
{
  if (dofset_id > 0)
    dserror(
        "The MultiDiscretizationWrapper provides only the combined \n"
  ...
  if (not NodeRowMap()->MyGID(ngid))
    dserror(
        "The given node (GID: %d) is no part of the MultiDiscretizationWrapper. \n"
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
int XSTR::MultiDiscretizationWrapper::NumStandardDof(
    const unsigned& dofset_id, const DRT::Node* node) const
{
  if (dofset_id > 0)
    dserror(
        "The MultiDiscretizationWrapper provides only the combined \n"
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
std::vector<int> XSTR::MultiDiscretizationWrapper::Dof(unsigned nds, const DRT::Node* node) const
{
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::Dof(std::vector<int>& dof, const DRT::Node* node,
    unsigned dofset_id, unsigned nodaldofset_id, const DRT::Element* element) const
{
  if (dofset_id > 0)
    dserror(
        "The MultiDiscretizationWrapper provides only the combined \n"
  ...
  if (not NodeRowMap()->MyGID(ngid))
    dserror(
        "The given node is no part of the MultiDiscretizationWrapper. Maybe \n"
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::GetCondition(
    const std::string& name, std::vector<DRT::Condition*>& out) const
{
  ...
  for (unsigned i = 0; i < rcp_out.size(); ++i) out[i] = rcp_out[i].get();
}
    ...
    const std::string& name, std::vector<Teuchos::RCP<DRT::Condition>>& out) const
{
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    if (count != partial_out.size())
      dserror(
          "Mismatch in partial number of conditions found!\n"
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
DRT::Condition* XSTR::MultiDiscretizationWrapper::GetCondition(const std::string& name) const
{
  XDisMap::const_iterator cit;
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    if (cond->Geometry().size() == 0 and cond->Nodes()->size() == 0) continue;
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::SetState(
    unsigned nds, const std::string& name, Teuchos::RCP<const Epetra_Vector> full_state)
{
  if (nds > 0)
    dserror(
        "Currently we support only one dofset in this SetState method,"
        "due to the way how the ExtractVector method is working.");

  XDisMap::const_iterator cit;
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    Teuchos::RCP<Epetra_Vector> partial = map_extractor_->ExtractVector(*full_state, cit->first);
    ...
    unsigned nds, const std::string& name) const
{
  if (nds > 0)
    dserror(
        "Currently we support only one dofset in this GetState method,"
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    dserror(
        "Check if we have to sum the interface DoF's up??? Currently we "
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
const Teuchos::RCP<LINALG::SparseOperator>& XSTR::MultiDiscretizationWrapper::XFieldSysMat(
    enum XFEM::FieldName field, unsigned mat_id)
{
  ...
  {
    std::pair<XSysMatMap::iterator, bool> check = partial_system_matrices_.insert(XSysMatPair(
        field, std::vector<Teuchos::RCP<LINALG::SparseOperator>>((mat_id + 1), Teuchos::null)));

    // this should not/cannot happen
    if (not check.second) dserror("The new pair couldn't be inserted!");
    // set the iterator
    mat_it = check.first;
  }
  // ------------------------------------------------------------------------
  ...
  {
    if (mat_it->second.size() < (mat_id + 1)) mat_it->second.resize((mat_id + 1), Teuchos::null);
  }
  ...
  if (sys_mat.is_null())
  {
    sys_mat = Teuchos::rcp<LINALG::SparseOperator>(
        new LINALG::BlockSparseMatrix<LINALG::DefaultBlockMatrixStrategy>(
            map_extractor_->SlDofMapExtractor(field), map_extractor_->SlDofMapExtractor(field), 81,
            true, true));
  }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::EvaluateNeumann(Teuchos::ParameterList& params,
    Epetra_Vector& full_systemvector, LINALG::SparseOperator* full_systemmatrix)
{
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    // ------------------------------------------------------------------------
    psys_vec = Teuchos::rcp(new Epetra_Vector(*cit->second->DofRowMap(), true));
    ...
    // ------------------------------------------------------------------------
    map_extractor_->AddVector(*psys_vec, cit->first, full_systemvector, 1.0);
    ...
    // ------------------------------------------------------------------------
    if (full_systemmatrix)
      map_extractor_->AddMatrix(*XFieldSysMat(cit->first, 0), cit->first, *full_systemmatrix, 1.0);
  }
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::Evaluate(Teuchos::ParameterList& params,
    Teuchos::RCP<LINALG::SparseOperator> full_systemmatrix1,
    Teuchos::RCP<LINALG::SparseOperator> full_systemmatrix2,
    Teuchos::RCP<Epetra_Vector> full_systemvector1, Teuchos::RCP<Epetra_Vector> full_systemvector2,
  ...
  for (cit = discret_map_.begin(); cit != discret_map_.end(); ++cit)
  {
    ...
    for (unsigned i = 0; i < 3; ++i)
    {
      if (not full_systemvectors[i].is_null())
        psys_vec[i] =
            Teuchos::rcp<Epetra_Vector>(new Epetra_Vector(*cit->second->DofRowMap(0), true));
    }

    // ------------------------------------------------------------------------
    // get the partial system matrices
    // ------------------------------------------------------------------------
    if (not full_systemmatrix1.is_null()) psys_mat[0] = XFieldSysMat(cit->first, 0);
    if (not full_systemmatrix2.is_null()) psys_mat[1] = XFieldSysMat(cit->first, 1);

    // ------------------------------------------------------------------------
    // evaluate elements on each wrapped field discretization
    // ------------------------------------------------------------------------
    cit->second->Evaluate(params, psys_mat[0], psys_mat[1], psys_vec[0], psys_vec[1], psys_vec[2]);
    ...
    for (unsigned i = 0; i < 3; ++i)
    {
      if (not full_systemvectors[i].is_null())
      {
        map_extractor_->AddVector(*psys_vec[i], cit->first, *full_systemvectors[i], 1.0);
    ...
    for (unsigned i = 0; i < 2; ++i)
    {
      if (not full_systemmatrices[i].is_null())
      {
        ...
        if (not psysmat_ptr->Filled()) psysmat_ptr->Complete();
        map_extractor_->AddMatrix(*psysmat_ptr, cit->first, *full_systemmatrices[i], 1.0);
      }
    }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void XSTR::MultiDiscretizationWrapper::EvaluateCondition(Teuchos::ParameterList& params,
