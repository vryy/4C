...
for (j=0; j<actfield->dis[0].numnp; j++)
  actfield->dis[0].node[j].Id_loc = j;
...
#ifdef DEBUG
  dstrc_enter("init_dof_discretization");
#endif
/*----------------------------------------------------------------------*/
  ...
  /* notice: bandwith opimization has been done by gid, so just do a
     field-local renumbering of this */
  for (i=0; i<actdis->numnp; i++) actdis->node[i].Id_loc = i;
  ...
  for (i=0; i<actdis->numele; i++)
  {
    ...
    switch(actele->eltyp)
    {
        case el_shell8:
          ...
          {
            if (actele->node[j]->numdf < NUMDOF_SHELL8) actele->node[j]->numdf=NUMDOF_SHELL8;
          }
          break;
        case el_shell9:
#ifdef D_SHELL9
          ...
          for (j=0; j<actele->numnp; j++)
          {
            actele->node[j]->numdf = actele->e.s9->numdf;
          }
#endif /*D_SHELL9*/
          break;
        case el_beam3:
          for (j=0; j<actele->numnp; j++)
          {
            if (actele->node[j]->numdf < 6) actele->node[j]->numdf=6;
          }
          break;
        case el_brick1:
          for (j=0; j<actele->numnp; j++)
          {
            if (actele->node[j]->numdf < 3) actele->node[j]->numdf=3;
          }
          break;
        case el_wall1:
          for (j=0; j<actele->numnp; j++)
          {
            if (actele->node[j]->numdf < 2) actele->node[j]->numdf=2;
          }
          break;
        case el_fluid2:
          for (j=0; j<actele->numnp; j++)
      ...
      for (j=0; j<actele->numnp; j++)
      {
        if (actele->node[j]->numdf < 5) actele->node[j]->numdf=5;
      }
      break;
#endif
        case el_fluid3:
          for (j=0; j<actele->numnp; j++)
          {
            if (actele->node[j]->numdf < 4) actele->node[j]->numdf=4;
          }
          break;
        case el_ale3:
          for (j=0; j<actele->numnp; j++)
        ...
        case el_axishell:
          for (j=0; j<actele->numnp; j++)
          {
            if (actele->node[j]->numdf < 2) actele->node[j]->numdf=3;
          }
          break;
#ifdef D_INTERF
        case el_interf:
        ...
        case el_ls2:
          for (j=0; j<actele->numnp; j++)
            if (actele->node[j]->numdf < 1) actele->node[j]->numdf=1;
          break;
#endif
        default:
          dserror("Unknown type of element, cannot assign number of dofs");
  ...
  {
    actdis->node[i].dof  = (INT*)CCACALLOC(actdis->node[i].numdf,sizeof(INT));
    if (!(actdis->node[i].dof))
      dserror("Allocation of dof in NODE failed");
    /*------------------------- allocate the arrays to hold solutions */
    max = IMAX(3,actdis->node[i].numdf);

    amdef("sol",&(actdis->node[i].sol),1,max,"DA");
    amzero(&(actdis->node[i].sol));

    amdef("sol_incr",&(actdis->node[i].sol_increment),1,max,"DA");
    amzero(&(actdis->node[i].sol_increment));

    amdef("sol_res",&(actdis->node[i].sol_residual),1,max,"DA");
    amzero(&(actdis->node[i].sol_residual));
  }
...
/*----------------------------------------------------------------------*/
#ifdef DEBUG
dstrc_enter("assign_dof_discretization");
#endif
/*----------------------------------------------------------------------*/

/*------------------------------------------------- init all dofs to -2 */
for (i=0; i<actdis->numnp; i++)
  for (j=0; j<actdis->node[i].numdf; j++)
    actdis->node[i].dof[j]=-2;
...
for (j=0; j<actdis->numnp; j++)
{
   ...
   for (l=0; l<actnode->numdf; l++)
   {
	 ...
	 if (actnode->gnode->couple->couple.a.ia[l][0]!=0)
	 {
	     iscouple_find_node_comp_discretization(
               actnode,
               actdis,
               &partnernode,
               actnode->gnode->couple->couple.a.ia[l][0],
...
/*--------------------------------------- the node does have conditions */
   else
   {
	 ...
	 for (l=0; l<actnode->numdf; l++)
	 {
	    ...
	    {
	       coupleID = actnode->gnode->couple->couple.a.ia[l][1];
	       find_assign_coupset_discretization(actdis,coupleID,&counter);
	    }
	    else if (dirich==0 && couple==0 && geocouple==1)
	    {
	       /* the coupling Id */
...
actdis->numdf = counter;
/*----------------------------------------------------------------------*/
#ifdef DEBUG
dstrc_exit();
#endif
return;
} /* end of assign_dof_discretization */
...
#ifdef DEBUG
  dstrc_enter("init_dof_discretization_xfem");
#endif
/*----------------------------------------------------------------------*/

  /* check */
  if (xfem_data.xfem_optimize != 1)
    dserror("xfem_data.xfem_optimize!=1");
  ...
  */
  ...
  for (i=0; i<actdis->numnp; i++)
  {
    ...
    if (!(actdis->node[i].dof))
      dserror("Allocation of dof in NODE failed");
    /*------------------------- allocate the arrays to hold solutions */
    max = IMAX(5,actdis->node[i].numdf);
...
#endif
  return;
} /* end of init_dof_discretization_xfem */
#endif
  ...
  )
{
  INT i,j,l;                         /* some counters */
  INT counter;
  INT dirich;                        /* flags for conditions */
  NODE    *actnode;                  /* node and coupling partner in progress */

/*----------------------------------------------------------------------*/
#ifdef DEBUG
  dstrc_enter("assign_dof_discretization_xfem");
#endif
/*----------------------------------------------------------------------*/
...
#endif
  return;
} /* end of assign_dof_discretization_xfem */
#endif
