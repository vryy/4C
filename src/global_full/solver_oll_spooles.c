...
/*!----------------------------------------------------------------------
\brief controls solver Spooles for oll Matrices

<pre>                                                              mn 05/03
This function controls the solver Spooles for the solution of Matrices in
the oll format.
</pre>
\param *actsolv        FIELD  (i) the active solver
\param *actpart        PARTITION    (i) the active partition
\param *spo            SPOOLMAT     (i) the msr matrix
    ...
    case 0:
      ...
      if (spo->is_factored==0)
      {
        ...
        for (i=0; i<oll->rdim; i++)
        {
          ...
          while(actentry != NULL )
          {
            InpMtx_inputRealEntry(spo->mtxA,actentry->r,actentry->c,actentry->val);
