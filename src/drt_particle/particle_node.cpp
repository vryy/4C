...
void PARTICLE::ParticleNode::Print(std::ostream& os) const
{
  ...
  if (history_particle_.size()) os << "in contact with particles (Id): ";
  for (std::map<int, Collision>::const_iterator i = history_particle_.begin();
       i != history_particle_.end(); ++i)
  {
    os << i->first << "  ";
  }

  if (history_particle_adhesion_.size()) os << "in adhesive interaction with particles (Id): ";
  for (std::map<int, Adhesion>::const_iterator i = history_particle_adhesion_.begin();
       i != history_particle_adhesion_.end(); ++i)
  {
    os << i->first << "  ";
  }

  if (history_particle_.size()) os << "in contact with walls (Id): ";
  for (std::map<int, Collision>::const_iterator i = history_particle_.begin();
       i != history_particle_.end(); ++i)
  {
    os << i->first << "  ";
  ...
  if (history_particle_adhesion_.size()) os << "in adhesive interaction with walls (Id): ";
  for (std::map<int, Adhesion>::const_iterator i = history_particle_adhesion_.begin();
       i != history_particle_adhesion_.end(); ++i)
  {
    os << i->first << "  ";
...
void PARTICLE::ParticleNode::Pack(DRT::PackBuffer& data) const
{
  ...
  for (std::map<int, Collision>::const_iterator i = history_wall_.begin(); i != history_wall_.end();
       ++i)
  {
  ...
  for (std::map<int, Adhesion>::const_iterator i = history_wall_adhesion_.begin();
       i != history_wall_adhesion_.end(); ++i)
  {
    // one collision event with a wall element
    AddtoPack(data, i->first);
    AddtoPack(data, i->second.adhesion);
