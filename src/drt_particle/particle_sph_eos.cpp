...
double PARTICLE::EquationOfState_GenTait::DensityToPressure(
    const double& density, const double& density0)
{
  if (exponent_ == 1)
    ...
    const double& density, const double& mass, const double& density0)
{
  ...
  if (exponent_ == 1)
    return -std::pow(speedOfSound_, 2) * mass *
           (std::log(std::pow(mass, 2) / (density0 * density)) -
...
/*----------------------------------------------------------------------*
 | constructor                                             sfuchs 11/17 |
 *----------------------------------------------------------------------*/
PARTICLE::EquationOfState_IdealGas::EquationOfState_IdealGas(const double& speedOfSound)
    : EquationOfState_Base(), speedOfSound_(speedOfSound)
{
  return;
}
