 ...
 *----------------------------------------------------------------------*/
CAVITATION::Algorithm::Algorithm(const Epetra_Comm& comm, const Teuchos::ParameterList& params)
    : PARTICLE::Algorithm(comm, params),
      dim_(3),
      coupalgo_(DRT::INPUT::IntegralValue<INPAR::CAVITATION::CouplingStrategyOverFields>(
          params, "COUPALGO")),
      void_frac_strategy_(DRT::INPUT::IntegralValue<INPAR::CAVITATION::VoidFractionCalculation>(
          params, "VOID_FRACTION_CALC")),
      fluidfrac_reconstr_(
          DRT::INPUT::IntegralValue<INPAR::CAVITATION::FluidFracReconstructionMethod>(
              params, "FLUIDFRAC_PROJ_METHOD")),
      ...
      blendingsteps_(0),
      initbubblevelfromfluid_(
          (bool)DRT::INPUT::IntegralValue<int>(params, "INIT_BUBBLEVEL_FROM_FLUID")),
      ...
      computeradiusRPbased_(
          (bool)DRT::INPUT::IntegralValue<int>(params, "COMPUTE_RADIUS_RP_BASED")),
      dtsub_(Teuchos::null),
      pg0_(Teuchos::null),
      count_(0),
      del_(Teuchos::null),
      ...
      storepg0_(Teuchos::null)
{
  ...
  Teuchos::RCP<ADAPTER::FluidBaseAlgorithm> fluid = Teuchos::rcp(new ADAPTER::FluidBaseAlgorithm(
      DRT::Problem::Instance()->CavitationParams(), fluidparams, "fluid", false));
  ...
  if (gravity_acc_.Norm2() > 0.0)
  {
    ...
    if (condition.size() != 1)
      dserror(
          "exactly one VOL NEUMANN boundary condition expected to represent body forces in fluid");
    ...
    for (int i = 0; i < dim_; ++i)
    {
      ...
      if (gravity_acc_(i) != 0.0 and (*onoff)[i] == 0)
        dserror(
            "body force for %d. dof deactivated in VOL NEUMANN bc for fluid although body force "
            "acts on particles in this direction.",
            i);
    }
    ...
    if (startfuncno < 0) dserror("pressure field needs to be initialized due to gravity load");
  }
  ...
  if (!simplebubbleforce_)
  {
    INPAR::FLUID::GradientReconstructionMethod recomethod =
        DRT::INPUT::IntegralValue<INPAR::FLUID::GradientReconstructionMethod>(
            fluidparams, "VELGRAD_PROJ_METHOD");
    if (recomethod == INPAR::FLUID::gradreco_none)
      dserror(
          "Please specify which gradient-reconstruction method you want to use: check "
          "VELGRAD_PROJ_METHOD");
    ...
    if (recomethod == INPAR::FLUID::gradreco_l2)
    {
      ...
      if (fluidparams.get<int>("VELGRAD_PROJ_SOLVER") < 0)
        dserror(
            "no solver for L2 projection of velocity gradient specified: check "
            "VELGRAD_PROJ_SOLVER");
    }
  }
  ...
  if (coupalgo_ == INPAR::CAVITATION::TwoWayFull_strong && not computeradiusRPbased_)
    dserror(
        "iteratively staggered coupling scheme is tailored for use with radius adaptions using RP "
        "equation");
      ...
      coupalgo_ == INPAR::CAVITATION::TwoWayFull_strong)
  {
    ...
    if (id == -1) dserror("no cavitation fluid material specified");
    ...
    if (fluidfrac_reconstr_ == INPAR::CAVITATION::fluidfracreco_l2)
    {
      if (DRT::Problem::Instance()->CavitationParams().get<int>("FLUIDFRAC_PROJ_SOLVER") < 0)
        dserror(
            "no solver for L2 projection of fluid fraction specified: check FLUIDFRAC_PROJ_SOLVER");
    }
  }
  else
  {
    ...
    if (fluid_->TimIntScheme() == INPAR::FLUID::timeint_afgenalpha)
      dserror(
          "momentum coupled or one-way coupled cavitation problem does not work with TIMEINTEGR = "
          "Af_Gen_Alpha");

    // check for correct physical type of fluid
    if (fluid_->PhysicalType() != INPAR::FLUID::incompressible)
      dserror(
          "two way momentum and one way coupled cavitation problems only works with PHYSICAL_TYPE "
          "= Incompressible");

    // check fluid material
    int id = DRT::Problem::Instance()->Materials()->FirstIdByType(INPAR::MAT::m_fluid);
    if (id == -1) dserror("specify fluid material");
  }

  if (timestepsizeratio_ < 1)
    dserror("fluid time step must be a multiplicative greater or equal unity. Your choice: %d",
        timestepsizeratio_);
  ...
  if (DRT::Problem::Instance()->Restart() != 0)
  {
    ...
    restartparticles_ =
        DRT::Problem::Instance()->CavitationParams().get<int>("RESTARTSTEP_PARTICLES");
...
void CAVITATION::Algorithm::TimeloopIterStaggered()
{
  ...
  while (NotFinished() || (particles_->StepOld() - restartparticles_) % timestepsizeratio_ != 0)
  {
    ...
    while (not converged)
    {
      ...
      if ((particles_->Step() - restartparticles_) % timestepsizeratio_ == 1 or
          timestepsizeratio_ == 1)
      {
      ...
      if ((particles_->Step() - restartparticles_) % timestepsizeratio_ == 0)
        ConvergenceCheckAndRelaxation(
            outeriter, pressnorm_L2, radiusnorm_L2, converged, particlereset);
      ...
      // transfer particles into their correct bins at least every 10th of the fluid time step size;
      // underlying assumptions: CFL number will be not too far from one, bubbles have appr. the
      // same velocity as the fluid
      if (particles_->Time() > Time() - Dt() + 0.1 * count_ * Dt() ||
          BinStrategy()->HavePBC() == true)
      {
        ++count_;
        ...
        deletedparticlesduringsubcycling_.insert(deletedparticlesduringsubcycling_.end(),
            deletedparticles->begin(), deletedparticles->end());
      }

      // update displacements, velocities, accelerations
      // after this call we will have disn_==dis_, etc
      // update time and step
      if (particlereset == false) Update(converged);
    }
...
void CAVITATION::Algorithm::ComputePressAndRadiusNorm(double& pressnorm_L2, double& radiusnorm_L2)
{
  ...
  {
    if (MyRank() == 0)
      std::cout << "radius norm is almost zero: " << radiusnorm_L2 << " --> set to one "
                << std::endl;
    radiusnorm_L2 = 1.0;
  }
 ...
 *----------------------------------------------------------------------*/
void CAVITATION::Algorithm::ConvergenceCheckAndRelaxation(const int outeriter,
    const double pressnorm_L2, const double radiusnorm_L2, bool& converged, bool& particlereset)
{
  // do not iterate if particle field is empty and leave here
  if (particles_->Radiusn()->GlobalLength() == 0)
  {
  ...
  if (outeriter == 1)
  {
    ...
    // dynamic relaxation
    {
      ...
      if (MyRank() == 0) std::cout << "omega for dynamic relaxation is: " << omega << std::endl;
  ...
  {
    // compute delta press ...
    Teuchos::RCP<Epetra_Vector> deltapress =
        Teuchos::rcp(new Epetra_Vector(*fluid_->ExtractPressurePart(fluid_->Velnp())));
    ...
    {
      if (MyRank() == 0)
        std::cout << "\n relative L2 error press: " << pressincnorm_L2 / pressnorm_L2 << " >? "
                  << ittol_ << "\n"
                  << " and relative L2 error radius: " << radiusincnorm_L2 / radiusnorm_L2 << " >? "
                  << ittol_ << "\n"
      ...
      // do dynamic relaxation
      {
        ...
        if (denom == 0.0) denom = 1.0e6;

        // Aikten factor
        // nu^{i+1} = nu^i + (nu^i -1) . (r^{i+1} - r^i)^T . (-r^{i+1}) / |r^{i+1} - r^{i}|^2
        // nom = ( r^{i+1} - r^i )^T . r^{i+1} --> use -nom
        // Uli's implementation: mu_ = mu_ + (mu_ - 1.0) * nom / (denom*denom). with '-' included in
        // nom
        ...
        if (MyRank() == 0)
          std::cout << " info: abs norm of difference of coupling radius and real radius is: "
                    << diffnorm << "\n"
      ...
      particlereset = true;
      // care about time and step already here as this must be already reset for next call to
      // PrepareRelaxation()
      particles_->SetTimeStep(storetime_, storestep_);
    }
    else
    {
      ...
      if (MyRank() == 0)
      {
        std::cout << "\n relative L2 error press: " << pressincnorm_L2 / pressnorm_L2 << " < "
                  ...
                  << " info: abs norm of difference of coupling radius and real radius is: "
                  << diffnorm << "\n"
                  << " info: relative norm of differences is: " << diffnorm / radiusnorm
                  << std::endl;
      }
    ...
    if (outeriter > itmax_) dserror("maximum number (%i) of outer iterations reached", itmax_);
  }
...
void CAVITATION::Algorithm::ResetParticleData()
{
  ...
  else  // recover old layout in case particles have left the domain
  {
    ...
    for (int lid = 0; lid < oldnoderowmap.NumMyElements(); ++lid)
    {
      ...
      for (int d = 0; d < dim_; ++d) pos[d] = (*storedis_)[lid * 3 + d];
      Teuchos::RCP<DRT::Node> oldparticle =
          Teuchos::rcp(new PARTICLE::ParticleNode(oldnodeid, &pos[0], MyRank()));
    ...
    for (std::list<int>::const_iterator it = deletedparticlesduringsubcycling_.begin();
         it != deletedparticlesduringsubcycling_.end(); ++it)
    {
 ...
 *----------------------------------------------------------------------*/
void CAVITATION::Algorithm::SetupSystem() { return; }
...
void CAVITATION::Algorithm::InitCavitation()
{
  ...
  Teuchos::RCP<Epetra_Map> fluidelecolmapold =
      Teuchos::rcp(new Epetra_Map(*fluiddis_->ElementColMap()));
  ...
  Teuchos::RCP<Epetra_Map> binrowmap =
      DistributeBinsToProcsBasedOnUnderlyingDiscret(fluiddis_, rowfluideles);
  ...
  Teuchos::RCP<Teuchos::ParameterList> adaptedcavitationdyn =
      Teuchos::rcp(new Teuchos::ParameterList(cavitationdyn));
  ...
  adaptedcavitationdyn->set<int>(
      "RESTARTEVRY", 1000000000);  // very large number as restart is enforced for particles
  adaptedcavitationdyn->set<int>(
      "RESULTSEVRY", timestepsizeratio_ * cavitationdyn.get<int>("RESULTSEVRY"));

  // create particle time integrator
  Teuchos::RCP<ADAPTER::ParticleBaseAlgorithm> particles = Teuchos::rcp(
      new ADAPTER::ParticleBaseAlgorithm(*adaptedcavitationdyn, BinStrategy()->BinDiscret()));
  ...
  if (initbubblevelfromfluid_) InitBubbleVelFromFluidVel();
...
void CAVITATION::Algorithm::InitBubblePressure()
{
  ...
  for (int i = 0; i < BinStrategy()->BinDiscret()->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    for (int d = 0; d < dim_; ++d) particleposition(d) = (*bubblepos)[posx + d];

    // state vector of fluid has already been set earlier ("veln")
    const bool fluidelefound = ComputePressureAtBubblePosition(
        currparticle, particleposition, elemat1, elemat2, elevec1, elevec2, elevec3);
    if (fluidelefound == false)
      dserror(
          "no underlying fluid element for pressure computation was found for bubble %d at "
          "positions %f %f %f on proc %d",
          currparticle->Id(), particleposition(0), particleposition(1), particleposition(2),
          MyRank());
...
void CAVITATION::Algorithm::InitBubbleVelFromFluidVel()
{
  ...
  for (int i = 0; i < BinStrategy()->BinDiscret()->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    const bool fluidelefound = ComputeVelocityAtBubblePosition(
        currparticle, particleposition, elemat1, elemat2, elevec1, elevec2, elevec3);
    if (fluidelefound == false)
      dserror(
          "no underlying fluid element for velocity computation was found for bubble %d at "
          "positions %f %f %f on proc %d",
          currparticle->Id(), particleposition(0), particleposition(1), particleposition(2),
          MyRank());

    for (int d = 0; d < dim_; ++d) (*bubblevelnp)[posx + d] = elevec1(d);
  }
...
void CAVITATION::Algorithm::SetCouplingStates(Teuchos::ParameterList& p, bool init)
{
  ...
  double theta = 1.0;
  if ((particles_->Step() - restartparticles_) % timestepsizeratio_ != 0)
    theta = (double)((particles_->Step() - restartparticles_) % timestepsizeratio_) /
            (double)timestepsizeratio_;
  ...
  if (not simplebubbleforce_)
  {
    ...
    if (timestepsizeratio_ == 1 || init)
    {
      ...
      velgradcol_interpol_ =
          Teuchos::rcp(new Epetra_MultiVector(*fluiddis_->NodeColMap(), dim_ * dim_));
    ...
    else
    {
      ...
      if ((particles_->Step() - restartparticles_) % timestepsizeratio_ == 1)
      {
        Teuchos::RCP<Epetra_MultiVector> tmp =
            FLD::UTILS::ProjectGradient(fluiddis_, fluid_->Veln(), false);
      ...
      Teuchos::RCP<Epetra_MultiVector> velgradcol =
          Teuchos::rcp(new Epetra_MultiVector(*velgradcolnp_));
  ...
  else
  {
    ...
    if (theta != 1.0) vel_interpol->Update(1.0 - theta, *velcoln_, theta);
...
void CAVITATION::Algorithm::SetFluidFraction()
{
  ...
  Teuchos::rcp_dynamic_cast<FLD::FluidImplicitTimeInt>(fluid_)->GenAlphaIntermediateValues(
      fluidfracaf, fluidfracam);
  ...
  for (std::set<int>::const_iterator it = inflowfluiddofs_.begin(); it != inflowfluiddofs_.end();
       ++it)
...
void CAVITATION::Algorithm::CalculateAndApplyForcesToParticles(bool init)
{
  ...
  const bool timeforcalcfluidforces =
      (Step() == 0 or ((particles_->Step() - restartparticles_) % timestepsizeratio_ == 0));

  // vectors to be filled with forces,
  // note: global assemble is needed for fluidforces due to the case with large bins and small fluid
  // eles
  Teuchos::RCP<Epetra_Vector> bubbleforces =
  ...
  else
  {
    // get fluid material
    int id = DRT::Problem::Instance()->Materials()->FirstIdByType(INPAR::MAT::m_fluid);
    if (id == -1) dserror("no fluid material specified");
    const MAT::PAR::Parameter* mat = DRT::Problem::Instance()->Materials()->ParameterById(id);
  ...
  for (int i = 0; i < BinStrategy()->BinDiscret()->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    const DRT::Element* targetfluidele = GetUnderlyingFluidEleData(currparticle, particleposition,
        elecoord, p, elevector1, elevector2, elevector3, elevector4, elevector5);

    // if no underlying fluid element could be found do not assemble forces for this bubble and
    // continue with next bubble
    if (targetfluidele == NULL) continue;
    ...
    for (int d = 0; d < dim_; ++d) v_rel(d) = elevector1[d] - v_bub(d);
    ...
    if (output)
    {
      ...
      std::cout << "pos_bub: " << particleposition(0) << " " << particleposition(1) << " "
                << particleposition(2) << " " << std::endl;
      std::cout << "radius_bub: " << r_bub << std::endl;
      std::cout << "v_bub: " << v_bub(0) << " " << v_bub(1) << " " << v_bub(2) << " " << std::endl;
      std::cout << "v_fl: " << elevector1[0] << " " << elevector1[1] << " " << elevector1[2] << " "
                << std::endl;
    ...
    for (int d = 0; d < dim_; ++d) rot_u(d) = elevector3(d);
    ...
    for (int d = 0; d < dim_; ++d) Du_Dt(d) = elevector2[d];
    ...
    else  // dbc controlled bubble
    {
      ...
      for (int d = 0; d < dim_; ++d) Dv_Dt(d) = (*bubbleacc)[posx + d];
    ...
    for (int d = 0; d < dim_; ++d)
    {
      ...
      if (v_rel(d) > 1.0e-12)
      {
        ...
        {
          std::cout << "in v_rel > 0: deltav from acceleration: " << deltav
                    << " vs v_rel: " << v_rel(d) << std::endl;
          std::cout << "WARNING: Time step for particle " << currparticle->Id()
                    << " is too large ! "
                       "Maximum time step should be smaller than:"
                    << fabs(v_rel(d) / (2 * acc)) << std::endl;
        }
      }
    ...
    for (int d = 0; d < dim_; ++d) forcecurrbubble[d] = bubbleforce(d);
    ...
    switch (coupalgo_)
    {
      ...
      case INPAR::CAVITATION::TwoWayMomentum:
      {
        ...
        DRT::UTILS::shape_function_3D(
            funct, elecoord(0), elecoord(1), elecoord(2), targetfluidele->Shape());
    ...
    if (output)
    {
      ...
      std::cout << "t: " << particles_->Time() << " gravity force       : " << gravityforce
                << std::endl;
      ...
      if (simplebubbleforce_)
      {
        ...
        std::cout << "t: " << particles_->Time() << " buoy_force          : " << buoy_force
                  << std::endl;

        static LINALG::Matrix<3, 1> inertia_force(false);
        inertia_force.Update(rho_l, Du_Dt);
        inertia_force.Scale(vol_b);
        std::cout << "t: " << particles_->Time() << " inertia_force       : " << inertia_force
                  << std::endl;
      }
      else
      {
        ...
        std::cout << "t: " << particles_->Time() << " pressgrad force     : " << pressgrad_force
                  << std::endl;

        static LINALG::Matrix<3, 1> viscous_force(false);
        viscous_force.Update(2.0 * mu_l * vol_b, visc_stress);
        std::cout << "t: " << particles_->Time() << " viscous force       : " << viscous_force
                  << std::endl;
      }
      ...
      std::cout << "t: " << particles_->Time() << " dragforce force     : " << dragforce
                << std::endl;
      std::cout << "t: " << particles_->Time() << " liftforce force     : " << liftforce
                << std::endl;
      std::cout << "t: " << particles_->Time() << " added mass force    : " << addedmassforce
                << std::endl;

      // sum over all bubble forces
      std::cout << "t: " << particles_->Time() << " particle force      : " << bubbleforce
                << std::endl;

      // fluid force
      std::cout << "t: " << particles_->Time() << " fluid force         : " << couplingforce
                << std::endl;
    }
  ...
  if (not timeforcalcfluidforces || coupalgo_ == INPAR::CAVITATION::OneWay ||
  ...
  // add fluid force contributions to the global vector
  {
    ...
    for (std::map<int, double>::const_iterator it = fluidforce_contribution.begin();
         it != fluidforce_contribution.end(); ++it)
    {
    ...
    const int err =
        fluidforces->SumIntoGlobalValues(fluidforce_contribution.size(), &gids[0], &vals[0]);
  ...
  switch (coupalgo_)
  {
    ...
    case INPAR::CAVITATION::TwoWayFull_strong:
    {
      ...
      if (numentries != fluidforces->MyLength()) dserror("dofrowmaps of equal size expected");
 ...
 *----------------------------------------------------------------------*/
DRT::Element* CAVITATION::Algorithm::GetUnderlyingFluidEleData(DRT::Node* currparticle,
    LINALG::Matrix<3, 1>& particleposition, LINALG::Matrix<3, 1>& elecoord,
    const Teuchos::ParameterList& p, Epetra_SerialDenseVector& elevector1,
    Epetra_SerialDenseVector& elevector2, Epetra_SerialDenseVector& elevector3,
    Epetra_SerialDenseVector& elevector4, Epetra_SerialDenseVector& elevector5)
{
  ...
  DRT::MESHFREE::MeshfreeMultiBin* currbin =
      static_cast<DRT::MESHFREE::MeshfreeMultiBin*>(currele[0]);

  // use cache for underlying fluid element and elecoords if possible
  if (underlyingelecache_.empty())
  {
    targetfluidele = GetEleCoordinatesFromPosition(
  ...
  {
    std::cout << "INFO: currparticle with Id: " << currparticle->Id()
              << " and position: " << particleposition(0) << " " << particleposition(1) << " "
              << particleposition(2) << " "
              << " does not have an underlying fluid element -> no forces calculated" << std::endl;

    std::vector<double> tmpposition(dim_);
    for (int d = 0; d < dim_; ++d) tmpposition[d] = particleposition(d);
    int bubbleBinId = BinStrategy()->ConvertPosToGid(&(tmpposition[0]));

    std::cout << "particle is in binId: " << bubbleBinId << " while currbin->Id() is "
              << currbin->Id() << " . The following number of fluid eles is in this bin:"
              << currbin->NumAssociatedEle(bin_fluidcontent_) << std::endl;
...
void CAVITATION::Algorithm::GetUnderlyingFluidEleDataT(const LINALG::Matrix<3, 1>& particleposition,
    const DRT::Element* targetfluidele, LINALG::Matrix<3, 1>& elecoord,
    const Teuchos::ParameterList& p, Epetra_SerialDenseVector& elevector1,
    Epetra_SerialDenseVector& elevector2, Epetra_SerialDenseVector& elevector3,
    Epetra_SerialDenseVector& elevector4, Epetra_SerialDenseVector& elevector5)
{
...
#ifdef INLINED_ELE_EVAL
  {
    ...
    std::map<int, std::vector<std::vector<double>>>::iterator it =
        evelacc_cache_.find(targetfluidele->Id());
    ...
    */
    ...
    if (!simplebubbleforce_)
    {
      ...
      std::map<int, Epetra_SerialDenseVector>::iterator it =
          evelgrad_cache_.find(targetfluidele->Id());
      ...
      else
      {
        ...
        DRT::UTILS::ExtractMyNodeBasedValues(
            targetfluidele, evelgrad_extracted, velgradcol_interpol_, nsdsquare);
        evelgrad_cache_.insert(
            std::pair<int, Epetra_SerialDenseVector>(targetfluidele->Id(), evelgrad_extracted));
      ...
      elevector5[0] = 2.0 * evelgrad2(0, 0) + evelgrad2(1, 1) + evelgrad2(3, 1) + evelgrad2(2, 2) +
                      evelgrad2(6, 2);
      elevector5[1] = evelgrad2(3, 0) + evelgrad2(1, 0) + 2.0 * evelgrad2(4, 1) + evelgrad2(5, 2) +
                      evelgrad2(7, 2);
      elevector5[2] = evelgrad2(6, 0) + evelgrad2(2, 0) + evelgrad2(7, 1) + evelgrad2(5, 1) +
                      2.0 * evelgrad2(8, 2);
...
#else
  {
    ...
    // call the element specific evaluate method (elevec1 = fluid vel u; elevec2 = mat deriv of
    // fluid vel, elevec3 = rot of fluid vel)
    targetfluidele->Evaluate(
    ...
    if (!simplebubbleforce_)
    {
      ...
      // call the element specific evaluate method (elevec4 = pressure gradient; elevec5 = viscous
      // stress term)
      targetfluidele->Evaluate(
          params, *fluiddis_, *lm_f, elematrix1, elematrix2, elevector4, elevector5, elevector3);
    }
  }
#endif
...
void CAVITATION::Algorithm::ComputeRadius()
{
  ...
  for (int i = 0; i < BinStrategy()->BinDiscret()->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    // compute pressure at bubble position and skip computation in case no underlying fluid element
    // was found
    const bool fluidelefound = ComputePressureAtBubblePosition(
        currparticle, particleposition, elemat1, elemat2, elevec1, elevec2, elevec3);
    if (fluidelefound == false)
    ...
    while (particle_timenp - subtime > 2.0 * dtsub)
    {
      ...
      IntegrateRadius(gid, subtime, pvapor, pg0i, dtsub, rho_l, mu_l, gamma, fluid_timenp,
          fluid_timen, pfluidnp, pfluidn, r_bub_0, r_bub_k, r_bub_kp, r_dot_bub_k, allowadaption,
          dt_min, dt_max);
      ...
      if ((r_bub_kp - r_bub_k) < -0.07 * r_bub_kp or (r_bub_kp - r_bub_k) > 0.05 * r_bub_kp)
      {
        ...
        std::cout << "bubble-id: " << currparticle->Id()
                  << " at position: x: " << particleposition(0) << " y: " << particleposition(1)
                  << " z: " << particleposition(2) << " , at time: " << std::setprecision(12)
                  << subtime << " current radius: " << r_bub_kp
                  << " has a relative change of radius of: " << (r_bub_kp - r_bub_k) / r_bub_kp
                  << std::endl
                  << std::endl;
      }
    ...
    // the last time steps should be divided in two equal parts in order to reach the final point
    // exactly
 ...
 *----------------------------------------------------------------------*/
void CAVITATION::Algorithm::IntegrateRadius(const int bubbleid, double& subtime,
    const double pvapor, const double pg0i, double& dtsub, const double rho_l, const double mu_l,
    const double gamma, const double fluid_timenp, const double fluid_timen, const double pfluidnp,
    const double pfluidn, const double r_bub_0, const double r_bub_k, double& r_bub_kp,
    double& r_dot_bub_k, const bool allowadaption, const double dt_min, const double dt_max)
{
  ...
  while (localtruncerr > epsilon)
  {
    ...
    const double RDot_3 =
        r_dot_bub_k + dtsub * (2.0 / 9.0 * kRDot_1 + 1.0 / 3.0 * kRDot_2 + 4.0 / 9.0 * kRDot_3);

    r_dot_bub_kp =
        r_dot_bub_k + dtsub * (2.0 / 9.0 * kRDot_1 + 1.0 / 3.0 * kRDot_2 + 4.0 / 9.0 * kRDot_3);
    r_bub_kp =
        r_bub_k + dtsub * (2.0 / 9.0 * r_dot_bub_k + 1.0 / 3.0 * RDot_1 + 4.0 / 9.0 * RDot_2);

    // computation of normalized local truncation error abs(R[n+1]-R_emb[n+1])/R[n+1]
    localtruncerr = abs(dtsub *
                        (-5.0 / 72.0 * r_dot_bub_k + 1.0 / 12.0 * RDot_1 + 1.0 / 9.0 * RDot_2 -
                            1.0 / 8.0 * RDot_3) /
                        r_bub_kp);
    ...
    if (allowadaption == true)
    {
      ...
      if (localtruncerr > 1.0e-12)
      {
        // standard case
        scale = std::min(
            std::max(SafetyFactor * pow(epsilon / localtruncerr, 1.0 / 3.0), MinScale), MaxScale);
      }
#ifdef DEBUG
      else
      {
    ...
    if (dtsub <= dt_min and localtruncerr > epsilon)
      dserror(
          "with given dtMin requested local truncation error cannot be achieved for bubble id: %d",
          bubbleid);
  ...
  if (r_bub_kp < 0.0) dserror("radius of bubble has negative value");
 ...
 *----------------------------------------------------------------------*/
double CAVITATION::Algorithm::f_R_dot(const double r_bub, const double r_dot_bub,
    const double rho_l, const double sigma, const double mu_l, const double pbk,
    const double pambientk)
{
  ...
  return (inv_r_bub / rho_l) * (-3.0 / 2.0 * rho_l * r_dot_bub * r_dot_bub + pbk - pambientk -
                                   2.0 * sigma * inv_r_bub - 4.0 * mu_l * r_dot_bub * inv_r_bub);
}
...
void CAVITATION::Algorithm::ParticleInflow()
{
  ...
  if ((particles_->Step() - restartparticles_) % timestepsizeratio_ != 0) return;

  {
    ...
    for (std::set<int>::const_iterator i = latestinflowbubbles_.begin();
         i != latestinflowbubbles_.end(); ++i)
    {
      int bubblegid = *i;
      ...
      if (bubblelid != -1)
      {
        ...
        {
          const double blendingfac =
              (double)(((Step() + 1) % blendingsteps_) / (double)(Step() % blendingsteps_));
  ...
  if (globaltimeforinflow == 0) return;
  ...
  for (biniter = bubble_source_.begin(); biniter != bubble_source_.end(); ++biniter)
  {
    ...
    for (particleiter = biniter->second.begin(); particleiter != biniter->second.end();
         ++particleiter)
    {
      ...
      if (Time() < timedelay || Time() > stopinflowtime) continue;
      ...
      Teuchos::RCP<DRT::Node> newparticle =
          Teuchos::rcp(new PARTICLE::ParticleNode(newbubbleid, &inflow_position[0], MyRank()));
      ...
      // in the rare case when the random amplitude leads to particles that are located in col bins,
      // special treatment is necessary
      if (homelessparticles.size() != 0)
      {
        BinStrategy()->BinDiscret()->AddNode(newparticle);
        // assign node to an arbitrary row bin -> correct placement will follow in the timeloop in
        // TransferParticles
        DRT::MESHFREE::MeshfreeMultiBin* firstbinindis =
            dynamic_cast<DRT::MESHFREE::MeshfreeMultiBin*>(
                BinStrategy()->BinDiscret()->lRowElement(0));
  ...
  if (inflowradiusblending_)
  {
    ...
    // allreduce the ids of inflow bubbles in order to do blending of the radius on all procs later
    // on
    ...
    if (initialblendingsteps != 0 && initialblendingsteps != blendingsteps_)
      dserror(
          "inflow frequency has changed which is not yet supported --> see latestinflowbubbles_");
  }
  else
  {
    latestinflowbubbles_.clear();
  ...
  if (inflowcounter)
  {
    std::cout << "Inflow of " << inflowcounter << " bubbles on proc " << MyRank() << " at time "
              << particles_->Time() << " using " << ss.str() << " blending steps" << std::endl;
  }
  ...
  for (biniter = bubble_source_.begin(); biniter != bubble_source_.end(); ++biniter)
  {
         ...
         ++particleiter)
    {
      ...
      for (int d = 0; d < dim_; ++d) (*disn)[lid + d] = currparticle->X()[d];
      ...
      if (initbubblevelfromfluid_)
      {
        // bubble position is needed to get current velocity for that bubble
        static LINALG::Matrix<3, 1> particleposition;
        for (int d = 0; d < dim_; ++d) particleposition(d) = currparticle->X()[d];
        ...
        if (fluidelefound == false)
          dserror(
              "no underlying fluid element for velocity computation was found for bubble %d "
              "at positions %f %f %f on proc %d. Check whether init vel from fluid is desired?",
              currparticle->Id(), particleposition(0), particleposition(1), particleposition(2),
              MyRank());

        for (int d = 0; d < dim_; ++d) (*veln)[lid + d] = elevec1(d);
      }
      else
      {
        ...
        for (int d = 0; d < dim_; ++d) (*veln)[lid + d] = inflow_vel[d] * curvefac;
      }
      ...
      // const double mass = density * 4.0/3.0 * M_PI * inflow_radius * inflow_radius *
      // inflow_radius;
      (*massn)[lid] =
          initDensity * 4.0 / 3.0 * M_PI * inflow_radius * inflow_radius * inflow_radius;
      ...
      if (computeradiusRPbased_)
      {
        ...
        // state vector of fluid has already been set earlier ("vel")
        const bool fluidelefound = ComputePressureAtBubblePosition(
            currparticle, pos, elemat1, elemat2, elevec1, elevec2, elevec3);
        if (fluidelefound == false)
          dserror(
              "no underlying fluid element for pressure computation was found for bubble %d at "
              "positions %f %f %f on proc %d",
...
{
  if (MyRank() == 0)
    IO::cout << "INFO: Restart for particles from step " << restartparticles_
             << " and for fluid from step " << restart
  ...
  if (particles_->Radiusn()->GlobalLength() != 0)
  {
    ...
    latestinflowbubbles_.clear();
    for (unsigned i = 0; i < in->size(); i++) latestinflowbubbles_.insert((*in)[i]);
...
void CAVITATION::Algorithm::SetupGhosting(Teuchos::RCP<Epetra_Map> binrowmap,
    std::map<int, std::set<int>>& rowfluideles, Teuchos::RCP<Epetra_Map> fluidelecolmapold)
{
  //--------------------------------------------------------------------
  // 1st and 2nd step
  //--------------------------------------------------------------------
  ...
  Teuchos::RCP<Epetra_Map> fluidelecolmap = BinStrategy()->ExtendGhosting(
      &(*fluidelecolmapold), rowfluideles, extendedfluidghosting, Teuchos::null, BinColMap());
  ...
  for (int k = 0; k < BinStrategy()->BinDiscret()->NumMyColNodes(); k++)
  {
    ...
    for (int dim = 0; dim < 3; dim++) projpoint(dim) = pos[dim];
    bool foundele = false;
    for (int i = 0; i < fluiddis_->NumMyColElements(); i++)
    {
      ...
      const LINALG::SerialDenseMatrix xyze(
          GEO::getCurrentNodalPositions(Teuchos::rcp(fluidele, false), currentpositions));
  ...
  {
    {
      ...
      for (std::set<int>::iterator it = latestinflowbubbles_.begin();
           it != latestinflowbubbles_.end(); it++)
        out->push_back(*it);
  ...
  if (MyRank() == 0)
  {
    ...
    if ((particles_->Step() - restartparticles_) % writeevry == 0)
    {
      ...
      if (substep != 0)
        ...
        IO::cout << "particle substep no. " << timestepsizeratio_ << " / " << timestepsizeratio_
                 << IO::endl;
    }
  }
  return;
...
void CAVITATION::Algorithm::BuildBubbleInflowCondition()
{
  ...
  for (size_t i = 0; i < conds.size(); ++i)
  {
    if (i > 0)
      dserror(
          "only taken care of one particle inflow condition so far. "
    ...
    if (std::abs(inflowtime / Dt() - (int)(inflowtime / Dt())) > EPS9)
      dserror("1/inflow_freq with inflow_freq = %f cannot be divided by fluid time step %f",
          inflowtime, Dt());

    // check for initial overlap of particles
    const double inflow_vel_mag =
        sqrt((*inflow_vel)[0] * (*inflow_vel)[0] + (*inflow_vel)[1] * (*inflow_vel)[1] +
             (*inflow_vel)[2] * (*inflow_vel)[2]);
    if (initbubblevelfromfluid_ == false && initial_radius / inflow_vel_mag > inflowtime)
    {
      if (detectoverlap)
        dserror(
            "Overlap for inflowing bubbles expected: initial_radius/inflow_vel_mag = %f s > "
            "inflow_freq = %f s",
            initial_radius / inflow_vel_mag, inflowtime);
      else
        std::cout
    ...
    {
      const double dist_z = ((*vertex2)[2] - (*vertex1)[2]) /
                            ((((*num_per_dir)[2] - 1) != 0) ? ((*num_per_dir)[2] - 1) : 1);
      source_pos[2] = (*vertex1)[2] + z * dist_z;
      for (int y = 0; y < (*num_per_dir)[1]; ++y)
      {
        const double dist_y = ((*vertex2)[1] - (*vertex1)[1]) /
                              ((((*num_per_dir)[1] - 1) != 0) ? ((*num_per_dir)[1] - 1) : 1);
        source_pos[1] = (*vertex1)[1] + y * dist_y;
        for (int x = 0; x < (*num_per_dir)[0]; ++x)
        {
          const double dist_x = ((*vertex2)[0] - (*vertex1)[0]) /
                                ((((*num_per_dir)[0] - 1) != 0) ? ((*num_per_dir)[0] - 1) : 1);
          ...
          if (found != -1)
          {
            Teuchos::RCP<BubbleSource> bubbleinflow =
                Teuchos::rcp(new BubbleSource(bubbleinflowid, source_pos, *inflow_vel,
                    inflow_vel_funct, initial_radius, inflow_freq, timedelay, stopinflowtime));
    ...
    if (check == 0)
      dserror(
          "Weird! Inflow condition found but could not be assigned to bins. Source outside of "
          "bins?");
  }
  ...
  // because time derivative of fluid fraction should not be considered close to the inflow
  {
    ...
    for (std::map<int, std::list<Teuchos::RCP<BubbleSource>>>::const_iterator biniter =
             bubble_source_.begin();
         biniter != bubble_source_.end(); ++biniter)
    {
      ...
      int ijk_range[] = {ijk[0] - ibinrange, ijk[0] + ibinrange, ijk[1] - ibinrange,
          ijk[1] + ibinrange, ijk[2] - ibinrange, ijk[2] + ibinrange};
      ...
      for (std::vector<int>::const_iterator i = binIds.begin(); i != binIds.end(); ++i)
      {
        ...
        for (int ifluidele = 0; ifluidele < currbin->NumAssociatedEle(bin_fluidcontent_);
             ++ifluidele)
        {
    ...
    // loop all elements and find adjacent fluid row nodes, more exactely the last (=pressure) dof
    // of this node current implementation is not prepared for repartitioning of fluid domain
    // (remedy: GatherAll fluid nodes close to inflow and check for gid when applying zeros to the
    // vector)
    for (std::set<DRT::Element*>::const_iterator iter = neighboringfluideles.begin();
         iter != neighboringfluideles.end(); ++iter)
    {
 ...
 *----------------------------------------------------------------------*/
CAVITATION::BubbleSource::BubbleSource(const int bubbleinflowid,
    std::vector<double> inflow_position, std::vector<double> inflow_vel, const int inflow_vel_funct,
    const double inflow_radius, const double inflow_freq, const double timedelay,
