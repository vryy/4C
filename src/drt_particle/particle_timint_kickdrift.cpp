...
\brief SPH 2nd order (explicit) time integration scheme denoted as kick-drift-kick scheme as
presented in Adami 2013, Eqs (14)-(17)
...
/*----------------------------------------------------------------------*/
/* Constructor */
PARTICLE::TimIntKickDrift::TimIntKickDrift(const Teuchos::ParameterList& ioparams,
...
{
  // Kick-drift-kick integration scheme with modified particle convection \tilde{v} due to modified
  // acceleration contribution \tilde{a}, see e.g. Adami2013, Eqs. (14)-(17)
  /*
  ...
  */
  ...
  // Attention: At the end of IntegrateStep(), positions disn_=r_{n+1} and velocities veln_=v_{n+1}
  // at the end time t_{n+1} are stored, while accelerations accn_=a_{n+1/2} are stored at the mid
  // time t_{n+1/2}!!!
  ...
  // and \tilde{a}_{n+1/2}=\tilde{a}(r_{n+1},v_{n+1/2}) based on r_{n+1} and v_{n+1/2} as well as
  // external forces at t_{n+1}
...
void PARTICLE::TimIntKickDrift::GetExtremeValues()
{
  ...
  if (min_pvp_dist < alltimemin_pvp_dist_) alltimemin_pvp_dist_ = min_pvp_dist;
  if (min_pvw_dist < alltimemin_pvw_dist_) alltimemin_pvw_dist_ = min_pvw_dist;
  if (min_pressure < alltimemin_pressure_) alltimemin_pressure_ = min_pressure;
  if (max_pressure > alltimemax_pressure_) alltimemax_pressure_ = max_pressure;
