...
/*----------------------------------------------------------------------*/
/* Constructor */
PARTICLE::TimIntExpl::TimIntExpl(const Teuchos::ParameterList& ioparams,
...
void PARTICLE::TimIntExpl::Init()
{
  ...
  switch (particle_algorithm_->ParticleInteractionType())
  {
    ...
    case INPAR::PARTICLEOLD::NormalAndTang_DEM:
    {
      if (DRT::Problem::Instance()->Materials()->FirstIdByType(
        ...
        collhandler_ = Teuchos::rcp(new PARTICLE::ParticleCollisionHandlerDEM(
  ...
  if (collhandler_ != Teuchos::null)
  {
    ...
    if (numparticle > 0)
    {
      ...
      if (maxradius > collhandler_->GetMaxRadius()) dserror("Input parameter MAX_RADIUS invalid");

      double minradius = -1.;
      (*radius_)(0)->MinValue(&minradius);
      if (minradius < collhandler_->GetMinRadius()) dserror("Input parameter MIN_RADIUS invalid");
    }
  }
