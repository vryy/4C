 ...
 *----------------------------------------------------------------------*/
PARTICLE::TimIntStrategyBase::TimIntStrategyBase(
...
/*----------------------------------------------------------------------*
 | constructor                                               fang 10/17 |
 *----------------------------------------------------------------------*/
PARTICLE::TimIntStrategySpheres::TimIntStrategySpheres(
    TimInt* const   timint   //!< time integrator
    ) :
    ...
    TimIntStrategyBase(timint)
{
  ...
  timint->radius_ = Teuchos::rcp(new TIMINT::TimIntMStep<Epetra_Vector>(0,0,timint->NodeRowMapView(),true));
...
double PARTICLE::TimIntStrategySpheres::ComputeKineticEnergy() const
{
  ...
  for(int i=0; i<timint_->discret_->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    kineticenergy += .5*((*timint_->mass_)[i]*kineticenergy_trans+(*timint_->inertia_)[i]*kineticenergy_rot);
  }
...
{
  // output warning
  dserror("Prediction and correction of angular velocity not yet tested for spherical particles! "
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::TimIntStrategySpheres::RotateOrientVector(
    const double   dt   //!< time step size
    ) const
{
  for(int i=0; i<timint_->discret_->NodeRowMap()->NumMyElements(); ++i)
...
void PARTICLE::TimIntStrategySpheres::SetInitialRadii() const
{
  ...
  for(int i=0; i<timint_->discret_->NumMyRowNodes(); ++i)
  {
    const ParticleRadiusNode* const particle = dynamic_cast<const ParticleRadiusNode* const>(timint_->discret_->lRowNode(i));
...
{
  // safety check
  if(timint_->mass_ == Teuchos::null)
    dserror("Invalid mass vector!");
...
void PARTICLE::TimIntStrategyEllipsoids::OutputOrientation() const
{
  ...
  const Teuchos::RCP<Epetra_MultiVector> orient_tensor = Teuchos::rcp(new Epetra_MultiVector(*timint_->discret_->NodeRowMap(),9));
  ...
  for(int inode=0; inode<timint_->discret_->NodeRowMap()->NumMyElements(); ++inode)
  {
    ...
    for (int i=0; i<3; ++i)
      for (int j=0; j<3; ++j)
        orient_tensor->ReplaceMyValue(inode,j*3+i,E_rot(i,j));
  }
    ...
    ) const
{
  ...
  for(int i=0; i<timint_->discret_->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    // multiply transformed angular velocity at time t_n by inertia vector to obtain angular momentum at time t_n
    ...
    // divide transformed angular momentum at new time by inertia vector to obtain angular velocity at new time
    ...
    for(unsigned dim=0; dim<3; ++dim)
      (*timint_->angVeln_)[i*3+dim] = temp(dim);
  }
...
/*----------------------------------------------------------------------*
 | update particle orientation vector                        fang 10/17 |
 *----------------------------------------------------------------------*/
void PARTICLE::TimIntStrategyEllipsoids::RotateOrientVector(
    const double   dt   //!< time step size
    ) const
{
  // initialize vectors
  ...
  for(int i=0; i<timint_->discret_->NodeRowMap()->NumMyElements(); ++i)
  {
    ...
    if(angVel > 1.e-16)
    {
      ...
      for(int dim=0; dim<3; ++dim)
        (*timint_->orient_)[i*3+dim] = orient(dim);
    }
  }
