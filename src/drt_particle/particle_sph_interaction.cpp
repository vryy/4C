...
PARTICLE::ParticleSPHInteractionHandler::ParticleSPHInteractionHandler(
    ...
    const Teuchos::ParameterList& particledynparams, bool norender)
    : discret_(discret),
      ...
      max_pressure_(-1.0e10)
{
  ...
  if (particlemat.size() == 0) dserror("No particle material defined!");
  ...
  for (unsigned int i = 0; i < particlemat.size(); ++i)
  {
    ...
    const MAT::PAR::ExtParticleMat* extparticlemat =
        dynamic_cast<const MAT::PAR::ExtParticleMat*>(particlemat[i]);
      ...
      norender == false)
  {
    Teuchos::RCP<Rendering> rendering = Teuchos::rcp(new PARTICLE::Rendering(
        particle_algorithm_, Teuchos::rcp(this, false), weightFunctionHandler_));
  ...
  if (freeSurfaceType_ == INPAR::PARTICLE::TwoPhase)
  {
    ...
    if (fabs(extParticleMat_[0]->artificialViscosity_ - extParticleMat_[1]->artificialViscosity_) >
        1.0e-12)
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Init(Teuchos::RCP<const Epetra_Vector> disn,
    Teuchos::RCP<const Epetra_Vector> veln, Teuchos::RCP<const Epetra_Vector> radiusn,
    Teuchos::RCP<const Epetra_Vector> mass, Teuchos::RCP<const Epetra_Vector> density0)
{
  ...
  if (!neighbours_p_.empty())
  {
    ...
    std::cout << "However, lid row/col ids were not updated (because not yet updated). It is safe "
                 "only when not parallel\n";
    std::cin.get();
  }
  InitColParticles();

  // set up positions and radii to set up the neighbours
  if (disn != Teuchos::null) SetStateVector(disn, PARTICLE::Dis);

  if (radiusn != Teuchos::null) SetStateVector(radiusn, PARTICLE::Radius);

  // keep going with the remaining state vectors
  if (veln != Teuchos::null) SetStateVector(veln, PARTICLE::Vel);
  if (mass != Teuchos::null) SetStateVector(mass, PARTICLE::Mass);
  if (density0 != Teuchos::null) SetStateVector(density0, PARTICLE::Density0);
...
void PARTICLE::ParticleSPHInteractionHandler::InitColParticles()
{
  ...
  for (int lidNodeCol = 0; lidNodeCol < numcolelements; ++lidNodeCol)
  {
    ...
    if (min_voidparticle_id > 0 and particle->Id() >= (min_voidparticle_id - 1))
    {
      ...
      if (boundaryparticle) dserror("Boundary particles should have IDs < min_voidparticle_id!");
    }
    else if (boundaryparticle)
      phase_color = -1;

    colParticles_[lidNodeCol] = ParticleSPH(particle->Id(), particle->Owner(), lm, boundaryparticle,
        phase_color, extParticleMat_[material_id]->initDensity_, extParticleMat_[material_id]);
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::InitDensityAndMass(const double particle_volume,
    const Teuchos::RCP<Epetra_Vector> density, const Teuchos::RCP<Epetra_Vector> mass,
    const Teuchos::RCP<Epetra_Vector> disn, const Teuchos::RCP<Epetra_Vector> radiusn)
{
  if (density == Teuchos::null) dserror("Empty vector density!");

  if (mass == Teuchos::null) dserror("Empty vector mass!");
...
void PARTICLE::ParticleSPHInteractionHandler::InitBoundaryData(
    Teuchos::RCP<const Epetra_Vector> accn, const LINALG::Matrix<3, 1>& g)
{
  SetStateVector(accn, PARTICLE::Acc);

  // loop over the boundary particles
  for (std::map<int, std::map<int, InterDataPvP>>::iterator ii = neighbours_bp_.begin();
       ii != neighbours_bp_.end(); ++ii)
  {
    int id_i = ii->first;
    ...
    // loop over the boundary particles
    for (std::map<int, InterDataPvP>::iterator jj = neighbours_bp_[id_i].begin();
         jj != neighbours_bp_[id_i].end(); ++jj)
    {
      int id_j = jj->first;
      ...
      sumjrhojrijWij.Update(interData_ij.rRelNorm2_ * particle_j.density_ * interData_ij.w_ij_,
    ...
    if (sumjWij > 0)
    {
      ...
      // In case of a no-slip boundary condition, we apply a quasi-linear extrapolation according to
      // Ref. Adami2012, Eq.(23)
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::SetStateVector(
    Teuchos::RCP<const Epetra_Vector> stateVector, const PARTICLE::StateVectorType svt)
{
  TEUCHOS_FUNC_TIME_MONITOR("PARTICLE::ParticleSPHInteractionHandler::SetStateVector");

  // safety check
  if (stateVector == Teuchos::null) dserror("The given state vector is empty!");
  ...
  for (int lidNodeCol = 0; lidNodeCol < discret_->NodeColMap()->NumMyElements(); ++lidNodeCol)
  {
    ...
    switch (svt)
    {
      ...
      case PARTICLE::Acc:
      {
        DRT::UTILS::ExtractMyValues<LINALG::Matrix<3, 1>>(
            *stateVectorCol, data.boundarydata_.acc_, data.lm_);
        break;
      }
      case PARTICLE::CFG:
      {
        DRT::UTILS::ExtractMyValues<LINALG::Matrix<3, 1>>(
            *stateVectorCol, data.freesurfacedata_.CFG_, data.lm_);
        break;
      }
      case PARTICLE::CFG_FF:
      {
        DRT::UTILS::ExtractMyValues<LINALG::Matrix<3, 1>>(
            *stateVectorCol, data.freesurfacedata_.CFG_FF_, data.lm_);
        break;
      }
      case PARTICLE::CFG_F1S:
      {
        DRT::UTILS::ExtractMyValues<LINALG::Matrix<3, 1>>(
            *stateVectorCol, data.freesurfacedata_.CFG_F1S_, data.lm_);
        break;
      }
      case PARTICLE::CFG_F2S:
      {
        DRT::UTILS::ExtractMyValues<LINALG::Matrix<3, 1>>(
            *stateVectorCol, data.freesurfacedata_.CFG_F2S_, data.lm_);
        break;
      }
      // node based vectors
...
void PARTICLE::ParticleSPHInteractionHandler::AddNewNeighbours()
{
  ...
  // loop over the boundary particles
  for (std::map<int, ParticleSPH*>::iterator ii = boundaryparticles_.begin();
       ii != boundaryparticles_.end(); ++ii)
  {
    int id_i = ii->first;
...
void PARTICLE::ParticleSPHInteractionHandler::AddNewNeighbours_p(
    ParticleSPH& particle_i, const std::list<DRT::Node*>& neighboursLinf_p)
{
  ...
  for (std::list<DRT::Node*>::const_iterator jj = neighboursLinf_p.begin();
       jj != neighboursLinf_p.end(); ++jj)
  {
    ...
    if (particle_j.gid_ != particle_i.gid_)  // standard case: interaction ij with i != j
    {
      ...
      // TODO: In future applications (e.g. implicit time integration), where neighbours are not
      // searched for every new configuration this criterion might be to strict!!!
      if (rRelNorm2 - std::max(particle_i.radius_, particle_j.radius_) > 0) continue;
      ...
      {
        dserror(
            "Particle distance to small: rRelNorm2=%f, initAverageDist=%f. Danger of tensile "
            "instability!",
            rRelNorm2, initAverageDist);
      }

#endif

      // check for minimal distance
      if (rRelNorm2 < min_pvp_dist_) min_pvp_dist_ = rRelNorm2;
      ...
      (neighbours_p_[lidNodeRow_i])[lidNodeCol_j] =
          InterDataPvP(rRelVersor_ij, rRelNorm2, w_ij, dw_ij, ddw_ij);

      if (particle_i.boundarydata_.boundaryparticle_ == false and
          particle_j.boundarydata_.boundaryparticle_ == false and
          particle_i.freesurfacedata_.phase_color_ != particle_j.freesurfacedata_.phase_color_)
...
void PARTICLE::ParticleSPHInteractionHandler::AddNewNeighbours_bp(
    const ParticleSPH& particle_i, const std::list<DRT::Node*>& neighboursLinf_p)
{
       ...
       jj != neighboursLinf_p.end(); ++jj)
  {
    ...
    if (particle_j.boundarydata_.boundaryparticle_ == false)
    {
      ...
      // check for minimal distance
      if (rRelNorm2 < min_pvw_dist_) min_pvw_dist_ = rRelNorm2;
      ...
      LINALG::Matrix<3, 1> rRelVersor_ij(rRel_ij);
      rRelVersor_ij.Scale(1 / rRelNorm2);
      (neighbours_bp_[particle_i.gid_])[particle_j.gid_] =
          InterDataPvP(rRelVersor_ij, rRelNorm2, w_ij, dw_ij, ddw_ij);
    }
  }
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::AddNewNeighbours_fsp(const ParticleSPH& particle_i,
    const std::list<DRT::Node*>& neighboursLinf_p, const bool mark_fs_indirect)
{
       ...
       jj != neighboursLinf_p.end(); ++jj)
  {
    ...
    if (particle_i.gid_ == particle_j.gid_) continue;
    ...
    // The quantity initAverageDist represents the average initial distance between  particles
    // considering there mass and initial density.
    double initAverageDist =
        PARTICLE::Utils::Volume2EffDist(particle_i.mass_ / particle_i.density0_, WF_DIM_);
    if (rRelNorm2 < PARTICLE_TENSILESAFETYFAC * initAverageDist)
      dserror(
    ...
    LINALG::Matrix<3, 1> rRelVersor_ij(rRel_ij);
    rRelVersor_ij.Scale(1 / rRelNorm2);
    (neighbours_fsp_[particle_i.gid_])[particle_j.gid_] =
        InterDataPvP(rRelVersor_ij, rRelNorm2, w_ij, dw_ij, ddw_ij);

    // mark particle_j as indirect free surface particle in case in has not yet been marked as
    // direct free-surface particle
    if (mark_fs_indirect and particle_j.freesurfacedata_.freesurfaceparticletype_ != FS_DIRECT and
        particle_j.boundarydata_.boundaryparticle_ == false)
...
void PARTICLE::ParticleSPHInteractionHandler::ShiftPeriodicBoundaryPair(
    const LINALG::Matrix<3, 1>& dis_1, LINALG::Matrix<3, 1>& dis_2, const double& radius_1,
    const double& radius_2)
{
  // in case both positions 1 and 2 are close to two opposite periodic boundaries, the position
  // dis_2 is shifted correspondingly it is sufficient to only shift dis_2 if the terms evaluated do
  // only depend on the relative distance between two positions
  ...
  for (unsigned int dim = 0; dim < 3; ++dim)
  {
    ...
    if (particle_algorithm_->BinStrategy()->HavePBC(dim))
    {
      ...
      if (pbc_length < 2.0 * max_radius)
        dserror(
            "Particles are not allowed to interact directly AND across the periodic boundary. The "
            "minimal periodic length has to be larger than two time the maximum particle radius!");

      if (dis_1(dim) >= (xaabb(dim, 1) - max_radius) and dis_2(dim) <= (xaabb(dim, 0) + max_radius))
        dis_2(dim) += pbc_length;
      else if (dis_2(dim) >= (xaabb(dim, 1) - max_radius) and
               dis_1(dim) <= (xaabb(dim, 0) + max_radius))
        dis_2(dim) -= pbc_length;
    }
  }
    ...
    const Teuchos::RCP<Epetra_Vector> densityDotn)
{
  ...
  // checks
  if (densityDotn == Teuchos::null) dserror("densityDotn is empty");
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    ...
    if (particle_i.boundarydata_.boundaryparticle_ == true) continue;
    ...
    for (boost::unordered_map<int, InterDataPvP>::const_iterator jj =
             neighbours_p_[lidNodeRow_i].begin();
         jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
    {
      ...
      {
        // For 2-Phase flow we take a slightly modified version according to Adami et al. 2012, Eq
        // (6) As mentioned in this reference, for boundary particles the initial volume is taken.
        double volume_j = 0.0;
        if (particle_j.boundarydata_.boundaryparticle_ == true)
    ...
    // assemble all contributions to particle i (we do this at once outside the j-loop since the
    // assemble operation is expensive!)
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_pvp_acc(const Teuchos::RCP<Epetra_Vector> accn,
    const Teuchos::RCP<Epetra_Vector> trvl_acc, const Teuchos::RCP<Epetra_Vector> acc_A,
    const double time)
{
  TEUCHOS_FUNC_TIME_MONITOR("PARTICLE::ParticleSPHInteractionHandler::Inter_pvp_acc");

  // checks
  if (accn == Teuchos::null) dserror("accn is empty");

  // Modified particle transport velocities either due to constant background pressure according to
  // Adami2013 or due to XSPH according to Monaghan1989 The terms associated with the constant
  // background pressure are similar to the real pressure terms but with the pressures p_i and p_j
  // replaced by the background pressure. All the related variables and terms are marked by a prefix
  // p0, e.g. p0_momentum_ij etc.. All terms related to XSPH are marked by a prefix xsph
  if (trvl_acc != Teuchos::null)
  {
    if (background_pressure_ < 0.0 and xsph_dampfac_ < 0.0 and xsph_stiffac_ < 0.0)
      dserror(
          "Negative values of background pressure, xsph_dampfac and xsph_stifffac! Why is the "
          "vector trvl_acc handed in for this case? Set flag TRANSPORT_VELOCITY to no!");

    if (transport_velocity_ == false)
      dserror("Vector trvl_acc handed in even though TRANSPORT_VELOCITY is set to no?!?");

    if ((xsph_dampfac_ < 0.0 and xsph_stiffac_ > 0.0) or
        (xsph_dampfac_ > 0.0 and xsph_stiffac_ < 0.0))
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    ...
    // init the sum of accelerations acting on particle i (due to particles j and due to artificial
    // viscous damping)
    ...
    // ********** damping (optional) **********
    // If required, add artificial viscous damping force in order to determine static equilibrium
    // configurations
    if (damping_factor_ > 0) sumj_accn_ij.Update(-damping_factor_, particle_i.vel_);
    ...
    for (jj = neighbours_p_[lidNodeRow_i].begin(); jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
    {
      ...
      if (background_pressure_ > 0)
        Inter_backgroundPressure(
            &sumj_trvl_accn_ij, generalCoeff_ij, particle_i, particle_j, interData_ij);
      ...
      if (transport_velocity_)
        Inter_transportVelocity_divA(
            &sumj_accn_ij, &sumj_accn_ij_A, generalCoeff_ij, particle_i, particle_j, interData_ij);
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_pvp_Laplace_x(const ParticleSPH& particle_j,
    const InterDataPvP& interData_ij, const LINALG::Matrix<3, 1>& x_ij,
    LINALG::Matrix<3, 1>& Laplace_x_ij)
{
  // This method determines the contribution of the particle pair i and j to the Laplace operator
  // applied to an arbitrary vector x according to Espanol2003 Eq. (28, first line). This formula is
  // valid independent from the spatial dimension (1D, 2D or 3D).
 ...
 *-----------------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_pvp_GradDiv_x(const ParticleSPH& particle_j,
    const InterDataPvP& interData_ij, const LINALG::Matrix<3, 1>& x_ij,
    LINALG::Matrix<3, 1>& GradDiv_x_ij)
{
  // This method determines the contribution of the particle pair i and j to the Laplace operator
  // applied to an arbitrary vector x according to Espanol2003 Eq. (28, second line). In this
  // reference, the formula is only stated for the 3D case. By conducting the derivation in Appendix
  // A for the 2D as well as for the 1D case, it can be shown that in these cases the original
  // coefficient 5 (3D) has to be set to 4 (2D) or 3 (1D), respectively.
 ...
 *--------------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::MF_mW(const Teuchos::RCP<Epetra_Vector> mW,
    const Teuchos::RCP<Epetra_Vector> unity_vec, const Teuchos::RCP<Epetra_Vector> unity_vec_grad,
    const Teuchos::RCP<Epetra_Vector> phaseColor)
{
  TEUCHOS_FUNC_TIME_MONITOR("PARTICLE::ParticleSPHInteractionHandler::MF_mW");

  // checks
  if (mW == Teuchos::null) dserror("mW is empty");

  // erase the vectors
  mW->PutScalar(0.0);
  if (unity_vec != Teuchos::null) unity_vec->PutScalar(0.0);
  if (unity_vec_grad != Teuchos::null) unity_vec_grad->PutScalar(0.0);
  if (phaseColor != Teuchos::null) phaseColor->PutScalar(0.0);
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
         ...
         jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
    {
      ...
      if (boundaryParticle_j)
      {
        ...
        // data of the interacting particle_i (for multi-phase flows, the material data might
        // strongly differ from particle to particle).
        double pressure_j = particle_j.boundarydata_.pressureMod_;
        // determine phase particle_i belongs to and material (boundary particles have phase color
        // -1 and get the same material as phase 0)
        const int matID_i = std::max(particle_i.freesurfacedata_.phase_color_, 0);

        density_j = equationOfStateHandler_[matID_i]->PressureToDensity(
            pressure_j, particle_i.density0_);  // based on material data of fluid particle_i
#endif
      }
      ...
      // For density summation, we apply the variant miW_ij proposed by Hu et al. 2006 Eq. (15)
      // (with particle_i.mass_ instead of particle_j.mass_)
      double miW_ij = interData_ij.w_ij_ * particle_i.mass_;
      double mW_ij = interData_ij.w_ij_ * particle_j.mass_;
      sumj_mW_ij += miW_ij;
      if (unity_vec != Teuchos::null) sumj_m_rho_W_ij += mW_ij / density_j;
      if (unity_vec_grad != Teuchos::null)
        sumj_m_rho_dW_ij.Update(
            particle_j.mass_ / density_j * interData_ij.dw_ij_, interData_ij.rRelVersor_ij_, 1.0);
    }  // loop over j
  ...
  // loop over all boundary particles (not only the one with fluid neighbours!) and set density and
  // color field
  for (std::map<int, ParticleSPH*>::iterator ii = boundaryparticles_.begin();
       ii != boundaryparticles_.end(); ++ii)
  {
    ...
    // Since the density of the boundary particles particle_i should remain unchanged, we have to
    // assemble the initial density value again after all DoFs of the vector mW have been cleared
    // above. This procedure is not required for boundary particle density calculation (or position
    // update) via time integration since vanishing entries for densityDot in the boundary particle
    // DoFs automatically yield an unchanged density.
...
{
  // Enforce static contact angle by prescribing normal vector of particles close to the triple
  // point as proposed in Breinlinger et al. 2013.
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
        ...
        particle_i.freesurfacedata_.havephasechange_ == true)
    {
      ...
      // The contact angle force variant 2 does not required to prescribe the normal vector for
      // particles close to the triple point
      ...
      else
      {
        ...
        else
        {
          ...
          if (fi > 1.0000000001) dserror("fi must be smaller than 1.0!");

          if (fi < -0.0000000001) dserror("fi must be larger than 0.0!");
...
void PARTICLE::ParticleSPHInteractionHandler::MF_SmoothedCFG(
    const Teuchos::RCP<Epetra_Vector> CFG_FF, const Teuchos::RCP<Epetra_Vector> CFG_F1S,
    const Teuchos::RCP<Epetra_Vector> CFG_F2S, const Teuchos::RCP<Epetra_Vector> distWall)
{
  // checks
  if (CFG_FF == Teuchos::null) dserror("CFG_FF empty!");

  if (CFG_F1S == Teuchos::null) dserror("CFG_F1S empty!");

  if (CFG_F2S == Teuchos::null) dserror("CFG_F2S empty!");
  ...
  if (freeSurfaceType_ != INPAR::PARTICLE::TwoPhase)
  {
    ...
    for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
    {
           ...
           jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
      {
        ...
        // Boundary particles are not considered here
        if (particle_j.boundarydata_.boundaryparticle_ == true) continue;
        ...
        if (interData_ij.w_ij_ != 0)
        {
          ...
          {
            sumj_m_rho_dW_ij_FF.Update(particle_j.mass_ / particle_j.density_ *
                                           (particle_j.freesurfacedata_.color_field_ -
                                               particle_i.freesurfacedata_.color_field_) *
                                           interData_ij.dw_ij_,
                interData_ij.rRelVersor_ij_, 1.0);
          }
        }
           ...
           surfaceTensionType_ == INPAR::PARTICLE::ST_CONTI_ADAMI)
  {
    ...
    for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
    {
           ...
           jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
      {
        ...
        if (interData_ij.w_ij_ != 0)
        {
              ...
              particle_j.boundarydata_.boundaryparticle_ == false)
          {
            ...
            if (particle_i.freesurfacedata_.phase_color_ !=
                    particle_j.freesurfacedata_.phase_color_ and
                particle_j.freesurfacedata_.phase_color_ == 0)
              tilde_c_ji = particle_i.density_ / (particle_i.density_ + particle_j.density_);

            if (particle_i.freesurfacedata_.phase_color_ !=
                    particle_j.freesurfacedata_.phase_color_ and
                particle_i.freesurfacedata_.phase_color_ == 0)
              tilde_c_ji = -particle_i.density_ / (particle_i.density_ + particle_j.density_);
            ...
            if (CFG_FF != Teuchos::null)
            {
              sumj_m_rho_dW_ij_FF.Update(
                  (V_i * V_i + V_j * V_j) / V_i * tilde_c_ji * interData_ij.dw_ij_,
                  interData_ij.rRelVersor_ij_, 1.0);
            }
          }

          // interaction of (valid) solid particle j with fluid particle i
          if (particle_j.boundarydata_.boundaryparticle_ == true and
              particle_i.boundarydata_.boundaryparticle_ == false)
          {
            ...
            sumj_m_rho_dW_ij_F1S.Update(
                -V_j * V_j / V_i * interData_ij.dw_ij_, interData_ij.rRelVersor_ij_, 1.0);
          }
        }
      }  // loop over j
      ...
      if (particle_i.boundarydata_.haveboundaryneighbor_ == true and
          particle_i.boundarydata_.boundaryparticle_ == false)
      {
             ...
             jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
        {
          ...
          if (particle_j.boundarydata_.boundaryparticle_ == false) continue;

          double dist = interData_ij.rRelNorm2_ * normalWall.Dot(interData_ij.rRelVersor_ij_);

          if (dist < distWall_aux) distWall_aux = dist;
        }
    ...
    const Teuchos::RCP<Epetra_Vector> CFG_FF)
{
  ...
  // The basic idea of this extrapolation of the CFG to the triple point particles is, to modify the
  // direction of CFG (unit CFG), but to keep the original magnitude.
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    ...
    if (particle_i.freesurfacedata_.havephasechange_ == true and
        particle_i.boundarydata_.haveboundaryneighbor_ == true and
        particle_i.boundarydata_.boundaryparticle_ == false)
    {
      double norm_CFG_FF_i(CFG_FF_i.Norm2());

      // If the norm is very small, the particle is at the fringe of the phase transition region and
      // has negligible influence. Instead of a very small number, nothing is assembled at all.
      if (norm_CFG_FF_i < 1.0e-12 / particle_i.radius_) continue;
      ...
      // 2. step: weighted sum (smooth transition) between original CFG_FF_i and extrapolated
      // average vector CFG_FF_average_i
      ...
      if (norm > 1.0e-12 / particle_i.radius_) CFG_FF_mod_i.Scale(norm_CFG_FF_i / norm);
#else
      LINALG::Matrix<3, 1> CFG_FF_mod_i(CFG_FF_i);
#endif

#ifdef PARTICLE_ST_BOUNDARYSCALEFAC
      // Increase the surface tension forces in the triple zone by scaling factor to account for
      // reduced CFG magnitude due to lack of particle support
    ...
    const INPAR::PARTICLE::FreeSurfaceType freeSurfaceType)
{
  ...
  // checks
  if (density == Teuchos::null) dserror("density empty");
  ...
  if (freeSurfaceType == INPAR::PARTICLE::InteriorReinitialization)
  {
    ...
    // remains untouched!
    for (int lidNodeCol_i = 0; lidNodeCol_i != discret_->NodeColMap()->NumMyElements();
         ++lidNodeCol_i)
    {
      ParticleSPH& particle_i = colParticles_[lidNodeCol_i];
         ...
         ++lidNodeCol_i)
    {
      ...
      if (PARTICLE_REINITSHIFT == 1)
      {
        if (particle_i.freesurfacedata_.freesurfaceparticletype_ == FS_NONE)
          ...
          particle_i.density_ =
              particle_i.freesurfacedata_.density_sum_ / particle_i.freesurfacedata_.color_field_;
      }
      else if (PARTICLE_REINITSHIFT == 2)
      {
        if (particle_i.freesurfacedata_.color_field_ >= 1.0)
         ...
         ++lidNodeCol_i)
    {
      ...
      if (PARTICLE_REINITSHIFT == 1)
      {
        ...
        else
        {
          ...
          double density_0 = equationOfStateHandler_[matID_i]->PressureToDensity(
              p0, particle_i.density0_);  // based on material data of fluid particle_i

          particle_i.density_ = particle_i.freesurfacedata_.density_sum_ +
                                density_0 * (1.0 - particle_i.freesurfacedata_.color_field_);
        }
      }
      else if (PARTICLE_REINITSHIFT == 2)
      {
    ...
    const Teuchos::RCP<Epetra_Vector> fspType)
{
  ...
  // In the following, particles with color_field < PARTICLE_COLORLIMIT are classified as
  // free-surface particles of type FS_DIRECT. They are summed up in specific data containers
  // freesurfaceparticles_ (containing the particles) and neighbours_fsp_ (containing their
  // neighbours). All neighbours of these particles are classified as free-surface particles of type
  // FS_INDIRECT. These get no extra entries in freesurfaceparticles_ and neighbours_fsp_ so far!

  const int numcolelements = discret_->NodeColMap()->NumMyElements();

  // loop over the particles (no superpositions)
  for (unsigned int lidNodeCol_i = 0; lidNodeCol_i != (unsigned int)(numcolelements);
       ++lidNodeCol_i)
  {
    // determine the particle_i
    ParticleSPH& particle_i = colParticles_[lidNodeCol_i];

    // Decide if norm of CFG_ is large enough to yield a reasonable approximation for the surface
    // normal, which is not dominated by numerical noise. A similar criterion is suggested by Morris
    // et al. 2000, Eq.(20)
    ...
    if (particle_i.freesurfacedata_.color_field_ < PARTICLE_COLORLIMIT and
        particle_i.boundarydata_.boundaryparticle_ == false)
    {
  ...
  // loop over the boundary particles
  for (std::map<int, ParticleSPH*>::iterator ii = freesurfaceparticles_.begin();
       ii != freesurfaceparticles_.end(); ++ii)
  {
    int id_i = ii->first;
    // extract the node underlying the particle
    DRT::Node* currparticle = discret_->gNode(id_i);
...
double PARTICLE::ParticleSPHInteractionHandler::Inter_generalCoeff_ij(
    const ParticleSPH& particle_i, const ParticleSPH& particle_j, const InterDataPvP& interData_ij)
{
  ...
  else
  {
    ...
    if (freeSurfaceType_ != INPAR::PARTICLE::TwoPhase)
    {
      // According to Adami et al. 2012 Eq (28), the density of boundary particle is determined
      // based on the extrapolated pressure in Eq (27) and the equation of state based on material
      // data / mass of the interacting particle_i (for multi-phase flows, the material data might
      // strongly differ from particle to particle).
      double pressure_j = particle_j.boundarydata_.pressureMod_;
      // determine phase particle_i belongs to and material (boundary particles have phase color -1
      // and get the same material as phase 0)
      const int matID_i = std::max(particle_i.freesurfacedata_.phase_color_, 0);
      density_j = equationOfStateHandler_[matID_i]->PressureToDensity(
          pressure_j, particle_i.density0_);  // based on material data of fluid particle_i
    }
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_pressure(LINALG::Matrix<3, 1>* accn_ij,
    const double& generalCoeff_ij, const ParticleSPH& particle_i, const ParticleSPH& particle_j,
    const InterDataPvP& interData_ij)
{
  ...
  {
    // According to Adami et al. 2012 Eq (28), the density of boundary particle is determined based
    // on the extrapolated pressure in Eq (27) and the equation of state based on material data /
    // mass of the interacting particle_i (for multi-phase flows, the material data might strongly
    // differ from particle to particle).
...
void PARTICLE::ParticleSPHInteractionHandler::Inter_backgroundPressure(
    LINALG::Matrix<3, 1>* trvl_accn_ij, const double& generalCoeff_ij,
    const ParticleSPH& particle_i, const ParticleSPH& particle_j, const InterDataPvP& interData_ij)
{
  // get states of particle i
  const double density_i = particle_i.density_;
  ...
  else
  {
    ...
    double dw_ij_tilde =
        weightFunctionHandler_->DW(interData_ij.rRelNorm2_, radius_i / PARTICLE_P0ZHANG);
    const double p0_gradP0_Rho2_ij =
        mod_background_pressure_i * mass_j * dw_ij_tilde / std::pow(density_i, 2);
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_xsph(LINALG::Matrix<3, 1>* trvl_accn_ij,
    const ParticleSPH& particle_i, const ParticleSPH& particle_j, const InterDataPvP& interData_ij)
{
  // get states of particle i
  const double density_i = particle_i.density_;
  ...
  const LINALG::Matrix<3, 1> vel_j(
      (boundaryParticle_j) ? particle_j.boundarydata_.velModVisc_ : particle_j.vel_);
  // get states of (boundary) particle j
  double density_j = 0.0;
  double mass_j = 0.0;
  if (!boundaryParticle_j)
  ...
  double w_ij_stiff =
      weightFunctionHandler_->W(interData_ij.rRelNorm2_, radius_i / PARTICLE_P0ZHANG);
...
void PARTICLE::ParticleSPHInteractionHandler::Inter_transportVelocity_divA(
    LINALG::Matrix<3, 1>* accn_ij, LINALG::Matrix<3, 1>* accn_ij_A, const double& generalCoeff_ij,
    const ParticleSPH& particle_i, const ParticleSPH& particle_j, const InterDataPvP& interData_ij)
{
  // get states of particle i
  const double density_i = particle_i.density_;
  ...
  const LINALG::Matrix<3, 1> velConv_j(
      (boundaryParticle_j) ? LINALG::Matrix<3, 1>(true) : particle_j.velConv_);
  double density_j = 0.0;
  if (!boundaryParticle_j)
  {
    density_j = particle_j.density_;
  ...
  // Note: in the original formulation according to Adami2013, the contributions of boundary
  // particles to the tensor A are simply set to zero (according to an email by Stefan Adami, not
  ...
  if (not interactionVariant2_)
  {
    ...
    if (not boundaryParticle_j) p0_A_scaled.Update((1.0 / std::pow(density_j, 2)), A_j, 1.0);
  }
  else
  {
    p0_A_scaled.Update(0.5, A_i, 1.0);
    if (not boundaryParticle_j) p0_A_scaled.Update(0.5, A_j, 1.0);
  }

  p0_A_e_ij.MultiplyNN(1.0, p0_A_scaled, interData_ij.rRelVersor_ij_, 0.0);

  if (no_veldiff_term_ == false) accn_ij->Update(generalCoeff_ij, p0_A_e_ij, 1.0);
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_laminarViscosity(LINALG::Matrix<3, 1>* accn_ij,
    const double& generalCoeff_ij, const ParticleSPH& particle_i, const ParticleSPH& particle_j,
    const InterDataPvP& interData_ij)
{
  // get states of particle i
  ...
  if (visc_i > 1e-12 and visc_j > 1e-12) viscosity = 2.0 * visc_i * visc_j / (visc_i + visc_j);
  ...
  if (not interactionVariant2_)
  {
    ...
    // The following two viscous terms are taken from Espanol2003, Eq(30) and re-expressed in terms
    // of mass densities in an equivalent manner. This is necessary since Espanol2003 only specifies
    // the case m_i=m_j=m.
    ...
    // convection
    const double cCrRelVersorDotVrel_rho2rRelNorm2 =
        convectionCoeff * rRelVersorDotVrel * inv_rho2rRelNorm2;
    accn_ij->Update(
        generalCoeff_ij * cCrRelVersorDotVrel_rho2rRelNorm2, interData_ij.rRelVersor_ij_, 1.0);
  }
  else
  {
...
void PARTICLE::ParticleSPHInteractionHandler::Inter_artificialViscosity(
    LINALG::Matrix<3, 1>* accn_ij, const ParticleSPH& particle_i, const ParticleSPH& particle_j,
    const InterDataPvP& interData_ij)
{
  // get states of particle i
  const double density_i = particle_i.density_;
  ...
  const double h_ij = 0.5 * (weightFunctionHandler_->SmoothingLength(radius_i) +
                                weightFunctionHandler_->SmoothingLength(radius_j));
  ...
  // The parameter epsilon avoids division by zero when particles get to close. The value
  // epsilon=0.01 is a typical choice (see Adami et al. 2012).
  const double epsilon = 0.01;

  const double rRelVersorDotVrel = interData_ij.rRelVersor_ij_.Dot(vRel_ij);

  const double art_visc_fac =
      particle_i.extParticleMat_->artificialViscosity_ * h_ij * c_ij * rRelVersorDotVrel *
      interData_ij.rRelNorm2_ /
      (density_ij * (std::pow(interData_ij.rRelNorm2_, 2) + epsilon * std::pow(h_ij, 2)));

  // the following term represents the acceleration resulting from artificial viscosity as applied
  // in Adami et al. 2012, Eq. (11)
 ...
 *----------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::Inter_surfaceTension(LINALG::Matrix<3, 1>* accn_ij,
    const ParticleSPH& particle_i, const ParticleSPH& particle_j, const InterDataPvP& interData_ij,
    const double& time)
{
  // get states of (boundary) particle j
  ...
  Teuchos::RCP<PARTICLE::WeightFunction_QuinticBspline> weightFunction =
      Teuchos::rcp(new PARTICLE::WeightFunction_QuinticBspline(WF_DIM_));
  ...
  {
    potential_ij = -surftenspotAff_ * weightFunction->W(interData_ij.rRelNorm2_, radius0) +
                   weightFunction->W(interData_ij.rRelNorm2_, radius_ij);
    s_ij = surftensfacff_;
  }
  else  // case of fluid-solid interaction
  {
    potential_ij = -surftenspotAfs_ * weightFunction->W(interData_ij.rRelNorm2_, radius0) +
                   weightFunction->W(interData_ij.rRelNorm2_, radius_ij);
    s_ij = surftensfacfs_;
  }
  ...
  {
    double timefac = 1.0;
    if (time >= 0.0) timefac = SurfTensionTimeFac(time);

    // pair-wise interaction force to model surface tension following Kordilla et al. 2013 and
    // Tartakovsky et al. 2016
    accn_ij->Update(
        -s_ij * potential_ij * timefac / particle_i.mass_, interData_ij.rRelVersor_ij_, 1.0);
  }
...
void PARTICLE::ParticleSPHInteractionHandler::Inter_fspvp_Adami_1(
    const Teuchos::RCP<Epetra_Vector> accn, const Teuchos::RCP<Epetra_Vector> curvature,
    double time)
{
  if (surftensfacff_ <= 0.0) return;

  if (curvature != Teuchos::null) curvature->PutScalar(0.0);

  // loop over the free surface particles (with superpositions -> pairs ij and ji evaluted
  // seperately!) Calculate surface curvature according to Adami et al. 2010, Eq.(20)
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    // determine particle i
    const int gid_i = discret_->NodeRowMap()->GID(lidNodeRow_i);
    ...
    if (particle_i.freesurfacedata_.CFG_FF_.Norm2() < 1.0e-12 / particle_i.radius_ or
        particle_i.boundarydata_.boundaryparticle_ == true)
      continue;

#ifdef PARTICLE_ONLYPHASE1ACC
    if (particle_i.freesurfacedata_.phase_color_ == 1) continue;
#endif
    ...
    // denominator according to Morris et al. 2000, Eq. (24). Attention: Here we need a special
    // color field and cannot reuse the color field already calculated earlier, since here only the
    // particle with a sufficiently high color field gradient are considered!!!
    double sum_Wij = 0.0;
    // auto contribution
    sum_Wij +=
        weightFunctionHandler_->W0(particle_i.radius_) * particle_i.mass_ / particle_i.density_;
    ...
    for (jj = neighbours_p_[lidNodeRow_i].begin(); jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
    {
      ...
      // Only particles, whose color field gradient is non-zero
      if (particle_j.freesurfacedata_.CFG_FF_.Norm2() < 1.0e-12 / particle_i.radius_ or
          particle_j.freesurfacedata_.CFG_F2S_.Norm2() < 1.0e-10 * particle_i.radius_ or
          particle_j.boundarydata_.boundaryparticle_ == true)
        continue;
      ...
      if (interData_ij.dw_ij_ != 0)
      {
        ...
        double V_j = particle_j.mass_ / particle_j.density_;
        sum_nij_eij_dWij +=
            (n_i.Dot(interData_ij.rRelVersor_ij_) - n_j.Dot(interData_ij.rRelVersor_ij_)) * V_j *
            interData_ij.dw_ij_;
    ...
    double kappa_i = 0.0;
    if (abs(sum_Wij) > 1.0e-10) kappa_i = -sum_nij_eij_dWij / sum_Wij;
    ...
    double time)
{
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    ...
    // Only particles, whose color field gradient is large enough (in order to avoid the noise of
    // small contributions). --> See Morris et al. 2000, Eq.(20)
    if (particle_i.freesurfacedata_.validNormal_ == false or
        particle_i.boundarydata_.boundaryparticle_ == true)
      continue;
    ...
    for (jj = neighbours_p_[lidNodeRow_i].begin(); jj != neighbours_p_[lidNodeRow_i].end(); ++jj)
    {
      ...
      // Only particles, whose color field gradient is large enough (in order to avoid the noise of
      // small contributions). --> See Morris et al. 2000, Eq.(20)
      if (particle_j.freesurfacedata_.validNormal_ == false or
          particle_j.boundarydata_.boundaryparticle_ == true)
        continue;
...
void PARTICLE::ParticleSPHInteractionHandler::Inter_fspvp_Adami_ContactAngleVar2(
    const Teuchos::RCP<Epetra_Vector> accn, double time)
{
  if (surftensfacff_ <= 0.0) return;
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    ...
    // Only particles, whose color field gradient is non-zero
    if (particle_i.freesurfacedata_.CFG_FF_.Norm2() < 1.0e-12 / particle_i.radius_ or
        particle_i.freesurfacedata_.CFG_F2S_.Norm2() < 1.0e-10 * particle_i.radius_ or
        particle_i.boundarydata_.boundaryparticle_ == true or
        particle_i.boundarydata_.haveboundaryneighbor_ == false)
      continue;
    ...
    // The factor 2.0 is required since the boundary dirac delta functioin is only evaluated in the
    // fluid and not in the solid phase (integral gives 0.5 and not 1)!
    double force = surftensfacff_ * (cosalpha - cosalpha0) *
                   particle_i.freesurfacedata_.CFG_FF_.Norm2() * 2.0 * normal_wi_norm;
 ...
 *------------------------------------------------------------------------------*/
void PARTICLE::ParticleSPHInteractionHandler::ViscousCoefficients(const double viscosity,
    const double bulkViscosity, double& convectionCoeff, double& diffusionCoeff)
{
#ifndef PARTICLE_ONLYLAPLACETERM
  // Attention: The trace of a tensor as required to derive equation (28) from equation (25) in
  // Espanol2003 depends on the spatial dimension --> SPH approximations of the Laplace operator
  ...
  convectionCoeff = 0.0;
  if (bulkViscosity > 0) dserror("Bulk Viscosity not considered here!");
#endif
  ...
  {
    dserror(
        "The diffusion coefficient is negative! The following equation should hold: "
        "5*dynamicViscosity >= 3*bulkViscosity");
  }

  if (convectionCoeff < 0)
  {
    dserror(
        "The convection coefficient is negative! Are you sure that the dynamic viscosity and the "
        "bulk modulus are positive?");
  }
...
void PARTICLE::ParticleSPHInteractionHandler::Density2Pressure(
    const Teuchos::RCP<const Epetra_Vector> density, Teuchos::RCP<Epetra_Vector>& pressure)
{
  // checks
  ...
  if (pressure == Teuchos::null) pressure = Teuchos::rcp(new Epetra_Vector(density->Map(), true));
  ...
  for (int lidNodeRow_i = 0; lidNodeRow_i < density->MyLength(); ++lidNodeRow_i)
  {
    ...
    // and get the same material as phase 0)
    const int matID_i = std::max(particle_i.freesurfacedata_.phase_color_, 0);
    (*pressure)[lidNodeRow_i] = equationOfStateHandler_[matID_i]->DensityToPressure(
        (*density)[lidNodeRow_i], particle_i.density0_);
  }
}
...
void PARTICLE::ParticleSPHInteractionHandler::DetermineIntEnergy(double& intenergy)
{
  ...
  for (unsigned int lidNodeRow_i = 0; lidNodeRow_i != neighbours_p_.size(); ++lidNodeRow_i)
  {
    ...
    // Boundary particles are not considered here (there is no potential existent for the pressure /
    // density values extrapolated to the boundary particles) On the long term, a more elaborate
    // scheme might be desirable that at least consideres the work contributions of boundary
    // particles!
    if (particle_i.boundarydata_.boundaryparticle_) continue;

    // determine phase particle_i belongs to and material (boundary particles have phase color -1
    // and get the same material as phase 0)
    const int matID_i = std::max(particle_i.freesurfacedata_.phase_color_, 0);
    intenergy += equationOfStateHandler_[matID_i]->DensityToEnergy(
        particle_i.density_, particle_i.mass_, particle_i.density0_);
  }
  return;
}
