...
#ifdef FLUID2_IS_TERM2
    /* Stabilisierung der Konvektion ( L_conv_u) */
    eforce_(vi*2 + 4) += -(ttimetauM*conv_c_(vi)*conv_old_(0)) ;
    eforce_(vi*2 + 5) += -(ttimetauM*conv_c_(vi)*conv_old_(1)) ;
#endif
...
#ifdef FLUID2_IS_TERM8
    /* Stabilisierung der Viskosität ( L_pres_p) */
    eforce_(vi*2 + 4) += -2.0*nu_*ttimetauMp*(gradp_(0)*viscs2_(0, 0, vi) + gradp_(1)*viscs2_(0, 1, vi)) ;
    eforce_(vi*2 + 5) += -2.0*nu_*ttimetauMp*(gradp_(0)*viscs2_(0, 1, vi) + gradp_(1)*viscs2_(1, 1, vi)) ;
#endif
...
#ifdef FLUID2_IS_TERM16
    /* Konvektionsstabilisierung */
    eforce_(vi*2 + 4) += timetauM*conv_c_(vi)*rhsint_(0) ;
    eforce_(vi*2 + 5) += timetauM*conv_c_(vi)*rhsint_(1) ;
#endif
