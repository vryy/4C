...
for (vi=0; vi<4; ++vi)
{
  for (ui=0; ui<4; ++ui)
  {
    ...
    estif_(vi*3, ui*3 + 2)     += ttimetauM*(conv_c_(vi) + conv_g_(vi))*pderxy_(0, ui) ;
    estif_(vi*3 + 1, ui*3)     += ttimetauM*funct_(ui)*gradp_(1)*derxy_(0, vi) ;
    estif_(vi*3 + 1, ui*3 + 1) += ttimetauM*funct_(ui)*gradp_(1)*derxy_(1, vi) ;
    estif_(vi*3 + 1, ui*3 + 2) += ttimetauM*(conv_c_(vi) + conv_g_(vi))*pderxy_(1, ui) ;
#endif
...
for (vi=4; vi<iel; ++vi)
{
  for (ui=0; ui<4; ++ui)
  {
    ...
    estif_(vi*2 + 4, ui*3 + 2) += ttimetauM*(conv_c_(vi) + conv_g_(vi))*pderxy_(0, ui) ;
    estif_(vi*2 + 5, ui*3)     += ttimetauM*funct_(ui)*gradp_(1)*derxy_(0, vi) ;
    estif_(vi*2 + 5, ui*3 + 1) += ttimetauM*funct_(ui)*gradp_(1)*derxy_(1, vi) ;
    estif_(vi*2 + 5, ui*3 + 2) += ttimetauM*(conv_c_(vi) + conv_g_(vi))*pderxy_(1, ui) ;
#endif
