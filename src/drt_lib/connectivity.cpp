    ...
    std::vector<Teuchos::RCP<Epetra_Map> >& body_node_maps )
{
  ...
  for ( int i=0; i<num_inodes; ++i )
  {
    ...
    {
      if ( my_body_nset.find( node ) != my_body_nset.end() )
        break;
      ++body_id;
    }
...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void DRT::Connectivity::createBodyNodeMaps(
    const MapFormat format,
    const Epetra_Comm& comm,
    const Connection& connections,
    const node_sets& my_body_nsets,
    std::vector<Teuchos::RCP<Epetra_Map> >& body_node_maps )
{
  ...
  while ( p<num_procs )
  {
    ...
    while ( rsum > 0 )
    {
      for ( int i=0; i<receivefrom[comm.MyPID()]; ++i)
      {
        ...
        if ( rlength != 1 )
          dserror( "Wrong length!" );

        if ( build_bid != -1 and build_bid != rdata[0] )
    ...
    const MapFormat format )
{
  ...
  switch ( format )
  {
    ...
    case MapFormat::column:
      std::transform( my_node_set.begin(), my_node_set.end(),
          std::back_inserter( my_node_vec ),
  ...
  return Teuchos::rcp( new Epetra_Map( -1, my_node_vec.size(),
      my_node_vec.data(), 0, comm ) );
}

/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
void DRT::Connectivity::receiveAnyOfMyGids(
    received_gid_set& all_received_ngids,
    const std::vector<node_set*>& my_new_body_nsets,
    const Epetra_Comm& comm )
{
  ...
  for ( int i=0; i<num_bodies; ++i )
  {
    ...
    if ( my_new_body_nsets[i] )
      tmp_ptr = Teuchos::rcpFromRef(*my_new_body_nsets[i]);
    ...
    for ( auto pg = my_ghosted_ngids.begin(); pg!=my_ghosted_ngids.end(); ++pg, ++mpir )
    {
      ...
      exporter.ISend( from_proc, to_proc, pg->second.data(), pg->second.size(),
          100, *mpir );
    }
...
//          "from proc #" << rfrom_proc << std::endl;
      ...
      if ( rtag != 100 )
        dserror("The received tag seems to be wrong! (tag=%d)", rtag);

      std::copy( rdata.begin(),rdata.end(),
          std::back_inserter( received_ngids[rfrom_proc] ) );
    }

    // wait till all data were successfully sent and received
    for ( auto mpi_request : mpi_requests )
      exporter.Wait( mpi_request );
  }
}
...
void DRT::Connectivity::fillMyConnectedNodeSet(
    node_set& connected_nset,
    const DRT::DiscretizationInterface& full_discret )
{
  std::set<int> done_element_ids;
  std::set<int> connected_node_gids;
  for ( const DRT::Node* node : connected_nset )
    connected_node_gids.insert( node->Id() );
  std::vector<const DRT::Node*> connected_nodes( connected_nset.begin(),
      connected_nset.end() );
