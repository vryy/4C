     ...
     const bool verbose)
{
  ...
  BuildInternalFaces(col_pbcmapmastertoslave, verbose);
  ...
  if (verbose)
  {
    ...
    int summyfaces = intfacerowptr_.size();
    int summall = 0;
 ...
 *----------------------------------------------------------------------*/
void DRT::DiscretizationFaces::BuildInternalFaces(Teuchos::RCP<std::map<int,std::vector<int> > > col_pbcmapmastertoslave,
                                                  const bool verbose)
{
    ...
    for (unsigned int iele = 0; iele < nele; iele++)
    {
      ...
      else
      {
        ...
        for (unsigned int inode=0;inode<nnode;inode++) // master face nodes
        {
          ...
          {
            if (nodes[knode] == nodes_face_master[inode])
              position = knode;
          }
    ...
    if (masternodes != col_pbcmapmastertoslave->end())
    {
      ...
      if (slave_peid == -1)
      {
        ...
        for (std::size_t numcond = 0; numcond < mypbcs.size(); numcond++)
        {
          ...
          else dserror("Unknown type for pbc!");
        }
        ...
        for (std::size_t inode = 0; inode < mynodeids.size(); inode++)
        {
          ...
          else
          {
            ...
            if (found)
              myfurthermasternodeids.push_back(mynodeids[inode]);
          }
        }
          ...
          if ((mymasternodeids.size() + myfurthermasternodeids.size()) == mynodeids.size())
          {
            ...
            for (std::size_t rr=0; rr < mymasternodeids.size(); rr++)
            {
              ...
              if (((*col_pbcmapmastertoslave)[mymasternodeids[rr]]).size() == 1)
              {
                ...
                local_pbcmapmastertoslave[mymasternodeids[rr]]=((*col_pbcmapmastertoslave)[mymasternodeids[rr]])[0];
              }
              ...
              else
              {
                ...
                for (std::size_t imnode=0; imnode < mymasternodeids.size(); imnode++)
                {
                  for (int ipbc=0; ipbc<numpbcpairs; ipbc++)
                  {
                    std::set<int>::iterator iter = (mastertopbcset[ipbc]).find(mymasternodeids[imnode]);
                    if (iter != (mastertopbcset[ipbc]).end())
                      pbcspermaster[ipbc] += 1;
                  }
                }
                ...
                if (numpbcpairs == 3 and ((*col_pbcmapmastertoslave)[mymasternodeids[rr]]).size() == 3)
                ...
                {
                  if (ipbc != slavepbcid)
                    remainingmasterpbcids.push_back(ipbc);
                }

                // loop all slave nodes of the current master and
                // check which node fulfills above conditions
                int actslaveid = -999;
                for (std::size_t islave=0; islave<((*col_pbcmapmastertoslave)[mymasternodeids[rr]]).size(); islave++)
                {
                  // get id
                  actslaveid = ((*col_pbcmapmastertoslave)[mymasternodeids[rr]])[islave];

                  // check first criterion -> (i)
                  if ((slavetopbcset[slavepbcid]).find(actslaveid) != (slavetopbcset[slavepbcid]).end())
                  {
                    std::size_t found = 0;
                    // if satisfied
                    ...
                    {
                      if ((mastertopbcset[remainingmasterpbcids[k]]).find(actslaveid) != (mastertopbcset[remainingmasterpbcids[k]]).end())
                        found++;
                    }
            ...
            // next go to the special masters which occur as slaves in the list col_pbcmapmastertoslave
            // and are indeed edge or corner nodes of master surfaces
            if (myfurthermasternodeids.size()>0)
            {
              ...
              for (std::size_t imnode=0; imnode < myfurthermasternodeids.size(); imnode++)
              {
                for (int ipbc=0; ipbc<numpbcpairs; ipbc++)
                {
                  std::set<int>::iterator iter = (mastertopbcset[ipbc]).find(myfurthermasternodeids[imnode]);
                  if (iter != (mastertopbcset[ipbc]).end())
                    pbcspermaster[ipbc] += 1;
                }
              }
              ...
              for (int ipbc=0; ipbc<numpbcpairs; ipbc++)
              {
                if (pbcspermaster[ipbc] == (int)(mymasternodeids.size()+myfurthermasternodeids.size()))
                {
                  masterpbcid = ipbc;
                  break;
                }
              ...
              for (std::size_t ifnode=0; ifnode<myfurthermasternodeids.size(); ifnode++)
              {
                ...
                for (master_it=col_pbcmapmastertoslave->begin(); master_it!=col_pbcmapmastertoslave->end(); master_it++)
                {
                ...
                bool three_sets_edge_node = false;
                if (numpbcpairs == 3 and mypotslaveids.size() == 3)
                  three_sets_edge_node = true;
                ...
                {
                  if (ipbc != slavepbcid)
                    remainingslavepbcids.push_back(ipbc);
                }
                ...
                if (numpbcpairs == 3 and mypotslaveids.size() == 7)
                {
                  ...
                  for (std::size_t k=0; k<remainingmasterpbcids.size(); k++)
                  {
                    if ((mastertopbcset[remainingmasterpbcids[k]]).find(actnodeid) != (mastertopbcset[remainingmasterpbcids[k]]).end())
                  ...
                  else
                  {
                    ...
                    {
                      if (slavepbcid == 1) slavecond_1 = 2;
                      else if (slavepbcid == 2) slavecond_1 = 1;
                      else dserror("Same pbc ids?");
                    }
                    else if (furthermastercond == 1)
                    {
                      if (slavepbcid == 0) slavecond_1 = 2;
                      else if (slavepbcid == 2) slavecond_1 = 0;
                      else dserror("Same pbc ids?");
                    }
                    else if (furthermastercond == 2)
                    {
                      if (slavepbcid == 0) slavecond_1 = 1;
                      else if (slavepbcid == 1) slavecond_1 = 0;
                      else dserror("Same pbc ids?");
                    }
                    else dserror("Unknown pbc id!");
                  }
                }
...
//                for (myiter=mastertopbcset[remainingslavepbcids[0]].begin(); myiter!=mastertopbcset[remainingslavepbcids[0]].end(); myiter++)
//                {
//                  std::cout<< *myiter << std::endl;
//                }
//                std::cout<< "master 2 \n" << masterpbcid << std::endl;
//                for (myiter=mastertopbcset[remainingslavepbcids[1]].begin(); myiter!=mastertopbcset[remainingslavepbcids[1]].end(); myiter++)
//                  std::cout<< *myiter << std::endl;
                ...
                for (std::size_t islave=0; islave<mypotslaveids.size(); islave++)
                {
                  ...
                  if ((slavetopbcset[slavepbcid]).find(actslaveid) != (slavetopbcset[slavepbcid]).end())
                  {
                    ...
                    if (not corner_node)
                    {
                      ...
                      {
                        if ((slavetopbcset[remainingslavepbcids[k]]).find(actslaveid) != (slavetopbcset[remainingslavepbcids[k]]).end())
                          found++;
                      }
                    }
                    else
                    {
                      ...
                      if ((mastertopbcset[furthermastercond]).find(actslaveid) != (mastertopbcset[furthermastercond]).end())
                        found++;
                      if ((slavetopbcset[slavecond_1]).find(actslaveid) != (slavetopbcset[slavecond_1]).end())
                        found++;
                    }
          ...
          for (std::size_t kk=0; kk<mymasternodeids.size(); kk++)
          {
            ...
            {
              if ((*nofool)->Id() == mymasternodeids[kk])
                counter++;
            }
          }
          for (std::size_t kk=0; kk<myfurthermasternodeids.size(); kk++)
          {
            ...
            for(nofool=noderowptr_.begin(); nofool != noderowptr_.end(); ++nofool)
            {
              if ((*nofool)->Id() == myfurthermasternodeids[kk])
                counter++;
            }
          }
          if (counter == 0)
          ...
          if (add_salve_ele_to_face)
          {
            ...
            for(fool=elecolptr_.begin(); fool != elecolptr_.end(); ++fool)
            {
              if ((*fool)->Id() == master_peid)
                master_ele = *fool;
            }

            // look for the corresponding slave face in the list of all faces
            std::map<std::vector<int>, InternalFacesData>::iterator pbc_surf_it = surfmapdata.find(myslavenodeids);
            ...
            for (unsigned int inode=0;inode<nnode;inode++) // master face nodes
            {
              ...
              {
                if (slave_nodes[knode]->Id() == local_pbcmapmastertoslave[nodes_face_master[inode]->Id()])
                  position = knode;
              }
...
////            {
////              std::cout << "master node id " << nodes_face_master[kk]->Id() << std::endl;
////              std::cout << "slave node id " << slave_nodes[kk]->Id() << std::endl;
////              std::cout << "slave node position " << localtrafomap[kk] << std::endl;
////            }
//            }
 ...
 *----------------------------------------------------------------------*/
void DRT::DiscretizationFaces::BuildIntFaceRowMap()
{
 ...
 *----------------------------------------------------------------------*/
void DRT::DiscretizationFaces::BuildIntFaceColMap()
{
 ...
 *----------------------------------------------------------------------*/
const Epetra_Map* DRT::DiscretizationFaces::IntFacesRowMap() const
{
#ifdef DEBUG
 ...
 *----------------------------------------------------------------------*/
const Epetra_Map* DRT::DiscretizationFaces::IntFacesColMap() const
{
#ifdef DEBUG
 ...
 *----------------------------------------------------------------------*/
int DRT::DiscretizationFaces::NumGlobalIntFaces() const
{
#ifdef DEBUG
 ...
 *----------------------------------------------------------------------*/
int DRT::DiscretizationFaces::NumMyRowIntFaces() const
{
#ifdef DEBUG
 ...
 *----------------------------------------------------------------------*/
int DRT::DiscretizationFaces::NumMyColIntFaces() const
{
