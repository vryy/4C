...
void DRT::UTILS::Evaluate(
    ...
    std::vector<Teuchos::RCP<Epetra_Vector> > & systemvectors,
    const Epetra_Map * col_ele_map )
{
  dsassert( systemmatrices.size() <= 2, "Currently a maximum number of two "
      "system-matrices is supported!" );
  dsassert( systemvectors.size() <= 3, "Currently a maximum number of three "
      "system-vectors is supported!" );

  if ( systemmatrices.size() < 2 )
    systemmatrices.resize( 2, Teuchos::null );
  if ( systemvectors.size() < 3 )
      systemvectors.resize( 3, Teuchos::null );

  DRT::AssembleStrategy strategy( 0, 0, systemmatrices[0], systemmatrices[1],
      systemvectors[0], systemvectors[1], systemvectors[2] );
    ...
    const Epetra_Map * col_ele_map )
{
  ...
  // that does nothing
  {
    ...
    ParObjectFactory::Instance().PreEvaluate( discret,eparams,
  ...
  for ( int i=0; i<numcolele; ++i )
  {
...

    {
      ...
      int err = actele->Evaluate( eparams,discret,la,
                                  strategy.Elematrix1(),
                                  strategy.Elematrix2(),
                                  strategy.Elevector1(),
                                  strategy.Elevector2(),
                                  strategy.Elevector3() );
      if ( err )
      dserror("Proc %d: Element %d returned err=%d",
        discret.Comm().MyPID(), actele->Id(),err);
    }
