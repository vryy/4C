...
void DRT::UTILS::MatchElementDistributionOfMatchingConditionedElements(
    DRT::Discretization&     dis_template,
    DRT::Discretization&     dis_to_redistribute,
    const std::string& condname_template,
    const std::string& condname_redistribute )
{
  // clone communicator of target discretization
  Teuchos::RCP<Epetra_Comm> com = Teuchos::rcp( dis_template.Comm().Clone());
  if(com->NumProc()>1)
  {
    ...
    {
      IO::cout(IO::verbose)<<"+-----------------------------------------------------------------------------+"<<IO::endl;
      IO::cout(IO::verbose)<<"|   Match element distribution of discr. "<<std::setw(11)<<dis_to_redistribute.Name()
          <<"                          |"<<IO::endl;
  ...
  dis_from_template_condition = discreator->CreateMatchingDiscretizationFromCondition(
      ...
      condname_template, ///< name of the condition, by which the derived discretization is identified
  ...
  const Epetra_Map* template_cond_dis_elecolmap
      = dis_from_template_condition->ElementColMap();
  ...
  for (it=matched_ele_map.begin(); it!=matched_ele_map.end(); ++it)
  {
    std::cout<<"ELEMENT : "<<it->first<<" ->  ( "<<it->second[0]<<", "<<it->second[1]<<", "<<it->second[2]<<" )"<<" on PROC "
        <<dis_to_redistribute.Comm().MyPID()<<" map size = "<<matched_ele_map.size()<<std::endl;
  }
  ...
  for(int lid = 0; lid<dis_to_redistribute.ElementColMap()->NumMyElements();lid++)
  {
    ...
    DRT::Element* ele = dis_to_redistribute.gElement(dis_to_redistribute.ElementColMap()->GID(lid));
    DRT::Node** nodes = ele->Nodes();
    for(int node=0;node<ele->NumNode();node++)
    {
      DRT::Condition* nodal_cond =
      nodes[node]->GetCondition(condname_redistribute);
      if(nodal_cond!=NULL)
      {
        conditionedele = true;
        break;
    ...
    if(not conditionedele)
    {
      ...
      if(ele->Owner()==com->MyPID())
        redistribute_rowelegid_vec.push_back(ele->Id());
    }
  ...
  {
    if(dis_template.gNode(dis_template.NodeColMap()->GID(lid))->GetCondition(condname_template)!=NULL)
  ...
  for (it=matched_node_map.begin(); it!=matched_node_map.end(); ++it)
  {
    ...
    if((int)(it->second)[2]==1)
      redistribute_rownodegid_vec.push_back(it->first);
  ...
  for (it=matched_node_map.begin(); it!=matched_node_map.end(); ++it)
  {
    std::cout<<"NODE : "<<it->first<<" ->  ( "<<it->second[0]<<", "<<it->second[1]<<", "<<it->second[2]<<" )"<<" on PROC "
        <<dis_to_redistribute.Comm().MyPID()<<" map size = "<<matched_node_map.size()<<std::endl;
  }
  ...
  for(int lid = 0; lid<dis_to_redistribute.NodeRowMap()->NumMyElements();lid++)
  {
    DRT::Condition* testcond =
        dis_to_redistribute.gNode(dis_to_redistribute.NodeRowMap()->GID(lid))->GetCondition(condname_redistribute);
    if(testcond==NULL)
      redistribute_rownodegid_vec.push_back(dis_to_redistribute.gNode(dis_to_redistribute.NodeRowMap()->GID(lid))->Id());
  }
  // add col nodes
  for(int lid = 0; lid<dis_to_redistribute.NodeColMap()->NumMyElements();lid++)
  {
    DRT::Condition* testcond =
        dis_to_redistribute.gNode(dis_to_redistribute.NodeColMap()->GID(lid))->GetCondition(condname_redistribute);
    if(testcond==NULL)
      redistribute_colnodegid_vec.push_back(dis_to_redistribute.gNode(dis_to_redistribute.NodeColMap()->GID(lid))->Id());
  }

  // construct redistributed node row map
  Teuchos::RCP<Epetra_Map> redistributed_noderowmap =
      Teuchos::rcp(new Epetra_Map(-1, redistribute_rownodegid_vec.size(), &redistribute_rownodegid_vec[0], 0, *com));
 ...
 *                                                          rauch 10/16 |
 *----------------------------------------------------------------------*/
void DRT::UTILS::MatchElementRowColDistribution(
    const DRT::Discretization& dis_template,
    const DRT::Discretization& dis_to_redistribute,
    std::vector<int>& row_id_vec_to_fill,
    std::vector<int>& col_id_vec_to_fill)
{
  ...
  for (it=matched_ele_map.begin(); it!=matched_ele_map.end(); ++it)
  {
    ...
    // we also want to own all nodes of this element.
    if((int)(it->second)[2]==1)
      row_id_vec_to_fill.push_back(it->first);
...
/*----------------------------------------------------------------------*
 *----------------------------------------------------------------------*/
void DRT::UTILS::MatchNodalRowColDistribution(
    const DRT::Discretization& dis_template,
    const DRT::Discretization& dis_to_redistribute,
    std::vector<int>& row_id_vec_to_fill,
    std::vector<int>& col_id_vec_to_fill)
