    ...
    const std::string*               geofilepath)
{
  ...
  case INPAR::geometry_full:
  {
    ...
    Teuchos::RCP<ElementReader> er = Teuchos::rcp(new DRT::INPUT::ElementReader(dis, reader, fullsectionname, elementtypes));
  ...
  case INPAR::geometry_box:
  {
    ...
    Teuchos::RCP<DomainReader> dr = Teuchos::rcp(new DRT::INPUT::DomainReader(dis, reader, fullsectionname, elementtypes));
        ...
        for (; file; ++filecount)
        {
          ...
          else if (tmp=="FNODE")
          {
            ...
            if (tmp2 =="FIBER1") //fiber information
            {
              ...
              cosy[0] = cosy[1] = cosy[2] = cosy[3] = cosy[4] = cosy[5] = angle[0] = angle[1] = 0.;
              for (unsigned i=0; i<diss.size(); ++i)
              {
                // create node and add to discretization
                Teuchos::RCP<DRT::FIBER::FiberNode> node = Teuchos::rcp(new DRT::FIBER::FiberNode(nodeid,coords,fiber,cosy,angle,myrank));
                diss[i]->AddNode(node);
              }
            }
