...
def read_geometry(path):
    with open(path, "rb") as f:
        ...
        try:
            ...
            if node_id_given:
                geometry["node_ids"] = read_ints(f, number_nodes)

            # read coordinates
            geometry["coordinates"] = (
                read_floats(f, number_nodes * 3).reshape(3, number_nodes).transpose()
            )

            # read different element types
            element_type = read_string(f, 80).rstrip("\x00")
            while not element_type.startswith("part") and not element_type.startswith(
                "END"
            ):
                ...
                if element_type not in KNOWN_ELEMENTS:
                    raise ValueError("Unknown element type {0}".format(element_type))

                number_nodes_per_element = KNOWN_ELEMENTS[element_type]

                element_nodes = read_ints(
                    f, number_elements * number_nodes_per_element
