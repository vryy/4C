...
if __name__ == "__main__":
    ...
    if len(case_file["geometries"]) != 1:
        raise ValueError(
            "Currently only 1 geometry is supported. Found {0} geometries".format(
    ...
    geometry = read_geometry(
        os.path.join(basepath, case_file["geometries"][0]["geofile"])
    )
    ...
    for r in result_description:
        if not r["enabled"]:
            print("{0}: {1} is DISABLED. SKIPPING!".format(r["dis"], r["quantity"]))
            continue

        try:
            variable_name, dim = get_corresponding_quantity(r["dis"], r["quantity"])
        except NotImplementedError:
            print("{0}: {1} is NOT IMPLEMENTED".format(r["dis"], r["quantity"]))
            errors += 1
            continue

        # check whether variable is implemented for postprocessing tests
        if variable_name == None:
            print("{0}: {1} is NOT TESTABLE. SKIPPING!".format(r["dis"], r["quantity"]))
            continue

        # check whether variable exists in case file
        if variable_name not in case_file["variables"]:
            print(
                "{0}: {1} at node {2}: variable could NOT BE FOUND in ensight file".format(
                    r["dis"], r["quantity"], r["node"]
                )
            )
            errors += 1
            continue
        ...
        if not found_nid:
            print(
                "{0}: {1} at node {2}: node could NOT BE FOUND in ensight file".format(
                    r["dis"], r["quantity"], r["node"]
                )
            )
            errors += 1
        ...
        if variable["type"].endswith("per node"):
            ...
            if abs(myvalue - r["value"]) > tolerance:
                print(
                    "{0}: {1} at node {2} is WRONG --> actresult={3}, givenresult={4}, abs(diff)={5} > {6}".format(
                        r["dis"],
                        r["quantity"],
                        r["node"],
                        myvalue,
            ...
            print(
                "{0}: {1} at node {2} is CORRECT, abs(diff)={3} < {4}".format(
                    r["dis"],
                    r["quantity"],
                    r["node"],
                    abs(myvalue - r["value"]),
    ...
    if success == 0 and errors == 0:
        print(
            "Nothing has been tested. Ensight comparison is not possible for your kind of results! You need to adapt it to test your quantities as well."
        )
        sys.exit(1)
