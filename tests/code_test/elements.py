...
class Element( object ):
    ...
    def checkjacobi(self,nodemap):
        # complain by default
        print "default in element type",self.name()
        return 1
...
def readelement(line):
    ...
    if line[2]=="HEX27":     return Hex20Element(    [int(line[i]) for i in range(3,23)])
    if line[2]=="HEX20":     return Hex20Element(    [int(line[i]) for i in range(3,23)])
    if line[2]=="HEX8":      return Hex8Element(     [int(line[i]) for i in range(3,11)])
    if line[2]=="PYRAMID13": return Pyramid13Element([int(line[i]) for i in range(3,16)])
    if line[2]=="PYRAMID5":  return Pyramid5Element( [int(line[i]) for i in range(3, 8)])
    if line[2]=="QUAD4":     return Quad4Element(    [int(line[i]) for i in range(3, 7)])
    if line[2]=="QUAD8":     return Quad8Element(    [int(line[i]) for i in range(3,11)])
    if line[2]=="QUAD9":     return Quad9Element(    [int(line[i]) for i in range(3,12)])
    if line[2]=="TET10":     return Tet10Element(    [int(line[i]) for i in range(3,13)])
    if line[2]=="TET4":      return Tet4Element(     [int(line[i]) for i in range(3, 7)])
    if line[2]=="TRI3":      return Tri3Element(     [int(line[i]) for i in range(3, 6)])
    if line[2]=="TRI6":      return Tri6Element(     [int(line[i]) for i in range(3, 9)])
    if line[2]=="WEDGE15":   return Wedge15Element(  [int(line[i]) for i in range(3,18)])
    if line[2]=="WEDGE6":    return Wedge6Element(   [int(line[i]) for i in range(3, 9)])
    if line[2]=="LINE2":     return Line2Element(    [int(line[i]) for i in range(3, 5)])
...
def readnodes(sections):
    ...
    nodeids = []
    nodemap = {}
    for line in progress("read nodes",sections["NODE COORDS"]):
        nid = int(line[1])
        node = (float(line[3]),float(line[4]),float(line[5]))
        nodes.append(node)
        nodeids.append(nid)
        nodemap[nid] = node
    return nodes, nodeids, nodemap


def surfacemesh(section):
    volumeelements = {}

    for e in progress("read elements",section):
    ...
    mesh = {}
    for localnodes in progress("extract surface",volumeelements):
...
def bcdictionary(sections):
    ...
    # check if sets module is loaded
    try:
      Set()
    except NameError:
    ...
    if sections.has_key('DNODE-NODE TOPOLOGY'):
        ...
        if sections['DNODE-NODE TOPOLOGY']:
            for line in  progress('Extract line bc', sections['NODE-NODE TOPOLOGY']):
                nodeID = int(line[1])
                lineID = int(line[3])
    ...
    if sections.has_key('DLINE-NODE TOPOLOGY'):
        ...
        if sections['DLINE-NODE TOPOLOGY']:
            for line in  progress('Extract line bc', sections['DLINE-NODE TOPOLOGY']):
                nodeID = int(line[1])
                lineID = int(line[3])
                # Set needs as input a list. Solely an integer would give a TypeError
                try:
    ...
    if sections.has_key('DSURF-NODE TOPOLOGY'):
        ...
        if sections['DSURF-NODE TOPOLOGY']:
            for line in  progress('Extract surf bc', sections['DSURF-NODE TOPOLOGY']):
                nodeID = int(line[1])
                surfID = int(line[3])
                try:
    ...
    if sections.has_key('DVOL-NODE TOPOLOGY'):
        ...
        if sections['DVOL-NODE TOPOLOGY']:      
            for line in  progress('Extract vol bc', sections['DVOL-NODE TOPOLOGY']):
