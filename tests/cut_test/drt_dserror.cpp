...
{
  if (elementerrorcount != 0) dserror("inconsistent error system: %d", elementerrorcount);
  elementcall = true;
}
...
{
  elementcall = false;
  if (elementerrorcount != 0) dserror("%d element errors occured", elementerrorcount);
}
...
{
  if (not elementcall) dserror("element error outside of element loop");
  elementerrorcount += 1;
  fprintf(
      DRT::Problem::Instance()->ErrorFile()->Handle(), "element %d error: %s\n", ele, err.c_str());
